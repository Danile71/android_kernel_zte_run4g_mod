...
# Set environment for EAMKE 
if ($ENV{"EMAKEFLAGS"})
{
    $ENV{"EMAKE"} = 1;
    print "Start Build with EMAKE for build accelerator.You can disable emake with \"unset EMAKEFLAGS\" in your console\n";
}
# End
...
$ini = "makeMtk.ini";
$enINI = (-e $ini) ? 1 : 0;
(($#ARGV < 0) || (($#ARGV < 1) && (($enINI == 0) && (lc($ARGV[0]) !~ /^[listp|banyan_addon|banyan_addon_x86|banyan_opensdk]/)))) && &Usage;
...
if (($project ne "banyan_addon") && ($project ne "banyan_addon_x86") && ($project ne "banyan_opensdk")) {
...
if (join(' ', @acts) =~ /($actPttn)/ || $project eq "banyan_addon" || $project eq "banyan_addon_x86" || $project eq "banyan_opensdk")
{
...
if ($#acts == -1 && $project eq "banyan_opensdk")
{
  &chkMustEnv;
  print "$makeCmd banyan_opensdk" . "\n";
  $result += &p_system("$makeCmd banyan_opensdk");
  exit $result;
}
...
Actions:
  ...
  banyan_opensdk :Build MTK sdk plusopen.
