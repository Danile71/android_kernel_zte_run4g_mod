...
# Set PHONY
# *************************************************************************
.PHONY : new newall remake remakeall clean cleanall clean-md32\
         preloader trustzone md32 kernel android \
...
EMAKECMD      :=  emake $(MAKEJOBS) $(CMD_ARGU) $(MAKE_DEBUG)
...
TRUST_TEE_IMAGES := $(OUT_DIR)/target/product/$(PROJECT)/trustzone/bin/trustzone.bin
LK_IMAGES     := $(MTK_ROOT_OUT)/BOOTLOADER_OBJ/build-$(PROJECT)/lk.bin
LOGO_IMAGES   := $(MTK_ROOT_OUT)/BOOTLOADER_OBJ/build-$(PROJECT)/logo.bin
ifeq ($(strip $(FLAVOR)),)
  MD32_IMAGES   := $(MD32_WD)/build-md32_$(PROJECT)/md32_p.bin $(MD32_WD)/build-md32_$(PROJECT)/md32_d.bin
else
  MD32_IMAGES   := $(MD32_WD)/build-md32_$(PROJECT)_$(FLAVOR)/md32_p.bin $(MD32_WD)/build-md32_$(PROJECT)_$(FLAVOR)/md32_d.bin
endif
# for muti-kernel
...
EMAKECMD    +=  TARGET_PRODUCT=$(PROJECT) GEMINI=$(GEMINI) EVB=$(EVB) FLAVOR=$(FLAVOR)
...
TRUSTZONE_ALL_PATH :="$(TRUSTZONE_PROTECT_PRIVATE_SECURITY_PATH) $(TRUSTZONE_PROTECT_PRIVATE_SEC_DRM_PATH) $(TRUSTZONE_PROTECT_PRIVATE_MTEE_PATH) $(TRUSTZONE_PROTECT_PRIVATE_SEC_DRV_PATH) $(TRUSTZONE_PROTECT_PRIVATE_DRM_PATH) $(TRUSTZONE_PROTECT_BSP_PATH) $(TRUSTZONE_PROTECT_BSP_PLATFORM_PATH) $(TRUSTZONE_PLATFORM_PATH) $(TRUSTZONE_PATH)"
endif
...
ifeq ($(TRUSTZONE_PARTITION),yes)
	...
	$(hide) rm -f $(MODULE_LOG) $(MODULE_LOG)_err
    ifneq ($(ACTION), )
	$(hide) rm -rf $(OUT_DIR)/target/product/$(PROJECT)/trustzone
	$(hide) rm -rf $(OUT_DIR)/target/product/$(PROJECT)/trustzone.bin
    else
      ifneq ($(MTK_IN_HOUSE_TEE_SUPPORT),yes) # then, build trustonic tee
	$(hide) cd $(TRUST_TEE_WD) && \
	  (./build.sh $(PROJECT) $(ACTION) $(DEAL_STDOUT) && \
	  cd $(MKTOPDIR) && \
	  ...
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG))
      else # mtk in-house tee
	(source build/envsetup.sh;TARGET_PRODUCT=$(TARGET_PRODUCT) FLAVOR=$(FLAVOR) ONE_SHOT_MAKEFILE=$(TRUSTZONE_ALL_PATH) m $(MAKEJOBS) $(SNOD) $(DEAL_STDOUT);exit $${PIPESTATUS[0]})  && \
          $(SHOWRSLT) $$? $(LOG)$@.log || \
          $(SHOWRSLT) $$? $(LOG)$@.log
...
ifeq ($(BUILD_MD32),yes)
  ifneq ($(wildcard $(MD32_WD)),)
     ifeq ($(filter $(wildcard $(MD32_IMAGES)),$(MD32_IMAGES)),$(MD32_IMAGES))
	$(hide) echo Skip $@, binary already exists.
     else
	$(hide) echo $(SHOWTIME) $(SHOWBUILD)ing $@...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_MODULE_LOG)
	$(hide) rm -f $(MODULE_LOG) $(MODULE_LOG)_err
       ifneq ($(ACTION), )
	$(hide) cd $(MD32_WD) && \
	  (eval `/mtkoss/Modules/$(MODULE_VERSION)/$(MODULE_ARCH)/bin/modulecmd bash load $(MD32_TOOLCHAIN_MODULE)`; make PROJECT=md32_$(call lc,$(MTK_PLATFORM)) md32_$(call lc,$(MTK_PLATFORM)) clean $(DEAL_STDOUT) && \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION)) && cd $(MKTOPDIR)
       else
	$(hide) cd $(MD32_WD) && \
	  (eval `/mtkoss/Modules/$(MODULE_VERSION)/$(MODULE_ARCH)/bin/modulecmd bash load $(MD32_TOOLCHAIN_MODULE)`; make PROJECT=md32_$(call lc,$(MTK_PLATFORM)) $(MAKEJOBS_MD32) md32_$(call lc,$(MTK_PLATFORM)) $(ACTION) $(DEAL_STDOUT) && \
	  cd $(MKTOPDIR) && \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG))
       endif
...
clean-md32:
	$(hide) echo $(SHOWTIME) $(SHOWBUILD)ing $@...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bDELETE: $(MD32_IMAGES)
	$(hide) rm -f $(MD32_IMAGES)
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\b==\> [OK] $(SHOWTIME)
...
# enable EMAKE for building kernel
    ifneq ($(EMAKE),yes)
	$(hide) cd $(KERNEL_WD) && \
...
# enable EMAKE for build Android
  ifneq ($(EMAKE),yes)
	...
	$(hide) ($(EMAKECMD) $(ACTION) $(DEAL_STDOUT);exit $${PIPESTATUS[0]})
	$(hide) $(if $(filter clean,$(ACTION)),,$(call chkImgSize,$(ACTION),$(PROJECT),$(SCATTER_FILE),$(if $(strip $(ACTION)),$(CHECK_IMAGE),$(ANDROID_IMAGES)),$(DEAL_STDOUT),&&)) \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION)
  endif
  endif
