...
//#include <cust_acc_kxtik1013.h>  //aimi modify
#include <cust_acc.h>  //aimi add
...
#if 1//def MTK_AUTO_DETECT_ACCELEROMETER 
...
static int kxtik1013_init_flag = -1;//0;    //0<==>OK,-1<==>fail
...
/*----------------------------------------------------------------------------*/
#ifdef CUSTOM_KERNEL_SENSORHUB
int KXTIK1013_SCP_SetPowerMode(bool enable, int sensorType)
{
...
EXPORT_SYMBOL(KXTIK1013_SCP_SetPowerMode);
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
...
static int KXTIK1013_ResetCalibration(struct i2c_client *client)
{
    ...
    unsigned int len;
		...
		{
			pCustData = (KXTIK1013_CUST_DATA *)&data.set_cust_req.custData;
	
			data.set_cust_req.sensorType = ID_ACCELEROMETER;
			data.set_cust_req.action = SENSOR_HUB_SET_CUST;
			pCustData->resetCali.action = KXTIK1013_CUST_ACTION_RESET_CALI;
...
static int KXTIK1013_ReadCalibration(struct i2c_client *client, int dat[KXTIK1013_AXES_NUM])
{
	...
	#else
	    if (0!=(err = KXTIK1013_ReadOffset(client, obj->offset))) {
...
static int KXTIK1013_WriteCalibration(struct i2c_client *client, int dat[KXTIK1013_AXES_NUM])
{
	...
	GSE_LOG("OLDOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n", 
		...
		data.set_cust_req.sensorType = ID_ACCELEROMETER;
		data.set_cust_req.action = SENSOR_HUB_SET_CUST;
		pCustData->setCali.action = KXTIK1013_CUST_ACTION_SET_CALI;
		pCustData->setCali.data[KXTIK1013_AXIS_X] = dat[KXTIK1013_AXIS_X];
		pCustData->setCali.data[KXTIK1013_AXIS_Y] = dat[KXTIK1013_AXIS_Y];
		pCustData->setCali.data[KXTIK1013_AXIS_Z] = dat[KXTIK1013_AXIS_Z];
...
static int KXTIK1013_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	GSE_LOG("KXTIK1013_SetPowerMode %d!\n ",enable);
...
static int KXTIK1013_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	GSE_LOG("KXTIK1013_SetDataFormat OK! \n");
...
static int KXTIK1013_SetMotion(struct i2c_client *client, u8 dataformat)
{
	...
	KXTIK1013_SetPowerMode(client, true);
	GSE_LOG("KXTIK1013_SetMotion  OK! \n");
	return KXTIK1013_SetDataResolution(obj);    
}
...
static void kxtik1013_eint_work_func(struct work_struct *work)
{
    ...
    if (err = hwmsen_get_interrupt_data(ID_SIGNIFICANT_MOTION, &sensor_data)) {
        GSE_ERR("call hwmsen_get_interrupt_data fail = %d\n", err);
        return err;
    }

    //mt_eint_unmask(CUST_EINT_GSE_1_NUM);
	GSE_ERR("gsensor eint occored\n");
...
/* Gionee BSP1 chengx 20140828 modify for xxxx end */
	...
	GSE_LOG("kxtik1013_init_client OK!\n");
#ifdef CONFIG_KXTIK1013_LOWPASS
...
static int KXTIK1013_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if((res = KXTIK1013_ReadData(client, obj->data))!=0)
	{        
...
static int KXTIK1013_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if(0!=(res = KXTIK1013_ReadData(client, obj->data)))
	{        
		GSE_ERR("I2C error: ret value=%d", res);
		return EIO;
	}
		...
		case SENSOR_ENABLE:
			...
			else
			{
                ...
                if (((value ==0) && (priv->motion_detect_enable == false)) || ((value == 1) && (priv->motion_detect_enable == true))) {
	                ...
	                GSE_ERR("enable the motione sensor value = %d\n", value);
                    if (err = hwmsen_get_interrupt_data(ID_SIGNIFICANT_MOTION, &sensor_data)) {
...
{
    struct kxtik1013_i2c_data *obj = obj_i2c_data;
    KXTIK1013_CUST_DATA *p_cust_data;
    SCP_SENSOR_HUB_DATA data;
    ...
    p_cust_data = (KXTIK1013_CUST_DATA *)data.set_cust_req.custData;

    data.set_cust_req.sensorType = ID_ACCELEROMETER;
    data.set_cust_req.action = SENSOR_HUB_SET_CUST;
    sizeOfCustData = sizeof(*(obj->hw));
    p_cust_data->setCust.action = KXTIK1013_CUST_ACTION_SET_CUST;
    max_cust_data_size_per_packet = sizeof(data.set_cust_req.custData) - offsetof(KXTIK1013_SET_CUST, data);
    ...
    for (i=0;sizeOfCustData>0;i++)
    {
        ...
        len += offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, custData) + offsetof(KXTIK1013_SET_CUST, data);
        SCP_sensorHub_req_send(&data, &len, 1);
    }
    ...
    data.set_cust_req.sensorType = ID_ACCELEROMETER;
    data.set_cust_req.action = SENSOR_HUB_SET_CUST;
    p_cust_data->resetCali.action = KXTIK1013_CUST_ACTION_RESET_CALI;
...
static int kxtik1013_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		if (0!=(err = KXTIK1013_SetPowerMode(obj->client, false))) //old aimi
#else //#ifndef CUSTOM_KERNEL_SENSORHUB
		if (0!=(err = KXTIK1013_SCP_SetPowerMode(false, ID_ACCELEROMETER)))
		...
		sensor_power = false;   
#ifndef CUSTOM_KERNEL_SENSORHUB  /********aimi add ***********/	
		KXTIK1013_power(obj->hw, 0);
#endif //#ifndef CUSTOM_KERNEL_SENSORHUB  /********aimi add ***********/	
	}
	return err;
}
...
#ifndef CUSTOM_KERNEL_SENSORHUB 
	if(err = kxtik1013_init_client(client, 0)) //old aimi
#else //#ifndef CUSTOM_KERNEL_SENSORHUB
    if ((err = KXTIK1013_SCP_SetPowerMode(enable_status, ID_ACCELEROMETER)))
#endif //#ifndef CUSTOM_KERNEL_SENSORHUB		
	{
...
/* Gionee BSP1 chengx 20140828 modify for xxxx begin */
#if defined(KXTIK1013_MOTION_SENSOR_FUNCTION) //aimi
    if (obj->motion_detect_enable)
    {
        kxtik1013_set_motion_enable(obj->client, obj->motion_detect_enable);
        mt_eint_unmask(CUST_EINT_GSE_1_NUM);
	    GSE_LOG("enable the motion detect func\n");
    }
    else
    {
        kxtik1013_set_motion_enable(obj->client, obj->motion_detect_enable);
...
#else //#ifndef CUSTOM_KERNEL_SENSORHUB
		if ((err = KXTIK1013_SCP_SetPowerMode(false, ID_ACCELEROMETER)))
#endif //#ifndef CUSTOM_KERNEL_SENSORHUB
		{
            GSE_ERR("write power control fail!!\n");
            return;
...
#ifndef CUSTOM_KERNEL_SENSORHUB
	KXTIK1013_power(obj->hw, 1); //old aimi
#endif //#ifndef CUSTOM_KERNEL_SENSORHUB
...
//	GSE_FUN();
	...
	else
	{
		...
		{
#ifdef CUSTOM_KERNEL_SENSORHUB
            err = KXTIK1013_SCP_SetPowerMode(enable_status, ID_ACCELEROMETER);
            if (0 == err)
            {
               sensor_power = enable_status;
			}
#else
			err = KXTIK1013_SetPowerMode(obj_i2c_data->client, enable_status);
#endif
			GSE_LOG("Gsensor not in suspend KXTIK1013_SetPowerMode!, enable_status = %d\n",enable_status);
		}
		else
		{
...
static int gsensor_set_delay(u64 ns)
{
	...
	mutex_lock(&gsensor_scp_en_mutex);
	err = KXTIK1013_SetBWRate(obj_i2c_data->client, sample_delay);
	mutex_unlock(&gsensor_scp_en_mutex);
	if(err != KXTIK1013_SUCCESS ) //0x2C->BW=100Hz
	{
		GSE_ERR("Set delay parameter error!\n");
...
#else
    mutex_lock(&gsensor_scp_en_mutex);
	KXTIK1013_ReadSensorData(obj_i2c_data->client, buff, KXTIK1013_BUFSIZE);
	mutex_unlock(&gsensor_scp_en_mutex);
...
/* Gionee BSP1 chengx 20140826 modify for CR01371160 end */
	...
	obj->hw = get_cust_acc_hw();//get_cust_acc_hw_kxtik();
	if(obj->hw)
		GSE_LOG("Gsensor : get_cust_acc_hw() OK.\n");
	else
		GSE_ERR("Gsensor : get_cust_acc_hw() failed.\n");
	if((0 != hwmsen_get_convert(obj->hw->direction, &obj->cvt)))
	{
...
#if 1//def MTK_AUTO_DETECT_ACCELEROMETER
	if(0!=(err = kxtik1013_create_attr(&(kxtik1013_init_info.platform_diver_addr->driver)))) {
...
/* Gionee BSP1 chengx 20140828 modify for xxxx begin */
	...
	if(err = hwmsen_attach(ID_SIGNIFICANT_MOTION, &motionobj))
	{
		GSE_ERR("attach fail = %d\n", err);
		goto exit_attach_failed;
	}
...
exit_attach_failed:
#if 1//def 1//MTK_AUTO_DETECT_ACCELEROMETER
...
static int kxtik1013_local_init(void)
{
	struct acc_hw *hw = get_cust_acc_hw();//get_cust_acc_hw_kxtik();
...
static int __init kxtik1013_init(void)
{
	...
	i2c_register_board_info(hw->i2c_num, &i2c_kxtik1013, 1);   

	
#if 0	
    //Gionee yang_yang CR01324878 begin
    	i2c_register_board_info(1, &i2c_kxtik1013, 1);
    //Gionee yang_yang CR01324878 end
...
#endif
#endif
    acc_driver_add(&kxtik1013_init_info);
