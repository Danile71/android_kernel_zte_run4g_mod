...
#define CONFIG_ID_TEMPERATURE 0
...
static int bmp_get_pressure(struct i2c_client *client, char *buf, int bufsize)
{
	...
	/*modify by wxj 2014.11.14*/
	if ((obj->last_temp_measurement >= jiffies) || (obj->last_temp_measurement +
...
static int bmp180_set_delay(u64 ns)
{
	return 0;
}
...
static int __init bmp_init(void)
{
	...
	baro_driver_add(&bmp180_init_info);
