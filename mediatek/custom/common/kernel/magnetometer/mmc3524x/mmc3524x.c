...
#define MMC3524X_NEW_ARCH   //susport kk new msensor arch

#ifdef MMC3524X_NEW_ARCH
#include "mag.h"
...
#define MMCFUNC(func) printk("mmc3524x " func " is called\n")
#else
#define MMCFUNC(func)
#endif
...
static int mmc3524x_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		MSE_LOG("in suspend mode, can't operate device!\n");
		return -1;
	}
	#endif
...
static int mmc3524x_create_attr(struct device_driver *driver)
{
	...
	{
		if((err = driver_create_file(driver, mmc3524x_attr_list[idx])) != 0)
		{
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	if(atomic_read(&data->trace) & MMC_FUN_DEBUG)
	{
		MSE_FUN("mmc3524x_orientation_operate");
	}
#endif
...
//	atomic_set(&(data->suspend), 0);
...
#ifdef MMC3524X_NEW_ARCH
			if ((err = mmc3524x_create_attr(&(mmc3524x_init_info.platform_diver_addr->driver))) != 0) {
					printk(KERN_ERR "mmc3524x_create_attr create attribute err = %d\n", err);
					goto exit_sysfs_create_group_failed;
			}
#else

	/* Register sysfs attribute */
	if((err = mmc3524x_create_attr(&mmc3524x_platform_driver.driver)) != 0)
	{
	...
	if((err = misc_register(&mmc3524x_device)) != 0)
	{
...
#else
	err = mmc3524x_delete_attr(&mmc3524x_platform_driver.driver);
#endif
	if(err != 0)
	{
...
static int mmc3524x_local_init(void)
{
	...
	{
		printk(KERN_ERR "add mmc3524x_i2c_driver error\n");
		return -1;
	}
