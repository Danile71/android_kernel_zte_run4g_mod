...
//    DrvPlatformLyrTouchDeviceResetHw(); // Change to use SW reset to make sure FW stay at ROM code
...
//        nRegData2 = RegGet16BitValue(0x0F00); // Add for analysis
//        DBG("*** reg(0x0F, 0x00) = 0x%x ***\n", nRegData2);

//        nRegData3 = RegGet16BitValue(0x1E04); // Add for analysis
...
u8 DrvFwCtrlGetChipType(void)
{
    ...
    if (nChipType != CHIP_TYPE_MSG21XX &&   // (0x01) 
        ...
        nChipType != CHIP_TYPE_MSG22XX)     // (0x04)
    {
        nChipType = 0;
    }
