...
static void tpd_create_attributes(struct device *dev, struct tpd_attrs *attrs)
{
	...
	for (; num>0;num--)
		device_create_file(dev, attrs->attr[num-1]);
}
...
static int tpd_probe(struct platform_device *pdev) {
    ...
    if(0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 2) || 0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "270", 3))
    {
...
static int __init tpd_device_init(void) {
    ...
    printk("MediaTek touch panel driver init: Done\n");
    return 0;
}
