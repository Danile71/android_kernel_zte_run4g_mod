...
get_SWNR_ENC_enable_ISO_threshold(RAWIspCamInfo const& rCamInfo)
{
    ...
    else if ((rCamInfo.eIspProfile == EIspProfile_MFB_Capture_EE_Off_SWNR) ||
             (rCamInfo.eIspProfile == EIspProfile_MFB_PostProc_ANR_EE_SWNR) ||
             (rCamInfo.eIspProfile == EIspProfile_MFB_PostProc_Mixing_SWNR)) {
...
#define LERP(x, lo_x, lo_y, hi_x, hi_y)\
    ...
    //{6, 6, 6, 6, 6, 6, 6, 6, 6, 6};
    MY_LOG("[evaluate_Shading_Ratio] rto[10] = { %d, %d, %d, %d, %d, %d, %d, %d, %d, %d }", rto[0], rto[1], rto[2], rto[3], rto[4], rto[5], rto[6], rto[7], rto[8], rto[9]);
    ...
    if (total_RA_num_frames_ < 20){
        ...
        if (avgISO < iso[0]) {
            ...
            i4Rto = LERP(avgISO, iso[i-1], rto[i-1], iso[i], rto[i]);
        }
    }
    return i4Rto;
}
