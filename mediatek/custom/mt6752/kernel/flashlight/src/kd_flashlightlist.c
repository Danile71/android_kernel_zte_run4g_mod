...
MINT32 default_flashlight_ioctl(MUINT32 cmd, MUINT32 arg) {
    ...
    switch(cmd)
    {
        case FLASHLIGHTIOC_G_FLASHTYPE:
            ...
            if(copy_to_user((void __user *) arg , (void*)&iFlashType , 4))
            {
                logI("[default_flashlight_ioctl] ioctl copy to user failed ~");
                return -EFAULT;
            }
            break;
    	default :
...
static int setFlashDrv(int sensorDev, int strobeId)
{
	...
	if(sensorDevIndex<0 || strobeIndex<0 || partIndex<0)
		return -1;
...
static long flashlight_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    logI("flashlight_ioctl cmd=0x%x(nr=%d), senorDev=0x%x ledId=0x%x arg=0x%x",cmd, _IOC_NR(cmd), kdArg.sensorDev, kdArg.strobeId ,(unsigned int)kdArg.arg);
    ...
    switch(cmd)
    {
        ...
        case FLASH_IOC_IS_LOW_POWER:
            logI("FLASH_IOC_IS_LOW_POWER");
            {
                ...
                if(copy_to_user((void __user *) kdArg.arg , (void*)&isLow , 4))
    			{
		...
		case FLASH_IOC_GET_MAIN2_PART_ID:
			{
				...
				if(copy_to_user((void __user *) kdArg.arg , (void*)&partId , 4))
				{
		...
		case FLASH_IOC_UNINIT:
			{
				...
				if(pF!=0)
				{
					i4RetValue = pF->flashlight_release((void*)kdArg.arg);
					pF=0;
