...
ACHStatus AudioCmdHandler::DLCustSPHMagiParamToNV(void *pParam)
{
    int write_size = 0;
    int size = 0;

    SXLOGD("AudioCmdHandler::DLCustSPHMagiParamToNV() in");
    ...
    SXLOGD("DLCustSPHMagiParamToNV- GetMagiSpeechParamFromNVRam read size=%d", size);
    if (size != sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT))
    {
        SXLOGD("DLCustSPHMagiParamToNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT), size);
        free(pSPHMagiPara);
        pSPHMagiPara = NULL;
        return ACHFailed;
    }
	 ...
	 if (*((int *)pParam) != size)
	 {
		 SXLOGE("DLCustSPHMagiParamToNV miss data !!, received size = %d", *((int *)pParam));
		 free(pSPHMagiPara);
		 pSPHMagiPara = NULL;
		 return ACHFailed;
	 }
    ...
    for (int i = 0; i < 32 ;  i++)
    {
        SXLOGV("MAGI_CONFERENCE_STRUCT[%d] =%d", i, ((unsigned short *)pSPHMagiPara) + i);
    }

    write_size = SetMagiConSpeechParamToNVRam(pSPHMagiPara);
    if (write_size != sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT))
    {
        SXLOGD("DLCustSPHMagiParamToNV Down load to NVRAM fail, structure size=%d,writed_size=%d", sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT), write_size);
        free(pSPHMagiPara);
        pSPHMagiPara = NULL;
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::ULCustSPHMagiParamFromNV(void *pParam, int *len)
{
    int size = 0;
    int dataLen = 0;

    SXLOGD("AudioCmdHandler::ULCustSPHMagiParamFromNV() in");
    AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT *pSPHMagiPara = (AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT *)malloc(sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT));
    if (pSPHMagiPara == NULL || pParam == NULL)
    {
        return ACHFailed;
    }
    ...
    if (size != sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT))
    {
        SXLOGD("ULCustSPHMagiParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_MAGI_CONFERENCE_STRUCT), size);
        free(pSPHMagiPara);
        pSPHMagiPara = NULL;
        return ACHFailed;
    }
    ...
    for (int i = 0; i < 32 ;  i++)
    {
        SXLOGD("MAGI_CONFERENCE_STRUCT[%d] =%d", i, ((unsigned short *)pSPHMagiPara) + i);
    }
    ...
    SXLOGD("ULCustSPHMagiParamFromNV the data sent to PC is %d", dataLen);
    *((int *)pParam) = dataLen;
...
ACHStatus AudioCmdHandler::DLCustSPHHACParamToNV(void *pParam)
{
    int write_size = 0;
    int size = 0;

    SXLOGD("AudioCmdHandler::DLCustSPHHACParamToNV() in");
    ...
    SXLOGD("DLCustSPHHACParamToNV- GetHACSpeechParamFromNVRam read size=%d", size);
    if (size != sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT))
    {
        SXLOGD("DLCustSPHHACParamToNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT), size);
        free(pSPHHACPara);
        pSPHHACPara = NULL;
        return ACHFailed;
    }
	 ...
	 if (*((int *)pParam) != size)
	 {
		 SXLOGE("DLCustSPHHACParamToNV miss data !!, received size = %d", *((int *)pParam));
		 free(pSPHHACPara);
		 pSPHHACPara = NULL;
		 return ACHFailed;
	 }
    ...
    for (int i = 0; i < 32; i++)
    {
        SXLOGD("HAC_STRUCT[%d] =%d", i, ((unsigned short *)pSPHHACPara) + i);
    }
	for (int i = 0; i < 270;  i++)
    {
        SXLOGD("HAC_STRUCT[%d] =%d", i + 32, ((short *)pSPHHACPara) + i + 32);
    }

    write_size = SetHACSpeechParamToNVRam(pSPHHACPara);
    if (write_size != sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT))
    {
        SXLOGD("DLCustSPHHACParamToNV Down load to NVRAM fail, structure size=%d,writed_size=%d", sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT), write_size);
        free(pSPHHACPara);
        pSPHHACPara = NULL;
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::ULCustSPHHACParamFromNV(void *pParam, int *len)
{
    int size = 0;
    int dataLen = 0;

    SXLOGD("AudioCmdHandler::ULCustSPHHACParamFromNV() in");
    AUDIO_CUSTOM_HAC_PARAM_STRUCT *pSPHHACPara = (AUDIO_CUSTOM_HAC_PARAM_STRUCT *)malloc(sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT));
    if (pSPHHACPara == NULL || pParam == NULL)
    {
        return ACHFailed;
    }
    ...
    if (size != sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT))
    {
        SXLOGD("ULCustSPHHACParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_CUSTOM_HAC_PARAM_STRUCT), size);
        free(pSPHHACPara);
        pSPHHACPara = NULL;
        return ACHFailed;
    }
    ...
    SXLOGD("ULCustSPHHACParamFromNV the data sent to PC is %d", dataLen);
    *((int *)pParam) = dataLen;
    *len = 2 * (dataLen + sizeof(int));
...
ACHStatus AudioCmdHandler::DLCustACFParamToNV(void *pParam)
{
    ...
    SXLOGD("Received - ACF Param bes_loudness_Sep_LR_Filter = %d", pCustomPara->bes_loudness_Sep_LR_Filter);

    SXLOGD("Received - ACF Param bes_loudness_Num_Bands = %d", pCustomPara->bes_loudness_Num_Bands);
    SXLOGD("Received - ACF Param bes_loudness_Flt_Bank_Order = %d", pCustomPara->bes_loudness_Flt_Bank_Order);
    SXLOGD("Received - ACF Param bes_loudness_DRC_Delay = %d", pCustomPara->bes_loudness_DRC_Delay);
    SXLOGD("Received - ACF Param bes_loudness_Lim_Th = %d", pCustomPara->bes_loudness_Lim_Th);
    SXLOGD("Received - ACF Param bes_loudness_Lim_Gn = %d", pCustomPara->bes_loudness_Lim_Gn);
    SXLOGD("Received - ACF Param bes_loudness_Lim_Const = %d", pCustomPara->bes_loudness_Lim_Const);
    SXLOGD("Received - ACF Param bes_loudness_Lim_Delay = %d", pCustomPara->bes_loudness_Lim_Delay);
...
ACHStatus AudioCmdHandler::ULCustACFParamFromNV(void *pParam, int *len)
{
    ...
    if (size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        ...
        SXLOGD("Ori - ACF Param bes_loudness_Sep_LR_Filter = %d", pACFPara->bes_loudness_Sep_LR_Filter);
    
        SXLOGD("Ori - ACF Param bes_loudness_Num_Bands = %d", pACFPara->bes_loudness_Num_Bands);
        SXLOGD("Ori - ACF Param bes_loudness_Flt_Bank_Order = %d", pACFPara->bes_loudness_Flt_Bank_Order);
        SXLOGD("Ori - ACF Param bes_loudness_DRC_Delay = %d", pACFPara->bes_loudness_DRC_Delay);
        SXLOGD("Ori - ACF Param bes_loudness_Lim_Th = %d", pACFPara->bes_loudness_Lim_Th);
        SXLOGD("Ori - ACF Param bes_loudness_Lim_Gn = %d", pACFPara->bes_loudness_Lim_Gn);
        SXLOGD("Ori - ACF Param bes_loudness_Lim_Const = %d", pACFPara->bes_loudness_Lim_Const);
        SXLOGD("Ori - ACF Param bes_loudness_Lim_Delay = %d", pACFPara->bes_loudness_Lim_Delay);
...
#else
char* strFLTID[]={"ACF","HCF","ACF-SUB","Music-DRC","RingTone-DRC"};
...
char* strUpdateFLTID[]={"UpdateACFHCFParameters=0","UpdateACFHCFParameters=1","UpdateACFHCFParameters=2","UpdateACFHCFParameters=3","UpdateACFHCFParameters=4"};


ACHStatus AudioCmdHandler::DLCustAudioFLTParamToNV(void *pParam,AudioFltTunningType eFLTID)
{
    int write_size = 0;
    int size = 0;
    int dataLen = 0;
    ...
    {
        SXLOGD("AudioCmdHandler DLCustAudioFLTParamToNV() FilterID too big ..[%d]",dFLTID);
        return ACHFailed;
    }

    SXLOGD("AudioCmdHandler::DLCustAudioFLTParamToNV() in FilterID = %s",strFLTID[dFLTID]);
    ...
    SXLOGD("DLCustAudioFLTParamToNV ,acf param size=%d", sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT));
    size = GetAudioCompFltCustParamFromNV(AudioCompFltType_tMapID[(int)dFLTID], pACFPara);
    if (size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        SXLOGD("DLCustAudioFLTParamToNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT), size);
        free(pACFPara);
        pACFPara = NULL;
        return ACHFailed;
    }
    ...
    if (dataLen != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        SXLOGE("DLCustAudioFLTParamToNV data miss !!");
        free(pACFPara);
        pACFPara = NULL;
        return ACHFailed;
    }
    ...
    {
       SXLOGV("Don't Modify SWIP Rev. [0x%x] To [0x%x] (SWIPRev Setting Fail)",dRev,pCustomPara->SWIPRev);
       pCustomPara->SWIPRev = dRev;
    }

    SXLOGV("Received - bes_loudness_L_hpf_fc [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_hpf_fc);
    SXLOGV("Received - bes_loudness_L_hpf_order [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_hpf_order);
    SXLOGV("Received - bes_loudness_L_lpf_fc [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_lpf_fc);
    SXLOGV("Received - bes_loudness_L_lpf_order [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_lpf_order);
    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received  - loudness bpf coeffs bes_loudness_L_bpf_fc[%d] = %d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_fc[i]);
    }

    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received - loudness bpf coeffs bes_loudness_L_bpf_bw[%d]=%d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_bw[i]);
    }

    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received - loudness bps coeffs bes_loudness_L_bpf_gain[%d]=%d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_gain[i]);
    }

    SXLOGV("Received - bes_loudness_R_hpf_fc [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_hpf_fc);
    SXLOGV("Received - bes_loudness_R_hpf_order [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_hpf_order);
    SXLOGV("Received - bes_loudness_R_lpf_fc [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_lpf_fc);
    SXLOGV("Received - bes_loudness_R_lpf_order [%d]",pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_lpf_order);
    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received  - loudness bpf coeffs bes_loudness_R_bpf_fc[%d] = %d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_fc[i]);
    }

    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received - loudness bpf coeffs bes_loudness_R_bpf_bw[%d]=%d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_bw[i]);
    }

    for (int i = 0; i < 8; i++)
    {
        SXLOGV("Received - loudness bps coeffs bes_loudness_R_bpf_gain[%d]=%d", i, pCustomPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_gain[i]);
    }


    SXLOGD("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    SXLOGD("Received - AudioFLT Param bes_loudness_Sep_LR_Filter = %d", pCustomPara->bes_loudness_Sep_LR_Filter);
    SXLOGD("Received - AudioFLT Param bes_loudness_WS_Gain_Max = %d", pCustomPara->bes_loudness_WS_Gain_Max);
    SXLOGD("Received - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pCustomPara->bes_loudness_WS_Gain_Min);

    SXLOGD("Received - AudioFLT Param bes_loudness_Filter_First = %d", pCustomPara->bes_loudness_Filter_First);

    SXLOGD("Received - AudioFLT Param bes_loudness_Num_Bands = %d", pCustomPara->bes_loudness_Num_Bands);
    SXLOGD("Received - AudioFLT Param bes_loudness_Flt_Bank_Order = %d", pCustomPara->bes_loudness_Flt_Bank_Order);

    SXLOGD("Received - AudioFLT Param DRC_Delay = %d", pCustomPara->DRC_Delay);
    SXLOGD("Received - AudioFLT Param Lim_Th = %d", pCustomPara->Lim_Th);
    SXLOGD("Received - AudioFLT Param Lim_Gn = %d", pCustomPara->Lim_Gn);
    SXLOGD("Received - AudioFLT Param Lim_Const = %d", pCustomPara->Lim_Const);
    SXLOGD("Received - AudioFLT Param Lim_Delay = %d", pCustomPara->Lim_Delay);  
    ...
    SXLOGD("Received - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pCustomPara->bes_loudness_WS_Gain_Min);
    SXLOGD("Received - AudioFLT Param bes_loudness_Filter_First = %d", pCustomPara->bes_loudness_Filter_First);
    SXLOGD("Received - AudioFLT Param bes_loudness_Att_Time = %d", pCustomPara->bes_loudness_Att_Time);
    SXLOGD("Received - AudioFLT Param bes_loudness_Rel_Time = %d", pCustomPara->bes_loudness_Rel_Time);
    ...
    SXLOGD("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    SXLOGD("Received - AudioFLT Param bes_loudness_DRC_Forget_Table[0][0] = %d", pCustomPara->bes_loudness_DRC_Forget_Table[0][0]);
    SXLOGD("Received - AudioFLT Param bes_loudness_WS_Gain_Max = %d", pCustomPara->bes_loudness_WS_Gain_Max);
    SXLOGD("Received - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pCustomPara->bes_loudness_WS_Gain_Min);
    SXLOGD("Received - AudioFLT Param bes_loudness_Filter_First = %d", pCustomPara->bes_loudness_Filter_First);
    SXLOGD("Received - AudioFLT Param bes_loudness_Filter_First = %d", pCustomPara->bes_loudness_Filter_First);
    ...
    write_size = SetAudioCompFltCustParamToNV(AudioCompFltType_tMapID[dFLTID], pACFPara);
    if (write_size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        SXLOGD("DLCustAudioFLTParamToNV Down load to NVRAM fail, structure size=%d,writed_size=%d", sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT), write_size);
        free(pACFPara);
        pACFPara = NULL;
        return ACHFailed;
    }
...
ACHStatus AudioCmdHandler::ULCustAudioFLTParamFromNV(void *pParam, int *len,AudioFltTunningType eFLTID)
{
    int size = 0;
    int dFLTID = (int)eFLTID;
    ...
    if (eFLTID>=AUDIO_FLT_MAX)
    {
        SXLOGD("AudioCmdHandler ULCustAudioFLTParamFromNV() FilterID too big ..[%d]",dFLTID);
        return ACHFailed;
    }

    SXLOGD("AudioCmdHandler::ULCustAudioFLTParamFromNV() in FilterID = %s",strFLTID[dFLTID]);
    ...
    SXLOGD("ULCustAudioFLTParamFromNV ,acf param size=%d", sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT));
    size = GetAudioCompFltCustParamFromNV(AudioCompFltType_tMapID[dFLTID], pACFPara);
    if (size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        SXLOGD("ULCustAudioFLTParamFromNV Up load from NVRAM fail, structure size=%d,read_size=%d", sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT), size);
        free(pACFPara);
        pACFPara = NULL;
        return ACHFailed;
    }
        ...
        SXLOGV("Ori - SWIPRev [0x%x]",pACFPara->SWIPRev);
        SXLOGV("Ori - bes_loudness_L_hpf_fc [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_hpf_fc);
        SXLOGV("Ori - bes_loudness_L_hpf_order [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_hpf_order);
        SXLOGV("Ori - bes_loudness_L_lpf_fc [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_lpf_fc);
        SXLOGV("Ori - bes_loudness_L_lpf_order [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_lpf_order);
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori  - loudness bpf coeffs bes_loudness_L_bpf_fc[%d] = %d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_fc[i]);
        }
    
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori - loudness bpf coeffs bes_loudness_L_bpf_bw[%d]=%d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_bw[i]);
        }
    
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori - loudness bps coeffs bes_loudness_L_bpf_gain[%d]=%d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_L_bpf_gain[i]);
        }
        
        SXLOGV("Ori - bes_loudness_R_hpf_fc [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_hpf_fc);
        SXLOGV("Ori - bes_loudness_R_hpf_order [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_hpf_order);
        SXLOGV("Ori - bes_loudness_R_lpf_fc [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_lpf_fc);
        SXLOGV("Ori - bes_loudness_R_lpf_order [%d]",pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_lpf_order);
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori  - loudness bpf coeffs bes_loudness_R_bpf_fc[%d] = %d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_fc[i]);
        }
    
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori - loudness bpf coeffs bes_loudness_R_bpf_bw[%d]=%d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_bw[i]);
        }
    
        for (int i = 0; i < 8; i++)
        {
            SXLOGV("Ori - loudness bps coeffs bes_loudness_R_bpf_gain[%d]=%d", i, pACFPara->bes_loudness_f_param.V5F.bes_loudness_R_bpf_gain[i]);
        }

    
        SXLOGD("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        SXLOGD("Ori - AudioFLT Param bes_loudness_Sep_LR_Filter = %d", pACFPara->bes_loudness_Sep_LR_Filter);
        SXLOGD("Ori - AudioFLT Param bes_loudness_WS_Gain_Max = %d", pACFPara->bes_loudness_WS_Gain_Max);
        SXLOGD("Ori - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pACFPara->bes_loudness_WS_Gain_Min);
        
        SXLOGD("Ori - AudioFLT Param bes_loudness_Filter_First = %d", pACFPara->bes_loudness_Filter_First);
    
        SXLOGD("Ori - AudioFLT Param bes_loudness_Num_Bands = %d", pACFPara->bes_loudness_Num_Bands);
        SXLOGD("Ori - AudioFLT Param bes_loudness_Flt_Bank_Order = %d", pACFPara->bes_loudness_Flt_Bank_Order);

        SXLOGD("Ori - AudioFLT Param DRC_Delay = %d", pACFPara->DRC_Delay);
        SXLOGD("Ori - AudioFLT Param Lim_Th = %d", pACFPara->Lim_Th);
        SXLOGD("Ori - AudioFLT Param Lim_Gn = %d", pACFPara->Lim_Gn);
        SXLOGD("Ori - AudioFLT Param Lim_Const = %d", pACFPara->Lim_Const);
        SXLOGD("Ori - AudioFLT Param Lim_Delay = %d", pACFPara->Lim_Delay);
    ...
    SXLOGD("Ori - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pACFPara->bes_loudness_WS_Gain_Min);
    SXLOGD("Ori - AudioFLT Param bes_loudness_Filter_First = %d", pACFPara->bes_loudness_Filter_First);
    SXLOGD("Ori - AudioFLT Param bes_loudness_Att_Time = %d", pACFPara->bes_loudness_Att_Time);
    SXLOGD("Ori - AudioFLT Param bes_loudness_Rel_Time = %d", pACFPara->bes_loudness_Rel_Time);
    ...
    SXLOGD("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    SXLOGD("Ori - AudioFLT Param bes_loudness_DRC_Forget_Table[0][0] = %d", pACFPara->bes_loudness_DRC_Forget_Table[0][0]);
    SXLOGD("Ori - AudioFLT Param bes_loudness_WS_Gain_Max = %d", pACFPara->bes_loudness_WS_Gain_Max);
    SXLOGD("Ori - AudioFLT Param bes_loudness_WS_Gain_Min = %d", pACFPara->bes_loudness_WS_Gain_Min);
    SXLOGD("Ori - AudioFLT Param bes_loudness_Filter_First = %d", pACFPara->bes_loudness_Filter_First);
    SXLOGD("Ori - AudioFLT Param bes_loudness_Filter_First = %d", pACFPara->bes_loudness_Filter_First);
...
ACHStatus AudioCmdHandler::DLCustHCFParamToNV(void *pParam)
{
    ...
    SXLOGD("Received - HCF Param bes_loudness_Sep_LR_Filter = %d", pCustomPara->bes_loudness_Sep_LR_Filter);

    SXLOGD("Received - HCF Param bes_loudness_Num_Bands = %d", pCustomPara->bes_loudness_Num_Bands);
    SXLOGD("Received - HCF Param bes_loudness_Flt_Bank_Order = %d", pCustomPara->bes_loudness_Flt_Bank_Order);
    SXLOGD("Received - HCF Param bes_loudness_DRC_Delay = %d", pCustomPara->bes_loudness_DRC_Delay);
    SXLOGD("Received - HCF Param bes_loudness_Lim_Th = %d", pCustomPara->bes_loudness_Lim_Th);
    SXLOGD("Received - HCF Param bes_loudness_Lim_Gn = %d", pCustomPara->bes_loudness_Lim_Gn);
    SXLOGD("Received - HCF Param bes_loudness_Lim_Const = %d", pCustomPara->bes_loudness_Lim_Const);
    SXLOGD("Received - HCF Param bes_loudness_Lim_Delay = %d", pCustomPara->bes_loudness_Lim_Delay);
...
ACHStatus AudioCmdHandler::ULCustHCFParamFromNV(void *pParam, int *len)
{
    ...
    if (size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        ...
        return ACHFailed;
            ...
            SXLOGD("Ori - HCF Param bes_loudness_Sep_LR_Filter = %d", pHCFPara->bes_loudness_Sep_LR_Filter);
        
            SXLOGD("Ori - HCF Param bes_loudness_Num_Bands = %d", pHCFPara->bes_loudness_Num_Bands);
            SXLOGD("Ori - HCF Param bes_loudness_Flt_Bank_Order = %d", pHCFPara->bes_loudness_Flt_Bank_Order);
            SXLOGD("Ori - HCF Param bes_loudness_DRC_Delay = %d", pHCFPara->bes_loudness_DRC_Delay);
            SXLOGD("Ori - HCF Param bes_loudness_Lim_Th = %d", pHCFPara->bes_loudness_Lim_Th);
            SXLOGD("Ori - HCF Param bes_loudness_Lim_Gn = %d", pHCFPara->bes_loudness_Lim_Gn);
            SXLOGD("Ori - HCF Param bes_loudness_Lim_Const = %d", pHCFPara->bes_loudness_Lim_Const);
            SXLOGD("Ori - HCF Param bes_loudness_Lim_Delay = %d", pHCFPara->bes_loudness_Lim_Delay);
