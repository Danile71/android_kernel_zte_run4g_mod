...
#define RILD_NAME_FOR_EMD "ril-daemon-md2"
...
void* emd_uart_reader(void* arg)
{
    ...
    while(1)
    {
        ...
        retry=0;
        do{
            ret = write(emd_uart_cfifo, (void *)read_buf+ret, size-wrote_size);
            if(ret>0)
            {
                wrote_size+=ret;
                retry=0;
            ...
            else
            {
                ...
                {
                    LOGD("Write data into cfifo retry %d, fail\n",RETRY_TIMES);
                    retry=1;
                }
            }
...
void* emd_uart_writer(void* arg)
{
    ...
    while(1)
    {
        ...
        retry=0;
        do{
            ret = write(emd_uart_fd, (void *)write_buf+ret, size-wrote_size);
            if(ret>0)
            {
                wrote_size+=ret;
                retry=0;
...
#ifdef MTK_EXTMD_NATIVE_DOWNLOAD_SUPPORT
    LOGD("[NATIVE_DOWNLOAD_SUPPORT Defined!\n");
