 ...
 *      
 */
void fill_point_buffer(unsigned int *fill_addr, unsigned int src_color, LCM_SCREEN_T phical_screen)
{
    ...
    } else if (16 == phical_screen.bits_per_pixel) {
        if(11 == phical_screen.blue_offset) {
            *fill_addr = ARGB8888_TO_BGR565(src_color);
        } else {
            *fill_addr = ARGB8888_TO_RGB565(src_color);
        }
    } else {
 ...
 *
 */
void fill_rect_with_content_by_32bit(unsigned int *fill_addr, RECT_REGION_T rect, unsigned int *src_addr, LCM_SCREEN_T phical_screen)
{
    int draw_height = rect.bottom - rect.top;
    ...
    {   
        for(j = rect.left; j < rect.right; j++) 
        {
            ...
            fill_point_buffer(dst_addr, *color_addr, phical_screen);
        }
    }
}
 ...
 *
 */
void fill_rect_with_content_by_16bit(unsigned int *fill_addr, RECT_REGION_T rect, unsigned int *src_addr, LCM_SCREEN_T phical_screen)
{
    int draw_height = rect.bottom - rect.top;
...
void fill_rect_with_color_by_32bit(unsigned int *fill_addr, RECT_REGION_T rect, unsigned int src_color, LCM_SCREEN_T phical_screen)
{
    ...
    {   
        for(j = rect.left; j < rect.right; j++) 
        {
            ...
            fill_point_buffer(dst_addr, src_color, phical_screen);
        }
    }
}
...
void fill_rect_with_content(void * fill_addr, RECT_REGION_T rect, unsigned int *src_addr, LCM_SCREEN_T phical_screen)

{
    ...
    if (phical_screen.fill_dst_bits == 16) {
        fill_rect_with_content_by_16bit((unsigned int *)fill_addr,  rect, src_addr, phical_screen);
