...
static bool isNonEscapingLocalObject(const Value *V) {
  ...
  if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(V)){
    if (!GV->hasInternalLinkage() || GV->isConstant() || GV->isThreadLocal())
      return false;

    bool HasMultipleAccessingFunctions = false;
    const Function *AccessingFunction = 0;

    for (Value::const_use_iterator i = V->use_begin(), e = V->use_end(); i != e; ++i) {
      if (const Instruction *I = dyn_cast<Instruction>(*i)) {
        if (!HasMultipleAccessingFunctions) {
          const Function *F = I->getParent()->getParent();
          if (!AccessingFunction)
            AccessingFunction = F;
          else if (AccessingFunction != F)
            HasMultipleAccessingFunctions = true;
        }
      }
    }

    if (!HasMultipleAccessingFunctions && AccessingFunction){
      return !PointerMayBeCaptured(V, true, /*StoreCaptures=*/true);
    }
  }
