...
bool GVN::processLoad(LoadInst *L) {
  ...
  if (DepI){
    if (StoreInst *SI = dyn_cast<StoreInst>(DepI)){
      MemDepResult preDep = MD->getDependency(DepI);
      if (preDep.getInst()){
        if (LoadInst *preDepLI = dyn_cast<LoadInst>(preDep.getInst())){
          if (SI->getValueOperand() == preDepLI &&
            L->getPointerOperand() == preDepLI->getPointerOperand()){
            Dep = preDep;
          }
        }
      }
