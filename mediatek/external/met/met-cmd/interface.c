...
int met_process_arg(int option, char *arg)
{
	...
	if ((i>=0)&&(i<mdevice_num)) {
		if (arg != NULL) {
			...
			if (ret != 0)
				return ret;
			/* to check the return code of argu parse*/
			if (0 == sys_read(path, value, 16, 0)) {
				if (0 != atoi(value)) 
					return -1;
			}
			mdevice[i].mode = 2;
...
/* get_num_range can parse the following format of numbers       */
/* 1. number group divided by ' ', ',' or '\t'                   */
/* 2. each number group could be a single number or a range that */
/*    linked by '-' (n or n1-n2)                                 */
/* usage:                                                        */
/*    Call this function with a valid string(*pStr) at first     */
/*    time. And then continuous call this function with a NULL   */
/*    string (*pStr = NULL) until the returned value is 0.       */
/* return value:                                                 */
/*    0: no more value returned                                  */
/*    1: single value returned, *pV1 is valied only              */
/*    2: a range is returned, both *pV1 & *pV2 are valied        */
int get_num_range(char *pStr, int *pV1, int *pV2)
{
	static char *pTok, *pSavePtr;

	/* get a group of number */
	pTok = strtok_r(pStr, " ,\t\n", &pSavePtr);
	if (pTok == NULL)
		return 0;	/* no more values */

	/* get the first number */
	if (get_num_forward(pTok, pV1, strlen(pTok)) == 0)
		return 0;	/* something wrong, return 0 */

	/* get the second number */
	pTok = strchr(pTok, '-');
	if (pTok == NULL)
		return 1;	/* get only one value this round */
	if (get_num_forward(pTok+1, pV2, strlen(pTok+1)) == 0)
		return 1;	/* something wrong, return 1 */
	else
		return 2;	/* we get a range */
}
...
void logbuf_info_print(logbuf_info_t info[], int max_cpu, FILE *fp_out)
{
	...
	*/
	...
	fprintf(fp_out, "met-info [000] 0.0: last_time: %s", ctime(&tv.tv_sec));
}
