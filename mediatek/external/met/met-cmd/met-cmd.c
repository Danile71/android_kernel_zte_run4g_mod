...
static int start(char *trace_events)
{
	...
	memset(buf, 0, 16);
	sys_read("/sys/kernel/debug/tracing/current_tracer", buf, 16, NOMSG_OPERATION);
...
int get_hardware_info(FILE *fp_out)
{
	...
	while ((p=fgets(buf, 4096, fp)) != NULL) {
		if (strncasecmp(p, "system type", 10) == 0) {
			/* to detect WCN Product*/
			strtok(p, ":");
			tmp = strtok(NULL, ": \t\n");
...
int output_ftrace_event(FILE *fp_out)
{
	...
	//Get Max CPU
	get_num_backward(buf + strlen(buf) - 1, &g_max_cpu, 5);

	if (g_max_cpu >= 8) {
		printf("!!! ERROR: g_max_cpu=%d is unreasonable!!\n", g_max_cpu);
		return -1;
	}
	...
	logbuf_info_print(lbf_info, g_max_cpu, fp_out);
...
/**************************************************************************/
/* WARNNING:                                                              */
/*   This founction can only be called after setup the value of g_max_cpu */
/**************************************************************************/
static int output_cpu_frequency(FILE *fp_out)
{
#define CPUINFO_MAX_FREQ(cpu)	"/sys/devices/system/cpu/cpu%d/" \
				"cpufreq/cpuinfo_max_freq", (cpu)
#define RELATED_CPUS(cpu)	"/sys/devices/system/cpu/cpu%d/" \
				"cpufreq/related_cpus", (cpu)
	int cpu, rel_cpu;
	int max_freq[MAX_CPU_COUNT], cur_max_freq = 0;
	char path[256];
	char buf[256];
	int rc, v1, v2;

	/* init max freq */
	for (cpu = 0; cpu <= g_max_cpu; cpu++)
		max_freq[cpu] = 0;

	for (cpu = 0; cpu <= g_max_cpu; cpu++) {
		/* test if we already get the max freq for this cpu */
		if (max_freq[cpu] != 0) {
			continue;
		}

		/* try to get the max freq for this cpu */
		/* if failed, then just move to the next */
		snprintf(path, sizeof(path), CPUINFO_MAX_FREQ(cpu));
		if (sys_read(path, buf, sizeof(buf), NOMSG_IF_NOEXIST) != 0)
			continue;
		if (get_num_forward(buf, &cur_max_freq, sizeof(buf)) == 0)
			continue;
		max_freq[cpu] = cur_max_freq;

		/* ok, we get the max_freq */
		/* check the related_cpus and set the same max freq for them */
		snprintf(path, sizeof(path), RELATED_CPUS(cpu));
		if (sys_read(path, buf, sizeof(buf), NOMSG_IF_NOEXIST) != 0)
			continue;
		rc = get_num_range(buf, &v1, &v2);
		while (rc) {
			/* rc must be 1 or 2 now */
			switch (rc) {
			case 1:
				v2 = v1;
				break;
			...
			if (v1 < 0 || v1 > g_max_cpu
			    || v2 < 0 || v2 > g_max_cpu)
				break;		/* skip unreasonable values */
			for (rel_cpu = v1; rel_cpu <= v2; rel_cpu++) {
				max_freq[rel_cpu] = cur_max_freq;
			}
			rc = get_num_range(NULL, &v1, &v2);
		}
	}

	/* print out the result */
	fprintf(fp_out, "met-info [000] 0.0: met_cpu_max_freq:");
	for (cpu = 0; cpu <= g_max_cpu; cpu++) {
		if (max_freq[cpu]) {
			fprintf(fp_out, "%s %d, %d",
				cpu ? "," : "", cpu, max_freq[cpu]);
		} else {
			/* for those CPUs that can't get the max freq, */
			/* make a 'fake' value for them, XD.....       */
			fprintf(fp_out, "%s %d, %d",
				cpu ? "," : "", cpu, cur_max_freq);
		}
	}
	fprintf(fp_out, "\n");
