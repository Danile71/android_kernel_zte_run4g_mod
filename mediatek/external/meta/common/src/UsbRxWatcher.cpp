...
UsbRxWatcher::UsbRxWatcher(void)
{
	...
	FtModModemInfo * mdInfo =new FtModModemInfo();
	mdInfo->getModemCapability(&ft_modem_cap_list);
}
							 ...
							 unsigned int len)
{
	...
	if(nRemainLen > 0)
	{
		memcpy(szRemainBuf+nRemainLen, buf, len);
		data = szRemainBuf;
		len = nRemainLen + len;
	}
	else
	...
	if (NULL != pFrm)
	{
		if(pFrm->getIsValid() == 1)
		{
			...
			if(len > u16Length)
			{
				memcpy(szRemainBuf, data+u16Length, len-u16Length);
				nRemainLen = 0;
...
Frame *UsbRxWatcher::decodeMDFrame(void *pdata, unsigned int len,unsigned char frmType,unsigned short &u16Length)
{
	...
	while(u16Length != len)
	{
		...
		else if(u16Length == newLen + 3 + m_frm_len_byte)   // 3 + m_frm_len_byte = start byte + data len + data type + checksum
		{
			
			META_LOG("[Meta] New modem frame tail");
			bFrm = true;
		}
		...
		if(bFrm)
		{
			...
			return sendMdTask(pdata,len, frmType);
		}
...
Frame * UsbRxWatcher::dispatchFrame(unsigned char ch, unsigned char *buf_ptr, unsigned int input_len, unsigned char *src,unsigned short &u16Length)
{
			...
			ch <= RS232_COMMAND_TYPE_MD_DATA_TUNNEL_END)//0xD0 ~ 0xD7
	{
		...
		return decodeLTEFrame(input_len,++src,ch,u16Length);
	}
	else
	{
...
Frame *UsbRxWatcher::decodeLTEFrame(unsigned int input_len,unsigned char * src,unsigned char frmType,unsigned short &u16Length)
{
	unsigned char * local_ptr = src;
	unsigned char	ch;
	int data_len = 0;
	...
	META_LOG("[Meta] (decodeLTEFrame) input_len = %d, m_uFrameLength = %d",input_len,m_uFrameLength);
	...
	while(u16Length!=input_len)
	{
		...
		{
			if(ft_modem_cap_list.modem_cap[m_md_index].ch_type == FT_MODEM_CH_TUNNELING)
			{
				m_cTstFrameState = RS232_FRAME_STX;
				...
				m_checksum = STX_OCTET;
				META_LOG("[Meta] (decodeLTEFrame) u16Length2 = %d",u16Length);
