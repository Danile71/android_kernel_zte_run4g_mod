...
linux_gps_init (void)
{
    ...
    MND_MSG("Start get COMBO_IOC_GPS_IC_HW_VERSION \n");
	...
	MND_MSG("zqh:userprofile.BEE_enabled (%d) \n",userprofile.BEE_enabled );	    
...
void linux_signal_handler(int signo)
{
    ...
    if (signo == SIGTERM)
    {
        MND_MSG("Meta or factory test done");
        unlink(GPS_PROPERTY);
        stop_mnl_process();
    }
    exit_meta_factory = 1;
	...
	for (ne = 0; ne < nevents; ne++) {
		if ((events[ne].events & (EPOLLERR|EPOLLHUP)) != 0) {
			MND_ERR("wait: (%d %d %d), event: 0x%X from %d", obj->cur_accept_socket, 
				obj->sig_rcv_fd, obj->mnl_rcv_agps_fd, events[ne].events, events[ne].data.fd);            
