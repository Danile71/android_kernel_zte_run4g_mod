		...
		if (SOURCE_TYPE_BUF == mMPImageInfos[i].sourceType) {
			if (NULL != mMPImageInfos[i].imageBuf && mMPImageInfos[i].imageSize > 0) {
				size += mMPImageInfos[i].imageSize;
                                ///M: ALPS01438247 for the mt6577 only pass the jpeg raw data except the first frame
				size += 4096;
...
bool MpoEncoder::encode_internal(const char * outputMpo, const unsigned char * data, const int mtkMpoType)
{
    ...
    MpoOutputStream *finalMpoMemoryOutputStream = new MpoMemoryOutputStream(data, length, false); 
    finalOutputStream = finalMpoMemoryOutputStream; 
    ...
    if (NULL != MPImages[0]) {
        if (false == MPImages[0]->writeStream(finalOutputStream)) { 
    ...
    if (!encodeFailed) {
        if (false == MPImages[0]->reWriteStream(finalOutputStream,  
