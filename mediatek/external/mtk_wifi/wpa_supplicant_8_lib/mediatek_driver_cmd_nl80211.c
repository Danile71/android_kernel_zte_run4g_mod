...
static int testmode_available_channel_handler(struct nl_msg *msg, void *arg)
{
    ...
    unsigned char i = 0;
	  wpa_printf(MSG_DEBUG, "[P2P_ACS] Available Channel Callback");
    nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
    	  genlmsg_attrlen(gnlh, 0), NULL);
        ...
        if(sinfo[i]) { 		
            switch(i) {
            	case NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1:
                    ...
                    wpa_printf(MSG_DEBUG, "[P2P_ACS] ch_2g_base1=0x%08x", available_chan->ch_2g_base1);
                    break;
            	case NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34:
                    available_chan->ch_5g_base34 = nla_get_u32(sinfo[i]);
                    wpa_printf(MSG_DEBUG, "[P2P_ACS] ch_5g_base34=0x%08x", available_chan->ch_5g_base34 );
                    break;
            	case NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149:
                    available_chan->ch_5g_base149 = nla_get_u32(sinfo[i]);
                    wpa_printf(MSG_DEBUG, "[P2P_ACS] ch_5g_base149=0x%08x", available_chan->ch_5g_base149);
                    break;                    
            	case NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184:
                    available_chan->ch_5g_base184 = nla_get_u32(sinfo[i]);
                    wpa_printf(MSG_DEBUG, "[P2P_ACS] ch_5g_base184=0x%08x", available_chan->ch_5g_base184);
                default:
                    break;
            }
        }
...
static void wpa_driver_nl80211_scan_loading_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "[P2P_ACS] Scan loading timeout - update channel list");
	                        ...
	                        size_t data_len)
{
    ...
    wpa_printf(MSG_DEBUG, "~nl80211~: Test Mode buflen = %d, ifindex=%d\n", data_len,drv->ifindex);
...
int wpas_get_available_channel(struct wpa_supplicant *wpa_s, u8 *buf)
{
	...
	os_memset(&params, 0, sizeof(params));
	wpa_printf(MSG_DEBUG, "[P2P_ACS] Get available channel");
				  ...
				  size_t buf_len )
{
    ...
    wpa_printf(MSG_DEBUG, "%s: iface %s  recv cmd %s",__FUNCTION__, bss->ifname,cmd);
	...
	if (os_strncasecmp(cmd, "POWERMODE ", 10) == 0) {
        ...
        ret = linux_set_iface_flags(drv->global->ioctl_sock, 
		...
		ret = linux_set_iface_flags(drv->global->ioctl_sock, drv->first_bss.ifname, 0);
...
#endif /* CONFIG_WAPI_SUPPORT */
	} else if (os_strncmp(cmd, "POORLINK_RSSI ", 14) == 0) {
		struct wpa_driver_poorlink_params data;
		data.hdr.index = 0x4;
		data.hdr.index = data.hdr.index | (0x01 << 24);
		data.hdr.buflen = sizeof(struct wpa_driver_poorlink_params);
		data.link_speed = 0;
		data.rssi = (s8)atoi(cmd+14);
		wpa_driver_nl80211_testmode(priv, (u8*)&data, sizeof(struct wpa_driver_poorlink_params));
	} else if (os_strncmp(cmd, "POORLINK_LINKSPEED ", 19) == 0) {
		struct wpa_driver_poorlink_params data;
		data.hdr.index = 0x4;
		data.hdr.index = data.hdr.index | (0x01 << 24);
		data.hdr.buflen = sizeof(struct wpa_driver_poorlink_params);
		data.link_speed = (u8)atoi(cmd+19);
		data.rssi = 0;
		wpa_driver_nl80211_testmode(priv, (u8*)&data, sizeof(struct wpa_driver_poorlink_params));
#ifdef CONFIG_MTK_WFD_SINK
...
#define P2P_CACHE_FILE "/data/misc/wifi/p2p_devices.cache"
		...
		*value++ = '\0';
		...
		unsigned long sw_cmd = 0xFFFF1253;
		unsigned long idx = 0;

		wpa_printf(MSG_DEBUG, "[P2P_ACS] Invoke scan loading");
		wpa_driver_nl80211_driver_sw_cmd(priv, 1, &sw_cmd, &idx);
		
		wpa_printf(MSG_DEBUG, "[P2P_ACS] Timeout= 1 second");
		eloop_cancel_timeout(wpa_driver_nl80211_scan_loading_timeout, drv, drv->ctx);
		eloop_register_timeout(1, 0, wpa_driver_nl80211_scan_loading_timeout,
				   drv, drv->ctx);
		ret = 0;
    }
#endif
    ...
    if(handled == 0) {
	    ...
	    if ((os_strncasecmp(cmd, "WLS_BATCHING", 12) == 0))
			...
			wpa_printf(MSG_DEBUG, "%s: buf = %s ret = %d used = %u total = %u", 
