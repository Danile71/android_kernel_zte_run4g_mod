...
#define MTK_NFC_HCE_SUPPORT   /// HCE
#define MTK_NFC_REDUCE_SHDLC_T1_TIMEOUT_SUPPORT  // for low polling issue
...
typedef enum {
    ...
    MTK_NFC_JNI_SELECT_RF_INTERFACE_REQ,
    MTK_NFC_JNI_SELECT_RF_INTERFACE_RSP,    
    #endif

    #ifdef MTK_NFC_HCE_SUPPORT 
    /// -- HCE [START]
    MTK_NFC_JNI_SEND_RAW_FRAME_REQ,
    MTK_NFC_JNI_SEND_RAW_FRAME_RSP,
    MTK_NFC_JNI_SET_DEFAULT_ROUTE_REQ,
    MTK_NFC_JNI_SET_DEFAULT_ROUTE_RSP,
    MTK_NFC_JNI_ADD_AID_ROUTING_REQ,
    MTK_NFC_JNI_ADD_AID_ROUTING_RSP,
    MTK_NFC_JNI_REMOVE_AID_ROUTING_REQ,
    MTK_NFC_JNI_REMOVE_AID_ROUTING_RSP,
    MTK_NFC_JNI_UPDATE_ROUTING_REQ,
    MTK_NFC_JNI_UPDATE_ROUTING_RSP,
    MTK_NFC_JNI_NOTIFY_HCE_DATA_RSP,
    /// -- HCE [END]
    #endif
...
#define MTK_NFC_SE_SEND_DATA_MAX_LEN  (4096)
...
//set default route req
typedef struct mtk_nfc_ee_set_default_route_req
{
    UINT8 default_route;
} mtk_nfc_ee_set_default_route_req_t;

//set default route rsp
typedef struct mtk_nfc_ee_set_default_route_rsp
{
    INT32  result;  //0:success, 1:fail
} mtk_nfc_ee_set_default_route_rsp_t;

//add aid req
typedef struct mtk_nfc_ee_add_aid_req
{
    UINT8 nfcee_id;
    UINT8 aid_len;
    UINT8 aid[MTK_NFC_MAX_AID_LEN];
} mtk_nfc_ee_add_aid_req_t;

//add aid rsp
typedef struct mtk_nfc_ee_add_aid_rsp
{
    INT32  result;  //0:success, 1:fail
} mtk_nfc_ee_add_aid_rsp_t;

//remove aid req
typedef struct mtk_nfc_ee_remove_aid_req
{
    UINT8 aid_len;
    UINT8 aid[MTK_NFC_MAX_AID_LEN];
} mtk_nfc_ee_remove_aid_req_t;

//remove aid rsp
typedef struct mtk_nfc_ee_remove_aid_rsp
{
    INT32  result;  //0:success, 1:fail
} mtk_nfc_ee_remove_aid_rsp_t;

//update routing table
typedef struct mtk_nfc_ee_update_routing_req
{
    UINT8 state; // 0: disable, 1: enable
    
}mtk_nfc_ee_update_routing_req_t;

typedef struct mtk_nfc_ee_update_routing_rsp
{
    INT32  result;  //0:success, 1:fail
    
}mtk_nfc_ee_update_routing_rsp_t;

typedef struct mtk_nfc_ee_send_raw_frame_req
{
    UINT32 u4DataLen;
    UINT8 au1Data[MTK_NFC_SE_SEND_DATA_MAX_LEN];
}mtk_nfc_ee_send_raw_frame_req_t;

typedef struct mtk_nfc_ee_send_raw_frame_rsp
{
    INT32  result;  //0:success, 1:fail
}mtk_nfc_ee_send_raw_frame_rsp_t;

typedef struct mtk_nfc_ee_notify_host_emu_data
{
    INT32 u4DataLen;
    UINT8 au1Data[MTK_NFC_SE_SEND_DATA_MAX_LEN];
}mtk_nfc_ee_notify_host_emu_data_t;

/// -- HCE [END]
#endif

typedef struct mtk_nfc_select_rf_interface_req
{
    UINT8 u1RfDiscoverID;
    UINT8 u1RfInterface;    
    UINT8 u1RfProtocol;
}mtk_nfc_select_rf_interface_req_t;

typedef struct mtk_nfc_select_rf_interface_rsp{
    UINT32 u4Result; // 0 success, 1 fail, 2 invalid param
    UINT32 u4Type;
    UINT32 u4State;
}mtk_nfc_select_rf_interface_rsp_t;
