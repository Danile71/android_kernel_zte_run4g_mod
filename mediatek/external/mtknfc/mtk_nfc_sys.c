...
)
{
    ...
    else
    {
        ...
        NFC_PRINT("[TIMER]stop timer_slot(%d)\r\n", timer_slot); 
        mtk_nfc_sys_timer_stop(timer_slot);
    }   
...
NFC_FILE mtk_nfc_sys_file_open(const CHAR *szFileName, UINT32 i4Mode)
{
    ...
    NFC_PRINT("mtk_nfc_sys_file_open, fopen start. file,%s, mode,%s. \r\n", szFileName, szMode);
    fp = fopen(szFileName, szMode);
    NFC_PRINT("mtk_nfc_sys_file_open, fopen stop. \r\n");
