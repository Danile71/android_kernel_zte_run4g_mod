	...
	const XMLCh *localName) const
{
    ...
    for (XMLSize_t i = 0; i < len; ++i) {
		...
		// changed by mtk94010 for attribute get (xxx:yyy and yyy)
		// it will find yyy in xxx:yyy string, not need equal 
#if 0
...
#endif
		{
			...
			else
			{
			    if(nLocalName == 0)
			    {
			        // end with it  xxx:yyy, localName maybe yyy
			        if(XMLString::endsWith(node->getNodeName(),localName))
			        {
						// change it to char * and compare:
						char* c_nodeName =  xercesc::XMLString::transcode(node->getNodeName());
						char* c_localName = xercesc::XMLString::transcode(localName);
						char* suffix = NULL;
						
						if((suffix = strstr(c_nodeName, ":")) != NULL)
						{
						    // find it
							if(strcmp(suffix + 1, c_localName) == 0)
							{
							    xercesc::XMLString::release(&c_nodeName);
								xercesc::XMLString::release(&c_localName);
								return (int)i;
							}
						}
