                                    ...
                                    , MemoryManager* const  manager)
{
    ...
    char *tempDst = (char *)toFill;
	const wchar_t **tempSrc = (const wchar_t **)&wideCharBuf;
	
    size_t mblen = mywcsrtombs(tempDst, tempSrc, maxBytes);
    if (mblen == (size_t)-1)
    {
        if (allocatedArray)
          manager->deallocate(allocatedArray);
                                    ...
                                    , MemoryManager* const  manager)
{
	...
	wchar_t* tempDst = (wchar_t *)wideCharBuf;
	const char **tempSrc = (const char **)&toTranscode;
	
    if (mymbsrtowcs(tempDst, tempSrc, maxChars) == (size_t)-1)
    {
        if (allocatedArray)
          manager->deallocate(allocatedArray);
        return false;
...
size_t IconvLCPTranscoder::mywcslen(const wchar_t *src)
{
    if(src == NULL)
    {
        return 0;
...
// add by mtk94010
size_t IconvLCPTranscoder::mywcsrtombs(char *dst, const wchar_t **src, size_t len)
{    
    if(src == NULL || *src == NULL || dst == NULL)
    {
        return (size_t)TRANSCODING_ERROR;
    }

	// get wchar length
	const char*  s = (const char*)*src;   
	size_t src_len = mywcslen(*src);

	size_t index = 0;
	size_t count = len - 1 > src_len ? src_len : len - 1;
                                    ...
                                    MemoryManager* const manager)
{
    ...
    while (toTranscode[srcCursor] || src)
    {
        ...
        size_t len = mywcsrtombs(resultString + dstCursor, &src, resultSize - dstCursor);
...
size_t IconvLCPTranscoder::mymbsrtowcs(wchar_t *dst, const char **src, size_t len)
{
	...
	size_t count = src_len < len - 1? src_len : len -1;
	size_t index = 0;
	for(;index < count; ++ index)
	{
	    dst[index] = s[index];
                                     ...
                                     MemoryManager* const manager)
{
    ...
    while(true)
    {
        size_t len = mymbsrtowcs(tmpString + dstCursor, &src, resultSize - dstCursor);
        if (len == TRANSCODING_ERROR)
        {
            dstCursor = 0;
            break;
