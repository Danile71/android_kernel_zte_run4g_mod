...
static int get_AT_command(char *origin_at_command)
{
	...
	if(ptr == NULL)
	{
		...
		strncpy(at_command, origin_at_command, 128);
...
static int test_module()
{
		...
		while(1)
		{
            ...
            strncpy(p, at_command, AT_COMMAND_LEN);
...
char ** trans_verinfo(const char *str, int *line)
{
    ...
    memset(pstrs, 0, g_nr_lines * sizeof(char*));
    while (len--) {
        if ('\n' == *end) {
            pstrs[row] = (char*)malloc((end - start + 1) * sizeof(char));
            memset(pstrs[row], 0, (end - start + 1) * sizeof(char));
            if (!pstrs[row]) {
                LOGE("In factory mode: malloc failed\n");
                return NULL;
            }
