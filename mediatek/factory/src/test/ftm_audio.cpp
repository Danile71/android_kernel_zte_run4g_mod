...
static const char ouput_dev_name[3][16] = {"Reveiver", "Headset", "Speaker"};
...
static void *Audio_Headset_detect_thread(void *mPtr)
{
    ...
    */
    ...
    while (1)
    {
        ...
        if (false)    // Headset device change
        {
        ...
        {
            if ( false)    // without Earphone plug in/out
            {
            ...
            else    // with Earphone plug in/out
            {
                ...
                if (g_loopback_item == 1)
                {
                    ...
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC1_ON);  // enable MIC1 loopback
                }
                else if (g_loopback_item == 2)
                {
                    usleep(3000);
                    ...
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC2_ON);  // enable MIC2 loopback
                }
            }
        }
...
static void * Speaker_Monitor_Record_Thread(void *mPtr)
{
    ...
    android::AudioFtmBase::createAudioFtmInstance()->GetSpkMonitorParam((void *)&SpkMonitorParam);
    recordInit(hds->recordDevice, &samplerate);
    while (1) {
       memset(pbuffer,0,sizeof(pbuffer));
       memset(pbufferL,0,sizeof(pbufferL));
	   ...
	   for(i = 0 ; i < 4096 ; i++)
       {
        ...
        if(checkCnt >= NUMBER_CONFIDENCE)
        {
            ...
            if(R_CLUSTER_NUMBER == 512)
            {
                ...
                android::AudioFtmBase::createAudioFtmInstance()->SetSpkMonitorParam((void*)&SpkMonitorParam);
#if 0
...
int mAudio_speaker_monitor_autotest_entry(struct ftm_param *param, void *priv)
{
    ...
    android::AudioFtmBase::createAudioFtmInstance()->SetStreamOutPostProcessBypass(true);//Bypass Compensation filter
    android::AudioFtmBase::createAudioFtmInstance()->EnableSpeakerMonitorThread(false);//Disable speaker monitor thread
    ...
    if(mc->mod->test_result == FTM_TEST_PASS)
        usleep(2000000);
    android::AudioFtmBase::createAudioFtmInstance()->EnableSpeakerMonitorThread(true);
    android::AudioFtmBase::createAudioFtmInstance()->SetStreamOutPostProcessBypass(false);//don't bypass compensation filter
    mc->exit_thd = true;
    ...
    do
    {
        ...
        switch (chosen)
        {
            case ITEM_RINGTONE:
                ...
                {
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC1_OFF);  // disable Receiver MIC1 loopback
                }
                else if (privChosen == ITEM_MIC2)
                {
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC2_OFF);  // disable Receiver MIC2 loopback
                }
                else if (privChosen == ITEM_MIC3)
                {
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC3_OFF);  // disable Receiver MIC3 loopback
                }
                else if (privChosen == ITEM_MIC4)
                {
                    usleep(3000);
                    PhoneMic_EarphoneLR_Loopback(MIC4_OFF);  // disable Receiver MIC4 loopback
                }
            ...
            case ITEM_MIC1:
                ...
                usleep(3000);
                PhoneMic_EarphoneLR_Loopback(MIC1_ON);  // enable Receiver MIC1 loopback
            ...
            case ITEM_MIC2:
                ...
                usleep(3000);
                PhoneMic_EarphoneLR_Loopback(MIC2_ON);  // enable Receiver MIC2 loopback
            ...
            case ITEM_MIC3:
                ...
                usleep(3000);
                PhoneMic_EarphoneLR_Loopback(MIC3_ON);  // enable Receiver MIC3 loopback
            ...
            case ITEM_MIC4:
                ...
                usleep(3000);
                PhoneMic_EarphoneLR_Loopback(MIC4_ON);  // enable Receiver MIC4 loopback
...
int mAudio_receiver_entry(struct ftm_param *param, void *priv)
{
    ...
    memset(uName, 0, sizeof(uName));
    sprintf(uName, "Audio.ManualTest");
    pTestType = ftm_get_prop(uName);

    if (pTestType != NULL )
    {
        test_type = (int)atoi(pTestType);
        ...
        if (!get_is_ata())
        {
            ...
            switch (chosen)
            {
                case ITEM_MICBIAS_ON: /*Open Mic*/
                    ...
                    if (!isMicOn)
                    {
                        recordInit(WIRED_HEADSET, &samplerate);
                        memset(pbuffer, 0, sizeof(pbuffer));
