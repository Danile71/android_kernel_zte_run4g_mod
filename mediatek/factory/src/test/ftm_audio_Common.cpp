...
{
    // config this is not android mode first
    android::AudioFtmBase::setIsInAndroidMode(false);
...
{
    ALOGD("%s(), bEnable = %d, bHeadsetMic = %d", __FUNCTION__, bEnable, bHeadsetMic);
    if (bHeadsetMic == true)
    {
    ...
    {
        return gAudioFtmBase->PhoneMic_EarphoneLR_Loopback(bEnable);
    }
}
...
int SineGenTest(char sinegen_test)
{
    return gAudioFtmBase->SineGenTest(sinegen_test);
}
...
static void *Audio_Wave_Playabck_routine(void *arg)
{
    ...
    int32_t *pbuffer32 = NULL;
    void *pBitConverBuf = NULL;
#endif
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
    ReadBlockLen = hwBufferSize >> 1;
    pBitConverBuf = new char[hwBufferSize];
#endif
...
#else
        int16_t *Sample = (int16_t *)outBuffer;
        pbuffer32 = (int32_t*)pBitConverBuf;

        while (out_size > 0)
        {
            *pbuffer32++ = ((int32_t)(*Sample++)) << 16;
            *pbuffer32++ = ((int32_t)(*Sample++)) << 16;
            out_size -= 4;
        }

        writedata = streamOutput->write(pBitConverBuf, hwBufferSize);
#endif
        ALOGD("Audio_Wave_Playabck_routine write to hardware... read = %d writedata = %d", readdata, writedata);
    }
...
/// record
bool recordInit(int device_in, uint32_t *sample_rate)
{
    if (gAudioStreamIn == NULL)
    {
        ...
        else if (device_in == SPK_FEED_INPUT)
        {
            sampleRate = 48000;
            device = AUDIO_DEVICE_IN_SPK_FEED;
            gAudioStreamIn = gAudioHardware->openInputStream(device, &format, &channel, &sampleRate, &status, (android_audio_legacy::AudioSystem::audio_in_acoustics)0);
        }
#endif
        else
...
int Audio_VSCurrent_GetFrequency()
{
    ...
    int32_t center_freq, counter, adc_value, checkidx, changefreqflag;
    ...
    ********************************************/
    ...
    while (1)
    {
        ...
        counter = vibspkwrite_counter;
        if ((counter -  checkidx)>= TONE_FRAME_COUNT)
        {
...
#else
    //memcpy(&cali_param.bes_loudness_bpf_coeff_L, &AUD_VIBRFACTORY_FILTER_COEF_Table[vib_idx], sizeof(uint32_t)*VIBSPK_AUD_PARAM_SIZE);
    memcpy(&cali_param.bes_loudness_f_param.V5ViVSPK.bes_loudness_bpf_coeff, &AUD_VIBRFACTORY_FILTER_COEF_Table[vib_idx], sizeof(uint32_t)*VIBSPK_AUD_PARAM_SIZE);
#endif
...
extern "C" {
    ...
    /// Output device test
    int SineGenTest(char sinegen_test) { return true; }
    ...
    bool recordInit(int device_in, uint32_t *sample_rate) { return true; }
