...
#ifdef FEATURE_FTM_SWCHR_HW_DETECT
#define CH_EXT_SWCHR             _IOW('k', 22, int)

int is_swchr_hw_detect(void)
{
    int fd = -1;
    int ret = 0;
    int adc_in_data[2] = {1,1}; 
    ...
    if (fd >= 0)
    {
	ret = ioctl(fd, CH_EXT_SWCHR, adc_in_data);
    	close(fd);
    }
    else
    {
    ...
    LOGD(TAG "[is_swchr_hw_detect] result=%d\n", adc_in_data[0]); 
...
int get_charging_current(void)
{
...
#elif FEATURE_FTM_SWCHR_I_CUST_MOHM           
            temp_charging_current = ((temp_i_sen-temp_bat_sen)*1000)/FEATURE_FTM_SWCHR_I_CUST_MOHM;
#else           
    ...
    #ifdef FEATURE_FTM_SWCHR_HW_DETECT
    if(is_swchr_hw_detect())
    {
        LOGD(TAG "[FEATURE_FTM_SWCHR_HW_DETECT] PASS\n");	
        temp_charging_current = 499;
    }
    else
    {
        LOGD(TAG "[FEATURE_FTM_SWCHR_HW_DETECT] FAIL\n");	
        temp_charging_current = 0;
    }
    #else
...
int battery_entry(struct ftm_param *param, void *priv)
{
  ...
  if(temp_v_chr < AUTO_TEST_VCHR_VALUE)
  {
	...
	{
		LOGD(TAG "[FTM_BAT 4] %d,%d,%d,%d,%d,%d,%d,%d\n", 
			temp_v_bat, temp_chr_cuttent, i, i_loop_time, 
			AUTO_TEST_VBAT_VALUE, AUTO_TEST_THD_VALUE, AUTO_TEST_CHR_CURRENT_VALUE, temp_v_chr);
