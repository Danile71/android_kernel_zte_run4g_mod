...
int idle_entry(struct ftm_param *param, void *priv)
{
    ...
    /* Make MD into flight mode */
        ...
        #else
            snprintf(ccci_md5_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE, MD_SYS5));
            LOGD(TAG "Open CCCI MD5 dev node: %s\n", ccci_md5_dev);
            fd_atcmd_dt= openDeviceWithDeviceName(ccci_md5_dev);
        #endif
        ...
        #ifdef MTK_MD_SHUT_DOWN_NT
            snprintf(ccci_md5_power_ioctl_dev, 32, "%s", ccci_get_node_name(USR_FACTORY_IDLE_IOCTL, MD_SYS5));
            LOGD(TAG "Open CCCI MD5 Power IOCTL dev node: %s\n", ccci_md5_power_ioctl_dev);
            fd_ioctlextmd = openDeviceWithDeviceName(ccci_md5_power_ioctl_dev);
            if (-1 == fd_ioctlextmd)
            {
                LOGD(TAG "Fail to open CCCI MD5 IOCTL interface\n");
                return 0;
            }
            ExitFlightMode_PowerOffModem(fd_atcmd_dt,fd_ioctlextmd,FALSE);
        #else
    ...
    #if defined(MTK_EXTERNAL_MODEM_SLOT) && !defined(EVDO_DT_SUPPORT)
        #ifdef MTK_MD_SHUT_DOWN_NT
            ExitFlightMode_PowerOffModem(fd_atcmd_dt,fd_ioctlextmd,TRUE);
            closeDevice(fd_ioctlextmd);
        #endif
