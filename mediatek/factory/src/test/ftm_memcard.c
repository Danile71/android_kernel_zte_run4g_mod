    ...
    if (mc_fix->avail && !mc_fix->mounted) {
        ...
        if(mc_fix->id > hot_plug_host && mc_removable->avail){
            ...
            if(mc_fix->id == CUST_EMMC_ID) {
				...
				sprintf(mc_fix->dev_path, "/dev/block/platform/mtk-msdc.0/by-name/intsd");
#else			
...
static void mcard_update_info(struct mcard *mc, char *info)
{
    ...
    if (mc->avail && !mc->mounted) {
        ...
        #ifdef MTK_EMMC_SUPPORT
            sprintf(mc->dev_path, "/dev/block/mmcblk%dp1\0", 1);
        #else
            sprintf(mc->dev_path, "/dev/block/mmcblk%dp1\0", 0);
        #endif

        fd = open(mc->dev_path, O_RDONLY);
        if (fd < 0) {
            #ifdef MTK_EMMC_SUPPORT
            sprintf(mc->dev_path, "/dev/block/mmcblk%d\0", 1);
            #else
            sprintf(mc->dev_path, "/dev/block/mmcblk%d\0", 0);
            #endif
            fd = open(mc->dev_path, O_RDONLY);
            if (fd < 0) {
                sprintf(mc->dev_path, "/emmc@fat\0");
                fd = open(mc->dev_path, O_RDONLY);
            }
        }
...
#ifdef MTK_GPT_SCHEME_SUPPORT               
                    sprintf(fat_partition_path, "/dev/block/platform/mtk-msdc.0/by-name/intsd");
#else 
...
int mcard_init(void)
{
    ...
    ma->mod      = mod;
       ...
       mc->id       = CUST_EMMC_ID;
       #else
       if (!access("/emmc@fat", F_OK) || !access("/dev/block/platform/mtk-msdc.0/by-name/intsd", F_OK)) {
          /* it has internal sd*/
           mc->id = CUST_EMMC_ID;
       }
       else {
          /* it ONLY has one EXTERNAL sd */
          mc->id = CUST_MCARD_ID;
       }
       #endif
    #else
