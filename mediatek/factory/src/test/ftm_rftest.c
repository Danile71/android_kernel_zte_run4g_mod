...
static void *rf_update_thread(void *priv)
{
    ...
    if(!rf->test_done) {
        ...
        while(retryCount < 100){
            ...
            LOGD(TAG "RF_TEST_RSSI_LEVEL=%d, rssi_level=%d\n",RF_TEST_RSSI_LEVEL, rssi_level);				
            if(rssi_level < RF_TEST_RSSI_LEVEL && rssi_level!=0){
                ret = 1;
                LOGD(TAG "rssi_level pass in RF test");				
                break;
            }else if(rssi_level >= RF_TEST_RSSI_LEVEL){
                ret = 0;
