...
int FlashSimpleDrv::init(unsigned long sensorDev)
{
	...
	mVer = ioctl(m_fdSTROBE,FLASH_IOC_GET_PROTOCOL_VERSION,0);
	int value;
	int err;
	if(mVer==1)
	{

	    err = ioctlEx(m_fdSTROBE,FLASH_IOC_GET_MAIN_PART_ID, mSensorDev, (int)(&value));
	    err = ioctlEx(m_fdSTROBE,FLASH_IOC_GET_SUB_PART_ID, mSensorDev, (int)(&value));
