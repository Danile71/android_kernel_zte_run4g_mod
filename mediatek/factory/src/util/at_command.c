...
bool is_support_modem(int mIndex)
{
    bool ret = false;
...
int open_ccci(int ccci_handle[])
{
    char dev_node[MAX_MODEM_INDEX][32] = {0};
    int idx[MAX_MODEM_INDEX] = {1,2,5};
    ...
    for(i = 0; i < MAX_MODEM_INDEX; i++)
    {
        if(is_support_modem(idx[i]))
        {
            snprintf(dev_node[i], 32, "%s", ccci_get_node_name(USR_FACTORY_DATA, md_sys[i]));
            ccci_handle[j] = openDeviceWithDeviceName(dev_node[i]);
            if(ccci_handle[j] == -1)
            {
                LOGD(TAG "Open ccci in modem1 fail\n");
                return -1;
            }
            j++;
        }
        ...
        #if defined(PURE_AP_USE_EXTERNAL_MODEM)
            ccci_handle[j] = openDeviceWithDeviceName("/dev/ttyUSB1");
            initTermIO(ccci_handle[j], 5);
            if(ccci_handle[j] == -1)
            {
                LOGD(TAG "Open ccci in modem1 fail\n");
                return -1;
    ...
    for (i = 0; i<50; i++) usleep(50000); //sleep 2.5s wait for modem bootup
...
int read_ack_for_ate_factory_mode(const int fd, char *rbuff, int length, int *flag)
{
    ...
    while((has_read<length) && (temp_flag == *flag))
    {
...
int wait4_ack2 (const int fd, char *pACK, char *pACK1, char *pACK2,  char *pACK3, int timeout)
{
    char buf[BUF_SIZE] = {0};
	char *  p = NULL;
    ...
    for(rdCount = 0; rdCount < LOOP_MAX; ++rdCount) 
    {
		...
		if ((pACK != NULL) && (pACK1 != NULL) && (pACK2 != NULL) && (pACK3 != NULL))   
        {
...
const char* dial112(const int fd)
{
	...
	if(wait4_ack2(fd, "+ECPI: 1,6", "ESPEECH", "+ECPI: 1,2", "+ECPI: 1,3", 30000))
		{
