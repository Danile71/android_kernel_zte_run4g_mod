...
#undef MTK_AVI_SUPPORT_RAWAAC
#define MTK_AVI_SUPPORT_RAWAAC
...
static bool get_mp3_frame_size(uint32_t header, size_t * frame_size,
...
sp < MetaData > MtkAVIExtractor::getTrackMetaData(size_t index, uint32_t flags) {
    status_t err;
    if ((err = readMetaData()) != OK) {
        return NULL;
    }
...
status_t MtkAVIExtractor::parseMOVIMore(bool full) {
    ...
    LOGI("parse movi chunks to setup index from 0x%08x to 0x%08x full=%d", (int32_t) pos,
         (int32_t) oldEnd, full);
...
status_t MtkAVIExtractor::parseAVIH(MtkAVIOffT pos, MtkAVIOffT end) {
    ...
    LOGI("width %d, height %d, tracks %d, padding %d", mWidth, mHeight, mNumTracks,
         header.paddingGranularity);
    return OK;
}
...
status_t MtkAVIExtractor::parseSTRL(MtkAVIOffT pos, MtkAVIOffT oldEnd, int index) {
    ...
    while (pos < oldEnd) {
        ...
        if (list.ID == BFOURCC('L', 'I', 'S', 'T')) {
            ...
            case BFOURCC('s', 't', 'r', 'n'):
                ...
                if (errSTRN != OK)
                    LOGE("parse optional case STRN error, unable to support audio change");
                break;
#endif
            default:
    ...
    if (pos > oldEnd || !strhFound || !strfFound) {
        LOGE("strl of stream %d seems to be corrupted: end at 0x%08lx, "
             "expected end at 0x%08lx, strh=%d, strf=%d", index, pos, oldEnd, strhFound, strfFound);
        return ERROR_MALFORMED;
    }
    return OK;
}
...
status_t MtkAVIExtractor::parseChunkIndex(MtkAVIOffT pos, MtkAVIOffT end,
                                          sp < MtkAVISource > source,
    ...
    if (pHeader->indexType != kKeyAVIIndexOfChunks || pHeader->indexSubType != 0) {
    ...
    while (i < numbers) {
        ...
        for (int j = 0; j < read; ++j) {
            ...
            if ((entry.offset + baseOffset + (entry.size & 0x7fffffff)) > mFileSize) {
                LOGI("break addSample! offset %d, size %d,mFileSize %d",
                     entry.offset + baseOffset, entry.size & 0x7fffffff, mFileSize);
                hastobreak = true;
                break;
            }
...
status_t MtkAVIExtractor::parseSTRN(MtkAVIOffT off, MtkAVIOffT end, sp < MtkAVISource > source) {
...

    {
        ...
        meta->setCString(kKeyMediaLanguage, lang_code);
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, sp < MtkAVISource > source) {
    ...
    if (source->mIsVideo) {
        ...
        if (source->mIsAVC) {
            ...
            if (mDataSource->readAt(posBak + kSizeOfBitmapInfo, csd->data(),
                                    videoExtraConfigDataSize) != videoExtraConfigDataSize) {
                LOGD("error read extra data for H.264");
            }
            LOGD("H.264 extra data length is %d", videoExtraConfigDataSize);
            for (int s = 0; s < csd->size(); s++) {
                LOGD("%02d: %02x", s, csd->data()[s]);
            }
            ...
            if (source->AVCStyle == SIZE_NAL_TYPE) {
                ...
                if ((source->AVCSizeLength != 4)
                    ...
                    LOGE("abnormal nal size in sps");
                    source->AVCStyle = START_CODE_TYPE;
                }
                LOGI("AVCSizeLength = %ld", source->AVCSizeLength);
            }
...
#ifdef MTK_AUDIO_DDPLUS_SUPPORT
        if (!strcmp(mime, MEDIA_MIMETYPE_AUDIO_AC3)) {
            source->mIsAC3 = true;
        }
#endif
        ...
        if (!strcmp(mime, MEDIA_MIMETYPE_AUDIO_MS_ADPCM)
            ...
            if (source->mSize != 0) {
                meta->setData(kKeyExtraDataPointer, 0, source->strfData->data(),
                              source->strfData->size());
        ...
        if ((source->mSampleSize != source->mBlockAlign)
            ...
            LOGW("PCM sample size (%d) != block align (%d), fix it", source->mSampleSize,
                 source->mBlockAlign);
            source->mSampleSize = source->mBlockAlign;
        }
// ALPS01920718 refer to AVI Spec 5.1MP3 
if((source->mSampleSize == source->mBlockAlign)&&(source->mSampleSize == 1152)&&(!strcmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG))){
	source->mSampleSize = 0;
}
        ...
        LOGI("aHeader.formatTag = 0x%x aHeader.nChannels = %d aHeader.nSamplesPerSec = %d aHeader.nAvgBytesPerSec = %d aHeader.size = %d", aHeader.formatTag, aHeader.nChannels, aHeader.nSamplesPerSec, aHeader.nAvgBytesPerSec, aHeader.size);
...
status_t MtkAVIExtractor::parseSTRH(MtkAVIOffT pos, MtkAVIOffT end, sp < MtkAVISource > source) {
    ...
    LOGI("header scale %d rate %d length %d samplesize %d init %d start %d",
         header.scale, header.rate, header.length, header.sampleSize, header.initialFrames,
         header.start);
        ...
        if (list.ID == BFOURCC('L', 'I', 'S', 'T')) {
            ...
            default:
                if (times < 5 || times % 5000 == 0) {
                    LOGW("unknown chunk " FORMATFOURCC " in %s", PRINTFOURCC(list.ID),
                         __FUNCTION__);
                }
                times++;
                break;
...
status_t MtkAVISource::start(MetaData * params) {
    ...
    if (mIsPCM && mBitsPerSample == 8) {
        ...
        mGroup->add_buffer(new
                           MediaBuffer(MAX_MP3_FRAMESIZE >
                                       (2 * max_size) ? MAX_MP3_FRAMESIZE : (2 * max_size)));
        mGroup->add_buffer(new
                           MediaBuffer(MAX_MP3_FRAMESIZE >
                                       (2 * max_size) ? MAX_MP3_FRAMESIZE : (2 * max_size)));
    } else {
...
static int realhaveIframe(const uint8_t *start, int length)
...
bool isNALStartCodeType(const sp < ABuffer > &buffer) {
    ...
    int32_t iPPSPos = 0;
    if (NULL == buffer->data()) {
        LOGE("[error] buffer->data() is null!");
        return false;
    }
    iSPSPos = realAVCStart(buffer->data(), buffer->size());

    if (-1 == iSPSPos) {
        LOGE("isNALStartCodeType() NO SPS!!");
        return false;
    }
    iPPSPos = PPSStart(buffer->data(), buffer->size());
    if (-1 == iPPSPos) {
        LOGE("isNALStartCodeType() NO PPS!!");
        return false;
    }
    if (iPPSPos < iSPSPos) {
        LOGE("[error] PPS is in front of SPS.");
    }
...
uint32_t parseNALSize(const uint8_t sizeLength, uint8_t * data) {
  ...
  fail:
    LOGD("illeagel nal size case %d", sizeLength);
    return 0;
}
...
static int countAC3Frames(const uint8_t *start, int size)
{
    int ret = 0;
    for (int i = 0; i < size; ++i) {
        if (start[i] == 0x0B && start[i+1] == 0x77) {
		    ret++;
        }
    }
	LOGD("totally found %d frames in one chunk", ret);
	return ret;
}
...
status_t MtkAVISource::readNextChunk(uint8_t * data, int size, ssize_t & num_bytes_read, int offset) {
    ...
    num_bytes_read = mDataSource->readAt(nextOffset + offset, data, size);
...
status_t MtkAVISource::read(MediaBuffer ** out, const ReadOptions * options) {
    Mutex::Autolock autoLock(mLock);
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
        ...
        if (mBlockMode) {
            ...
            if (index >= totalSamples) {
                LOGI("seekTimeUs %lld,  mRate %d,  mScale %d, index %d, totalSamples %d",
                     seekTimeUs, mRate, mScale, index, totalSamples);
            ...
            while (low < high) {
                ...
                if ((mAvgBytesPerSec > 0) && isRawAACData) {
                    if (bVBRWise) {
                        ...
                        LOGD("[Jimmy] ln(%d), VBR seek to index(%d), totalSize(%d), mScale(%d), mRate(%d), mSampleSize(%d)", __LINE__, index, totalSize, mScale, mRate, mSampleSize);
                        break;
                    } else {
                        // CBR-like
                        dts = (int64_t) (totalSize) * 1000000LL / mAvgBytesPerSec;
                        LOGD("[Jimmy] ln(%d), CBR seek to dts(%lld), index(%d), totalSize(%d), mScale(%d), mRate(%d), mSampleSize(%d)", __LINE__, dts, index, totalSize, mScale, mRate, mSampleSize);
                    }
                    //dts = (int64_t)(totalSize) * 1000000LL / mAvgBytesPerSec ;
                } else {
                    dts = (int64_t) (totalSize) * 1000000LL * mScale / mRate / mSampleSize;
    ...
    if (mBuffer == NULL || (mMP3Header < 0 && mBuffer->range_length() < 4)) {
        ...
        if (mMP3Header < 0 && mBuffer && mBuffer->range_length() < 4) {
            memcpy((uint8_t *) mBuffer->data(),
                   (uint8_t *) mBuffer->data() + mBuffer->range_offset(), mBuffer->range_length());
    ...
    if (newBuffer) {
        ...
        ssize_t num_bytes_read =
            mDataSource->readAt(offset, (uint8_t *) mBuffer->data() + mBuffer->range_offset(),
                                size);
        ...
        if (mIsAudio && mSampleSize != 0) {
            ...
            if (isRawAACData) {
                ...
                if (bVBRWise) {
                    // VBR-like
                    dts = (int64_t) (mCurrentSampleIndex - 1) * 1000000LL * mScale / mRate;
                    LOGD("[Jimmy] Audio ln(%d), VBR dts(%lld), mCurrentSampleIndex(%d), mScale(%d), mRate(%d), mSampleSize(%d)", __LINE__, dts, mCurrentSampleIndex, mScale, mRate, mSampleSize);
                } else {
                    // CBR-like
                    dts = (int64_t) (totalSize) * 1000000LL / mAvgBytesPerSec;
                    LOGD("[Jimmy] Audio ln(%d), CBR dts(%lld), mCurrentSampleIndex(%d), mScale(%d), mRate(%d), mSampleSize(%d)", __LINE__, dts, mCurrentSampleIndex, mScale, mRate, mSampleSize);
                }
            } else {
                dts = (int64_t) (totalSize) * 1000000LL * mScale / mRate / mSampleSize;
            }
        ...
        LOGV("v=%d, dts=%lld off=%lx s=%x totalSize=%x tid=%d", mIsVideo, dts, offset, size,
             totalSize, gettid());
	...
	if (mIsAC3) {
		int frameCount = countAC3Frames((const uint8_t*)mBuffer->data() + mBuffer->range_offset(),
			                            mBuffer->range_length());
	    if (frameCount > 1) {
			LOGE("cannot support multi frames ac3");
			return ERROR_END_OF_STREAM;
	    }
    ...
    if (mIsMJPG) {
        ...
        if (newBuffer) {
            ...
            mBuffer->set_range(mBuffer->range_offset() + length, mBuffer->range_length() - length);
        }
#endif
        ...
        CHECK(mBuffer != NULL);
        mBuffer->set_range(mBuffer->range_offset() + start + length,
    ...
    if (mIsAVC) {
        if (AVCStyle == SIZE_NAL_TYPE) {
            ...
            start = AVCSizeLength;
            length = parseNALSize(AVCSizeLength, (uint8_t *) mBuffer->data());
            if (((length + start) != mBuffer->range_length()) || (length < 0)) {
                start = mBuffer->range_length();
                length = 0;
                *out = mBuffer;
                mBuffer = NULL;
            ...
            *out = clone;
            ...
            if (mWantsNALFragments) {
                ...
                if (newBuffer) {
                    ...
                    mBuffer->set_range(mBuffer->range_offset() + length,
        ...
        *out = mBuffer;
        ...
        do {
            ...
            LOGV("mp3 start %d header %x frameSize %d length %d bitrate %d", start, header,
                 frameSize, length, bitrate);
            ...
            if (start >= 0 && ret) {
                if (frameSize + start > length) {
                    ...
                    while (readret == OK) {
                        readret =
                            readNextChunk((uint8_t *) tmp->data() + existSize, needSize, bytesRead);
                    ...
                    if (readret == OK) {
                        memcpy(tmp->data(),
                               (uint8_t *) mBuffer->data() + mBuffer->range_offset() + start,
                               existSizeOrg);
                        mBuffer->release();
                        mBuffer = NULL;
                ...
                if (NeedRefinePos) {
                    ...
                    mBuffer->set_range(mBuffer->range_offset() + frameSize + start,
                                       mBuffer->range_length() - frameSize - start);
                    LOGI("refine pos: NeedRefinePos=%d,mCurrentDTS=%lld, begin=%d,end=%d",
                         NeedRefinePos, mCurrentDTS, mBuffer->range_offset(),
                         mBuffer->range_length());

                    if (seekTimeUs > mCurrentDTS
                        && seekTimeUs - mCurrentDTS < frameSize * 8000ll / bitrate) {
                        NeedRefinePos = false;
        ...
        *out = clone;
        ...
        if (mIsMPEG4 || mIsDIVX || mIsXVID) {
            if (mIsFirstBuffer) {
                ...
                int offsetStart =
                    seqStartAt((const uint8_t *) mBuffer->data(), mBuffer->range_length());
                if (offsetStart > 0) {
                    mBuffer->set_range(mBuffer->range_offset() + offsetStart,
                ...
                if (type == kKeyAVIUnknownFrame || offsetStart < 0) {
                    ...
                    offsetStart =
                        seqStartAt((const uint8_t *) ptr + 4, mBuffer->range_length() - 4);
                    if (offsetStart >= 0)
                        offsetStart += 4;

                    if (ptr[3] == 0xb3 && offsetStart >= 0) {
                        LOGI("found VOP_STARTCODE and GOP_STARTCODE at %x",
                             mSampleOffsets[mCurrentSampleIndex - 1]);
                        int off = seqStartAt((const uint8_t *) ptr + offsetStart + 4,
                                             mBuffer->range_length() - offsetStart - 4);
                        if (off >= 0) {
                    ...
                    if (offsetStart >= 0) {
                        LOGI("packed sample at %x, offset %x",
                             mSampleOffsets[mCurrentSampleIndex - 1], offsetStart);
                        mIsMultiple = true;
    ...
    while (start < (int) mSampleSizes.size()) {
        ...
        while (i + 3 < size) {
            ...
            if (get_mp3_frame_size(header1, &frame_size, NULL, NULL, NULL)) {
                ...
                LOGI("possible header %x at %x size %x, test %x", header1, offset + i,
                     frame_size, header2);
...
status_t MtkAVISource::generateCodecData(bool full) {
    ...
    LOGD("generate codec info , avc style is %d", SIZE_NAL_TYPE);

    if (mIsAVC && (AVCStyle == SIZE_NAL_TYPE)) {
        LOGI("length+nalu style h.264");
        return OK;
    }

    int haveIframe = 0;// mtk08123 for avc
    int AVCPos = 0;
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if ((strfData != NULL) && ((!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_MPEG4))
            ...
            found = true;
                                          ...
                                          ||
                                          (!strcasecmp
                                           (mime, MEDIA_MIMETYPE_VIDEO_SORENSON_SPARK)))) {
    ...
    #ifndef ANDROID_DEFAULT_CODE //decoder request like this, some special divx file, don't have vol info, will init drvier error
	if (isDIVX) {//ALPS01669252
            mFormat->setData(kKeyMPEG4VOS, 0, 0, 0);
            LOGE("divx codec, didn't add special data");
            return OK;
        }
    #endif
        ...
        if (mSampleSizes.size() > 0) {
            ...
            //in some file , the fragment of NAL belongs to first I frame maybe exist in
            // the data used to make CSD.
            if (mIsAVC) {
                 haveIframe = realhaveIframe(buffer->data(), size);
                 if(haveIframe > 0){
                 size = haveIframe;
              sp<ABuffer> buffer = new ABuffer(size);
              if (mDataSource->readAt(offset, (uint8_t*)buffer->data(), size) != size) {
                LOGE("avc failed to read data at 0x%08x", offset);
                return ERROR_IO;
               }
              }
            }
        ...
        if (mSampleSize > 1 && mScale == 1) {
            ...
            get_mp3_frame_size(mMP3Header, (size_t *) & framesize, NULL, NULL, NULL,
                               &sampleperframe);
        ...
        mFormat->setData(kKeyVorbisBooks, 0, data + len1 + len2 + 3, size - len1 - len2 - 3);
        mIsVorbis = true;
    }
...
bool MtkSniffAVI(const sp < DataSource > &source, String8 * mimeType, float *confidence,
                 sp < AMessage > *) {
    char header[12];
