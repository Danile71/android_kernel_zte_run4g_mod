...
#undef LOGV
#undef LOGM
#define LOGE(x, ...)   ALOGE(" "x,  ##__VA_ARGS__)
#define LOGD(x, ...)   ALOGD(" "x,  ##__VA_ARGS__)
#define LOGV(x, ...)   ALOGV(" "x,  ##__VA_ARGS__)
#define LOGM(x, ...)   ALOGV(" "x,  ##__VA_ARGS__)

flvParser::flvParser(void *source, flv_io_read_func_ptr read,
                     flv_io_write_func_ptr write, flv_io_seek_func_ptr seek) {
    ...
    if (!mfile)
        LOGE("flvParser:Error failed to Initialize FLV parser");

    LOGV("flvParser:mfile=0x%08x\n", mfile);
...
flv_file_str *flvParser::flv_open_file(flv_iostream_str * iostream) {
    ...
    if (!iostream) {
        LOGE("flv_open_file: error1, iostreamis NULL\n");
        return NULL;
    }
    flvfile = (flv_file_str *) calloc(1, sizeof(flv_file_str)); //malloc + set0
    if (!flvfile) {
        LOGE("flv_open_file: error2, alloc mem fail\n");
        return NULL;
    }
    LOGM("[memory] alloc 0x%08x\n", flvfile);
    ...
    if (!flvfile->mMeta) {
        LOGM("[memory] free 0x%08x\n", flvfile);
        free(flvfile);
        flvfile = NULL;
        LOGE("flv_open_file: error3, alloc mem fail\n");
        return NULL;
    }
    ...
    if (!mSeekTable) {
        ...
        free(flvfile);
        flvfile = NULL;
        LOGE("flv_open_file: error4, alloc mem fail\n");
        return NULL;
    }
    LOGM("[memory] alloc 0x%08x\n", mSeekTable);
...
FLV_ERROR_TYPE flvParser::IsflvFile() {
    ...
    if (tmp < 3) {
        LOGE("flv_parse_header: error read file,tmp=%d\n", tmp);
    ...
    if (tmp == 0) {
        LOGD("IsflvFile: this is an FLV file\n");
        return FLV_OK;
    }
    LOGD("IsflvFile:not FLV file, TAG=%s\n", TAG);
    return FLV_ERROR;
...
FLV_ERROR_TYPE flvParser::ParseflvFile() {
    ...
    LOGD("ParseflvFile: flv_parse_header done:cur_file_offset=0x%llx\n", mfile->cur_file_offset);
    ...
    LOGD("ParseflvFile: flv_parse_script done:cur_file_offset=0x%llx\n", mfile->cur_file_offset);

    ret = flv_setup_seektable();
    if (ret != FLV_OK)
        return ret;
    LOGD("ParseflvFile: flv_setup_seektable done:cur_file_offset=0x%llx\n", mfile->cur_file_offset);
...
FLV_ERROR_TYPE flvParser::flv_setup_seektable() {
    ...
    if (mfile->mMeta->filepositions && mfile->mMeta->times) {
        ...
        LOGD("flv_setup_seektable 1: seek MaxEntries=%d(limit %d)\n",
                        mSeekTable->MaxEntries, FLV_SEEK_ENTRY_MAX_ENTRIES);
        LOGD("flv_setup_seektable 1: seek TimeGranularity=%lld ms\n", mSeekTable->TimeGranularity);
        LOGD("flv_setup_seektable 1: seek SetEntries=%d\n", mSeekTable->SetEntries);
    ...
    LOGD("flv_setup_seektable 2: seek MaxEntries=%d\n", mSeekTable->MaxEntries);
    LOGD("flv_setup_seektable 2: seek TimeGranularity=%lld ms\n",
                   mSeekTable->TimeGranularity);
    LOGD("flv_setup_seektable 2: seek SetEntries=%d \n", mSeekTable->SetEntries);
...
FLV_ERROR_TYPE flvParser::flv_parse_script() {
    ...
    if (!pMeta) {
        LOGE("flv_parse_script: error1,calloc failed \n");
        return FLV_ERR_NO_MEMORY;
    }
    LOGD("[memory]flv_parse_script: Alloc 0x%08x \n", pMeta);

    while (1) {
        ret = flv_read_tag_header(&(pMeta->tag_header));    //FLV_TAG_HEADER_SIZE
        if (ret != FLV_OK) {
            LOGE("flv_parse_script: error2\n");
            ret = FLV_FILE_READ_ERR;
            break;
        }
        if (pMeta->tag_header.tag_type != FLV_TAG_TYPE_META) {
            LOGE("flv_parse_script:this not a script,tag_type is %d,cur_file_offset=0x%llx\n",
                 pMeta->tag_header.tag_type, mfile->cur_file_offset);
            mfile->mIoStream.seek(mfile->mIoStream.source,
                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            ret = FLV_OK;
            break;
        }
        //FLV_TAG_TYPE_META
        if (mfile->hasMeta == 0) {
            ...
            if (!pMeta->tag_data) {
                LOGE("flv_parse_script: error3,calloc failed \n");
                return FLV_ERR_NO_MEMORY;
            }
            LOGD("[memory]flv_parse_script: Alloc tag_data 0x%08x \n", pMeta->tag_data);
            ...
            if (tmp < pMeta->tag_header.tag_data_size) {
                ret = FLV_FILE_READ_ERR;
                LOGE("flv_parse_script: error4,read failed \n");
                break;
            }
            ...
            if (pMeta->tag_data) {
                LOGD("[memory]flv_parse_script: Free 0x%08x \n", pMeta->tag_data);
                free(pMeta->tag_data);
                pMeta->tag_data = NULL;
            }
        } else if (mfile->hasMeta == 1) {
            mfile->mIoStream.seek(mfile->mIoStream.source,
                                  pMeta->tag_header.tag_data_size, FLV_SEEK_FROM_CUR);
            LOGD("flv_parse_script: has parserd Meta, not parser this script isze=%d\n",
        ...
        if (tmp < FLV_TAG_PREV_SIZE) {
            LOGE("flv_parse_script(prev_tag): error read file,tmp=%d\n", tmp);
            return FLV_FILE_READ_ERR;
        }
        ...
        if (pMeta->tag_header.prv_tag_size != pMeta->tag_header.tag_data_size + FLV_TAG_HEADER_SIZE) {
            LOGE("flv_parse_script tag size err, file offset=0xllx\n", mfile->cur_file_offset);
        }
        //-- verify tag size end
        mfile->cur_file_offset =
            mfile->cur_file_offset + pMeta->tag_header.tag_data_size +
    ...
    if (ret != FLV_OK) {
        LOGE("flv_parse_script: error5\n");
        return FLV_ERROR;
    }
    mfile->duration = (uint64_t) mfile->mMeta->duration;    //ms
    mfile->file_size = (uint64_t) mfile->mMeta->file_size;

    //LOGD("flv_parse_script:  mfile->cur_file_offset=%lld \n", mfile->cur_file_offset);
    LOGD("flv_parse_script:  mfile->duration=%lld ,mfile->file_size=%lld\n",
...
FLV_ERROR_TYPE flvParser::flv_parse_amf_obj(uint8_t * amf_data,
                                            uint32_t amf_data_len,
                                            uint32_t * offset, char *key,
    ...
    *offset = *offset + 1;
    ...
    LOGV("FLV_AMF: amf_type =%d,depth=%d,key=%s,*offset=%d,amf_data_len=%d\n",
    ...
    case FLV_AMF_V0_TYPE_OBJECT:
    {
        ...
        uint32_t end;
        LOGD("FLV_AMF:  ========START BJECT key =%s ========\n", key);
        //if(depth==1 && key && (FLV_AMF_V0_TYPE_OBJECT == amf_type && (0 == strncmp(key, "keyframes", 9))))
        if (depth == 1 && key
            && (FLV_AMF_V0_TYPE_OBJECT == amf_type && (0 == strcmp(key, "keyframes"))))
            LOGD("AMF: file has seek table info\n");

        while (*offset < amf_data_len) {
            LOGD("FLV_AMF: IN OBJECT: key =%s,*offset=%d\n", strVal, *offset);
            strsize = flv_byteio_get_2byte(amf_data + (*offset));
            (*offset) = (*offset) + 2;
            if (strsize == 0) {
                ...
                (*offset) = (*offset) + 1;
                if (end == 9) {
                    LOGD(" FLV_AMF:  ========EXIT OBJECT key =%s ===*offset=%d=====\n", key, *offset);
                    break;
                }
            }
            flv_byteio_get_string((uint8_t *) strVal, strsize + 1, amf_data + (*offset));
    ...
    case FLV_AMF_V0_TYPE_MIXED_ARRAY:
    {
        ...
        LOGD("FLV_AMF:  ========START MIXED_ARRAY size =%d ========\n", array_size);
        while (*offset < amf_data_len) {
            LOGD("FLV_AMF: IN MIXED_ARRAY key =%s,*offset=%d\n", strVal, *offset);
            strsize = flv_byteio_get_2byte(amf_data + (*offset));
            (*offset) = (*offset) + 2;
            if (strsize == 0) {
                end = flv_byteio_get_byte(amf_data + (*offset));
                (*offset) = (*offset) + 1;
                if (end == 9) {
                    LOGD(" FLV_AMF:  ========EXIT MIXED_ARRAY key =%s ===*offset=%d=====\n", key, *offset);
                    break;
                }
            }
    ...
    if (depth == 1 && key
        && (FLV_AMF_V0_TYPE_NUMBER == amf_type || FLV_AMF_V0_TYPE_BOOL == amf_type)) {
    ...
    } else if (depth == 3 && key && (0 == strcmp(key, "filepositions"))
               && FLV_AMF_V0_TYPE_NUMBER == amf_type) {
        metaInfo->filepositions[metaInfo->fileposidx++] = (uint64_t) numVal;
    } else if (depth == 3 && key && (0 == strcmp(key, "times"))
               && FLV_AMF_V0_TYPE_NUMBER == amf_type) {
...
FLV_ERROR_TYPE flvParser::flv_parse_onMetaData(flv_tag_str * pMeta_tag, flv_meta_str * metaInfo) {
    ...
    if (type != FLV_AMF_V0_TYPE_STRING || 0 != strncmp(buffer, "onMetaData", 10)) {
        LOGE("flv_parse_meta_amf: error1 type=%d,%s\n", type, buffer);
        return FLV_ERROR;
    }
    offset = offset + 10;
    ...
    ret =
        flv_parse_amf_obj(pMeta_tag->tag_data,
                          pMeta_tag->tag_header.tag_data_size, &offset, "NULL", metaInfo, 0);
    if (ret == FLV_OK) {
        mfile->hasMeta = 1;
        LOGV("flv_parse_onMetaData:metaInfo:audio_codec_id=%f,video_codec_id=%f\n",
             metaInfo->audio_codec_id, metaInfo->video_codec_id);
        LOGV("flv_parse_onMetaData:metaInfo:duration=%f,file_size=%f\n",
             metaInfo->duration, metaInfo->file_size);
        LOGV("flv_parse_onMetaData:metaInfo:width=%f,height=%f\n",
             metaInfo->width, metaInfo->height);
        LOGV("flv_parse_onMetaData:metaInfo:frame_rate=%f,can_seek_to_end=%d\n",
...
FLV_ERROR_TYPE flvParser::flv_read_tag_header(flv_tag_header_info * tag_header) {
    ...
    if (tmp < read_size) {
        LOGE("flv_read_tag_header: error read file,tmp=%d\n", tmp);
        return FLV_FILE_READ_ERR;
    }
    ...
    LOGV("flv_read_tag_header:prv_tag_size=%d,tag_type=%d,tag_data_size=%d,tag_ts=%d\n",
...
FLV_ERROR_TYPE flvParser::flv_parse_header() {
    ...
    tmp = flv_byteio_read(data, FLV_TAG_PREV_SIZE, &mfile->mIoStream);
    if (tmp < FLV_TAG_PREV_SIZE) {
        LOGE("flv_parse_header(prev tag): error read file,tmp=%d\n", tmp);
        return FLV_FILE_READ_ERR;
    }

    mfile->cur_file_offset = mfile->header_size + FLV_TAG_PREV_SIZE;

    LOGD("flv_parse_header: version=%d,hasVideo=%d, hasAudio=%d,header size =%d\n",
...
flv_tag_str *flvParser::flv_tag_create() {
    ...
    if (!tag) {
        LOGE("flv_tag_create: error1, alloc mem fail\n");
        return NULL;
    }
    LOGM("[memory] alloc 0x%08x\n", tag);
    ...
    if (!ptr) {
        LOGE("flv_tag_create: error2, alloc mem fail\n");
        return NULL;
    }
    LOGM("[memory] alloc 0x%08x\n", ptr);
    tag->tag_data = ptr;
...
FLV_ERROR_TYPE flvParser::flv_read_a_tag(flv_tag_str * tag) {
    FLV_ERROR_TYPE ret;
    uint8_t *ptr = NULL;
    LOGV("flv_read_a_tag IN: file offset=0x%llx\n", mfile->cur_file_offset);

    if (!tag) {
        LOGE("flv_read_a_tag: error input is NULL\n");
        return FLV_ERROR;
    }
  ...
  READ_TAG_HEADER:
    ...
    if (ret != FLV_OK) {
        LOGE("flv_read_a_tag: error,read header is NULL\n");
        return ret;
    }
    ...
    if (tag->tag_header.tag_data_size > FLV_BS_BUFFER_SIZE) {
        ...
        if (!ptr) {
            LOGE("flv_read_a_tag: error, alloc mem fail\n");
            return FLV_ERROR;
        }
        LOGM("[memory] free 0x%08x\n", tag->tag_data);
        LOGM("[memory] alloc 0x%08x\n", ptr);
    ...
    if (tmp < 0) {
        LOGE("flv_read_a_tag: error read file,tmp=%d\n", tmp);
        return FLV_FILE_READ_ERR;
    }
    ...
    tmp = flv_byteio_read(data, FLV_TAG_PREV_SIZE, &mfile->mIoStream);
    if (tmp < FLV_TAG_PREV_SIZE) {
        LOGE("flv_read_a_tag(prev_tag): error read file,tmp=%d\n", tmp);
        return FLV_FILE_READ_ERR;
    }
    //verify tag size
    tag->tag_header.prv_tag_size = flv_byteio_get_4byte(data);
    if (tag->tag_header.prv_tag_size == 0) {
        LOGE("size is full of zero, need stop");
        return FLV_FILE_EOF;
    }
    if ((tag->tag_header.prv_tag_size != tag->tag_header.tag_data_size + FLV_TAG_HEADER_SIZE)
        ...
        LOGE("flv tag size err, file offset=0x%llx\n", mfile->cur_file_offset);
        ...
        //update file offset
        u8FileOffset =
            mfile->cur_file_offset + FLV_TAG_HEADER_SIZE +
            FLV_TAG_PREV_SIZE + tag->tag_header.tag_data_size;
        while (pu1DataBuf && mfile->hasVideo && u8SearchOffsetAcc < 0x1400000) {
            ...
            if (tmp < 0) {
                LOGE("flv_read_a_tag(findNextI): read to file end\n");
                free(pu1DataBuf);
                pu1DataBuf = NULL;
                return FLV_FILE_READ_ERR;
            }
            ...
            if (u8SearchOffset < tmp) {
                mfile->cur_file_offset =
                    u8FileOffset + u8SearchOffsetAcc + u8SearchOffset + FLV_TAG_PREV_SIZE;
                mfile->mIoStream.seek(mfile->mIoStream.source,
                                      mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                free(pu1DataBuf);
                pu1DataBuf = NULL;
        ...
        if (pu1DataBuf) {
            ...
            LOGE("flv_read_a_tag(alloc fail)\n");
        }
#endif
    }
    //-- verify tag size end
    ...
    //update file offset
    mfile->cur_file_offset =
        mfile->cur_file_offset + FLV_TAG_HEADER_SIZE + FLV_TAG_PREV_SIZE +
        tag->tag_header.tag_data_size;
    LOGV("flv_read_a_tag OUT: file offset=0x%llx\n", mfile->cur_file_offset);
    return FLV_OK;
}
...
void flvParser::flv_dump_seektable() {
    ...
    LOGD("-----flv_dump_seektable---\n");
    for (i = 0; i < mSeekTable->SetEntries; i++)
        LOGD("-----entry %d  ts=%lld  offset=%lld---\n", i,
                       pEntry[i].ulTime, pEntry[i].ulOffset);
}

int64_t flvParser::flv_seek_to_msec(int64_t msec) {
    int64_t newTs;
    uint32_t i;
    LOGD("flv_seek_to_msec: seekto %lld ms", msec);
    ...
    if (msec <= 0) {
        ...
        bUpdateSeekTable = true;
#endif
        LOGD("flv_seek_to_msec: seet to 0, mfile->cur_file_offset=0x%08x\n",
              mfile->cur_file_offset);
        return 0;
    } else if (mfile->hasSeekTable) {
        LOGD("flv_seek_to_msec: hasSeekTable path\n");
        flv_seek_table_entry *pEntry = (flv_seek_table_entry *) mSeekTable->pEntry;
#ifdef FLV_DIRECT_SEEK_SUPPORT
        bUpdateSeekTable = true;
#endif
        //flv_dump_seektable();
        for (i = 0; i < mSeekTable->MaxEntries; i++) {
            LOGV("flv_seek_to_msec1: msec =%lld pEntry[%d].ulTime=%lld, pEntry[%d].ulTime=%lld \n",
        ...
        if (i < mSeekTable->MaxEntries) {
            ...
            mfile->mIoStream.seek(mfile->mIoStream.source,
                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            LOGD("flv_seek_to_msec: return 1 %d newTs=%lld, offset=0x%llx",
                  i, newTs, mfile->cur_file_offset);
            return newTs;
        } else {
            ...
            mfile->mIoStream.seek(mfile->mIoStream.source,
                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            LOGD("flv_seek_to_msec: return 2 %d newTs=%lld, offset=0x%llx",
                           i, newTs, mfile->cur_file_offset);
            return newTs;
        }
    } else if (mfile->hasVideo) {
        LOGD("flv_seek_to_msec: NOT hasSeekTable path\n");
        flv_seek_table_entry *pEntry = (flv_seek_table_entry *) mSeekTable->pEntry;

        //1. search from the exist table
        for (i = 0; i < mSeekTable->SetEntries; i++) {
            LOGV("flv_seek_to_msec2: msec =%lld pEntry[%d].ulTime=%lld, pEntry[%d].ulTime=%lld \n",
                  msec, i, pEntry[i].ulTime, i + 1, pEntry[i + 1].ulTime);
        ...
        if (i < mSeekTable->SetEntries) {
            ...
            mfile->mIoStream.seek(mfile->mIoStream.source,
                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            LOGD("flv_seek_to_msec: return 3 %d newTs=%lld, offset=0x%llx",
          i, newTs, mfile->cur_file_offset);
            return newTs;
        } else {
            if (mSeekTable->SetEntries == mSeekTable->MaxEntries) {
                ...
                mfile->mIoStream.seek(mfile->mIoStream.source,
                                      mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                LOGD("flv_seek_to_msec: return 4 %d newTs=%lld, offset=0x%llx",
                     i, newTs, mfile->cur_file_offset);
                return newTs;
            } else {
                flv_tag_str tag;
                ...
                if ((mSeekTable->SetEntries != 0)) {
                    ...
                    LOGD("flv_seek_to_msec: jump to offset=0x%llx first", mfile->cur_file_offset);
                } else {
                    ts = 0;
                    mfile->cur_file_offset = mfile->data_tag_position;
                    mfile->mIoStream.seek(mfile->mIoStream.source,
                ...
                LOGD("flv_seek_to_msec: time diff = 0x%llx", (msec - ts));
                ...
                if (ret != FLV_OK) {
                    ...
                    if ((mSeekTable->SetEntries != 0)) {
                        ...
                        mfile->mIoStream.seek(mfile->mIoStream.source,
                                              mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        ts = pEntry[mSeekTable->SetEntries - 1].ulTime;
                        LOGD("flv_seek_to_msec: jump to offset=0x%llx first",
                              mfile->cur_file_offset);
                    } else {
                        mfile->cur_file_offset = mfile->data_tag_position;
                        mfile->mIoStream.seek(mfile->mIoStream.source,
                    ...
                    LOGM("[memory]flv_seek_to_msec alloc 0x%08x\n", ptr);
                    while (1) {
                        ret = flv_read_a_tag(&tag);
                        if (ret == FLV_FILE_EOF) {
                            LOGE("fatal error in bitstream, must eos");
                            if (tag.tag_data) {
                                LOGM
                                    ("[memory]flv_seek_to_msec free 0x%08x\n", tag->tag_data);
                                free(tag.tag_data);
                                tag.tag_data = NULL;
                            }
                            return -10;
                            ...
                            ts = pEntry[mSeekTable->SetEntries - 1].ulTime;
                            if (ts >= msec) {
                                LOGD("flv_seek_to_msec: find the tag: ts= %lld, entry=%d\n",
                                      ts, mSeekTable->SetEntries);
                                break;
                            }
                        } else if (ret == FLV_FILE_READ_ERR) {
                            LOGE("flv_seek_to_msec:EOS!!\n");
                            //flv_dump_seektable();
                            break;
                        } else if (ret == FLV_ERROR) {
                            LOGE("flv_seek_to_msec:Error!!\n");
                            break;
                        }
                    }

                    if (tag.tag_data) {
                        LOGM("[memory]flv_seek_to_msec free 0x%08x\n", tag->tag_data);
                        free(tag.tag_data);
                        tag.tag_data = NULL;
                    }
                    //flv_dump_seektable();
                    if (mSeekTable->SetEntries == 0 && ret != FLV_OK) {
                        ...
                        mfile->mIoStream.seek(mfile->mIoStream.source,
                                              mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        LOGE("flv_seek_to_msec: return 5:entry %d newTs=%lld, offset=0x%llx",
                             i, newTs, mfile->cur_file_offset);
                    } else {
                        //re-search table after setup more
                        for (i = 0; i < mSeekTable->SetEntries; i++) {
                            LOGV("flv_seek_to_msec 6: msec =%lld pEntry[%d].ulTime=%lld, pEntry[%d].ulTime=%lld \n",
                                msec, i, pEntry[i].ulTime, i + 1, pEntry[i + 1].ulTime);
                            if (msec >= pEntry[i].ulTime && msec < pEntry[i + 1].ulTime)    //concern : the ts gap between 2 entry should > 1/fps
                                break;
                        }
                        if (i < mSeekTable->SetEntries) {
                            ...
                            mfile->mIoStream.seek(mfile->mIoStream.source,
                                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                            LOGD("flv_seek_to_msec: return 7 %d newTs=%lld, offset=0x%llx",
                                 i, newTs, mfile->cur_file_offset);
                        } else {
                            ...
                            mfile->mIoStream.seek(mfile->mIoStream.source,
                                                  mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                            LOGD("flv_seek_to_msec: return 8 entry %d newTs=%lld, offset=0x%llx",
                                i, newTs, mfile->cur_file_offset);
                        }
                    }
        ...
        mfile->cur_file_offset = mfile->data_tag_position;
        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
        LOGD("flv_seek_to_msec: set to 0, mfile->cur_file_offset=0x%08x\n", mfile->cur_file_offset);
		
        return 0;
    }
    LOGD("flv_seek_to_msec: return 9 %d newTs=%lld, offset=%lld",
        i, newTs, mfile->cur_file_offset);
    return msec;
}

#ifdef FLV_DIRECT_SEEK_SUPPORT
FLV_ERROR_TYPE flvParser::flv_direct_seek_to_msec(int64_t Trgmsec,
                                                  int64_t Currmsec, int64_t * Retmsec) {
    uint32_t cnt = 0;
    int32_t read_size = 0;
    ...
    if ((mfile->duration == 0) || (mfile->file_size == 0)) {
        LOGD("flv_direct_seek_to_msec : duration or file_size is zero\n");
        return FLV_ERROR;
    }
    ...
    file_offset =
        mfile->cur_file_offset +
        ((Trgmsec - Currmsec - FLV_DIRECT_SEEK_THD) * (mfile->file_size) / mfile->duration);
    ...
    while (1) {
        ...
        LOGD("flv_direct_seek_to_msec : jump to 0x%llx/0x%llx/0x%llx\n",
        ...
        while (1) {
            ...
            if (buf_offset < read_size) {
                ...
                if (tag_ts > Trgmsec) {
                    LOGD("flv_direct_seek_to_msec : find time forward = %lld\n", tag_ts);
                    forward = false;
                    ts_diff = tag_ts - Trgmsec;
                    if ((file_offset < fwd_offset) || (fwd_offset == 0))
                        fwd_offset = file_offset;
                } else {
                    LOGD("flv_direct_seek_to_msec : find time behind = %lld\n", tag_ts);
                ...
                LOGD("flv_direct_seek_to_msec : read to file end\n");
                forward = false;
    ...
    if (tag_data) {
        LOGM("[memory]flv_direct_seek_to_msec free 0x%08x\n", tag_data);
        free(tag_data);
        tag_data = NULL;
    }
    LOGD("flv_direct_seek_to_msec : final offset = 0x%llx\n", mfile->cur_file_offset);
    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
...
uint32_t flvParser::flv_search_tag_pattern(uint8_t ** data, uint32_t size) {
    ...
    if (0 == size) {
        LOGD("flv_search_tag_pattern : size=0\n");
        return 0;
    }
    do {
        do {
        ...
        if ((start[i] == 0x17)
            && (((start[i + 1] == 0x00) && (start[i + 2] == 0x00)
                 && (start[i + 3] == 0x00) && (start[i + 4] == 0x00))
                || (start[i + 1] == 0x01))) {
    ...
    LOGD("flv_search_tag_pattern : find offset = 0x%x\n", offset);
    return offset;
}
#endif
...
uint32_t flvParser::flv_search_video_tag_pattern(uint8_t * data, uint32_t size) {
    ...
    LOGV("flv_search_video_tag_pattern : start=%d\n", start[0]);
    if (0 == size) {
        LOGD("flv_search_video_tag_pattern : size=0\n");
        return 0;
    }
    do {
        do {
        ...
        while (((zerocnt < 3) || (start[i] == 0x00)) && (i < (size - 5)));
        //LOGD("flv_search_video_tag_pattern : zerocnt=%d, start[i]=%d\n", zerocnt, start[i]);

        if ((zerocnt >= 3) && ((start[i] & 0xF0) >= 0x10) && ((start[i] & 0xF0) <= 0x50) && //Frame Type
            ...
            ((((start[i] & 0x0F) != 0x07) && ((start[i] & 0x0F) != 0x012)) ||   //neither AVC and HEVC video
             (((start[i + 1] == 0x00) && (start[i + 2] == 0x00) && (start[i + 3] == 0x00) && (start[i + 4] == 0x00)) || //AVC video with sequence header
              (start[i + 1] == 0x01)))) {
            //LOGD("flv_search_video_tag_pattern : i=%d, zerocnt=%d, start[i]=%d\n", i, zerocnt, start[i]);
            if ((i >= 15) && ((start[i - 11] & 0x1F) == 0x09)) {
                ...
                if ((i + tag_data_size + FLV_TAG_HEADER_SIZE) < size) {
                    ...
                    //LOGD("tag_data_size 0x%x, prv_tag_size 0x%x\n", tag_data_size, prv_tag_size);
    ...
    LOGV("flv_search_video_tag_pattern : find offset = 0x%x\n", offset);
    return offset;
}
...
uint32_t flvParser::flv_search_audio_tag_pattern(uint8_t * data, uint32_t size) {
    ...
    LOGV("flv_search_audio_tag_pattern : start=%d\n", start[0]);
    if (0 == size) {
        LOGD("flv_search_audio_tag_pattern : size=0\n");
        return 0;
    }
    do {
        do {
        ...
        //LOGD("flv_search_audio_tag_pattern : zerocnt=%d, start[i]=%d\n", zerocnt, start[i]);
        if ((zerocnt >= 3) && ((start[i] & 0xF0) != 0x07) &&    //sound format
            ((start[i] & 0xF0) != 0x08) &&
            ((start[i] & 0xF0) != 0x0e) && ((start[i] & 0xF0) != 0x0f)) {
            //LOGD("flv_search_audio_tag_pattern : i=%d, zerocnt=%d, start[i]=%d\n", i, zerocnt, start[i]);
    ...
    LOGV("flv_search_audio_tag_pattern : find offset = 0x%x\n", offset);
    return offset;
}
...
bool flvParser::flv_is_seekable() {
    if (!mfile) {
        LOGE("flv_is_seekable: !mfile ,can not seek\n");
        return false;
    } else if (mfile->hasSeekTable) {
        return true;
        LOGD("flv_is_seekable: hasSeekTable can seek\n");
    } else if (mfile->hasVideo) {
        LOGD("flv_is_seekable: hasVideo tag ,can seek\n");
        return true;
    } else {
        LOGD("flv_is_seekable: !hasVideo && !hasSeekTabletag ,can not seek\n");
        return false;
    }
    return false;
}
...
uint64_t flvParser::flv_get_file_size() {
    ...
    LOGD("flv_get_file_size: error return 0\n");
    return 0;
}
...
void flvParser::flv_set_file_size(uint64_t file_size) {
    ...
    if (mfile && ((0 == mfile->file_size) || (file_size != mfile->file_size))) {
        if (file_size != mfile->file_size) {
            ...
            int32_t read_size = 0;
            LOGD("flv_set_file_size: file size mismatch,meta/real 0x%x/0x%x\n",
                 mfile->file_size, file_size);
            tag_data = (uint8_t *) malloc(FLV_BS_BUFFER_SIZE * sizeof(uint8_t));
            file_offset =
                (file_size >
                 FLV_BS_BUFFER_SIZE) ? (file_size - FLV_BS_BUFFER_SIZE) : mfile->cur_file_offset;
            ...
            if (mfile->hasVideo) {
                ...
                read_size = flv_byteio_read(tag_data, FLV_BS_BUFFER_SIZE, &mfile->mIoStream);
                if (read_size < 0) {
                    LOGE("[Video]read size out of range, force to return");
                    if (tag_data) {
                        free(tag_data);
                        tag_data = NULL;
                    }
                ...
                LOGD("flv_set_file_size: start = %d\n", tag_data[0]);
                do {
                    buf_offset =
                        flv_search_video_tag_pattern(&tag_data
                                                     [total_offset], (read_size - total_offset));
                    total_offset += buf_offset;
                    LOGV("flv_set_file_size: video total_offset = %d\n", total_offset);
                }
                ...
                if ((0 != total_offset) && (total_offset >= 7)) {
                    ...
                    u8VideoDuration |= (flv_byteio_get_byte(&tag_data[total_offset - 4])
                                        << 24);
                }
            }
            //find audio tag
            if (mfile->hasAudio) {
                ...
                read_size = flv_byteio_read(tag_data, FLV_BS_BUFFER_SIZE, &mfile->mIoStream);
                if (read_size < 0) {
                    LOGE("[Audio]read size out of range, force to return");
                    if (tag_data) {
                        free(tag_data);
                        tag_data = NULL;
                    }
                ...
                do {
                    buf_offset =
                        flv_search_audio_tag_pattern(&tag_data
                                                     [total_offset], (read_size - total_offset));
                    total_offset += buf_offset;
                    LOGV("flv_set_file_size: audio total_offset = %d\n", total_offset);
                }
                while ((0 != buf_offset) && (total_offset < read_size));

                if ((0 != total_offset) && (total_offset >= 7)) {
                    u8AudioDuration = flv_byteio_get_3byte(&tag_data[total_offset - 7]);
                    u8AudioDuration |= (flv_byteio_get_byte(&tag_data[total_offset - 4])
                                        << 24);
                }
            }
            ...
            if (u8VideoDuration > 0 || u8AudioDuration > 0) {
                LOGD("flv_set_file_size: old duration/new vid/aud = %lld/%lld/%lld\n",
            ...
            if (tag_data) {
                LOGM("[memory]flv_set_file_size free 0x%08x\n", tag_data);
                free(tag_data);
                tag_data = NULL;
            }
        }
...
uint64_t flvParser::flv_get_duration() {
    ...
    LOGD("flv_get_duration: error return 0\n");
    return 0;
}
...
bool flvParser::flv_has_video() {
    ...
    LOGD("flv_has_video: error return false\n");
    return false;
}
...
bool flvParser::flv_has_audio() {
    ...
    LOGD("flv_has_audio: error return false\n");
    return false;
}
...
FLV_VIDEO_CODEC_ID flvParser::flv_get_videocodecid() {
    ...
    LOGD("flv_get_videocodecid: error return FLV_VIDEO_CODEC_ID_UNKHNOWN\n");
...
FLV_AUDIO_CODEC_ID flvParser::flv_get_audiocodecid() {
    ...
    LOGD("flv_get_audiocodecid: error return FLV_AUDIO_CODEC_ID_UNKHNOWN\n");
...
void flvParser::flv_get_resolution(uint32_t * width, uint32_t * height) {
    if (mfile && mfile->mMeta) {
        ...
        *height = (uint32_t) mfile->mMeta->height;
        ...
        LOGD("flv_get_resolution: error return 0\n");
        *width = 0;
        *height = 0;
    }
}
...
flv_meta_str *flvParser::flv_get_meta() {
    if (mfile && mfile->hasMeta) {
        ...
        LOGD("flv_get_meta: error return 0\n");
        return NULL;
    }
}
...
int64_t flvParser::flv_update_seek_table(flv_tag_str * cur_tag) {
    ...
    if (mfile->hasSeekTable) {
        LOGV("flv_update_seek_table: have ssektable already,not update\n");
        return 1;
    }

    if (mSeekTable->SetEntries == mSeekTable->MaxEntries) {
        LOGV("flv_update_seek_table: don't update table as entries full\n");
        return -1;
    }
#ifdef FLV_DIRECT_SEEK_SUPPORT
    if (!bUpdateSeekTable) {
        LOGV("flv_update_seek_table: don't update table because bUpdateSeekTable is false\n");
        return -1;
    }
#endif

    if (!cur_tag || cur_tag->tag_header.tag_type != FLV_TAG_TYPE_VIDEO) {
        LOGV("flv_update_seek_table: don't update table,as not video tag\n");
        return -1;
    }
    ...
    if ((check_byte & FLV_VIDEO_FRAME_TYPE_BITMASK) != FLV_VIDEO_FRAME_TYPE_KEY) {
        LOGV("flv_update_seek_table: don't update table,as not video key tag\n");
        return -1;
    }

    if (mSeekTable->SetEntries > 0
        && tag_ts < (mSeekTable->RangeTime + mSeekTable->TimeGranularity)) {
        LOGV("flv_update_seek_table:don't update table as cur ts=%lld < gap %lld\n",
                       tag_ts, (mSeekTable->RangeTime + mSeekTable->TimeGranularity));
        return -1;
    }
    ...
    LOGM("flv_update_seek_table: update %d entry,ts=%lld,Offset=0x%llx\n",
...
uint8_t flv_byteio_get_byte(uint8_t * data) {
    if (data) {
        ...
        LOGE("flv_byteio_get_byte error\n");
        return FLV_ERROR;
    }
}
...
void flv_byteio_get_string(uint8_t * string, uint32_t strlen, uint8_t * data) {
    ...
    char c;
    if (strlen >= 256) {
        LOGE("flv_byteio_get_string:  error strlen=%d\n", strlen);
        return;
    }
...
int32_t flv_byteio_read(uint8_t * Out, uint32_t size, flv_iostream_str * iostream) {
    ...
    if (tmp != size)
        LOGE("flv_byteio_read error: read %d,need read %d\n", tmp, size);
...
double flv_amf_number2double(uint64_t number) {
    if ((number + number) > 0xFFEULL << 52)
        return 0.0 / 0.0;
    return ldexp(((number & ((1LL << 52) - 1)) + (1LL << 52)) *
                 (number >> 63 | 1), (number >> 52 & 0x7FF) - 1075);
}
...
bool isSupportedStream(Type stream_type, uint32_t codec_id, sp < MetaData > StreamMeta) {
    if (stream_type == VIDEO) {
        ...
        default:
        {
            ALOGE("[FLV capability error]Unsupport video format!!!mStreamType=0x%x ", codec_id);
            return false;
        }
        }
...
void flv_parse_s263_info(uint8_t * data, uint32_t * width, uint32_t * height, uint32_t * key_frame) {
    ...
    if (flv_util_show_bits(data, bitcnt, 22) != FLV_START_CODE_0 &&
        ...
        LOGE("flv_parse_s263_info: FLV_START_CODE_0(0x020,0x021) not found!!");
        LOGE("[FLV Playback capability Error] capability not support as :Un-support S263 source_forma\n");
        return;
    }
    ...
    default:
    {
        LOGE("[source_format is not support!!");
        LOGE("[FLV Playback capability Error] capability not support as :Un-support S263 source_forma\n");
        return;
    }
    }
    ...
    *height = (*height + 15) & 0xFFFFFFF0;
    ...
    if (flv_util_show_bits(data, bitcnt, 2) == 0) {
        ...
        *key_frame = P_VOP;
        ...
        LOGE("FLV vop_coding_type is not support!!");
        return;
    }
    /*
...
static bool get_audio_tage_header_info(uint8_t header,
                                       int *out_format = NULL,
                                       int *out_sampling_rate = NULL,
                                       int *out_sampling_size = NULL, int *out_channels = NULL) {
    uint8_t tmp = 0;
...
static bool get_mp3_frame_size(uint32_t header, uint32_t * frame_size,
                               int *out_sampling_rate =
                               NULL, int *out_channels = NULL, int *out_bitrate = NULL) {
    *frame_size = 0;
    ...
    if ((header & 0xffe00000) != 0xffe00000) {
        LOGD("get_mp3_frame_size error1\n");
        return false;
    }

    unsigned version = (header >> 19) & 3;
    if (version == 0x01) {
        LOGD("get_mp3_frame_size error2\n");
        return false;
    }

    unsigned layer = (header >> 17) & 3;
    if (layer == 0x00) {
        LOGD("get_mp3_frame_size error3\n");
        return false;
    }
    ...
    if (bitrate_index == 0 || bitrate_index == 0x0f) {
        ...
        LOGD("get_mp3_frame_size error4\n");
        return false;
    }

    unsigned sampling_rate_index = (header >> 10) & 3;
    if (sampling_rate_index == 3) {
        LOGD("get_mp3_frame_size error5\n");
        return false;
    }
...
uint32_t flv_io_read_func_ptr(void *pFlvExtractor, void *aBuffer, uint32_t aSize) {
    ...
    LOGV("flv_io_read_func_ptr: iFlvParserReadOffset=%ld,aSize=%d\n",
                   _pExtractor->iFlvParserReadOffset, aSize);
    if (_pExtractor) {
        int32_t bytesRead = _pExtractor->mDataSource->readAt(_pExtractor->iFlvParserReadOffset,
                                                             aBuffer, aSize);
        _pExtractor->iFlvParserReadOffset += bytesRead;
        return bytesRead;
    }
    LOGE("flv_io_read_func_ptr:retrun 0\n");
    return 0;
}
...
uint64_t flv_io_seek_func_ptr(void *pFlvExtractor, uint64_t aOffset, FLV_SEEK_FLAG flag) {
    ...
    LOGV("flv_io_seek_func_ptr: iFlvParserReadOffset=%lld\n",
                   _pExtractor->iFlvParserReadOffset);
    return _pExtractor->iFlvParserReadOffset;
}

FLVSource::FLVSource(const sp < FLVExtractor > &extractor,
    ...
    if ((!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC))
        ...
        mType = AUDIO;
        LOGD("New FLVSource:mType = AUDIO\n");
    } else {
        mType = OTHER;
        LOGD("New FLVSource:mType = OTHER\n");
    }
}
...
status_t FLVSource::read(MediaBuffer ** out, const ReadOptions * options) {
    ...
    LOGV("+FLVSource::read use mode, seek to %lld us (mType=%d, mode=%d)", seekTimeUs, mType, mode);
    ...
    if (AUDIO == mType) {
        LOGV("FLVSource::read mAudioFrames.size()=%d", mExtractor->mAudioFrames.size());
            ...
            *out = mExtractor->DequeueAudioFrame(mExtractor->mtargetSampleTimeUs);
            ...
            if (FLV_FILE_EOF == retVal) {
                if (mExtractor->mAudioFrames.size() == 1) {
                    *out = mExtractor->DequeueAudioFrame(mExtractor->mtargetSampleTimeUs);
                    LOGD("FLVSource read AUDIO buffer3=0x%08x\n", *out);
                    return OK;
                }

                if (mExtractor->mAudioFrames.isEmpty()) {
                    // send EOS
                    LOGE("FLV_FILE_EOF (AUDIO)1");
                    return ERROR_END_OF_STREAM;
                }
            } else if (FLV_FILE_READ_ERR == retVal) {
                return ERROR_IO;
                ...
                if (mExtractor->mAudioFrames.size() >= 1) {
                    *out = mExtractor->DequeueAudioFrame(mExtractor->mtargetSampleTimeUs);
                    LOGV("FLVSource read AUDIO buffer4=0x%08x\n", *out);
                    return OK;
                }

                if (mExtractor->mAudioFrames.isEmpty()) {
                    // send EOS
                    LOGE("FLV_FILE_EOF (AUDIO)2\n");
                    return ERROR_END_OF_STREAM;
                }
            }                   // initial
        }
...
#endif
    if (VIDEO == mType) {
        LOGD("FLVSource::read mVideoFrames.size()=%d", mExtractor->mVideoFrames.size());

        if (mExtractor->mVideoFrames.size() >= FLV_CACHE_POOL_HIGH) {   // dequeue
            *out = mExtractor->DequeueVideoFrame(mExtractor->mtargetSampleTimeUs, mWantsNALFragments);
            (*out)->meta_data()->findInt64(kKeyTime, &cur_ts);
            LOGV("FLVSource read VIDEO buffer1=0x%08x,ts= %lld\n", *out, (cur_ts / 1000));
#if defined(ENABLE_PERF_JUMP_KEY_MECHANISM)
            if (bJumpToKey == true) {
                int32_t keyFrame;
                CHECK((*out)->meta_data()->findInt32(kKeyIsSyncFrame, &keyFrame));
                if (!keyFrame) {
                    LOGV("###FLVSource::read mVideoFrames bJumpToKey1 :this is not Key Frame=======\n");
                    goto READ_AGAIN;
                }
                bJumpToKey = false;
                LOGD("###FLVSource::read mVideoFrames bJumpToKey1: find KEY!!!");
            }
#endif
            return OK;
            ...
            retVal = mExtractor->CacheMore(CACHE_FRAME);
            if (FLV_FILE_EOF == retVal) {
                LOGE("FLVSource read: VIDEO CacheMore EOS,BUT still queued %d frames \n",
                               mExtractor->mVideoFrames.size());
                return ERROR_END_OF_STREAM; //will lost several frames
            } else if (FLV_FILE_READ_ERR == retVal) {
                LOGE("FLVSource read:VIDEO CacheMore FLV_FILE_READ_ERR\n");
                return ERROR_IO;
            }
            *out = mExtractor->DequeueVideoFrame(mExtractor->mtargetSampleTimeUs, mWantsNALFragments);
            (*out)->meta_data()->findInt64(kKeyTime, &cur_ts);
            LOGV("FLVSource read VIDEO buffer2=0x%08x,ts= %lld\n", *out, (cur_ts / 1000));
#if defined(ENABLE_PERF_JUMP_KEY_MECHANISM)
            if (bJumpToKey == true) {
                int32_t keyFrame;
                CHECK((*out)->meta_data()->findInt32(kKeyIsSyncFrame, &keyFrame));
                if (!keyFrame) {
                    LOGV("###FLVSource::read mVideoFrames bJumpToKey2 :this is not Key Frame\n");
                    goto READ_AGAIN;
                }
                bJumpToKey = false;
                LOGD("###FLVSource::read mVideoFrames bJumpToKey2: find KEY!!!");
            }
#endif
            return OK;
        } else {
            ...
            if (FLV_FILE_EOF == retVal) {
                if (mExtractor->mVideoFrames.size() == 1) {
                    *out = mExtractor->DequeueVideoFrame(mExtractor->mtargetSampleTimeUs, mWantsNALFragments);
                    (*out)->meta_data()->findInt64(kKeyTime, &cur_ts);
                    LOGV("FLVSource read VIDEO buffer3=0x%08x,ts= %lld\n", *out, (cur_ts / 1000));
                    return OK;
                }

                if (mExtractor->mVideoFrames.isEmpty()) {
                    // send EOS
                    LOGE("FLV_FILE_EOF (VIDEO)1");
                    return ERROR_END_OF_STREAM;
                }
            } else if (FLV_FILE_READ_ERR == retVal) {
                return ERROR_IO;
                ...
                if (mExtractor->mVideoFrames.size() >= 1) {
                    *out = mExtractor->DequeueVideoFrame(mExtractor->mtargetSampleTimeUs, mWantsNALFragments);
                    (*out)->meta_data()->findInt64(kKeyTime, &cur_ts);
                    LOGV("FLVSource read VIDEO buffer4=0x%08x,ts= %lld\n", *out, (cur_ts / 1000));
#if defined(ENABLE_PERF_JUMP_KEY_MECHANISM)
                    if (bJumpToKey == true) {
                        int32_t keyFrame;
                        CHECK((*out)->meta_data()->findInt32(kKeyIsSyncFrame, &keyFrame));
                        if (!keyFrame) {
                            LOGV("FLVSource::read mVideoFrames bJumpToKey3 :this is not Key Frame\n");
                            goto READ_AGAIN;
                        }
                        bJumpToKey = false;
                        LOGD("###FLVSource::read mVideoFrames bJumpToKey3: find KEY!!!");
                    }
#endif
                    return OK;
                }
                ...
                if (mExtractor->mVideoFrames.isEmpty()) {
                    // send EOS
                    LOGE("ERROR_END_OF_STREAM (VIDEO)2\n");
                    return ERROR_END_OF_STREAM;
                }
            }                   // initial
        }
...
sp < MetaData > FLVExtractor::getTrackMetaData(size_t index, uint32_t flags) {
    LOGD("getTrackMetaData:bHaveParsed=%d,index=%d\n", bHaveParsed, index);
    if (index >= mTracks.size()) {
        return NULL;
    }

    if ((flags & kIncludeExtensiveMetaData)
        && (false == bExtractedThumbnails)) {
        findThumbnail();
...
uint32_t FLVExtractor::flags() const {
    LOGD("flags: bSeekable=%d\n", bSeekable);
    if (bSeekable) {
        return CAN_SEEK_BACKWARD | CAN_SEEK_FORWARD | CAN_PAUSE | CAN_SEEK;
    } else {
        LOGD("[flags: can not seek,just can pasue\n");
        return CAN_PAUSE;
    }
}

FLVExtractor::~FLVExtractor() {
    LOGD("~FLVExtractor 0x%x, tid=%d", (unsigned int) this, gettid());
...
#ifdef MTK_VIDEO_HEVC_SUPPORT
uint8_t *flv_write_hevc(uint8_t * sps, int sps_len, uint8_t * pps, int pps_len, int *real_length) {
...
bool FLVExtractor::ParseFLV() {
    LOGD("+[FLV]FLVExtractor::ParseFLV");
    LOGD("=====================================\n");
    LOGD("[FLV Playback capability info]£º\n");
    LOGD("=====================================\n");
    LOGD("Resolution = \"[(8,8) ~ (1280£¬720)]\" \n");
    LOGD("Support Codec = \"Video:H264 ; Audio: MP3,AAC,PCM\" \n");
    LOGD("=====================================\n");

    if (!mflvParser) {
        LOGE("mflvParser is null, can not parse\n");
    }
    if (mflvParser->ParseflvFile() != FLV_OK) {
        LOGE("ParseflvFile Failed\n");
    }
    ...
    if (!metainfo) {
        LOGE("flv_get_meta failed \n");
        LOGE("[FLV Playback capability Error] capability not support as :Un-support file without meta data\n");
        metainfo = NULL;
        bHaveParsed = true;
        return false;
    }
...
#endif
    LOGD("----meta info from onMetaData--- \n");
    LOGD("bSeekable = %d\n", bSeekable);
    LOGD("iDurationMs = %lld ms\n", iDurationMs);
    LOGD("iWidth = %d,iHeight=%d \n", iWidth, iHeight);
    LOGD("video_codec_id = %d,audio_codec_id=%d \n", video_codec_id, audio_codec_id);
    LOGD("bHasVideo = %d \n", bHasVideo);
    LOGD("bHasAudio = %d \n", bHasAudio);
    LOGD("iDataSourceLength = %lld \n", (uint64_t) iDataSourceLength);
    ...
    if (audio_codec_id == FLV_AUDIO_CODEC_ID_ADPCM
        ...
        || audio_codec_id == FLV_AUDIO_CODEC_ID_UNKHNOWN) {
        bHasAudio = false;
        LOGE ("[FLV Playback capability Error] capability not support as :Un-support audio format=%d\n", audio_codec_id);
    }

    if (bHasVideo && (video_codec_id == FLV_VIDEO_CODEC_ID_AVC)) {
        if (mVideoConfigs.size() <= 0) {
            LOGE("H.264 video without config info, ignore it");
            bHasVideo = false;
        }
    }
    ...
    if (bHasVideo && ((video_codec_id == FLV_VIDEO_CODEC_ID_HEVC)
        ...
        if (mVideoConfigs.size() <= 0) {
            LOGE("H.265 video without config info, ignore it");
            bHasVideo = false;
        }
    }
    ...
    if (bHasAudio && (audio_codec_id == FLV_AUDIO_CODEC_ID_AAC)) {
        if (mAudioConfigs.size() <= 0) {
            LOGE("AAC Audio without config info, ignore it");
            bHasAudio = false;
        }
    }
    ...
    if (bHasVideo)              //setup video track
    {
        ...
        case FLV_VIDEO_CODEC_ID_SORENSON_SPARK:
        {
            ...
            LOGD("video_codec_id:MEDIA_MIMETYPE_VIDEO_SORENSON_SPARK\n");
            break;
        }
        case FLV_VIDEO_CODEC_ID_AVC:
        {
            VideoMeta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_AVC);
            LOGD("video_codec_id:MEDIA_MIMETYPE_VIDEO_AVC\n");
            CHECK(mVideoConfigs.size() > 0);
            flv_tag_str *pTag = (flv_tag_str *) & (mVideoConfigs.itemAt(0));
            VideoMeta->setData(kKeyAVCC, kTypeAVCC,
                               (pTag->tag_data +
                                FLV_VIDEO_AVC_TAG_DATA_OFFSET),
            ...
            if (pfile) {
                ...
                fwrite((void *) pTag->tag_data +
                       FLV_VIDEO_AVC_TAG_DATA_OFFSET, 1,
                       (pTag->tag_header.tag_data_size - FLV_VIDEO_AVC_TAG_DATA_RAW_OFFSET), pfile);
                fclose(pfile);
            }
#endif
            free(pTag->tag_data);
        ...
        case FLV_VIDEO_CODEC_ID_HEVC_XL:
        {
            ...
            LOGD("video_codec_id:MEDIA_MIMETYPE_VIDEO_HEVC\n");
            ...
            if ((pTag->tag_data[5] == 0)
                ...
                hevc_codec_info_size =
                    pTag2->tag_header.tag_data_size + pTag3->tag_header.tag_data_size + 28 + 5;
                ALOGD("hevc_codec_info_size is %d", hevc_codec_info_size);
                hevc_codec_info = (uint8_t *) malloc(hevc_codec_info_size * sizeof(uint8_t));
                hevc_codec_info =
                    flv_write_hevc(pTag2->tag_data +
                                   FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                                   pTag2->tag_header.tag_data_size -
                                   FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                                   pTag3->tag_data +
                                   FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                                   pTag3->tag_header.tag_data_size -
                                   FLV_VIDEO_AVC_TAG_DATA_OFFSET, &real_length);
                ...
                VideoMeta->setData(kKeyHVCC, kTypeHVCC,
                                   (pTag->tag_data +
                                    FLV_VIDEO_AVC_TAG_DATA_OFFSET),
                                   (pTag->tag_header.tag_data_size -
                                    FLV_VIDEO_AVC_TAG_DATA_OFFSET));
        ...
        case FLV_VIDEO_CODEC_ID_SCREEN_VIDEO:
        {
            ...
            LOGE("[FLV Playback capability Error] capability not support as : not support FLV_VIDEO_CODEC_ID_SCREEN_VIDEO\n");
            VideoMeta = NULL;
            break;
        }
        case FLV_VIDEO_CODEC_ID_VP6:
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_H263);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_VIDEO_CODEC_ID_VP6\n");
            VideoMeta = NULL;
            break;
        }
        ...
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_H263);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_VIDEO_CODEC_ID_VP6_ALPH\n");
            VideoMeta = NULL;
            break;
        }
        ...
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_VIDEO_H263);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_VIDEO_CODEC_ID_SCREEN2_VIDEO\n");
            VideoMeta = NULL;
            break;
        }
        default:
        {
            LOGE("out of bound video codec, no meta");
            VideoMeta = NULL;
            break;
        }
        }
    ...
    if (bHasAudio)              //setup Audio track
    {
        ...
        case FLV_AUDIO_CODEC_ID_AAC:
        {
            ...
            LOGD("audio_codec_id:MEDIA_MIMETYPE_AUDIO_AAC\n");
            CHECK(mAudioConfigs.size() > 0);
            flv_tag_str *pTag = (flv_tag_str *) & (mAudioConfigs.itemAt(0));
            AudioMeta->setData(kKeyCodecConfigInfo, 0,
                               (pTag->tag_data + FLV_AUDIO_AAC_TAG_DATA_RAW_OFFSET),
        ...
        case FLV_AUDIO_CODEC_ID_MP3_8K:
        {
            ...
            LOGD("audio_codec_id:MEDIA_MIMETYPE_AUDIO_MPEG\n");
            AudioMeta->setInt32(kKeyChannelCount, iChannel_cnt);
            AudioMeta->setInt32(kKeySampleRate, iSampleRate);
            break;
        }
...
#endif
            LOGD("audio_codec_id:FLV_AUDIO_CODEC_ID_PCM=%d\n", audio_codec_id);
            break;
        }
        case FLV_AUDIO_CODEC_ID_ADPCM:
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_MPEG);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_AUDIO_CODEC_ID_ADPCM\n");
            break;
        }
        case FLV_AUDIO_CODEC_ID_NELLYMOSER_8KHZ_MONO:
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_MPEG);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_AUDIO_CODEC_ID_NELLYMOSER_8KHZ_MONO\n");
            break;
        }
        case FLV_AUDIO_CODEC_ID_NELLYMOSER:
        {
            //meta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_MPEG);
            LOGE("[FLV Playback capability Error] capability not support as : not supportFLV_AUDIO_CODEC_ID_NELLYMOSER\n");
            break;
        }
        case FLV_AUDIO_CODEC_ID_G711_ALAW:
        {
            AudioMeta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_G711_ALAW);
            LOGD("audio_codec_id:MEDIA_MIMETYPE_AUDIO_G711_ALAW\n");
            break;
        }
        case FLV_AUDIO_CODEC_ID_G711_MLAW:
        {
            AudioMeta->setCString(kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_G711_MLAW);
            LOGD("audio_codec_id:MEDIA_MIMETYPE_AUDIO_G711_MLAW\n");
            break;
        }
        ...
        case FLV_AUDIO_CODEC_ID_SPEEX:
        {
            ...
            LOGE("[FLV Playback capability Error] capability not support as : not support FLV_AUDIO_CODEC_ID_SPEEX\n");
            break;
        }
        }
    ...
    for (size_t i = 0; i < mTracks.size(); ++i) {
        ...
        LOGD("mTracks i=%d,mime=%s", i, mime);
    }
...
bool FLVExtractor::FLVSeekTo(int64_t targetNPT) {
    ...
    LOGD("FLVSeekTo [%lld ms] return %lld", targetNPT, _new_ts);
    if (_new_ts < -1)
        return false;
    else
        return true;
...
void FLVExtractor::findThumbnail() {
    ...
    LOGD("+FLVExtractor::findThumbnail");
    ...
    if (false == bSeekable) {
        ...
        LOGD("findThumbnail: can not seek, kKeyThumbnailTime=0 \n");
    }                           //bSeekable == false
    else {
        for (size_t i = 0; i < mTracks.size(); ++i) {
            TrackInfo *info = &mTracks.editItemAt(i);
            ...
            while ((FLV_FILE_EOF != CacheMore(CACHE_VIDEO_KEY_FRAME))
                   && (mVideoFrames.size() < FLV_THUMBNAIL_SCAN_SIZE)) {
            }
            ...
            for (size_t i = 0; i < mVideoFrames.size(); i++) {
                ...
                LOGD("Thumbnail frame TS=%lld, Len=%d\n", _cur_frame_ts, out->range_length());
            ...
            LOGD("findThumbnail: can seek, _thumbnail_frame_ts=%lld \n", _thumbnail_frame_ts);
        }
...
status_t FLVExtractor::parseAVCCodecSpecificData(uint8_t * data, size_t size, int *buffer_size) {
...
FLV_ERROR_TYPE FLVExtractor::CacheMore(CacheType HowCache) {
    ...
    LOGV("+CacheMore -HowCache=%d-bHasVideo=%d,bHasAudio=%d\n", HowCache, bHasVideo, bHasAudio);

    if (iFlvParserReadOffset >= iDataSourceLength) {
        LOGE("CacheMore :read pointer is beyond the end of file!!!!");
        return FLV_FILE_EOF;
    }
    ...
    if (retVal == FLV_OK) {
        if (mTag->tag_header.tag_type == FLV_TAG_TYPE_VIDEO) {
            if (bHasVideo || CACHE_ANYHOW == HowCache) {
                ...
                LOGV("CacheMore :video codec id =%d", byteVal1 & 0x0F);
                ...
                if (HowCache == CACHE_VIDEO_KEY_FRAME) {
                    if ((byteVal1 & FLV_VIDEO_FRAME_TYPE_BITMASK) != FLV_VIDEO_FRAME_TYPE_KEY) {
                        LOGD("CacheMore CACHE_VIDEO_KEY_FRAME: skip this frame, not Key\n");
                        goto READ_TAG;
                    }
                }
                ...
                case FLV_VIDEO_CODEC_ID_SORENSON_SPARK:
                {
                    if (mTag->tag_header.tag_data_size <= FLV_VIDEO_S263_TAG_DATA_RAW_OFFSET) {
                        LOGE("CacheMore: S263£¬ no data in this tag,drop it,tag_data_size=%d!!\n", mTag->tag_header.tag_data_size);
                    } else {
                        mVideoFrames.push();
                        ...
                        LOGM("[memory]CacheMore mVideoFrames alloc buffer =0x%08x\n", tag->tag_data);

                        if (iWidth == 0 || iHeight == 0) {
                            uint32_t key_frame;
                            LOGD("CacheMore cache S263 Frame: check resolution\n");
                ...
                case FLV_VIDEO_CODEC_ID_AVC:
                {
                    if (mTag->tag_header.tag_data_size <= FLV_VIDEO_AVC_TAG_DATA_OFFSET) {
                        LOGE("CacheMore: AVC£¬ no data in this tag,drop it,tag_data_size=%d!!\n", mTag->tag_header.tag_data_size);
                    } else {
                        ...
                        if (byteVal2 == FLV_VIDEO_AVC_PACKET_TYPE_CONFIG) {
                            if (isAVCCSend) {
                                ...
                                memcpy(tag->tag_data, mTag->tag_data, mTag->tag_header.tag_data_size);
                                LOGM("[memory]CacheMore mVideoFrames alloc buffer =0x%08x\n", tag->tag_data);
                                LOGV("+CacheMore -HowCache=%d-tag_data_size=%d,tag_data_offset=%d\n",
                                ...
                                tag->tag_data = (uint8_t *)malloc(sizeof(uint8_t)*(mTag->tag_header.tag_data_size));
                                memcpy(tag->tag_data, mTag->tag_data, mTag->tag_header.tag_data_size);
                                LOGM("[memory]CacheMore mVideoConfigs alloc buffer =0x%08x\n", tag->tag_data);
                                LOGD("CacheMore cache avc csps/pps: check resolution\n");
                                flv_parse_avc_sps((mTag->tag_data + FLV_VIDEO_AVC_TAG_DATA_OFFSET),
                                                  (mTag->tag_header.tag_data_size - FLV_VIDEO_AVC_TAG_DATA_OFFSET),
                                                  &iWidth, &iHeight);
                            }
                ...
                {
                    if (mTag->tag_header.tag_data_size <= FLV_VIDEO_AVC_TAG_DATA_OFFSET) {
                        LOGE("CacheMore: HEVC£¬no data in this tag,drop it,tag_data_size=%d!!\n",
                                  mTag->tag_header.tag_data_size);
                    } else {
                        byteVal2 = *(mTag->tag_data + 1);
                        if (byteVal2 == FLV_VIDEO_AVC_PACKET_TYPE_CONFIG) {
                            ...
                            LOGD("CacheMore cache hevc sps/pps");
                ...
                LOGD("CacheMore :bhasVidoe is false && CACHE_ANYHOW ! == HowCache, so skip this video frame\n");
            }
        } else if (mTag->tag_header.tag_type == FLV_TAG_TYPE_AUDIO) {
            if (bHasAudio || CACHE_ANYHOW == HowCache) {
                byteVal1 = *(mTag->tag_data);
                LOGV("CacheMore :audio codec id =%d", (byteVal1 & 0xF0) >> 4);
                ...
                case FLV_AUDIO_CODEC_ID_MP3:
                {
                    if (mTag->tag_header.tag_data_size <= FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET) {
                        LOGE("CacheMore: MP3£¬ no data in this tag,drop it,tag_data_size=%d!!\n",
                                 mTag->tag_header.tag_data_size);
                    } else {
                        mAudioFrames.push();
                        ...
                        tag->tag_data = (uint8_t *) malloc(sizeof(uint8_t) * (mTag->tag_header.tag_data_size));
                        memcpy(tag->tag_data, mTag->tag_data, mTag->tag_header.tag_data_size);
                        LOGM("[memory]CacheMore mAudioFrames alloc buffer =0x%08x\n", tag->tag_data);
                        if (!bHaveParsed) {
                            ...
                            int out_bitrate;
                            bool ret;
                            header = U32_AT((uint8_t *) (tag->tag_data +
                                                    FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET));
                            LOGV("CacheMore MP3: headert=0x%08x\n", header);
                            ret = get_mp3_frame_size(header, &frame_size, &iSampleRate, &iChannel_cnt, &out_bitrate);
                            if (ret) {
                                LOGD("CacheMore MP3: iChannel_cnt=%d,iSampleSize=%d,iSampleRate=%d\n",
                ...
                case FLV_AUDIO_CODEC_ID_PCM_LE:
                {
                    if (mTag->tag_header.tag_data_size <= FLV_AUDIO_PCM_TAG_DATA_RAW_OFFSET) {
                        LOGE("CacheMore: PCM£¬ no data in this tag,drop it,tag_data_size=%d!!\n",
                                 mTag->tag_header.tag_data_size);
                    } else {
                        mAudioFrames.push();
                        flv_tag_str *tag = &mAudioFrames.editItemAt(mAudioFrames.size() - 1);
                        ...
                        if (!bHaveParsed) {
                            uint8_t Val8 = *((uint8_t *) (tag->tag_data));
                            get_audio_tage_header_info(Val8, NULL,
                                                       &iSampleRate, &iSampleSize, &iChannel_cnt);

                            LOGD("CacheMore PCM: iChannel_cnt=%d,iSampleSize=%d,iSampleRate=%d\n",
                                     iChannel_cnt, iSampleSize, iSampleRate);
                        }
                    }
                    break;
                ...
                case FLV_AUDIO_CODEC_ID_AAC:
                {
                    if (mTag->tag_header.tag_data_size <= FLV_AUDIO_AAC_TAG_DATA_RAW_OFFSET) {
                        LOGE("CacheMore: AAC£¬ no data in this tag,drop it,tag_data_size=%d!!\n",
                                 mTag->tag_header.tag_data_size);
                    } else {
                        byteVal2 = *(mTag->tag_data + 1);
                        if (byteVal2 == FLV_AUDIO_AAC_PACKET_TYPE_CONFIG) {
                            ...
                            tag->tag_data = (uint8_t *) malloc(sizeof(uint8_t) * (mTag->tag_header.tag_data_size));
                            memcpy(tag->tag_data, mTag->tag_data, mTag->tag_header.tag_data_size);
                            LOGM("[memory]CacheMore mAudioConfigs alloc buffer =0x%08x\n", tag->tag_data);
                            if (!bHaveParsed) {
                                uint8_t Val8 = *((uint8_t *) (tag->tag_data));
                                get_audio_tage_header_info(Val8,
                                                           NULL,
                                                           &iSampleRate,
                                                           &iSampleSize, &iChannel_cnt);

                                LOGD("CacheMore AAC: iChannel_cnt=%d,iSampleSize=%d,iSampleRate=%d\n",
                                          iChannel_cnt, iSampleSize, iSampleRate);
                            }
                ...
                LOGD("CacheMore :bhasAudio is false && CACHE_ANYHOW ! == HowCache,so skip this audio frame\n");
            }
        } else                  //not video && not audio
        {
            byteVal1 = *(mTag->tag_data);
            LOGE("CacheMore not video && not audio tag: skip it,byteVal1=0x%08x\n", byteVal1);
        }

        if (!bHaveParsed && (CACHE_ANYHOW == HowCache)
            && (u4ReadTagCnt > FLV_INITIAL_TAG_COUNT_THD)) {
            LOGE("FLVExtractor::CacheMore FLV_VIDEO_OR_AUDIO_FLAG Maybe Error");
        } else {
            if (!bHaveParsed && ((mVideoFrames.size() == 0 && bHasVideo)
                                 || (mAudioFrames.size() == 0 && bHasAudio))) {
                LOGV("CacheMore read again\n");
                goto READ_TAG;
            } else if (!bHaveParsed && CACHE_ANYHOW == HowCache
                       && (mVideoFrames.size() + mAudioFrames.size()) < 5) {
                LOGV("CacheMore CACHE_ANYHOW read again\n");
                goto READ_TAG;
            }
        }
        Ret = FLV_OK;
        ...
        LOGE("FLVExtractor::CacheMore FLV_FILE_READ_ERR FLV_END_OF_TRACK");
        Ret = FLV_FILE_EOF;
    } else if (retVal == FLV_FILE_EOF) {
        LOGE("FLVExtractor::CacheMore FLV_FILE_EOF FLV_END_OF_TRACK");
        Ret = FLV_FILE_EOF;
    } else if (retVal == FLV_ERROR) {
        LOGE("CacheMore FLV_ERROR");
        Ret = FLV_ERROR;
    }
    LOGV("CacheMore done :mAudioConfig size= %d,mVideoConfigs.size=%d,mVideoFrames.size=%d,mAudioFrames.size=%d\n",
              mAudioConfigs.size(), mVideoConfigs.size(), mVideoFrames.size(), mAudioFrames.size());
    return Ret;
}

MediaBuffer *FLVExtractor::DequeueVideoFrame(int64_t targetSampleTimeUs, bool isNalbase) {
    flv_tag_str *pTag = (flv_tag_str *) & (mVideoFrames.itemAt(0));
    bool isFramebase = !isNalbase;
    uint8_t byteVal = *(pTag->tag_data);
    LOGV("DequeueVideoFrame :video codec id =%d", byteVal & 0x0F);
    iDecVideoFramesCnt++;
    LOGV("DequeueVideoFrame :iDecVideoFramesCnt =%d,+FLVExtractor 0x%x, tid=%d",
              iDecVideoFramesCnt, (unsigned int) this, gettid());

    switch (byteVal & 0x0F) {
    case FLV_VIDEO_CODEC_ID_SORENSON_SPARK:
    {
        MediaBuffer *buffer =
            new MediaBuffer(pTag->tag_header.tag_data_size - FLV_VIDEO_S263_TAG_DATA_RAW_OFFSET);
        buffer->meta_data()->setInt64(kKeyTime, pTag->tag_header.tag_ts * 1000LL);
        uint8_t *data = (uint8_t *) buffer->data();
        memcpy(data, pTag->tag_data + FLV_VIDEO_S263_TAG_DATA_RAW_OFFSET,
               pTag->tag_header.tag_data_size - FLV_VIDEO_S263_TAG_DATA_RAW_OFFSET);
        ...
        LOGM("[memory]DequeueVideoFrame mVideoFrames Free buffer =0x%08x\n", pTag->tag_data);
        uint8_t byteVal = *(pTag->tag_data);
        if ((byteVal & FLV_VIDEO_FRAME_TYPE_BITMASK) == FLV_VIDEO_FRAME_TYPE_KEY) {
            buffer->meta_data()->setInt32(kKeyIsSyncFrame, 1);
            LOGD("DequeueVideoFrame :this is Key Frame\n");
        } else {
    ...
    case FLV_VIDEO_CODEC_ID_AVC:
    {
        uint8_t AVCPacketType = (*(pTag->tag_data + 1) && 0xff);
        if (0 == AVCPacketType) {
            int buffer_size = 0;
            ...
            if (OK == parseAVCCodecSpecificData(pTag->tag_data + FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                ...
                uint8_t *out_data = (uint8_t *) malloc(buffer_size * sizeof(uint8_t));
                out_data = cutAVCC2Buffer(pTag->tag_data + FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                                          pTag->tag_header.tag_data_size, buffer_size);
                ...
                //calc ts :
                uint32_t u4Var;
                u4Var = ((U32_AT((uint8_t *) (pTag->tag_data + FLV_VIDEO_AVC_TAG_CT_OFFSET)))
                         >> 8) & 0x00FFFFFF;

                LOGV("DequeueVideoFrame :u4Var =0x%08x", u4Var);
                if ((u4Var & 0x800000) != 0) {
        ...
        if (isFramebase) {
            ...
            int32_t CompositionTime;
            uint64_t ts;
            uint32_t total_nal_size = 0;
            Vector < flv_pure_nal > tmpNalVec;
            while ((flush != true)
                   && (curFrameVec < mVideoFrames.size())) {
                flv_tag_str *pTag = (flv_tag_str *) & (mVideoFrames.itemAt(curFrameVec));
                curFrameVec++;
                int AVCNALType = (*(pTag->tag_data + 5 + mNALLengthSize) && 0x1f);

                LOGV("DequeueVideoFrame:bf tag_data_size=%d tag_data_offset =%u, tag_data=0x%08x\n",
                ...
                if (this_nal_size > remain_date_in_tag) {
                    LOGE("DequeueVideoFrame: current nal size =%d,remain_date_in_tag=%d\n",
                              this_nal_size, remain_date_in_tag);
                    this_nal_size = remain_date_in_tag;
                }

                total_nal_size += (this_nal_size + 4);
                if (AVCNALType == 1 || AVCNALType == 5) {
                    ABitReader br(pTag->tag_data + 5 + mNALLengthSize + 1, this_nal_size);
                    unsigned first_mb_in_slice = parseUE(&br);
                    if (first_mb_in_slice == 0) {
                        // This slice starts a new frame.
                        flush = true;
                    }
                } else if (AVCNALType == 9 || AVCNALType == 7) {
                    // Access unit delimiter and SPS will be associated with the
                    // next frame.
                    flush = true;
                }

                if (firstTag) { // use first nal ts as whole frame ts
                    //calc ts :
                    u4Var = ((U32_AT((uint8_t *) (pTag->tag_data + FLV_VIDEO_AVC_TAG_CT_OFFSET)))
                             >> 8) & 0x00FFFFFF;
                    ...
                    if ((byteVal & FLV_VIDEO_FRAME_TYPE_BITMASK) == FLV_VIDEO_FRAME_TYPE_KEY) {
                        isSyncFrame = true;
                        LOGV("DequeueVideoFrame :this is Key Frame\n");
                    } else {
                        isSyncFrame = false;
                    }
                }
                firstTag = false;
                flv_pure_nal tmpNal;
                tmpNal.nal_data = (uint8_t *) malloc(this_nal_size * sizeof(uint8_t));
                memcpy(tmpNal.nal_data, (uint8_t *) (pTag->tag_data + pTag->tag_data_offset + mNALLengthSize), this_nal_size);
                tmpNal.nal_size = this_nal_size;
                tmpNalVec.push(tmpNal);
            }                   //end while flush != true
            MediaBuffer *buffer = new MediaBuffer(total_nal_size);
            uint8_t *data = (uint8_t *) buffer->data();
            int curCopyPos = 0;
            /* merge nals to a frame */
            for (int s = 0; s < curFrameVec; s++) {
                memcpy(data + curCopyPos, "\x00\x00\x00\x01", 4);
                curCopyPos += 4;
                memcpy(data + curCopyPos,
                       (uint8_t *) (tmpNalVec.itemAt(s).nal_data), tmpNalVec.itemAt(s).nal_size);
                curCopyPos += tmpNalVec.itemAt(s).nal_size;
            }
            buffer->set_range(0, total_nal_size);
            ...
            /* free memory used in tmpNalVdec */
            for (size_t i = 0; i < tmpNalVec.size(); i++) {
                flv_pure_nal *pure_nal = (flv_pure_nal *) & (tmpNalVec.itemAt(i));
                if (pure_nal->nal_data) {
                    LOGM("[memory]ClearVideoFrameQueue flush %d mVideoFrames Free buffer =0x%08x\n",
                             i, pTag->tag_data);
                    free(pure_nal->nal_data);
                    pure_nal->nal_data = NULL;
                }
            }
            tmpNalVec.clear();

            for (int t = 0; t < curFrameVec; t++) {
                flv_tag_str *tmpTag = (flv_tag_str *) & (mVideoFrames.itemAt(0));
                if (tmpTag->tag_data) {
                    free(tmpTag->tag_data);
                    tmpTag->tag_data = NULL;
                }
                mVideoFrames.removeItemsAt(0);
            }
            return buffer;
            break;
        } else {                // send nal by nal
            uint32_t this_nal_size, u4Var;
            int32_t CompositionTime;
            uint64_t ts;
            LOGV("DequeueVideoFrame:bf tag_data_size=%d tag_data_offset =%u, tag_data=0x%08x\n",
            ...
            LOGV("DequeueVideoFrame :CompositionTime =0x%08x current nal size =%d,tag_data_offset=%d\n",
            ...
            {
                LOGM("[memory]DequeueVideoFrame mVideoFrames Free buffer =0x%08x\n", pTag->tag_data);
                LOGV("DequeueVideoFrame:read all nals, current nal size =%d,tag_data_offset=%d\n", this_nal_size, pTag->tag_data_offset);
                free(pTag->tag_data);
                pTag->tag_data = NULL;
                mVideoFrames.removeItemsAt(0);
            }
    ...
    case FLV_VIDEO_CODEC_ID_HEVC_XL:
    {
        ...
        int32_t CompositionTime;
        uint64_t ts;
        LOGD("DequeueVideoFrame:bf tag_data_size=%d tag_data_offset =%u, tag_data=0x%08x\n",
                 pTag->tag_header.tag_data_size, pTag->tag_data_offset, pTag->tag_data);
        ...
        LOGV
            ("DequeueVideoFrame :CompositionTime =0x%08x current nal size =%d,tag_data_offset=%d\n",
             CompositionTime, this_nal_size, pTag->tag_data_offset);
        free(pTag->tag_data);
        pTag->tag_data = NULL;
        mVideoFrames.removeItemsAt(0);
...
MediaBuffer *FLVExtractor::DequeueAudioFrame(int64_t targetSampleTimeUs) {
    ...
    uint8_t byteVal = *(pTag->tag_data);
    LOGV("DequeueAudioFrame :audio codec id =%d", (byteVal & 0xF0) >> 4);
    iDecAudioFramesCnt++;
    LOGV("DequeueAudioFrame :iDecAudioFramesCnt =%d, tag_data_size=%d",
             iDecAudioFramesCnt, pTag->tag_header.tag_data_size);
    ...
    case FLV_AUDIO_CODEC_ID_MP3:
    {
        ...
        uint8_t *data = (uint8_t *) buffer->data();
        memcpy(data,
               pTag->tag_data + FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET,
               pTag->tag_header.tag_data_size - FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET);
        buffer->set_range(0, pTag->tag_header.tag_data_size - FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET);
        buffer->meta_data()->setInt64(kKeyTargetTime, targetSampleTimeUs);
        LOGM("[memory]DequeueAudioFrame mAudioFrames Free buffer =0x%08x\n", pTag->tag_data);
        free(pTag->tag_data);
        pTag->tag_data = NULL;
    ...
    case FLV_AUDIO_CODEC_ID_PCM_LE:
    {
        ...
        uint8_t *data = (uint8_t *) buffer->data();
        memcpy(data,
               pTag->tag_data + FLV_AUDIO_PCM_TAG_DATA_RAW_OFFSET,
               pTag->tag_header.tag_data_size - FLV_AUDIO_PCM_TAG_DATA_RAW_OFFSET);
    ...
    /*
       ...
       {
       LOGD("DequeueAudioFrame dump\n");
       flv_tag_str* pTag = (flv_tag_str*)&(mAudioFrames.itemAt(i));
       LOGD("i=%d,pTag->tag_header.tag_data_size=%d\n",i,pTag->tag_header.tag_data_size);
       LOGD("i=%d,pTag->tag_header.tag_type=%d\n",i,pTag->tag_header.tag_type);
       LOGD("i=%d,pTag->tag_data=0x%08x\n",i,pTag->tag_data);
       LOGD("i=%d,*(mTag->tag_data)=0x%08x\n",i,*(pTag->tag_data));
...
void FLVExtractor::ClearVideoFrameQueue() {
    LOGD("+ClearVideoFrameQueue\n");
    for (size_t i = 0; i < mVideoFrames.size(); i++) {
        LOGV("ClearVideoFrameQueue flush %d \n", i);
    ...
    LOGV("ClearVideoFrameQueue flush done\n");
    mVideoFrames.clear();
    LOGD("ClearVideoFrameQueue clear done,mVideoFrames.size()=%d\n", mVideoFrames.size());

    for (size_t i = 0; i < mVideoConfigs.size(); i++) {
        LOGV("ClearmVideoConfigsQueue flush %d \n", i);
        flv_tag_str *pTag = (flv_tag_str *) & (mVideoConfigs.itemAt(i));
        if (pTag->tag_data) {
            LOGM("[memory]ClearmVideoConfigsQueue flush %d mVideoConfigs Free buffer =0x%08x\n", i, pTag->tag_data);
            free(pTag->tag_data);
            pTag->tag_data = NULL;
        }
    }
    LOGV("ClearmVideoConfigsQueue flush done\n");
    mVideoConfigs.clear();
    LOGD("ClearmVideoConfigsQueue clear done,mVideoConfigs.size()=%d\n", mVideoConfigs.size());
}

void FLVExtractor::ClearAudioFrameQueue() {
    LOGD("+ClearAudioFrameQueue\n");
    for (size_t i = 0; i < mAudioFrames.size(); i++) {
        LOGV("ClearAudioFrameQueue flush %d \n", i);
        flv_tag_str *pTag = (flv_tag_str *) & (mAudioFrames.itemAt(i));
        if (pTag->tag_data) {
            LOGM("[memory]ClearAudioFrameQueue flush %d mAudioFrames Free buffer =0x%08x\n", i, pTag->tag_data);
            free(pTag->tag_data);
            pTag->tag_data = NULL;
        }
    }
    LOGV("ClearAudioFrameQueue flush done\n");
    mAudioFrames.clear();
    LOGD("-ClearAudioFrameQueue clear done,mAudioFrames.size()=%d\n", mAudioFrames.size());

    for (size_t i = 0; i < mAudioConfigs.size(); i++) {
        LOGV("ClearmAudioConfigsQueue flush %d \n", i);
        flv_tag_str *pTag = (flv_tag_str *) & (mAudioConfigs.itemAt(i));
        if (pTag->tag_data) {
            LOGM("[memory]ClearmAudioConfigsQueue flush %d mAudioConfigs Free buffer =0x%08x\n", i, pTag->tag_data);
            free(pTag->tag_data);
            pTag->tag_data = NULL;
        }
    }
    LOGV("ClearmAudioConfigsQueue flush done\n");
    mAudioConfigs.clear();
    LOGD("ClearmAudioConfigsQueue clear done,mAudioConfigs.size()=%d\n", mAudioConfigs.size());
}
...
bool SniffFLV(const sp < DataSource > &source, String8 * mimeType,
