...
class flvParser {
  public:
    flvParser(void *source, flv_io_read_func_ptr read, flv_io_write_func_ptr write,
    ...
    uint32_t flv_search_tag_pattern(uint8_t ** data, uint32_t size);
#endif
  ...
  private:
     FLV_ERROR_TYPE flv_parse_amf_obj(uint8_t * amf_data, uint32_t amf_data_len, uint32_t * offset,
                                      char *key, flv_meta_str * metaInfo, uint32_t depth);
    ...
    FLV_ERROR_TYPE flv_search_all_seek_tables(int64_t seekTimeMs, int64_t foundTimeMs,
...
class FLVExtractor:public MediaExtractor {
  ...
  public:
    ...
    MediaBuffer *DequeueVideoFrame(int64_t targetSampleTimeUs = -1, bool isNalbase = false);
...
bool SniffFLV(const sp < DataSource > &source, String8 * mimeType, float *confidence,
              sp < AMessage > *);
}
