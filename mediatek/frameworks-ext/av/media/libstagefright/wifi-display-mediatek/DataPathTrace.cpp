...
status_t WfdDebugInfo::addTimeInfoByKey(uint32_t  isVideo,int64_t key , const char* valueName, int64_t value)  {\
	...
	if(mLogEnable){
		ALOGI("addTimeInfoByKey:[%s] [key=%lld us]name=%s value = %lld  has %d infos,index=%d",
			isVideo?"video":"audio",key,valueName,value,mDebugInfoList[isVideo].size(),index);
	...
	if( index >= 0){//exist 
		...
		*/	
		...
		if(mLogEnable){
			index = mDebugInfoList[isVideo].indexOfKey(key) ;
			ALOGV("addTimeInfoByKey new+++:[%s] [key=%lld us]name=%s has %d infos,index=%d",
...
int64_t WfdDebugInfo::getTimeInfoByKey(uint32_t isVideo,int64_t key,const char* valueName)  {
	...
	index = mDebugInfoList[isVideo].indexOfKey(key) ;
	if(mLogEnable){
		ALOGV("getTimeInfoByKey: [%s]find key=%lld, valueName=%s,index=%d  ",isVideo?"video":"audio",key,valueName,index);
	}
	if( index >= 0){//exist 
		sp<AMessage> msg  = mDebugInfoList[isVideo].editValueAt(index);
