...
RTPReceiver::Source::Source(RTPReceiver *receiver, uint32_t ssrc)
    : mReceiver(receiver),
      ...
      mDeclareLostTimerPending(false){
		...
		///M: Find each packet type to make Assembler,ts 33
        if (33 != mActivePacketType) {
            mActiveAssembler = mReceiver->makeAssembler(33);
            mActivePacketType = 33;
        }
...
void RTPReceiver::Source::onPacketReceived(
	...
	///M: sequence number gap > 3000, drop
...
void RTPReceiver::Source::queuePacket(const sp<ABuffer> &packet) {
    ...
    if (mPackets.empty()) {
		...
		if(mPackets.size()>2)
			ALOGI("mPackets size is %d",mPackets.size());
        dequeueMore();
        return;
	#if 0	
    }
...
void RTPReceiver::Source::dequeueMore() {
	...
	///@}
		...
		///M: Find each packet type to make Assembler,ts const 33
        int32_t packetType;
         CHECK(packet->meta()->findInt32("PT", &packetType));
		ALOGD("packetType is %d",packetType);
...
status_t RTPReceiver::initAsync(
    ...
    for (;;) {
        localRTPPort = 19000;//PickRandomRTPPort();
    ...
    *outLocalRTPPort = localRTPPort;
    ...
    ALOGI("mRTPSessionID = %d.", mRTPSessionID);
...
status_t RTPReceiver::connect(
    ...
    if (mRTPMode == TRANSPORT_UDP) {
        ...
        ///Add by MTK @{ To avoid sink IOT problem
        /*
        ...
        case ANetworkSession::kWhatDatagram:
        {
            ...
            if (isRTP) {
				...
				if(mFirstRTPData)
				{
					mFirstRTPData = false;
					int64_t nowUs = ALooper::GetNowUs();
					ALOGI("WfdApp-1st_data Time:(%lld)us",nowUs);					
				}
#endif
...
/// @}
    ...
    meta->setInt32("seqNo", seqNo);

#ifdef MTK_WFD_SINK_SUPPORT 
     /*debug latency*/

	sp<WfdDebugInfo> debugInfo= defaultWfdDebugInfo();
       int64_t timeUs = (rtpTime * 100ll)/9;
	int64_t now = ALooper::GetNowUs();	   
    
      if(BaseOnMbit == mLatencyRule){
	  	
          int32_t Mbit;
          CHECK(meta->findInt32("Mbit",&Mbit));	
          if(Mbit == 1)
          {		
          	debugInfo->addTimeInfoByKey(true, timeUs, "rtprecvin", now/1000);
          }
	   ...
	   if((-1 != PretimeUs) && (PretimeUs != timeUs)){
	       debugInfo->addTimeInfoByKey(true, PretimeUs, "rtprecvin", (now - PreNowTime)/1000);
	   }
	   ...
	   PreNowTime = now;
           ...
           if(0 == seqNo % 10){
               debugInfo->addTimeInfoByKey(true, (int64_t)seqNo, "rtprecvin", now/1000);
           }   
      }
#endif	
	...
	if (property_get("media.wfd.dump", val, NULL)) 
	{		

		if (!strcasecmp("true", val) || !strcmp("1", val)) 
		{
           
		   dumpTs(data,size,true);
			
        }
		else if (!strcasecmp("false", val) || !strcmp("0", val))
		{		
		   dumpTs(data,size,false);
		}			
...
#ifdef MTK_WFD_SINK_SUPPORT
void RTPReceiver::dumpTs(const uint8_t * data, size_t size, bool bdtse)
{
	...
	if(bdtse)/*dump start*/
	{
		if(!mDumpTsFlag)
		{		
			mTsFile = fopen("/sdcard/wfdsink.ts", "w+");
			if(NULL == mTsFile)
			{
				ALOGD("Open file fail[/sdcard/wfdsink.ts]");
			}
			else
			{	
				ALOGD("Begin to dump ts file[/sdcard/wfdsink.ts]");
				mDumpTsFlag = true;	
			}
		...
		if(mTsFile)
		{
			fwrite(data+12, 1, size-12, mTsFile);
		}		
	...
	else/*dump stop*/
	{
		if(mDumpTsFlag)
		{
			...
			ALOGD("End to dump ts file[/mnt/sdcard/wfdsink.ts]");
			mDumpTsFlag = false;
		}
	}
...
status_t RTPReceiver::mtkRTPPause()
{
	if(mRTPSessionID)
	{
		mNetSession->mtkRTPRecvPause(mRTPSessionID);
	}
	if(mRTCPSessionID)
	{
		mNetSession->mtkRTPRecvPause(mRTCPSessionID);
	}
	return OK;	

}
status_t RTPReceiver:: mtkRTPResume()
{
	if(mRTPSessionID)
	{
		mNetSession->mtkRTPRecvResume(mRTPSessionID);
	}
	if(mRTCPSessionID)
	{
		mNetSession->mtkRTPRecvResume(mRTCPSessionID);
	}
	return OK;

}
int64_t RTPReceiver::getRTPRecvNum(){
	
	return mNetSession->getRTPRecvNum(mRTPSessionID);

}
status_t RTPReceiver::resetRTPRecvNum(){

	return mNetSession->resetRTPRecvNum(mRTPSessionID);
} 
