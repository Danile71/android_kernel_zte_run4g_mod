...
const char* UibcCapability::getSupportedCapabilities() {
    ...
    *outputHid = 0x00;
    ...
    if (strlen(outputCat) > 1) {
        outputCat[strlen(outputCat) - 1] = 0x0;
    }
...
bool UibcCapability::isGenericSupported(int devType) {
    ALOGD("isGenericSupported() mSupportedCatFlag=%d,mSupportedGenDevFlag=%d,devType=%d",
          mSupportedCatFlag, mSupportedGenDevFlag, devType);
    if ((mSupportedCatFlag & INPUT_CAT_GENERIC_FLAG) == 0x00)
        return false;

    return !!(mSupportedGenDevFlag & (0x01 << devType));
}

bool UibcCapability::isHidcSupported(int devType, int path) {
    ALOGD("isHidcSupported() devType=%d, path=%d", devType, path);
