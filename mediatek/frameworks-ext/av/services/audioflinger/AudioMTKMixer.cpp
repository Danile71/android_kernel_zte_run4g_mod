...
void AudioMTKMixer::TimeStretchBufferProvider::releaseBuffer(AudioBufferProvider::Buffer *pBuffer) {
	...
	//ALOGV("TimeStretchBufferProvider %d keeped.",mOutRemain);
	//ALOGV("release pBuffer-> raw %x, pBuffer->frameCount %d",pBuffer->raw ,pBuffer->frameCount  );
}
...
int AudioMTKMixer::getTrackName(audio_channel_mask_t channelMask, audio_format_t format, int sessionId)
{
    ...
    if (names != 0) {
        ...
        t->needs = 0;
        t->volume[0] = 0;   //UNITY_GAIN;   //modify for pop noise when sample rate switch, initial value set to 0 will ramp up when startup.
        t->volume[1] = 0;   //UNITY_GAIN;
    ...
    case TRACK:
...
#ifdef MTK_AUDIOMIXER_ENABLE_DRC//PUSH_ZERO
        case DO_PUSHZERO:
            track.mPushZero = true;
            MTK_ALOG_D("DO_PUSHZERO %d", track.mPushZero);
            break;
#endif
        case DO_POST_PROC:
            track.IsMixerDoPostProc = (uint8_t) valueInt;
            MTK_ALOG_V("DC_REMOVE %d", track.IsMixerDoPostProc);
            break;
#ifdef TIME_STRETCH_ENABLE
        case DO_TIMESTRETCH:
        ...
        case STEREO2MONO:
            ...
            if(track.mSteroToMono != (BLOUD_S2M_MODE_ENUM)valueInt) {
                ...
                ALOGD("setParameter(DRC, STEREO2MONO: %d)", valueInt);
            }
#endif
...
bool AudioMTKMixer::track_t::DoStereoMonoConvert(void *buffer, size_t byte)
{
...
void AudioMTKMixer::track_t::updateDRCParam(int devSampleRate)
{
...
{
    MTK_ALOG_V("setDRCHandler, mUIDRCEnable %d, mpDRCObj 0x%x, mStreamType %d, device %d, mDRCState %d, mSteroToMono %d, this 0x%x", mUIDRCEnable, mpDRCObj, mStreamType, device, mDRCState, mSteroToMono, this);
    ...
    while (e0) {
        ...
        while (e1) {
            ...
            if ((t.needs & NEEDS_RESAMPLE__MASK) == NEEDS_RESAMPLE_ENABLED) {
                ...
                {                    
                    //t.mPushZero = false; set false later
                    MTK_ALOG_D("DO_PUSHZERO bypass resamler");
                }
                #else
                t.resampler->setPTS(pts);
                t.hook(&t, numFrames, state->resampleTemp, aux, state);
            ...
            {   
                MTK_ALOG_V("Do DRC and volume control");
                t.applyDRC( state->aggregationBuffer, state->frameCount, state->pDRCTempBuffer);
                
                if(t.mPushZero)
                {
                    t.mPushZero = false; 
                    MTK_ALOG_V("DRC memset last frame");
                    memset(state->aggregationBuffer+( 64 * MAX_NUM_CHANNELS), 0, sizeof(int32_t) * (state->frameCount-64) * MAX_NUM_CHANNELS);
                }
