...
void AudioResamplerMtk32::init(int32_t SrcSampleRate)
{
    ...
    int OutChannelCount;
    if(mChannelCount>2){
        OutChannelCount = mChannelCount;
    }else{
        OutChannelCount = kDefaultOutChannelCount;
        }
        
    SXLOGD("init inChannelCount %d, OutChannelCount", mChannelCount,OutChannelCount);
    if(SrcSampleRate) {
        mBliSrc = new Blisrc(SrcSampleRate, mChannelCount, mSampleRate, OutChannelCount, mInFormat);
    }
    else {
        mBliSrc = new Blisrc(kDefaultInSampleRate, mChannelCount, mSampleRate, OutChannelCount, mInFormat);
    }
#else
    if(SrcSampleRate) {
...
#ifdef MTK_BESSURROUND_SUPPORT
	if(mInFormat == BLISRC_IN_Q1P15_OUT_Q1P15 )
	{
		out_frame_size = sizeof(short);
	}
	else
	{
		out_frame_size = sizeof(int);
	}
#endif
        size_t inBufSize = mBuffer.frameCount * mChannelCount * frame_size;
	#ifdef MTK_BESSURROUND_SUPPORT
        SXLOGV(" mChannelCount %d, out_frame_size %d", mChannelCount, out_frame_size);
	size_t outBuffSize = outFrameCount *outChannelCount*out_frame_size;
	#else	
...
#ifdef MTK_BESSURROUND_SUPPORT			
            consume = mBliSrc->resample(in,&inBufSize, ((short*)out) + outputIndex,&outBuffSize);
#else
...
#ifdef MTK_BESSURROUND_SUPPORT			
        outputIndex += outBuffSize >> 1;// for multi channel playing 16 bit         
        outFrameCount -= ( outBuffSize/(outChannelCount * out_frame_size));
        SXLOGV("outFrameCount %d,mChannelCount * out_frame_size %d ",outFrameCount,outChannelCount * out_frame_size);
#else
 	outputIndex += outBuffSize >> 2; 
	outFrameCount -= outBuffSize >> 3;
#endif
        
        #ifndef MTK_BESSURROUND_SUPPORT			    
        if(mInFormat == BLISRC_IN_Q1P31_OUT_Q1P31 ||mInFormat == BLISRC_IN_Q9P23_OUT_Q1P31 ){            
        ...
        mBuffer.frameCount = consume/(mChannelCount*frame_size); 
        #endif
...
{
    int32_t result;
    if(mBliParam.in_channel > 2)
    {
	    result = Blisrc_MultiChannel_GetBufferSize(&mInternalBufSize, &mTempBufSize, &mBliParam);
	    if (result < 0) {
	    ...
	    result = Blisrc_MultiChannel_Open(&mHandle,(void *)mInternalBuf, &mBliParam);
	    if(result >= 0) {
	        mInitCheck = OK;
	    } else {
...
int AudioResamplerMtk32::Blisrc::resample(void * inBuf, size_t *pInSize, void *outBuf, size_t * pOutSize)
{
    ...
    if(mBliParam.in_channel > 2){
    	value = Blisrc_MultiChannel_Process(mHandle, (char *)mTempBuf, (void *)inBuf, pInSize, (void *)outBuf, pOutSize);}
    else{
...
{
	if(mBliParam.in_channel > 2){
		return Blisrc_MultiChannel_Reset(mHandle);}
	else{
