...
public class AtParser {
    static final String LOG_TAG = "ATParser";
    ...
    private static final String MTKATCIRIL_PATTERN = "+MTKATCIJ";
    ...
    public AtCommandResult process(String raw_input) {
        ...
        if (!input.regionMatches(0, "AT", 0, 2)) {
            // Return ["ERROR"]
            Log.e(LOG_TAG,"ERROR, No 'AT' prefix.");
        ...
        while (index < input.length()) {
            ...
            if (isAtoZ(c)) {
                ...
                Log.d(LOG_TAG,"Basic Command:"+args);
                if (mBasicHandlers.containsKey((Character)c)) {
                    if (mBasicHandlers != null)
                    {
                    ...
                    // no handler
                    Log.d(LOG_TAG,"ERROR, No Basic Command");
                    result.addResult(
            ...
            if (c == '+' || c == '%') {
                ...
                Log.d(LOG_TAG,"mExtHandlers cmd ["+index+"]"+input+"+["+commandName+"]");

                if (!mExtHandlers.containsKey(commandName)) {
                    // no handler
                    Log.e(LOG_TAG,"ERROR, No command name in ExtHandler.");
                    result.addResult(
                            new AtCommandResult(AtCommandResult.ERROR));
                    return result;
                }
                ...
                Log.d(LOG_TAG,"endIndex="+endIndex);
                ...
                case TYPE_READ:
                    if (handler != null)
                    {
                        if(i + 1 < endIndex) {
                            try{
                                String subStr = input.substring(i + 1, endIndex);
                                String rspStr = input.substring(i+MTKATCIRIL_PATTERN.length()+1, endIndex);
                                if(subStr.startsWith(MTKATCIRIL_PATTERN)){
                                    args = new String[1];
                                    args[0] = rspStr;
                                    result.addResult(handler.handleReadCommand(args));
                                }else{
                ...
                case TYPE_TEST:
                    if (handler != null)
                    {
                        if(i + 2 < endIndex){
                            try{
                                String subStr = input.substring(i + 2, endIndex);
                                String rspStr = input.substring(i+MTKATCIRIL_PATTERN.length()+2, endIndex);
                                if(subStr.startsWith(MTKATCIRIL_PATTERN)){
                                    args = new String[1];
                                    args[0] = rspStr;
                                    result.addResult(handler.handleTestCommand(args));
                                }else{
                    ...
                    break;
                case TYPE_SET:
                    int isGenerateArgs = 0;
                    if(i + 1 < endIndex) {
                        String subStr = new String();
                        String rspStr = new String();
                        try{
                            subStr = input.substring(i + 1, endIndex);
                            rspStr = input.substring(i+MTKATCIRIL_PATTERN.length()+1, endIndex);
                        }catch(IndexOutOfBoundsException e){
                            Log.e(LOG_TAG,"IndexOutOfBoundsException: treat as normal mmi command.");
                            isGenerateArgs = 1;
                        }

                        if(0 == isGenerateArgs && subStr.startsWith(MTKATCIRIL_PATTERN)){
                            args = new String[1];
                            args[0] = rspStr;
                        }else{
                            args = generateArgs(input.substring(i + 1, endIndex));
                        }
                    }else{
                        args = generateArgs(input.substring(i + 1, endIndex));
                    }
                ...
                Log.d(LOG_TAG,"mExtHandlers,resultCode="+result.getResultCode()+",length="+input.length());
                if (result.getResultCode() != AtCommandResult.OK && result.getResultCode() != AtCommandResult.RILRSP) {
                    return result;   // short-circuit
                }
                if(result.getResultCode() == AtCommandResult.RILRSP) {
                    if(input.regionMatches(endIndex+1, "AT", 0, 2)){
                        Log.d(LOG_TAG,"regionMatches matches another RILESP."+input.substring(endIndex+1));
                        input = input.substring(endIndex+1);
                        index = 2;
                    }else{
                        Log.d(LOG_TAG,"regionMatches: No other RILESP matche.");
                        return result;
                    }
                }else{
                    index = endIndex;
