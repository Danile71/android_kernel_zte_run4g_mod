...
extern int g_HWUI_debug_dumpLayer;
extern int g_HWUI_debug_dumpTextureLayer;
...
static bool getProcessName(char* psProcessName, int size)
{
    ...
    char property[] = "debug.hwui.process.name";
    property_get(property, value, "0");
    if (strcmp(value, "0") != 0) {
        if (strcmp(value, psProcessName) == 0)
            return true;
        else {
            ALOGD("%s=%s, current=%s", property, value, psProcessName);
            return false;
        }
    }
...
static void setDebugLog(bool enable) {
    ...
    if (enable) {
        ...
        if (!getProcessName(procName, sizeof(procName))) {
            ALOGD("setHwuiLog doesn't work.");
        } else {
            for (uint32_t i = 0; i < e_HWUI_DEBUG_END; i++) {
                property_get(properties[i], value, "");
                if (value[0] != '\0') {
                    ALOGD("setHwuiLog: %s=%s", properties[i], value);
