...
public final class ActivityManagerPlus {
    ...
    /**
     * true if in IPO alarm boot
     */
    private boolean mAlarmBoot = false;
...
*/
    ...
    final void startHandler() {
        ...
        itFilter.addAction("android.intent.action.normal.boot");
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("android.intent.action.BOOST_DOWNLOADING".equals(intent.getAction())) {
                    ...
                    if (PowerOffAlarmUtility.isAlarmBoot()) {
                        ...
                        // restoreStates will be postponed to call when receving android.intent.action.normal.boot for alarm boot
                        stMgr.restoreStates(mContext);
                        if (!SystemProperties.get("ro.crypto.state", "unsupported").equals("encrypted")) {
                            Slog.i(TAG, "ipo BOOT_IPO: removeIPOWin");
                            mService.removeIPOWin(context);

                            Slog.i(TAG, "PMS wakeup");
                            PowerManager pm =
                ...
                } else if (mAlarmBoot && "android.intent.action.normal.boot".equals(intent.getAction())) {
                    ShutdownManager stMgr = new ShutdownManager();
                    stMgr.restoreStates(mContext);
                    mAlarmBoot = false; // reset AlarmBoot status
                }
            }
        }, itFilter);
    }
    ...
    */
        /// M: ALPS01538595 & ALPS01595887. After IPO boot, the recent app should be empty @{
    	synchronized(mService){
            List<ActivityManager.RecentTaskInfo> recentTaskInfoList = mService.getRecentTasks(1000,
                    ActivityManager.RECENT_WITH_EXCLUDED,UserHandle.USER_CURRENT);
    		
            int size = recentTaskInfoList.size();
    		
            for(int i = 0 ; i < size ; i++){
                ActivityManager.RecentTaskInfo info = recentTaskInfoList.get(i);
                if(info != null){
                    Intent intent = info.baseIntent;
                    if (intent != null
                            && intent.hasCategory(Intent.CATEGORY_HOME)){
                        Slog.i(TAG, "removeAllTasks: Don't remove home task. "
                            + intent);
                        continue;
                    }
                    Slog.i(TAG, "removeAllTasks name " + info.origActivity
                        + " /id=" + info.id);
                    mService.removeTask(info.persistentId, 0);
                }
            }
    	}
        /// @}
