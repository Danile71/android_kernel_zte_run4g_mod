...
status_t SFWatchDog::readyToRun() {
    ...
    // create rtt folder
    snprintf(cmds, sizeof(cmds), "mkdir %s", RTT_FOLDER_PATH);
    system(cmds);
...
bool SFWatchDog::threadLoop() {
    ...
    if (isSFThreadHang(ct)) {
        ...
        for (uint32_t i = 0; i < mNodeList.size(); i++) {
            if (mNodeList[i]->mStartTransactionTime == 0) {
                ...
                XLOGW("[SF-WD]   [%s] stop = %lld ms", mNodeList[i]->mName.string(), stopTime);
            }
        }
    ...
    snprintf(value, sizeof(value), "%lld", stopTime);
    if (stopTime >= 2147483247) {
        volatile nsecs_t tmpStopTime = stopTime;
        XLOGD("[SF-WD] tmpStopTime=(%lld, %lld)", tmpStopTime, stopTime);
        abort();
    }
