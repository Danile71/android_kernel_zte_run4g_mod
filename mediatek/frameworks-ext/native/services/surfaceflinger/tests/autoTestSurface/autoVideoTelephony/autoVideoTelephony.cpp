...
testConfig cfgWindow[][5] = {

    {
       ...
       { 1, SF_LAYER_1, { 0, 0, 0, 0, 0}, { 1,  1, 390, 1079, 1273}}, // 1080p VT call video layer
    ...
    
    {
      ...
      { 1, SF_LAYER_2, { 0, 0, 0, 0, 0}, { 1, 0, 1285, 600, 1776}}, // 1080p VT call video layer
...
status_t showTestFrame(ANativeWindow *w, const FRAME &f, int loop) { //, int repeat_times
    ...
    for (i = 0 ; i < loop ; i++){
        ...
        gb->lock(GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_HW_TEXTURE, rect, &ptr);
        {
            ...
            if (file != NULL)
            {
                if(f.fmt == HAL_PIXEL_FORMAT_YV12 || f.fmt == HAL_PIXEL_FORMAT_I420)
...
status_t main(int argc, char** argv) {
   ...
   //=============================================================================    
    ...
    //SurfaceComposerClient::openGlobalTransaction();
    //surfaceController[0]->setPosition(50, 250);
    //surfaceController[0]->setLayer(22000);
    //
    //surfaceController[1]->setPosition(400,600);  
    //surfaceController[1]->setLayer(23000);    
    //
    //
    //SurfaceComposerClient::closeGlobalTransaction();
