...
public class BleProfileServiceManager {
    ...
    /**
     * RET_ENABLED means background mode is enabled
     */
    public static final int RET_ENABLED = 1;
    /**
     * RET_DISABLED means background mode is disabled
     */
    public static final int RET_DISABLED = 0;
    /**
     * RET_DISABLED means background mode is unknown
     */
    public static final int RET_UNKNOWN = -1;

    /**
     * RET_SUCCESS means operation is successfully
     */
    public static final int RET_SUCCESS = 1;
    /**
     * RET_FAILURE means operation failed
     */
    public static final int RET_FAILURE = 0;
    /**
     * RET_FAIL_IN_BACKGROUND_MODE means it's already in background mode
     */
     ...
     *
     *
     * @return RET_ENABLED, if background mode is on
     *         RET_DISABLED,if background mode is off
     *         RET_FAILURE, if some errors occur
     */
    public int getBackgroundMode() {
        int nRet = RET_ENABLED;
        ...
        try {
            nRet = (mService.isBackgroundModeEnabled() ? RET_ENABLED
                    : RET_DISABLED);
        } catch (final RemoteException e) {
            nRet = RET_FAILURE;
            Log.e(TAG, e.toString());
     ...
     *
     *
     * @return RET_SUCCESS,if this operation starts
     *         RET_FAILURE,if this operation fails
     *         RET_FAIL_IN_BACKGROUND_MODE, if it's in background mode
     */
    public int launchServices() {
        ...
        if (DBG) {
            Log.d(TAG, "launchServices: nRet:" + nRet);
        }
        return nRet;
    }
    ...
    public int shutdownServices() {
        ...
        try {
            nRet = mService.shutdownServices();
        } catch (final RemoteException e) {
            nRet = RET_FAILURE;
            Log.e(TAG, e.toString());
        }
        ...
        if (DBG) {
            Log.d(TAG, "shutdownServices: nRet:" + nRet);
        }
        return nRet;
    }
