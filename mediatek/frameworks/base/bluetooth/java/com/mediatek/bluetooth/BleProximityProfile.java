...
public class BleProximityProfile extends BleProfile {
    ...
    private int mProfileID = BleProfile.BLE_PROFILE_PXP;
    ...
    IBleGattCallback mCallback = new IBleGattCallback() {
        ...
        public void onCharacteristicRead(IBleGatt gatt, BluetoothGattCharacteristic characteristic,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS && characteristic != null) {
                int txPower = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_SINT8,
                        0);
                if (VDBG) Log.v(TAG, "onCharacteristicRead: TxPower=" + txPower);
                mProfileCallback.onTxPowerRead(status, txPower, gatt.getDevice());
            } else {
                return;
            }
        }
    ...
    void open() {
        ...
        this.mGattClientIf.registerClientCallback(BleProfile.BLE_PROFILE_PXP, mCallback);
    }
    ...
    void close() {
        ...
        this.mGattClientIf.unregisterClientCallback(BleProfile.BLE_PROFILE_PXP);
    }
    ...
    public boolean setLinkLostAlertLevel(int level) {
        ...
        if (level < LINK_LOST_ALERT_NO || level > LINK_LOST_ALERT_HIGH) {
            return false;
        }
