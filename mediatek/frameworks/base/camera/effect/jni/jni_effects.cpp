        ...
        jstring effectName)
{
    ...
    if (effectName != NULL)
    {
        ...
        s8EffectName = String8(pName, env->GetStringLength(effectName)); 
        ...
        jobject faceBeautyEffectObject)
{
    ...
    cs->connectEffect(String8("faceBeauty"), g_effectClientFB); 
    // 
    if (g_effectClientFB == 0)
    {
...
* 
*******************************************************************************/
bool convertJavaImageToNative(JNIEnv* env, jobject image, ImageInfo *imgInfo, uint32_t *buffer[3])
{
    ...
    for (int i = 0; i < numOfPlane; i++)
    {
        ...
        // get buffer pointer
        buffers[i] = static_cast<uint32_t*>(env->GetDirectBufferAddress(g_plane.byteBuffer));
        if (buffers[i] == NULL) {
        ...
        jobject srcImage, jobject targetImage)
{
    ...
    MY_LOGD("src addr(0x%x, 0x%x, 0x%x", (uint32_t)src_buffer[0], (uint32_t)src_buffer[1], (uint32_t)src_buffer[2]); 

    //
    ImageInfo rDestImgInfo; 
    uint32_t *dest_buffer[3] = {0, 0, 0}; 
    ...
    MY_LOGD("dst addr(0x%x, 0x%x, 0x%x", (uint32_t)dest_buffer[0], (uint32_t)dest_buffer[1], (uint32_t)dest_buffer[2]); 
