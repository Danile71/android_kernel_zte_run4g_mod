...
public final class MtkHdmiManagerService extends IMtkHdmiManager.Stub {
    ...
    private void handleCallStateChanged(int state) {
        ...
        if (state == TelephonyManager.CALL_STATE_OFFHOOK) {
            ...
            if (isSignalOutputting()) {
                String contentStr = mContext.getResources().getString(R.string.hdmi_mutex_call_content);
                if (getDisplayType() == IHDMINative.DISPLAY_TYPE_MHL) {
        ...
        if (mInitialized) {
            if (mIsHdVideoPlaying && mCablePlugged) {
                ...
                if (type != IHDMINative.DISPLAY_TYPE_SMB) {
                    ...
                    log(TAG, "mIsHdVideoPlaying: " + mIsHdVideoPlaying + " mHdVideoRestore: " + mHdVideoRestore);
                    enableHdmi(false);
                    return;
                }
    ...
    private void unregisterCallListener() {
        if (hasCapability(IHDMINative.CAPABILITY_MUTEX_CALL) && mTelephonyManager != null) {
            mTelephonyManager.listen(mPhoneStateListener, PhoneStateListener.LISTEN_NONE);
        }
    }

    private void registerCallListener() {
        if (hasCapability(IHDMINative.CAPABILITY_MUTEX_CALL)) {
    ...
    private void handleNotification(boolean plugged) {
        ...
        if (plugged) {
            ...
            if (IHDMINative.DISPLAY_TYPE_MHL == getDisplayType()) {
    ...
    private int[] getSupportedResolutionsImpl() {
        ...
        if (FeatureOption.MTK_INTERNAL_HDMI_SUPPORT
                || FeatureOption.MTK_INTERNAL_MHL_SUPPORT) {
            return HdmiDef.getAllResolutions();
        } else if (FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
            resolutions = getDefaultSupportedResolutions();
        } else {
    ...
    private void getCapabilities() {
        synchronized (mHdmiNative) {
            mCapabilities = mHdmiNative.getCapabilities();
    ...
    private static class HdmiDef {
        ...
        public static int[] getPreferedResolutions() {
            ...
            if (FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
                ...
                prefered = new int[] { AUTO + RESOLUTION_1280X720P_60HZ,
                        AUTO + RESOLUTION_1920X1080P_30HZ,
                        AUTO + RESOLUTION_1920X1080P_60HZ,
                        AUTO + RESOLUTION_720X480P_60HZ };
            }
            return prefered;
        }
