...
static jint getCapabilities(JNIEnv* env, jobject clazz) {
    jint result = 0;
#if defined (MTK_HDMI_SUPPORT)
    if (hdmi_ioctl(MTK_HDMI_GET_CAPABILITY, (int)&result) == false) {
        result = 0;
    }
#endif
    ALOGI("[HDMI] JNI com.mediatek.hdmi.HDMINative.getCapabilities(%d)\n", result);
    return result;
}
...
//    { "GetDeviceStatus","(ZZ)Z", 	(void*)GetDeviceStatus},
    ...
    { "getCapabilitiesNative",     "()I",       (void*)getCapabilities},
};
