...
package com.mediatek.lomoeffect;
...
public class LomoEffect {
    private static final String TAG = "LomoEffect_Framework";
    private EventHandler mEventHandler;
    private static LomoEffect sLomoEffect;
    ...
    LomoEffect() {
        ...
        native_setup(new WeakReference<LomoEffect>(this));
    }
    
    public static LomoEffect openLomoEffect() {
        if (sLomoEffect == null) {
            sLomoEffect = new LomoEffect();
        }
        return sLomoEffect;
    }
    ...
    public void setEffectsCallback(EffectsCallback listener) {
        mEffectsListener = listener;
    }
    
    public void setSurfaceToNative(Surface surface, int surfaceNumber) {
        native_setSurfaceToNative(surface, surfaceNumber);
    }
    
    public void displayEffect(byte[] effectData, int surfaceNumber) {
        native_displayEffect(effectData, surfaceNumber);
    }
    
    public void initializeEffect(int previewWidth, int previewHeight, 
    		int effectNumOfPage, int format) {
        native_initializeEffect(previewWidth, previewHeight, effectNumOfPage, format);
    }
    
    public void registerEffectBuffers(int bufferWidth, int bufferHeight, byte[][] buffers) {
        native_registerEffectBuffers(bufferWidth, bufferHeight, buffers);
    }
    
    public void processEffect(byte[] previewData, int[] effectId) {
    ...
    private static void postEventFromNative(Object lomoeffect_pref, int what) {
        LomoEffect le = (LomoEffect)((WeakReference)lomoeffect_pref).get();
