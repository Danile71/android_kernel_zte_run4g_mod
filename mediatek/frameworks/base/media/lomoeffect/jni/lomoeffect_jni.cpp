#define LOG_TAG "Lomo_J_v0508_02"  //reduce log msg
#include <string.h>
...
uint32_t u32lEffectBuffARRAY[12][3];
...
//	ALOGI("writeEffectDataToBuffers, size:%d, g_bufferCurson:%d,g_effectNumOfPage : %d", g_EffectBuffers.size(),g_bufferCurson,g_effectNumOfPage);
    ...
    for (int i = start; i < start + g_effectNumOfPage; i++) {
        ...
        if (1) {
            ...
            uint32_t u8lEffectBuff[3];
            int effectId=buffer->m_effectId;
            u8lEffectBuff[0]=(uint32_t)dstBuffer;
            u8lEffectBuff[1]=(uint32_t)(dstBuffer+(size*4/6));
            u8lEffectBuff[2]=(uint32_t)(dstBuffer+(size*5/6));
            //            ALOGD("effectId = %d",effectId);
            #ifdef MTK_CAM_LOMO_SUPPORT     
            if(bAddrDumpEnable)
            {
                ALOGD("Enque i-start, effect, addr (g_bufferCurson)=  %d, %d, 0x%8x (%d)",i - start, effectId,(uint32_t)dstBuffer,g_bufferCurson );
            }
            #endif
            if(effectId!=(-1))
            {
    ...
    for (int i = start; i < start + g_effectNumOfPage; i++)
    {
        if( u32lEffectIdx[i - start]!=(-1))
        {
            if(bAddrDumpEnable)
            {
                ALOGD("Do deque, i, effectidx, Addr(%d, %d, 0x%8x)",i, u32lEffectIdx[i - start],u32lEffectBuffARRAY[i - start][0]);
            }
        ...
        else
        {
            if(bAddrDumpEnable)
            {
                ALOGD("No deque, i, effectidx, Addr(%d, %d, 0x%8x) No flush addr",i, u32lEffectIdx[i - start],u32lEffectBuffARRAY[i - start][0]);   
            }
        }
    }
    #elif 0
...
void processEffect(JNIEnv *env, jclass thiz, jbyteArray data, jintArray ids)
{
    ...
    #ifdef MTK_CAM_LOMO_SUPPORT
    ALOGD("previewBuffer 0x%x,0x%x",(uint32_t)previewBuffer,(uint32_t)previewBuffer);
