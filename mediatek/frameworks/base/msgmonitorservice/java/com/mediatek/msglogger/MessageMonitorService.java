...
public class MessageMonitorService extends IMessageLogger.Stub {
    ...
    private final class MSGLoggerDeathRecipient implements IBinder.DeathRecipient {
        final String currentMSGLoggerName;

        MSGLoggerDeathRecipient(String msgloggername) {
            Log.d(TAG,"MSGLoggerDeathRecipient :"+msgloggername);
            currentMSGLoggerName = msgloggername;
        }
        ...
        public void binderDied() {
            ...
            try {
                while(iter.hasNext()) 
                {
                    ...
                    if( currentMSGLoggerName.equals(key.msgLoggerName) )
                    {
                        Log.d(TAG,"MSGLoggerDeathRecipient - UnregisterMsgLogger:"+currentMSGLoggerName);
                        sMSGLoggerMap.remove(key);
                        break;
                    }
                }
            } catch(Exception e) {
                Log.d(TAG,"MSGLoggerDeathRecipient - UnregisterMsgLogger fail "+e);
            }
        }
    }
    ...
    public void registerMsgLogger(String msgLoggerName, int pid, int tid, IMessageLoggerWrapper callback) {
        ...
        MSGLoggerDeathRecipient mDeathRecipient = new MSGLoggerDeathRecipient(msgLoggerName);
        try {
