...
#include <sensorhub/SensorCondition.h>
#include <sensorhub/SensorAction.h>
#include <sensorhub/SensorContext.h>
#include <sensorhub/SensorData.h>
#include <sensorhub/SensorHubManager.h>
...
{
    SensorHubManager* const p = (SensorHubManager*)env->GetIntField(thiz, gServiceOffsets.context);
...
{
    sp<SensorHubManager> old = (SensorHubManager*)env->GetIntField(thiz, gServiceOffsets.context);
    if (mgr.get()) {
    ...
    env->SetIntField(thiz, gServiceOffsets.context, (int)mgr.get());
    return old;
}

static sp<SensorTriggerListener> getSensorTriggerListener(JNIEnv* env, jobject thiz)
{
    SensorTriggerListener* const p = (SensorTriggerListener*)env->GetIntField(thiz, gServiceOffsets.listener_context);
...
{
    sp<SensorTriggerListener> old = (SensorTriggerListener*)env->GetIntField(thiz, gServiceOffsets.listener_context);
    if (listener.get()) {
    ...
    env->SetIntField(thiz, gServiceOffsets.listener_context, (int)listener.get());
    return old;
}
...
static void nativeInit(JNIEnv *env)
{
    ...
    gServiceOffsets.context = env->GetFieldID(clazz, "mNativeContext", "I");
    if (gServiceOffsets.context == NULL) {
        ALOGE("nativeInit: failed! No mNativeContext field!");
        return;
    }
    gServiceOffsets.listener_context = env->GetFieldID(clazz, "mListenerContext", "I");
