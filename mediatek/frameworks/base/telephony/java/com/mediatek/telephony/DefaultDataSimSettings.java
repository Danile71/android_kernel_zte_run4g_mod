...
public class DefaultDataSimSettings {
    ...
    public static void setDataDefaultSim(Context context, ContentResolver contentResolver, List<SimInfoRecord> simInfos,
        ...
        logd("setDataDefaultSim: nNewCardCount = " + nNewCardCount + ", nSimCount = " + nSimCount
                + ",n3gSIMSlot = " + n3gSIMSlot + ", telephonyExt = " + telephonyExt);
        if (!FeatureOption.MTK_BSP_PACKAGE) {
            ...
            if (nSimCount > 1) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    if (oldGprsDefaultSIM == Settings.System.DEFAULT_SIM_NOT_SET) {
                        // Only enable data on CDMA SIM for EVDO project.
                        if (telephonyExt.isDefaultDataOn()
                                && (!FeatureOption.EVDO_DT_SUPPORT || simInfos.get(0).mSimSlotId == n3gSIMSlot)) {
                    ...
                    if (oldGprsDefaultSIM == Settings.System.DEFAULT_SIM_NOT_SET) {
                        boolean enabled = Settings.Secure.MOBILE_DATA_DEFAULT == 1 ? true : false;
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) { 
                ...
                if (isSimRemoved(oldGprsDefaultSIM,simIdForSlot,PhoneConstants.GEMINI_SIM_NUM) && nSimCount > 0) {
                    boolean enabled = Settings.Secure.MOBILE_DATA_DEFAULT == 1 ? true : false;
                    logd("set data to : " + enabled + ", op default data on: " + telephonyExt.isDefaultDataOn());
                    connectivityManager.setMobileDataEnabled(enabled);
                } else if ((nNewCardCount > 0 || (hasSimRemoved && nSimCount > 0)) && telephonyExt.isDefaultDataOn()){
                    logd("set data to : new card on ");
                    connectivityManager.setMobileDataEnabled(telephonyExt.isDefaultDataOn());
                }
            }
            ...
            logd("setDataDefaultSim 1: gprsDefaultSIM = " + gprsDefaultSIM + ", data enable = " + connectivityManager.getMobileDataEnabled());
            if (gprsDefaultSIM != Settings.System.DEFAULT_SIM_NOT_SET
                    && gprsDefaultSIM != Settings.System.GPRS_CONNECTION_SIM_SETTING_NEVER
                    && (!FeatureOption.EVDO_DT_SUPPORT || connectivityManager.getMobileDataEnabled())) {
            ...
            gprsDefaultSIM = Settings.System.getLong(contentResolver, Settings.System.GPRS_CONNECTION_SIM_SETTING, Settings.System.DEFAULT_SIM_NOT_SET);
            logd("final data settings: " + gprsDefaultSlot + ", default gprs sim: " + gprsDefaultSIM);
        } else {
            ConnectivityManager connectivityManager = (ConnectivityManager)context.getSystemService(context.CONNECTIVITY_SERVICE);
            int gprsDefaultSlot = Settings.System.getInt(contentResolver, Settings.System.GPRS_CONNECTION_SETTING, Settings.System.GPRS_CONNECTION_SETTING_DEFAULT) - 1;
    ...
    */                        
    ...
    private static boolean isSimRemoved(long defSimId,long[] curSim, int numSim) {     
        ...
        logd("isSimRemoved: " + isDefaultSimRemoved + ", defSimId: " + defSimId + ", numSim: " + numSim);
        return isDefaultSimRemoved;
    }

    private static int get3GSimId() {
        if (FeatureOption.MTK_GEMINI_3G_SWITCH || FeatureOption.MTK_SIM_SWITCH) {
