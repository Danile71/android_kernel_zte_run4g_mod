...
public class DefaultVtSimSettings {
    ...
    public static void setVtDefaultSim(ContentResolver contentResolver,
        ...
        if (!FeatureOption.MTK_BSP_PACKAGE && FeatureOption.MTK_VT3G324M_SUPPORT) {
            ...
            int n3gSIMSlot = getDefaultVtSlotId();

            if (isSupportSimSwitch()) {
    ...
    private static int getDefaultVtSlotId() {
        if (isSupportSimSwitch()) {
            ITelephonyEx iTelephonyEx = ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICEEX));
            try {
                if (iTelephonyEx != null) {
                    int slotId = iTelephonyEx.getHsCapabilitySIM();
                    logd("getDefaultVtSlotId() + slotId : " + slotId);
                    if ((slotId >= 0) && (slotId < PhoneConstants.GEMINI_SIM_NUM)) {
                        //Property value shall be 1~4, convert to PhoneConstants.GEMINI_SIM_x
                        return slotId;
                    } else {
                        logd("getDefaultVtSlotId() got invalid property value:" + slotId);
                    }
                }
            } catch (RemoteException e) {
                Log.e(LOG_TAG, "iTelephonyEx exception");
            }
        }
        return PhoneConstants.GEMINI_SIM_1;
    }

    private static boolean isSupportSimSwitch() {
        return FeatureOption.MTK_GEMINI_3G_SWITCH || FeatureOption.MTK_SIM_SWITCH;
    }
