...
public class InternationalRoamingController extends Handler implements
    ...
    // System properties used for test, dynamically change home network mcc and
    // switch phone delay time.
    private static final String PROPERTY_TEST_HOME_NETWORK_MCC = "test.mtk.home.network.mcc";
    private static final String PROPERTY_TEST_SWITCH_PHONE_DELAY = "test.mtk.switch.phone.delay";
    ...
    // Block switch phone for some reason.
    private boolean mBlockSwitchPhone;
    ...
    private int mCurrentNetworkMode = InternationalRoamingConstants.NW_MODE_INTERNATIONAL;
    ...
    public InternationalRoamingController(Context context, Object dualModePhone, Object gsmPhone,
        ...
        mCurrentNetworkMode = getNetworkMode();
    }

    @Override
    public int switchPhone(int mode, boolean fromUser) {
        logd("switchPhone: mode = " + mode + ", fromUser = " + fromUser + ", mBlockSwitchPhone = "
                + mBlockSwitchPhone + ", phone type = " + mDualModePhone.getPhoneType());

        if (fromUser) {
            mHasSearchedOnGsm = false;
            mHasSearchedOnCdma = false;
            mIgnoreSearchedState = true;
            // Reset block switch phone flag, prevent user choose a forbidden
            // network and enter manual network selection again caused issues.
            mBlockSwitchPhone = false;
        }

        if (mBlockSwitchPhone) {
            logd("switchPhone failed because all switch phone message is blocked.");
            return InternationalRoamingConstants.SIM_SWITCH_RESULT_ERROR_BLOCKED;
        }
        ...
        if (noNeedSwitchPhoneInCdmaOnlyGsmOnly(mode)) {
            return InternationalRoamingConstants.SIM_SWITCH_RESULT_ERROR_IN_ONLY_C_G_MODE;
        }
    ...
    @Override
    public void blockSwitchPhone(boolean block) {
        mBlockSwitchPhone = block;
    }
    ...
    public boolean isHomeNetwork(String mcc) {
        ...
        String testMcc = SystemProperties.get(PROPERTY_TEST_HOME_NETWORK_MCC, "");
        if (!TextUtils.isEmpty(mcc) && !TextUtils.isEmpty(testMcc)) {
        ...
        switch (msg.what) {
            ...
            case EVENT_SERVICE_STATE_CHANGED:
                ...
                logd("EVENT_SERVICE_STATE_CHANGED: regState = " + regState
                        + ", mBlockSwitchPhone = " + mBlockSwitchPhone);
                ...
                if (regState == ServiceState.REGISTRATION_STATE_HOME_NETWORK
                    ...
                    if ((regState == ServiceState.REGISTRATION_STATE_NOT_REGISTERED_AND_NOT_SEARCHING 
                            || isRegDeniedWithNoCellAvaiable(regState))
    ...
    /**
     * If the service state is Denied and lac is 0 and cid is 0 for the first no service condition to switch phone.
     */
    private boolean isRegDeniedWithNoCellAvaiable(int regState) {
        boolean regDeniedWithNoCellAvaiable = false;
        if (mDualModePhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM
                && (regState == ServiceState.REGISTRATION_STATE_REGISTRATION_DENIED)) {
            GsmCellLocation gsmCellLoc = (GsmCellLocation)((GSMPhone)mDualActivePhone).getCellLocation();
            int lac = gsmCellLoc.getLac();
            int cid = gsmCellLoc.getCid();
            logd("isRegDeniedWithNoCellAvaiable: lac = " + lac + " cid = " + cid);
            if (lac == 0 && cid == 0) {
                regDeniedWithNoCellAvaiable = true;
            }
        }
        logd("isRegDeniedWithNoCellAvaiable: regDeniedWithNoCellAvaiable = " + regDeniedWithNoCellAvaiable);
        return regDeniedWithNoCellAvaiable;
    }
    ...
    private int calcSwitchPhoneDelayedTime() {
        ...
        final int testDelayTime = SystemProperties.getInt(PROPERTY_TEST_SWITCH_PHONE_DELAY, -1);
        if (testDelayTime != -1) {
            delayedTime = testDelayTime;
        } else if (System.currentTimeMillis() - mRadioRebootTime < SPEED_UP_SWITCH_DURATION) {
            // Switch phone directly to speed up network register time if device
            // first booted or radio on from airplane mode.
            delayedTime = 0;
            logd("calcSwitchPhoneDelayedTime: SPEED_UP_SWITCH_DURATION = "
                    + SPEED_UP_SWITCH_DURATION + " mRadioRebootTime = " + mRadioRebootTime);
        } else {
            ...
            delayedTime = powerManager.isScreenOn() ? RADIO_NO_SERVICE_SCREENON_TIMEOUT
                    : RADIO_NO_SERVICE_SCREENOFF_TIMEOUT;
        }
        logd("calcSwitchPhoneDelayedTime: testDelayTime = " + testDelayTime + ", delayedTime = "
                + delayedTime);
        return delayedTime;
    }
    ...
    private void finishToSwitchPhone() {
        ...
        PhoneFactory.setDualtalkMode(InternationalRoamingConstants.SWITCHING_TELEPHONYMODE_DONE);
    }
    ...
    private void loadDualPhoneImsiCompleted(int dualSimId, String cdmaImsi, String gsmImsi) {
        ...
        // IR Settings uses this system setting.
    ...
    private class SimSwithHandler extends Handler {
        ...
        private static final String PROPERTY_MUX_REPORT_CASE = "mux.report.case";
            ...
            switch (msg.what) {
                ...
                case EVENT_SWITCH_TELEPHONY_MODE:
                    if (mChangeToGemini) {
                        ...
                        PhoneFactory.setDualtalkMode(InternationalRoamingConstants.SWITCHING_TELEPHONYMODE_NOT_DUALTALKMODE);
                    } else {
                        logd("[SSH] EVENT_SWITCH_SIM_INTERFACE: GSM -> CDMA.");
                        setTelephonyMode(mDualSimId, SIM_SWITCH_MODE_CDMA,
                                obtainMessage(EVENT_SWITCH_RIL_SOCKET));
                        PhoneFactory.setDualtalkMode(InternationalRoamingConstants.SWITCHING_TELEPHONYMODE_DUALTALKMODE);
                    }
                    break;
                ...
                case EVENT_SWITCH_RIL_SOCKET:
                    ...
                    if (mChangeToGemini) {
                        ...
                        if (mDualModePhone.isWCDMAPrefered()) {
                            mDualActivePhone.mCi.switchRilSocket(Phone.NT_MODE_WCDMA_PREF, mDualSimId);
                        } else {
                            mDualActivePhone.mCi.switchRilSocket(Phone.NT_MODE_GSM_ONLY, mDualSimId);
                        }
                    } else {
                ...
                case EVENT_SWITCH_PHONE_INSTANCE:
                    ...
                    // [WR] Work around, the EVENT_SWITCH_PHONE_INSTANCE maybe
                    // received twice when switching phone very seldomly, caused
                    // terrible failure of the same message instance sent twice,
                    // we only send the message when undering switching.
                    if (!mUnderSimSwitching) {
                        logd("[SSH] Handle abnormal case, EVENT_SWITCH_PHONE_INSTANCE message sent twice!");
                        return;
                    }
                    ...
                    // Set modem reboot mode, make the device silent boot without inputting PIN.
                    SystemProperties.set(PROPERTY_MUX_REPORT_CASE, String.valueOf(2));
                    finishToSwitchPhone();
    ...
    private final BroadcastReceiver mAirplaneModeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!TextUtils.isEmpty(action) && action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
                final boolean enabled = intent.getBooleanExtra("state", false);
                logd("Airplane mode changed to " + enabled + ", mBlockSwitchPhone = " + mBlockSwitchPhone);
    ...
    /**
     * Save the network mode to Settings.
     * 
     * @param mode The network mode.
     */
    private void saveCurrentNetworkMode(final int mode) {
        logd("setCurrentNetworkMode: mode =" + mode);
        Settings.System.putInt(mContext.getContentResolver(), CURRENT_NETWORK_MODE, mode);
    }

    /**
     * Get the saved network mode. Default value is
     * InternationalCardConstants.NW_MODE_INTERNATIONAL.
     * 
     * @return the latest mode.
     */
    private int getSavedNetworkMode() {
        int mode = Settings.System.getInt(mContext.getContentResolver(), CURRENT_NETWORK_MODE,
                InternationalRoamingConstants.NW_MODE_INTERNATIONAL);
        logd("getCurrentNetworkMode: mode = " + mode);
        return mode;
    }

    /**
     * Set the net work mode, if set the cdma only and gsm only, try to switch
     * phone. If set the international mode, only set the mode and do not switch
     * phone.
     * 
     * @param networkMode The network mode.
     */
    public void setNetworkMode(int networkMode) {
        logd("setNetworkMode: networkMode = " + networkMode);
        mCurrentNetworkMode = networkMode;
        if (networkMode == InternationalRoamingConstants.NW_MODE_CDMA_ONLY) {
            switchPhone(InternationalRoamingConstants.SIM_SWITCH_MODE_CDMA, true);
        } else if (networkMode == InternationalRoamingConstants.NW_MODE_GSM_ONLY) {
            switchPhone(InternationalRoamingConstants.SIM_SWITCH_MODE_GSM, true);
        }
        saveCurrentNetworkMode(networkMode);
    }

    /**
     * Get the network mode from the saved network mode.
     * 
     * @return the get saved network mode.
     */
    public int getNetworkMode() {
        return getSavedNetworkMode();
    }

    /**
     * If the network selection mode is CDMA only or WCDMA(GSM) and target mode
     * is not the same, do not need switch phone.
     * 
     * @param mode target switch mode.
     * @return Whether need to switch phone.
     */
    private boolean noNeedSwitchPhoneInCdmaOnlyGsmOnly(int mode) {
        boolean noNeedSwitchPhone = false;
        final int phoneType = mDualModePhone.getPhoneType();
        if (((mCurrentNetworkMode == InternationalRoamingConstants.NW_MODE_CDMA_ONLY) 
                && (phoneType == PhoneConstants.PHONE_TYPE_CDMA))
                || ((mCurrentNetworkMode == InternationalRoamingConstants.NW_MODE_GSM_ONLY) 
                && (phoneType == PhoneConstants.PHONE_TYPE_GSM))) {
            noNeedSwitchPhone = true;
        }
        logd("noNeedSwitchPhoneInCdmaOnlyGsmOnly: noNeedSwitchPhone = " + noNeedSwitchPhone);
        return noNeedSwitchPhone;
    }
}
