...
public class LteWorldPhoneOp01 extends Handler implements IWorldPhone {
    ...
    private boolean isAllowCampOn(String plmnString, int slotId) {
        ...
        if (sUserType == sType1User || sUserType == sType2User) {
            ...
            sWaitForDesignateService = false;
            if (mdType == LteModemSwitchHandler.MD_TYPE_LTG
                    || mdType == LteModemSwitchHandler.MD_TYPE_TG) {
                sDenyReason = CAMP_ON_NOT_DENIED;
                logd("Camp on OK, RAT=2G only");
    ...
    private void handlePlmnChange(AsyncResult ar, int slotId, boolean isDcEvent) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            if (sMajorSlot == slotId && sWaitForDesignateService
                    && sDenyReason != DENY_CAMP_ON_REASON_NEED_SWITCH_TO_FDD) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                if (simStatus.equals(IccCardConstants.INTENT_VALUE_ICC_IMSI)) {
                    ...
                    if (sMajorSlot == NO_3G_CAPABILITY) {
                        ...
                        logd("3/4G capability off. protocolSim: " + protocolSim);
                        if (slotId == protocolSim
                                && getUserType(sImsi[slotId]) == sType3User) {
                            if (sIsLteDcSupport) {
                ...
                if (sDefaultBootuUpModem == LteModemSwitchHandler.MD_TYPE_TDD) {
                    if (sIsLteDcSupport) {
    ...
    private void handleSwitchModem(int toModem) {
        ...
        if (sMajorSlot == AUTO_SELECT_DISABLE
                || sDefaultBootuUpModem == LteModemSwitchHandler.MD_TYPE_UNKNOWN) {
    ...
    public int onNetworkModeChanged(int currentNwMode, int newNwMode) {
        ...
        if (sMajorSlot == AUTO_SELECT_DISABLE
            ...
            if (sIsLteDcSupport && (sUserType == sType1User || sUserType == sType2User)
        ...
        if (getModemSelectionMode() == SELECTION_MODE_AUTO) {
            if (majorSim == NO_3G_CAPABILITY) {
                ...
                if (sUserType == sType1User || sUserType == sType2User) {
                    ...
                    setRatMode(Phone.NT_MODE_GSM_ONLY, slotId);
                    logd("[setDesiredRatMode]-");
                    return;
                } else {
