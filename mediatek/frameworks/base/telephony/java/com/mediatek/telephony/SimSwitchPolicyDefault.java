...
/**
 * Sim Switch Policy (MTK Default)
 * 
 * @hide
 */
public class SimSwitchPolicyDefault implements ISimSwitchPolicy {

    private static final String LOG_TAG = "SimSwitchPolicyDefault";
    ...
    public int selectCapabilitySim(Context ctx, int capability, String iccId3GSim, int id3GSim, String iccId1, String iccId2, String iccId3, String iccId4) {
        ...
        Log.i(LOG_TAG, "selectCapabilitySim (iccId3GSim="+iccId3GSim+", capability="+capability+", id3GSim="+id3GSim+", iccId1="+iccId1+", iccId2="+iccId2+", iccId3="+iccId3+", iccId4="+iccId4+")");
        ...
        if (iccId3GSim != null) {
            ...
            if ((id3GSim == PhoneConstants.GEMINI_SIM_1) && (iccId3GSim.equals(iccId1))) {
                Log.i(LOG_TAG, "Found capability SIM at same slot (SIM1), stays.");
                selected = ISimSwitchPolicy.CAPABILITY_NOCHANGE;
            } else if ((id3GSim == PhoneConstants.GEMINI_SIM_2) && (iccId3GSim.equals(iccId2))) {
                Log.i(LOG_TAG, "Found capability SIM at same slot (SIM2), stays.");
                selected = ISimSwitchPolicy.CAPABILITY_NOCHANGE;
            } else if ((id3GSim == PhoneConstants.GEMINI_SIM_3) && (iccId3GSim.equals(iccId3))) {
                Log.i(LOG_TAG, "Found capability SIM at same slot (SIM3), stays.");
                selected = ISimSwitchPolicy.CAPABILITY_NOCHANGE;
            } else if ((id3GSim == PhoneConstants.GEMINI_SIM_4) && (iccId3GSim.equals(iccId4))) {
                Log.i(LOG_TAG, "Found capability SIM at same slot (SIM4), stays.");
                selected = ISimSwitchPolicy.CAPABILITY_NOCHANGE;
            } else if (id3GSim == -1) {
                Log.i(LOG_TAG, "Found capability SIM is OFF, stays.");
                selected = ISimSwitchPolicy.CAPABILITY_NOCHANGE;
            }
            
            if (selected == ISimSwitchPolicy.CAPABILITY_INVALID) { // check if not decided yet.
            ...
            if (selected == ISimSwitchPolicy.CAPABILITY_INVALID) { // check if not decided yet.
                    ...
                    (id3GSim == PhoneConstants.GEMINI_SIM_4 && iccId4 == null))
                {
                    ...
                    if (iccId1 != null) {
                        Log.i(LOG_TAG, "scenario 2: Need to switch capability to the other inserted SIM1");
                        selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM1;
                    } else if (iccId2 != null) { 
                        Log.i(LOG_TAG, "scenario 2: Need to switch capability to the other inserted SIM2");
                        selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM2;
                    } else if (iccId3 != null) { 
                        Log.i(LOG_TAG, "scenario 2: Need to switch capability to the other inserted SIM3");
                        selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM3;
                    } else if (iccId4 != null) { 
                        Log.i(LOG_TAG, "scenario 2: Need to switch capability to the other inserted SIM4");
                        selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM4;
                    }
                }
            }
            ...
            if (iccId1 == null) {
                if (iccId2 != null) {
                    Log.i(LOG_TAG, "scenario 3: Need to switch capability to the inserted SIM2");
                    selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM2;
                } else if (iccId3 != null) { 
                ...
                Log.i(LOG_TAG, "found SIM1. stays.");
                selected = ISimSwitchPolicy.CAPABILITY_GEMINI_SIM1;
            }
        }
