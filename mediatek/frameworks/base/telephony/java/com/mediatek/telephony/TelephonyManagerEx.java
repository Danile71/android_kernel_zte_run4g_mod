...
public class TelephonyManagerEx {
    ...
    public int getNetworkType(int simId) {
        if(getDataNetworkType(simId)!= TelephonyManager.NETWORK_TYPE_UNKNOWN)
            return getDataNetworkType(simId);
        else
            return getVoiceNetworkType(simId);	// return voice network type for better backward compatible 		
    }
   ...
   /**
    ...
    public boolean setScAddress(String address, int slotId) {
        try {
            ...
            if (telephonyEx != null) {
                telephonyEx.setScAddressGemini(address, slotId);
            }
            return true;
        } catch(RemoteException e1) {
            e1.printStackTrace();	   
