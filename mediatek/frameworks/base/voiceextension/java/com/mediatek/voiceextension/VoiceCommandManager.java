...
public class VoiceCommandManager {
    ...
    private static String TAG = "VieCmdMgr";
    ...
    private void handleServiceDisconnected(boolean notifyApp) {
        ...
        if (notifyApp && mCurListener != null) {
            mCurListener.onError(VoiceCommonState.SERVICE_DISCONNECTTED);
        }
        ...
        if (result == VoiceCommonState.SUCCESS) {
            if (getService() != null) {
                try {
                    ...
                    // TODO Auto-generated catch block
                    result = VoiceCommonState.SERVICE_DISCONNECTTED;
                    handleServiceDisconnected(false);
                    Log.e(TAG, "create Command Set error " + e.toString());
                }
            } else {
                result = VoiceCommonState.SERVICE_NOT_EXIST;
            }
        }
    ...
    /**
     * Sets up the commands list to voice command service.
     * 
            ...
            if (getService() != null) {
                try {
                    ...
                    mHandler.sendMessage(mHandler.obtainMessage(
                            ...
                            VoiceCommonState.SERVICE_DISCONNECTTED, 0));
                    handleServiceDisconnected(false);
                ...
                mHandler.sendMessage(mHandler.obtainMessage(
                        VoiceCommonState.API_COMMAND_SET_COMMANDS,
                        VoiceCommonState.SERVICE_NOT_EXIST, 0));
            }
        } else {
     ...
     * (http://xxxx). Through the latest customization algorithm on the website,
     ...
     * {@link VoiceCommandListener#onCommandsRecognized(int, int, String)}.
     * 
     * @throws IllegalAccessException
     *             when command set was not selected
     * 
    ...
    public void pauseRecognition() throws IllegalAccessException {
        if (mCurSetName != null && mCurListener != null) {
            if (getService() != null) {
                ...
                mHandler.sendMessage(mHandler.obtainMessage(
                        VoiceCommonState.API_COMMAND_PAUSE_RECOGNITION,
                        VoiceCommonState.SERVICE_NOT_EXIST, 0, mCurListener));
            }
        } else {
            // We need to select set firstly
            throw new IllegalAccessException("Command set wasn't selected ");
    ...
    class Token extends IVoiceExtCommandListener.Stub {
        ...
        @Override
        public void onCommandsRecognized(int retCode, int commandId,
                String commandStr) throws RemoteException {
            // TODO Auto-generated method stub
            Log.i(TAG, "onCommandsRecognized result retCode="
                    + retCode + " commandId=" + commandId + " commandStr="
                    + commandStr);
            if (mCurListener != null) {
                mCurListener.onCommandsRecognized(retCode, commandId,
                        commandStr);
            } else {
                // drop the result message if listener error
                Log.e(TAG, "onCommandsRecognized drop result");
            }
        }
