...
public interface IInternationalRoamingController {
    ...
    /**
     * Whether we need to block switch phone for some reason, such as during GSM
     * manual network selection to keep in GSM mode after select a forbidden network.
     * 
     * @param block
     */
    void blockSwitchPhone(boolean block);
    ...
    /**
     * Set the network mode, only for Engineer Mode of testing CDMA RF for VIA.
     * @param networkMode Network mode: international, cdma only or gsm only.
     */
    void setNetworkMode(int networkMode);

    /**
     * Get the network mode, only for Engineer Mode of testing CDMA RF for VIA.
     * @return Network mode: international, cdma only or gsm only.
     */
    int getNetworkMode();
}
