    ...
    for (size_t i = 0; i < SHF_CONDITION_ITEM_SIZE * 2; i++) {
        ...
        case DATA_TYPE_UINT32: {
            ...
            if(DATA_TYPE_FLOAT == type) {
                SensorData t1(pd->index, true, (float)pd->value.duint32/FLOAT_MULTIPLY_FACTOR);
                v.add(t1);
                SensorData t2(pd->index, false, (float)pd->last.duint32/FLOAT_MULTIPLY_FACTOR);
                v.add(t2);
            } else {
                SensorData t1(pd->index, true, pd->value.duint32);
                v.add(t1);
                SensorData t2(pd->index, false, pd->last.duint32);
                v.add(t2);
            }
        ...
        case DATA_TYPE_UINT64: {
            SensorData t1(pd->index, true, *((uint64_t*)(&pd->value.dtime)));
            v.add(t1);
            SensorData t2(pd->index, false, *((uint64_t*)(&pd->last.dtime)));
            v.add(t2);
