...
/*package*/ abstract class Base {
     ...
     */
    private final ConcurrentHashMap<Property, Value> mValues = new ConcurrentHashMap<Property, Value>(16, 0.9f, 1);
     ...
     * @param dependsOn the property the KeyPathProperty depends on
     * @return true if the value is updated successfully.
     *
     * @hide Internal Property system should be kept out of sight of users
     */
    public <T> boolean setKeyPathValue(String keyPath, Object value, Property dependsOn) {
        Property property = new KeyPathProperty(keyPath);
        if (dependsOn != null) {
            property.addDependsOn(dependsOn);
        }
    ...
    protected void applyAllProperties() {
        ...
        // Fix Klocwork ALPS01418817
        while (clazz != Base.class && clazz != null) {
    ...
    private void applyProperty(Property prop) {
        ...
        } else if (value.isDirty()) { // apply local value if it's dirty
    ...
    /**
     * Make all property dirty.
     * We need to do it after device suspend/resume
     */
    protected void makeAllPropertiesDirty() {
        for (Map.Entry entry : mValues.entrySet()) {
            Value value = (Value) entry.getValue();
            value.setDirty();
        }
