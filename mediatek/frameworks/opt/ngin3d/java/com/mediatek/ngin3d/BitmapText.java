...
public class BitmapText extends Container {
    ...
    private static final Point mZeroPoint = new Point(0, 0);
    ...
    private void setupCharImages() {
        ...
        mCharImages.clear();
        mTextContainer.removeAllChildren();
        ...
        for (int i = 0; i < mText.length(); i++) {
            ...
            mTextContainer.addChild(charImage);

            if (mDoubleSide) {
                charImage.setDoubleSided(true);
            }
            ...
            charImage.setAnchorPoint(mZeroPoint);
    ...
    private void applyAnchorPoint(int textWidth, int textHeight) {
        ...
        float offsetX = (float) textWidth * mAnchorPoint.x;
        float offsetY = (float) textHeight * mAnchorPoint.y;
        Point oldPos = getPosition();
        mTextContainer.setPosition(new Point(oldPos.x - offsetX, oldPos.y - offsetY));
    }
    ...
    public void setAnchorPoint(Point anchorPoint) {
        ...
        applyAnchorPoint(mTextWidth, mTextHeight);
    }
    ...
    /**
     * Set whether the Bitmap text is double-sided or not.
     *
    ...
    public void setDoubleSided(boolean enable) {
        ...
        for (int i = 0; i < mCharImages.size(); i++) {
            mCharImages.get(i).setDoubleSided(enable);
        }
    }
}
