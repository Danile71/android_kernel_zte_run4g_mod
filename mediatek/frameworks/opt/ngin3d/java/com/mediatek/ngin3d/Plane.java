...
public class Plane extends Actor {
    ...
    protected boolean applyValue(Property property, Object value) {
        ...
        if (property instanceof KeyPathProperty) {
            ...
            if (propertyName.equals(PROPNAME_SOURCE_EXT_TEXTURE2D_PROPERTY)) {
                if (null != value && value instanceof Plane) {
                    setPlaneSourceExtFromPlane(getPresentation(), nodeName, (Plane) value);
                }
                return true;
            }
        } else if (property.sameInstance(PROP_SRC_RECT)) {
            Box box = (Box) value;
    ...
    /**
     * Set the external Texture from Plane
     */
    public void setTexture2DExt(String name, Plane srcPlane) {
        if ((getPresentation() != null && srcPlane != null)
            && (srcPlane.getPresentation() != null && srcPlane.getPresentation().getTexture2D() != null)) {
            getPresentation().setTexture2DExt(name, srcPlane.getPresentation().getTexture2D());
        } else {
            setKeyPathValue(PROPNAME_SOURCE_EXT_TEXTURE2D_PROPERTY + "." + name, srcPlane);
        }
    }
     ...
     * @hide
     */
    private void setPlaneSourceExtFromPlane(ImageDisplay imageDisplay, String name, Plane srcPlane) {
        if (srcPlane.getPresentation() != null) {
            imageDisplay.setTexture2DExt(name, srcPlane.getPresentation().getTexture2D());
        } else {
            touchProperty(PROPNAME_SOURCE_EXT_TEXTURE2D_PROPERTY + "." + name);
        }
    }
}
