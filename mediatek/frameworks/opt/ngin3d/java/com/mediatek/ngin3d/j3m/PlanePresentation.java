...
public class PlanePresentation extends ActorPresentation
    ...
    private HashMap<String, Texture2D> mTextureExt = new HashMap<String, Texture2D>();
    ...
    private void replaceImageSource(ImageSource src, Texture2D texture) {
        ...
        applyTextureMapping();
        updateApparentSize();
        updateTextureFilter(mTexture);

        Log.i(TAG, "replaceImageSource:" + mTexture);
    }

    private void replaceImageSourceExt(String name, Texture2D texture) {
        Texture2D replaced = mTextureExt.get(name);
        if (replaced != null && replaced.equals(texture)) {
            return;
        }
        ((Solid) getSceneNode()).getAppearance().setTexture2D("M_DIFFUSE_TEXTURE_" + name, texture);
        mTextureExt.put(name, texture);
        updateTextureFilter(texture);
        Log.i(TAG, "replaceImageSourceExt() name: " + name + ", texture:" + texture);
    }
     ...
     *
     * @param the texture object to be assigned
     */
    @Override
    public void setTexture2D(Texture2D txt) {
        replaceImageSource(null, txt);
    }

    /**
     * Get ext texture object
     *
     * @return the object of texture
     */
    @Override
    public Texture2D getTexture2DExt(String name) {
        return mTextureExt.get(name);
    }

    /**
     * Set the ext1 texture object
     *
     * @param the texture object to be assigned
     */
    @Override
    public void setTexture2DExt(String name, Texture2D txt) {
        replaceImageSourceExt(name, txt);
    }
    ...
    public void setFilterQuality(int quality) {
        ...
        updateTextureFilter(mTexture);
        for(String name : mTextureExt.keySet()){
            updateTextureFilter(mTextureExt.get(name));
        }
    }
    ...
    private void updateTextureFilter(Texture2D txt) {
        updateTextureFilterInternal(txt, mFilterQuality, mUseMipmaps);
    }

    private void updateTextureFilterInternal(Texture2D text, int filterQuality, boolean useMipmaps) {
        if (text == null) {
            return;
        }

        if (filterQuality == Image.FILTER_QUALITY_LOW) {
            text.setMagFilter(Texture.FilterMode.NEAREST);
        } else {
            text.setMagFilter(Texture.FilterMode.LINEAR);
        }

        if (useMipmaps) {
            if (filterQuality == Image.FILTER_QUALITY_HIGH) {
                text.setMinFilter(Texture.FilterMode.LINEAR_MIPMAP_LINEAR);
            } else if (filterQuality == Image.FILTER_QUALITY_MEDIUM) {
                text.setMinFilter(Texture.FilterMode.LINEAR_MIPMAP_NEAREST);
            } else {
                text.setMinFilter(
                        Texture.FilterMode.NEAREST_MIPMAP_NEAREST);
            }

        } else if (filterQuality == Image.FILTER_QUALITY_LOW) {
            text.setMinFilter(Texture.FilterMode.NEAREST);
        } else {
            text.setMinFilter(Texture.FilterMode.LINEAR);
        }
    }
}
