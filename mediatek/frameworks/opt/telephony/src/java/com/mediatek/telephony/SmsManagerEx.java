...
public class SmsManagerEx implements ISmsManagerExt {
    ...
    /// M: For MTK multiuser in 3gdatasms:MTK_ONLY_OWNER_SIM_SUPPORT @{
    private static IOnlyOwnerSimSupport mOnlyOwnerSimSupport = null;
    /// @}
   ...
   /**
     ...
     * @hide
     */
    public boolean removeCellBroadcastMsg(int channelId,int serialId, int slotId) {
        Rlog.d(TAG, "RemoveCellBroadcastMsg, slotId="+slotId);
        boolean result = false;
        String isms = getSmsServiceName(slotId);
        ...
        try {
            ISms iccISms = ISms.Stub.asInterface(ServiceManager.getService(isms));
            if (iccISms != null) {
                result = iccISms.removeCellBroadcastMsg(channelId, serialId);
            } else {
                Rlog.d(TAG, "fail to get sms service");
                result = false;
            }
     ...
     * @hide
     */
    public boolean isImsSmsSupported(int slotId) {
        Rlog.d(TAG, "isImsSmsSupported");
        Rlog.d(TAG, "slotId="+slotId);
     ...
     * @hide
     */
    public String getImsSmsFormat(int slotId) {
        Rlog.d(TAG, "getImsSmsFormat");
        Rlog.d(TAG, "slotId="+slotId);
