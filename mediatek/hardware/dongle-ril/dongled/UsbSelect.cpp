...
int UsbSelect::handleUsbAutoSelect(NetlinkEvent *evt)
{
    ...
    if (strtoul(lastDevice.VID, NULL, 16) == strtoul(vid, NULL, 16) &&
        ...
        if (found) {
                ...
                SLOGE("handleUsbAutoSelect: new 3Gdongle insert, the libpath is: %s", device.libpath);
                
                property_set("rild.dongle.libpath", device.libpath);
                ...
                if(strtoul("12D1", NULL, 16) == strtoul(vid, NULL, 16)) {
                    ...
                    while(1) {
                        if ( ( property_get("rild.mark_switchuser", mark_switchuser, NULL) > 0)
                               && (mark_switchuser[0] == '1') ) {
                    ...
                    property_set("rild.mark_switchuser", "0");
                    while(1) {
                        if ( ( property_get("rild.mark_switchuser", mark_switchuser, NULL) > 0)
                               && (mark_switchuser[0] == '0') ) {
				...
				property_set("dongle.pluged", "1");
                SLOGE("restart Dongle Rild");
                //killRild();
                property_set("ctl.restart", "ril-3gddaemon");//restart rild 
