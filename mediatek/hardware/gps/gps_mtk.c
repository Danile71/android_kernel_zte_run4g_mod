...
#define  NMEA_MAX_SIZE  91 // 83 + (NMEA_MAX_SV_INFO * 2) // float type add 2 char. for each. // brian test
...
at_gps_command_parser(char* cmdline)
{
    ...
    if (!memcmp(command+6, "=", 1)){
	    ...
	    DBG("Close AGPS raw data");
            ...
            if (test_num >= 0){
                int ret = mt3326_gps_test_start(test_num, 1, 2, 0);                
                if (0 == ret){
	...
	if (!memcmp(command+7, "=", 1)){
		if((!memcmp(command+8, "?", 1))&&(!memcmp(command+9, "\0", 1))){
			...
			if(0 == MNL_AT_TEST_FLAG){
				...
				ret = mt3326_gps_test_start(0, 1, 2, 1);											  
			}else{
...
mt3326_gps_start()
{
    ...
    DBG("mt3326_gps_start enter while loop");
    while(s->start_flag == 1)
	  {
		    usleep(100000);
		    count++;
		    DBG("mt3326_gps_start:start_flag = %d delay=%d \n",s->start_flag,count*100);
	  }
...
int mt3326_gps_test_start(int test_num, int prn_num, int time_delay, int test_mode){
...
mt3326_gps_stop()
{
    ...
    DBG("mt3326_gps_stop enter while loop");
    while(s->start_flag == 0)
	  {
			usleep(100000);
			count++;
			DBG("mt3326_gps_stop:start_flag = %d delay=%d \n",s->start_flag,count*100);
		}
