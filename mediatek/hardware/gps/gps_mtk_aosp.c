...
static float report_time_interval;
...
void agps_data_conn_open(const char* apn) {
    char buff[1024] = {0};
    int offset = 0;
...
void agps_set_server(AGpsType type, const char* hostname, int port) {
    char buff[1024] = {0};
    int offset = 0;
...
void agps_ril_set_ref_location(const AGpsRefLocation *agps_reflocation, size_t sz_struct) {
    ...
    buff_put_struct(agps_reflocation, sizeof(AGpsRefLocation), buff, &offset);
...
#define  NMEA_MAX_SIZE  91 // 83 + (NMEA_MAX_SV_INFO * 2) // float type add 2 char. for each.
...
gps_download_epo_enable(void)
{
	...
	while(fgets(result, sizeof(result), fp))
	{
		...
		{
			gps_epo_enable = *val-'0';
		}
		if(!strcmp(key, "EPO_PERIOD"))
		{
			gps_epo_period = *val-'0';
		}
...
at_gps_command_parser(char* cmdline)
{
    ...
    if (!memcmp(command+6, "=", 1)){
	    ...
	    DBG("Close AGPS raw data");
            ...
            if (test_num >= 0){
                int ret = mtk_gps_test_start(test_num, 1, 2, 0);                
                if (0 == ret){
...
void mnld_to_gps_handler(int fd,GpsState* s) {//from AGPSD->MNLD->HAL->FWK
    ...
    switch(type) {
        ...
        case MNL_AGPS_CMD_AGPS_LOC: {//235
            ...
            buff_get_struct(&location, sizeof(mnl_agps_agps_location), buff, &offset);
