...
#if MTK_ENABLE_MD5
	{ 25, 512, "/dev/radio/pttyaudio-md2" },
#endif
...
#ifdef  MUX_ANDROID
		if (channel->ptslink != NULL) {
                        chown(channel->ptsname, AID_RADIO, AID_AUDIO);
			unlink(channel->ptslink);
...
static int poll_modem_ready(int	serial_device_fd, int to)
{
	...
	timeout.tv_usec = 0;
	int len;
    GSM0710_Buffer *buf_for_bootup = serial.in_buf_for_bootup;
	...
	do {
		...
		if (FD_ISSET(serial_device_fd, &rfds)) {
            ...
            gsm0710_buffer_write(buf_for_bootup, buf, len);
            LOGMUX(LOG_INFO, "serial device buff: %s", buf_for_bootup->data);
			errno = 0;
			if (memstr((char *)buf_for_bootup->data, buf_for_bootup->datacount,"+EIND: 128")) {
				LOGMUX(LOG_INFO, "Received +EIND: 128");
                pthread_mutex_lock(&buf_for_bootup->datacount_lock);
                buf_for_bootup->readp = buf_for_bootup->data;
                buf_for_bootup->writep = buf_for_bootup->data;
                buf_for_bootup->endp = buf_for_bootup->data + GSM0710_BUFFER_SIZE;
                buf_for_bootup->datacount = 0;
                memset(buf_for_bootup->data, 0, GSM0710_BUFFER_SIZE);
                pthread_mutex_unlock(&buf_for_bootup->datacount_lock);
				return 0;
			}
		}
	} while (sel);
	...
	while (1) {
		...
		if (sel < 0) {
			...
			if (FD_ISSET(serial_device_fd, &rfds)) {
                ...
                LOGMUX(LOG_INFO, "serial device buff: %s", buf_for_bootup->data);
				//errno = 0;
                if (memstr((char *)buf_for_bootup->data, buf_for_bootup->datacount,"OK")) {
                    ...
                    pthread_mutex_unlock(&buf_for_bootup->datacount_lock);
					return 0;
				} else if (memstr((char *)buf_for_bootup->data, buf_for_bootup->datacount, "ERROR")) {
	...
	while (1) {
		...
		if (sel < 0) {
			...
			if (FD_ISSET(serial_device_fd, &rfds)) {
                ...
                LOGMUX(LOG_INFO, "serial device buff: %s", buf_for_bootup->data);
				//errno = 0;
				if (memstr((char *)buf_for_bootup->data, len, "OK")) {
                    if(checkMuxFlag == 1) {
                        return 0;
                    } else {
					...
					return 0;
				} else if (memstr((char *)buf_for_bootup->data, buf_for_bootup->datacount, "+CMUX: READY")) {
...
int start_muxer(Serial *serial)
{
    ...
    gsm0710_buffer_destroy(serial->in_buf_for_bootup);
...
#else
#ifdef MTK_CSFB_IN_MMDC_PROJECT
    property_set("ctl.start", "ril-daemon-s");
    LOGMUX(LOG_INFO, "ril-daemon single started!");
#else
...
int main(int argc, char *argv[])
{
    ...
    if(MTK_DT_SUPPORT) {
        LOGMUX(LOG_INFO, "MTK_DT_SUPPORT");
    #ifdef MTK_RIL_MD2
        if(MTK_ENABLE_MD5) {
    ...
    //allocate memory for data structures
    //adv_frame_buf is not necessary in basic mode
    if ((serial.in_buf_for_bootup = gsm0710_buffer_init()) == NULL ||
        (cmux_mode == 1 &&
         (serial.adv_frame_buf = (unsigned char *)malloc((cmux_N1 + 3) * 2 + 2)) == NULL)) {
        LOGMUX(LOG_ERR, "Out of memory");
        exit(-1);
