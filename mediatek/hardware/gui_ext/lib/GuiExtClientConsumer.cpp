...
#define GUIEXT_LOGD(x, ...) XLOGV("[GuiExtC] "x, ##__VA_ARGS__)
#else
...
int GuiExtClientConsumer::getSlotFromBufferLocked(
    ...
    for (uint32_t i = 0; i < mSlots.size(); i++) {
        if (mSlots[i] != NULL && mSlots[i]->buffer.get() == buffer && mSlots[i]->buffer->handle == buffer->handle) {
            GUIEXT_LOGD("getSlotFromBufferLocked: find match buffer[%d]: b=%p, h=%p", mSlots[i]->idx, buffer, buffer->handle);
            return mSlots[i]->idx;
        }
    }
...
bool GuiExtClientConsumer::isValidBuffer(android_native_buffer_t* buffer)
{
    ...
    if (idx < 0) {
        ...
        GUIEXT_LOGD("isValidBuffer return true for buffer=%p, handle=%p", buffer, buffer->handle);
    }
