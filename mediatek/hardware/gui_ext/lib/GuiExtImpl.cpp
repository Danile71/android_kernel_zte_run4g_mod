...
status_t GuiExtPool::alloc(const sp<IBinder>& token, uint32_t gralloc_usage, uint32_t w, uint32_t h, uint32_t * poolId)
{
    ...
    *poolId = getPoolId();
    ...
    GUIEXT_LOGI("  alloc cost time=%lld ms, gralloc_usage=%x, w=%d, h=%d, poolId=%d", ns2ms(time_end - time_start),
...
sp<BufferQueue> GuiExtPoolItem::createBufferQueue(uint32_t w, uint32_t h, uint32_t num, String8 name)
{
    sp<BufferQueue> bq = new BufferQueue(/*new GraphicBufferAlloc()*/);
    bq->setDefaultBufferSize(w, h);

    bq->setConsumerName(name);
	bq->setBufferCount(num);
	bq->setMaxAcquiredBufferCount(2);

    // consumer connect
    wp<ConsumerListener> listener = static_cast<ConsumerListener*>(this);
    sp<BufferQueue::ProxyConsumerListener> proxy = new BufferQueue::ProxyConsumerListener(listener);
    status_t err = bq->consumerConnect(proxy, true);
    if (err != NO_ERROR) {
    ...
    err = bq->connect(sLife, NATIVE_WINDOW_API_MEDIA, true, &output);

    GUIEXT_LOGV("    createBufferQueue done, name=%s", bq->getConsumerName().string());
...
status_t GuiExtPoolItem::prepareBuffer(uint32_t gralloc_usage)
{
    ...
    err = prepareBuffer(mGPUUsedBq, GUI_EXT_USAGE_GPU, 0, gralloc_usage, mGpuUsedBufNum);
    GUIEXT_LOGV("    prepare %s buffer done", szUsageName[GUI_EXT_USAGE_GPU]);

    if (mIsHwcNeeded) {
#if SUPPORT_MULTIBQ_FOR_HWC
        for (uint32_t i = 0; i < mHwcUsedBqList.size(); i++) {
            err = prepareBuffer(mHwcUsedBqList[i]->bq, GUI_EXT_USAGE_HWC, mHwcUsedBqList[i]->type, gralloc_usage, mHwcUsedBqList[i]->bufNum);
...
status_t GuiExtPoolItem::prepareBuffer(sp<BufferQueue> bq, uint32_t usage, uint32_t type, uint32_t gralloc_usage, uint32_t bufNum)
{
    status_t err = NO_ERROR;
    ...
    for (uint32_t i = 0; i < bufNum; i++) {
        ...
        bq->dequeueBuffer(&buf, &fence, false, 0, 0, fmt, usg);
...
status_t GuiExtPoolItem::acquire(const sp<IBinder>& token, uint32_t usage, uint32_t type, sp<BinderDiedListener> listener, int *buf)
{
    ...
    sp<BufferQueue> bq = getBufferQueue(usage, type);
    if (bq == NULL) {
    ...
    sp<Fence> fence;
    uint32_t fmt = gAcquiredFormat[usage];
    status_t ret = bq->dequeueBuffer(buf, &fence, false, 0, 0, fmt, LOCK_FOR_USAGE);
...
sp<BufferQueue> GuiExtPoolItem::getBufferQueue(uint32_t usage, uint32_t type)
{
    sp<BufferQueue> bq = mGPUUsedBq;
...
void GuiExtPoolItem::dump(String8& result) const
{
    ...
    if (mIsHwcNeeded) {
        ...
        for (uint32_t i = 0; i < mHwcUsedBqList.size(); i++) {
            ...
            mHwcUsedBqList[i]->bq->dump(result, "        ");
