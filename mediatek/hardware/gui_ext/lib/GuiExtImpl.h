...
private:
    ...
    sp<BufferQueue> createBufferQueue(uint32_t w, uint32_t h, uint32_t num, String8 name);
    sp<BufferQueue> getBufferQueue(uint32_t usage, uint32_t type);

    status_t prepareBuffer(sp<BufferQueue> bq, uint32_t usage, uint32_t type, uint32_t gralloc_usage, uint32_t bufNum);
