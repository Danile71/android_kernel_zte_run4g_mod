...
void regDump(const android::sp<android::IDumpTunnel>& tunnel, const android::String8& key)
{
    using namespace android;
    sp<IServiceManager> sm = defaultServiceManager();
    if (sm == NULL)
    {
        XLOGE("service manager is NULL");
        return;
    }

    sp<IBinder> binder = defaultServiceManager()->getService(String16("GuiExtService"));
    if (binder == NULL)
    {
        XLOGE("gui ext is NULL");
        return;
    }

    sp<IGuiExtService> guiExt = interface_cast<IGuiExtService>(binder);
    guiExt->regDump(tunnel, key);
}

void unregDump(const android::String8& key)
{
    using namespace android;
    sp<IServiceManager> sm = defaultServiceManager();
    if (sm == NULL)
