...
detachDeviceLocked(android::sp<ICamDevice> pDevice)
{
    ...
    uint32_t const version = determineOpenDeviceVersionLocked(s8ClientAppMode, openId);
    // un-register device to MMSDK service 
    if (pDevice != 0 &&  version == CAMERA_DEVICE_API_VERSION_1_0)   //device init success, register itself to mmsdk service        
    {
...
)
{
    ...
    else if  ( version >= CAMERA_DEVICE_API_VERSION_3_0 )
    {
