...
)
{
    ...
    if (mi4DbgModuleType & dbgModuleID) {
        ...
        //
        MY_LOGI("Get: ID_%s(0x%04x), Size(%d), Addr(0x%x)", 
            mpDebugIdMap->stringFor(dbgModuleID).string(), dbgModuleID, 
            mDbgInfo[moduleIndex].u4BufSize, (MUINT32)&mDbgInfo[moduleIndex].puDbgBuf);
        //
...
)
{
    ...
    if (mi4DbgModuleType & dbgModuleID)
    {
        ...
        //
        MY_LOGI("Get: ID_%s(0x%04x), Index(%d), Size(%d), Addr(0x%x), APP%d", 
            mpDebugIdMap->stringFor(dbgModuleID).string(), dbgModuleID,
            dbgModuleIndex, mDbgInfo[dbgModuleIndex].u4BufSize, (MUINT32)mDbgInfo[dbgModuleIndex].puDbgBuf, dbgAppn);
        //
        ...
        case CMD_SET_DBG_EXIF:
        {
            ...
            else
            {
                if (u4ID != ID_ERROR) {
                    ...
                    //
                    MY_LOGI("Set: ID_%s(0x%04x), Size(%d), Addr(0x%x)", 
                        mpDebugIdMap->stringFor(u4ID).string(), u4ID, tmpDbgInfo.u4BufSize, (MUINT32)&tmpDbgInfo.puDbgBuf);
                }
                else {
                    MY_LOGE("ID_ERROR");
                    ret = MFALSE;
