...
init(
    ...
    CamDbgParam const& rCamDbgParam = CamDbgParam(0, 0, 0)
)
{
    // (0) CamExif: CamExifParam
    mCamExifParam = rCamExifParam;

    MY_LOGI("============================================ mCamExifParam ============================================");
    MY_LOGI("u4GpsIsOn(%d), u4GPSAltitude(%d), uGPSLatitude(%s), uGPSLongitude(%s), uGPSTimeStamp(%s), uGPSProcessingMethod(%s), u4Orientation(%d)",
            mCamExifParam.u4GpsIsOn, mCamExifParam.u4GPSAltitude, mCamExifParam.uGPSLatitude, mCamExifParam.uGPSLongitude, 
            mCamExifParam.uGPSTimeStamp, mCamExifParam.uGPSProcessingMethod, mCamExifParam.u4Orientation);
    MY_LOGI("u4Facing(%d), u4ZoomRatio(%d), u4ImgIndex(%d), u4GroupId(%u), u4FocusH(%d), u4FocusL(%d), u4RefocusPos(%d), uJpsFileName(%s)", 
            mCamExifParam.u4Facing, mCamExifParam.u4ZoomRatio, mCamExifParam.u4ImgIndex, mCamExifParam.u4GroupId, mCamExifParam.u4FocusH, mCamExifParam.u4FocusL,
            mCamExifParam.u4RefocusPos, mCamExifParam.uJpsFileName);
    ...
    MY_LOGI("============================================ mCamDbgParam ============================================");
    MY_LOGI("u4ShotMode(%d), u4CamMode(%d), bStereoCam(%d)",
            mCamDbgParam.u4ShotMode, mCamDbgParam.u4CamMode, mCamDbgParam.bStereoCam);
    MY_LOGI("======================================================================================================");
...
)
{
    ...
    MUINT32 const size = u4Size + 0x02;    // Data size contains 'Data size' descriptor (plus 2 bytes).
    MUINT32 appnReturnSize = 0;
    MY_LOGD("APP(%d) size(0x%x) addr(0x%x)", u4Appn, size, puBuf);
    //
    if ( puBuf != NULL )
    {
        err = mpBaseExif->exifAppnMake(u4Appn, pDst, puBuf, size, &appnReturnSize);
        if  ( err != 0 )
        {
            MY_LOGE(" exifAppnMake - err(%x)", err);
            goto lbExit;
...
{
    MY_LOGD("APP(%d) size(0x%x)", u4ReserveAppn, u4ReserveSize);
    MINT32  err = 0;
    MUINT8* pDst = puAppnBuf;
    MUINT32 appnReturnSize = 0;
    MUINT8* pReserveBuf = new MUINT8 [u4ReserveSize-0x02];
    //
    err = mpBaseExif->exifAppnMake(u4ReserveAppn, pDst, pReserveBuf, u4ReserveSize, &appnReturnSize);
    if  ( err != 0)
    {
        MY_LOGE(" exifAppnMake - err(%x)", err);
        goto lbExit;
...
queryExifApp1Info(exifAPP1Info_s*const pexifApp1Info)
{
    ...
    **********************************************************************************/
    ...
    // [Image Refocus: main sensor position (L or R)]
    // [Image Refocus: jps file name]
    ...
    pexifApp1Info->refocusPos   = mCamExifParam.u4RefocusPos;
    memset(pexifApp1Info->strJpsFileName,0,32);
    strcpy((char *)pexifApp1Info->strJpsFileName, (char*)mCamExifParam.uJpsFileName);
...
set3AEXIFInfo(EXIF_INFO_T* p3AEXIFInfo)
{
    ...
    if ( NULL != mp3AEXIFInfo )
    {
        ...
        MY_LOGI("============================= 3AEXIFInfo ==================================");
        MY_LOGI("FNumber: %d/10,    FocalLength: %d/100,    Scene mode: %d, AWB mode: %d,   flash: %d",
        ...
        MY_LOGI("AE meter mode: %d, AE ExpBias: %d, AE ExpTime: %d, ISO speed: %d,  Real ISO value: %d", 
