...
)
{
    ...
    *(unsigned int*)buf = pexifImgInfo->thumbSize;
    ...
    err = ifdListNodeModify(IFD_TYPE_ZEROIFD, IFD0_MTK_REFOCUSPOS, &pexifAPP1Info->refocusPos);
    if (err != EXIF_NO_ERROR) {
        return err;
    }
    // mtk definition: JPS file name for image refocus.
    err = ifdListNodeModify(IFD_TYPE_ZEROIFD, IFD0_MTK_REFOCUSJPS, pexifAPP1Info->strJpsFileName);
    if (err != EXIF_NO_ERROR) {
        return err;
    }
