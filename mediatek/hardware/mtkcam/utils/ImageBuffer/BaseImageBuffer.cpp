...
onCreate()
{
    ...
    for (size_t i = 0; i < mvImgBufInfo.size(); ++i)
    {
        ...
        {   // check  ROI(x,y) + ROI(w,h) <= heap stride(w,h)
            ...
            MY_LOGW_IF(mspImgBufHeap->getBufStridesInBytes(i) != (MUINT32)PIXEL2BYTE(getPlaneBitsPerPixel(i), planeStridesInPixels), 
...
{
    MBOOL ret = MFALSE;
    size_t const sizeInBytes       = getBufSizeInBytes(0);
