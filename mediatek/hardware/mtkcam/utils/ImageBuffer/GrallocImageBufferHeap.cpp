...
protected:  ////                    Info to Allocate.
    ...
    MUINT32                         mBufStridesInBytesToAlloc[3];   // buffer strides in bytes.
...
doAllocGB(MINT32& heapID, size_t& stridesInBytes)
{
    ...
    MINT32 idx = 0;
    MINT32 num = 0;
    MINT32 width  = stridesInBytes;
    MINT32 height = mImgSize.h;
    //
    mspGBuffer = new GraphicBuffer(width, height, mImgFormat, mExtraParam.usage);
    //
    MINT32 stride = mspGBuffer->getStride();
    if ( stride != width )
    {
        MY_LOGD("update w(%d)->stride(%d)", stridesInBytes, stride);
        stridesInBytes = stride;
    }
    //
    err = ::gralloc_extra_getIonFd(mspGBuffer->handle, &idx, &num);
    if ((NO_ERROR != err) || (num <= 0))
    {
        MY_LOGE("getIonFd(): idx/num=%d/%d, status[%s(%d)]", idx, num, ::strerror(-err), -err);
        goto lbExit;
    }
    else
    {   // current num should be 1
        heapID = mspGBuffer->handle->data[idx];
    }
    //
    return  MTRUE;
lbExit:
...
)
{
    ...
    for (MUINT32 i = 0; i < rvBufInfo.size(); i++)
    {
        ...
        //
        //  SW Access.
        pBufInfo->va = ( 0 != (usage & eBUFFER_USAGE_SW_MASK) ) ? (pMyBufInfo->u4Offset + (unsigned int)buf) : 0;
        //
        //  HW Access.
