...
{
    return IonImageBufferHeap::create(szCallerName, rImgParam, rExtraParam);
}
...
impInit(BufInfoVect_t const& rvBufInfo)
{
    MBOOL ret = MFALSE;
    //
    mIonDevFD = ::mt_ion_open("IonImageBufferHeap");
    if  ( 0 > mIonDevFD )
    {
...
doAllocIon(MyHeapInfo& rHeapInfo, MyBufInfo& rBufInfo)
{
    ...
    err = ::ion_alloc_mm(mIonDevFD, rBufInfo.sizeInBytes, rBufInfo.iBoundaryInBytesToAlloc, ion_prot_flags, &rHeapInfo.pIonHandle);
    if  ( 0 != err )
    {
