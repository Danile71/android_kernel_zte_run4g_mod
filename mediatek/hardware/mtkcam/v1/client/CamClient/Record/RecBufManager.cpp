    ...
    , mpBufPA(NULL)
{
    ...
    if(mpImgInfo->mbMetaMode)
    {
        ...
        GraphicBufferExtra::get().getBufInfo(mpGraphicBuffer->handle, &bufInfo);
        mpImgInfo->mu4ImgFormat = bufInfo.format;

        //(.2) query buffer stride
        if(mpImgInfo->mu4ImgFormat == HAL_PIXEL_FORMAT_YV12)
        {
            int UStrideMask = 7;
            int VStrideMask = 7;  //because of no way to get U and V stride
            mpImgInfo->mu4ImgStrideY = bufInfo.stride;
        ...
        int idx, num; //for?
        GraphicBufferExtra::get().getIonFd(mpGraphicBuffer->handle, &idx, &num);
        if ( num > 0 )
        {
            mi4BufIonFd = mpGraphicBuffer->handle->data[idx];
        }
        else
        {
            CAM_LOGD("[RecImgBuf::RecImgBuf]num: %d <= 0", num);
        }
