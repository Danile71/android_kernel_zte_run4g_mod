...
#define RECORDING_GRALLOC_USAGE     (GraphicBuffer::USAGE_SW_READ_RARELY | GraphicBuffer::USAGE_HW_VIDEO_ENCODER)
...
initBuffers()
{
    ...
    {
        mRecBufNum = 5 + (mpParamsMgr->getInt(CameraParameters::KEY_PREVIEW_FRAME_RATE)/30)*2 + 2;
    }
    else
    {
        mRecBufNum = eMAX_RECORD_BUFFER_NUM;
    }
    //
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<RecBufManager>const& rpBufMgr)
{
    ...
    for(i = 0; i < mvRecBufInfo.size(); i++)
    {
        if(mvRecBufInfo[i].Sta == REC_BUF_STA_EMPTY)
        {
            ...
            MY_LOGD("EQ:idx(%d),VA(0x%08X)",
                    i,
                    (uint32_t)(pCameraImgBuf->getVirAddr()));
            //
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (i = 0; i < queSize; i++)
    {
        ...
        {
            MY_LOGE("Can't find FD(%d)/VA(0x%08X)",
...
performRecordCallback(int32_t bufIdx, sp<RecImgBuf>const& pCameraImgBuf, int32_t const msgType)
{
    ...
    if  ( pCameraImgBuf != 0 )
    {
        ...
        if(mbMetaMode)
        {
           ...
           //* |  buffer type  |          payload                           |
           //* --------------------------------------------------------------
           *((uint32_t*)pCameraImgBuf->get_camera_memory()->data) = kMetadataBufferTypeCameraSource;
           *((buffer_handle_t*)(pCameraImgBuf->get_camera_memory()->data + sizeof(uint32_t))) = pCameraImgBuf->getGrallocBufferHandle();
        }
        //
        MY_LOGD("CB:FC(%02d),VA(0x%08X),CbVA(0x%08X),GrallocHandle(0x%08X),TS(%d.%06d)",
                mFrameCount,
    ...
    for(i=0; i<size; i++)
    {
        if(mvRecBufInfo[i].Sta  == REC_BUF_STA_CB)
        {
            if(mvRecBufInfo[i].CbVirAddr == opaque)
            {
                if(mbMetaMode)
                {
                    MY_LOGD("Idx(%d),FD(%d),VA(0x%08X),32byte(%d/0x%08X)",
                            i,
                            mvRecBufInfo[i].BufIonFd,
                            (uint32_t)opaque,
                            *(uint32_t*)opaque,
                            *(uint32_t*)(opaque + sizeof(uint32_t)));
                }
                else
                {
                    MY_LOGD("Idx(%d),FD(%d),VA(0x%08X)",
                            i,
                            mvRecBufInfo[i].BufIonFd,
                            (uint32_t)opaque);
                }
