...
initBuffers()
{
    ...
    //  (2) Allocate buffers.
    muImgBufIdx = 0;
#warning [FIXME] should according to real supported format/w/h in platform
#if 0  
...
#else 
    String8 const s8Format = String8(MtkCameraParameters::PIXEL_FORMAT_YUV422I);
    int width = 640; 
    int height = 480; 
    if(mi4PrvWidth != 0)
    {
        height = width * mi4PrvHeight / mi4PrvWidth;
    }
    else
    {
        height = 480;
    }
    MY_LOGD("[Gesture buffer] w: %d, h: %d, format: %s, %d", width, height, s8Format.string());   
    mpImgBufMgr = ImgBufManager::alloc(
        s8Format, 
        width, height, 
        eMAX_BUFFER_NUM, 
...
prepareAllTodoBuffers(sp<IImgBufQueue>const& rpBufQueue, sp<ImgBufManager>const& rpBufMgr)
{
    ...
    int iEnqCount = 0;
    {
        ...
        iEnqCount = eMAX_BUFFER_NUM - mImgBufList.size();
    }
    //
    //
    sp<ClientImgBuf> pImgBuf = NULL;
    for (int i = 0; i < iEnqCount; i++)
    {
        //  (.1) Determine which buffer to enque.
        {
            Mutex::Autolock _l(mModuleMtx);
            pImgBuf = rpBufMgr->getBuf(muImgBufIdx);
            muImgBufIdx = (muImgBufIdx+1) % eMAX_BUFFER_NUM;
        }
        //
        //  (.2) enque it into Processor
        ret = rpBufQueue->enqueProcessor(
            ImgBufQueNode(pImgBuf, ImgBufQueNode::eSTATUS_TODO)
        );
        //
        if  ( ! ret ) {
...
handleReturnBuffers(Vector<ImgBufQueNode>const& rvQueNode)
{
    ...
    for (int32_t i = 0; i < queSize; i++)
    {
        ...
        sp<ClientImgBuf>       pListImgBuf = NULL;
        {
            Mutex::Autolock _l(mModuleMtx);
            //
        ...
        MY_LOGD("i , idx = %d, %d", i, idxToCallback); 
        //
        //  (.4)  Perform callback.
        if  ( i == idxToCallback ) {
            //
            MY_LOGD_IF(
                ...
                i, rQueNode.getStatus(), reinterpret_cast<void*>( pListImgBuf->getVirAddr()), 
...
(
    sp<ClientImgBuf> const &pImgBuf, 
    sp<NSCam::IImageBuffer> *pImageBuffer
)
{
    //IImageBuffer pImageBuffer; 
    MUINT32 bufStridesInBytes[3] = {GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pImgBuf, 0), 
                                    GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pImgBuf, 1), 
                                    GET_IIMGBUF_IMG_STRIDE_IN_BYTE(pImgBuf, 2) 
                                   };

    MINT32 bufBoundaryInBytes[3] = {0,0,0};
    IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam(
                                                    Format::queryImageFormat(pImgBuf->getImgFormat().string()), 
                                                    MSize(pImgBuf->getImgWidth(),
                                                          pImgBuf->getImgHeight()), 
                                                          bufStridesInBytes, 
                                                          bufBoundaryInBytes, 
                                                          Format::queryPlaneCount(Format::queryImageFormat(pImgBuf->getImgFormat().string())));
    PortBufInfo_v1 portBufInfo = PortBufInfo_v1(-1, 
                                                (MUINT32)pImgBuf->getVirAddr(),
                                                 0, 
                                                 0, 
                                                 0);
    //
...
void
GestureClient::
performPreviewCallback(sp<ClientImgBuf>const& pImgBuf, int32_t const msgType)
{
    if  ( pImgBuf != 0 )
    {
        ...
        dumpImgBuf_If(pImgBuf);
        if (NULL != getGestureDevice())
        {
            if(pImgBuf == 0)
            {
                return;  
            ...
            int numOfDetected = 0; 
            hand_detection_result_t *pDetectedResult = new hand_detection_result_t[mMaxNumOfGestureDetected]; 

            sp<IImageBuffer> pImageBuffer = 0; 
            convertImgBufToIImageBuffer(pImgBuf, &pImageBuffer); 
            if (pImageBuffer != 0)
            {
                pImageBuffer->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN);
                //
                getGestureDevice()->ops->hand_detect(getGestureDevice(), 
                                                pImageBuffer.get(), 
                                                HAND_POSE_VICTORY, 
                                                &numOfDetected, 
                                                pDetectedResult); 
                pImageBuffer->unlockBuf(LOG_TAG);           
                MY_LOGD_IF(1, "Detected Num:%d", numOfDetected); 
                onGestureCallback(pDetectedResult[0]); 
            }

        }
        //
        //  [2] Callback
        CamProfile profile(__FUNCTION__, "GestureClient");
        ...
        profile.print_overtime(10, "mDataCb(%x) - index(%d)", msgType, pImgBuf->getBufIndex());
    }
}
