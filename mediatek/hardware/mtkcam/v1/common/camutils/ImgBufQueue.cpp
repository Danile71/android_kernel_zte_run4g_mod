...
{
    mTodoImgBufQueFmt = String8::empty();
}
...
enqueProvider(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ...
        "+ Que.size(%d); (CookieED/CookieDE)=(%#x,%#x) %s; Buffer[%s@%p@%d@%s@(%d)%dx%d@%d@Timestamp(%lld)]", 
        mDoneImgBufQue.size(), 
...
bool
ImgBufQueue::
queryFormat(String8& rFormat)
{
    if(mTodoImgBufQueFmt.isEmpty())
    {
        MY_LOGW_IF(1, "foramt is empty");
        return false;
    }
    else
    {
        rFormat = mTodoImgBufQueFmt;
        MY_LOGW_IF(1, "foramt(%s)",rFormat.string());
        return true;
    }
}
...
flushProcessor()
{
    ...
    for ( List<ImgBufQueNode>::iterator it = mTodoImgBufQue.begin(); it != mTodoImgBufQue.end(); it++ )
    {
        ...
        MY_LOGD_IF(
            ENABLE_LOG_PER_FRAME, 
            "%s: (CookieED/CookieDE)=(%#x/%#x); Buffer[%s@%p@%d@%s@(%d)%dx%d-%dBit@Timestamp(%lld)]", 
...
enqueProcessor(ImgBufQueNode const& rNode)
{
    ...
    MY_LOGV_IF(
        ENABLE_LOG_PER_FRAME, 
        "+ Que.size(%d); %s: (CookieED/CookieDE)=(%#x/%#x); Buffer[%s@%p@%d@%s@(%d)%dx%d-%dBit@Timestamp(%lld)]", 
    ...
    mTodoImgBufQueFmt = rNode.getImgBuf()->getImgFormat();
    //
    return  true;
}
