...
#define PROPERTY_FD_ON_ONLY_R8_NETWORK  "persist.radio.fd.on.only.r8.network"
...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    if (s_md_off &&
        ...
        request != RIL_REQUEST_CONFIG_SIM_SWITCH &&
	...
	if (radioState == RADIO_STATE_OFF &&
	    !(request == RIL_REQUEST_RADIO_POWER ||
            ...
            request == RIL_REQUEST_REMOVE_CB_MESSAGE ||
            ...
            //Start - WIFI Calling For EAP-AKA
            request == RIL_REQUEST_SIM_AUTHENTICATION  ||
            request == RIL_REQUEST_USIM_AUTHENTICATION ||
            //End - WIFI Calling For EAP-AKA
            #endif
            ...
            request == RIL_REQUEST_CONFIRM_PS_SWITCH ||
...
static void initializeCallback(void *param)
{
    ...
    if (rid == MTK_RIL_SOCKET_1) {		
        ...
        if ((err == 0 && p_response->success == 1) && (strncmp(p_response->finalResponse,"OK", 2) == 0)) {
            ...
            /* Enable FD only in R8 Network if system property "persist.radio.fd.on.only.r8.network" set to 1 */
            memset(property_value, 0, sizeof(property_value));
    ...
    if(isDualTalkMode()) {
        // for DSDA, notify GeminiPhone start to init
        RIL_onUnsolicitedResponse(RIL_UNSOL_SIM_INSERTED_STATUS, 0, sizeof(int), rid);
    }
#endif  /* MTK_GEMINI */
...
/* To check if new SS service class feature is supported or not */
void determineSSServiceClassFeature(RILId rid) {
    ATResponse *p_response = NULL;
    int err;
...
int getMtkShareModemCurrent() {
    return MTK_SHARE_MODEM_CURRENT;
}
