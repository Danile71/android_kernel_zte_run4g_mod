...
//For VoLTE conference call test
//#define VOLTE_TEST
#ifdef VOLTE_TEST
int bMergeConferenceCall = 0;
int ecpi132 = 0;
#endif
...
void requestLastCallFailCause(void *data, size_t datalen, RIL_Token t)
{
	...
	else if ((callFailCause > 127 && callFailCause != 2165 && callFailCause != 380) || callFailCause <= 0) {
...
// IMS MO
static void requestGetEutranVdp(void * data, size_t datalen, RIL_Token t)
{
    ATResponse* p_response = NULL;
    int err;
    char* line = NULL;
    ...
    err = at_send_command_singleline("AT+CEVDP?", "+CEVDP:", &p_response, CC_CHANNEL_CTX);

    if (err < 0 || NULL == p_response) {
        LOGE("requestGetEutranVdp Fail");
        goto error;
    }
...
static void requestSetEutranVdp(void * data, size_t datalen, RIL_Token t)
{
    int* n = (int *) data;
    ATResponse* p_response = NULL;
    int err;
...
static void requestGetUtranVdp(void * data, size_t datalen, RIL_Token t)
{
    ATResponse* p_response = NULL;
    int err;
    char* line = NULL;
    ...
    err = at_send_command_singleline("AT+CVDP?", "+CVDP:", &p_response, CC_CHANNEL_CTX);

    if (err < 0 || NULL == p_response) {
        LOGE("requestGetUtranVdp Fail");
        goto error;
    }
...
static void requestSetUtranVdp(void * data, size_t datalen, RIL_Token t)
{
    int* n = (int *) data;
    ATResponse* p_response = NULL;
    int err;
...
static void requestGetMmImsVoiceTermination(void * data, size_t datalen, RIL_Token t)
{
    ATResponse* p_response = NULL;
    int err;
    char* line = NULL;
    ...
    err = at_send_command_singleline("AT+CMMIVT?", "+CMMIVT:", &p_response, CC_CHANNEL_CTX);

    if (err < 0 || NULL == p_response) {
        LOGE("requestGetMmImsVoiceTermination Fail");
        goto error;
    }
...
static void requestSetMmImsVoiceTermination(void * data, size_t datalen, RIL_Token t)
{
    int* n = (int *) data;
    ATResponse* p_response = NULL;
    int err;
    ...
    if (err < 0 || NULL == p_response) {
        LOGE("requestSetMmImsVoiceTermination Fail");
        goto error;
    }
...
extern int rilCcMain(int request, void *data, size_t datalen, RIL_Token t)
{
	...
	case RIL_REQUEST_RETRIEVE_HELD_CALL:
		requestRetrieveHeldCall(data, datalen, t);
		break;	
...
extern int rilCcUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx *p_channel)
{
	...
	if (strStartsWith(s, "RING") || strStartsWith(s, "+CRING")) {
...
#ifdef VOLTE_TEST
		char callMode[5];
		char *originalEcpiStr = strdup(s);
		property_get("ril.volte.callmode", callMode, "0");
		LOGD("ril.volte.callmode = %s", callMode);
		if (strStartsWith(s, "+ECPI: 1,0") || strStartsWith(s, "+ECPI: 1,6")) {
			char *s1 = (char*)malloc(255);
			memset(s1, 0x00, 255);
			strncpy(s1, s, 17);
			strcat(s1, callMode);
			strcat(s1, strstr(s, ",\""));
			LOGD("new ECPI string = %s", s1);
			onCallProgressInfo((char *)s1, rid);
			free(s1);
		} else {
		   onCallProgressInfo((char *)s, rid);
		}
		
		/* To simulate VoLTE conference call scenario */
		LOGD("bMergeConferenceCall = %d", bMergeConferenceCall);
		if (bMergeConferenceCall) {
			if (strStartsWith(originalEcpiStr, "+ECPI: 1,132")) {
				ecpi132++;
			}
			if (strStartsWith(originalEcpiStr, "+ECPI: 2,132")) {
				ecpi132++;
			}
			LOGD("ecpi132 = %d", ecpi132);
			if (ecpi132 == 2) {
				simulateVoLteConferenceCallUrc(rid);
				bMergeConferenceCall = 0;
				ecpi132 = 0;
			}
		}

		free(originalEcpiStr);
		
		/* To simulate VoLTE SS Runtime Indication scenario */
		/*if (strStartsWith(s, "+ECPI: 1,130")) {
		   ssRuntimeIndTest(rid);
		}*/
#else		
		onCallProgressInfo((char *)s, rid);
#endif
		...
		if (atoi(callMode) == 99) {
			char *s1 = (char*)malloc(255);
			strcpy(s1, "+ECONFSRVCC: 2,1,2");
			LOGD("s1 = %s", s1);
			onVoLteEconfSrvccIndication((char *)s1, rid);
			free(s1);
		} else {
...
//For testing purpose
#ifdef VOLTE_TEST
void simulateVoLteConferenceCallUrc(RILId rid) {
	char *file = "/sdcard/confcallurc.txt";
	char data[256];
	FILE *fp = fopen(file, "r");
	LOGD("[simulateVoLteConferenceCallUrc]Open %s", file);
	if (fp == NULL)
	{
		LOGE("Can't open the file %s !", file);
		return;
	}
