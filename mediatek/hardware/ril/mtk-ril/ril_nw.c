...
int convert3GRssiValue(int RSCP_in_qdbm,int EcN0_in_qdbm){
    ...
    int baseband1=0,baseband2=0,baseband3=0;
    bool isFdd3G = false;
    bool isTdd3G = false;
    int rssi = 0;

    property_get(PROPERTY_GSM_BASEBAND_CAPABILITY,prop1 , "0");
    property_get(PROPERTY_GSM_BASEBAND_CAPABILITY2,prop2 , "0");
    property_get(PROPERTY_GSM_BASEBAND_CAPABILITY3,prop3 , "0");

    baseband1 = atoi(prop1);
    baseband2 = atoi(prop2);
    baseband3 = atoi(prop3);

    if((baseband1 | baseband2 | baseband3)& 0x04){
        isFdd3G = true;
    }else if((baseband1 | baseband2 | baseband3)& 0x08){
        isTdd3G = true;
    }

    LOGD("convert3GRssiValue baseband1=%d ,baseband2=%d ,baseband3=%d ,isFdd3G=%d,isTdd3G=%d ,RSCP_in_qdbm=%d ,EcN0_in_qdbm=%d",baseband1,baseband2,baseband3,isFdd3G,isTdd3G,RSCP_in_qdbm,EcN0_in_qdbm);
...
void requestSetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    ****************************************/
    ...
    //For LTE --Start
    //for 4G/3G/2G(auto) and 4G preferred
    case NT_LTE_GSM_WCDMA_TYPE:
    case NT_LTE_GSM_WCDMA_PREF_TYPE:
    case NT_LTE_GSM_WCDMA_RAT_ONLY_TYPE:
        rat = 6;
        rat1= 4;
        break;
    case NT_LTE_GSM_WCDMA_PREF_MMDC_TYPE:
    case NT_LTE_GSM_WCDMA_PREF_MMDC_RAT_ONLY_TYPE:
        rat = 5;
        rat1= 4;
        break;

    //for 3G or 2G Preferred (3G, 2G /4G)
    case NT_GSM_WCDMA_LTE_TYPE:
    case NT_GSM_WCDMA_LTE_RAT_ONLY_TYPE:
        rat = 6;
        rat1= 3;
        break;
    case NT_GSM_WCDMA_LTE_MMDC_TYPE:
    ...
    if(rat >= 0)
    {
        ...
        if(req_type >= NT_RAT_ONLY_TYPE)//ALPS00282643
            //[ALPS01629965]-START: reset <prefer_rat>
            //err = asprintf(&cmd,  "AT+ERAT=%d", rat);
            err = asprintf(&cmd,  "AT+ERAT=%d,0", rat);
            //[ALPS01629965]-END
        else
...
void requestGetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    //for LTE -- START
        ...
        // 4G Preferred
        return_type = NT_LTE_GSM_WCDMA_PREF_TYPE;
    } else if(nt_type == 6 && prefer_type == 3) {
        // 3G or 2G item (for CSFB mode)
        return_type = NT_LTE_GSM_WCDMA_PREF_TYPE;
    } else if(nt_type == 5 && prefer_type == 0) {
        // 4G/3G
        return_type = NT_LTE_WCDMA_TYPE;
    } else if(nt_type == 5 && prefer_type == 4) {
        // 4G Preferred (for SGLTE mode)
        return_type = NT_LTE_GSM_WCDMA_PREF_MMDC_TYPE;
    } else if(nt_type == 5 && prefer_type == 2) {
        // 3G or 2G item (for SGLTE mode)
        return_type = NT_GSM_WCDMA_LTE_MMDC_TYPE;
...
#ifdef MTK_LTE_DC_SUPPORT
        LOGD("onDelayGprsDetach %d", rid);
        RILChannelCtx * p_channel = getRILChannelCtx(RIL_DATA, rid);
        RIL_requestProxyTimedCallback(onDelayGprsDetach, &s_nw_ril_cntx[rid], &MMDC_TIMEVAL,
                    p_channel->id, "onDelayGprsDetach");
#else
...
//[MMDC]
void onDelayGprsDetach (void* param){
    RILId rid = *((RILId *) param);
    LOGD("onDelayGprsDetach rid = %d", rid);
    RIL_onUnsolicitedResponse (RIL_UNSOL_GPRS_DETACH, NULL, 0, rid);
}
