...
void requestResetRadio(void * data, size_t datalen, RIL_Token t)
{
    ...
    for(i=0; i<MTK_GEMINI_SIM_NUM; i++){
        ...
        RIL_onUnsolicitedResponse(RIL_UNSOL_STK_SETUP_MENU_RESET, NULL, 0, MTK_RIL_SOCKET_1+i);
    }
...
#if MTK_IMS_SUPPORT
        err = at_send_command("AT+CIREG=1", &p_response, OEM_CHANNEL_CTX);
        if (err != 0 || p_response->success == 0)
...
extern void set3GCapability(RIL_Token t, int setting)
{
    ...
    int baseband = 0;
    char baseband_str[PROPERTY_VALUE_MAX] = {0};
    int simId3G = RIL_get3GSIM()-1;
    if (simId3G == GEMINI_SIM_1) {
        property_get(PROPERTY_GSM_BASEBAND_CAPABILITY, baseband_str , "0");
        baseband = atoi(baseband_str);
    } else if (simId3G == GEMINI_SIM_2) {
        property_get(PROPERTY_GSM_BASEBAND_CAPABILITY2, baseband_str , "0");
        baseband = atoi(baseband_str);
    } else if (simId3G == GEMINI_SIM_3) {
        property_get(PROPERTY_GSM_BASEBAND_CAPABILITY3, baseband_str , "0");
        baseband = atoi(baseband_str);
    } else if (simId3G == GEMINI_SIM_4) {
        property_get(PROPERTY_GSM_BASEBAND_CAPABILITY4, baseband_str , "0");
        baseband = atoi(baseband_str);
    }
    int preferredNwType = NETWORK_MODE_WCDMA_PREF; // default value
    if ((baseband | 128) > 1) { // LTE
        preferredNwType = NETWORK_MODE_GSM_UMTS_LTE;
    } else {
        preferredNwType = NETWORK_MODE_WCDMA_PREF;
    }
    asprintf(&cmd, "AT+ES3G=%d, %d", setting, preferredNwType);
#else
    asprintf(&cmd, "AT+ES3G=%d, %d", setting, NETWORK_MODE_WCDMA_PREF);
#endif // MTK_LTE_SUPPORT
...
extern void requestSet3GCapability(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (setting == -1) {
        ...
        int baseband = 0;
        char baseband_str[PROPERTY_VALUE_MAX] = {0};
        if (old3GSim == GEMINI_SIM_1) {
            property_get(PROPERTY_GSM_BASEBAND_CAPABILITY, baseband_str , "0");
            baseband = atoi(baseband_str);
        } else if (old3GSim == GEMINI_SIM_2) {
            property_get(PROPERTY_GSM_BASEBAND_CAPABILITY2, baseband_str , "0");
            baseband = atoi(baseband_str);
        } else if (old3GSim == GEMINI_SIM_3) {
            property_get(PROPERTY_GSM_BASEBAND_CAPABILITY3, baseband_str , "0");
            baseband = atoi(baseband_str);
        } else if (old3GSim == GEMINI_SIM_4) {
            property_get(PROPERTY_GSM_BASEBAND_CAPABILITY4, baseband_str , "0");
            baseband = atoi(baseband_str);
        }
        int preferredNwType = NETWORK_MODE_WCDMA_PREF; // default value
        ...
        asprintf(&cmd, "AT+ES3G=%d, %d", modem_setting_value, preferredNwType);
#else
...
extern int getCardSlot() {
    if (isDualTalkMode()){
        ...
        char *line;
        int response = 1;
        err = at_send_command_singleline("AT+CSUS?", "+CSUS:", &p_response, getChannelCtxbyProxy(MTK_RIL_SOCKET_1));
...
extern int getMappingSIMByCurrentMode(RILId rid) {
    ...
    LOGI("getMappingSIMByCurrentMode[isDualTalkMode, getFirstModem, getTelephonyMode, getExternalModemSlot, RIL_is3GSwitched] [%d, %d, %d, %d, %d]", isDualTalkMode(), getFirstModem(), getTelephonyMode(), getExternalModemSlot(), RIL_is3GSwitched());
    if (isDualTalkMode()) {
                ...
                default:
                    ...
                    if (rid == MTK_RIL_SOCKET_1
                        ...
                        if (getExternalModemSlot() >= 0) {
                            ...
                            if (isSimSwitchSwapped()) {
                            #ifdef MTK_RIL_MD1
                                LOGI("Update property SIM2 (inMD)[%s, %s]", key2, value != NULL ? value : "");
                                property_set(key2, value);
                            #else
                                LOGI("Update property SIM1 (inMD)[%s, %s]", key1, value != NULL ? value : "");
                                property_set(key1, value);
                            #endif
                            } else {
                            #ifdef MTK_RIL_MD1
...
extern int rilOemMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_CONFIG_SIM_SWITCH:
            requestConfigSimSwitch(data, datalen, t);
            break;
        default:
