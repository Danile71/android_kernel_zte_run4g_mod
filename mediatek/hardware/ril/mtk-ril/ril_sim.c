 ...
 */
int getCardStatus(RIL_CardStatus_v6 **pp_card_status, RILId rid) {
...
//MTK-END [mtkXXXXX][120208][APLS00109092] Replace "RIL_UNSOL_SIM_MISSING in RIL.java" with "acively query SIM missing status"
    ...
    if (sim_status == SIM_ABSENT) {
        ...
        property_set(PROPERTY_ECC_LIST[simId],"");
        LOGD("Clear ecc list since there is no card");
        num_apps = 0;
    } else {
...
static void simSecurityOperation(void *data, size_t datalen, RIL_Token t, SIM_Operation op)
{
    ...
    if ((ENTER_PIN1 == op) || (CHANGE_PIN1 == op)) {
        opRetryCount[0] = retryCount.pin1;
    } else if ((ENTER_PIN2 == op) || (CHANGE_PIN2 == op)) {
        opRetryCount[0] = retryCount.pin2;
    } else if (ENTER_PUK1 == op) {
        opRetryCount[0] = retryCount.puk1;
    } else if (ENTER_PUK2 == op) {
        opRetryCount[0] = retryCount.puk2;
    }
    LOGD("simSecurityOperation: op=%d, RetryCount=%d", op, opRetryCount[0]);
...
/*      while (1)
    ...
    RIL_onRequestComplete(t, ret, &opRetryCount, sizeof(int *));
...
void makeSimRspFromUsimFcp(unsigned char ** simResponse)
{
    ...
    free(*simResponse);
    free(fcpByte);
    LOGD("makeSimRspFromUsimFcp, format_wrong: %d", format_wrong);
    if (format_wrong != 1) {
        *simResponse = byteArrayToHexString(simRspByte, GET_RESPONSE_EF_SIZE_BYTES);
        LOGD("simRsp done:%s", *simResponse);
    } else {
        *simResponse = NULL;
        LOGD("simRsp done, but simRsp is null because command format may be wrong");
    }
    ...
    if ((isUsimDetect[rid] == 1) && (p_args->command == CMD_GET_RESPONSE)) {
        ...
        if ((RIL_E_SUCCESS == ret) && (sr.simResponse != NULL)) {
            ...
            if (sr.simResponse == NULL) {
                LOGD("The response format may be wrong");
                ret = RIL_E_GENERIC_FAILURE;
            }
        }
...
void onSimMeLockEvent(const char *s, RILId rid) {
    int eventId = 0;
...
void requestSimMeLockUpdateSettings(void *data, size_t datalen, RIL_Token t) {
    ...
    LOGD("[SIM ME Lock] requestSimMeLockUpdateSettings: data=%s", data);

    asprintf(&cmd, "AT+ETMOSLB=1,%d,\"%s\"", ((strlen(data) + 1)/2), data);
    err = at_send_command(cmd, &p_response, SIM_CHANNEL_CTX);
    free(cmd);
