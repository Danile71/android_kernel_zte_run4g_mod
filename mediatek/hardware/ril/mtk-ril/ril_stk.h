...
#define MTK_REQUEST_STK_SEND_ENVELOPE_COMMAND_WITH_STATUS(data,datalen,token) \
        requestStkSendEnvelopeCommandWithStatus(data,datalen,token)
...
#define MTK_UNSOL_STK_CALL_CTRL(token,channel) \
        onStkCallControl(token,channel)
