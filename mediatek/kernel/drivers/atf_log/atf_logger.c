...
static int __init atf_log_init(void)
{
    ...
    // map control header
    atf_buf_vir_ctl = ioremap(atf_buf_phy_ctl, ATF_LOG_CTRL_BUF_SIZE);
    atf_log_len = atf_buf_vir_ctl->info.atf_buf_size;
    // map log buffer
    atf_log_vir_addr = ioremap(atf_buf_phy_ctl + ATF_LOG_CTRL_BUF_SIZE, atf_log_len);
    pr_notice("atf_buf_phy_ctl: 0x%x\n", atf_buf_phy_ctl);
    pr_notice("atf_buf_len: %u\n", atf_buf_len);
    pr_notice("atf_buf_vir_ctl: 0x%x\n", atf_buf_vir_ctl);
    pr_notice("atf_log_vir_addr: 0x%x\n", atf_log_vir_addr);
