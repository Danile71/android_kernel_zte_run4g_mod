...
#define CLR_OWN_RETRY 50
    ...
    do {
        ...
        if (ret) {
            ...
            //osal_dbg_assert_aee("<HIF_SDIO> sdio_readl ERROR","read CHLPCR by SDIO report error");
            goto out;
        }
        if ((chlcpr_value & C_FW_COM_DRV_OWN) == C_FW_COM_DRV_OWN) {
...
static void stp_sdio_tx_rx_handling(void *pData)
{
    ...
    while (!osal_thread_should_stop(&pInfo->tx_rx_thread))
    {
		...
		if ((0 != pInfo->sleep_flag) || (0 != pInfo->wakeup_flag))
		{
			...
			osal_raise_signal(&pInfo->isr_check_complete);
                ...
                if (0 == (own_fail_counter % 50))
                {
					...
					if ((0 == own_fail_counter) || (0 == (own_fail_counter/500)) )
        ...
        if (1000 < while_loop_counter)
        {
            ...
            STPSDIO_INFO_FUNC("sleeping check result. stp_is_ready(%d) irq_pending(%d) tx_packet_num(%d) rx_pkt_len(%d) chisr(0x%08x)\n",
                mtk_wcn_stp_is_ready(),
                pInfo->irq_pending,
            ...
            if ((0 != pInfo->sleep_flag) || (0 != pInfo->wakeup_flag))
            {
                ...
                {
                    STPSDIO_INFO_FUNC("STP tx window full, proberbly STP-SDIO cannot handle tx request. tx_packet_num:%d\n", pInfo->firmware_info.tx_packet_num);
                    osal_sleep_ms(200);
                }
            }
...
static void stp_sdio_txdbg_dump(void)
{
    ...
    for (i = 0; i < STP_SDIO_TXDBG_COUNT; ++i) {
        ...
        printk(KERN_ERR DFT_TAG"stp_sdio_txdbg_buffer idx(%x) bus_txlen(0x%x, %d) ts(%d)\n",
        ...
        for (j = 0; j < STP_SDIO_TX_ENTRY_SIZE && j < len; j += 16) {
            ...
            printk(KERN_ERR DFT_TAG"[0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x   0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x]\n",
                pbuf[0], pbuf[1], pbuf[2], pbuf[3], pbuf[4], pbuf[5], pbuf[6], pbuf[7],
                pbuf[8], pbuf[9], pbuf[10], pbuf[11], pbuf[12], pbuf[13], pbuf[14], pbuf[15]);
            msleep(10);
        }
        printk(KERN_ERR DFT_TAG"stp_sdio_txdbg_buffer dump ok\n");
    }
    ...
    printk(KERN_ERR DFT_TAG"Dump tx info: pkt_num(%d) fifo(%d) pkt_list.rd(0x%x, %ld) pkt_list.wr(0x%x, %ld)\n",
    ...
    for (i = 0; i < STP_SDIO_TX_PKT_LIST_SIZE; ++i) {
        ...
        printk(KERN_ERR DFT_TAG"tx_pkt_list idx(0x%x, %d) size(0x%x, %d), in_ts(%d), out_ts(%d)\n",
    ...
    for (i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
        ...
        printk(KERN_ERR DFT_TAG"pkt_buf.tx_buf dump ok\n");
    }
