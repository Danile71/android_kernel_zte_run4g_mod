...
{    
   ...
   WMT_DBG_FUNC("**jiffies_to_mesecs(0xffffffff) = %lu\n", 
    ...
    WMT_DBG_FUNC("**poll_during_time = %lu > %lu, sdio_during_count = %lu > %lu, query\n", 
            jiffies_to_msecs(poll_during_time), TIME_THRESHOLD_TO_TEMP_QUERY,
...
INT32 wmt_dev_tm_temp_query(void)
{
    ...
    for(index = 0; index < HISTORY_NUM ; index++)
    {
       ...
       {
            query_cond = 1;
            WMT_DBG_FUNC("high temperature (current temp = %d), we must keep querying temp temperature..\n", temp_table[index]);
       }            
    }
...
#define WMT_IOCTL_WMT_STP_ASSERT_CTRL   _IOW(WMT_IOC_MAGIC, 27, int)
    ...
    case WMT_IOCTL_SET_STP_MODE: /* stp/hif/fm mode */
        ...
        do {
            ...
            if (0 != iRet)
            {
                WMT_INFO_FUNC("wmt_lib_set_hif fail (%u)\n", arg);
                break;
            }
