        ...
        if(prAdapter->fgIsFwOwn == FALSE && wlanSendNicPowerCtrlCmd(prAdapter, 1) == WLAN_STATUS_SUCCESS) {
            ...
            while(1) {
                ...
                if ((u4Value & WCIR_WLAN_READY) == 0)
                    ...
                    DBGLOG(INIT, WARN, ("%s: Failure to get RDY bit cleared! CardRemoved[%u] BusFailed[%u] Timeout[%u]",
                        __FUNCTION__,
                        kalIsCardRemoved(prAdapter->prGlueInfo),
                        fgIsBusAccessFailed,
                        i));
    ...
    )
{
    ...
    if (prAdapter->prGlueInfo->u4Flag & GLUE_FLAG_HALT) {
        ...
        if(prAdapter->ucOidTimeoutCount >= WLAN_OID_NO_ACK_THRESHOLD) {
            if(!prAdapter->fgIsChipNoAck) {
                DBGLOG(INIT, WARN, ("No response from chip for %u times, set NoAck flag!\n", prAdapter->ucOidTimeoutCount));
            }
    ...
    )
{
    ...
    DBGLOG(RX, WARN, ("%s: IndicatedRfbList num = %lu\n", __FUNCTION__, prQueList->u4NumElem));
    ...
    )
{
    ...
    if (kalQoSFrameClassifierAndPacketInfo(prAdapter->prGlueInfo,
        ...
        if (fgIs1x) {
            ...
            prStaRec = cnmGetStaRecByAddress(prAdapter, GLUE_GET_PKT_BSS_IDX(prPacket), aucEthDestAddr);
...
#if CFG_SUPPORT_MULTITHREAD
        nicTxComposeSecurityFrameDesc(prAdapter, prCmdInfo, prSecFrameInfo->ucTxDescBuffer, NULL);
#endif
    ...
    if (GET_BSS_INFO_BY_INDEX(prAdapter, prCmdInfo->ucBssIndex)->eNetworkType == NETWORK_TYPE_AIS &&
        ...
        if (prSta) {
            ...
            if (authSendDeauthFrame(prAdapter,
                                    GET_BSS_INFO_BY_INDEX(prAdapter, prCmdInfo->ucBssIndex),
                                    prSta,
                                    (P_SW_RFB_T)NULL,
    ...
    )
{
    ...
    if (prRegInfo->prOldEfuseMapping->uc11AcTxPwrValid2G) {
        ...
        CMD_TX_AC_PWR_T     rCmdAcPwr;
        kalMemCopy(&rCmdAcPwr.rAcPwr ,&prRegInfo->prOldEfuseMapping->r11AcTxPwr2G, sizeof(AC_PWR_SETTING_STRUCT));
    ...
    kalMemCopy(&rCmdNvramSettings.rNvramSettings,&prRegInfo->prNvramSettings->u2Part1OwnVersion, sizeof(WIFI_CFG_PARAM_STRUCT));
    ...
    )
{
    ...
    prWifiVar->ucThreadPriority = (UINT_8)wlanCfgGetUint32(prAdapter,"ThreadPriority", WLAN_TX_THREAD_TASK_PRIORITY);
    ...
    prWifiVar->u4MaxTxDeQLimit = (UINT_32)wlanCfgGetUint32(prAdapter, "MaxTxDeQLimit", 0xFFFFFFFF);
    prWifiVar->ucAlwaysResetUsedRes = (UINT_32)wlanCfgGetUint32(prAdapter, "AlwaysResetUsedRes", 0x3);
