    ...
    )
{
    ...
    if (!prStaRec) {
        ...
        if(!prStaRec) {
            DBGLOG(BSS, WARN, ("STA_REC entry is full, cannot acquire new entry for ["MACSTR"]!!\n", 
    ...
    for (ucBssIndex = 0; ucBssIndex <= P2P_DEV_BSS_INDEX; ucBssIndex++) {
       ...
       if (fgReplyProbeResp) {
            if(nicTxGetFreeCmdCount(prAdapter) > (CFG_TX_MAX_CMD_PKT_NUM/2) ){
                ...
                bssSendBeaconProbeResponse(prAdapter, ucBssIndex, prMgtHdr->aucSrcAddr, u4CtrlFlagsForProbeResp);
            }
        }
    }
    ...
    )
{
    ...
    for(eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++){
        ...
        DBGLOG(BSS, INFO, ("Bcast: eAci = %d, ACM = %d, Aifsn = %d, CWmin = %d, CWmax = %d, TxopLimit = %d\n",
                   eAci,prACQueParms[eAci].ucIsACMSet ,
    ...
    )
{
    ...
    DBGLOG(SW4, INFO, ("Channel[%u] Band[%u] SCO[%u] Assoc40mBwAllowed[%u] 40mBwAllowed[%u] MaxBw[%u]\n",
