...
{  
    ...
    MTK_OUI_IE(pucBuffer)->aucCapability[0] = MTK_SYNERGY_CAP0;
    MTK_OUI_IE(pucBuffer)->aucCapability[1] = MTK_SYNERGY_CAP1;
    MTK_OUI_IE(pucBuffer)->aucCapability[2] = MTK_SYNERGY_CAP2;
    MTK_OUI_IE(pucBuffer)->aucCapability[3] = MTK_SYNERGY_CAP3;
	...
	)
{
	...
	kalMemZero((PVOID)prVhtSupportedMcsSet,
                sizeof(VHT_SUPPORTED_MCS_FIELD));
    ...
    )
{
     ...
     */
    //DBGLOG(RLM, INFO,("[Channel Switch] rlmRecIeInfoForClient\n"));
    ...
    IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
        ...
        case ELEM_ID_HT_CAP:
            ...
            kalMemCopy(prStaRec->aucRxMcsBitmask, prHtCap->rSupMcsSet.aucRxMcsBitmask, sizeof(prStaRec->aucRxMcsBitmask) /*SUP_MCS_RX_BITMASK_OCTET_NUM*/);
            ...
            else if(IS_FEATURE_DISABLED(prWifiVar->ucTxLdpc) || (wlanGetEcoVersion(prAdapter) <= 2)) {
                prStaRec->u2HtCapInfo &= ~HT_CAP_INFO_LDPC_CAP;
            }   
		...
		case ELEM_ID_VHT_CAP:
            ...
            prStaRec->u2VhtRxHighestSupportedDataRate= prVhtCap->rVhtSupportedMcsSet.u2RxHighestSupportedDataRate;
            prStaRec->u2VhtTxMcsMap= prVhtCap->rVhtSupportedMcsSet.u2TxMcsMap;
            prStaRec->u2VhtTxHighestSupportedDataRate= prVhtCap->rVhtSupportedMcsSet.u2TxHighestSupportedDataRate;
        ...
        case ELEM_ID_OP_MODE:
            ...
            if((prOPModeNotification->ucOpMode & VHT_OP_MODE_RX_NSS_TYPE) \
                ...
                fgHasOPModeIE = TRUE;
                ucVhtOpModeChannelWidth = ((prOPModeNotification->ucOpMode)& VHT_OP_MODE_CHANNEL_WIDTH);
            }
    ...
    )
{
    ...
    prBssInfo->fgUseShortSlotTime =
        ((prBssInfo->u2CapInfo & CAP_INFO_SHORT_SLOT_TIME) || (prBssInfo->eBand != BAND_2G4)) ? TRUE : FALSE;
    ...
    )
{
    ...
    IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
        ...
        case ELEM_ID_HT_CAP:
            ...
            else if(IS_FEATURE_DISABLED(prAdapter->rWifiVar.ucTxLdpc) || (wlanGetEcoVersion(prAdapter) <= 2)) {
                prStaRec->u2HtCapInfo &= ~HT_CAP_INFO_LDPC_CAP;
            }   
