    ...
    )
{
    ...
    DBGLOG(SCN,TRACE,("SCN: scanAddToBssDesc\n"));
    ...
    prBssDesc = scanSearchExistingBssDescWithSsid(prAdapter,
        ...
        DBGLOG(SCN, TRACE,("SCN: %x, %x, %x, %x, %x, %x\n", prWlanBeaconFrame->aucBSSID[0], 
        prWlanBeaconFrame->aucBSSID[1], prWlanBeaconFrame->aucBSSID[2],
        prWlanBeaconFrame->aucBSSID[3], prWlanBeaconFrame->aucBSSID[4],
        prWlanBeaconFrame->aucBSSID[5]));
    ...
    if (prBssDesc == (P_BSS_DESC_T)NULL) {
    	...
    	DBGLOG(SCN, TRACE,("SCN: RxCh=%d, BssCh=%d, BssRcpi=%d, RxRcpi=%d\n",
		HAL_RX_STATUS_GET_CHNL_NUM(prSwRfb->prRxStatus),prBssDesc->ucChannelNum,
        ...
        if (prBssDesc->eBSSType != eBSSType) {
            ...
            ASSERT(prSwRfb->prRxStatusGroup3);
            DBGLOG(SCN, TRACE,("SCN: CTime =%d, UpdateTime =%d\n", rCurrentTime,prBssDesc->rUpdateTime));
            if((prBssDesc->ucRCPI - HAL_RX_STATUS_GET_RCPI(prSwRfb->prRxStatusGroup3)) >= REPLICATED_BEACON_STRENGTH_THRESHOLD &&
    ...
    //4 <4> Update information from HIF RX Header
    {
        ...
        // 5G Band
            ...
            DBGLOG(SCN, TRACE,("SCN: IeNum=%d, HwNum=%d\n", ucIeHtChannelNum, ucHwChannelNum));
    ...
    if (prBssDesc) {
        ...
        if(((prBssDesc->eBSSType == BSS_TYPE_INFRASTRUCTURE && prConnSettings->eOPMode != NET_TYPE_IBSS)
            ...
            if (EQUAL_SSID(prBssDesc->aucSSID,
                ...
                ibssProcessMatchedBeacon(prAdapter, prAisBssInfo, prBssDesc, (UINT_8)HAL_RX_STATUS_GET_RCPI(prSwRfb->prRxStatusGroup3));
            }
...
#if 0 //Enable this dbg message would encounter KE. Please disable it after debugging.
	...
	else
	{
        ...
        if ((prBssDesc->ucChannelNum <= 48) && (prBssDesc->ucChannelNum >= 1))
        {
            if (prBssDesc->ucChannelNum <= 14)
                prChnLoad = (P_PARAM_CHN_LOAD_INFO)&(prAdapter->rWifiVar.rChnLoadInfo.rEachChnLoad[prBssDesc->ucChannelNum-1]);
            else
                prChnLoad = (P_PARAM_CHN_LOAD_INFO)&(prAdapter->rWifiVar.rChnLoadInfo.rEachChnLoad[(prBssDesc->ucChannelNum/4)+5]);
            prChnLoad->u2APNum++;
        }
