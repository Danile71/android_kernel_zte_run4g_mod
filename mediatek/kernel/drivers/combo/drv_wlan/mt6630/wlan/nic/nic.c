    ...
    )
{
    ...
    do {
        ...
        if(prAdapter->prSDIOCtrl == NULL) {
            DBGLOG(INIT, ERROR, ("Could not allocate %d bytes for interrupt response.\n",
    ...
    )
{
    ...
    if (EQUAL_MAC_ADDR (aucZeroMacAddr, prBssInfo->aucBSSID)) {
	    prBssInfo->ucBMCWlanIndex = secPrivacySeekForBcEntry(prAdapter, prBssInfo->ucBssIndex, prBssInfo->aucOwnMacAddr,
    ...
    DBGLOG(BSS, TRACE, ("[wlan index][Network] OwnMac="MACSTR" BSSID="MACSTR" NetType=%d BCIndex=%d\n",
        MAC2STR(prBssInfo->aucOwnMacAddr), MAC2STR(prBssInfo->aucBSSID), prBssInfo->eNetworkType, prBssInfo->ucBMCWlanIndex));
#endif
...
#if DBG
    DBGLOG(BSS, TRACE, ("[wlan index][Network]=%d activate=%d\n", ucBssIndex, 0));
    DBGLOG(BSS, TRACE, ("[wlan index][Network] OwnMac="MACSTR" BSSID="MACSTR" BCIndex = %d\n",
    ...
    )
{
    ...
    DBGLOG(REQ, TRACE, ("Update Bss[%u] ConnState[%u] OPmode[%u] BSSID["MACSTR"] AuthMode[%u] EncStatus[%u]\n",
        ucBssIndex, 
...
#if CFG_ENABLE_WIFI_DIRECT
        || ((GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eNetworkType == NETWORK_TYPE_P2P) && (prAdapter->fgIsP2PRegistered))
#endif
         ) {
    ...
    case NETWORK_TYPE_AIS:
        if(GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
            ...
            if(prAdapter->fgIsLinkQualityValid == FALSE
                ...
                nicUpdateRSSI(prAdapter, ucBssIndex, prEventLinkQuality->rLq[ucBssIndex].cRssi, prEventLinkQuality->rLq[ucBssIndex].cLinkQuality);
            }

            if(prAdapter->fgIsLinkRateValid == FALSE
                    || (kalGetTimeTick() - prAdapter->rLinkRateUpdateTime) > CFG_LINK_QUALITY_VALID_PERIOD) {
                nicUpdateLinkSpeed(prAdapter, ucBssIndex, prEventLinkQuality->rLq[ucBssIndex].u2LinkSpeed);
            }
