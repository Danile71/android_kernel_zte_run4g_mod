...
** $Log: nic_rx.c $
**
** 05 28 2014 eason.tsai
** [ALPS01070904] [Need Patch] [Volunteer Patch]
** add event to handle dump burst mode
**
    ...
    )
{
    ...
    //non BA session
        ...
        fgDrop = TRUE;
        if (!HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus) && HAL_RX_STATUS_IS_TKIP_MIC_ERROR(prRxStatus)) {
    ...
    )
{
    ...
    case EVENT_ID_TX_DONE:
        {
            ...
            DBGLOG(INIT, INFO,("EVENT_ID_TX_DONE PacketSeq:%u ucStatus: %u SN: %u\n",
    ...
    case EVENT_ID_STA_ABSENCE_TX:
        qmHandleEventStaAbsenceTx(prAdapter, prEvent);
        break;
    ...
    case EVENT_ID_BATCH_RESULT:
        ...
        case EVENT_ID_TDLS:
            ...
            TdlsexEventHandle(\
                ...
                (UINT32)(prEvent->u2PacketLength-8));
            break;
            ...
            for (i = 0; i < u2RxPktNum ; i++) {
                ...
                if (ALIGN_4(u4RxLength + HIF_RX_HW_APPENDED_LEN) < u4RxAvailAggLen) {
                    if (u4RxAggCount < u4CurrAvailFreeRfbCnt) {
                        ...
                        DBGLOG(RX, ERROR, ("[%s] RxAggCount(%d) is greater than AvailableFreeCount(%d)\n",
                            __func__,
                            u4RxAggCount,
                    ...
                    DBGLOG(RX, ERROR, ("[%s] Request_len(%d) is greater than Available_len(%d)\n",
                        __func__,
    ...
    )
{
    ...
    if (fgRobustAction && prSwRfb->prStaRec && 
        ...
        DBGLOG(RSN, INFO, ("[Rx]RobustAction %x %x %x\n", prSwRfb->prRxStatus->u2StatusFlag, prSwRfb->prRxStatus->ucWlanIdx, prSwRfb->prRxStatus->ucTidSecMode));
