    ...
    )
{
    ...
    for(u4Idx = 0; u4Idx < CFG_NUM_OF_RX_BA_AGREEMENTS; u4Idx++){
        ...
        cnmTimerInitTimer(prAdapter,
                    ...
                    (UINT_32)(&prQM->arRxBaTable[u4Idx]));
    ...
    )
{
    ...
    prStaRec->fgIsAbsenceTxAllowed = FALSE;
}
    ...
    )
{
    ...
    P_BSS_INFO_T    prBssInfo;
    BOOLEAN         fgCheckACMAgain, fgDropPacket;
    ...
    )
{
    ...
    while(ucLoop < CFG_NUM_OF_STA_RECORD) {
        ...
        if(prStaRec->fgIsTxAllowed) {
            ...
            /* Absent BSS handling */
            if(prBssInfo->fgIsNetAbsent && !prStaRec->fgIsAbsenceTxAllowed) {
            ...
            //4 <2.4> Update quota usage
            /* AP mode: Update STA in PS Free quota */
            if(prStaRec->fgIsInPS && pucPsStaFreeQuota) { 
#if 0
                ASSERT((*pucPsStaFreeQuota) >= u4CurStaForwardFrameCount);
            ...
            if(prBssInfo->fgIsNetAbsent && !prStaRec->fgIsAbsenceTxAllowed) {
#if 0
                ASSERT(prBssInfo->ucBssFreeQuota >= u4CurStaForwardFrameCount);
    ...
    )
{
    ...
    DBGLOG(QM, LOUD, ("(Non-Fairness) TC[%u] Left Resource[%u]\n",
        ucTC, 
        u4AvaliableResource));
    ...
    for(u4Tc = 0; u4Tc < QM_ACTIVE_TC_NUM; u4Tc++) {
        ...
        for (u4StaRecIdx = 0; u4StaRecIdx < CFG_NUM_OF_STA_RECORD; u4StaRecIdx++){
            ...
            if(prStaRec) {
                ...
                if ((prStaRec->fgIsValid) && 
                    (!prBssInfo->fgIsNetAbsent || prStaRec->fgIsAbsenceTxAllowed)) {
    ...
    )
{
    ...
    if(prRxStatusGroup4 == NULL){
        ...
        DBGLOG(QM, ERROR,("prSwRfb->pvHeader is 0x%08X !!!\n", (PUINT_32)prSwRfb->pvHeader));
...
{
    P_RX_BA_ENTRY_T prReorderQueParm = (P_RX_BA_ENTRY_T)u4Param;    
...
VOID
qmHandleEventCheckReorderBubble(
    IN P_ADAPTER_T prAdapter,
    ...
    )
{
    ...
    prReorderQueParm = qmLookupRxBaEntry(
                        prAdapter, 
        ...
        if (IS_BSS_ACTIVE(prBssInfo)) {
            if (prBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE &&
                ...
                if (EQUAL_MAC_ADDR(prBssInfo->aucBSSID,
                    ((P_WLAN_MAC_MGMT_HEADER_T)(prSwRfb->pvHeader))->aucBSSID)) {
    ...
    )
{
    ...
    prAcQueParams->u2CWmin =
        BIT(prAcParam->ucEcw & WMM_ECW_WMIN_MASK) - 1;
    ...
    )
{
    ...
    do {
        ...
        if(prStaRec) {
            ...
            if(prStaRec->fgIsInPS) {   
                ...
                if(nicTxGetResource(prAdapter, nicTxGetFrameResourceType(eFrameType, prMsduInfo)) >= QM_MGMT_QUEUED_THRESHOLD) {
                    eFrameAction = FRAME_ACTION_TX_PKT;
                    break;
                }
                //4 <3.2.2> Queue, if resource is not enough
                else {
                    DBGLOG(QM, TRACE, ("Queue packets (Sta[%u] in PS)\n", prStaRec->ucIndex));
                    eFrameAction = FRAME_ACTION_QUEUE_PKT;
                    break;
                }
            }

            fgAbsenceTx = prStaRec->fgIsAbsenceTxAllowed;
        }

        //4 <4> Queue, if BSS is absent
        if(prBssInfo->fgIsNetAbsent && !fgAbsenceTx){
    ...
    if (!IS_BSS_ACTIVE(prBssInfo)) {
        ...
        DBGLOG(QM, INFO, ("Drop packets Action (Inactive BSS %u).\n",prBssInfo->ucBssIndex));
        TX_INC_CNT(&prAdapter->rTxCtrl, TX_INACTIVE_BSS_DROP);
    ...
    do {
        if(eFrameType == FRAME_TYPE_MMPDU) {            
            ...
            /* MMPDU with prStaRec && fgIsInUse not check fgIsNetActive */
                ...
                if(prStaRec){
                    if(prStaRec->fgIsInPS) {
                       if( nicTxGetResource (prAdapter, TC4_INDEX)>= QM_MGMT_QUEUED_THRESHOLD) {
                            return FRAME_ACTION_TX_PKT;
                       }
                       else {
...
/*----------------------------------------------------------------------------*/
VOID
qmHandleEventStaAbsenceTx(
    IN P_ADAPTER_T prAdapter,
    IN P_WIFI_EVENT_T prEvent
    )
{
    P_EVENT_STA_CHANGE_ABSENCE_TX_T prEventStaAbsenceTx;
    P_STA_RECORD_T prStaRec;

    prEventStaAbsenceTx = (P_EVENT_STA_CHANGE_ABSENCE_TX_T)prEvent;
    prStaRec = QM_GET_STA_REC_PTR_FROM_INDEX(prAdapter, prEventStaAbsenceTx->ucStaRecIdx);

    if(prStaRec) {

        prStaRec->fgIsAbsenceTxAllowed = prEventStaAbsenceTx->ucAllowAbsenceTx ? TRUE:FALSE;

        DBGLOG(QM, INFO, ("Absence Tx: STA[%u] EN[%u]\n",
            prEventStaAbsenceTx->ucStaRecIdx,
            prStaRec->fgIsAbsenceTxAllowed));
    }
