    ...
    if(prDevStats) {
        ...
        if (rStatus != WLAN_STATUS_SUCCESS) {
            ...
            DBGLOG(REQ, INFO, ("BSSID: ["MACSTR"] TxFailCount %d LifeTimeOut %d \n", 
                   MAC2STR(arBssid), rQueryStaStatistics.u4TxFailCount, rQueryStaStatistics.u4TxLifeTimeoutCount));
    ...
    )
{
	...
	*/
	...
	if (rStatus != WLAN_STATUS_SUCCESS) { 
     ...
     //printk("\n mtk_cfg80211_change_station prAisBssInfo->fgTdlsIsChSwProhibited %d \n", prAisBssInfo->fgTdlsIsChSwProhibited);
    ...
    IN P_GLUE_INFO_T prGlueInfo)
{
    ...
    UINT_8 ucIdx=0,ucMax_24G_Chn_List=11 , ucChValidCnt=0;
	...
	//}
    	...
    	for(ucIdx = 0; ucIdx < ucMax_24G_Chn_List; ucIdx++) {
    		...
    		{
    			if ((PreferChannels[1].u2APNum >= prChnLoad->u2APNum) || (fgIsFistRecord == 1))
    			{
