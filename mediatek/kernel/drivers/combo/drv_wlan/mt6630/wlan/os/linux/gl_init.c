...
static struct cfg80211_ops mtk_wlan_ops = {
    ...
    #endif
    //.sched_scan_start           = mtk_cfg80211_sched_scan_start,  /* Remove schedule_scan because we need more verification for NLO */
    //.sched_scan_stop            = mtk_cfg80211_sched_scan_stop,  /* Remove schedule_scan because we need more verification for NLO */
    #if CFG_SUPPORT_TDLS
...
/* load EFUSE overriding part */
        ...
        for (i = 0 ; i < sizeof(RSSI_PATH_COMPASATION_T) ; i += sizeof(UINT_16)) {
            kalCfgDataRead16(prGlueInfo,
                    OFFSET_OF(WIFI_CFG_PARAM_STRUCT, rRssiPathCompensation) + i,
    ...
    )
{
    ...
    aucDebugModule[DBG_MEM_IDX] = DBG_CLASS_ERROR | \
            DBG_CLASS_WARN;
#endif /* DBG */
...
#if CFG_SUPPORT_TDLS
    prWdev->wiphy->flags |= WIPHY_FLAG_CUSTOM_REGULATORY | WIPHY_FLAG_SUPPORTS_FW_ROAM|WIPHY_FLAG_TDLS_EXTERNAL_SETUP|WIPHY_FLAG_SUPPORTS_TDLS;
#endif
    ...
    )
{
    ...
    do {
...
#if CFG_SUPPORT_CFG_FILE
        {
            ...
            u4ConfigReadLen = 0;
            if(pucConfigBuf) {
                if(kalReadToFile("/storage/sdcard0/wifi.cfg", 
...
bailout:
        ...
        if(prGlueInfo->prAdapter->rWifiVar.ucThreadPriority > 0) {
            ...
            sched_setscheduler(prGlueInfo->main_thread , prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling, &param);
#if CFG_SUPPORT_MULTITHREAD 
            sched_setscheduler(prGlueInfo->hif_thread , prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling, &param);
            sched_setscheduler(prGlueInfo->rx_thread , prGlueInfo->prAdapter->rWifiVar.ucThreadScheduling, &param);
#endif            
        ...
        {
            P_GL_BOW_INFO prBowInfo = kalGetBowInfo();

            /* Store GlueInfo pointer in BoW Info */
            prBowInfo->prGlueInfo = prGlueInfo;
            prBowInfo->fgIsNetRegistered = FALSE;
            prBowInfo->fgIsRegistered = TRUE;

            /* Store BoW Info pointer in GlueInfo */
            prGlueInfo->prBowInfo = prBowInfo;
        }
#endif
    ...
    )
{
...
#if CFG_ENABLE_BT_OVER_WIFI
    if(prGlueInfo->prBowInfo->fgIsNetRegistered) {
    ...
    prGlueInfo->prBowInfo->fgIsRegistered = FALSE;
#endif
