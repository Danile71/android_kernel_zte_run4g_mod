...
#elif defined(MT6630)
    pcFwName = "/storage/sdcard0/"CFG_FW_FILENAME"_MT6630";
    filp = filp_open(pcFwName, O_RDONLY, 0);
    if (IS_ERR(filp)) {
        pcFwName = "/etc/firmware/"CFG_FW_FILENAME"_MT6630";
        filp = filp_open(pcFwName, O_RDONLY, 0);
    }
#else
    filp = filp_open("/etc/firmware/"CFG_FW_FILENAME, O_RDONLY, 0);
    ...
    )
{
    ...
    if(prSkb->len > ETH_802_3_MAX_LEN){
		DBGLOG(RX, ERROR, ("kalRxIndicateOnePkt [prSkb = 0x%08X][prSkb->len = %d][prSkb->protocol = 0x%02X] \n", (PUINT_8)prSkb, prSkb->len, prSkb->protocol));
    ...
    )
{
    ...
    case WLAN_STATUS_MEDIA_CONNECT:
        ...
        if (prGlueInfo->fgIsRegistered == TRUE) {
            /* retrieve channel */
            ucChannelNum = wlanGetChannelNumberByNetwork(prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex);
            if(ucChannelNum <= 14) {
            ...
            if(eStatus == WLAN_STATUS_MEDIA_CONNECT
                ...
                struct ieee80211_channel *prChannel = NULL;
                UINT_8 ucChannelNum = wlanGetChannelNumberByNetwork(prGlueInfo->prAdapter, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex);
    ...
    )
{
    ...
    BOOLEAN fgSecFrameProcessDone = FALSE;
    ...
    if(GLUE_GET_PKT_IS_1X(prSkb)) {
        if(wlanProcessSecurityFrame(prGlueInfo->prAdapter, (P_NATIVE_PACKET)prSkb)) {
            ...
            fgSecFrameProcessDone = TRUE;
        }
    }
#endif    

    /* Handle normal data frame */
    if(!fgSecFrameProcessDone) {
        ...
        if (GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]) 
			...
			DBGLOG(TX, INFO, ("Stop subqueue for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%ld] PER-Q_CNT[%ld]\n", 
					ucBssIndex, 
					u2QueueIdx, 
					prSkb->len,
...
#else
        if (GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum) >= CFG_TX_STOP_NETIF_QUEUE_THRESHOLD) {
            netif_stop_queue(prDev);
			
			DBGLOG(TX, INFO, ("Stop queue for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%ld] PER-Q_CNT[%ld]\n", 
					ucBssIndex, 
					u2QueueIdx, 
					prSkb->len,
					GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
    ...
    DBGLOG(TX, LOUD, ("Enqueue frame for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%ld] PER-Q_CNT[%ld]\n", 
        ucBssIndex, 
        u2QueueIdx, 
        prSkb->len,
        GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
...
{
    DBGLOG(QM, INFO, ("Reset NetDev[0x%08x] statistics\n", prDev));
    ...
    )
{
    ...
    if((GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum) <= 0)) {
        ...
        printk("TxPendingFrameNum[%u] CurFrameId[%u]\n", prGlueInfo->i4TxPendingFrameNum, GLUE_GET_PKT_ARRIVAL_TIME(pvPacket));
    ...
    DBGLOG(TX, LOUD, ("Release frame for BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%ld] PER-Q_CNT[%ld]\n", 
        ucBssIndex, 
        u2QueueIdx, 
        prSkb->len,
        GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
    ...
    if(fgIsValidDevice == TRUE) {
        ...
        if (netif_subqueue_stopped(prDev, prSkb) &&
            ...
            netif_wake_subqueue(prDev, u2QueueIdx);
			 DBGLOG(TX, INFO, ("WakeUp Queue BSS[%u] QIDX[%u] PKT_LEN[%u] TOT_CNT[%ld] PER-Q_CNT[%ld]\n", 
		        ucBssIndex, 
		        u2QueueIdx, 
		        prSkb->len,
		        GLUE_GET_REF_CNT(prGlueInfo->i4TxPendingFrameNum),
...
kalQoSFrameClassifierAndPacketInfo (
    ...
    OUT PBOOLEAN pfgIs802_3,
    OUT PBOOLEAN pfgIsVlanExists
    )
{
    UINT_32 u4PacketLen;
    ...
    *pucPriorityParam = prSkb->priority;
    ...
    )
{
    ...
    *pfgIsPAL = FALSE;
    ...
    u2EtherTypeLen = (aucLookAheadBuf[ETHER_TYPE_LEN_OFFSET] << 8) | (aucLookAheadBuf[ETHER_TYPE_LEN_OFFSET + 1]);
...
/*----------------------------------------------------------------------------*/
VOID
kalClearSecurityFramesByBssIdx(
    IN P_GLUE_INFO_T prGlueInfo,
...
int hif_thread(void *data)
{
    ...
    while (TRUE) {
        ...
        if(!(prGlueInfo->u4Flag & GLUE_FLAG_HIF_PROCESS)) {
        ...
        do{
            ret = wait_event_interruptible(prGlueInfo->waitq_hif,
                ((prGlueInfo->u4Flag & GLUE_FLAG_HIF_PROCESS)!= 0));
        }while(ret != 0);   
        ...
        /* TX Commands */
		if (test_and_clear_bit(GLUE_FLAG_HIF_TX_CMD_BIT, &prGlueInfo->u4Flag)) {     
            wlanTxCmdMthread(prGlueInfo->prAdapter);
		}
       
        /* Process TX data packet to SDIO request */
		if (test_and_clear_bit(GLUE_FLAG_HIF_TX_BIT, &prGlueInfo->u4Flag)) {
            nicTxMsduQueueMthread(prGlueInfo->prAdapter);
		}

        /* Set FW own */
		if (test_and_clear_bit(GLUE_FLAG_HIF_FW_OWN_BIT, &prGlueInfo->u4Flag)) {
...
int rx_thread(void *data)
{
    ...
    while (TRUE) {
        ...
        if(!(prGlueInfo->u4Flag & GLUE_FLAG_RX_PROCESS)) {
        ...
        do{
            ret = wait_event_interruptible(prGlueInfo->waitq_rx,
                ((prGlueInfo->u4Flag & GLUE_FLAG_RX_PROCESS)!= 0));
        } while (ret != 0);
        ...
        if (test_and_clear_bit(GLUE_FLAG_RX_TO_OS_BIT, &prGlueInfo->u4Flag)) {
...
int tx_thread(void *data)
{
    ...
    while (TRUE) {
        ...
        if(!(prGlueInfo->u4Flag & GLUE_FLAG_TX_PROCESS)) {
        ...
        do{
            ret = wait_event_interruptible(prGlueInfo->waitq,
                     ((prGlueInfo->u4Flag & GLUE_FLAG_TX_PROCESS)!= 0));
        }while(ret != 0);         
...
#if CFG_SUPPORT_MULTITHREAD 
        /* Process RX */
        if (test_and_clear_bit(GLUE_FLAG_RX_BIT, &prGlueInfo->u4Flag))
        {                  
            nicRxProcessRFBs(prGlueInfo->prAdapter);
        }
		if (test_and_clear_bit(GLUE_FLAG_TX_CMD_DONE_BIT, &prGlueInfo->u4Flag))
        {
    ...
    )
{
    ...
    DBGLOG(INIT, INFO, ("%s: EN-Q CMD TYPE[%u] ID[0x%02X] SEQ[%u] to CMD Q\n", 
        __FUNCTION__,
        prCmdInfo->eCmdType, 
        prCmdInfo->ucCID,
...
#if CFG_SUPPORT_MULTITHREAD
VOID
kalSetTxEvent2Hif (P_GLUE_INFO_T pr) {
    if(!pr->hif_thread) {
        return;
    }
    ...
    set_bit(GLUE_FLAG_HIF_TX_BIT, &pr->u4Flag);
    wake_up_interruptible(&pr->waitq_hif);
}

VOID
kalSetFwOwnEvent2Hif (P_GLUE_INFO_T pr) {
    if(!pr->hif_thread) {
        return;
    }
    ...
    set_bit(GLUE_FLAG_HIF_FW_OWN_BIT, &pr->u4Flag);
    wake_up_interruptible(&pr->waitq_hif);
}

VOID
kalSetTxEvent2Rx (P_GLUE_INFO_T pr) {
    if(!pr->rx_thread) {
        return;
    }
    ...
    set_bit(GLUE_FLAG_RX_TO_OS_BIT, &pr->u4Flag);
    wake_up_interruptible(&pr->waitq_rx);
}

VOID
kalSetTxCmdEvent2Hif (P_GLUE_INFO_T pr) {
    if(!pr->hif_thread) {
        return;
    }
    ...
    set_bit(GLUE_FLAG_HIF_TX_CMD_BIT, &pr->u4Flag);
    wake_up_interruptible(&pr->waitq_hif);
}
#endif
/*----------------------------------------------------------------------------*/
...
#endif
                            i4Freq,                 /* in MHz */
                            RCPI_TO_dBm((UINT_8)HAL_RX_STATUS_GET_RCPI(prSwRfb->prRxStatusGroup3)),
                            prSwRfb->pvHeader,
                            prSwRfb->u2PacketLen,
                            GFP_ATOMIC);
    } while (FALSE);
    ...
    )
{
    ...
    if(!prDev || !(prDev->ip_ptr) || !((struct in_device *)(prDev->ip_ptr))->ifa_list){
        DBGLOG(INIT, INFO, ("IPv4 address is not avaliable for dev(0x%08x)\n", prDev));
    ...
    )
{
    ...
    if(!prDev || !(prDev->ip6_ptr)){
        DBGLOG(INIT, INFO, ("IPv6 address is not avaliable for dev(0x%08x)\n", prDev));
...
kalSetNetAddress(
    ...
    IN UINT_32 u4NumIPv4Addr,
    IN PUINT_8 pucIPv6Addr,
    IN UINT_32 u4NumIPv6Addr
    )
{
    ...
    for(i = 0; i < u4NumIPv4Addr; i++) {
        ...
        prParamNetAddr = (P_PARAM_NETWORK_ADDRESS) ((UINT_32) prParamNetAddr +
            (UINT_32)(u4AddrLen + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress)));
    }    
    ...
    )
{
    ...
    if(prNetDevPrivate->prGlueInfo != prGlueInfo) {
        DBGLOG(REQ, WARN, ("%s: unexpected prGlueInfo(0x%08x)!\n", __FUNCTION__, prNetDevPrivate->prGlueInfo));
    }
