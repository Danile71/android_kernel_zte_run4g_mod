...
int conn_md_dmp_msg (P_CONN_MD_QUEUE p_msg_list, uint32 src_id, uint32 dst_id)
{
	...
	list_for_each(pos, &p_msg_list->list)
	{
		...
		{
			counter ++;
			CONN_MD_INFO_FUNC("p_msg:0x%08x, src_id:0x%08x, dest_id:0x%08x, msg_len:%d\n", \
				p_msg, \
			...
			{
				printk("%02x ", p_msg->local_para.data[i]);
				if (7 == (i % 8))
					printk("\n");
			}
			printk("\n");
...
int _conn_md_del_msg_by_uid (uint32 u_id)
{
	...
	while (flag)
	{
		...
		list_for_each(pos, &p_msg_list->list)
		{
			...
			{
				flag = 1;
				CONN_MD_DBG_FUNC("message for uid(0x%08x) found in queue list, dest_id(0x%08x), src_id(0x%08x)\n", u_id,p_msg->ilm.dest_mod_id , p_msg->ilm.src_mod_id );
				break;
			}
		}
...
int conn_md_send_msg (ipc_ilm_t *ilm)
{
	...
	if (NULL != p_new_msg)
	{
		...
		CONN_MD_DBG_FUNC("enqueue new message to msg queue list succeed, enqueued msg counter:%d\n", p_msg_list->counter);
	...
	while (1)
	{
		...
		/*if not empty, remove all list structure to list of p_conn_md->act_queue*/
			...
			if (!list_empty(&p_act_queue->list))
			{
				...
				CONN_MD_ERR_FUNC("p_act_queue list is not empty, this should never happen!!!---*?*---!!!\n");
			}
		...
		list_for_each(pos, &p_act_queue->list){
			...
			{
				mutex_unlock(&p_user_list->lock);
				CONN_MD_WARN_FUNC("source user id (0x%08x) is not registered or not enabled yet, drop ilm msg\n", p_cur_ilm->src_mod_id);
#if (ACT_QUEUE_DBG == 1)				
				/*free message structure*/
				list_del(pos);
				kfree(p_msg);
			...
			if (NULL == p_user)
			{
				...
				CONN_MD_WARN_FUNC("target user id (0x%08x) is not registered or enabled yet, drop ilm msg\n", p_cur_ilm->dest_mod_id);
#if (ACT_QUEUE_DBG == 1)				
				/*free message structure*/
				list_del(pos);
				kfree(p_msg);
