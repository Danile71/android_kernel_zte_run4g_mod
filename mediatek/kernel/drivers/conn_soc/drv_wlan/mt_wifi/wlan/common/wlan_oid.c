    ...
    )
{
	...
	PCHAR head, p, p2;
    UINT_32 tokens;
	INT32 scanfreq, mscan, bestn, rtt;

    DBGLOG(SCN, TRACE, ("[BATCH] command=%s, len=%u\n", (PCHAR)pvSetBuffer, (UINT32)u4SetBufferLen));
	...
	if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
        ...
        if (tokens != 3) {
            DBGLOG(SCN, TRACE, ("[BATCH] Parse fail: tokens=%u, SCANFREQ=%d MSCAN=%d BESTN=%d\n", 
                (UINT32)tokens, scanfreq, mscan, bestn));
            return -EINVAL;
        }
        ...
        while ((p2 = kalStrSep((char **)&p, ",")) != NULL) {
            ...
            if (*p2 == 'A') {
                ...
                DBGLOG(SCN, TRACE, ("Scanning Channel:%u,  freq: %d\n",
                                (UINT32)prRfChannelInfo->ucChannelNum,
                                (UINT32)nicChannelNum2Freq(prRfChannelInfo->ucChannelNum)));
        ...
        DBGLOG(SCN, TRACE, ("[BATCH] SCANFREQ=%u MSCAN=%u BESTN=%u RTT=%u\n", 
            (UINT32)rCmdBatchReq.u4Scanfreq, 
            (UINT32)rCmdBatchReq.ucMScan, 
            (UINT32)rCmdBatchReq.ucBestn, 
            (UINT32)rCmdBatchReq.ucRtt));
