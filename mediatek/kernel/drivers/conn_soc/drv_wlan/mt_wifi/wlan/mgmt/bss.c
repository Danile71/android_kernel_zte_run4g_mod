...
#if 0 //CFG_AUTO_CHANNEL_SEL_SUPPORT
		if (eAci == WMM_AC_BE_INDEX)
		{
			prACQueParms[eAci].u2Aifsn = auAifs[WMM_AC_VO_INDEX];
			prACQueParms[eAci].u2CWmin = BIT(auCWminLog2[WMM_AC_VO_INDEX])-1;
			prACQueParms[eAci].u2CWmax = BIT(auCWmaxLog2[WMM_AC_VO_INDEX])-1;
		}
