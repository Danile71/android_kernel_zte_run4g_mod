				...
				if(prChnlReqInfo->eChannelReqType == CHANNEL_REQ_TYPE_GO_START_BSS&&!fgIsPureAP)
					{
						...
						DBGLOG(P2P, INFO,("NFC:GO Skip Scan and Only Froce on %s[%d]\n",prChnlReqInfo->eBand==1?"2.4G":"5G",prChnlReqInfo->ucReqChnlNum));
...
{
    if(prAdapter && prAdapter->prGlueInfo && (prAdapter->prGlueInfo->u4Flag & GLUE_FLAG_HALT))
		return;
    do {
        ASSERT_BREAK((prAdapter != NULL) &&
                                            (prP2pBssInfo != NULL) &&
    ...
    )
{
    ...
    BOOL fgIsPureAP = prAdapter->rWifiVar.prP2pFsmInfo->fgIsApMode;
    ...
    do {
        ...
        bssComposeBeaconProbeRespFrameHeaderAndFF(
            ...
            fgIsPureAP? prP2pBssInfo->u2CapInfo : prProbeRspFrame->u2CapInfo);
    ...
    if(fgEnable) {    
      ...
      /* TODO the change schedule API shall be provided by OS glue layer */
        ...
        sched_setscheduler(prAdapter->prGlueInfo->main_thread , SCHED_NORMAL, &param);
			...
			DBGLOG(P2P, WARN, ("main_thread is null, please check if the wlanRemove \
