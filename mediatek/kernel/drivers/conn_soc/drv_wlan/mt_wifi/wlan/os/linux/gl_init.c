...
//        UINT_8 aucBuf[32] = {0};
	...
	DBGLOG(INIT, INFO, ("*********wlanEarlySuspend ok************\n"));
}
    ...
    )
{
    ...
    do {
...
#if CFG_SUPPORT_CFG_FILE
        {
            ...
            u4ConfigReadLen = 0;
            printk(KERN_ERR "CFG_FILE: Read File...\n");
            if(pucConfigBuf) {
    ...
    )
{
    ...
    P_GLUE_INFO_T prGlueInfo = NULL;
    P_ADAPTER_T prAdapter = NULL;
    UINT_8 ucWaitMcastQueueCnt = 100;
    ...
    /* avoid remove & p2p off command simultaneously */
{
    ...
    /*wait at most 2s if multicast queue passed g_u4HaltFlag check,
    and before call wait_for_completion(&prGlueInfo->rPendComp)*/
    printk("prGlueInfo->u4Flag=0x%x \n", prGlueInfo->u4Flag);
    while (fgIsWorkMcStart && ucWaitMcastQueueCnt > 0
        && completion_done(&prGlueInfo->rPendComp)) {
        DBGLOG(INIT, INFO, ("wait mcast work queue, %d times \n", ucWaitMcastQueueCnt));
        ucWaitMcastQueueCnt--;
        kalMsleep(10);
    }
    if (fgIsWorkMcStart == TRUE) {
        if ( !ucWaitMcastQueueCnt)
            DBGLOG(INIT, ERROR, ("wait too long for kworker!!! \n"));
