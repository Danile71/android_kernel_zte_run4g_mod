...
static int md_ccif_probe(struct platform_device *dev)
{
	...
	md_hw_info_t *md_hw = (md_hw_info_t *)kzalloc(sizeof(md_hw_info_t), GFP_KERNEL);
	if(md_hw == NULL) {
...
#ifdef CONFIG_OF
    modem_ccif_driver.driver.of_match_table = get_ccci_of_match_table("CCIF");
#endif
