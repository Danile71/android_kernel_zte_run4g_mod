...
#define NAPI_QUEUE_MASK 0 // Rx, only Rx-exclusive port can enable NAPI, set it to 0 if disabled NAPI
...
#if 0
static void dump_emi_latency(void)
{
...
static void cldma_dump_all_gpd(struct ccci_modem *md)
{
	...
	for(i=0; i<QUEUE_LEN(md_ctrl->txq); i++) {
		...
		cldma_dump_gpd_ring(md->index+1, req->gpd, tx_queue_buffer_number[i]);
#if 0 // UT code
	...
	for(i=0; i<QUEUE_LEN(md_ctrl->rxq); i++) {
		...
		cldma_dump_gpd_ring(md->index,req->gpd, rx_queue_buffer_number[i]);
#if 0 // UT code
		// use request's link head to traverse
...
static void cldma_dump_register(struct ccci_modem *md)
{
    struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
	md_cd_lock_cldma_clock_src(1);
	printk("[CCCI%d-DUMP]dump AP CLDMA Tx register, active=%x\n",md->index, md_ctrl->txq_active);
	ccci_mem_dump(md->index, md_ctrl->cldma_ap_base+CLDMA_AP_UL_START_ADDR_0, CLDMA_AP_UL_CHECKSUM_CHANNEL_ENABLE-CLDMA_AP_UL_START_ADDR_0+4);
	printk("[CCCI%d-DUMP]dump AP CLDMA Rx register, active=%x\n",md->index, md_ctrl->rxq_active);
	ccci_mem_dump(md->index, md_ctrl->cldma_ap_base+CLDMA_AP_SO_ERROR, CLDMA_AP_DEBUG_ID_EN-CLDMA_AP_SO_ERROR+4);
	printk("[CCCI%d-DUMP]dump AP CLDMA MISC register\n",md->index);
	ccci_mem_dump(md->index, md_ctrl->cldma_ap_base+CLDMA_AP_L2TISAR0, CLDMA_AP_CHNL_IDLE-CLDMA_AP_L2TISAR0+4);
	printk("[CCCI%d-DUMP]dump MD CLDMA Tx register\n");
	ccci_mem_dump(md->index, md_ctrl->cldma_md_base+CLDMA_AP_UL_START_ADDR_0, CLDMA_AP_UL_CHECKSUM_CHANNEL_ENABLE-CLDMA_AP_UL_START_ADDR_0+4);
	printk("[CCCI%d-DUMP]dump MD CLDMA Rx register\n",md->index);
	ccci_mem_dump(md->index, md_ctrl->cldma_md_base+CLDMA_AP_SO_ERROR, CLDMA_AP_DEBUG_ID_EN-CLDMA_AP_SO_ERROR+4);
	printk("[CCCI%d-DUMP]dump MD CLDMA MISC register\n",md->index);
	ccci_mem_dump(md->index, md_ctrl->cldma_md_base+CLDMA_AP_L2TISAR0, CLDMA_AP_CHNL_IDLE-CLDMA_AP_L2TISAR0+4);
...
static void cldma_dump_packet_history(struct ccci_modem *md)
{
    ...
    struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
	for(i=0; i<QUEUE_LEN(md_ctrl->txq); i++) {
		printk("[CCCI%d-DUMP]dump txq%d packet history, ptr=%d, tr_done=%p, tx_xmit=%p\n",md->index, i,
	...
	for(i=0; i<QUEUE_LEN(md_ctrl->rxq); i++) {
		printk("[CCCI%d-DUMP]dump rxq%d packet history, ptr=%d, tr_done=%p\n",md->index, i,
...
static int cldma_rx_collect(struct md_cd_queue *queue, int budget, int blocking, int *result)
{
	...
	*result = UNDER_BUDGET;
	...
	while(1) { // not hardware own
		...
		// update skb
		dma_unmap_single(NULL, rgpd->data_buff_bd_ptr, skb_data_size(req->skb), DMA_FROM_DEVICE);
...
#endif
			if(likely(req->skb)) {
				rgpd->data_buff_bd_ptr = dma_map_single(NULL, req->skb->data, skb_data_size(req->skb), DMA_FROM_DEVICE);
				// checksum of GPD
...
static void cldma_rx_queue_init(struct md_cd_queue *queue)
{
	...
	for(i=0; i<rx_queue_buffer_number[queue->index]; i++) {
		req = ccci_alloc_req(IN, rx_queue_buffer_size[queue->index], blocking, 0);
		...
		gpd->data_buff_bd_ptr = dma_map_single(NULL, req->skb->data, skb_data_size(req->skb), DMA_FROM_DEVICE);
...
static int md_cd_stop_queue(struct ccci_modem *md, unsigned char qno, DIRECTION dir)
{
	...
	if(dir==IN) {
		...
		md_cd_lock_cldma_clock_src(1);
		spin_lock_irqsave(&md_ctrl->cldma_timeout_lock, flags);
		cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_L2RIMSR0, CLDMA_BM_ALL_QUEUE&(1<<qno));
		count = 0;
		md_ctrl->rxq_active &= (~(CLDMA_BM_ALL_QUEUE&(1<<qno)));
		do {
			cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_STOP_CMD, CLDMA_BM_ALL_QUEUE&(1<<qno));
			cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_STOP_CMD); // dummy read
			ret = cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_STATUS) & (1<<qno);
...
static int md_cd_start_queue(struct ccci_modem *md, unsigned char qno, DIRECTION dir)
{
	...
	if(dir==IN) {
		...
		if(md->md_state!=RESET && md->md_state!=GATED && md->md_state!=INVALID) {
			cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_RQSAR(md_ctrl->rxq[qno].index), md_ctrl->rxq[qno].tr_done->gpd_addr);
			cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_START_CMD, CLDMA_BM_ALL_QUEUE&(1<<qno));
...
static void md_cd_wdt_work(struct work_struct *work)
{
	...
	if(*((int *)(md->mem_layout.smem_region_vir+CCCI_SMEM_OFFSET_EPON)) == 0xBAEBAE10) { //hardcode
		...
		md_cd_lock_cldma_clock_src(1);
		plat_misc_api((void*)md->index, resv_api_1, NULL);
		md_cd_lock_cldma_clock_src(0);
...
static inline int cldma_sw_init(struct ccci_modem *md)
{
	...
	io_remap_md_side_register(md->index, md_ctrl);
...
static int md_cd_init(struct ccci_modem *md)
{
	...
	for(i=0; i<md->port_number; i++) {
		...
		port->ops->init(port);
		if((port->flags&PORT_F_RX_EXCLUSIVE) && ((1<<port->rxq_index)&NAPI_QUEUE_MASK) && port->rxq_index!=0xFF) {
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	retry = CLDMA_CG_POLL;
	while(retry-->0) {
		plat_misc_api(md_ctrl, resv_api_0, &resv_api_ret);
		if(resv_api_ret) {
...
static int md_cd_stop(struct ccci_modem *md, unsigned int timeout)
{
	...
	plat_misc_api((void*)md->index, resv_api_2, NULL); // Check EMI before
...
#if defined (CONFIG_MTK_AEE_FEATURE)
				aee_kernel_dal_show("After AP send EPOF, MD didn't go to sleep in 4s\n");
#endif
	...
	cldma_write32(md_ctrl->ap_ccif_base+0x1000, APCCIF_ACK, cldma_read32(md_ctrl->ap_ccif_base+0x1000, APCCIF_RCHNUM));

	plat_misc_api((void*)md->index, resv_api_3, NULL); // Check EMI after
	return 0;
}
...
static void dump_runtime_data(struct ccci_modem *md, struct modem_runtime *runtime)
{
	...
	*p = runtime->misc_prefix;
	...
	CCCI_INF_MSG(md->index, TAG, "Feature5                    0x%x\n", runtime->feature_5_val[0]);
...
static int md_cd_dump_info(struct ccci_modem *md, MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	if(flag & DUMP_FLAG_CCIF) {
		...
		if(length<sizeof(ccif_sram) && length>0) {
			CCCI_ERR_MSG(md->index, TAG, "dump CCIF SRAM length illegal %d/%d\n", length, sizeof(ccif_sram));
			dest_buff = (unsigned int*)ccif_sram;
		} else {
			length = sizeof(ccif_sram);
		}
...
static ssize_t md_cd_dump_store(struct ccci_modem *md, const char *buf, size_t count)
{
	...
	if(strncmp(buf, "ccif", count-1) == 0) {
		CCCI_INF_MSG(md->index, TAG, "AP_CON(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_CON), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_CON));
		CCCI_INF_MSG(md->index, TAG, "AP_BUSY(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_BUSY), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_BUSY));
		CCCI_INF_MSG(md->index, TAG, "AP_START(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_START), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_START));
		CCCI_INF_MSG(md->index, TAG, "AP_TCHNUM(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_TCHNUM), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_TCHNUM));
		CCCI_INF_MSG(md->index, TAG, "AP_RCHNUM(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_RCHNUM), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_RCHNUM));
		CCCI_INF_MSG(md->index, TAG, "AP_ACK(%x)=%x\n", (unsigned int)(md_ctrl->ap_ccif_base+APCCIF_ACK), cldma_read32(md_ctrl->ap_ccif_base, APCCIF_ACK));

		CCCI_INF_MSG(md->index, TAG, "MD_CON(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_CON), cldma_read32(md_ctrl->md_ccif_base, APCCIF_CON));
		CCCI_INF_MSG(md->index, TAG, "MD_BUSY(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_BUSY), cldma_read32(md_ctrl->md_ccif_base, APCCIF_BUSY));
		CCCI_INF_MSG(md->index, TAG, "MD_START(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_START), cldma_read32(md_ctrl->md_ccif_base, APCCIF_START));
		CCCI_INF_MSG(md->index, TAG, "MD_TCHNUM(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_TCHNUM), cldma_read32(md_ctrl->md_ccif_base, APCCIF_TCHNUM));
		CCCI_INF_MSG(md->index, TAG, "MD_RCHNUM(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_RCHNUM), cldma_read32(md_ctrl->md_ccif_base, APCCIF_RCHNUM));
		CCCI_INF_MSG(md->index, TAG, "MD_ACK(%x)=%x\n", (unsigned int)(md_ctrl->md_ccif_base+APCCIF_ACK), cldma_read32(md_ctrl->md_ccif_base, APCCIF_ACK));
...
void md_cd_traffic_monitor_func(unsigned long data)
{
	...
	for(i=0; i<md->port_number; i++) {
		...
		if(port->flags & PORT_F_RX_FULLED)
			port_full |= (1<<i);
		if(port->tx_busy_count != 0) {
			CCCI_INF_MSG(md->index, TAG, "port %s busy count %d\n", port->name, port->tx_busy_count);
	...
	CCCI_INF_MSG(md->index, TAG, "traffic(ch): [0]%ld, [1]%ld, [2]%ld, [3]%ld, [4]%ld, [5]%ld\n", 
		md_ctrl->logic_ch_pkt_cnt[0], md_ctrl->logic_ch_pkt_cnt[1],
		md_ctrl->logic_ch_pkt_cnt[2], md_ctrl->logic_ch_pkt_cnt[3],
		md_ctrl->logic_ch_pkt_cnt[4], md_ctrl->logic_ch_pkt_cnt[5]);
}
#endif
...
static int ccci_modem_probe(struct platform_device *dev)
{
	...
	CCCI_INF_MSG(-1, TAG, "modem CLDMA module probe\n");

	// Allocate modem hardware info structure memory
	md_hw = kzalloc(sizeof(md_hw_info_t), GFP_KERNEL);
	if(md_hw == NULL) {
		CCCI_INF_MSG(-1, TAG, "md_cldma_probe:alloc md hw mem fail\n");
		return -1;
	}

	ret = get_modem_hw_info(dev, &dev_cfg, md_hw);
	if(ret != 0) {
	...
	md->ops = &md_cd_ops;
	CCCI_INF_MSG(md_id, TAG, "md_cldma_probe:md_cldma=%p,md_cldma=%p\n",md,md->private_data);
...
#ifdef CONFIG_OF
	modem_cldma_driver.driver.of_match_table = get_ccci_of_match_table("MDCLDMA");
#endif
