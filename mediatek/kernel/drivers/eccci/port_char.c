...
static ssize_t dev_char_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	CCCI_DBG_MSG(port->modem->index, CHAR, "read on %s for %d\n", port->name, count);
	spin_lock_irqsave(&port->rx_req_lock, flags);
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(port->flags & PORT_F_USER_HEADER) {
		if(count > (CCCI_MTU+header_len)) {
			CCCI_ERR_MSG(port->modem->index, CHAR, "reject packet(size=%d ), larger than MTU on %s\n", count, port->name);
			return -ENOMEM;
		}
	}
	...
	CCCI_DBG_MSG(port->modem->index, CHAR, "write on %s for %d of %d\n", port->name, actual_count, count);
	...
	if(req) {
		...
		CCCI_DBG_MSG(port->modem->index, CHAR, "write done on %s, l=%d r=%d\n", port->name, actual_count, ret);
		if(ret) {
...
static long dev_char_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_GET_MD_STATE:
		...
		if(md_status_show_count[md->index] == 0) {
			CCCI_INF_MSG(md->index, CHAR, "MD state %d\n", state);
			md_status_show_count[md->index]++;
		}
    ...
    case CCCI_IOC_GET_MD_SBP_CFG:
        if (!md->sbp_code_default) {
            ...
            if(md->index==MD_SYS1){
            #if defined(MTK_MD_SBP_CUSTOM_VALUE)
            sbp_custom_value=MTK_MD_SBP_CUSTOM_VALUE;
            #else
            sbp_custom_value="";
            #endif
            }else if(md->index==MD_SYS2){
            #if defined(MTK_MD2_SBP_CUSTOM_VALUE)
            sbp_custom_value=MTK_MD2_SBP_CUSTOM_VALUE;
            #else
            sbp_custom_value="";
            #endif
            }                
            ...
            if (!ret){
               ...
               CCCI_INF_MSG(md->index, CHAR, "CCCI_IOC_GET_MD_SBP_CFG: get config sbp code fail! ret:%d, Config val:%s\n"
...
static int port_char_recv_req(struct ccci_port *port, struct ccci_request *req)
{
	...
	if(!atomic_read(&port->usage_cnt) && (port->rx_ch!=CCCI_UART2_RX && port->rx_ch!=CCCI_FS_RX)) // to make sure muxd always can get +EIND128 message
		goto drop;
