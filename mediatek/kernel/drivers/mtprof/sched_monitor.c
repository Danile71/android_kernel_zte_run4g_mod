...
static void event_duration_check(struct sched_block_event *b)
{
    ...
    switch(b->type){
        ...
        case evt_SOFTIRQ:
            ...
            //Check Preempt Count
            if(b->preempt_count != preempt_count()){
                printk("[SOFTIRQ WARN] SoftIRQ:%d, Preempt Count Unbalanced: Prev:0x%08x, End:0x%08x)\n", (int)b->last_event, b->preempt_count, preempt_count());
                /* to aware something wrong*/
                aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DUMMY_DUMP | DB_OPT_FTRACE, "Preempt Count Unbalanced","Softirq WARN\n");
            }
            break;
        case evt_TASKLET:
            ...
            //Check Preempt Count
            if(b->preempt_count != preempt_count()){
                printk("[TASKLET WARN] Tasklet:%pS, Preempt Count Unbalanced: Prev:0x%08x, End:0x%08x\n", (void *)b->last_event, b->preempt_count, preempt_count());
                /* to aware something wrong*/
                aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DUMMY_DUMP | DB_OPT_FTRACE, "Preempt Count Unbalanced","Tasklet WARN\n");
            }
            break;
        case evt_HRTIMER:
        ...
        case evt_STIMER:
            ...
            if(b->preempt_count != preempt_count()){
                printk("[STIMER WARN] SoftTIMER:%pS, Preempt Count Unbalanced: Prev:0x%08x, End:0x%08x\n", (void *)b->last_event, b->preempt_count, preempt_count());
            }
            break;
...
#endif
    spin_unlock_irqrestore(&mt_irq_count_lock, flags);
    for(eint_num =0 ;eint_num < eint_drv_get_max_channel(); eint_num++){
        mt_eint_count[eint_num] = eint_drv_get_count(eint_num);
    }
}
#else
...
static int mt_sched_monitor_write(struct file *filp, const char *ubuf,
