...
static void battery_update(struct battery_data *bat_data)
{
	...
	battery_xlog_printk(BAT_LOG_CRTI, "UI_SOC=(%d)\n", BMT_status.UI_SOC);	
#else
	if( (BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR) ) {     
		if ( BMT_status.bat_exist ) {  /* charging */
...
static void mt_battery_average_method_init(kal_uint32 *bufferdata, kal_uint32 data, kal_int32 *sum)
{
	...
	if(BMT_status.charger_exist == KAL_TRUE)
	{
		if(previous_charger_exist == KAL_FALSE)
		{
			...
			#else
			if (BMT_status.charger_type == STANDARD_CHARGER || DISO_data.diso_state.cur_vdc_state == DISO_ONLINE) {
			#endif
...
{
	if (cmd_discharging == 1) {
		battery_xlog_printk(BAT_LOG_CRTI, "[mt_battery_CheckBatteryStatus] cmd_discharging=(%d)\n", cmd_discharging);
		BMT_status.bat_charging_state = CHR_ERROR;
...
CHARGER_TYPE mt_charger_type_detection(void)
{
    ...
    #else
    if(BMT_status.charger_type == CHARGER_UNKNOWN && DISO_data.diso_state.cur_vusb_state == DISO_ONLINE)
    #endif
    {
        battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE,&CHR_Type_num);
        BMT_status.charger_type = CHR_Type_num;	

        #if defined(MTK_KERNEL_POWER_OFF_CHARGING)&&(defined(MTK_PUMP_EXPRESS_SUPPORT) || defined(MTK_PUMP_EXPRESS_PLUS_SUPPORT))
 	    if (BMT_status.UI_SOC == 100)
		{
...
#if defined(MTK_DUAL_INPUT_CHARGER_SUPPORT)
            battery_xlog_printk(BAT_LOG_CRTI, "turn off charging for no avaliable charging source\n");
...
#ifdef X2_CHARGING_STANDARD_SUPPORT
   if(lenovo_battery_charging_thread(&g_BatteryNotifyCode) == KAL_FALSE){
	  mt_battery_update_status();
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case ADC_CHANNEL_READ:            
            ...
            {
            #if defined(SOC_BY_3RD_FG)
			    adc_out_data[0] =  0 - battery_meter_get_battery_current();
            /*End,Lenovo-sw chailu1 modify 2014-4-23, support TI's FG fuction*/  
	    #else
...
{
	int pre_diso_state = (DISO_data.diso_state.pre_otg_state |(DISO_data.diso_state.pre_vusb_state << 1) \
						 |(DISO_data.diso_state.pre_vdc_state << 2)) & 0x7;
	battery_xlog_printk(BAT_LOG_CRTI, "[DISO]auxadc IRQ threaded handler triggered, pre_diso_state is %s\n",DISO_state_s[pre_diso_state]);
	switch(pre_diso_state)
	{
		...
		case DC_WITH_USB:
			...
			if((BMT_status.charger_type==STANDARD_HOST) || (BMT_status.charger_type==CHARGING_HOST) )//disconnect if connected
			{
				mt_usb_disconnect();          
			}
		...
		case DC_ONLY:
			...
			break;
		default:
			battery_xlog_printk(BAT_LOG_CRTI, "[DISO]VUSB auxadc threaded handler triggered ERROR OR TEST\n");
			break;
	}
	return IRQ_HANDLED;
}
