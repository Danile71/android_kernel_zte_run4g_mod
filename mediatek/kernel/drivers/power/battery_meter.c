...
extern int max77819_fg_get_vell();
...
//modify by willcai 2015-5-29 for charing begin 
#ifdef X2_CHARGING_STANDARD_SUPPORT
	if(Res <= 0)	
	{	
		TBatt_Value = -20;	
...
int force_get_tbat(kal_bool update)
{
    ...
    if (update == KAL_TRUE || pre_bat_temperature_val == -1) {
	    ...
	    bm_print(BM_LOG_CRTI, "[force_get_tbat] %d,%d,%d,%d,%d,%d %d\n", 
	        bat_temperature_volt_temp, bat_temperature_volt, fg_current_state, fg_current_temp, fg_r_value, bat_temperature_val, update);
...
void fgauge_construct_table_by_temp(void)
{
    ...
    curr_temp = force_get_tbat(KAL_FALSE);
...
//modify end
	...
	bm_print(BM_LOG_CRTI, "[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
		gFG_voltage, (100 - fgauge_read_capacity_by_v(gFG_voltage)), g_rtc_fg_soc, gFG_DOD0 ,oam_v_ocv_init, force_get_tbat(KAL_FALSE));
...
void oam_run(void)
{
	...
	bm_print(BM_LOG_CRTI, "[oam_run_time] last time=%d, now time=%d, delta time=%d\n", 
	...
	bm_print(BM_LOG_CRTI, "[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
	 ...
	 bm_print(BM_LOG_CRTI, "[oam_run_inf] %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", 
           oam_v_ocv_1,oam_v_ocv_2,vol_bat,oam_i_1,oam_i_2,oam_r_1,oam_r_2,oam_car_1,oam_car_2,gFG_BATT_CAPACITY_aging,force_get_tbat(KAL_FALSE), oam_d0);  
...
void fgauge_algo_run_init(void)
{
	...
	kal_bool charging_enable = KAL_FALSE;
#if defined (MTK_KERNEL_POWER_OFF_CHARGING) && !defined(SWCHR_POWER_PATH)
...
// ============================================================ //
#ifdef MTK_MAX77819_SUPPORT
int get_battery_voltage_by_max77819(void)                 
{                                                         
int result=0;                                             
int times=0,i=3;                                          
int temp=0;
...
{                                                         
temp=max77819_fg_get_vell();
                              
while((temp<2000||temp>4400)&&((i--)>0))                  
{                                                         
 mdelay(100);                                            
 temp=max77819_fg_get_vell();                             
}
...
//modify by willcai 2014-5-29 begin
    ...
    return val;
/*end willcai  modify 2014-5-29, support TI's FG fuction*/
...
kal_int32 battery_meter_get_battery_temperature(void)
{
    ...
    return batt_temp;
#else
	bm_print(BM_LOG_CRTI, "force_get_tbat\n");
...
static void battery_aging_check(void)
{
...
#if defined (MD_SLEEP_CURRENT_CHECK)
	bm_print(BM_LOG_CRTI, "sleeptime=(%d)s, car_be = %d, car_af = %d, xts = %d, tom = %d\n", suspend_time, columb_before_sleep, columb_after_sleep, xts.tv_sec, tom.tv_sec);
	...
	if(hw_ocv_after_sleep < vbat) {
		...
		bm_print(BM_LOG_CRTI, "be_ocv=(%d), af_ocv=(%d), D0=(%d), car=(%d)\n",
