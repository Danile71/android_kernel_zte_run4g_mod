...
public class ConnectivityServiceExt extends DefaultConnectivityServiceExt
{
    ...
    private boolean isPsDataAvailable() {
        ...
        try {
            ...
            if (tme == null) {
                Xlog.e(MTK_TAG, "TelephonyManagerEx.getDefault() = null");
                return false;
            }

            boolean isSIM1Ready = (tme.getSimState(PhoneConstants.GEMINI_SIM_1) == TelephonyManager.SIM_STATE_READY);
            boolean isSIM2Ready = false;
            if(FeatureOption.MTK_GEMINI_SUPPORT){
               isSIM2Ready = (tme.getSimState(PhoneConstants.GEMINI_SIM_2) == TelephonyManager.SIM_STATE_READY);
            }

            if(!isSIM1Ready && !isSIM2Ready) {
               Xlog.v(MTK_TAG, "isSIM1Ready:" + isSIM1Ready + "; isSIM1Ready:" + isSIM1Ready);
               return false;
            }
