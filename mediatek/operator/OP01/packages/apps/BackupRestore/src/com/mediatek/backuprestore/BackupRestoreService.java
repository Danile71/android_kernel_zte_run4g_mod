...
/*        //set mode backup or restore
        public void setBackupRestoreMode(String commandMode) {
            mCommandMode = commandMode;
        }*/

        public void updateMaxPercent(ArrayList<?> list) {
            BackupRestoreNotification.getInstance(BackupRestoreService.this).setMaxPercent(list.size());
        }
        ...
        public void cancelBackupRestore() {
            ...
            stopForeground(true);
            mCurrentComposerCount = 0;
            MyLogger.logD(CLASS_TAG, "cancelBackupRestore and stopForeground and mCurrentComposerCount = 0");
        }
    ...
    public void onStart(Composer composer) {
        ...
        bundle.putString(Constants.MESSAGE_CONTENT, ModuleType.getModuleStringFromType(this, composer.getModuleType()));
        msg.setData(bundle);
        MyLogger.logD(CLASS_TAG, "onStart mHandler.sendMessage(msg) mHandler != null"+ composer.getModuleType());
        if (mHandler != null) {
        ...
        if (mCurrentProgress.mMax != 0 && mCommandMode.equals(Constants.BACKUP)) {
            BackupRestoreNotification.getInstance(BackupRestoreService.this).initBackupNotification(composer.getModuleType(),
                    mCurrentComposerCount);
            startForeground(Constants.STARTFORGROUND, BackupRestoreNotification.getInstance(BackupRestoreService.this)
                    .getNotification());
        } else if (mCurrentProgress.mMax != 0 && mCommandMode.equals(Constants.RESTORE)) {
            BackupRestoreNotification.getInstance(BackupRestoreService.this).initRestoreNotification(composer.getModuleType(), mCurrentComposerCount, mOldData);
            startForeground(Constants.STARTFORGROUND, BackupRestoreNotification.getInstance(BackupRestoreService.this)
                    .getNotification());
        }
    ...
    public void onOneFinished(Composer composer, boolean result) {
        ...
        if (composer.getModuleType() == ModuleType.TYPE_APP && mBackupRestoreFolder != null) {
            ...
            MyLogger.logD(CLASS_TAG, "onOneFinished entity = "+entity +"; mParasMap = "+mParasMap +"; mParasMap.get(ModuleType.TYPE_APP) = "+ mParasMap.get(ModuleType.TYPE_APP));
            if(mParasMap != null && mParasMap.get(ModuleType.TYPE_APP) != null && mParasMap.get(ModuleType.TYPE_APP).get(mCurrentProgress.mCurNum - 1) != null) {
                entity.setKey(mParasMap.get(ModuleType.TYPE_APP).get(mCurrentProgress.mCurNum - 1));
                mAppResultList.add(entity);
            }
            BackupRestoreNotification.getInstance(BackupRestoreService.this).updateNotification(composer.getModuleType(),
                    mCurrentProgress.mCurNum);
            startForeground(Constants.STARTFORGROUND, BackupRestoreNotification.getInstance(BackupRestoreService.this)
                    .getNotification());
        }
    ...
    public void onEnd(Composer composer, boolean result) {
        ...
        if(composer.getModuleType() != ModuleType.TYPE_APP && BackupRestoreNotification.getInstance(BackupRestoreService.this) != null){
            BackupRestoreNotification.getInstance(BackupRestoreService.this).updateNotification(composer.getModuleType(),mCurrentComposerCount);
            if(BackupRestoreNotification.getInstance(BackupRestoreService.this).getNotification() != null) {
                startForeground(Constants.STARTFORGROUND, BackupRestoreNotification.getInstance(BackupRestoreService.this).getNotification());
            }
        }
