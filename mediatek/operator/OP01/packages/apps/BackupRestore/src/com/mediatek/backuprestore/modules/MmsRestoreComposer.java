        ...
        if ((mRecordList != null) && (mIndex < mRecordList.size())) {
           ...
           MyLogger.logD(CLASS_TAG, "mIdx:" + (mIndex) + ",mMsgUri:" + mMsgUri.toString() + ",simId:" + simId);
            ...
            if (result) {
                ...
                tmpContent.mMsgInfo.put("m_size", Integer.toString(pduByteArray.length));
                ...
                try {
                    tmpContent.mGenericPdu = new PduParser(pduByteArray).parse(true);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private class MmsRestoreThread extends Thread {
        ...
        public void run() {
            for (int j = 0; (mPduList != null) && (j < mPduList.size()); ++j) {
                ...
                GenericPdu genericPdu = content.mGenericPdu;
                
                if (genericPdu != null ) {
                    MyLogger.logD(CLASS_TAG, MyLogger.MMS_TAG + "MmsRestoreThread parse finish");
                    Uri tmpUri = null;
                    try {
                        tmpUri = persister.persist_ex(genericPdu, mMsgUri,true, msgInfo);
