...
public class MusicBackupComposer extends Composer {
    ...
    private static final Uri[] MUSICURIARRAY = {
    ...
    private static final String[] PROJECTION = new String[] { Audio.Media._ID, Audio.Media.DATA };
    ...
    public final boolean init() {
        ...
        for (int i = 0; i < mMusicCursorArray.length; ++i) {
            if (MUSICURIARRAY[i] == Audio.Media.EXTERNAL_CONTENT_URI) {
                String path = SDCardUtils.getStoragePath();
                if (path != null) {
                    String externalSDPath = "%" + path.subSequence(0, path.lastIndexOf(File.separator) + 1) + "%";
                    mMusicCursorArray[i] = mContext.getContentResolver().query(MUSICURIARRAY[i], PROJECTION,
                            Audio.Media.DATA + " not like ?", new String[] { externalSDPath }, null);
                } else {
                    mMusicCursorArray[i] = mContext.getContentResolver().query(MUSICURIARRAY[i], PROJECTION, null,
                            null, null);
                }
            } else {
                mMusicCursorArray[i] = mContext.getContentResolver().query(MUSICURIARRAY[i], PROJECTION, null, null,
                        null);
            }
    ...
    public final boolean implementComposeOneEntity() {
        ...
        for (int i = 0; i < mMusicCursorArray.length; ++i) {
            if (mMusicCursorArray[i] != null && !mMusicCursorArray[i].isAfterLast()) {
                ...
                try {
                    // String tmpName = mParentFolderPath + File.separator +
                    // ModulePath.FOLDER_MUSIC +
                    // data.subSequence(data.lastIndexOf(File.separator),
                    // data.length()).toString();
                    String tmpName = data.subSequence(data.lastIndexOf(File.separator) + 1, data.length()).toString();
                    ...
                    MyLogger.logE(CLASS_TAG, MyLogger.MUSIC_TAG + " IndexOutOfBoundsException");
                    e.printStackTrace();
                }
    ...
    public void onStart() {
        ...
        if (getCount() > 0) {
            ...
            try {
                mZipFileHandler = new BackupZip(path + File.separator + ModulePath.MUSICZIP);
            } catch (IOException e) {
                if (super.mReporter != null) {
                    super.mReporter.onErr(e);
                }
    ...
    /*
     * private void deleteFolder(File file) { if (file.exists()) { if
     * (file.isFile()) { int count =
     * mContext.getContentResolver().delete(Audio.Media.EXTERNAL_CONTENT_URI,
     * Audio.Media.DATA + " like ?", new String[] { file.getAbsolutePath() });
     * MyLogger.logD(CLASS_TAG, "deleteFolder():" + count + ":" +
     * file.getAbsolutePath()); file.delete(); } else if (file.isDirectory()) {
     ...
     * { try { File f1 = new File(srcFile); if (f1.exists() && f1.isFile()) {
     * InputStream inStream = new FileInputStream(srcFile); FileOutputStream
     * outStream = new FileOutputStream(destFile); byte[] buf = new byte[1024];
     * int byteRead = 0; while ((byteRead = inStream.read(buf)) != -1) {
     * outStream.write(buf, 0, byteRead); } outStream.flush();
     * outStream.close(); inStream.close(); } } catch (IOException e) { throw e;
