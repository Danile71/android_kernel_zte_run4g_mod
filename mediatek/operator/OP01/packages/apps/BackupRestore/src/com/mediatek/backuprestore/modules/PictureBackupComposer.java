...
public class PictureBackupComposer extends Composer {
    ...
    private static final Uri[] PICTUREURIARRAY = { 
    ...
    public boolean init() {
        ...
        for (int i = 0; i < mPictureCursorArray.length; ++i) {
            if (PICTUREURIARRAY[i] == Images.Media.EXTERNAL_CONTENT_URI) {
                String path = SDCardUtils.getStoragePath();
                if (path != null) {
                    String externalSDPath = "%" + path.subSequence(0, path.lastIndexOf(File.separator) + 1) + "%";
                    mPictureCursorArray[i] = mContext.getContentResolver().query(PICTUREURIARRAY[i], PROJECTION,
                            MediaStore.Images.Media.DATA + " not like ?", new String[] { externalSDPath }, null);
                } else {
                    mPictureCursorArray[i] = mContext.getContentResolver().query(PICTUREURIARRAY[i], PROJECTION, null,
                            null, null);
                }
            } else {
                mPictureCursorArray[i] = mContext.getContentResolver().query(PICTUREURIARRAY[i], PROJECTION, null,
    ...
    public boolean implementComposeOneEntity() {
        ...
        for (int i = 0; i < mPictureCursorArray.length; ++i) {
            if (mPictureCursorArray[i] != null && !mPictureCursorArray[i].isAfterLast()) {
                int dataColumn = mPictureCursorArray[i].getColumnIndexOrThrow(MediaStore.Images.Media.DATA);
    ...
    public final void onStart() {
        ...
        if (getCount() > 0) {
            ...
            try {
                mZipFileHandler = new BackupZip(path + File.separator + ModulePath.PICTUREZIP);
            } catch (IOException e) {
                if (super.mReporter != null) {
                    super.mReporter.onErr(e);
                }
     ...
     * mContext.getContentResolver().delete(MediaStore
     * .Images.Media.EXTERNAL_CONTENT_URI, MediaStore.Images.Media.DATA +
     * " like ?", new String[] { file.getAbsolutePath() });
