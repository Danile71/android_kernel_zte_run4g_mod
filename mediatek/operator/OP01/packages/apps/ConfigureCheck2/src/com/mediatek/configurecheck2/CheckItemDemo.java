...
class CheckAtciInEM extends CheckItemBase {
    ...
    public boolean onCheck() {
        ...
        if (SystemProperties.get(ATCI_USERMODE).equals("1")
                ...
                && 1 == Settings.Global.getInt(getContext().getContentResolver(), Settings.Global.ACM_ENABLED, 0) 
...
class Check2gRoaming extends CheckItemBase {
    private static final String TAG = "Check2gRoaming";
    ...
    Check2gRoaming(Context c, String key) {
        super(c, key);
        setProperty(PROPERTY_AUTO_CHECK | PROPERTY_AUTO_CONFG);
        setTitle(R.string.roaming_2g_title);
        if (key.equals(CheckItemKeySet.CI_2G_ROAMING_DISABLE)) {
            setNote(R.string.roaming_2g_disable_note);
        } else {
            throw new IllegalArgumentException("Error key = " + key);
        }
    }
    ...
    public boolean onCheck() {
        ...
        CTSCLog.d(TAG, "Get " + KEY_MISC_CONFIG + " = " + mConfig);
        
        if ((mConfig & 1) != 0) {
            setValue(R.string.ctsc_enabled);
            if (getKey().equals(CheckItemKeySet.CI_2G_ROAMING_DISABLE)) {
                mResult = check_result.WRONG;
            }
        } else {
            setValue(R.string.ctsc_disabled);
