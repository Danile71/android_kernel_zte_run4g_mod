...
public class ServerTimeout extends DefaultServerTimeoutExtension {
    ...
    private boolean passDisconnectCheck() {
        ...
        if (MovieUtils.isRtspOrSdp(getPlayer().getVideoType())) {
            ...
            if (LOG) {
                Log.v(TAG, "passDisconnectCheck() now=" + now + ", mLastDisconnectTime=" + mLastDisconnectTime
    ...
    private void notifyServerTimeout() {
        if (mServerTimeoutDialog == null) {
            ...
            mServerTimeoutDialog = builder.setTitle(mPluginContext.getString(R.string.server_timeout_title))
                ...
                .setPositiveButton(mPluginContext.getString(R.string.resume_playing_resume), new OnClickListener() {
    
                    public void onClick(DialogInterface dialog, int which) {
                        if (LOG) {
                            Log.v(TAG, "PositiveButton.onClick() mIsShowDialog=" + mIsShowDialog);
                        }
                        getPlayer().startVideo(true, getPlayer().getVideoPosition(), getPlayer().getVideoLastDuration());
