...
public class WifiExt extends DefaultWifiExt implements AdapterView.OnItemSelectedListener {
    ...
    public View getPriorityView(/*View view, int priorityId, int setterId*/) {
        ...
        if (priorityType == System.WIFI_PRIORITY_TYPE_DEFAULT) {
            ...
            if (mPrioritySpinner != null) {
                ...
                Xlog.d(TAG, "getPriorityView(), priorityOrder=" + priorityOrder + ", mPriority=" + mPriority);
                Xlog.d(TAG, " " + priorityCount);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        if (parent.equals(mPrioritySpinner)) {
            ...
            Xlog.d(TAG, "onItemSelected(), " + mPriority);
        }
    }
    @Override
    ...
    public int getPriority() {
        Xlog.d(TAG, "getPriority" + mPriority);
        return mPriority;
    }
    ...
    public void initConnectView(Activity activity, PreferenceScreen screen) {
        ...
        mConnectReminderPref.setDialogTitle(mContext.getString(R.string.wifi_reminder_frequency_title));
        mConnectReminderPref.setEntries(mContext.getResources().getTextArray(R.array.wifi_reminder_entries));
        mConnectReminderPref.setEntryValues(mContext.getResources().getTextArray(R.array.wifi_reminder_values));
    ...
    private Preference.OnPreferenceClickListener mPreferenceclickListener = new Preference.OnPreferenceClickListener() {
        ...
        public boolean onPreferenceClick(Preference preference) {
            ...
            try {
                ...
                Toast.makeText(mContext, R.string.wifi_priority_missing_app, Toast.LENGTH_SHORT).show();
            }
            return true;
        }
    };
    ...
    public void initPreference(ContentResolver contentResolver) {
        ...
        if (mConnectReminderPref != null) {
            ...
            CharSequence[] array = mContext.getResources().getTextArray(R.array.wifi_reminder_entries);
    ...
    public void hideWifiConfigInfo(Builder builder , Context context) {
     ...
     public int getEapMethodbySpinnerPos(int spinnerPos, String ssid, int security) {
        Xlog.d(TAG, "getEapMethodbySpinnerPos() spinnerPos = " + spinnerPos);
        if (FeatureOption.MTK_EAP_SIM_AKA) {
            Xlog.d(TAG, "getEapMethodbySpinnerPos() ssid = " + ssid);
            Xlog.d(TAG, "getEapMethodbySpinnerPos() security = " + security);
            if (ssid != null
                && ((CMCC_SSID.equals(ssid) && SECURITY_EAP == security)
                    || (CMCC_AUTO_SSID.equals(ssid) && SECURITY_EAP == security))) {
                if (spinnerPos == 1) {
    ...
    public int getPosByEapMethod(int spinnerPos, String ssid, int security) {
        Xlog.d(TAG, "getPosByEapMethod() spinnerPos = " + spinnerPos);
        if (FeatureOption.MTK_EAP_SIM_AKA) {
            Xlog.d(TAG, "getPosByEapMethod() ssid = " + ssid);
            Xlog.d(TAG, "getPosByEapMethod() security = " + security);
            if (ssid != null
                && ((CMCC_SSID.equals(ssid) && SECURITY_EAP == security)
                    || (CMCC_AUTO_SSID.equals(ssid) && SECURITY_EAP == security))) {
        ...
        Xlog.d(TAG, "getPosByEapMethod() pos = " + spinnerPos);
        if (spinnerPos < 0) {
            spinnerPos = 0;
        }
        return spinnerPos;
