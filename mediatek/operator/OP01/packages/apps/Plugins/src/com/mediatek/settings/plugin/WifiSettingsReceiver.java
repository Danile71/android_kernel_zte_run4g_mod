    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiManager.WIFI_AP_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (state == WifiManager.WIFI_AP_STATE_ENABLED) {
                ...
                int dualSimMode = Settings.System.getInt(context.getContentResolver(),
                		Settings.System.DUAL_SIM_MODE_SETTING, -1);
	            if (FeatureOption.MTK_GEMINI_SUPPORT) {
			        long dataSimId = Settings.System.getLong(context.getContentResolver(),
		                    Settings.System.GPRS_CONNECTION_SIM_SETTING,
		                    Settings.System.DEFAULT_SIM_NOT_SET);
		            if (dataSimId > 0 && dualSimMode > 0) {
                        if ((dualSimMode == ALL_RADIO_ON)
                            || (dataSimId == dualSimMode)) {
		                	isDataOn = true;
                        }
		            }
	        	} else {
		            ConnectivityManager cm = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
		            if (cm != null && dualSimMode > 0) {
		                isDataOn = cm.getMobileDataEnabled();
		            }
		        }
            ...
            {       
                ...
                if (configs != null && FeatureOption.MTK_EAP_SIM_AKA)
                {
                    for (WifiConfiguration config : configs)
                    {
                        ...
                        if (CMCC_SSID.equals(removeDoubleQuotes(config.SSID)) && (config.toString().contains("eap SIM") || config.toString().contains("eap AKA")))
                        {
                    ...
                    for (WifiConfiguration config : configs)
                    {
                        ...
                        if (CMCC_SSID.equals(removeDoubleQuotes(config.SSID)) && (config.toString().contains("eap SIM") || config.toString().contains("eap AKA")))
                        {
                            if (config.simSlot != null)
                            {
                                ...
                                Log.i(TAG, "WifiSettingsReceiver onReceive [sim plugout] CMCC_SSID configs = " + config.toString());
                                if (FeatureOption.MTK_GEMINI_SUPPORT)
                                {
