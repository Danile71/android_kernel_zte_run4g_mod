...
public class OP06BrowserRegionalPhoneExt extends DefaultBrowserRegionalPhoneExt {
    ...
    protected void finalize() throws Throwable {
        ...
        Xlog.i(TAG, "mObserver unregisterContentObserver.");
    }

    public void updateBookmarks(Context cntx) {    
        Xlog.i(TAG, "Enter: " + "updateBookmarks" + " --OP06 implement");
        String timeStampPref = readPrefValue(cntx, PREF_BKM_MCC_MNC_TIMESTAMP);        
        String timeStampDB = null;
        boolean isUpdated = false;
        appContext = cntx;
        Xlog.i(TAG, "timestamp in preference = " + timeStampPref);
        ...
        if(cr != null && cr.getCount() > 0) {
            ...
            Xlog.i(TAG, "timestamp read from DB = " + timeStampDB);
            cr.close();
            cr = null;
        } else {
            if (cr != null) {
        	...
        	Xlog.i(TAG, "Register contentobserver to listen database change. \n");
        ...
        if(timeStampPref == null || !(timeStampPref.equals(timeStampDB))) {
            Xlog.i(TAG, "Need to read bkm from DB ");
...
//            Xlog.i(TAG, "Number of rows in result " +cursor.getCount());
            if(cursor != null && cursor.getCount() > 0) {
                ...
                while (!cursor.isAfterLast()) {
                    ...
                    Xlog.i(TAG, "DATA read from DB = " + url + "," + title + "," + thumbnail + "," + is_folder + "," + parent);
        ...
        Xlog.i(TAG, "is updated = " + isUpdated);
        //return isUpdated;
    }
    
    public String getSearchEngine(SharedPreferences mPrefs, Context cntx) {

        Xlog.i(TAG, "Enter: " + "getsearchengine" + " --OP06 implement");
        ...
        if(cr != null && cr.getCount() > 0) {
            ...
            Xlog.i(TAG, "timestamp read from DB = " + timeStampDB);
            Xlog.i(TAG, "nameDB read from DB = " + nameDB);
        }
        if (cr != null) {
            cr.close();
        }
        if(timeStampPref == null || !timeStampPref.equals(timeStampDB)) {
            Xlog.i(TAG, "Need to read search engine from DB ");
            Xlog.i(TAG, "searchengine = " + nameDB);
    ...
    private int sameNameBkmExist(String bkmToAdd, int parentId) {
        ...
        Cursor cursor = mContext.getContentResolver().query(
            ...
            null);
        Xlog.d(TAG, "Query for same name " + bkmToAdd);
        if (cursor != null) {
            Xlog.d(TAG, "cursor not null, value = " + cursor);
            try {
        ...
        Xlog.d(TAG, "cursor is null, no duplicates");
        return -1;
    }
...
private class RegionalPhoneContentObserver extends ContentObserver { 
    ...
    public void onChange(boolean selfChange, Uri uri) {
        Xlog.i(TAG, "onChange(boolean selfChange, Uri uri)" + selfChange + ", " + uri.toString());
        Xlog.i(TAG, "Content observer called, inside it");
        String timeStampDB = null;
        ...
        if(cr != null && cr.getCount() > 0) {
            ...
            timeStampDB = cr.getString(cr.getColumnIndex(RegionalPhone.SEARCHENGINE.MCC_MNC_TIMESTAMP));
            nameDB = cr.getString(cr.getColumnIndex(RegionalPhone.SEARCHENGINE.SEARCH_ENGINE_NAME));
            Xlog.i(TAG, "In Contentobserver:timestamp read from DB = " + timeStampDB);
        }
        if (cr != null) {
            cr.close();
        }
        //if(timeStampBkmPref == null || !(timeStampBkmPref.equals(timeStampDB)))
         {
            Xlog.i(TAG, "In Contentobserver:Need to read bkm from DB ");
            Cursor cursor = mContext.getContentResolver().query(
                    uri,   // The content URI
                new String[]{RegionalPhone.BROWSER._ID, RegionalPhone.BROWSER.BOOKMARK_URL, RegionalPhone.BROWSER.BOOKMARK_TITLE,
                    RegionalPhone.BROWSER.THUMBNAIL, RegionalPhone.BROWSER.IS_FOLDER, RegionalPhone.BROWSER.PARENT},// The columns to return for each row
                    ...
                    null,                     // Selection criteria
                    null);
//            Xlog.i(TAG, "In Contentobserver:Number of rows in result " +cursor.getCount());
            if(cursor != null && cursor.getCount() > 0) {
                Xlog.i(TAG, "In Contentobserver:Number of rows in result " +cursor.getCount());
                cursor.moveToFirst();
                while (!cursor.isAfterLast()) {
                    ...
                    String is_folder = cursor.getString(cursor.getColumnIndex(RegionalPhone.BROWSER.IS_FOLDER));
                    String parent = cursor.getString(cursor.getColumnIndex(RegionalPhone.BROWSER.PARENT));
                    Xlog.i(TAG, "In Contentobserver:DATA read from DB = " + url + "," + title + "," + thumbnail + "," + is_folder + "," + parent);
                    updatePrefValue(PREF_BKM_MCC_MNC_TIMESTAMP, timeStampDB);
                    try {
                        ContentValues values = new ContentValues();
                        values.put(BrowserContract.Bookmarks.TITLE, title);
                        ...
                        Uri newUri = null;
                        int existId = sameNameBkmExist(title, PARENT);
                        Xlog.i(TAG, "In contentobserver:already exist result = " + existId);
                        if(existId != -1) {
                            newUri = ContentUris.withAppendedId(BrowserContract.Bookmarks.CONTENT_URI, existId);
                            Xlog.i(TAG, "In contentobserver:new URI updated after replace = " + newUri);
                            int rows = mContext.getContentResolver().update(newUri, values, null, null);
                            Xlog.i(TAG, "In contentobserver:update result = " + rows);
                        } else {
                        newUri = mContext.getContentResolver().insert(BrowserContract.Bookmarks.CONTENT_URI, values);
                        }
                        Xlog.i(TAG, "In Contentobserver:new URI updated = " + newUri);
                        if(newUri != null) {
                            //isUpdated = true;
                        }                    
                    } catch (IllegalStateException e) {
                        Xlog.e(TAG, "In Contentobserver:exception when try to update ");
                    }
                    cursor.moveToNext();
                }// end of while
                cursor.close();
        ...
        if(timeStampSearchPref == null || !timeStampSearchPref.equals(timeStampDB)) {
            Xlog.i(TAG, "In Contentobserver:Need to read search engine from DB ");
            Xlog.i(TAG, "In Contentobserver:searchengine = " + nameDB);
            updatePrefValue(PREF_SRCH_MCC_MNC_TIMESTAMP, timeStampDB);
        }
        if (nameDB != null) {
            if(!sentBroadcast) {
            ...
            intent.putExtra(SEARCH_ENGINE_PREF,nameDB);
                Xlog.i(TAG, "In Contentobserver:sending broadcast");
