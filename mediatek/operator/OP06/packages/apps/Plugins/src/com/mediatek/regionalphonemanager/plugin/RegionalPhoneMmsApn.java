...
public class RegionalPhoneMmsApn {
    ...
    public long insert(final Context context, ContentValues values) {
		...
		try {
			spn = mTelephonyService.getMvnoPattern(PhoneConstants.MVNO_TYPE_SPN, mSimId);
			imsi = mTelephonyService.getMvnoPattern(PhoneConstants.MVNO_TYPE_IMSI, mSimId);
			pnn = mTelephonyService.getMvnoPattern(PhoneConstants.MVNO_TYPE_PNN, mSimId);
			Log.d(TAG, "spn = " + spn);
			Log.d(TAG, "imsi = " + imsi);
			Log.d(TAG, "pnn = " + pnn);
