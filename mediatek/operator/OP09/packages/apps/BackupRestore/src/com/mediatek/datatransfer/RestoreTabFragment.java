...
public class RestoreTabFragment extends PreferenceFragment /*AbstractRestoreFragment*/ {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = rootView =  inflater.inflate(R.layout.restore_loading, null);
		loading = (LinearLayout) view.findViewById(R.id.restore_loading_container);
		loadingContent = (TextView) view.findViewById(R.id.restore_loading_content);
		return view;
	}
	
	public void showLoading(boolean show,int content){
		if (getActivity() != null && loading != null){
			loading.setVisibility(show?View.VISIBLE:View.GONE);
			if(show&&content>0&&loadingContent!=null){
				loadingContent.setText(content);
			}
		}else{
			Log.e(CLASS_TAG, "[RestoreTabFragment: showLoading] getActivity() = " + getActivity()+ " loading view = " + loading);
		}
	}
    ...
    public void onResume() {
        ...
        mIsActive = true;
        // refresh
        if (SDCardUtils.isSdCardAvailable(this.getActivity())) {
            startScanFiles();
        }
    }
        ...
        mHandler = new Handler() {
            ...
            public void handleMessage(final Message msg) {
                ...
                case MessageID.SCANNER_FINISH:
                	showLoading(false,R.string.activity_loading);
                    if (getActivity() != null) {
                    	MyLogger.logD(CLASS_TAG, "scan result [msg.obj] is " + msg.obj);
...
//        mLoadingDialog.show();
    	showLoading(true,R.string.activity_loading);
        if (mFileScanner == null) {
    ...
    private void addScanResultsAsPreferences(Object obj) {
        ...
        // personal data INTERNAL
        List<BackupFilePreview> items = map.get(Constants.SCAN_RESULT_KEY_PERSONAL_DATA_EXTERNAL);
        if (items != null && !items.isEmpty()) {
        	noRecord = false;
            addPreferenceCategory(ps, com.android.internal.R.string.storage_sd_card);
     ...
     // personal data EXTERNAL
        items = map.get(Constants.SCAN_RESULT_KEY_PERSONAL_DATA_INTERNAL);
        if (items != null && !items.isEmpty()) {
        	noRecord = false;
        	addPreferenceCategory(ps, com.android.internal.R.string.storage_phone);
            for (BackupFilePreview item : items) {
                addRestoreCheckBoxPreference(ps, item, "personal data");
            }
        }
...
//                + item.getFileName());
    ...
    class DeleteActionMode implements ActionMode.Callback {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            setAllItemChecked(false);
            showCheckBox(true);
            ((CustomViewPager)mListView.getParent().getParent()).setFibiddenScroll(true);
            return true;
        }
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            showCheckBox(false);
            ((CustomViewPager)mListView.getParent().getParent()).setFibiddenScroll(false);
        }
...
//            if (activity != null) {
//            	showLoading(false,R.string.delete_please_wait);
//            }
            startScanFiles();
        ...
        protected void onPreExecute() {
            Activity activity = getActivity();
//            if (activity != null) {
//            	showLoading(true,R.string.delete_please_wait);
//            }
            if (activity != null && mDeletingDialog != null) {
