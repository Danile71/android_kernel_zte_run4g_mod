...
public class CalllogRestoreComposer extends Composer {
	...
	private ArrayList<CallLogsData> calllogDatas;
	private int mIndex;
	...
	public int getCount() {
		// TODO Auto-generated method stub
		int count = 0;
		if(calllogDatas!=null){
			count = calllogDatas.size();
		}
	...
	public boolean isAfterLast() {
		...
		boolean result = true;
        if (calllogDatas != null) {
            result = (mIndex >= calllogDatas.size()) ? true : false;
        }
	...
	public boolean init() {
		...
		try {
			calllogDatas = getCallLogEntry();
			otherPhone = isOtherPhone();
			MyLogger.logD(CLASS_TAG, "[init]====otherPhone?==="+otherPhone);
		} catch (Exception e) {
	...
	private boolean isOtherPhone() {
		...
		MyLogger.logD(CLASS_TAG, "[isOtherPhone]====otherPhone?==="+preview.isOtherDeviceBackup());
	...
	private ArrayList<CallLogsData> getCallLogEntry() {
		...
		try {
			File file = new File(mParentFolderPath + File.separator + ModulePath.FOLDER_CALLLOG + File.separator + ModulePath.NAME_CALLLOG);
...
//					MyLogger.logD(CLASS_TAG,"startsWith(ID) = " +calllogData.id);
				...
				if(line.startsWith(CallLogsData.SIMID)){
					calllogData.simid = Utils.slot2SimId(Integer.parseInt(getColonString(line)), mContext);
//					MyLogger.logD(CLASS_TAG,"startsWith(ID) = " +calllogData.id);
				}
...
//					MyLogger.logD(CLASS_TAG,calllogData.toString());
	...
	protected boolean implementComposeOneEntity() {
        ...
        CallLogsData calllogFileEntry = calllogDatas.get(mIndex++);
