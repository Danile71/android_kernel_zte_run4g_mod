...
public class SmsRestoreComposer extends Composer {
    ...
    public boolean implementComposeOneEntity() {
        ...
        if (vMsgFileEntry != null) {
            ...
            if (values == null) {
                ...
                int mboxType = vMsgFileEntry.getBoxType();
    ...
    private ContentValues parsePdu(SmsRestoreEntry pdu) {
        ...
        values.put(Sms.READ,pdu.getSeen());
...
//                simCardid = "-1";
//            } else {
//                SIMInfo si = SIMInfo.getSIMInfoBySlot(mContext, soltid);
//                if (si == null) {
//                    simCardid = "-1";
//                } else {
//                    simCardid = String.valueOf(si.mSimId);
//                }
//            }
//        } else {
//            simCardid = "-1";
//        }
        values.put(Sms.SIM_ID,pdu.getSimCardid());
                
        values.put(Sms.DATE, pdu.getTimeStamp());
        values.put(Sms.TYPE, (pdu.getBoxType()));
...
//    private static final String DATE = "Date:";
    ...
    private static final String BEGIN_VMSG = "BEGIN:VMSG";
    private static final String VERSION = "VERSION:";
    private static final String XIRMS_TYPE = "X-IRMS-TYPE:MSG";
    private static final String VMESSAGE_DELIVER = "X-MESSAGE-TYPE:";
    private static final String XSEEN = "X-MESSAGE-STATUS:";
    private static final String XSLOT = "X-MESSAGE-SLOT:";
    private static final String XLOCKED = "X-MESSAGE-LOCKED:";
    private static final String BEGIN_VCARD = "BEGIN:VCARD";
    private static final String VCARD_VER= "VERSION:2.1";
    private static final String END_VCARD = "END:VCARD";
    ...
    public ArrayList<SmsRestoreEntry> getSmsRestoreEntry() {
        ...
        try {
            ...
            while ((line = buffreader.readLine()) != null) {
                ...
                if (line.startsWith(VMESSAGE_DELIVER) && !appendbody) {
                    smsentry.setBoxType(line.substring(VMESSAGE_DELIVER.length()).equals("DELIVER")?1:2);
                    Log.d(CLASS_TAG,"deliver = " + (line.substring(VMESSAGE_DELIVER.length()).equals("DELIVER")?"1":"2"));
                }
...
//                }
                if (line.startsWith(XSEEN) && !appendbody) {
                    smsentry.setSeen(line.substring(XSEEN.length()).equals("READ")?"1":"0");
                    Log.d(CLASS_TAG,"startsWith(XSEEN)");
                }
                if (line.startsWith(XSLOT) && !appendbody) {
                	int slot = Integer.parseInt(line.substring(XSLOT.length()));
                    smsentry.setSimCardid(Utils.slot2SimId(slot, mContext));
                ...
                if (line.startsWith(DATE) && !appendbody) {
                	long result = Utils.unParseDate(line.substring(DATE.length()));
