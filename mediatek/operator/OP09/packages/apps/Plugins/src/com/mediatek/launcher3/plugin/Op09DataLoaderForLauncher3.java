...
public class Op09DataLoaderForLauncher3 extends DefaultDataLoader {
    private static final String TAG = "Op09DataLoaderForLauncher3";
    ...
    public Op09DataLoaderForLauncher3(Context context) {
        super(context);
        mContext = context;
    }

    @Override
    public void createAllAppsTable(SQLiteDatabase db, Context context) {
        LauncherLog.d(TAG, "creating all apps table.");

        mMaxIdInAllAppsList = 1;

        db.execSQL("CREATE TABLE allapps (" + "_id INTEGER PRIMARY KEY,"
                + "title TEXT," + "intent TEXT," + "screen INTEGER,"
                + "cellX INTEGER," + "cellY INTEGER," + "itemType INTEGER,"
                + "visible INTEGER NOT NULL DEFAULT 1,"
                + "modified INTEGER NOT NULL DEFAULT 0" + ");");

        setFlagToLoadAllAppsLater(context);
    }
    ...
    private long addItemToAllAppsList(SQLiteDatabase db, ContentValues values, TypedArray a,
        ...
        if (mEmptyCells != 0) {
            ...
            if (index < mEmptyCells) {
                throw new IllegalArgumentException(
                        "addItemToAllAppsList: Wrong xml configuration! Index is smaller than empty cells: index="
                                + index + ", emptycells=" + mEmptyCells);
            }
        ...
        try {
            ...
            LauncherLog.w(TAG, "Can't find the activity info in PackageManager: " + packageName + "/" + className, e);
            mEmptyCells++;
            return -1;
        }
        ...
        values.put(AllApps.ITEM_TYPE, LauncherSettings.Favorites.ITEM_TYPE_APPLICATION);

        int cellIndex = changePositionToIndex(Integer.valueOf(x), Integer.valueOf(y));
        if (cellIndex >= getNumberOfIconsInOnePage()) {
            throw new IllegalArgumentException(
                    "Wrong xml configuration! The cell number must be smaller than 16: cellIndex is "
                            + cellIndex);
        }
        if (cellIndex != mCellIndex + 1) {
            throw new IllegalArgumentException(
                    "Wrong xml configuration! The cellX and cellY must be ascend and continous: cellIndex is "
                            + cellIndex + ", last index is " + mCellIndex);
        }
        ...
        try {
            ...
            while ((type != XmlPullParser.END_TAG || parser.getDepth() > depth)
                ...
                if (TAG_APPLICATION_ITEM.equals(name)) {
                    ...
                    id = addItemToAllAppsList(db, values, a, packageManager, intent, componentNames);
                    a.recycle();
                }
                if (id >= 0) {
                    i++;
    ...
    private void setFlagToLoadAllAppsLater(Context context) {
