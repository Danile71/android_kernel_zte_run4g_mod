...
public class Op09MmsMultiDeleteExt extends DefaultMmsMultiDeleteExt {
    ...
    public boolean deleteMassTextMsg(String[] msgIds) {
        ...
        MmsLog.d(TAG, "deleteMassTextMsg in Plugin method");
        return true;
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                context.getContentResolver().update(
                    Sms.CONTENT_URI,
                    values,
                    "(ipmsg_id in ( select distinct ipmsg_id from sms where ipmsg_id < 0 and _id in ("
                        + ids + " ))) or ( _id in ( " + ids + " ) and ipmsg_id >= 0 )", null);
            }
        }).start();
        return true;
    }
