...
public class Op09MmsTextSizeAdjustExt extends DefaultMmsTextSizeAdjustExt {
    ...
    public Op09MmsTextSizeAdjustExt(Context context) {
        super(context);
    ...
    public void init(IMmsTextSizeAdjustHost host, Activity activity) {
        MmsLog.d(TAG, "init(): init IMmsTextSizeAdjustHost.");
    ...
    public void setHostContext(Context context) {
        ...
        if (mHostContext != null) {
            mTextsize = Op09MmsUtilsExt.getTextSize(mHostContext);
    ...
    public void refresh() {
        MmsLog.d(TAG, "refresh(): mHostContext = " + mHostContext);
        //get the lasted font size and set.
        if (mHostContext != null) {
            float size = Op09MmsUtilsExt.getTextSize(mHostContext);
            MmsLog.d(TAG, "refresh(): Current textSize = " + mTextsize + ", new textSize = " + size);
            if (mTextsize != size) {
                mTextsize = size;
                mListener.setTextSize(mTextsize);
    ...
    public class OP09OnScaleListener extends ScaleDetector.SimpleOnScaleListener {
        ...
        protected void performChangeText(float size) {
            MmsLog.d(TAG, "performChangeText(): Current textSize = " + mTextsize + ", new textSize = " + size);
            if (mTextsize != size) {
                mTextsize = size;
                IMmsTextSizeAdjustHost host = getHost();
                if (host != null) {
                    MmsLog.d(TAG, "performChangeText(): Calling host.setTextSize()");
