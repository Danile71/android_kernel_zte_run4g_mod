...
public class Op09MusicTrackBrowserExtension extends DefaultMusicTrackBrowser {
    ...
    private static final String SELECT_FOLDER_FROM_FILEMANAGER = "com.mediatek.filemanager.DOWNLOAD_LOCATION";
    private static final String SELECT_FILE_FROM_FILEMANAGER = "com.mediatek.filemanager.ADD_FILE";

    private final int REQUEST_TO_ADD_FOLDER_TO_PLAY = PluginUtils.ACTIVITY_REQUEST_CODE_BASE_ID;
    private final int REQUEST_TO_ADD_SONG_TO_PLAY = PluginUtils.ACTIVITY_REQUEST_CODE_BASE_ID + 1;
    private final int REQUEST_TO_ADD_FOLDER_AS_PLAYLIST = PluginUtils.ACTIVITY_REQUEST_CODE_BASE_ID + 2;
    ...
    public boolean onOptionsItemSelectedForPlugin(Context context, MenuItem menuItem, String activityName, Activity activity, PluginUtils.IMusicListenter musicListener, Bundle options) {
    ...
    public void onActivityResultForPlugin(int requestCode, int resultCode, Intent intent,
            Context context, PluginUtils.IMusicListenter musicListener, String activityName, Activity activity, Bundle options) {
            ...
            if (activityName.equals(PluginUtils.PLAYLIST_BROWSER_ACTIVITY)) {
                 ...
                 if (requestCode == REQUEST_TO_ADD_FOLDER_AS_PLAYLIST) {
                     ...
                     if (intent != null) {
                         if (intent != null) {
                             addFolderToMusic(context, intent.getStringExtra("download path"), -1, false, musicListener);
                         }
                     }
                 }
                 ...
                 if (requestCode == REQUEST_TO_ADD_SONG_TO_PLAY) {
                      ...
                      if (intent != null) {
                          addFolderToMusic(activity, intent.getStringExtra("download path"), -1, true, musicListener);
                      }                      
                 }
            }
    }

    private void addSongToPlay(Context context, Uri uri, PluginUtils.IMusicListenter musicListener) {
    ...
    private long [] getSongListForCursorExceptSubFolder(Cursor cursor, String folderPath) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            Log.d(TAG, "getSongListForCursorExceptSubFolder: path = " + path + ", folderPath = " + folderPath);
    ...
    private void addFolderToMusic(Context context, String folderPath, long needMoveToFirstAudioId, boolean needPlay, PluginUtils.IMusicListenter musicListener) {
