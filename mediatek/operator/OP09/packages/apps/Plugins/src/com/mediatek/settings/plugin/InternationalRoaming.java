...
/**
 * M: The class <code>InternationalRoaming</code> is main class of International
 * Roaming about China Telecom.
 */
public class InternationalRoaming extends Activity implements TabListener {

    private static final String TAG = "IR Settings";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        mSlot1Fragement.setSlotId(PhoneConstants.GEMINI_SIM_1);
        mSlot2Fragement.setSlotId(PhoneConstants.GEMINI_SIM_2);
        ...
        Xlog.d(TAG, "onCreate: savedInstanceState = " + savedInstanceState + ",mSlot1Fragement = "
                + mSlot1Fragement + ", mSlot2Fragement = " + mSlot2Fragement + ",mCurSlot = "
                + mCurSlot);
        if (mCurSlot == PhoneConstants.GEMINI_SIM_1) {
            actionBar.selectTab(simSlot1);
        } else if (mCurSlot == PhoneConstants.GEMINI_SIM_2) {
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        Xlog.d(TAG, "onSaveInstanceState: outState = " + outState + ", mCurSlot = " + mCurSlot);
    }
    ...
    public void onRestoreInstanceState(Bundle state) {
        ...
        Xlog.d(TAG, "onRestoreInstanceState: state = " + state + ", mCurSlot = " + mCurSlot);
    }

    @Override
    public void onTabSelected(Tab tab, FragmentTransaction ft) {
        mCurSlot = tab.getPosition();
        Xlog.d(TAG, "onTabSelected: tab.getPosition() = " + tab.getPosition() + ", mCurSlot = "
                + mCurSlot);
