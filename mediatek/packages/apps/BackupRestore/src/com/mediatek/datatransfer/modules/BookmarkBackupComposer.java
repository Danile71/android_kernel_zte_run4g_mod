...
public class BookmarkBackupComposer extends Composer {
    ...
    public boolean init() {
        boolean result = true;
        String where = BrowserContract.Accounts.ACCOUNT_NAME + " IS null";

        mCursor = mContext.getContentResolver()
                .query(BrowserContract.Bookmarks.CONTENT_URI, null, BrowserContract.Bookmarks.IS_FOLDER + "=0 and "
                + where, null, null);
    ...
    public void onEnd() {
        ...
        String path = mParentFolderPath + File.separator + ModulePath.FOLDER_BOOKMARK;
        ...
        if (filenames.size() > 0) {
            FileUtils.scanPathforMediaStore(filenames, mContext);
        }
