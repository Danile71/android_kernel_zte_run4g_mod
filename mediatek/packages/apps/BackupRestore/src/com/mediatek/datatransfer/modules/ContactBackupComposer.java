...
public class ContactBackupComposer extends Composer {
    ...
    public void onEnd() {
        ...
        String fileName = path + File.separator + ModulePath.NAME_CONTACT;
        if (fileName != null ) {
            FileUtils.scanPathforMediaStore(fileName, mContext);
        }
    }
...
//             List<SIMInfo> simInfoList = SIMInfo.getInsertedSIMList(mContext);
             ...
             if(len > 0) {
                 ...
                 if ("-1".equals(conditionList.get(0))){
                     ...
                     condition.append(" AND 'Local Phone Account' = (" +
                     "select view_raw_contacts." + RawContacts.ACCOUNT_TYPE 
                     +" from view_raw_contacts where view_raw_contacts._id = view_contacts.name_raw_contact_id) ) ");
                 } else {
                    condition.append(Contacts.INDICATE_PHONE_SIM + " =" + conditionList.get(0));
                 }
                 for(int i = 1; i < len; ++i) {
