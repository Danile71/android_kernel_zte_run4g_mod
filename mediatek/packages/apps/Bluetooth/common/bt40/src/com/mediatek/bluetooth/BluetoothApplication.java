...
public class BluetoothApplication extends Application {
    ...
    private final BroadcastReceiver multiuser_mReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
				Log.d(TAG, "receive ACTION_USER_SWITCHED");
				enableProfEntry(context, ProfileConfig.PROFILE_ID_PRXM); 	
            } 
        }
    };
    ...
    public void onCreate() {
		...
		enableProfEntry((Context)this,ProfileConfig.PROFILE_ID_PRXM);       

	    IntentFilter filter_multiuser = new IntentFilter();
		filter_multiuser.addAction(Intent.ACTION_USER_SWITCHED);
	    registerReceiver(multiuser_mReceiver, filter_multiuser);
	...
	private void enableProfEntry(Context context,String profileId)
	{
		//PackageManager pm = getPackageManager();
		ComponentName compName;
		int enableFlag = PackageManager.COMPONENT_ENABLED_STATE_ENABLED;

        IPackageManager pms = ActivityThread.getPackageManager();
		...
		if(profileId.equals(ProfileConfig.PROFILE_ID_PRXM))
		{
			try {
				...
				//Log.d(TAG, "user id: " + context.getUserId());
				Log.d(TAG, "user id: " + ActivityManager.getCurrentUser());

				try {
					pms.setComponentEnabledSetting(compName, enableFlag, PackageManager.DONT_KILL_APP, ActivityManager.getCurrentUser());				
