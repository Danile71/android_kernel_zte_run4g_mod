 ...
 ********************************************************************************************/
	...
	public void onCreate() {
		...
		if (sm == null) {
			printErr("Get Storage-Manager failed. Stop FTP service.");
			stopSelf();
		}

		sd_paths = sm.getVolumePaths();
		if (sd_paths != null) {
			mSDCount = sd_paths.length;
		}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mServiceInitiated) {
			...
			printLog("Pre-enable FTP Server, root: " + root + " mSDCount: " + mSDCount);
			...
			if (mServerPreferences.getBoolean(SERVER_KEY_ENABLE, false)) {
				...
				if (mSDCount > 1) {
					...
					String sdPath = Environment.getExternalStorageDirectory().getAbsolutePath();
					if (SystemUtils.isExternalStorageMounted(this, sdPath)) {
						serverEnableReqNative(root);
					}
				}
			}
