    ...
    private void openEmergencyAlertNotification(CellBroadcastMessage message, long msgRowId) {
        ...
        if (message.isEtwsMessage()) {
            ...
            if(optAddingAlertVibration != null){
            	Log.d(TAG, "optAddingAlertVibration not null");
            	alertVibrate = optAddingAlertVibration.getAlertVibration(message.getServiceCategory());
            	Log.d(TAG, "finaaly called vibration plugin");
            }
        ...
        audioIntent.putExtra(CellBroadcastAlertAudio.ALERT_AUDIO_MESSAGE_ID, message.getServiceCategory());
