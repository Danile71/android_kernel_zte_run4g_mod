...
public class ShowPreviewActivity extends Activity implements ServiceConnectedOperation {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /* [Problem Description] When import event for the first time, HandleProgressActivity or
         * ShowHandleResultActivity doesn't finish in the task (for example, press the home key).
         * Then import event once again, ShowPreviewActivity comes out, click 'cancel' button,
         * HandleProgressActivity or ShowHandleResultActivity appears. It's not normal.
         * [Solution] When ShowPreviewActivity is started by external application, finish itself
         * and start new ShowPreviewActivity with intent which has flags Intent.FLAG_ACTIVITY_CLEAR_TASK.
         */
        boolean internalStart = mIntent.getBooleanExtra(INTERNAL_START, false);
        if (internalStart == false) {
            finish();
            mIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
            mIntent.putExtra(INTERNAL_START, true);
            startActivity(mIntent);
            LogUtils.d(TAG, "onCreate. Start ShowPreviewActivity internal.");
            return;
        }
