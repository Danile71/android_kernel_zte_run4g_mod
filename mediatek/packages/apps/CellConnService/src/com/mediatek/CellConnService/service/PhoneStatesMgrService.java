...
//    static final int VERIFY_TYPE_PUK2 = 505;
    ...
    private boolean mIsWaitingForSimReadyEvent = false;
    private boolean mIsVerify = false;
    private int mItselfDualSimMode = -1;
    private boolean mGotItselfDualSimMode = false;
            ...
            if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(intent.getAction())) {
                ...
                if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
                    ...
                    if (airplaneModeON) {
                        ...
                        if ((mIsTurningOffFlightModeOrPowerOnRadio || mIsWaitingForSimReadyEvent) && (mICallBackList.size() > 0)) {
                            Log.d(TAG, "Send result(abort) when flight mode is turned on again.");
                            Message msg = Message.obtain(mServiceHandler, MSG_ID_CHECKROAMING);
                            msg.arg1 = mICallBackList.get(0).getReqSlot();
                            if (mServiceHandler == null) {
                            ...
                            mResult = CellConnMgr.RESULT_ABORT;
                            mServiceHandler.sendMessage(msg);
                            mIsWaitingForSimReadyEvent = false;
                        }
                    } else {
                        // radio on
                        mPhoneStates[0] = PHONE_STATE_RADIOON;

                        if ((mICallBackList.size() > 0)) {
                            mIsWaitingForSimReadyEvent = true;
                        }
                    }
                }
                ...
                Log.d(TAG, "BroadcastReceiver duslSimMode = " + dualSimMode + " ,mItselfDualSimMode = " + mItselfDualSimMode);
                boolean forceAbort = false;

                if (mItselfDualSimMode == dualSimMode && mGotItselfDualSimMode == false) {
                    mGotItselfDualSimMode = true;
                    Log.d(TAG, "Got myself DUAL_SIM_MODE_CHANGED first time");
                }
                ...
                for (int i = 0 ; i < mSIMCount ; i++) {
                    if(dualSimMode < 0) {
                        ...
                        if (mGotItselfDualSimMode && mSimOnStates[i] == 1) {
                            forceAbort = true;
                        }
                        mPhoneStates[i] = PHONE_STATE_RADIOOFF;
                    }
                    Log.d(TAG, "mPhoneStates[" + i + "] = " + mPhoneStates[i] + " ,mSimOnStates = " + mSimOnStates[i]);
                }

                if (forceAbort && (mICallBackList.size() > 0)) {
                    // To abort radio change because user change dual sim mode again
                    Log.d(TAG, "To abort radio change because user change dual sim mode again");
                    Message msg = Message.obtain(mServiceHandler, MSG_ID_CHECKRADIO);
                    msg.arg1 = mICallBackList.get(0).getReqSlot();
                    if (mServiceHandler == null) {
                        Log.d(TAG, "verifyPhoneState wait looper");
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
            case MSG_ID_CHECKRADIO: {
                Log.d(TAG, "handleMessage: check radio off, mResult = " + mResult);

                if (mResult == CellConnMgr.RESULT_ABORT) {
                    broadcastCallback(handleSlot, reqItem.getReqType(), CellConnMgr.RESULT_ABORT);
                } else {
            ...
            case MSG_ID_CHECKSIMMELOCK:
                ...
                if (FeatureOption.SIM_ME_LOCK_MODE == 0) {
                    ...
                    Log.d(TAG, "handleMessage SIM-ME lock is not supported");
                    broadcastCallback(handleSlot, REQUEST_TYPE_UNKNOWN, CellConnMgr.RESULT_ABORT);
                }
                break;
    ...
    public void onCreate() {
        ...
        this.mSimOnStates = new int[this.mSIMCount];
    ...
    private void broadcastCallback(int slot, int reqType, int nRet) {
        ...
        mIsWaitingForSimReadyEvent = false;
        mIsVerify = false;
        mGotItselfDualSimMode = false;
        mItselfDualSimMode = -1;
        for (int i = 0; i < this.mSIMCount; ++i) {
            this.mSimOnStates[i] = 0;
        }
    ...
    private void powerRadioOn(int slot) {
        ...
        if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
            if (1 == flightMode) {
                ...
                mItselfDualSimMode = dualSimMode;
