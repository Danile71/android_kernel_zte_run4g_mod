...
import com.redbend.vdm.log.VdmLogLevel;
...
public class DmController {
    ...
    public DmController(final Context context) {
        ...
        if (Options.USE_SMS_REGISTER) {
            int simSlot = DmCommonFun.getSimRegisterID(context);
            if (simSlot != -1) {
                TelephonyManagerEx telMgrEx = TelephonyManagerEx.getDefault();
                String mccmnc = MTKPhone.getSimOperatorGemini(telMgrEx, simSlot);
                ...
                if (FeatureOption.MTK_IMS_SUPPORT) {
            ...
            String mccmnc = teleMgr.getSimOperator();
    ...
    private void updateRecovery() {
        ...
        try {
            String[] str = { "/system/bin/fota1", "--f", FOTADeltaFiles.DELTA_FOLDER, "--r" };
            process = Runtime.getRuntime().exec(str);
            Log.i(TAG.CONTROLLER, "[updateRecovery] process start");
            process.waitFor();
            Log.i(TAG.CONTROLLER, "[updateRecovery] process exitValue = " + process.exitValue());
        } catch (IOException e) {
            e.printStackTrace();
