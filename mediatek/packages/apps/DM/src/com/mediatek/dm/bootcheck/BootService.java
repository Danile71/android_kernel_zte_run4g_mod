...
public class BootService extends Service {
    ...
    private void unregisterConnReceiver() {
        if (mConnReceiver != null) {
            ...
            Log.d(CLASS_TAG, "--------connectivity receiver unregistered.--------");
        }
    }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_TIMER_CONN:
                ...
                BootService.this.registerReceiver(mConnReceiver, filter);
                Log.d(CLASS_TAG, "==[MSG_TIMER_CONN]connetivity receiver registered.==");
    ...
    private BroadcastReceiver mConnReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            Log.d(CLASS_TAG, "[ConnReceiver] onReceive");
            ...
            if (ConnectivityManager.CONNECTIVITY_ACTION.equals(action)) {
                Log.d(CLASS_TAG, "[ConnReceiver]: got connectivity change event!");

                ConnectivityManager connMgr = (ConnectivityManager) context
                        .getSystemService(Context.CONNECTIVITY_SERVICE);
                NetworkInfo netInfo = connMgr.getActiveNetworkInfo();
                if (netInfo == null) {
                    Log.w(CLASS_TAG, "[ConnReceiver]netInfo is null");
                    return;
                }

                int networkType = netInfo.getType();
                if (!Options.USE_DIRECT_INTERNET) {
                    int simId = DmCommonFun.getSimRegisterID(context);
                    if (simId == -1) {
                        Log.e(CLASS_TAG,
                                "[ConnReceiver]->Get Register SIM ID error in connetivity receiver");
                        return;
                    }
                    int networkSimId = MTKConnectivity.getSimId(netInfo);
                    int intentSimId = intent.getIntExtra(MTKConnectivity.EXTRA_SIM_ID, 0);

                    Log.i(CLASS_TAG,
                            new StringBuilder("[ConnReceiver]->type == ")
                                    .append(netInfo.getTypeName()).append("(").append(networkType)
                                    .append(")").append("\n")
                                    .append("[ConnReceiver]->intent_sim_Id == ")
                                    .append(intentSimId).append("\n")
                                    .append("[dm-conn]->network_sim_Id == ").append(networkSimId)
                                    .append("\n").append("[ConnReceiver]->registered_sim_Id == ")
                                    .append(simId).toString());

                    if (intentSimId != simId || networkType == MTKConnectivity.TYPE_WIFI) {
                        Log.e(CLASS_TAG, "[ConnReceiver]->SimId or connect type is incorrect");
                        return;
                    }
                ...
                if (netInfo.isConnected()) {
                    ...
                    // same time
                    if (aquireTrigger()) {
                        Log.i(CLASS_TAG, "[ConnReceiver]: network connected, trigger DmService.");
                        triggerDmService();
                        unregisterConnReceiver();
                    } else {
                        Log.d(CLASS_TAG, "[ConnReceiver]: already triggered, bypass.");
                    }
                }
            }
        }
