...
public class DmBootstrapHandler implements BootMsgHandler, SessionInitiator {
        ...
        AlertDialog dialog = new AlertDialog.Builder(DmService.getInstance())
                ...
                .setNegativeButton(android.R.string.cancel, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        Xlog.i(TAG, " Entered PIN Code: " + pinCode);
    ...
    private String getNetworkId() {
        ...
        // int slotId = getGprsConnectionSlotId();
        int slotId = 0;
        
        if (MTKPhone.getPhoneTypeGemini(mTelephonyManager, slotId) == TelephonyManager.PHONE_TYPE_GSM) {
            networkId = getImsi(slotId);
            Log.i(TAG, "getNetworkId, type is GSM, id is imsi = " + networkId);
        } else if (MTKPhone.getPhoneTypeGemini(mTelephonyManager, slotId) == TelephonyManager.PHONE_TYPE_CDMA) {
            //TODO CDMA networkID = SSD witch is calculated by CAVE, with parameters RAND ESN(UIM ID), IMSI and SSD-A
            String meid = getDeviceId(slotId);
    ...
    private String getDeviceId(int slot) {
        ...
        if (mTelephonyManager != null) {
            if (MTKPhone.getPhoneTypeGemini(mTelephonyManager, slot) == TelephonyManager.PHONE_TYPE_CDMA) {
                meid = MTKPhone.getDeviceIdGemini(mTelephonyManager, slot);
            }
        }
        if (meid != null) {
    ...
    private String getImsi(int slot) {
        String imsi = null;
        if (mTelephonyManager != null) {
            if (MTKPhone.hasIccCardGemini(mTelephonyManager, slot)) {
                imsi = MTKPhone.getSubscriberIdGemini(mTelephonyManager, slot);
            } else {
                Log.e(TAG, "there is no valid SIM card in the phone!");
            }
        } else {
