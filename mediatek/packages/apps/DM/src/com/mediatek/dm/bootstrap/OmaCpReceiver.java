...
public class OmaCpReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (CP_DM_CAPABILITY_ACTION.equals(intentAction)) {
            ...
            int simId = intent.getIntExtra(EXTRA_SIM_ID, 0);
            ...
            Log.i(TAG.CP, new StringBuilder("In receiver: server address = ").append(dmServerAddr)
                    .append(" simId = ").append(simId).toString());
    ...
    private boolean handleCpConfigMessage(int simId, String serverAddr) {
        ...
        Log.i(TAG.CP,
                new StringBuilder("Enter config DmServer addr : server addr = ").append(serverAddr)
                        .append("sim id is ").append(simId).toString());
        if (serverAddr == null) {
        ...
        if (Options.USE_SMS_REGISTER) {
            int sim = DmCommonFun.getSimRegisterID(mContext);
            if (sim == -1 || simId != sim) {
                Log.e(TAG.CP,
