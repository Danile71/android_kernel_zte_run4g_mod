...
public class DmClient extends Activity implements OnDismissListener {
    ...
    private void setNetworkTimeoutAlarm() {
        ...
        if (mAlarmManager == null) {
            Log.w(TAG.CLIENT, "setAlarm alarmMgr is null");
            return;
        }
        if (mAlarmManager != null) {
            mNetworkTimeoutPendingIntent = PendingIntent.getBroadcast(this, 0, intent,
                    PendingIntent.FLAG_CANCEL_CURRENT);
            mAlarmManager.cancel(mNetworkTimeoutPendingIntent);
            mAlarmManager.setExact(AlarmManager.RTC_WAKEUP, (System.currentTimeMillis() + TIME_OUT_VALUE
                    * ONE_SEVOND), mNetworkTimeoutPendingIntent);
        }
    }
