 ...
 */
public class FumoExtNodeIoHandler implements NodeIoHandler {

    private static final String TAG = "DM/FumoExtNode";
     ...
     *            Context
     * @param uri
     *            FUMO EXT node path of tree.xml
     */
    public FumoExtNodeIoHandler(Context context, String uri) {
        Log.i(TAG, "FumoExtNodeIoHandler constructed");
        mContext = context;
        mUri = uri;
    }
    ...
    public int read(int offset, byte[] data) throws VdmException {
        ...
        byte[] temp = (mDmTree.getStringValue(mUri)).getBytes();

        int numberRead = 0;
        int dlength = data.length;
        int tlength = temp.length;
        for (; numberRead < dlength - offset; numberRead++) {
            if (numberRead < tlength) {
    ...
    public void write(int offset, byte[] data, int totalSize) throws VdmException {
        ...
        VdmTree mDmTree = new VdmTree();
        mDmTree.replaceStringValue(mUri, new String(data));
