...
public class DmScomoDownloadDetailActivity extends Activity implements OnDmScomoUpdateListener {
    ...
    private PendingIntent mNetworkTimeoutPendingIntent;
    ...
    private void checkNetwork() {
        ...
        try {
            ...
            if (MTKPhone.APN_ALREADY_ACTIVE == result) {
    ...
    private void setNetworkTimeoutAlarm() {
        Log.d(CLASS_TAG, "DownloadDetailActivity->setNetworkTimeoutAlarm");
        Intent intent = new Intent(DmConst.IntentAction.NET_DETECT_TIMEOUT);
        mAlarmManager = (AlarmManager) this.getSystemService(Context.ALARM_SERVICE);
        if (mAlarmManager == null) {
            Log.w(CLASS_TAG, "alarm manager is null, return");
        }
        if (mAlarmManager != null) {
            mNetworkTimeoutPendingIntent = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);
            mAlarmManager.cancel(mNetworkTimeoutPendingIntent);
            mAlarmManager.setExact(AlarmManager.RTC_WAKEUP, (System.currentTimeMillis() + TIME_OUT_VALUE * ONESEVOND),
                    mNetworkTimeoutPendingIntent);
        }
    }

    private void cancleNetworkTimeoutAlarm() {
        if (mAlarmManager != null && mNetworkTimeoutPendingIntent != null) {
            mAlarmManager.cancel(mNetworkTimeoutPendingIntent);
            mAlarmManager = null;
            mNetworkTimeoutPendingIntent = null;
        }
    }
