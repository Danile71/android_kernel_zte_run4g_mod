...
static int read_int_val(RPCClient *msg_client) {
    ...
    if (type != PARAM_TYPE_INT) {
        XLOGD("FAIL TO read_int_val: INVALID TYPE");
        return -1;
    }
...
static int cat(char *buffer, int bsize, const char* file) {
    ...
    if (fp == NULL) {
        XLOGE("cat popen fail, errno: %d, %s", errno, strerror(errno));
        return -1;
    }
    ...
    XLOGD("cat: >%s, errno:%d", buffer, errno);
    pclose(fp);
    return 0;
}
...
static int readfile(char *buffer, int bsize, const char* file) {
    ...
    int index = 0;
    if (fp == NULL) {
        XLOGD("fopen fail: errno:%d, %s", errno, strerror(errno));
        return -1;
    }
...
static int parse_cpu_test_request(RPCClient *msg_client, cpu_test_request_t *request) {
    ...
    if (request->cpu_info_len > 0) {
        ...
        if (request->core_bit_len > 0) {
            ...
            if (request->test_cores == NULL) {
                XLOGD("parse_cpu_test_request; malloc fail for test_cores");
                return -1;
            }
    ...
    if (request->test_data_len > 0) {
        ...
        if (request->test_data == NULL) {
            XLOGD("parse_cpu_test_request; malloc fail for test_data");
            return -1;
        }
...
static int get_test_core_val(int core_index, cpu_test_request_t * request) {
    if (request->core_bit_len <= 0 || core_index >= request->core_num || core_index < 0) {
        XLOGD("get_test_core_val invalid argument: core_index:%d, core_num:%d", core_index, request->core_num);
        return -1;
    }
    int n = 32 / request->core_bit_len;
...
void * apmcu_test(void * argvoid) {
    ...
    XLOGD("apmcu_test result : %s", arg->result);
    pthread_exit(NULL);
    return NULL;
}
...
static char* get_apmcu_test_file(int index, int core_index, cpu_test_request_t* request) {
    ...
    if (index < 0 || index >= test_count || core_index < 0 || core_index >= core_count) {
        XLOGE("get_apmcu_test_file: Invalid argument, index:%d, core_index:%d", index, core_index);
        return NULL;
    }
...
void doApMcuTest(RPCClient* msgSender, cpu_test_request_t* request, int mode) {
    ...
    if (mode == MODE_RADIO_BACKUP_RESTORE) {
        if (request->test_data_len < 3) {
            XLOGD("doApMcuTest MODE_RADIO_BACKUP_RESTORE, invalid test_data");
        ...
        if (core_num > CORE_NUMBER_MAX || core_num <= 0) {
            XLOGD("doApMcuTest, invalid core_num:%d", core_num);
            msgSender->PostMsg("INVALID_CORE_NUM");
            return;
        }
        if (request->test_data_len < 2) {
            XLOGD("MODE_CHECK_CUSTOM, invalid test_data");
            msgSender->PostMsg("INVALID_TEST_DATA");
            return;
        }
        index = request->test_data[1];
        if (index == INDEX_TEST_IDLE2MAX) {
            ...
            if (ret < 0) {
                XLOGD("fail to get original affinity");
            ...
            if (ret < 0) {
                XLOGD("fail to set affinity to cpu0");
            ...
            if (ret < 0) {
                XLOGD("fail to restore process affinity with cpu");
            }
...
int ModuleCpuStress::ApMcu(RPCClient* msgSender) {
    ...
    if (ret < 0) {
        XLOGD("parse_cpu_test_request fail");
        goto err_out;    
    }
    if (request->test_data_len < 1) {
        XLOGD("ApMcu: fail to get mode; invalid test_data_len:%d", request->test_data_len);
        goto err_out;
    }
...
static char* get_swcodec_test_command(int core_index) {
    ...
    if (core_index < 0 || core_index >= len) {
        XLOGD("get_swcodec_test_command INVALID core_index:%d", core_index);
        return NULL;
    }
    int chip = em_jni_get_chip_id();
...
void doSwCodecTest(RPCClient* msgSender, cpu_test_request_t* request, int mode) {
    ...
    if (mode == MODE_RADIO_BACKUP_RESTORE) {
        if (request->test_data_len < 3) {
            XLOGD("doSwCodecTest MODE_RADIO_BACKUP_RESTORE, invalid test_data");
            msgSender->PostMsg("INVALID_TEST_DATA");
            return;
        }
        ...
        int core_num = request->core_num;
        if (core_num > CORE_NUMBER_MAX || core_num <= 0) {
            XLOGD("doSwCodecTest, invalid core_num:%d", core_num);
            msgSender->PostMsg("INVALID_CORE_NUM");
            return;
        }
        for (int i = 0; i < core_num; i++) {
        ...
        XLOGD("doSwCodecTest Unknown mode:%d", mode);
        msgSender->PostMsg("UNKNOWN_MODE");
        return;
    }
    XLOGD("doSwCodecTest result>%s", result);
    msgSender->PostMsg(result);
}
...
int handle_custom_mode_check_change(RPCClient * msgSender, cpu_test_request_t* request) {
    ...
    if (enable_num > 0 && disable_num > 0) {
        XLOGD("INVALID request; no support enable & disable meanwhile");
        goto err_out;
    }
...
int  handle_check_custom_force_core(RPCClient * msgSender, cpu_test_request_t* request) {
    int ret = 0;
    XLOGD("ENTER handle_check_custom_force_core");
    if (request->cpu_info_len <= 0) {
        XLOGD("[handle_check_custom_force_core] invalid cpu_info_len");
        msgSender->PostMsg((char*)ERROR);
        return -1;
    }

    if (request->test_data_len < 2) {
        XLOGD("[handle_check_custom_force_core] invalid test_data_len");
        msgSender->PostMsg((char*)ERROR);
        return -1;
    }
    ...
    if (test_files_num < core_num) {
        XLOGD("[handle_check_custom_force_core] test_files_num:%d must be no less than core_num:%d ", core_num, test_files_num);
        msgSender->PostMsg((char*)ERROR);
        return -1;
    }
...
int handle_check_test_mode_v2(RPCClient * msgSender, cpu_test_request_t* request, bool checked) {
    char* ret_str = ERROR;
    if (request->cpu_info_len <= 0) {
        XLOGD("handle_check_test_mode_v2, invalid cpu_info_len");
...
int handle_custom_setting_changed_v2(RPCClient * msgSender, cpu_test_request_t* request) {
    ...
    int ret = 0;
    if (request->cpu_info_len <= 0) {
        XLOGD("handle_custom_setting_changed_v2 invalid cpu_info_len:%d", request->cpu_info_len);
        msgSender->PostMsg(ERROR);
        return -1;
    }
...
int handle_custom_force_core_v2(RPCClient * msgSender, cpu_test_request_t* request) {
    ...
    int ret = 0;
    if (request->test_data_len < 2) {
        XLOGD("handle_custom_force_core_v2 invalid test_data_len:%d", request->test_data_len);
        msgSender->PostMsg(ERROR);
        return -1;
    }
    if (request->cpu_info_len <= 0) {
        XLOGD("handle_custom_force_core_v2 invalid cpu_info_len:%d", request->cpu_info_len);
        msgSender->PostMsg(ERROR);
        return -1;
    }
    int index = request->test_data[1];
    ...
    if (test_file_num < core_num) {
        XLOGD("handle_custom_force_core_v2 test_file_num must be no less than core_num");
        msgSender->PostMsg(ERROR);
        return -1;
    }
    switch (index) {
    ...
    case INDEX_FORCE_CORE_CUSTOM:
        ...
        break;
    default:
        XLOGD("handle_custom_force_core_v2 UNKNOWN index:%d", index);
        ret_str = ERROR;
        ret = -1;
        break;
    }
...
int ModuleCpuStress::BackupRestore(RPCClient* msgSender) {
    ...
    if (request->test_data_len < 1) {
        XLOGD("BackupRestore: fail to get mode; invalid test_data_len:%d", request->test_data_len);
        goto err_out;
    }
    mode = request->test_data[0];
    switch (mode) {
    case MODE_RADIO_BACKUP_RESTORE:
    {
        int index = 0;
        if (request->test_data_len < 2) {
            XLOGD("BackupRestore: fail to get index invalid test_data_len:%d", request->test_data_len);
            goto err_out;
        }
        index = request->test_data[1];
    ...
    case MODE_CUSTOM_V2:
        ...
        break;
    default:
        XLOGD("BackupRestore UNKOWN mode:%d", mode);
        goto err_out;
    }
    goto out;
err_out:
