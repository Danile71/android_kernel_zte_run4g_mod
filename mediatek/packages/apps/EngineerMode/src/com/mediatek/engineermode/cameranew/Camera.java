...
public class Camera extends ActivityBase implements FocusManager.Listener, View.OnTouchListener, SurfaceHolder.Callback {
    ...
    private static int[] AWB_EVCOMP = {0, -10, 10};
    private static int AWB_EVCOMP_Index = 0;
    private static int AWB_TEMP_Index;
    private static int AWB_ROUND = 0;
    ...
    private boolean handleThroughFocusCapture() {
        ...
        if (mDBMode == DB3A.DB3A_AF) {
            if (mAfThroughRepeat > 1 && AF_EVSTEPSIZE > 0) {
                AF_EVCOMPENSATE = (-1 * (mAfThroughRepeat >> 1) + mAfThroughRepearPointer) * AF_EVSTEPSIZE;
    ...
    private void overrideEngParameter() {
        ...
        case DB3A_AE:
            ...
            mParameters.setEngParameter3(Integer.toString(mDBAEState.getCurrentState()));
            mCameraDevice.setParameters(mParameters);
            break;
        case DB3A_AF:
            // 3A - AF
...
//                    sendEmptyMessage(EVENT_CAPTURE_ACTION);
        ...
        if (mDBMode != DB3A.DB3A_OFF) {
            ...
            switch(mDBMode) {
                ...
                case DB3A_AF:
                    ...
                    break;
                case DB3A_AWB: // 3A - AWB
                    AWB_TEMP_Index = AWB_EVCOMP_Index - 1;
                    if((AWB_TEMP_Index < 0) || (AWB_TEMP_Index > 2)) {
                        AWB_TEMP_Index = 0;
                    }
                    info = String.format("[AWB] Item: %s, EV %d", DB3A_AWBTestItem, AWB_EVCOMP[AWB_TEMP_Index]);
                    mTvCameraInfo.setText(info);
                    break;
                case DB3A_FLASH_AE:
                    break;
    ...
    private void startPreview() {
        ...
        if (getCustomParameter(0).toUpperCase().equals("AE")) { // 3A - AE
            ...
            mDBAEState.setStart(-21);
            mDBAEState.setEnd(20);
            mDBAEState.setStep(1);
    ...
    private void setPictureSize(int camOri) {
        ...
        if (mCaptureMode == 0) {
            if (mCaptureSize == 1 || mCaptureSize == 2
                || (mCaptureSize >= 5 && mCaptureSize <= 10)) { // capture
        ...
        if (mDBMode != DB3A.DB3A_OFF) {
            ...
            switch(mDBMode) {
                ...
                case DB3A_AWB: // 3A - AWB
                    ...
                    evComp = Integer.toString(AWB_EVCOMP[AWB_EVCOMP_Index]) + "EV";
