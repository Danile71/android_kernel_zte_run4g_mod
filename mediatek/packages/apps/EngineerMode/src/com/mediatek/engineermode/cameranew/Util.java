...
public class Util {
    ...
    public static boolean toleranceRatio(double target, double candidate) {
        ...
        if (candidate > 0) {
            tolerance = Math.abs(target - candidate) <= 0.001;
        }
    ...
    public static double findFullscreenRatio(Context context, Parameters parameters) {
        ...
        if (parameters != null) {
            ...
            Elog.i(TAG, "fullscreen = " + fullscreen + " x = " + point.x + " y = " + point.y);
    ...
    public static List<String> buildSupportedPictureSize(Parameters parameters, double targetRatio) {
        ...
        if (sizes != null) {
            for (Size size : sizes) {
                if (toleranceRatio(targetRatio, (double)size.width / size.height)) {
                    list.add(Util.buildSize(size.width, size.height));
                }
            }
        }
        Elog.d(TAG, "buildSupportedPictureSize(" + parameters + ", " + targetRatio + ")" + list.size());
        for (String added : list) {
            Elog.d(TAG, "buildSupportedPictureSize() add " + added);
        }
        return list;
    }
}
