...
public class CfuActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (mGeminiPhone.getPhonebyId(mSlot).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                mSlot = PhoneConstants.GEMINI_SIM_2;
            }
        } else {
    ...
    private void sendAtCommand(int msg) {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mGeminiPhone.invokeOemRilRequestStringsGemini(createCmd(msg),
                    mResponseHander.obtainMessage(msg), mSlot);
        } else {
            mPhone.invokeOemRilRequestStrings(createCmd(msg), mResponseHander.obtainMessage(msg));
        }
    }
