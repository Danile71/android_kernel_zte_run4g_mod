...
public class LteNetworkInfoDetail extends Activity implements OnClickListener {
    ...
    private Handler mATCmdHander = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case MSG_NW_INFO:
                ...
                if (ar.exception == null) {
                    ...
                    for (int i = 0; i < mItemCount; i++) {
                        for (int j = 0; j < mItem[i].types.length; j++) {
                            if (!isLteDcMode() || mItem[i].types[j] < 70) {
                ...
                break;
            case MSG_NW_INFO_LTEDC:
                ar = (AsyncResult) msg.obj;
                if (ar.exception == null) {
                    String[] data = (String[]) ar.result;
                    Xlog.v(TAG, "data[0] is : " + data[0]);
                    Xlog.v(TAG, "flag is : " + data[0].substring(FLAG_DATA_BIT));
                    mFlagLteDc = Integer.valueOf(data[0].substring(FLAG_DATA_BIT));
                    mFlagLteDc = mFlagLteDc | FLAG_OFFSET_BIT;
                    Xlog.v(TAG, "flag change is : " + mFlagLteDc);
                    for (int i = 0; i < mItemCount; i++) {
                        for (int j = 0; j < mItem[i].types.length; j++) {
                            if (mItem[i].types[j] >= 70) {
                                String[] atCommand = new String[2];
                                atCommand[0] = "AT+EINFO=" + mFlagLteDc + "," + mItem[i].types[j] + ",0";
                                atCommand[1] = "+EINFO";
                                sendATCommandLteDc(atCommand, MSG_NW_INFO_OPEN);
                            }
                        }
                    }
                }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (null != checked) {
            for (int i = 0; i < checked.length; i++) {
                if (1 == checked[i]) {
                    mItem[mItemCount] = new NetworkInfoPage(i, pageLabels[i], UrcParser.getTypes(i));
                    mItemCount++;
                }
            }
    ...
    private void showNetworkInfo() {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            String info = new String(UrcParser.parse(page, types[i], raw == null ? null : raw.toCharArray()));
            text += info;
        }
        mInfo.setText(text);
    }
    ...
    private void registerNetwork() {
        if (isLteDcMode()) {
            ...
            sendATCommandLteDc(new String[] {"AT+EINFO?", "+EINFO"}, MSG_NW_INFO_LTEDC);
        }
    ...
    private void unregisterNetwork() {
        if (isLteDcMode()) {
            ...
            mFlagLteDc = mFlagLteDc & FLAG_OR_DATA;
            Xlog.v(TAG, "LTEDC close flag is :" + mFlagLteDc);
            sendATCommandLteDc(new String[] {"AT+EINFO=" + mFlagLteDc, ""}, MSG_NW_INFO_CLOSE);
        }
    ...
    private void sendATCommandLteDc(String[] atCommand, int msg) {
        Xlog.v(TAG, "sendATCommandLteDc :" + atCommand[0]);
    ...
    private void sendATCommand(String[] atCommand, int msg) {
        Xlog.v(TAG, "sendATCommand :" + atCommand[0]);
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
    ...
    private boolean isLteDcMode() {
        return PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.isLteDcMode();
    }
}
