...
public class VoiceCommonSetting extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mWakeupMode = Settings.System.getInt(getContentResolver(), Settings.System.VOICE_WAKEUP_MODE, 0);
        if (mWakeupMode != VoiceSettingWrapper.WAKEUP_MODE_KEYWORD && 
                mWakeupMode != VoiceSettingWrapper.WAKEUP_MODE_KEYWORD_SPEAKER) {
            Toast.makeText(this, "Invalid voice wake-up mode:" + mWakeupMode, Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
        initUiComponent();
    ...
    private void initUiByData() {
        int clean = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_CLEAN);
        int noisy = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_NOISY);
        int training = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_TRAINING);
        int testing = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_TESTING);
    ...
    private void setRecognitionSetting(int wakeMode, int clean, int noisy, int training, int testing) {
        VoiceSettingWrapper.setWakeupRecognitionParam(wakeMode, VoiceSettingWrapper.RECOGNITION_CLEAN, clean);
        VoiceSettingWrapper.setWakeupRecognitionParam(wakeMode, VoiceSettingWrapper.RECOGNITION_NOISY, noisy);
        VoiceSettingWrapper.setWakeupRecognitionParam(wakeMode, VoiceSettingWrapper.RECOGNITION_TRAINING, training);
        VoiceSettingWrapper.setWakeupRecognitionParam(wakeMode, VoiceSettingWrapper.RECOGNITION_TESTING, testing);
    }
    ...
    private boolean checkSetResult() {
        ...
        int clean2 = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_CLEAN);
        int noisy2 = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_NOISY);
        int training2 = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_TRAINING);
        int testing2 = VoiceSettingWrapper.getWakeupRecognitionParam(mWakeupMode, VoiceSettingWrapper.RECOGNITION_TESTING);
