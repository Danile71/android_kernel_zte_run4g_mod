...
import com.mediatek.bluetooth.fmOverBt.FmOverBtService;
import com.mediatek.bluetooth.fmOverBt.IFmOverBtService;
...
public class FMRadioService extends Service implements FMRecorder.OnRecorderStateChangedListener {
    ...
    // get FmOverBtService instance to get bluetooth state
    private IFmOverBtService mBtService = null;

    private boolean mIsBtServiceBinded = false;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_TOFMSERVICE_POWERDOWN.equals(action) || ACTION_FROMATVSERVICE_POWERUP.equals(action)
                ...
                // phone shut down, so exit FM
                    ...
                    || ACTION_PRE_SHUTDOWN.equals(action)) {
                /**
                 * 
            ...
            } else if (FmOverBtService.ACTION_FM_OVER_BT_CONTROLLER.equals(action)) {
                // change to controller if necessary
                // stop FM player btw.
                int fmOverBTState = intent.getIntExtra(FmOverBtService.EXTRA_RESULT_STATE,
                        FmOverBtService.FMSTART_SUCCESS);
    ...
    private boolean connectBtService() {
        LogUtils.d(TAG, "connectBtService() mBtService:" + mBtService);
        if (!isBtSupported()) {
            LogUtils.d(TAG, "connectBtService: chip not supported");
            return false;
        }
        if (mBtService == null) {
            mIsBtServiceBinded = getApplicationContext().bindService(new Intent(FmOverBtService.class.getName()),
                    mBtServiceConnection, Context.BIND_AUTO_CREATE);
            LogUtils.d(TAG, "connectBtService: bind bt service." + ", mIsBtServiceBinded:" + mIsBtServiceBinded);
        }
        return mIsBtServiceBinded;
    }

    private boolean disconnectBtService() {
        int btState = 0;
        LogUtils.d(TAG, "disconnectBtService() mBtService:" + mBtService + ", mIsBtServiceBinded:" + mIsBtServiceBinded);
        if (!isBtSupported()) {
            LogUtils.d(TAG, "disconnectBtService: chip not supported");
            return false;
        }
        if (mBtService == null) {
            LogUtils.d(TAG, "disconnectBtService: mBtService is null");
            return false;
        }

        try {
            btState = mBtService.getState();
        } catch (RemoteException re) {
            LogUtils.e(TAG, "binder error!!");
        }
        LogUtils.d(TAG, "disconnectBtService: btState=" + btState);
        // unbind Bt service
        if (mIsBtServiceBinded) {
            LogUtils.d(TAG, "unbind bt service");
            getApplicationContext().unbindService(mBtServiceConnection);
            mBtService = null;
            mIsBtServiceBinded = false;
        }
        // Notify BT to power down
        sendBroadcast(new Intent(FM_POWER_DOWN_MSG));
        return true;
    }
    ...
    private void waitIfTxSearching() {
        LogUtils.d(TAG, ">>> waitIfTxSearching " + FMRadioNative.getFmStatus(CURRENT_TX_SCAN));
        long start = System.currentTimeMillis();
        // true for TX is searching
        while (FMRadioNative.getFmStatus(CURRENT_TX_SCAN) == true) {
            if (System.currentTimeMillis() - start > 5000) {
                LogUtils.e(TAG, "waitIfTxSearching timeout");
                break;
            }
            try {
        ...
        LogUtils.e(TAG, "<<< waitIfTxSearching");
    }
...
//        setBtStatus(false);
    ...
    private boolean chipSupportOverBt() {
        ...
        LogUtils.e(TAG, "chipSupportOverBt() bluetooth.fmoverbt: " + result);
        return (1 == result);
    }
...
//            enableFMAudio(true);
    ...
    public void onCreate() {
        ...
        if (FeatureOption.MTK_BT_FM_OVER_BT_VIA_CONTROLLER) {
            filter.addAction(FmOverBtService.ACTION_FM_OVER_BT_CONTROLLER);
...
//        initService(iCurrentStation);
    ...
    private void handleBtConnectState(int connectState) {
        ...
        case BluetoothA2dp.STATE_PLAYING:
            ...
            enableFMAudio(false);
            if (!mIsBtServiceBinded) {
                connectBtService();//add for first start FM then open BT
            }
            connectBtDevice();
            break;
    ...
    private void fmOverBtController(int fmOverBTState) {
        ...
        case FmOverBtService.FMSTART_SUCCESS:
        ...
        case FmOverBtService.FMSTART_FAILED:
            enableFMAudio(true);
            break;

        case FmOverBtService.FMSTART_ALREADY:
            LogUtils.d(TAG, "ACTION_FM_OVER_BT_CONTROLLER: FM over BT already on-going!");
            break;
            
        case FmOverBtService.FMSTOPPED_IND:
            LogUtils.d(TAG, "ACTION_FM_OVER_BT_CONTROLLER: FM over BT is disconnected");
    ...
    private final ServiceConnection mBtServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            mBtService = IFmOverBtService.Stub.asInterface(service);
            if (null == mBtService) {
                LogUtils.d(TAG, "FmOverBtService onServiceConnected mBtService is null");
                return;
            }
            final String msg;
            int btState = -1;
            ...
            try {
                mBtService.setAudioPathToAudioTrack(mICallBack);
            } catch (RemoteException re) {
                LogUtils.e(TAG, "binder error!!");
            }
            LogUtils.d(TAG, "onServiceConnected: btState=" + btState);

            //if (btState == BluetoothA2dp.STATE_NOT_PLAYING || btState == BluetoothA2dp.STATE_CONNECTED ||
            //        btState == BluetoothA2dp.STATE_PLAYING) {
            if (btState == FmOverBtService.A2DP_STATE_CONNECTED) {
                LogUtils.d(TAG, FM_POWER_UP_MSG + " sent to A2dp service!!");
                sendBroadcast(new Intent(FM_POWER_UP_MSG));
            } else {
                if (mIsSpeakerUsed != isSpeakerPhoneOn()) {
                    setSpeakerPhoneOn(mIsSpeakerUsed);
                }
        ...
        public void onServiceDisconnected(ComponentName name) {
            ...
            // un expected disconnected -- that is, its process crashed
            LogUtils.d(TAG, "FmOverBtService onServiceDisconnected");
        }
    };
