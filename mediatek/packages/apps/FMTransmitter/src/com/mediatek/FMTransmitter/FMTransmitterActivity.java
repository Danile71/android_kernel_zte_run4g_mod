...
//    private static final String FMTRANSMITTERACTIVITY_FRAGMENT_TAG = "FMTransmitterTag";
    ...
    private FMTransmitterStation mFMTrasmitterStation;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        FMTransmitterStation.setSearchChannelListenter(mFMTXSearchChannelListener);
    }
                ...
                public void onServiceConnected(ComponentName className,
                    ...
                    if (null == mService) {
                        ...
                        if (!isServiceInit()) {
                            ...
                            FMTxLogUtils.i(TAG, "onServiceConn,isSearching:" + isSearching());
                            if (!isSearching()) {
                                mIsPlaying = isTxPowerUp();
                                refreshAllButtonsImages();
                                refreshAllButtonsStatus();
                                ...
                                FMTxLogUtils.d(TAG, "isSearching is true when onserviceconn");
    ...
    public void onDestroy() {
        ...
        FMTxLogUtils.v(TAG, "onDestroy,getTxStatus():" + getTxStatus() + ",mIsSeeking:"
                + mIsSeeking);
        switch (getTxStatus()) {
            case TXOPENED:
                if (mIsSeeking == false && !closeTxDevice()) {
            ...
            case TXPOWERUP:
                ...
                break;
            case TXCLOSED:
                if (mIsSeeking == false && mIsServiceStarted) {
                    boolean bRes = stopService(new Intent(FMTransmitterActivity.this,
                            FMTransmitterService.class));
                    if (!bRes) {
        ...
        FMTransmitterStation.setSearchChannelListenter(null);
    ...
    private void refreshAllButtonsImages() {
        FMTxLogUtils.v(TAG, ">>> FMTransmitterActivity.refreshAllButtonsImages,mIsPlaying"
                + mIsPlaying);
    ...
    private void refreshAllButtonsStatus() {
        FMTxLogUtils.v(TAG, ">>> FMTransmitterActivity.refreshAllButtonsStatus,mIsPlaying:"
                + mIsPlaying);
    ...
    public boolean turnToFrequency(float frequency) {
        ...
        if (null == mService) {
            ...
            try {
                ...
                mCurrentStation = (int) (frequency * BASE_NUMBER);
                disableRDS();
                if (bRet) {
                    enableRDS();
                }
    ...
    FMTransmitterStation.IFMTXSearchChannelListenter mFMTXSearchChannelListener = new FMTransmitterStation.IFMTXSearchChannelListenter() {
        public void onSearchStatusChanged(boolean isSearching, int currentStation) {
            DialogFragment prefragdialog = null;
            if (sFragmentManager != null) {
                prefragdialog = (DialogFragment) sFragmentManager
                        .findFragmentByTag(FMTANSMITTERACTIVITY_FRAGMENT_TAG);
            }
            FMTxLogUtils.d(TAG, "onSearchStatusChanged,isSearching = " + isSearching
                    + ",currentStation:" + currentStation);
            if (isSearching == false && prefragdialog != null) {
                prefragdialog.dismissAllowingStateLoss();
                ...
                mIsSeeking = false;
                mIsPlaying = isTxPowerUp();
                FMTransmitterActivity.this.runOnUiThread(new Runnable() {
                    public void run() {
                        mTextStationValue.setText(formatStation(finalCurrentStation));
                        enableAllButtons(true);
                        refreshAllButtonsImages();
                        refreshAllButtonsStatus();
                    }
