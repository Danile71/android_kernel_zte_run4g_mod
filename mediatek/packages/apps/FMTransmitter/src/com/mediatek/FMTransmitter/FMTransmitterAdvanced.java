    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mEditTextFrequency.addTextChangedListener(new TextWatcher() {
            public void afterTextChanged(Editable s) {
                ...
                // Cannot be null text; text length bigger than 1; the first
                // char is not 0.
                if (null != cs && cs.length() > 1 && '0' != cs.charAt(0)) {
                    if ((iFreq >= FMTransmitterStation.LOWEST_STATION && iFreq < FMTransmitterStation.INVALID_LOWEST_STATION)
                            || (iFreq > FMTransmitterStation.INVALID_HIGEST_STATION && iFreq <= FMTransmitterStation.HIGHEST_STATION)) {
                        bEnableOKButton = true;
                    }
                }
                mButtonOK.setEnabled(bEnableOKButton);
                if (bEnableOKButton || cs == null || (cs != null && cs.length() == 0)) {
                    mEditTextFrequency.setError(null);
                } else {
                    mEditTextFrequency.setError(getString(R.string.toast_input_valid));
                }
                FMTxLogUtils.d(TAG, "<<< afterTextChanged");
            }
...
//                    }
    ...
    public void onResume() {
        super.onResume();
        // M: make the error info syn by language switch,ALPS01605184. @{
        if (mHasError) {
            mEditTextFrequency.setError(getString(R.string.toast_input_valid));
        }
        //@}
    ...
    public void onPause() {
        ...
        // M: make the error info syn by language switch, ALPS01605184. @{
        if (mEditTextFrequency.getError() != null) {
            mEditTextFrequency.setError(null);
            mHasError = true;
        }
        //@}
