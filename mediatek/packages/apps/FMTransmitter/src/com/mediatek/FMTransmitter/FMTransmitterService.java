...
public class FMTransmitterService  extends Service {
    ...
    private FMTransmitterStation mFMTransmitterStation = new FMTransmitterStation();
        ...
        */
        ...
        public boolean powerUpTx(float frequency) {
            ...
            if (!sIsDevicePowerOn) {
                ...
                FMTransmitterNative.setFmStatus(CURRENT_TX_ON, true);//TX on
                FMTransmitterNative.setFmStatus(CURRENT_RX_ON, false);//RX off
        ...
        public boolean powerDownTx() {
            ...
            if (sIsDevicePowerOn) {
                ...
                boolean powerdownSuccess = nativePowerDown();
                if (powerdownSuccess) {
                    //0, FM_RX; 1, FM_TX
        ...
        private boolean nativePowerDown() {
            boolean isTxOn = FMTransmitterNative.getFmStatus(CURRENT_TX_ON);
            boolean powerdownSuccess = false;
            if (isTxOn) {
                FMTxLogUtils.d(TAG, "nativePowerDown start power down FM Tx");
                powerdownSuccess = FMTransmitterNative.powerdown(1);
                FMTxLogUtils.d(TAG, "nativePowerDown end power down FM Tx, return:" + powerdownSuccess);
            } else {
                FMTxLogUtils.d(TAG, "nativePowerDown RX is on, no need do power down FM Tx, return true");
                powerdownSuccess = true;
            }
            return powerdownSuccess;
        }
        ...
        public float[] searchChannelsForTx(float frequency, int direction, int number)
        {
            ...
            sIsSearching = true;
            FMTransmitterNative.setFmStatus(CURRENT_TX_SCAN, true);
            FMTransmitterStation.IFMTXSearchChannelListenter FMTXSearchChannelListenter = FMTransmitterStation
                    .getSearchChannelListenter();
            if (FMTXSearchChannelListenter != null) {
                FMTxLogUtils.e(TAG, "search begin,onSearchStatusChanged");
                FMTXSearchChannelListenter.onSearchStatusChanged(sIsSearching, -1);
            }
            ...
            if (!openTxDevice()) {
                ...
                FMTransmitterNative.setFmStatus(CURRENT_TX_ON, false);
                return null;
            }
            if (sIsDevicePowerOn) {
            ...
            if (FMTransmitterNative.powerup(frequency)) {
                    ...
                    //0, FM_RX; 1, FM_TX
                    if (!FMTransmitterNative.powerdown(0)) {
            ...
            sIsSearching = false;
            FMTXSearchChannelListenter = FMTransmitterStation.getSearchChannelListenter();
            if (FMTXSearchChannelListenter != null && channelList != null) {
                FMTxLogUtils.e(TAG, "search end,onSearchStatusChanged");
                int currentStation = (int) (channelList[0] * BASE_NUMBER);
                FMTXSearchChannelListenter.onSearchStatusChanged(sIsSearching, currentStation);
            }
            FMTransmitterNative.setFmStatus(CURRENT_TX_SCAN, false);
            return channelList;
