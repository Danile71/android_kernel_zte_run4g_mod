...
public class FileManagerSearchActivity extends AbsBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Init search path from saved bundle if have, otherwise get from intent
        if (savedInstanceState != null) {
            mSearchPath = savedInstanceState.getString(CURRENT_PATH);
        } else {
            mSearchPath = getIntent().getStringExtra(CURRENT_PATH);
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        // Store search path so that when activity re-create we can restore it(such as switch language)
        outState.putString(CURRENT_PATH, mSearchPath);
