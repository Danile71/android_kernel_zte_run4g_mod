...
public class PressRawActivity extends Activity {
	...
	double pressure2altitude(double p) {
		double i, j, h, altitude;
		i = 44330.0;
		j = 1013.25;
		h = 5.255;

		altitude = i*(1-(Math.pow(p/j, 1/h)));
		return altitude;
	}
	/*
	double pressure2altitude(double p) {
	...
	*/
	public void calcSensitivity(float pressValue, float h) {
		...
		if (EndFlag) {
			value = maxPress - minPress;
			buffer = "avgChangeScale: " + value;
			change_label.setText(buffer);
			altChange_label.setText("altchangeScale=" + (maxAltitude - minAltitude) + "\n");
			EndFlag = false;
	...
	protected void onResume() {
		...
		boolean tempSupported = sensorMgr.registerListener(
				mSensorEventListener, mTempSensor, SENSOR_DELAY_UI);
		boolean pressSupported = sensorMgr.registerListener(
				mSensorEventListener, mPressSensor, SENSOR_DELAY_UI);
		...
		Log.d(TAG, "tempSupported: " + tempSupported + "; pressSupported: "
				+ pressSupported);
	}
	...
	public final SensorEventListener mSensorEventListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			...
			if (type == Sensor.TYPE_PRESSURE) {
				...
				// display altitude
				altitude = (float) pressure2altitude(event.values[0]);
