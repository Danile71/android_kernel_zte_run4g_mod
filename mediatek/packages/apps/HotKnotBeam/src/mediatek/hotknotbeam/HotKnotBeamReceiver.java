...
public class HotKnotBeamReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if(intent.getAction().equalsIgnoreCase(HotKnotBeamService.HOTKNOTBEAM_START)) {
            context.startService(serviceIntent);
        } else if(intent.getAction().equalsIgnoreCase(HotKnotBeamService.HOTKNOT_DL_COMPLETE)) {

            String intentInfo = intent.getStringExtra(HotKnotBeamService.HOTKNOT_EXTRA_APP_INTENT);
            Uri uri = (Uri) intent.getParcelableExtra(HotKnotBeamService.HOTKNOT_EXTRA_APP_URI);
            String mimetype = intent.getStringExtra(HotKnotBeamService.HOTKNOT_EXTRA_APP_MIMETYPE);
            boolean isCheck = intent.getBooleanExtra(HotKnotBeamService.HOTKNOT_EXTRA_APP_ISCHECK, true);
