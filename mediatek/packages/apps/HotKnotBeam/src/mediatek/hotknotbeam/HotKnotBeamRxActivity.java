...
import com.mediatek.hotknotbeam.MimeUtilsEx;

public class HotKnotBeamRxActivity extends Activity implements View.OnClickListener, Handler.Callback {
        ...
        synchronized(mItemLock) {
            ...
            if(mInfo.isGroup()) {
                if(fileType == HotKnotFileType.IMAGE) {
                    title = mResource.getString(R.string.rx_group_ui_image_status, mInfo.mOrder, mInfo.mCount, device);
                } else if(fileType == HotKnotFileType.VIDEO) {
                    title = mResource.getString(R.string.rx_group_ui_video_status, mInfo.mOrder, mInfo.mCount, device);
                } else if(fileType == HotKnotFileType.MUSIC) {
                    title = mResource.getString(R.string.rx_group_ui_music_status, mInfo.mOrder, mInfo.mCount, device);
                } else {
                    title = mResource.getString(R.string.rx_group_ui_status, mInfo.mOrder, mInfo.mCount, device);
                }
            } else {
                if(fileType == HotKnotFileType.IMAGE) {
    ...
    private boolean updateProgressBar() {
        ...
        synchronized(mItemLock) {
            ...
            if(mInfo.mState == State.RUNNING || mInfo.mState == State.CONNECTING) {
                ...
                int progress = (int) ((currentBytes * MAX_PROGRESS_VALUE / totalBytes));
                Log.d(TAG, "File progress:" + progress);
                mProgressBar.setProgress(progress);
                mHandler.sendEmptyMessageDelayed(MSG_POLLING, HotKnotBeamConstants.FILE_UI_PROGRESS_POLL);
                return true;
    ...
    private void showUiDialog(int id) {
        ...
        if(id == HOTKNOT_FAILED_DIALOG && mUiFailedDialog == null) {
            mUiFailedDialog = new AlertDialog.Builder(HotKnotBeamRxActivity.this, AlertDialog.THEME_HOLO_DARK).create();
            ...
            mUiFailedDialog.setButton(mResource.getString(android.R.string.ok), new DialogInterface.OnClickListener() {
                @Override
