        ...
        public boolean handleBackupFeatures(BaseCommand cmd) {
            ...
            if (cmd instanceof GetAllGroupsForBackupReq) {
                ...
                if (null == result) {
                    ...
                    rsp.setResults(result);
                    String[] newPaths = req.getNewPaths();
                    if (null == newPaths || newPaths.length == 0) {
                    	rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    } else {
	                    String path = new File(newPaths[0]).getParent();
	                    Uri data = Uri.parse("file://" + path);
	                    Debugger.logI("MediaFileRenameReq: data = " + data);
	                    MainService.this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, data));
                    }
                }
                // Send response command or append it to batch
                onRespond(rsp);
