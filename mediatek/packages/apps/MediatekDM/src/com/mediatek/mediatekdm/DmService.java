...
import com.mediatek.mediatekdm.DmOperation.Connection;
    ...
    private class DmServiceHandler extends Handler implements IOperationStateObserver {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case IServiceMessage.MSG_OPERATION_PROCESS_NEXT:
                    ...
                    // FIXME: check network availability
                    DmOperation nextOperation = mOperationManager.peekFirst();
                    ...
                    if (Connection.MOBILE.equals(nextOperation.getProperty(KEY.CONNECTION))) {
                        Log.d(TAG.SERVICE, " mIsStealDataConn next is : " + mNiaManager.mIsStealDataConn);
                        if (!DmDataConnection.getInstance(DmService.this).getMobileDataEnabled()) {
                            mNiaManager.mIsStealDataConn = true;
                            int ret = DmDataConnection.getInstance(DmService.this).setMobileDataEnabled();
                            Log.i(TAG.SERVICE, "Wap push message enable DM WAP conn...ret=" + ret);
                        }
                        if (!mDataConnection.isConnected()) {
                    ...
                    if (result == TriggerResult.BUSY) {
                        ...
                        break;
                    } /* else if (result == TriggerResult.NO_PENDING_OPERATION) this is processed already. */
        ...
        public void notify(State state, State previousState, Object extra) {
            if (state == State.IDLE) {
                ...
                DmOperation operation = (DmOperation) extra;
                String connection = operation.getProperty(KEY.CONNECTION, Connection.AUTO);
                if (connection.equals(Connection.MOBILE)) {
...
//            Utilities.removeDirectoryRecursively(getFilesDir());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (!DmApplication.getInstance().checkPrerequisites()) {
            Log.d(TAG.SERVICE, "checkPrerequisites fail");
            cutHP();
            return START_NOT_STICKY;
        }
        ...
        /*
         * Incoming intents can be organized in the following category:
         *  1. Initial Trigger: SI or CI actions issued either by DM server, user (via Settings) or device (polling
         *     & reboot check). This kind of intents will activate the network.
         *  2. Interactions with UI components: For example, alert or notification responses. This kind of intents 
         *     will not activate the network.
         *  3. Miscellaneous:
         */
        if (action.equals(IntentAction.DM_WAP_PUSH)) {
            ...
            if (type != null && type.equals(DmConst.IntentType.DM_NIA)) {
            	...
            	boolean isCMCC = intent.getBooleanExtra(DmConst.DM_NIA_SERVER.IS_CMCC_DMACC_SERVER, false);
                ...
                if (isCMCC) {
                	operation.setProperty(KEY.CONNECTION, Connection.MOBILE);
                	if (DmDataConnection.getInstance(this).getMobileDataEnabled()) {
                	    operation.setProperty(KEY.IS_STEAL_DATA_Connection, false);
                	} else {
                	    operation.setProperty(KEY.IS_STEAL_DATA_Connection, true);
                	}
                }
            ...
            if (intent.getBooleanExtra("response", false)) {
                ...
                mNiaManager.RecoverStealDataConnection();
            ...
            if (intent.getBooleanExtra("found", false)) {
                ...
                // Connect to network using configuration from operation.
            } else {
                if (!mOperationManager.isBusy() && !mOperationManager.hasNext()) {
            ...
            if (!accepted) {
                ...
                return START_STICKY;
            } // else: trigger required. dispatchCommand must enqueue an operation.
        }
        
        // Get the networking config for the current/upcoming operation
        DmOperation operation = DmOperationManager.getInstance().current();
        if (operation == null) {
        	operation = DmOperationManager.getInstance().peekFirst();
        }
        if (operation != null) {
            String connection = operation.getProperty(KEY.CONNECTION, Connection.AUTO);
            if (connection.equals(Connection.MOBILE)) {
                if (operation.getBooleanProperty(KEY.IS_STEAL_DATA_Connection, false)) {
                    if (!DmDataConnection.getInstance(this).getMobileDataEnabled()) {
                        mNiaManager.mIsStealDataConn = true;
                        int ret = DmDataConnection.getInstance(this).setMobileDataEnabled();
                        Log.i(TAG.SERVICE, "Wap push message enable DM WAP conn...ret=" + ret);
                    }
                    Log.d(TAG.SERVICE, "set  mIsStealDataConn is :" + mNiaManager.mIsStealDataConn);
                }
                try {
        	...
        	} else if (connection.equals(Connection.WIFI)) {
    ...
    public PLHttpConnection getHttpConnection() {
        DmOperation current = mOperationManager.current();
        for (IDmComponent component : DmApplication.getInstance().getComponents()) {
            PLHttpConnection connection = component.getHttpConnection(current);
            if (connection != null) {
                return connection;
            }
        }
