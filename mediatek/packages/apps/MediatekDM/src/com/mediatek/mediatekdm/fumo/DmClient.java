...
/**
 * UI for firmware update progress include server initiated and client initiated. This activity only can be triggered by
 * other components, e.g. DmService and DmEntry. The control logic of firmware update is implemented in the activity, too.
 * See See 5.1.5.4.3 in China Mobile Device Management Service Terminal Specification 3.0.0
 */
public class DmClient extends Activity {
    ...
    private void startToWork() {
        ...
        if (!DmConfig.getInstance().useMobileDataOnly() || DmOperationManager.getInstance().isInRecovery()) {
    ...
    /**
     * Show other error (error message except network error) UI and will query again if click retry button.
     */
     ...
     * 
     * @param DownloadDescriptor dd - Download descriptor of delta package to download.
     */
    ...
    private void onNewVersionDetected(DownloadDescriptor dd) {
        ...
        String description =
                mDlDescriptions.getText() + " " + mDd.getField(Field.DD_VERSION) + " (" + mDd.size + " Bytes)";
                    ...
                    if (connMgr != null) {
                        ...
                        if (net != null) {
                            ...
                            if (netType == ConnectivityManager.TYPE_MOBILE) {
                                ...
                                Log.e(TAG.CLIENT, "[ERROR] invalid network type: " + netType + "." + net.getTypeName());
                            }
                        } else {
    ...
    /**
     * Show downloading UI and will: 1. Pause download if click pause button; 2. Show confirm cancel download dialog if click
     * cancel button; 3. Downloading back ground if click back key.
     * 
    ...
    private void updateDlProgress(long dlSize, long totalSize) {
        ...
        CharSequence text =
                Integer.toString(ratial) + "%    " + Integer.toString((int) dlSize) + " Bytes / "
     ...
     * 
     * @param String [] updateTypes - string array which contains the text content of update types.
     */
    private void onShowUpdateList(String[] updateTypes) {
        ...
        mSingleChoiceList.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_list_item_single_choice,
                updateTypes));
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        switch (id) {
            ...
            case DIALOG_NOENOUGHSPACE:
                return DialogFactory.newAlert(this).setTitle(R.string.noenoughspace)
            ...
            case DIALOG_CANCELDOWNLOAD:
                return DialogFactory.newAlert(this).setTitle(R.string.cancel).setMessage(R.string.canceldownload)
                        .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int whichButton) {
                                // On user confirm cancel download
                                mFumo.clearDlStateAndReport(0, MdmFumoUpdateResult.ResultCode.USER_CANCELED.val);
            ...
            case DIALOG_GPRS_DOWNLOAD:
                return DialogFactory.newAlert(this).setTitle(R.string.gprs_download_title)
                        ...
                        .setPositiveButton(R.string.start_new_session, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int whichButton) {
                                showDownloadingUI(OP_START);
                            }
                        }).setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
    ...
    private void setNetworkTimeoutAlarm() {
        ...
        if (sAlarmManager != null) {
            sNetworkTimeoutIntent = PendingIntent.getBroadcast(this, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);
            sAlarmManager.cancel(sNetworkTimeoutIntent);
            sAlarmManager.set(AlarmManager.RTC_WAKEUP, (System.currentTimeMillis() + TIME_OUT_VALUE * ONE_SECOND),
    ...
    static class ApnConHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (state != DmFumoState.WAP_CONNECTING) {
                Log.w(TAG.CLIENT, "apnConnHandler state is not STATE_DETECT_WAP, the status = " + state);
                return;
            }
    ...
    private IDmFumoStateObserver mFumoObserver = new IDmFumoStateObserver() {
        @Override
        public void notify(final int newState, final int previousState, final DmOperation operation, final Object extra) {
            Log.d(TAG.FUMO, "DmClient notified with state " + newState + " and operation " + operation);
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    switch (newState) {
                        case DmFumoState.NEW_VERSION_FOUND:
                            if (mBinder != null) {
                                if (operation.getProperty(DmOperation.KEY.TYPE).equals(DmOperation.Type.TYPE_DL)
                                        && operation.getBooleanProperty(DmOperation.KEY.FUMO_TAG, false)) {
                                    if (!operation.getProperty(DmOperation.KEY.INITIATOR).equals("Server")) {
