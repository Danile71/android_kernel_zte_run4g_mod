...
/**
 * PlainBooleanHandler is an abstract NodeIoHandler implementation which provides writable control and a simple wrapper for
 * straightforward Boolean data access interface. Descendants of this class should override readValue and writeValue methods
 * to provide simple Boolean read/write function for PlainBooleanHandler. The interaction protocol (e.g. size inspection) is
 * taken care of by PlainBooleanHandler.
 * 
 * @author mtk81226
 */
public abstract class PlainBooleanHandler implements NodeIoHandler {
    ...
    public void write(int offset, byte[] data, int totalSize) throws MdmException {
        Log.d(TAG.NODEIOHANDLER,
                "+PlainBooleanHandler.write(" + mUri + ", " + offset + ", " + data + ", " + totalSize + ")");
        if (!mWritable) {
            ...
            if (offset == 0 && data.length == totalSize) {
                ...
                if (offset == 0 && data.length == 0 && totalSize != 0) {
                    ...
                    if (offset + data.length == totalSize) {
                        ...
                        writeValue((new String(mWriteCache, 0, totalSize)).equalsIgnoreCase(mTrueString));
                        mWriteCache = null;
                    }
                }
            }
