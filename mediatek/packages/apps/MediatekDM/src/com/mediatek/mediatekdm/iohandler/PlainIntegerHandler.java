...
/**
 * PlainIntegerHandler is an abstract NodeIoHandler implementation which provides writable control and
 * a simple wrapper for straightforward Integer data access interface. Descendants of this class should
 * override readValue and writeValue methods to provide simple Integer read/write function for PlainIntegerHandler.
 * The interaction protocol (e.g. size inspection) is taken care of by PlainIntegerHandler.
 * @author mtk81226
 *
 */
public abstract class PlainIntegerHandler implements NodeIoHandler {
    ...
    public void write(int offset, byte[] data, int totalSize) throws MdmException {
        Log.d(TAG.NODEIOHANDLER,
                "+PlainIntegerHandler.write(" + mUri + ", " + offset + ", " + data + ", " + totalSize + ")");
        if (!mWritable) {
