...
public class DmLawmoHandler implements LawmoHandler {
    ...
    public DmLawmoHandler(DmService service) {
        mService = service;
        mDmAgent = PlatformManager.getInstance().getDmAgent();
        mDmTree = new MdmTree();
    }
    ...
    public LawmoOperationResult executeFactoryReset() {
        ...
        try {
            LawmoComponent component = (LawmoComponent) DmApplication.getInstance().findComponentByName(LawmoComponent.NAME);
            component.getLawmoManager().setPendingAction(LawmoAction.FACTORY_RESET_EXECUTED);
            mDmAgent.setWipeFlag();
            File wipeFile = new File(Path.getPathInData(mService, Path.WIPE_FILE));
    ...
    public LawmoOperationResult executePartiallyLock() {
        ...
        try {
            ...
            mDmTree.replaceIntValue(MdmTree.makeUri(LawmoComponent.ROOT_URI, STATE_URI), LawmoState.PARTIALLY_LOCKED.val);
            mDmTree.writeToPersistentStorage();
            Log.i(TAG.LAWMO, "After write, lawmo staus is " + mDmTree.getIntValue(MdmTree.makeUri(LawmoComponent.ROOT_URI, STATE_URI)));
    ...
    public LawmoOperationResult executeUnLock() {
        ...
        try {
            ...
            // 3. Update lawmo state & return result
            mDmTree.replaceIntValue(MdmTree.makeUri(LawmoComponent.ROOT_URI, STATE_URI), LawmoState.UNLOCKED.val);
            mDmTree.writeToPersistentStorage();
            Log.i(TAG.LAWMO, "After write, lawmo staus is " + mDmTree.getIntValue(MdmTree.makeUri(LawmoComponent.ROOT_URI, STATE_URI)));
