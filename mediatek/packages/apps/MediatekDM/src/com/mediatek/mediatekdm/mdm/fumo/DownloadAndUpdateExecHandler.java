...
class DownloadAndUpdateExecHandler implements NodeExecuteHandler {
    ...
    public int execute(byte[] data, String correlator) throws MdmException {
        MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[FUMO] +DownloadAndUpdateExecHandler.execute()");
        ...
        SessionInitiator initiator = new SimpleSessionInitiator(MdmFumo.SESSION_INITIATOR_DL + "|" + "DownloadAndUpdate");
        mFumo.mPendingSession.add(initiator);
        engine.triggerDLSession(
                tree.getStringValue(MdmTree.makeUri(mFumo.getRootUri(), MdmFumo.Uri.DOWNLOADANDUPDATE_PKGURL)),
                new FumoDownloadPromptHandler(mFumo),
                initiator);
        // Set action bits.
        mFumo.getEngine().setSessionAction(MdmFumo.SESSION_ACTION_KEY, FumoAction.DOWNLOAD_AND_UPDATE_EXECUTED);
        MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[FUMO] -DownloadAndUpdateExecHandler.execute()");
