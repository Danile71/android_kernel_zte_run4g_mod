...
class DownloadExecHandler implements NodeExecuteHandler {
    ...
    public int execute(byte[] data, String correlator) throws MdmException {
        ...
        // Save uri
        mFumo.getPLRegistry()
                .setStringValue(MdmFumo.Registry.EXEC_URI, MdmTree.makeUri(mFumo.getRootUri(), MdmFumo.Uri.DOWNLOAD));
        // Enqueue a DL session to engine.
        MdmEngine engine = mFumo.getEngine();
        MdmTree tree = mFumo.getTree();
        SessionInitiator initiator = new SimpleSessionInitiator(MdmFumo.SESSION_INITIATOR_DL + "|" + "Download");
        mFumo.mPendingSession.add(initiator);
        engine.triggerDLSession(tree.getStringValue(MdmTree.makeUri(mFumo.getRootUri(), MdmFumo.Uri.DOWNLOAD_PKGURL)),
