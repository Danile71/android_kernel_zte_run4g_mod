...
class FumoSSO implements SessionStateObserver {
    ...
    public void notify(SessionType type, SessionState state, int lastError, SessionInitiator initiator) {
        MdmEngine.getLogger().logMsg(
                MdmLogLevel.DEBUG,
                "[FUMO] +FumoSSO.notify(" + type + ", " + state + ", " + lastError + ", " + initiator.getId() + ")");
        ...
        if (type == SessionType.DM && state == SessionState.ABORTED) {
            for (SessionInitiator si : mFumo.mPendingSession) {
                MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "Remove fumo pending session " + si.getId());
                mFumo.getEngine().removePendingTrigger(si);
            }
        }
        if (initiator.getId().startsWith(MdmFumo.SESSION_INITIATOR_PREFIX)) {
            if (state == SessionState.STARTED) {
                MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "Pending session started, remove it from pending list.");
            ...
            switch (type) {
                ...
                case DL:
                    try {
                        if (state == SessionState.STARTED) {
                            MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[FUMO] DL session started");
                            mFumo.getPLRegistry().setStringValue(MdmFumo.Registry.DL_LAST_SESSION_ID, initiator.getId());
                        } else if (state == SessionState.COMPLETE) {
                            MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[FUMO] DL session completed");
                    ...
                    break;
                default:
                    MdmEngine.getLogger().logMsg(MdmLogLevel.WARNING, "Unsupported session type " + type.toString());
                    break;
            }
        }
