...
public class MdmLawmo {
    ...
    public void setAvailableWipeList(String listItemName, boolean isToBeWiped) throws MdmException {
        logMsg(MdmLogLevel.INFO, "setAvailableWipeList..list item name:" + listItemName + ", wiped: " + isToBeWiped);
    }
    ...
    private void registerLawmoHandler() throws MdmException {
        mTree.registerExecute(MdmTree.makeUri(getPathOperations(), Uri.FACTORYRESET), new NodeExecuteHandler() {
            public int execute(byte[] data, String correlator) throws MdmException {
                mEngine.setSessionAction(SESSION_ACTION_KEY, LawmoAction.FACTORY_RESET_EXECUTED);
                saveExecInfo(correlator, Uri.FACTORYRESET);
                return valueOfResult(mHandler.executeFactoryReset());
            }
        });

        mTree.registerExecute(MdmTree.makeUri(getPathOperations(), Uri.FULLYLOCK), new NodeExecuteHandler() {
            public int execute(byte[] data, String correlator) throws MdmException {
                mEngine.setSessionAction(SESSION_ACTION_KEY, LawmoAction.FULLY_LOCK_EXECUTED);
                saveExecInfo(correlator, Uri.FULLYLOCK);
                return valueOfResult(mHandler.executeFullyLock());
            }
        });

        mTree.registerExecute(MdmTree.makeUri(getPathOperations(), Uri.PARTIALLYLOCK), new NodeExecuteHandler() {
            public int execute(byte[] data, String correlator) throws MdmException {
                mEngine.setSessionAction(SESSION_ACTION_KEY, LawmoAction.PARTIALLY_LOCK_EXECUTED);
                saveExecInfo(correlator, Uri.PARTIALLYLOCK);
                return valueOfResult(mHandler.executePartiallyLock());
            }
        });

        mTree.registerExecute(MdmTree.makeUri(getPathOperations(), Uri.UNLOCK), new NodeExecuteHandler() {
            public int execute(byte[] data, String correlator) throws MdmException {
        ...
        mTree.registerExecute(MdmTree.makeUri(getPathOperations(), Uri.WIPE), new NodeExecuteHandler() {
            public int execute(byte[] data, String correlator) throws MdmException {
    ...
    private void saveExecInfo(String correlator, String leafnode) throws MdmException {
        ...
        mPLRegistry.setStringValue(Registry.EXEC_URI, MdmTree.makeUri(getPathOperations(), leafnode));
    }
