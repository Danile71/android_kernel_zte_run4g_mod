...
public class DPDataAddDelHandler implements NodeOnAddHandler, NodeOnDeleteHandler {
    ...
    public void onAdd(String uri) {
        try {
            mScomo.getTree().registerNodeIoHandler(MdmTree.makeUri(mDp.getDeliveredUri(), Uri.DATA),
    ...
    public void onDelete(String uri) {
        try {
            mScomo.getTree().unregisterNodeIoHandler(MdmTree.makeUri(mDp.getDeliveredUri(), Uri.DATA));
        } catch (MdmException e) {
            e.printStackTrace();
        }
    }
