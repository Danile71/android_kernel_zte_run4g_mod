...
public class DPDeliveredSubtreeAddHandler implements NodeOnAddHandler {
    ...
    public void onAdd(String uri) {
        ...
        String deliveredRoot = MdmTree.makeUri(mScomo.getRootUri(), Uri.INVENTORY, Uri.DELIVERED) + MdmTree.URI_SEPERATOR;
        ...
        if (uri.startsWith(deliveredRoot)) {
            ...
            if (index == -1 || index == trailing.length() - 1) {
                ...
                try {
                    tree.registerOnAddHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.INSTALL), opHandler);
                    tree.registerOnAddHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.INSTALLINACTIVE), opHandler);
                    tree.registerOnAddHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.REMOVE), opHandler);
                    tree.registerOnAddHandler(MdmTree.makeUri(uri, Uri.DATA), dataHandler);
                    tree.registerOnDeleteHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.INSTALL), opHandler);
                    tree.registerOnDeleteHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.INSTALLINACTIVE), opHandler);
                    tree.registerOnDeleteHandler(MdmTree.makeUri(uri, Uri.OPERATIONS, Uri.REMOVE), opHandler);
