...
public class DPInstallExecHandler extends ScomoExecHandler implements NodeExecuteHandler {
    ...
    public int execute(byte[] data, String correlator) throws MdmException {
        ...
        // Set action bits.
        mScomo.getEngine().setSessionAction(MdmScomo.SESSION_ACTION_KEY, ScomoAction.INSTALL_EXECUTED);
        ...
        if (handler != null) {
            if (handler.confirmInstall(mDp)) {
                ...
                ScomoOperationResult result = handler.executeInstall(mDp, mDp.getDeliveryPkgPath(), true);
                if (result.mAsync) {
                    mScomo.logMsg(MdmLogLevel.DEBUG, "Async installation");
                    saveExecInfo(correlator, MdmTree.makeUri(mDp.getDeliveredUri(), Uri.OPERATIONS, Uri.INSTALL));
                    mScomo.logMsg(MdmLogLevel.DEBUG, "-DPInstallExecHandler.execute()");
                    return 0;
                } else {
