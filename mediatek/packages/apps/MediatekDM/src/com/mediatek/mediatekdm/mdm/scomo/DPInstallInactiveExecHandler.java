...
public class DPInstallInactiveExecHandler extends ScomoExecHandler implements NodeExecuteHandler {
    private MdmScomoDp mDp;

    public DPInstallInactiveExecHandler(MdmScomo scomo, MdmScomoDp dp) {
        super(scomo);
        mDp = dp;
    }

    public int execute(byte[] data, String correlator) throws MdmException {
        mScomo.logMsg(MdmLogLevel.DEBUG, "+DPInstallInactiveExecHandler.execute()");
        /* data is ignored */
        MdmScomoDpHandler handler = mDp.getHandler();

        // Set action bits.
        mScomo.getEngine().setSessionAction(MdmScomo.SESSION_ACTION_KEY, ScomoAction.INSTALL_INACTIVE_EXECUTED);

        if (handler != null) {
            if (handler.confirmInstall(mDp)) {
                mScomo.logMsg(MdmLogLevel.DEBUG, "Install proceeded");
                mDp.setInstallStatus(InstallStatus.STARTED);
                mScomo.getTree().replaceIntValue(MdmTree.makeUri(mDp.getDeliveredUri(), Uri.STATUS),
                        InventoryDeliveredStatus.INSTALL_PROGRESSING.val);
                ScomoOperationResult result = handler.executeInstall(mDp, mDp.getDeliveryPkgPath(), false);
                if (result.mAsync) {
                    mScomo.logMsg(MdmLogLevel.DEBUG, "Async installation");
                    saveExecInfo(correlator, MdmTree.makeUri(mDp.getDeliveredUri(), Uri.OPERATIONS, Uri.INSTALLINACTIVE));
                    mScomo.logMsg(MdmLogLevel.DEBUG, "-DPInstallInactiveExecHandler.execute()");
                    return 0;
                } else {
                    ...
                    if (result.mResult.val != MdmScomoResult.SUCCESSFUL) {
                        ...
                        mScomo.getTree().replaceIntValue(MdmTree.makeUri(mDp.getDeliveredUri(), Uri.STATE),
