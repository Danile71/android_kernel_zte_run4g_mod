...
public class MdmScomoDc {
    ...
    public static enum DeployStatus {
        INVALID,
        ACTIVATION_DONE,
        ACTIVATION_START_POSTPONED,
        ACTIVATION_STARTED,
        DEACTIVATION_DONE,
        DEACTIVATION_STARTED,
        DEACTIVATION_START_POSTPONED,
        REMOVAL_DONE,
        REMOVAL_STARTED,
        REMOVAL_START_POSTPONED,
    }
    ...
    public void addToInventory(String id,
            ...
            String description,
            String envType,
            boolean isActive) throws MdmException {
        ...
        tree.addChildLeafNode(uri, Uri.PKGIDREF, "chr", null, (pkgId == null ? empty : pkgId.getBytes()));
        tree.addChildLeafNode(uri, Uri.NAME, "chr", null, (name == null ? empty : name.getBytes()));
        tree.addChildLeafNode(uri, Uri.DESCRIPTION, "chr", null, (description == null ? empty : description.getBytes()));
        tree.addChildLeafNode(uri, Uri.VERSION, "chr", null, (version == null ? empty : version.getBytes()));
        ...
        tree.addChildLeafNode(uri, Uri.ENVTYPE, "chr", null, (envType == null ? empty : envType.getBytes()));
        ...
        tree.registerExecute(MdmTree.makeUri(uri, Uri.ACTIVATE), new DCActivateExecHandler(mScomo, this));
        tree.registerExecute(MdmTree.makeUri(uri, Uri.DEACTIVATE), new DCDeactivateExecHandler(mScomo, this));
        tree.registerExecute(MdmTree.makeUri(uri, Uri.REMOVE), new DCRemoveExecHandler(mScomo, this));
        mScomo.logMsg(MdmLogLevel.DEBUG, "-MdmScomoDc.addToInventory");
    }
    ...
    /**
     * Trigger report session (generic alert). Should be called by the client application after remove, activate, deactivate,
     * were executed. The report will send the result set by setDeploymentResult(MdmScomoResult)
     * 
     * @throws MdmException
     */
    public void triggerReportSession() throws MdmException {
        String data = "<ResultCode>" + mDeploymentResult.val + "</ResultCode><Identifier>" + getId() + "</Identifier>";
     ...
     *            range: 1250-1299: Successful - Vendor Specified. 1450-1499: Client Error - Vendor Specified.
     * @throws MdmException MEMORY if not enough memory for the action, or BAD_INPUT if invalid input.
     */
    ...
    /**
     * Execute uninstallation. Called by the client to execute the uninstallation of a a deployment component that is
     * available on the device. (Asynchronous uninstallation)
     * 
     * @throws MdmException
     */
    ...
    /**
     * Execute activation. Called by the client to execute the activation of a a deployment component that is available on
     * the device. (Asynchronous activation)
     * 
     * @throws MdmException
     */
    ...
    /**
     * Execute deactivation. Called by the client to execute the deactivation of a a deployment component that is available
     * on the device. (Asynchronous deactivation)
     * 
     * @throws MdmException
     */
    ...
    public void deleteFromInventory() throws MdmException {
        ...
        try {
            MdmTree tree = mScomo.getTree();
            String dcName = tree.findFirstNodeByName(MdmTree.makeUri(
                    mScomo.getRootUri(), Uri.INVENTORY, Uri.DEPLOYED), mName);
