...
public class MdmScomoDp {
     ...
     * 
     * @param dpName SCOMO DP name which is also the node name under Delivered.
     * @param handler SCOMO DP handler.
     * @param scomo SCOMO instance.
     */
     ...
     * 
     * @param handler MdmScomoDpHandler instance.
     */
    ...
    /**
     * Destroy a SCOMO DP Instance. You must call this method to allow graceful cleanup of this SCOMO DP instance.
     */
    public void destroy() {
    ...
    /**
     * Execute installation. Called by the client to execute the installation of a a delivery package that is available on
     * the device. (Asynchronous installation)
     * 
     * @throws MdmException
     */
    ...
    void executeInstall(boolean active) throws MdmException {
        if (mHandler != null) {
            ...
            ScomoOperationResult result = mHandler.executeInstall(this, mScomo.getRootUri(), active);
     ...
     * @throws MdmException MEMORY if not enough memory for the action, TREE_ACCESS_DENIED if access denied NODE_MISSING if
     *             PkgName node is missing from the DM tree.
     */
    ...
    public String getDescription() throws MdmException {
        return mScomo.getTree().getStringValue(
                MdmTree.makeUri((mDelivered ? getDeliveredUri() : getDownloadUri()), Uri.DESCRIPTION));
    }
    ...
    public String getInstallParams() throws MdmException {
        return mScomo.getTree().getStringValue(
                MdmTree.makeUri((mDelivered ? getDeliveredUri() : getDownloadUri()), Uri.INSTALLPARAMS));
    }
    ...
    public String getDeliveryPkgPath() throws MdmException {
        ...
        if (mAlternativeDownload) {
            return mScomo.getPLDlPkg().getFilename(engine.getPLRegistry().getStringValue(
                    Registry.makePath(Registry.ROOT, Registry.PACKAGE, mName, Registry.PATH)));
        } else {
    ...
    /**
     * Trigger report session (generic alert). Should be called by the client application after the installer has executed
     * the installation or upon a failure in the steps of executing a SCOMO DP session (e.g. upon download failure, user
     * abort, etc.)
     * 
     * @param resultCode Result code to be sent to the server. Should be one of VdmScomoResult values or in vendor-specific
     *            range: 1250-1299: Successful - Vendor Specified. 1450-1499: Client Error - Vendor Specified. 1550-1599: DL
     *            Server Error - Vendor Specified.
     * @throws MdmException
     */
    public void triggerReportSession(MdmScomoResult resultCode) throws MdmException {
        String data = "<ResultCode>" + resultCode.val + "</ResultCode><Identifier>" + getId() + "</Identifier>";
        MdmEngine engine = mScomo.getEngine();
        ...
        // NOTE We only use the first correlator as this interface can only report one result. the correlator information are
        ...
        if (correlators != null && correlators.length() != 0) {
            ...
            if (correlatorList.length > 1) {
                registry.setStringValue(correlatorsPath, correlators.substring(correlator.length() + 1));
            } else {
        ...
        String source = registry.getStringValue(
                Registry.makePath(Registry.ROOT, "exec", account, correlator, "source"));
        if (source == null || source.length() == 0) {
            throw new MdmException(MdmError.INTERNAL);
        }
        mScomo.getEngine().triggerReportSession(
                correlator,
                "text/plain",
                mScomo.getAlertType(),
                "informational",
        ...
        engine.getPLRegistry().deleteKeysByPrefix(Registry.makePath(Registry.ROOT, "exec", account, correlator));
    }
     ...
     * 
     * @throws MdmException INVALID_CALL if SCOMO DP is not is in a middle of a download session, MEMORY if not enough memory
     *             for the action, MO_STORAGE if there was an error accessing MO external storage, or TREE_ACCESS_DENIED if
     *             access denied.
     */
    public void resumeDLSession() throws MdmException {
        ...
        engine.triggerDLSession(
                ...
                new DPDownloadPromptHandler(mScomo, this),
