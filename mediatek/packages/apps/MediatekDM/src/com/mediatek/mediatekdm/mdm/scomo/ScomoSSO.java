...
public class ScomoSSO implements SessionStateObserver {
    ...
    public void notify(SessionType type, SessionState state, int lastError, SessionInitiator initiator) {
        ...
        // If the first DM session failed, than we will not trigger the successive DL session. Only DL session
        ...
        if (initiator.getId().startsWith(Initiator.PREFIX)) {
            if (state == SessionState.STARTED) {
                mScomo.logMsg(MdmLogLevel.DEBUG, "Pending session started, remove it from pending list.");
            ...
            switch (type) {
                case DM:
                    break;
                case DL:
                    String[] initiatorComponents = initiator.getId().split(Initiator.SEPERATOR_REGEXP);
                    ...
                    try {
                        if (state == SessionState.STARTED) {
                            mScomo.logMsg(MdmLogLevel.DEBUG, "DL session for DP " + dpName + " started");
                            mScomo.getTree().replaceIntValue(MdmTree.makeUri(dp.getDownloadUri(), Uri.STATUS),
                                    DownloadStatus.DOWNLOAD_PROGRESSING.val);
                            mScomo.getPLRegistry().setStringValue(MdmScomo.Registry.DL_LAST_SESSION_ID, initiator.getId());
                        } else if (state == SessionState.COMPLETE) {
                            mScomo.logMsg(MdmLogLevel.DEBUG, "DL session for DP " + dpName + " completed");
                            mScomo.getTree().replaceIntValue(MdmTree.makeUri(dp.getDownloadUri(), Uri.STATUS),
                            ...
                            if (command.equals(Uri.DOWNLOAD)) {
                                ...
                                if (handler != null) {
                                    if (handler.confirmInstall(dp)) {
                                        ...
                                        ScomoOperationResult result = handler.executeInstall(
                                                dp,
                                        ...
                                        if (!result.mAsync) {
                                            ...
                                            mScomo.logMsg(MdmLogLevel.DEBUG, "Install asynchronously");
                                        }
                                    } else {
                                        dp.setInstallActive(command.equals(Uri.DOWNLOADINSTALL));
                                        mScomo.logMsg(MdmLogLevel.DEBUG, "Install for DP " + dpName + " postponed");
                                    }
                                } else {
                                    dp.triggerReportSession(new MdmScomoResult(MdmScomoResult.NOT_IMPLEMENTED));
                                }
                            }
                        } else if (state == SessionState.ABORTED) {
                            mScomo.logMsg(MdmLogLevel.DEBUG,
                                    "DL session for DP " + dpName + " aborted with error " + lastError);
                            mScomo.getTree().replaceIntValue(MdmTree.makeUri(dp.getDownloadUri(), Uri.STATUS),
                    ...
                    break;
                default:
                    mScomo.logMsg(MdmLogLevel.WARNING, "Unsupported session type " + type.toString());
                    break;
            }
