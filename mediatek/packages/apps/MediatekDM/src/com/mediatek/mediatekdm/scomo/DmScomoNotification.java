...
public class DmScomoNotification implements IDmScomoStateObserver, IDmScomoDownloadProgressObserver {
    ...
    public DmScomoNotification(DmService service) {
        ...
        mDownloadDetailPendingIntent = PendingIntent.getActivity(service, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
    }

    @SuppressWarnings("deprecation")
    private void notifyDownloadNotification(
        ...
        Notification.Builder notifyBuilder = new Notification.Builder(mService).setOngoing(true)
                ...
                .setContentTitle(contentTitle).setContentText(contentText).setProgress(100, progress, false);
        mNotificationManager.notify(NotificationInteractionType.TYPE_SCOMO_NOTIFICATION, notifyBuilder.getNotification());
    }

    @SuppressWarnings("deprecation")
    private void notifyConfirmNotification(
            CharSequence tickerText,
            CharSequence contentTitle,
            CharSequence contentText,
            int state,
        ...
        PendingIntent pendingIntent = PendingIntent.getActivity(mService, 0, intent, PendingIntent.FLAG_ONE_SHOT);
    ...
    @SuppressWarnings("deprecation")
    @Override /* IDmScomoStateObserver */
    public void notify(int state, int previousState, DmOperation operation, Object extra) {
        ScomoComponent component = (ScomoComponent) DmApplication.getInstance().findComponentByName(ScomoComponent.NAME);
        ...
        if (state == DmScomoState.DOWNLOADING) {
            ...
            CharSequence tvText = mService.getResources().getString(R.string.downloading, scomoState.getName());
            notifyDownloadNotification(tickerText, text, tvText, scomoState.currentSize, scomoState.totalSize);
        } else if (state == DmScomoState.DOWNLOAD_PAUSED) {
            Log.d(TAG.SCOMO, "SCOMO Notificaition notified with state " + state + " and extra " + extra);
            if (extra != null && ((String) extra).equals("USER_CANCELED")) {
                ...
                CharSequence contentText = scomoState.currentSize / 1024 + "KB/" + scomoState.totalSize / 1024 + "KB";
                CharSequence contentTitle = mService.getText(R.string.scomo_downloading_paused);
                notifyDownloadNotification(
                        tickerText,
                        contentTitle,
                        contentText,
            ...
            Notification.Builder notifyBuilder = new Notification.Builder(mService)
                    ...
                    .setContentTitle(
                            mService.getText(R.string.installing_scomo))
                    .setContentText(
                            mService.getResources().getString(R.string.installing,
                                    scomoState.getName()));
            mNotificationManager.notify(
                    NotificationInteractionType.TYPE_SCOMO_NOTIFICATION,
                    notifyBuilder.getNotification());
        }
    }

    @Override /* IDmScomoDownloadProgressObserver */
