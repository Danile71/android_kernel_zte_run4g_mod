...
public class DmScomoState implements Serializable {
    ...
    // There is no DP to download. SI session committed no SCOMO download action or SI session failed.
    ...
    /**
     * verbose is used by SCOMO listeners to decide whether to interact with user. verbose will be set in MmiConfirmation
     * (ALERT 1101), and be reset after DM session.
     */
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (!dpName.equals("")) {
            try {
                currentDp = MdmScomo.getInstance(ScomoComponent.ROOT_URI, null).createDP(dpName, mDpHandler);
    ...
    public static void store(Context context, DmScomoState state) {
        ...
        PlatformManager.getInstance().atomicWrite(context.getFileStreamPath(SCOMO_STATE_FILE), state);
    }
