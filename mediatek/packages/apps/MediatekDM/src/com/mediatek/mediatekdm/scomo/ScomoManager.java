...
public class ScomoManager extends SessionHandler implements MdmScomoHandler, MdmScomoDpHandler,
    ...
    public ScomoManager(DmService service) {
        ...
        try {
            ...
            // FIXME This will overwrite the DP loaded when ScomoState is initialized. Is this correct?
            ArrayList<MdmScomoDp> dps = MdmScomo.getInstance(ScomoComponent.ROOT_URI, this).getDps();
            if (dps != null && dps.size() != 0) {
                mScomoState.currentDp = dps.get(0);
            }
            Log.w(TAG.SCOMO, "Overwrite currentDp with " + mScomoState.currentDp.getName() + "retrieved from engine.");
            ...
            public void run() {
                try {
                    if (lastDp != null) {
                        ...
                        reportOperation.initReportScomo(
                                result.val,
    ...
    public ScomoOperationResult executeInstall(final MdmScomoDp dp, String deliveryPkgPath, boolean isActive) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                        ...
                        new DmScomoPackageManager.ScomoPackageInstallObserver() {
                            public void packageInstalled(final String pkgName, final int status) {
                                mHandler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        Log.e(TAG.SCOMO, "Package " + pkgName + " installed with status: " + status);
                                        if (status == DmScomoPackageManager.STATUS_OK) {
                                            try {
                                                String dpId = dp.getId();
                                                MdmScomo scomo = MdmScomo.getInstance(
                                                        ScomoComponent.ROOT_URI,
                                                        ScomoManager.this);
                                                MdmScomoDc dc = scomo.createDC(pkgName, ScomoManager.this);
                                                dc.deleteFromInventory();
                                                dc.destroy();
                                                dc = scomo.createDC(pkgName, ScomoManager.this);
                                                PackageManager pm = ScomoManager.this.mService.getPackageManager();
                                                String versionString = null;
                                                try {
                                                    versionString = pm.getPackageInfo(pkgName, 0).versionName;
                                                } catch (NameNotFoundException e) {
                                                    Log.e(TAG.SCOMO, "No package found! Use null as version string");
                                                }
                                                dc.addToInventory(
                                                        pkgName,
                                                        pkgName,
    ...
    protected void dmComplete() {
        ...
        for (InteractionResponse ir : interactions) {
            if (ir.type == InteractionType.CONFIRMATION && ir.response == InteractionResponse.POSITIVE) {
        ...
        if ((scomoAction & ScomoAction.DOWNLOAD_EXECUTED) != 0 ||
            ...
            clearFiles();
            // Cut in line
            DmOperation dlOperation = new DmOperation(DmOperation.generateId(), DL_TIME_OUT, DL_MAX_RETRY);
    ...
    protected void dlStart() {
        ...
        if (mScomoState.state == DmScomoState.DOWNLOAD_PAUSED || mScomoState.state == DmScomoState.DOWNLOADING) {
    ...
    public void dlComplete() {
        // Override super.dlComplete() to finish current operation later (after installation is finished).
    }
    ...
    public void cancelDlPkg() {
        ...
        if (mOperationManager.isInRecovery()) {
            ...
            if (operation.getProperty(KEY.TYPE).equals(Type.TYPE_DL) &&
                ...
                setScomoState(DmScomoState.DOWNLOAD_PAUSED, mOperationManager.current(), "USER_CANCELED");
                mOperationManager.finishCurrent();
            } else {
    ...
    private void scomoScanPackageInternal() {
        ...
        try {
            ...
            for (String pkgName : appNamesTmp) {
                ...
                MdmScomoDc dc = scomoInstance.createDC(pkgName, this);
                dc.addToInventory(pkgName, pkgName, null, appVersions.get(pkgName), null, null, true);
            }
            new MdmTree().writeToPersistentStorage();
        } catch (MdmException e) {
            e.printStackTrace();
    ...
    public void reportResult(DmOperation operation) {
        if (operation.getProperty(KEY.TYPE).equals(Type.TYPE_REPORT_SCOMO)) {
            ...
            mService.getController().triggerReportSession(
                    ...
                    operation.getProperty("report_mark"),
                    operation.getProperty("report_source"),
                    operation.getProperty("report_target"),
