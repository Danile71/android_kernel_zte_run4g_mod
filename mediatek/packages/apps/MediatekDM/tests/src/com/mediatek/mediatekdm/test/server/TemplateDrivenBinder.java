...
public class TemplateDrivenBinder extends IMockServer.Stub {
    ...
    protected class MockServerThread extends Thread implements TemplateHttpRequestHandler.IControlPanel {
        ...
        public MockServerThread() {
            super();
            Log.d(TAG, "MockServerThread(" + android.os.Process.myPid() + ", " + android.os.Process.myTid() + ")");
            mRequestExit = false;
            try {
                ...
                mBreakPoint = mParameters.containsKey(KEY_BREAKPOINT) ? Integer.parseInt(mParameters.get(KEY_BREAKPOINT)) : -1;
                mBreakTime = mParameters.containsKey(KEY_BREAKTIME) ? Integer.parseInt(mParameters.get(KEY_BREAKTIME)) : -1;
            } catch (IOException e) {
                throw new Error(e);
            }
        }
        ...
        @Override
        public void run() {
            Log.d(TAG, "MockServerThread.run(" + android.os.Process.myPid() + ", " + android.os.Process.myTid() + ")");
            ...
            while (!Thread.interrupted() && mTestResult == TestResult.Unknown) {
                ...
                try {
                    ...
                    parameters.setBooleanParameter(CoreConnectionPNames.STALE_CONNECTION_CHECK, false);
                    ...
                    registry.register(
                            "*",
                            new TemplateHttpRequestHandler(
                                    mParameters.get(KEY_TYPE),
    ...
    public int getResult() throws RemoteException {
        Log.d(TAG, "TemplateDrivenBinder.getResult(" + android.os.Process.myPid() + ", " + android.os.Process.myTid()
                + ")");
