...
public class ImportContactsActivity extends OOBEStepPreferenceActivity 
    ...
    private boolean mImportEnable = false;
    ...
    */
    ...
    /*
    * update sim info preference to the screen
    */
    private void updateSimInfoPreference() {
        Xlog.d(TAG, "updateSimInfoPreference: " );
        ...
        if (screen == null ) {
            Xlog.d(TAG, "screen == null ");
            return;
        } 
        Xlog.d(TAG, "screen.removeAll()");
        screen.removeAll();
    ...
    */
    private void updateSimPrefStatus() {
        Xlog.d(TAG, "updateSimPrefStatus: " );
        ...
        if (simList != null && mSimInfoPreMap.size() > 0) {
            ...
            for (SimInfoRecord simInfo : simList) {
                ...
                if (p != null && simInfo.mSimInfoId == p.getSimId()) {
                    //p.setStatus(getSimState(simInfo.mSimSlotId));                                       
                    phbReady = isPhoneBookReady((int)simInfo.mSimSlotId);                            
                    p.setCheck(phbReady);
                    p.setEnabled(phbReady);  
                    if (phbReady) {
                        mImportEnable = true;
                    }    
                    Xlog.d(TAG, "phbReady:" + phbReady + " mImportEnable:" + mImportEnable);                
                }
            }
            mImportBtn.setEnabled(mImportEnable);
        }
    }
    ...
    */
    ...
    /*
    * get the slot sim contact status:  ready or not
    * 
    */
    private  boolean isPhoneBookReady(int slotId) {

        if (null == mITelephonyEx) {
            Xlog.d(TAG, "[isPhoneBookReady]mITelephonyEx == null");
            return false;
        }
        
        boolean isPbReady = false;
        try {
            isPbReady = mITelephonyEx.isPhbReady(slotId);
        } catch (RemoteException e) {
            Xlog.d(TAG, "[isPhoneBookReady]catch exception:");
        }
        
        Xlog.d(TAG, "[isPhoneBookReady]slotId:" + slotId + ",isPbReady:" + isPbReady);
        return isPbReady;
    }
