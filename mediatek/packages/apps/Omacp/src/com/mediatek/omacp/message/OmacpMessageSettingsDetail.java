...
public class OmacpMessageSettingsDetail extends Activity implements OnClickListener {
    ...
    private static final int NETWPIN = 0;

    private static final int USERPIN = 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail onCreate savedInstanceState = "
    ...
    protected void onDestroy() {
        MTKlog.d(TAG, "OmacpMessageSettingsDetail onDestroy");
    ...
    private void initActivityState(Intent intent) {
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail initActivityState intentData is : "
                    + intentData);
        ...
        if (intentData != null) {
            ...
            MTKlog.d(TAG, "OmacpMessageSettingsDetail mApList is : " + mApList);
            MTKlog.d(TAG, "OmacpMessageSettingsDetail mNapList is : " + mNapList);
            MTKlog.d(TAG, "OmacpMessageSettingsDetail mPxList is : " + mPxList);
    ...
    private void sendIntentsToApn() {
        ...
        if (!apnIntentList.isEmpty()) {
            ...
            MTKlog.d(TAG, "OmacpMessageSettingsDetail sendBroadcast apn intent is : " + intent);
                ...
                if (application.mToProxy != null) {
                    for (int proxyIndex = 0; proxyIndex < application.mToProxy.size(); proxyIndex++) {
                        if (application.mToProxy.get(proxyIndex).equalsIgnoreCase(proxy)) {
                            MTKlog.d(TAG, "application's TO-PROXY match. application index is : "
                                    + apIndex);
                            needModify = true;
                            break;
                        }
    ...
    private void checkPxNapId(NapdefClass napDef, Intent intent) {
        ...
        if (mPxList == null) {
            MTKlog.e(TAG, "OmacpMessageSettingsDetail mPxList is null ");
            return;
        }
        boolean flag = false;
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail checkPxNapId flag is : " + flag);
        // add px parameters
    ...
    private boolean checkFlagByNapId(ArrayList<String> toNapIdList, NapdefClass napDef) {
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail checkFlagByNapId flag is : " + flag);
        return flag;
    }
    ...
    private boolean addValidApplication(ApplicationClass application,
        ...
        MTKlog.d(TAG, "addValidApplication  isInclueded : " + isInclueded);
    ...
    private void addEmailToResultList(Intent intent, ApplicationClass application,
        ...
        if (application.mAppid.equalsIgnoreCase(OmacpMessageUtils.SMTP_APPID)
            ...
            if (checkIfApplicationSupport(application.mAppid)) {
                MTKlog.i(TAG, "OmacpMessageSettingsDetail addEmailToResultList intent is : " + intent);
                this.sendBroadcast(intent);
            } else {
    ...
    private void sendEmailIntent(ArrayList<Intent> emailIntentList, ResultType result) {
        ...
        if (checkIfApplicationSupport(OmacpMessageUtils.SMTP_APPID)) {
            MTKlog.d(TAG, "OmacpMessageSettingsDetail sendEmailIntent intent is : " + intent);
            this.sendBroadcast(intent);
        } else {
            // If not support, then label it failed directly
            result.mResult = RESULT_CONSTANT_FAILED;
    ...
    private void handleApplicationResultList() {
        if (!mApplicationResultList.isEmpty()) {
            ...
            MTKlog.d(TAG, "OmacpMessageSettingsDetail handleApplicationResultList isNeedSendTimeoutMsg is : " + isNeedSendTimeoutMsg);
    ...
    private void sendApplicationsByApnResult(boolean installSuccess) {
        MTKlog.d(TAG, "OmacpMessageSettingsDetail sendApplicationsByApnResult: " + installSuccess);
        if (installSuccess) {
            ...
            if (mApList != null && !mApList.isEmpty()) {
                ...
                if (info != null && info.getType() == ConnectivityManager.TYPE_MOBILE) {
                    ...
                    MTKlog.d(TAG, "OmacpMessageSettingsDetail mResultReceiver register apn switch receiver.");
                } else {
                    sendIntentsToApplications();
                }
            } else {
    ...
    private void sendApplicationsByAppResult(int size, String appId, boolean installSuccess) {
        MTKlog.d(TAG, "OmacpMessageSettingsDetail sendApplicationsByAppResult size = "
                + size + " appId = " + appId + " installSuccess = " + installSuccess);
        for (int i = 0; i < size; i++) {
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail sendApplicationsByAppResult flag: " + flag);
        if (flag) {
            mTimerHandler.removeMessages(EVENT_APPLICATION_INSTALL_TIME_OUT);
            handleFinishInstall();
        }
    ...
    protected void onStop() {
        MTKlog.d(TAG, "OmacpMessageSettingsDetail onStop");
        super.onStop();
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case EVENT_APPLICATION_INSTALL_TIME_OUT:
                    MTKlog.d(TAG, "OmacpMessageSettingsDetail application install time out......");
                    int size = mApplicationResultList.size();
                    for (int i = 0; i < size; i++) {
                        if (mApplicationResultList.get(i).mResult == RESULT_CONSTANT_NOT_RETURNED) {
                ...
                case EVENT_APN_INSTALL_TIME_OUT:
                    MTKlog.d(TAG, "OmacpMessageSettingsDetail apn install time out......");
                ...
                case EVENT_APN_SWITCH_TIME_OUT:
                    MTKlog.d(TAG, "OmacpMessageSettingsDetail apn switch time out......");
                    sendIntentsToApplications();
                    break;
                default:
                    MTKlog.d(TAG, "OmacpMessageSettingsDetail no proper event type.");
                    break;
            }
        }
    };
