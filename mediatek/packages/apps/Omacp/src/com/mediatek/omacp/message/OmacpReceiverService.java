...
public class OmacpReceiverService extends Service {
    ...
    public void onCreate() {
        MTKlog.d(TAG, "OmacpReceiverService onCreate");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        MTKlog.d(TAG, "OmacpReceiverService onStartCommand");
    ...
    public void onDestroy() {
        MTKlog.d(TAG, "OmacpReceiverService onDestroy");
        mServiceLooper.quit();
    }
    ...
    private void handleOmacpReceived(Intent intent, int error) {
        MTKlog.d(TAG, "OmacpReceiverService handleOmacpReceived: " + intent.getAction());
    ...
    private boolean handlePinVerify(Intent intent, byte[] intentData, NetWorkPinData pinData) {
        ...
        String sec = null;
        String mac = null;
        if (contentTypeParamaters == null) {
            MTKlog.e(TAG, "OmacpReceiverService contentTypeParamaters is null.");
        } else {
            pinData.mMac = contentTypeParamaters.get("MAC");
            pinData.mSec = contentTypeParamaters.get("SEC");
            sec = pinData.mSec;
            mac = pinData.mMac;
            MTKlog.d(TAG, "OmacpReceiverService handleOmacpReceived: MAC is : " + mac + " "
                        + "SEC is : " + sec);
            }

        // sec == "" indicates Network PIN, in this case, check SIM's IMSI, if
        // verify fails, then ignore this message
        if (sec != null && (sec.equalsIgnoreCase("") || sec.equalsIgnoreCase("0")) && mac != null && !mac.equalsIgnoreCase("")) {
            ...
            if (!correct) {
                MTKlog.e(TAG, "OmacpReceiverService Network PIN IMSI verify failed. Will ignore this message.");
                return false;
            }
        }
        return true;
    ...
    private void parseOmacpMessage(Intent intent, byte[] intentData, ContentValues value, NetWorkPinData pinData) {
        ...
        // get sim id
        int simId = intent.getIntExtra(PhoneConstants.GEMINI_SIM_ID_KEY, -1);
        MTKlog.d(TAG, "OmacpReceiverService handleOmacpReceived from : " + sender + " "
                    + "service center is : " + serviceCenter + " " + "simId is : " + simId);

        if (sender == null || serviceCenter == null) {
            MTKlog.e(TAG, "OmacpReceiverService handleOmacpReceived: sender or service center is null!");
            return;
        }
        ...
        MTKlog.d(TAG, "parseOmacpMessage MAC is : " + mac + " " + "SEC is : " + sec + " MIMIE = " + intent.getType());
    ...
    private void getParser(Intent intent, OmacpParser parser, byte[] intentData) {
        ...
        MTKlog.d(TAG, "getParser mimeType = " + intent.getType());
