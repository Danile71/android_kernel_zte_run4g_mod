...
public class ChatListFragment extends Fragment implements OnSelectedItemListener,
    ...
    private void loadGroupInvitation() {
        ...
        ConcurrentHashMap<GroupChat, Intent> groupInviteMap = RcsNotification.getInstance()
                .getTempGroupInvite();
        for (Map.Entry<GroupChat, Intent> entry : groupInviteMap.entrySet()) {
            GroupChat key = entry.getKey();
    ...
    private Chat getChatSession(String sessionId) {
    ...
    @Override
    public void onAddedGroupInvite(final GroupChat chatSession, final Intent intent) {
        mUiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                String info = null;
                if (message != null) {
                    date = message.getReceiptDate();
                } else {
                    Logger.e(TAG, "The invitation date is null");
                }
                InvitationStruct invite =
                        new InvitationStruct(drawable, date, info, intent);
    ...
    public class One2OneChatMap extends ChatMap implements IOne2OneChatWindow {
        ...
        @Override
        public IReceivedChatMessage addReceivedMessage(final ChatMessage message,
                final boolean isRead) {
            Logger.d(TAG, "ABCG addReceivedMessage() entry the message is " + message + "display: " + message.getDisplayName());
        ...
        @Override
        public ISentChatMessage addSentMessage(final ChatMessage message, int messageTag) {
        ...
        private class ChatDateUpdater implements ISentChatMessage, IReceivedChatMessage, Comparable<ChatDateUpdater> {
            ...
            public ChatDateUpdater(ChatMessage message) {
                mMessage = message;
                mDate = (Date) mMessage.getReceiptDate().clone();
            }
