...
public class GroupChatWindow extends ChatScreenWindow implements ChatView.IGroupChatWindow {
    ...
    public IFileTransfer addReceivedFileTransfer(FileStruct file, boolean isAutoAccept, boolean isRead) {
        Logger.d(TAG, "addReceivedFileTransfer entry: mTag = " + mTag + ", mGroupChatFragment = "
                + mGroupChatFragment + ", isRead ="+isRead);
        IFileTransfer iFileTransfer = null;

        if (shouldLoadData()) {
            if (mGroupChatFragment != null) {
                iFileTransfer = mGroupChatFragment.addReceivedFileTransfer(file, isAutoAccept, isRead);
            }
        }
    ...
    public void addLoadHistoryHeader(boolean showLoader) {
        ...
        if (shouldLoadData()) {
            if (mGroupChatFragment != null) {
                mGroupChatFragment.addLoadHistoryHeader(showLoader);
            }
        }

        Logger.d(TAG, "addLoadHistoryHeader exit");
    }
    ...
    public void addUnreadMessage(ChatMessage message) {
        Logger.d(TAG, "addUnreadMessage entry, notice null check is in outer, " + "the tag is: "
                + mTag + " message is: " + message.getMessage());
 ...
 @Override
	...
	public void setmChatID(String chatID) {
		if (mGroupChatFragment != null) {
            mGroupChatFragment.setmChatID(chatID);
        } else {
            Logger.d(TAG, "setmChatID() mGroupChatFragment is null");
        }
	...
	public void loadMessageIdList() {
		if (mGroupChatFragment != null) {
            mGroupChatFragment.loadMessageIdList();
        } else {
            Logger.d(TAG, "loadMessageIdList() mGroupChatFragment is null");
        }
