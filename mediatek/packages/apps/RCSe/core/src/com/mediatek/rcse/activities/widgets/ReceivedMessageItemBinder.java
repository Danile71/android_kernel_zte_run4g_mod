        ...
        if (message != null) {
            ...
            if (phoneNumUri != null) {
                ...
                if (PhoneUtils.isANumber(phoneNum)) {
                	if(ContactsListManager.getInstance().isLocalContact(phoneNum))
                	{
                	...
                	else
                	{
                		...
                		//remoteName = RichMessaging.getInstance().getContactAliasName(phoneNum);
                		String tempName = ContactsListManager.getInstance().getDisplayNameByPhoneNumber(phoneNum);
                		String aliasName = ((GroupChatFragment.ReceivedMessage) message).getAliasName();
                		if(aliasName !=null && (aliasName.equals("0") || aliasName.equals("") || aliasName.equals(tempName)))
                		{
                			remoteName = tempName;
                		}
                		else
                		{
                			remoteName = tempName + " " +"~" +aliasName;
                		}
                		Logger.d(TAG, "remote name is "+remoteName + " alias is ="+aliasName);
