...
public class UnreadMessagesContainer {
     ...
     * @return The latest unread message.
     */
    public synchronized ChatMessage getLatestUnreadMessage() {
        Logger.d(TAG, "getLatestUnreadMessageChat entry");
        if (mTail == null) {
            Logger.d(TAG, "getLatestUnreadMessageChat mTail is null");
            return null;
        ...
        if (instance == null) {
            ...
            if (messages == null) {
                ...
                if (length == 0) {
                    ...
                    ChatMessage message = (ChatMessage) messageArray[length - 1];
                    if (null != message) {
                        Logger.d(TAG, "getLatestUnreadMessage exit, the message text is "
                                + message.getMessage() + " the remote contact is "
                                + message.getContact());
                        return message;
                    } else {
    ...
    public synchronized void loadLatestUnreadMessage() {
        ...
        ChatMessage unreadMessage = getLatestUnreadMessage();
