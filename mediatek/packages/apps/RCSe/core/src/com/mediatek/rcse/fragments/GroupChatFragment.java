...
public class GroupChatFragment extends ChatFragment implements ChatView.IGroupChatWindow,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Logger.d(TAG, "onCreate() GroupChatFragment tag: " + mTag + "participantlist:"+ mGroupChatParticipantList);
        List<Participant> participants = new ArrayList<Participant>();
        ...
        alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                ...
                Message controllerMessage = ControllerImpl.getInstance().obtainMessage(
                        ChatController.ADD_GROUP_SUBJECT, mTag, subject);                             
                ...
                if(((ChatScreenActivity)getActivity()).isUrlShare == true)
                {
	                ...
	                controllerMessage =
	                    controller.obtainMessage(
	                        ...
	                        ((ChatScreenActivity)getActivity()).chat.getChatTag(), message);
...
@Override
    public void onCreateContextMenu(ContextMenu menu, View v,
            ...
            Logger.d(TAG, "onCreateContextMenu position:"+ itemID_position);
    ...
    public void showStatusDialog(Context context, IChatMessage chatMessage) {
        ...
        else if(chatMessage instanceof GroupChatFragment.SentFileTransfer)
        {
        	...
        	intent.putExtra("statusmap", ((GroupChatFragment.SentFileTransfer) chatMessage).mFileStatusMap);
    ...
    public void onDestroy() {
        ...
        Message controllerMessage = ControllerImpl.getInstance().obtainMessage(
                ChatController.EVENT_CLEAR_EXTRA_MESSAGE_GROUP_CHAT, mTag, null);
        controllerMessage.sendToTarget(); 
        super.onDestroy();
    }
    ...
    private int onSentMessage(String content) {
        ...
        ChatMessage message = new ChatMessage("", Utils.DEFAULT_REMOTE, content, new Date(), true, RcsSettings.getInstance().getJoynUserAlias());
    ...
    public IReceivedChatMessage addReceivedMessage(ChatMessage message, boolean isRead) {
        Logger.d(TAG, "addReceivedMessage() mIsBottom: " + mIsBottom + " message: " + message.getMessage()
                + " isRead: " + isRead);
        if (message != null) {
    ...
    public class SentFileTransfer extends FileTransfer {
    	public ConcurrentHashMap<String, Integer> mFileStatusMap = new ConcurrentHashMap<String, Integer>();
        public SentFileTransfer(FileStruct fileStruct) {
            super(fileStruct);
            mStatus = Status.PENDING;
            for(Participant participant : mParticipantList)
            {
            	mFileStatusMap.put(participant.getContact(), Status.PENDING.ordinal() );
            }
        }
        
        public void setStatus(Status status, String contact) {
        	Logger.d(TAG, "SentFileTransfer,setStatus()  status: " + status + "contact :"+ contact + " name:"+ mFileStruct.mName);
        	//update the map
    ...
    public void updateParticipants(final List<ParticipantInfo> participantList) {
        ...
        mActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                {
                	Logger.d(TAG, "updateParticipants entry size: " + mGroupChatParticipantList.size());
                	//only this participant remains in chat, others have left, chat window should close
                	closeGroupChat();
                }
                ...
                mParticipantList = participants;
                updateChatUi();
                if (Logger.getIsIntegrationMode() && mActivity instanceof PluginGroupChatActivity) {
                    ((PluginGroupChatActivity) mActivity).updateParticipants(mParticipantList);
                } else {
                    Logger.d(TAG, "updateParticipants() it is not integration mode or not instace of PluginGroupChatActivity.");
                }
    ...
    public class SentMessage implements ISentChatMessage, IChatMessage  {
		...
		public void updateStatus(Status s, String contact) {
			Logger.d(TAG, "SentMessage, updateStatus()  status: " + s + "contact :"+ contact + " id:"+ mMessage.getId());
            //update the map
    ...
    public static class ReceivedMessage implements IReceivedChatMessage, IChatMessage  {
        ...
        public String getAliasName(){
        	if (mMessage == null) {
                Logger.w(TAG, "ABCG mMessage is null, as a result no alias returned");
                return null;
            }
    ...
    private void handleGetHistory() {
    	Logger.d(TAG, "handleGetHistory entry mCurrentNumberMsgLoaded=" + mCurrentNumberMsgLoaded);
    	try{
    		reloadMessageIdList();
    	if(mMessageIdArray != null && mMessageIdArray.size() > mCurrentNumberMsgLoaded)
    	{
    		List<Integer> mMessageIdCurrentArray = null;
    		if((mMessageIdArray.size()-(1+mCurrentNumberMsgLoaded+ GroupChatFragment.LOAD_DEFAULT))<0)
    		{
    			mMessageIdCurrentArray = mMessageIdArray;
	    			addLoadHistoryHeader(false);
    		} else {
	    			//mMessageIdCurrentArray = mMessageIdArray.subList(mMessageIdArray.size()-(mCurrentNumberMsgLoaded+ GroupChatFragment.LOAD_DEFAULT), mMessageIdArray.size());
	    			mMessageIdCurrentArray = mMessageIdArray.subList(0, mCurrentNumberMsgLoaded+ GroupChatFragment.LOAD_DEFAULT);
	    			addLoadHistoryHeader(true);
    		}
    	    
    	    //mCurrentNumberMsgLoaded = mMessageIdCurrentArray.size();
    	    Logger.d(TAG, "handleGetHistory size current id list" + mMessageIdCurrentArray.size());
    	    try {
                Message controllerMessage = ControllerImpl
                        ...
                        .obtainMessage(
	                        ...
	                        mTag, null);
	                controllerMessage.sendToTarget();
	                Message controllerMessage1 = ControllerImpl
	                        .getInstance()
	                        .obtainMessage(
    ...
    public void addLoadHistoryHeader(boolean showHeader) {
    	...
    	if (mMessageAdapter != null && mMessageAdapter.mHeaderView != null) {
            ...
            mMessageAdapter.mHeaderView.setOnClickListener(new View.OnClickListener() {
...
@Override
	...
	public void loadMessageIdList() {
        ...
        try{
            cursor = RichMessagingDataProvider
                .getInstance().getAllMessageforGroupChat(mChatID);
            mMessageIdArray.clear();
            Logger.d(TAG, "loadmessageIdList() cursor count = " + cursor.getCount() + ",chatid =" + mChatID);
	    ...
	    if(mMessageIdArray != null)
	    {
	        if (!mMessageIdArray.isEmpty()) {
	            ...
	            Logger.d(TAG, "loadmessageIdList() message id array "+ mMessageIdArray);    
	            /*try {
	                Message controllerMessage = ControllerImpl
	                        .getInstance()
	                        .obtainMessage(
	                                ChatController.EVENT_RELOAD_MESSAGE,
	                                mTag.toString(), mMessageIdArray);
	                controllerMessage.sendToTarget();
	            } catch (Exception e) {
