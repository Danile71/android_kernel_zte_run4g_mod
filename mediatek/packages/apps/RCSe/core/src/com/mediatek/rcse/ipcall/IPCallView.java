...
import com.mediatek.rcse.plugin.phone.LiveVideoPlayer;
import com.mediatek.rcse.plugin.phone.LiveVideoRenderer;
...
public class IPCallView extends Activity implements SurfaceHolder.Callback {
	 ...
	 * Audio player
	 */
	private LiveIPCallPlayer outgoingAudioPlayer = null;

	/**
	 * Audio renderer
	 */
	private LiveIPCallRenderer incomingAudioRenderer = null;

	/**
	 * Video player
	 */
	private LiveIPCallPlayer outgoingVideoPlayer = null;
	
	/**
	 * Video renderer
	 */
	private LiveIPCallRenderer incomingVideoRenderer = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (action != null){
			...
			if (action.equals("ExitActivity")) {
				...
				incomingAudioRenderer = new LiveIPCallRenderer(this);
				outgoingAudioPlayer = new LiveIPCallPlayer();
				
				// IP call already established - recover existing session
				if (action.equals("recover")) {
					// get sessionId from Intent
					sessionId = getIntent().getStringExtra(IPCallIntent.EXTRA_CALL_ID);	
					if (sessionId == null) {
    ...
    public IPCallEventListener sessionEventListener = new IPCallEventListener();
	/**
	 * IP call session event listener
	 */
	private class IPCallEventListener extends IPCallListener {
...
//					stopSession();
		...
		public void handleAddVideoAccepted() throws RemoteException {
			...
			handler.post(new Runnable() {
				public void run() {
					...
					// Update Camera
                    outgoingHeight = outgoingVideoPlayer.getVideoCodec().getVideoHeight();
                    outgoingWidth = outgoingVideoPlayer.getVideoCodec().getVideoWidth();
                    if (logger.isActivated()) {
		...
		public void onCallStarted() {
			if (logger.isActivated()) {
				logger.info("sessionEventListener - onCallStarted()");
			}
			handler.post(new Runnable() {
				public void run() {
					hideProgressDialog();
		...
		public void onCallError(final int error) {
			if (logger.isActivated()) {
				logger.info("sessionEventListener - onCallError()");
			}		
			Thread thread = new Thread() {
				public void run() {
					stopSession();
	...
	private void getIncomingSession() {
		...
		handler.post(new Runnable() {
			public void run() {
				try {
					session = IPCallSessionsData.getInstance().callApi.getIPCall(sessionId);
					session.addEventListener(sessionEventListener);
				...
				try {
					...
					if (videoSelected) {						
						...
						session = IPCallSessionsData.getInstance().callApi.initiateCall(
								...
								incomingAudioRenderer, new IPCallEventListener());
					} else {
						session = IPCallSessionsData.getInstance().callApi.initiateCall(
								remoteContact, outgoingAudioPlayer,
								incomingAudioRenderer, new IPCallEventListener());
				...
				public void run() {
					try {
						...
						try {
							...
							direction = (session.getDirection() == IPCallSessionsData.TYPE_OUTGOING_IPCALL) ? "outgoing"
									: "incoming";
						} catch (Exception e1) {
							exitActivity("Call Api exception - unable to get session");
						}
		...
		Thread thread = new Thread() {
			public void run() {
				try {
					...
					session.acceptInvitation(outgoingAudioPlayer, incomingAudioRenderer);
				} catch (final Exception e) {
		...
		Thread thread = new Thread() {
			public void run() {
				...
				try {
					...
					session.acceptInvitation(outgoingVideoPlayer, incomingVideoRenderer);
					videoConnected = true;
        	...
        	if (logger.isActivated()){
                ...
                try {
                    if (VideoSettings.isCodecsManagedByStack(getApplicationContext())) {
                        incomingVideoRenderer = new LiveIPCallRenderer(this);
                    } else {
                        incomingVideoRenderer = new LiveIPCallRenderer(this/*createSupportedCodecList(VideoSettings.getCodecsList(getApplicationContext()))*/);
                    }
                    //TODO Fix Error
    ...
    private void  instantiateOutgoingVideoPlayer() {
        	if (logger.isActivated()){
                ...
                if (VideoSettings.isCodecsManagedByStack(getApplicationContext())) {
                    outgoingVideoPlayer = new LiveIPCallPlayer();
                    if (logger.isActivated()){ logger.info("Codecs Managed By Stack");}
                } else {
                    outgoingVideoPlayer = new LiveIPCallPlayer(/*createSupportedCodecList(VideoSettings.getCodecsList(getApplicationContext())), incomingVideoRenderer*/);
                }
     ...
     * @return codecs list
     */
    private VideoCodec[] createSupportedCodecList(boolean[] codecs) {
        ...
        VideoCodec[] supportedMediaCodecs = new VideoCodec[size];
