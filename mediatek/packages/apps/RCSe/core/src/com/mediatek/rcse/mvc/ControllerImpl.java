...
public class ControllerImpl implements IChatController {
            ...
            switch (msg.what) {
                ...
                case ChatController.EVENT_TEXT_CHANGED:
                    ...
                    if (chatMessage instanceof Boolean) {
                        ...
                        if (tempChat != null) {
							 Logger.w(TAG, "ChatController.EVENT_TEXT_CHANGED: isempty :" + isEmpty);
                            tempChat.hasTextChanged(isEmpty);
                        } else {
                            Logger.w(TAG, "ChatController.EVENT_TEXT_CHANGED: iChat = null");
                        }
                    } else {
				...
				case ChatController.EVENT_GROUP_FILE_TRANSFER_INVITATION:
                    // Use chatMessage to get file path from view
                    handleGroupSendFileTransferInvitation(chatWindowTag, chatMessage, msg.getData());
                    break;
                ...
                case ChatController.EVENT_RELOAD_MESSAGE:
				 ...
				 case ChatController.EVENT_RELOAD_NEW_MESSAGE:
                    Logger.d(TAG, "handleMessage() EVENT_RELOAD_NEW MESSAGE chatMessage: "
                            + chatMessage);
				...
				case ChatController.EVENT_DELETE_MESSAGE_LIST:
					Logger.d(TAG, "handleDelteMessage() entry  the Message is " + chatMessage);
                	handleDeleteMessageList(chatWindowTag,(List<String>)chatMessage);
                	break;
    ...
    private void handleClearChatHistory(Object tag) {
        ...
        if (tag == null) {
            ...
            //contentResolver.delete(RichMessagingData.CONTENT_URI, null, null);  // TODo check this
    ...
    private void handleClearChatHistoryMemory(Object tag) {
        Logger.d(TAG, "handleClearChatHistoryMemory() entry, the tag is: " + tag);
    ...
    private void handleClearAllChatHistoryMemory() {
    	...
    	AndroidFactory.getApplicationContext().sendBroadcast(intentReload);
	...
	private void handleGroupSendFileTransferInvitation(Object tag, Object filePath, Bundle data) {
        RegistrationApi api = ApiManager.getInstance().getRegistrationApi();
        if (null != api && api.isRegistered()) {
            Parcelable fileTransferTag = (data != null ? data
                    .getParcelable(ModelImpl.SentFileTransfer.KEY_FILE_TRANSFER_TAG) : null);
            mModel.handleGroupSendFileTransferInvitation(tag, (String) filePath, fileTransferTag);
        } else {
            Logger.w(TAG, "handleSendFileTransferInvitation, "
                    + "api is null or not registered, registertion status is null");
            Toast.makeText(ApiManager.getInstance().getContext(), R.string.file_transfer_off_line,
    ...
    private boolean handleSendMessageByContact(String contact, String message, int messageTag) {
        ...
        IChat1 chat = mModel.getOne2oneChatByContact(contact);
        if (null != chat) {
            chat.sendMessage(message, messageTag);
            return true;
        }
