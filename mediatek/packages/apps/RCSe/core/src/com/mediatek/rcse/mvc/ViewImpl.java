...
public class ViewImpl implements IChatWindowManager {
    ...
    private abstract static class ChatWindowDispatcher implements IChatWindow {
        ...
        public IChatWindowMessage getSentChatMessage(String messageId) {
            ...
            if (message instanceof ISentChatMessage || message instanceof SentFileTransfer) {
                return message;
            }
            return null;
        }
            ...
            if (null != messageId) {
                for (IChatItemDispatcher chatItem : mChatItemList) {
                    if (chatItem instanceof BaseChatMessage
                            && messageId.equals(((BaseChatMessage) chatItem).getId())) {
                    	...
                    	if(messageId.equals(((FileTransferDispatcher) chatItem).mFileStruct.mFileTransferTag)){
                    		Logger.i(TAG, "FileTransferDispatcher() id is" + messageId);
                    		Collection<IFileTransfer> fileTransfers = ((FileTransferDispatcher) chatItem).mChatWindowMap.values();
        ...
        public IReceivedChatMessage addReceivedMessage(ChatMessage message, boolean isRead) {
            ...
            Collection<IChatWindow> chatWindows = mChatWindowMap.values();
            BaseChatMessage receivedMessage = new ReceivedChatMessage(message, isRead);
            for (IChatWindow chatWindow : chatWindows) {
        ...
        public ISentChatMessage addSentMessage(ChatMessage message, int messageTag) {
            ...
            Collection<IChatWindow> chatWindows = mChatWindowMap.values();
            BaseChatMessage sentMessage = new SentChatMessage(message, messageTag);
            for (IChatWindow chatWindow : chatWindows) {
    ...
    private static class GroupChatWindowDispatcher extends ChatWindowDispatcher implements
       ...
       public IFileTransfer addReceivedFileTransfer(FileStruct file, boolean isAutoAccept, boolean isRead) {
           Logger.i(TAG, "addReceivedFileTransfer() FileStruct is" + file);
           Collection<IChatWindow> chatWindows = mChatWindowMap.values();
           FileTransferDispatcher fileTransferDispatcher = new ReceivedFileTransferDispatcher(file, isAutoAccept, isRead);
           for (IChatWindow chatWindow : chatWindows) {
               if (chatWindow instanceof IGroupChatWindow) {
       ...
       public IFileTransfer addSentFileTransfer(FileStruct file) {
        ...
        public void onAddChatWindowManager(IChatWindowManager chatWindowManager) {
            ...
            if (null != chatWindow) {
                ...
                ((IGroupChatWindow) chatWindow).setFileTransferEnable(mFileTransferDisableReason);
                setmChatID(mChatID);
                loadMessageIdList();
                    ...
                    if (((GroupChatWindow) chatWindow)
                        ...
                        try{
                            cursor = RichMessagingDataProvider
                                .getInstance().getAllMessageforGroupChat(mChatID);
                            Logger.d(TAG, "onAddChatWindowManager() cursor count = " + cursor.getCount() + ",chatid =" + mChatID);
                	        if (cursor != null && cursor.moveToFirst()) {
                	            ...
                	            Logger.d(TAG, " onAddChatWindowManager() empty cursor");              
                	        }
                        } catch(Exception e){
                        	e.printStackTrace();
                        }
                        ...
                        if(messageIdArray != null)
                        {
                        	if (!messageIdArray.isEmpty()) {
                	            Collections.reverse(messageIdArray);
                	            Logger.d(TAG, "onAddChatWindowManager() message id array "+ messageIdArray.size());    
                        try {
                            Message controllerMessage = ControllerImpl
                                    .getInstance()
                                    .obtainMessage(
		...
		public void clearExtraMessage() {
        	//need to keep default number of messages, so remove extra message from memory
        	Logger.i(TAG, "clearExtraMessage entry" );
        	if(mChatItemList.size() > GroupChatFragment.LOAD_DEFAULT) {
	        	//List<IChatItemDispatcher> mChatItemListTemp =  mChatItemList.subList(mChatItemList.size() - GroupChatFragment.LOAD_DEFAULT -1, mChatItemList.size()-1);
	        	//mChatItemList = new CopyOnWriteArrayList<IChatItemDispatcher>(mChatItemListTemp);
                	int diff = mChatItemList.size() - GroupChatFragment.LOAD_DEFAULT;
                	for(int i=0; i < diff; i++)
                	{
                		mChatItemList.remove(0);
                	}
	                Logger.d(TAG, "GroupChatDispatcher size of mChatItemList "+mChatItemList.size());
                }
        ...
        public void removeChatMessage(String messageId) {
            if (null != messageId) {
                ...
                for (IChatItemDispatcher chatItem : mChatItemList) {
                    if (chatItem instanceof BaseChatMessage
                            && messageId.equals(((BaseChatMessage) chatItem).getId())) {
                        Logger.e(TAG, "removeChatMessage() found the message !="+chatItem);
                        mChatItemList.remove(chatItem);
                    } else if (chatItem instanceof FileTransferDispatcher
                            && messageId.equals(((FileTransferDispatcher) chatItem).mFileStruct.mFileTransferTag.toString())) {
                        Logger
     ...
     */
    public abstract static class BaseChatMessage implements IChatWindowMessage, IChatItemDispatcher {
    ...
    private static class ReceivedFileTransferDispatcher extends FileTransferDispatcher {
        ...
        IFileTransfer getFileTransfer(IChatWindow chatWindow, FileStruct fileStruct) {
        	...
        	{
        		return ((IGroupChatWindow) chatWindow).addReceivedFileTransfer(fileStruct, mIsAutoAccept, mIsRead);
        	}
        	else if(chatWindow instanceof One2OneChatWindowAdapter)
        	{
        		return ((IOne2OneChatWindow) chatWindow).addReceivedFileTransfer(fileStruct, mIsAutoAccept);
