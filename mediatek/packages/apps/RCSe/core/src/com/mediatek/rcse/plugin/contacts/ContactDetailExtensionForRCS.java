...
public class ContactDetailExtensionForRCS extends ContactDetailExtension {
	...
	private int mIMValue = 1;
	private int mFTValue = 1;
				...
				if (vewVtCallDivider != null && secondaryActionDivider != null
                	...
                	if (Logger.getIsIntegrationMode()) 
                	{
                		...
                		if(mode == 1)
                		{
    						...
    						//secondaryActionButton.setImageDrawable(c);
    						...
    						//set blocking icon to be visible
                            pluginActionViewContainer.setVisibility(View.VISIBLE);
    						pluginActionButton.setVisibility(View.VISIBLE);
    						pluginActionButton.setImageDrawable(mContactPlugin.getBlockingDrawable(mNumber));
	...
	public Intent getExtentionIntent(int im, int ft, String commd) {
		Intent intent = null;
		boolean imValue = mContactPlugin.isImSupported(mNumber);
		boolean ftValue = mContactPlugin.isFtSupported(mNumber);
		Log.i(TAG, "[getExtentionIntent] im : " + imValue + " | ft : " + ftValue
				+ " | commd : " + commd);
		if(imValue == true) {
		   mIMValue = 1;
		}
	...
	private OnClickListener mBlockingButtonOnClickListner = new View.OnClickListener() {
		...
		public void onClick(View view) {
			...
			try{
			    blockedStatus = mContactPlugin.getBlockedStatus(number);
			}
			catch(Exception e)
			{
			    e.printStackTrace();
			...
			if(blockedStatus) {
			    ...
			    alertDialogBuilder
                .setMessage(number+" is already blocked by you. Do you wish to unblock?")
                .setCancelable(true)
                .setPositiveButton("Yes",new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog,int id) {
                            ...
                            try{
                                boolean isUnBlockedSuccess = mContactPlugin.blockContact(number, false);
                                if(isUnBlockedSuccess == true)
                                {
                                    Toast.makeText(context, mName + " successfully unblocked", Toast.LENGTH_LONG).show();
                                }
                            }
                            catch(Exception e){
                                e.printStackTrace();
				...
				alertDialogBuilder
					...
					.setPositiveButton("Yes",new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,int id) {
							...
							try{
							    isBlockedSuccess = mContactPlugin.blockContact(number, true);
							    if(isBlockedSuccess == true)
							    {
							        Toast.makeText(context, mName + " successfully blocked", Toast.LENGTH_LONG).show();
							    }
							}
							catch(Exception e){
							    e.printStackTrace();
