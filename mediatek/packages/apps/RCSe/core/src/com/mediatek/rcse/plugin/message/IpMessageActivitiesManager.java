...
//            Recordintent.setType(ContentType.AUDIO_AMR);
    ...
    private void startRemoteInvitationDialog(Context context, Intent intent, long threadId,
        ...
        if (null != sessionId) {
            ...
            GroupChat chatSession = PluginGroupChatWindow.getChatSession(sessionId);
            try {
                if (null != chatSession) {
                    ...
                    /*if (null != instantMessage) {
						Logger.d(TAG, "startRemoteInvitationDialog msg is "
								+ instantMessage);
    ...
    private static void startContactSelectionActivity(int type, Context context, ArrayList<Participant> originalContact) {
        if (type == SelectContactType.IP_MESSAGE_USER) {
            ...
            intentSelect.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);          
            ...
            AndroidFactory.getApplicationContext().startActivity(intentSelect);
        } else {
