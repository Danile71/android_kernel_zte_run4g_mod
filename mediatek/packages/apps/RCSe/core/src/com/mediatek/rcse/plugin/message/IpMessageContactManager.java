    ...
    private String getSubjectByID(long threadID){
    	 ...
    	 try {
			cursor = AndroidFactory.getApplicationContext().getContentResolver().
			            query(IntegratedMessagingData.CONTENT_URI_INTEGRATED, null, 
			            "" + IntegratedMessagingData.KEY_INTEGRATED_MODE_THREAD_ID + " = " + threadID +" ",
			         null ,
			         null
			        );
			...
			if(cursor!=null && cursor.moveToFirst()){
				...
				grpSubject = cursor.getString(cursor.getColumnIndex(IntegratedMessagingData.KEY_INTEGRATED_MODE_GROUP_SUBJECT));
			}
...
if (!number.startsWith(IpMessageConsts.JOYN_START)) {
		...
		number = PhoneUtils.formatNumberToInternational(number);
		Logger.d(TAG, "isIpMessageNumber() after formatting number is " + number);
        ContactsListManager contactListManager = ContactsListManager.getInstance();
    ...
    public int getStatusByNumber(String number) {
		...
		number = PhoneUtils.formatNumberToInternational(number);
        Logger.d(TAG, "getStatusByNumber() after formatting number is " + number);
        ...
        if (!TextUtils.isEmpty(number) && STATUS_MAP.containsKey(number)) {
            ...
            Logger.d(TAG,
                    "getStatusByNumber() the target number is not in the cache, fetch capabilities");            
            CapabilityService capabilityApi = ApiManager.getInstance().getCapabilityApi();
            if (capabilityApi != null) {
                try {
                    Capabilities capability = capabilityApi.getContactCapabilities(number);
