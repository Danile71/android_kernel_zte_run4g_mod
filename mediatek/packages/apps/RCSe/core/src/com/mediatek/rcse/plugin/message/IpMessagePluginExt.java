...
public class IpMessagePluginExt extends IpMessagePluginImpl implements PluginApiManager.RegistrationListener {
    ...
    private SettingsManager mIpMessageSettingsManager = null;
    ...
    private static boolean isRCSeConfigured = false;
    ...
    public ResourceManager getResourceManager(Context context) {
       ...
       if(!PluginUtils.isRCSeConfigured())
       {
           Logger.d(TAG, " getResourceManager isRCSeConfigured false ");
    ...
    public ActivitiesManager getActivitiesManager(Context context) {
        ...
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getActivitiesManager isRCSeConfigured false ");
    ...
    public ChatManager getChatManager(Context context) {
        ...
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getChatManager isRCSeConfigured false ");
    ...
    public SettingsManager getSettingsManager(Context context) {
        Logger.d(TAG, "getSettingsManager() entry! context = "
                + context.getApplicationInfo().packageName);
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getSettingsManager isRCSeConfigured false ");
            return super.getSettingsManager(context);
        }
        if (Logger.getIsIntegrationMode()) {
            initialize(context);
            return mIpMessageSettingsManager;
        } else {
            Logger.d(TAG, "getSettingsManager() in chat app mode");
            return super.getSettingsManager(context);
        }
    }
    ...
    private void initialize(Context context) {
        if (mHostContext == null) {
            ...
            mIpMessageSettingsManager = new IpMessageSettingsManger(context);
			...
			if (mReceiver == null && intentFilter == null) {
				...
				intentFilter.addAction(PluginUtils.ACTION_DB_CHANGE_RELOAD);
				...
				intentFilter.addAction(PluginUtils.ACTION_SEND_URL);
				intentFilter.addAction(PluginUtils.ACTION_SEND_RESIZED_FILE);
    ...
    public MessageManager getMessageManager(Context context) {
        Logger.d(TAG, "getMessageManager() entry! context = " + context);
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getMessageManager isRCSeConfigured false ");
    ...
    public ServiceManager getServiceManager(Context context) {
        ...
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getServiceManager isRCSeConfigured false ");
    ...
    public NotificationsManager getNotificationsManager(Context context) {
        ...
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getNotificationsManager isRCSeConfigured false ");
    ...
    public ContactManager getContactManager(Context context) {
        ...
        if(!PluginUtils.isRCSeConfigured())
        {
            Logger.d(TAG, " getContactManager isRCSeConfigured false ");
				...
				protected Void doInBackground(Void... params) {
					try {
                        ...
                        else if(intent.getAction().equals(PluginUtils.ACTION_MODE_CHANGE))
						{
							...
							PluginUtils.mMessagingUx = -1;
							PluginUtils.FAILED_MESSAGE_CACHE.evictAll();
						}
                        else if(intent.getAction().equals(PluginUtils.ACTION_SEND_URL))
						{
							String url = intent.getStringExtra(PluginApiManager.RcseAction.SHARE_URL);
							String number = intent.getStringExtra(PluginApiManager.RcseAction.CONTACT_NUMBER);
                        ...
                        else if(intent.getAction().equals(PluginUtils.ACTION_SEND_RESIZED_FILE))
						{
							boolean compress = intent.getBooleanExtra("compressImage", false);							
							mMessageManager.sendResizedFiles(compress);
							
						}
                        else if(intent.getAction().equals(PluginUtils.ACTION_DB_CHANGE_RELOAD))
						{
                        	mMessageManager.getsCacheRcseMessage().clear();					
        					Logger.d(TAG,
        							"SIM Change Plugin cache not empty"+ PluginUtils.ACTION_DB_CHANGE_RELOAD);					
        			    	ContentResolver contentResolver = mHostContext.getContentResolver();
        					contentResolver
        								.delete(Uri.parse("content://sms/"), "ipmsg_id>0", null);
        					 PluginController.obtainMessage(ChatController.EVENT_RELOAD_NEW_MESSAGE,
