...
public class IpMessageResourceMananger extends ResourceManager {
     ...
     public Drawable getSingleDrawable(int id) {
        ...
        if (resource != null) {
        	if(sDrawableMap.get(id)==null)
        			return null;
			if ((id == drawable.ipmsg_file_transfer_pause || id == drawable.ipmsg_file_transfer_resume)
					&& !RcsSettings.getInstance().isFileTransferHttpSupported()) {
				Logger.d(TAG,
						"getSingleDrawable() for pause/resume & MSRP FT, So return null");
				return null;
			}
    ...
    public String getSingleString(int id) {
		...
		if(resource!= null)
		{
			Integer idLocal = sStringMap.get(id);
			if(idLocal!=null)
				string =  resource.getString(idLocal.intValue());
		}
		return string;
    }
