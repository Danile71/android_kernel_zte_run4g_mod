...
public class PluginIpVoiceMessage extends IpVoiceMessage {
    ...
    public void analysisAttribute() {
        ...
        try {
            ...
            Logger.d(TAG, "analysisAttribute(), PluginIpVoiceMesage cursor = " + cursor);
            if (cursor != null && cursor.getCount() != 0) {
                cursor.moveToFirst();
                duration = cursor.getInt(cursor.getColumnIndex(MediaStore.Audio.AudioColumns.DURATION));
            } else {
                cursor2 = cr.query(MediaStore.Audio.Media.INTERNAL_CONTENT_URI, new String[] {
                    MediaStore.Audio.AudioColumns.DURATION
                }, whereClause, null, null);
                Logger.d(TAG, "analysisAttribute(), PluginIpVoiceMesage else loop cursor2 = " + cursor2);
                if (cursor2 != null && cursor2.getCount() != 0) {
                    cursor2.moveToFirst();
                    duration = cursor2.getInt(cursor2
                            .getColumnIndex(MediaStore.Audio.Media.DURATION));
                } else {
                    Logger.w(TAG, "analysisAttribute(), cursor2 is null!");
                }
            }
