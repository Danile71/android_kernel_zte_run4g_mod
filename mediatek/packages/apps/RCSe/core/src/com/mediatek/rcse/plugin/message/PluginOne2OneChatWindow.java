...
public class PluginOne2OneChatWindow extends IRemoteOne2OneChatWindow.Stub implements WindowTagGetter {
    ...
    public IRemoteFileTransfer addReceivedFileTransfer(FileStructForBinder file, boolean fileAutoaccept) throws RemoteException {
        ...
        fileTransferString = fileTransferString + " " + file.fileSize / PluginUtils.SIZE_K + "KB";
        if(file.fileSize < PluginUtils.SIZE_K)
        {           
            fileTransferString = PluginUtils.getStringInRcse(R.string.file_transfer_title) + " " + file.fileSize % PluginUtils.SIZE_K + "B";
        }
        ...
        if (-1 < ipMsgId) {
            ...
            if(idInMms != -1 && !PluginUtils.reloadingMessages)
            {
            	Logger.d(TAG, "addReceivedFileTransfer(),AUTOACCEPT idInMms = " + idInMms);
            	idInMms = -1;
            }
            if (idInMms == -1) {
    ...
    public IRemoteSentChatMessage addSentMessage(ChatMessage message, int messageTag) throws RemoteException {
        Logger.d(TAG, "addSentMessage() text: " + message.getMessage() + " messageTag: " + messageTag);
