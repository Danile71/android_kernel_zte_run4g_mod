...
public class PluginSentChatMessage extends IRemoteSentChatMessage.Stub {
    ...
    public PluginSentChatMessage(IpMessageManager messageManager, ChatMessage message, int messageTag) {
        ...
        if (-1 != messageTag && PluginUtils.getIdInMmsDb(messageTag)!= -1 /*&& !message.getContact().startsWith(PluginGroupChatWindow.GROUP_CONTACT_STRING_BEGINNER)*/) {
            ...
            PluginUtils.updatePreSentMessageInMmsDb(message.getId(), messageTag);
        } else {
            messageIdInMms =
                PluginUtils.storeMessageInDatabase(message.getId(),
                        message.getMessage(), message.getContact(), PluginUtils.OUTBOX_MESSAGE,0,messageTag);
    ...
    public PluginSentChatMessage(IpMessageManager messageManager, ChatMessage message, int messageTag, String groupSubject) {
        mPluginChatWindowMessage = new PluginChatWindowMessage(message);
        mMessageManager = messageManager;
        Logger.d(TAG, "PluginSentChatMessage(), messageManager: " + messageManager + "message = " + message
                + " ,messageTag: " + messageTag);
        Long messageIdInMms = null;
        if (-1 != messageTag && PluginUtils.getIdInMmsDb(messageTag)!= -1 && mMessageManager.removePresentMessage(messageTag)) {
            messageIdInMms = PluginUtils.getIdInMmsDb(messageTag);
            Logger.d(TAG, "PluginSentChatMessage() this is a present message, we'll update it." +
                    " messageIdInMms: " + messageIdInMms);
            PluginUtils.updatePreSentMessageInMmsDb(message.getId(), messageTag);
