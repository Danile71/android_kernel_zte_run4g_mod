...
public class ImageSharingPlugin extends SharingPlugin {
    ...
    public ImageSharingPlugin(final Context context) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void result) {
                ...
                intentFilter.addAction(ImageSharingIntent.ACTION_NEW_INVITATION);
    ...
    private void addOutGoingListener(final String imageName) {
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    mOutgoingImageSharingSession = mImageSharingApi.shareImage(IDENTIFY_TEL
                            + getVodafoneAccount(mNumber), imageName,mOutgoingSessionEventListener);
     ...
     */
    private class OutgoingSessionEventListener extends ImageSharingListener {
...
*/
        ...
        public void onImageShared(String arg0) {
            ...
            showFullImage(arg0);
				mOutgoingImageSharingSession.removeEventListener(mOutgoingSessionEventListener);
     ...
     */
    private class IncomingSessionEventListener extends ImageSharingListener {
        ...
        public void onImageShared(String arg0) {
            ...
            mMainHandler.post(new Runnable() {
				...
				mIncomingImageSharingSession.removeEventListener(mIncomingSessionEventListener);
            if (mWakeLockCount.decrementAndGet() >= 0) {
                mWakeLock.release();
            }
            Logger.d(TAG, "handleImageTransfered exit");
    ...
    private void showFullImage(final String imageName) {
        new AsyncTask<Void, Void, Bitmap>() {
            ...
            @Override
            protected void onPostExecute(Bitmap bitmap) {
                Logger.d(TAG, "showFullImage()123, onPostExecute entry");
                if (mDisplayArea != null) {
                    if (mCallScreenHost != null) {
                        ...
                        ImageView imageview =
                                (ImageView) mImageSharingView.findViewById(R.id.shared_image);
                        imageview.setImageBitmap(bitmap);
                        Logger.d(TAG, "showFullImage()3, mCallScreenHost is null");
                    }
                ...
                mMainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (mCallScreenHost != null) {
                            ...
                            Logger.w(TAG, "showFullImage()2, mCallScreenHost is null");
                        }
                    }
                });
    ...
    private void onUserAcceptImageSharing(Context context, String fileName, long fileSize) {
		...
		maxFileSize = mImageSharingApi.getConfiguration().getWarnSize() * 1024;
		warningFileSize = mImageSharingApi.getConfiguration().getMaxSize() * 1024;
        ...
        if (fileSize >= warningFileSize && warningFileSize != 0) {
            boolean isRemind = AppSettings.getInstance().restoreRemindWarningLargeImageFlag();
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPreExecute() {
	            ...
	            if(mIncomingThumbnail != null){
					Logger.d(TAG, "ThumbnailAsyncTask view is not null");
    ...
    private class ImageSharingInvitationReceiver extends BroadcastReceiver {
        ...
        private void handleImageSharingInvitation(Context context, Intent intent) {
            ...
            mIncomingContact = intent.getStringExtra(ImageSharingIntent.EXTRA_CONTACT);
            mIncomingContactDisplayname =
                    intent.getStringExtra(ImageSharingIntent.EXTRA_DISPLAY_NAME);
            mIncomingSessionId = intent.getStringExtra(ImageSharingIntent.EXTRA_SHARING_ID);
            mIncomingImageName = intent.getStringExtra(ImageSharingIntent.EXTRA_FILENAME);
            mIncomingImageSize = intent.getLongExtra(ImageSharingIntent.EXTRA_FILESIZE, 0);
			...
			Logger.w(TAG, "handleImageSharingInvitation reject session");
                        } catch (JoynServiceException e) {
                            e.printStackTrace();
                        }
                    }
        ...
        try {
            if (mRichCallStatus == RichCallStatus.CONNECTED) {
                ...
                Logger.w(TAG, "addIncomingImageSharingListener() mIncomingImageSharingSession: "
                        + mIncomingImageSharingSession);
                if (mIncomingImageSharingSession != null) {
                    mIncomingImageSharingSession.addEventListener(mIncomingSessionEventListener);
                    success = true;
                }
            } else {
                mImageSharingStatus = ImageSharingStatus.INCOMING_BLOCKING;
            ...
            if (mIncomingImageSharingSession != null) {
                ...
                if (currentStorageSize > 0) {
                    if (receivedFileSize <= currentStorageSize) {
                        mIncomingImageSharingSession.acceptInvitation();
                        Utils.setInImageSharing(true);
                        success = true;
                    } else {
                        mIncomingImageSharingSession.rejectInvitation();
    ...
    private void startJoynChat() {
		...
		int integratedMode = RcsSettings.getInstance().getMessagingUx();
		Logger.d(TAG, "startJoynChat entry, mode is " + integratedMode);
    ...
    private void prepareStartImageSharing(final String imageName) {
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (AppSettings.getInstance().isEnabledCompressingImageFromDB()) {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (which == DialogInterface.BUTTON_POSITIVE) {
                if (mCheckRemind != null) {
                    ...
                    AppSettings.getInstance().saveRemindWarningLargeImageFlag(isCheck);
                }
                if (mFileName != null) {
