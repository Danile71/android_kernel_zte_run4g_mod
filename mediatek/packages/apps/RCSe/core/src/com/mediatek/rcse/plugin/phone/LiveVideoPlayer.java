 ...
 */
public class LiveVideoPlayer extends VideoPlayer implements Camera.PreviewCallback, RtpStreamListener {
 ...
 */
    ...
    public void open(VideoCodec codec, String remoteHost, int remotePort) {
        ...
        try {
            ...
            // Codec dimensions
            nativeH264EncoderParams.setFrameWidth(selectedVideoCodec.getVideoWidth());
            nativeH264EncoderParams.setFrameHeight(selectedVideoCodec.getVideoHeight());
            ...
            nativeH264EncoderParams.setProfilesAndLevel(selectedVideoCodec.getParameters());
            ...
            if (logger.isActivated()) {
                logger.info("Init H264Encoder " + selectedVideoCodec.getParameters() + " " +
                        selectedVideoCodec.getVideoWidth() + "x" + selectedVideoCodec.getVideoHeight() + " " +
    ...
    public int getVideoCodecWidth() {
        if (selectedVideoCodec == null) {
            ...
            return selectedVideoCodec.getVideoWidth();
        }
    }
    ...
    public int getVideoCodecHeight() {
        if (selectedVideoCodec == null) {
            ...
            return selectedVideoCodec.getVideoHeight();
        }
    }
    ...
    public void setVideoCodec(VideoCodec mediaCodec) {
        if (mediaCodec.compare(supportedMediaCodecs[0])) {            
            if (mediaCodec.getVideoHeight() == 0 || mediaCodec.getVideoWidth() == 0) {
                selectedVideoCodec = new VideoCodec(mediaCodec.getEncoding(),
                		mediaCodec.getPayloadType(),
                		mediaCodec.getClockRate()	,
                		mediaCodec.getFrameRate(),
                		mediaCodec.getBitRate(),
            ...
            videoFormat = (VideoFormat) MediaRegistry.generateFormat(mediaCodec.getEncoding());
        } else {
