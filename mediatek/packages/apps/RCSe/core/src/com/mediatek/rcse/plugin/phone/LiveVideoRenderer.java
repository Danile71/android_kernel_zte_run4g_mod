 ...
 * @author jexa7410
 */
public class LiveVideoRenderer extends VideoRenderer implements RtpStreamListener {
	...
	private void notifyPlayerEventResized(int width, int height) {
		...
		while (ite.hasNext()) {
			try {
				//((IVideoRendererListener)ite.next()).mediaResized(width, height);
			} catch (Exception e) {
				if (logger.isActivated()) {
					logger.error("Can't notify listener", e);
				}
...
//TODO check why notification causes issue . Since it only prints a log , we are ok for now.
