...
public class SharingPlugin implements ICallScreenPlugIn, CapabilitiesChangeListener {
    ...
    public SharingPlugin(final Context context) {
        ...
        mImageSharingApi = new ImageSharingService(mContext, mImageSharingApiListener);
        mVideoSharingApiListener = new VideoSharingApiListener();
        mVideoSharingApi = new VideoSharingService(mContext, mVideoSharingApiListener);
        connectRichCallApi();
    ...
    private class ImageSharingApiListener implements JoynServiceListener {
		...
		public void onServiceConnected() {
            ...
            try {
            	mImageSharingApi.addServiceRegistrationListener(listener);
            	mIsRegistered = mImageSharingApi.isServiceRegistered();
            ...
            if (mCallScreenHost != null) {
                ...
                Logger.d(TAG, "onServiceConnected mCallScreenHost is null");
            }
    ...
    private class VideoSharingApiListener implements JoynServiceListener {
        private static final String TAG = "ImageSharingApiListener";
    ...
    public void start(String number) {
        ...
        mNumber = number;
        Logger.d(TAG, "start exit myNumber" +mNumber);
    }
    ...
    public boolean isImageShareSupported(String number) {
        ...
        mNumber = number;
		Logger.w(TAG, "isImageShareSupported(), myNumber: " + mNumber);
        if (mRichCallStatus != RichCallStatus.CONNECTED) {
    ...
    public boolean isVideoShareSupported(String number) {
        ...
        mNumber = number;
		Logger.w(TAG, "isVideoShareSupported(), myNumber: " + mNumber);
        if (mRichCallStatus != RichCallStatus.CONNECTED) {
    ...
    public void onCapabilitiesChanged(String contact, ContactInformation contactInformation) {
        Logger.w(TAG, "onCapabilitiesChanged() contactInformation: " + contactInformation
    ...
    public void registerForCapabilityChange(final String number) {
        ...
        mNumber = number;
		Logger.w(TAG, "registerForCapabilityChange myNumber: " + number);
    }
    ...
    public void onPhoneStateChange(CallManager cm) {
        ...
        if (hasActiveBgCall ^ mIsOnHold) {
             ...
             numbers.add(getVodafoneAccount(mNumber));            
             PluginApiManager.getInstance().queryNumbersPresence(numbers);
    ...
    protected String getVodafoneAccount(String normalNumber) {
        Logger.w(TAG, "getVodafoneAccount() entry, normalNumber: " + normalNumber);
        String vodafoneAccount = normalNumber;
            if (normalNumber != null) {
                try {
                    String number = normalNumber;
					try {
						number = mImageSharingApi.getJoynAccountViaNumber(normalNumber);
					} catch (JoynServiceException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					mNumber = normalNumber;
					Logger.w(TAG, "getVodafoneAccount() entry, normalNumber: "  +"myNumber:" + mNumber);
                    if (number != null) {
                    ...
                    e.printStackTrace();
                    Logger.e(TAG, "getVodafoneAccount() JoynServiceException, return null");
                }
            }
