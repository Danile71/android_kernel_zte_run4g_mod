...
DialogInterface.OnClickListener, SensorEventListener{
	...
	public VideoSharingPlugin(Context context) {
		...
		intentFilter.addAction(VideoSharingIntent.ACTION_NEW_INVITATION);
			...
			if (mIncomingVideoSharingSession != null) {
				try {
					if (mIncomingVideoSharingSession.getState() == ESTABLISHED
							|| mIncomingVideoSharingSession.getState() == PENDING) {
						Resources resources = mContext.getResources();
						String message = resources
								.getString(R.string.now_allowed_video_share_by_network);
						showToast(message);
					...
					if (mShareStatus == ShareStatus.LIVE_OUT
						...
						if (vodafoneAccount != null) {
							...
							mOutgoingVideoSharingSession =
									mVideoSharingApi.shareVideo(TEL_URI_SCHEMA
											+ getVodafoneAccount(mNumber), mOutgoingVideoPlayer,mOutgoingSessionEventListener);
						} else {
							Logger.w(TAG, "vodafoneAccount is null");
						}
					} 
					if (mOutgoingVideoSharingSession != null) {
						Logger.w(TAG, "mOutgoingVideoSharingSession started.");
					} else {
	 ...
	 */
	//private IVideoSharingListener mOutgoingSessionEventListener = new IVideoSharingListener.Stub() 
	 private class OutgoingSessionEventListener extends VideoSharingListener{
		private static final String TAG = "OutgoingSessionEventListener";

		// Session is started
		public void onSharingStarted() {
			Logger.v(TAG, "onSharingStarted(), mShareStatus: " + mShareStatus
				...
				if (mOutgoingVideoSharingSession != null) {
					if (is3GMobileNetwork()) {
						if (mIncomingVideoSharingSession != null) {
							...
							Logger.v(TAG, "onSharingStarted(),Entered in else of mIncomingVideoSharingSession");
							// Tell the host to update & show toast							
							showRemoteAcceptMessage();
						}
					} else {
						Logger.v(TAG, "onSharingStarted(),Entered in else of 3g");
						// Tell the host to update & show toast						
						showRemoteAcceptMessage();
					}
				} else {
	 ...
	 */
	//private IVideoSharingListener mIncomingSessionEventListener = new IVideoSharingListener.Stub() 
	private class IncomingSessionEventListener extends VideoSharingListener{
		...
		public void onSharingAborted() {
			...
			mMainHandler.post(new Runnable() {
				...
				public void run() {
					if (mIsVideoSharingSender.get()) {
						Logger.v(TAG, "handleSessionAborted() sender true");
						removeVeiwsAtReceiverTerminatedByRemote();
					}
					else
					{
						Logger.v(TAG, "handleSessionAborted() sender false");
				RCSeInCallUIExtension.getInstance().resetDisplayArea();
						mVideoSharingDialogManager.showTerminatedByNetworkDialog();
					}
			}
		...
		public void onSharingError(final int error) {
			...
			mMainHandler.post(new Runnable() {
				...
				public void run() {					
					if (!mIsVideoSharingSender.get())
					{
						Logger.v(TAG, "handleSharingError(), mIsVideoSharingSender false entry");
						RCSeInCallUIExtension.getInstance().resetDisplayArea();
			mVideoSharingDialogManager.showTerminatedByRemoteDialog();
		}
				}
	...
	private void finishReceiverTwoWayLocal()
	{
		...
		new AsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				...
				if (mIncomingVideoSharingSession != null) {
					try {
						mIncomingVideoSharingSession.removeEventListener(mIncomingSessionEventListener);
						mIncomingVideoSharingSession.abortSharing();
					} catch (JoynServiceException e) {
						e.printStackTrace();
					} finally {
	...
	public void finishSenderTwoWaylocal()
	{
		...
		new AsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				...
				if (mOutgoingVideoSharingSession != null) {
					try {
						mOutgoingVideoSharingSession.removeEventListener(mOutgoingSessionEventListener);
			...
			AsyncTask.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						mIncomingVideoSharingSession.rejectInvitation();
					} catch (JoynServiceException e) {
						e.printStackTrace();
					} finally {
						mIncomingVideoSharingSession = null;
	...
	private void handleVideoSharingInvitation(Context context, Intent intent) {
		...
		String mIncomingContact = intent.getStringExtra(VideoSharingIntent.EXTRA_CONTACT);
		// Display invitation dialog
		mIncomingSessionId = intent.getStringExtra(VideoSharingIntent.EXTRA_SHARING_ID);
		mIncomingVideoFormat = intent.getStringExtra(VideoSharingIntent.EXTRA_ENCODING);
		mVideoDuration = intent.getLongExtra("videosize", -1);
		Logger.v(TAG, "handleVideoSharingInvitation Received Duration is()"+ mVideoDuration + "number is" + mIncomingContact +" & mNumber is" + mNumber);
				...
				if (is3GMobileNetwork() && mOutgoingVideoSharingSession != null) {
					try {
						if (mOutgoingVideoSharingSession.getState() == ESTABLISHED
								|| mOutgoingVideoSharingSession.getState() == PENDING) {
	...
	private void handleFirstReceiveInvitation() {
		...
		try {
			...
			mIncomingVideoSharingSession.acceptInvitation(mIncomingRemoteVideoRenderer);
	...
	private void handleSecondReceiveInvitation() {
		...
		try {
			...
			mIncomingVideoSharingSession.acceptInvitation(mOutgoingRemoteVideoRenderer);
		} catch (JoynServiceException e) {
			e.printStackTrace();
		}
	}
	...
	private void getIncomingVideoSharingSession() {
		...
		try {
			mIncomingVideoSharingSession = mVideoSharingApi.getVideoSharing(mIncomingSessionId);
			if (mIncomingVideoSharingSession != null) {
				mIncomingSessionEventListener = new IncomingSessionEventListener();
				mIncomingVideoSharingSession.addEventListener(mIncomingSessionEventListener);
			} else {
				Logger.w(TAG, "mIncomingVideoSharingSession is null");
			}
		} catch (JoynServiceException e) {
