...
public class PluginApiManager {
    ...
    private IRemoteBlockingRequest coreApi = null;
    ...
    public boolean blockContact(String contact, boolean status) throws RemoteException
    {
        ...
        Logger.d(TAG, "blockContact() entry with status "+ status +"coreApi = "+coreApi );
        boolean isBlockedSuccess = false;
        if(coreApi != null)
        {
            isBlockedSuccess = coreApi.blockContact(contact, status);
        }
    ...
    {
        //call to ApiService through aidl
        Logger.d(TAG, "getBlockedStatus() entry "+coreApi);
        boolean isBlockedAlready = false;
        if(coreApi != null)
        {
            isBlockedAlready = coreApi.getBlockedStatus(contact);
        }
    ...
    private ServiceConnection apiConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            coreApi = IRemoteBlockingRequest.Stub.asInterface(service);
            Logger.d(TAG, "service connecting for blocking request " + coreApi);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, final Intent intent) {
            ...
            for (CapabilitiesChangeListener listener : mCapabilitiesChangeListenerList) {
            	...
            	mCapabilitiesApi = new CapabilityService(mContext, new MyJoynServiceListener());
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (mCapabilitiesApi == null) {
                    ...
                    ContactInformation info = new ContactInformation();
                    if (capabilities == null) {
                        capabilities = new Capabilities(false, false, false, false, false, false, false, null, false, false, false, false, false);
                    }
                    info.isRcsContact = capabilities.isSupportedRcseContact() ? 1 : 0;
                    info.isImSupported = capabilities.isImSessionSupported();
                    info.isFtSupported = capabilities.isFileTransferSupported();
    ...
    private PluginApiManager(Context context) {
        ...
        boolean result =
            mContext.bindService(new Intent(IRemoteBlockingRequest.class.getName()),
                    apiConnection, Context.BIND_AUTO_CREATE);
    ...
    public class MyJoynServiceListener implements JoynServiceListener{
		...
		public void onServiceConnected() {
			try{
				...
				PluginApiManager.this.mCapabilitiesApi.addCapabilitiesListener(mMyCapabilitiesListener);
				}
				catch (JoynServiceException e) {
					e.printStackTrace();
				}
		...
		public void onServiceDisconnected(int error) {
			try{
				...
				{
				PluginApiManager.this.mCapabilitiesApi.removeCapabilitiesListener(mMyCapabilitiesListener);
