...
public class RcsNotification {
    ...
    public interface GroupInviteChangedListener {
         ...
         */
        void onAddedGroupInvite(final GroupChat chatSession, final Intent intent);
    ...
    private void updateTempGroupInvitationInfos(String chatId) {
        Logger.d(TAG, "updateTempGroupInvitationInfos() entry, chatId: " + chatId + " size: "
                + mTempGroupInvitationInfos.size());
        try {
            for (Map.Entry<GroupChat, Intent> entry : mTempGroupInvitationInfos.entrySet()) {
     ...
     */
    private synchronized void addGroupInvite(GroupChat chatSession, GroupInvitationInfo info) {
    ...
    private Chat getChatSession(Intent intent) {
        ...
        String sessionId = intent.getStringExtra(ChatIntent.EXTRA_CONTACT);
        if (instance == null) {
		...
		try {
			...
			if(totalChats != null){
			Logger.w(TAG, "aaa getChatSession size: " + totalChats.size());
			Logger.d(TAG, "The chat session is null3 : " + totalChats.size());
			for(Chat setElement: totalChats) {
		        if(setElement.getRemoteContact().equals(sessionId)) {
    ...
    private FileTransfer getFileTransferSession(String transferId) {
        ...
        try {
            ...
            Logger.e(TAG, "getFileTransferSession() Get file session failed");
            e.printStackTrace();
    ...
    private void handlePluginGroupChatInvitation(Context context, Intent invitation, boolean autoAccept) {
        ...
        String groupSubject = invitation.getStringExtra(GroupChatIntent.EXTRA_SUBJECT);
		Logger.v(TAG, "handlePluginGroupChatInvitation() subject: "+ groupSubject);
        ...
        // Add mms db
        String contact = PluginGroupChatWindow.generateGroupChatInvitationContact(chatId);
        if(autoAccept)
        {
    ...
    private void autoAcceptGroupChat(Context context, Intent invitation) {
        ...
        ChatMessage msg = invitation.getParcelableExtra(FIRST_MESSAGE);
        if (msg != null) {
            messages.add(msg);
        }
        invitation.putParcelableArrayListExtra(MESSAGES, messages);
        ModelImpl.getInstance().handleNewGroupInvitation(invitation, false);
    ...
    private GroupInvitationInfo buildNotificationInfo(Context context, Intent invitation) {
        ...
        ArrayList<ChatMessage> messages = new ArrayList<ChatMessage>();
        messages.add((ChatMessage) invitation.getParcelableExtra(FIRST_MESSAGE));
        invitation.putParcelableArrayListExtra(MESSAGES, messages);
        ...
        try {
            participants = new ArrayList<String>(chatSession.getAllParticipants());
			if (participants != null) {
				Logger.e(TAG, "buildNotificationInfo getAllParticipants" + participants.size());
			}
    ...
    private void showFileTransferNotification(Context context, Intent invitation,
        ...
        String sessionId = invitation.getStringExtra(FileTransferIntent.EXTRA_TRANSFER_ID);
    ...
    public static synchronized void handleInvitation(Context context, Intent intent) {
        ...
        if (ChatIntent.ACTION_NEW_CHAT.equalsIgnoreCase(action)) {
            Chat chatSession = RcsNotification.getInstance().getChatSession(intent);
            if (chatSession == null) {
            ...
            try {
				if (!(intent.getBooleanExtra("isGroupChatExist", false))) {
					boolean autoAccept = intent.getBooleanExtra(AUTO_ACCEPT,
							false);
					RcsNotification.getInstance().handleGroupChatInvitation(
							context, intent, autoAccept);
				}
			} catch (Exception e) {
        ...
        else if (ImageSharingIntent.ACTION_NEW_INVITATION.equalsIgnoreCase(action)) {
            handleImageSharingInvitation(context, intent);
        } else if (VideoSharingIntent.ACTION_NEW_INVITATION.equalsIgnoreCase(action)) {
    ...
    private static void handleChatInvitation(Context context, Intent invitation) {
        ...
        invitation.putParcelableArrayListExtra(FIRST_MESSAGE, messages);
        ModelImpl.getInstance().handleO2OInvitation(invitation, false);
    ...
    public void onReceiveMessageInBackground(Context context, Object newTag,
            ChatMessage message, List<Participant> participantList, final int id) {
    ...
    public void onReceiveMessageInBackground(Context context, Object newTag,
        ...
        Logger.d(TAG,
                "onReceiveMessageInBackground(), the newTag is " + newTag + " isStoreAndFoward "
                        + isStoreAndFoward + "messageid is " + message.getId());
    ...
    private static String getRemoteContact(ChatMessage message) {
        Logger.d(TAG, "formatRemote() entry, message.getRemote() is " + message.getContact());
