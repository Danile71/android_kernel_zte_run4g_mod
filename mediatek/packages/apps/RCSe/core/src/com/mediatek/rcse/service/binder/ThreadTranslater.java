...
public final class ThreadTranslater {
    ...
    public static void putTagIntoDB(Long threadId, String tag)
    {
         ...
         values.put(IntegratedMessagingData.KEY_INTEGRATED_MODE_THREAD_ID, threadId);
   	     values.put(IntegratedMessagingData.KEY_INTEGRATED_MODE_TAG, tag);	     
   	     
         try {
 			Uri uri = AndroidFactory.getApplicationContext().getContentResolver().insert(IntegratedMessagingData.CONTENT_URI_INTEGRATED_TAG, values);
 		} catch (Exception e) {
    ...
    private static String getTagByID(long threadID){
   	 ...
   	 try {
			cursor = AndroidFactory.getApplicationContext().getContentResolver().
			            query(IntegratedMessagingData.CONTENT_URI_INTEGRATED_TAG, null, 
			...
			if(cursor!=null && cursor.moveToFirst()){
				...
				tag = cursor.getString(cursor.getColumnIndex(IntegratedMessagingData.KEY_INTEGRATED_MODE_TAG));
			}
