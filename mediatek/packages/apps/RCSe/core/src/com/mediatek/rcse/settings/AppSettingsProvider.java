...
public class AppSettingsProvider extends ContentProvider {
    ...
    static {
        uriMatcher.addURI("com.orangelabs.rcs.appsettings", "appsettings", APP_SETTINGS);
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
        	db.execSQL("CREATE TABLE " + TABLE + " ("
        			+ AppSettingsData.KEY_ID + " integer primary key autoincrement,"
            ...
            addParameter(db, AppSettingsData.CSH_INVITATION_VIBRATE, 			AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.FILETRANSFER_INVITATION_RINGTONE, 	"");
            addParameter(db, AppSettingsData.FILETRANSFER_INVITATION_VIBRATE, 	AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.CHAT_INVITATION_RINGTONE, 			"");
            addParameter(db, AppSettingsData.CHAT_INVITATION_VIBRATE, 			AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.ENABLE_AUTO_ACCEPT_FT_ROMING,      AppSettingsData.FALSE);
            addParameter(db, AppSettingsData.ENABLE_AUTO_ACCEPT_FT_NOROMING,    AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.RCSE_COMPRESSING_IMAGE,            AppSettingsData.FALSE);
            addParameter(db, AppSettingsData.COMPRESS_IMAGE_HINT,               AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.WARNING_LARGE_IMAGE_HINT,          AppSettingsData.TRUE);
            addParameter(db, AppSettingsData.RCSE_CHAT_WALLPAPER,               "");
            addParameter(db, AppSettingsData.JOYN_MESSAGING_DISABLED_FULLY_INTEGRATED,AppSettingsData.FALSE);
        ...
        private void addParameter(SQLiteDatabase db, String key, String value) {
            String sql = "INSERT INTO " + TABLE + " (" +
            	AppSettingsData.KEY_KEY + "," +
            	AppSettingsData.KEY_VALUE + ") VALUES ('" +
            	key + "','" + value + "');";
            db.execSQL(sql);
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        	...
        	while(oldDataCursor.moveToNext()){
        		String key = null;
        		String value = null;
        		int index = oldDataCursor.getColumnIndex(AppSettingsData.KEY_KEY);
        		if (index!=-1) {
        	...
        	for (int i=0; i<valuesList.size();i++) {
        		...
        		String whereClause = AppSettingsData.KEY_KEY + "=" + "\""+ values.getAsString(AppSettingsData.KEY_KEY) + "\"";
