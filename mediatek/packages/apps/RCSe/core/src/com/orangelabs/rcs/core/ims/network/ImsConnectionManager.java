...
public class ImsConnectionManager implements Runnable {
	...
	//volte_debug_flag to enable and test volte coe
	//private boolean volte_debug = true;
 ...
 /**
  ...
  /**
	...
	public ImsConnectionManager(ImsModule imsModule) throws CoreException {
		...
		//RcsUaAdapter.createInstance(AndroidFactory.getApplicationContext());
...
/**
        ...
        //AndroidFactory.getApplicationContext().registerReceiver(volteServiceListner, new IntentFilter(RcsUaAdapter.VOLTE_SERVICE_NOTIFY_INTENT));
	}
...
if (imsActivationFlag) {
    ...
    private BroadcastReceiver volteServiceListner = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
        	if (logger.isActivated()) {
        		logger.debug("volteServiceListner : call connection event");
        	}
        	 // Reconnect with a connection event
            connectionEvent(new Intent(ConnectivityManager.CONNECTIVITY_ACTION));
