...
public class SipMessageFactory {
	 ...
	 * @throws SipException
	 */
    public static SipRequest createCpimSubscribe(SipDialogPath dialog, int expirePeriod) throws SipException {
		try {
			if (logger.isActivated()) {
			...
			Header preferredServiceHeader = SipUtils.HEADER_FACTORY.createHeader(SipUtils.HEADER_P_PREFERRED_SERVICE, FeatureTags.FEATURE_CPM_SESSION);
			subscribe.addHeader(preferredServiceHeader);
	 ...
	 * @throws SipException
	 */
	public static SipRequest createCpimMessage(SipDialogPath dialog, String contentType, String content) throws SipException {
		return createMessage(dialog, null, contentType, content.getBytes());
	}
	 ...
	 * @throws SipException
	 */
	public static SipRequest createCpimMessage(SipDialogPath dialog, String featureTag, String contentType, byte[] content) throws SipException {
		try {			
			 if (logger.isActivated()) {
     ...
     * @throws SipException
     */
    public static SipRequest createCpmInvite(SipDialogPath dialog,	String[] featureTags, String sdp) throws SipException {
		try {
			if (logger.isActivated()) {
			     logger.info("ABCH createCpmInvite: target" + dialog.getTarget());
			 }
     ...
     * @throws SipException
     */
    public static SipRequest createCpmMultipartInvite(SipDialogPath dialog,
    		String[] featureTags,
			String multipart,
			String boundary)
            throws SipException {
     ...
     * @throws SipException
     */
    public static SipRequest createCpimInvite(SipDialogPath dialog,
    		String[] featureTags,
			String content,
    ...
    public static SipRequest createInvite(SipDialogPath dialog,
		...
		try {
			...
			if (dialog.getSessionExpireTime() >= SessionTimerManager.MIN_EXPIRE_PERIOD) {			
				...
				Header min_se = SipUtils.HEADER_FACTORY.createHeader(SipUtils.HEADER_MIN_SE," "+SessionTimerManager.MIN_EXPIRE_PERIOD);
	...
	public static SipResponse create200OkInviteResponse(SipDialogPath dialog, String[] featureTags, String sdp) throws SipException {
		try {
			...
			if (logger.isActivated()) {
				logger.info("ABC sesion timer CREATE INVITE response " + dialog.getSessionExpireTime());
			}
	 ...
	 * @throws SipException
	 */
	public static SipResponse createCpm200OkInviteResponse(SipDialogPath dialog, String[] featureTags, String sdp) throws SipException {
		try {
			if (logger.isActivated()) {
		     	logger.info("ABCH createCpm200OkInviteResponse: target" + dialog.getSipStack().getContact());
			 }
			// Create the response
			Response response = SipUtils.MSG_FACTORY.createResponse(200, (Request)dialog.getInvite().getStackMessage());
	 ...
	 * @throws SipException
	 */
    public static SipRequest createCpimRefer(SipDialogPath dialog, String toContact, String subject, String contributionId) throws SipException {
		try {			
			// Create the request
	 ...
	 * @throws SipException
	 */
    public static SipRequest createCpimRefer(SipDialogPath dialog, List<String> participants, String subject, String contributionId) throws SipException {
    	try {
			// Create the request
		    Request refer = dialog.getStackDialog().createRequest(Request.REFER);
