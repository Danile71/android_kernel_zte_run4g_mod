...
/*M: 0025-comment-IOT-ID_RCS_6_10_1.patch 
...
/*@*/
    ...
    public static Capabilities extractCapabilities(SipMessage msg) {
    	...
    	for(int i=0; i < tags.size(); i++) {
    		...
    		if (tag.contains(FeatureTags.FEATURE_3GPP_VIDEO_SHARE)) {
    				...
    				|| tag.contains(FeatureTags.FEATURE_3GPP_ICSI_MMTEL_VIDEO)) { // @tct-stack-[IOT][ID_RCS_6_10_2]modified by fang.wu@tcl.com) 
    				*/
            ...
            if (tag.contains(FeatureTags.FEATURE_RCSE_INTEGERATED_IM)) {
	...
	public static void updateExternalSupportedFeatures(Context context) {
		try {
			...
			String mime = org.gsma.joyn.capability.CapabilityService.EXTENSION_MIME_TYPE + "/*"; 
			intent.setType(mime);			
		...
		if (richcall) {
	        ...
	        if (video | image) {
		        ...
		        if (video) {
					...
					/* M : 
					// @tct-stack-[IOT][ID_RCS_6_10_1]PR 619465-modified by fang.wu@tcl.com for begin
					+                    MediaCodec[] supportedCodecs = CodecsUtils.getPlayerCodecList();
					...
					+                        videoSharingConfig.append("m=video 0 RTP/AVP " + fmt.getPayload() + SipUtils.CRLF);
					+                        videoSharingConfig
					+                                .append("a=rtpmap:" + fmt.getPayload() + " " + fmt.getCodec() + SipUtils.CRLF);
					+                        for (VideoCodec codec : codecs) {
					+                            videoSharingConfig.append("a=fmtp:" + codec.getPayload() + " " + codec.getCodecParams()
					+                                    + SipUtils.CRLF);
					+                        }
					+                    }
					+                    // @tct-stack-[IOT][ID_RCS_6_10_1]PR 619465-modified by fang.wu@tcl.com end
