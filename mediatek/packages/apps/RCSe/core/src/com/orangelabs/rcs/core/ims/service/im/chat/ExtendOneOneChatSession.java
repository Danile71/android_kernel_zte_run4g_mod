...
public class ExtendOneOneChatSession extends GroupChatSession {
	...
	private SipRequest createInviteRequest(String content) throws SipException {
		...
		if(!RcsSettings.getInstance().isCPMSupported()){
			...
			if (logger.isActivated()) {
	    		logger.info("CPMS createInviteRequest ExtendO2O 0");
	    	}
       ...
       /**
		...
		if(RcsSettings.getInstance().isCPMSupported()){
			if (logger.isActivated()) {
	    		logger.info("CPMS createInviteRequest ExtendO2O 1");
	    	}
			if(getConversationID() != null){
				invite.addHeader(ChatUtils.HEADER_CONVERSATION_ID, getConversationID());
			}
			if(oneoneSession.getContributionID() != null){
				invite.addHeader(ChatUtils.HEADER_INREPLY_TO_CONTRIBUTION_ID, oneoneSession.getContributionID());
			}
		}
