...
public class ImdnManager extends Thread {
		...
		while((delivery = (DeliveryStatus)buffer.getObject()) != null) {
			try {
				if(!RcsSettings.getInstance().isCPMSupported()){
					if (logger.isActivated()) {
						logger.info("Start background processing NOCPIM");
					}
				// Send SIP MESSAGE
				sendSipMessageDeliveryStatus(delivery, null); // TODO: add sip.instance
					...
					if (logger.isActivated()) {
						logger.info("Start background processing CPIMS");
					}
					sendCpimSipMessageDeliveryStatus(delivery, null);
				}
        ...
        Thread thread = new Thread(){
            public void run() {
                ...
                sendSipMessageDeliveryStatus(delivery, remoteInstanceId);
					...
					if (logger.isActivated()) {
						logger.info("Start background processing CPMS");
					}
            		sendCpimSipMessageDeliveryStatus(delivery, remoteInstanceId);
            	}
            }
        };
        thread.start();
	 ...
	 */
	private void sendCpimSipMessageDeliveryStatus(DeliveryStatus deliveryStatus, String remoteInstanceId) {
		try {
			...
			if (logger.isActivated()) {
       			logger.debug("sendCpimSipMessageDeliveryStatus " + deliveryStatus.getStatus() + " for message " + deliveryStatus.getMsgId());
       		}
			...
			if(true){
				if (logger.isActivated()) {
            		logger.info("send CPIM Message1");
            	}
	            msg = SipMessageFactory.createCpimMessage(dialogPath,
	        		FeatureTags.FEATURE_OMA_IM, CpimMessage.MIME_TYPE, cpim.getBytes());
			}
			else{
				 msg = SipMessageFactory.createMessage(dialogPath,
	        		FeatureTags.FEATURE_CPM_MSG, CpimMessage.MIME_TYPE, cpim.getBytes());
			}
            ...
            if (ctx.getStatusCode() == 407) {
     	       ...
     	       if(false){
					...
					if (logger.isActivated()) {
            			logger.info("send CPIM Message2");
            		}
 	 				 msg = SipMessageFactory.createCpimMessage(dialogPath,
 	 	        		FeatureTags.FEATURE_CPM_MSG, CpimMessage.MIME_TYPE, cpim.getBytes());
 	 			}
...
/**
	...
	private void sendSipMessageDeliveryStatus(DeliveryStatus deliveryStatus, String remoteInstanceId) {
		try {
			...
			if (logger.isActivated()) {
       			logger.info("Send delivery status " + deliveryStatus.getStatus() + " for message " + deliveryStatus.getMsgId());
       		}
