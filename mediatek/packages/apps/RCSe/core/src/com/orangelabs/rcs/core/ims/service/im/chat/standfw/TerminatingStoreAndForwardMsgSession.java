...
public class TerminatingStoreAndForwardMsgSession extends OneOneChatSession implements MsrpEventListener {
	...
	public TerminatingStoreAndForwardMsgSession(ImsService parent, SipRequest invite) {
		...
		// Set feature tags
			setFeatureTags(ChatUtils.getSupportedFeatureTagsForGroupChat());
			 ...
			 if (logger.isActivated()) {
				logger.info("TerminatingStoreAndForwardMsgSession  CPMS");
			 }
			// Set feature tags
		...
		if(RcsSettings.getInstance().isCPMSupported()){
			if (logger.isActivated()) {
				logger.info("TerminatingStoreAndForwardMsgSession1  CPMS");
			 }
	...
	public void run() {
		try {
			...
			if(!RcsSettings.getInstance().isCPMSupported()){
				 ...
				 if (logger.isActivated()) {
					logger.info("TerminatingStoreAndForwardMsgSession2  CPMS");
				 }
			...
			if(RcsSettings.getInstance().isCPMSupported()){
				if (logger.isActivated()) {
					logger.info("TerminatingStoreAndForwardMsgSession3  CPMS");
				 }
