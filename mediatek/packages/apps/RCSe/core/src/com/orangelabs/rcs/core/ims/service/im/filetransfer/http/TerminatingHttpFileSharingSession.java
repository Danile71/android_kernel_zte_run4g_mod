...
public class TerminatingHttpFileSharingSession extends HttpFileTransferSession implements HttpTransferEventListener {
    ...
    public TerminatingHttpFileSharingSession(ImsService parent, ChatSession chatSession,
            FileTransferHttpInfoDocument fileTransferInfo, String msgId,String displayName,String remote) {
        super(parent, ContentManager.createMmContentFromFilename(fileTransferInfo.getFilename(),
                ...
                remote, null, chatSession.getSessionID(),
		...
		if(displayName != null){
			if (logger.isActivated()) {
               logger.info("displayName is not null" + displayName);
            }
        	setDisplayName(displayName);
		}
		else{
			if (logger.isActivated()) {
               logger.info("displayName is  null" + getRemoteDisplayName());
            }
			setDisplayName(getRemoteDisplayName());
		}
		...
		if (logger.isActivated()) {
	   		logger.error("lemghth of thumnail file is: " + getThumbnail().length + "Thumnail url: " + getThumbUrl());
	   	}
      }
    }
