...
public class HttpsProvisioningSMS {
    ...
    public void registerSmsProvisioningReceiver(final String smsPort, final String requestUri,
                            ...
                            if(smsData.contains(HttpsProvisioningUtils.RESET_CONFIG_SUFFIX)) {
                                ...
                                Thread t = new Thread() {
                                    public void run() {
                                    	...
                                    	/**M : added to retaing the MSISDN value in case of 
                                    	 *  SMS based PUSH provisioning
                                    	 */
                                    	String currentMSISDN = "";
                                    	currentMSISDN = RcsSettings.getInstance().getMsisdn();
                                    	
                                    	LauncherUtils.resetRcsConfig(context);
                                    	
                                    	RcsSettings.getInstance().setMsisdn(currentMSISDN);
                                    	/**
                                    	 * @
                                    	 */
        ...
        if (logger.isActivated()) {
            logger.debug("Registered SMS provider receiver in port: " + smsPort);
        }
    }
