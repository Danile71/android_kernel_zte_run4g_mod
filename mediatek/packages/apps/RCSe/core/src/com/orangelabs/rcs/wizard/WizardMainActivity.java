...
public class WizardMainActivity extends Activity {
   ...
   private static Timer WizardCloseTimer = null;
   
   
   private void cancelWizardCloseTimer(){
	   if(WizardCloseTimer != null){
		   WizardCloseTimer.cancel();
		   WizardCloseTimer = null;
	   }
   }
   private void enableWizardCloseTimer(){
	   
	   
	   if (logger.isActivated()) {
  			logger.debug("enableWizardCloseTimer");
  	   }
	   ...
	   cancelWizardCloseTimer();
	   
	   WizardCloseTimer = new Timer();
	   
	   WizardCloseTimer.schedule(new TimerTask() {
           public void run() {
        	   ...
        	   try {
        		   ...
        		   sendMessageToService(RcsWizardManagerService.MSG_CLOSE_TIMER);
        	   ...
        	   WizardCloseTimer.cancel(); // also just top the timer thread, otherwise, you may receive a crash report
               WizardCloseTimer = null;
 			...
 			// status = stopService(new Intent(this,RcsWizardManagerService.class));
 			   ...
 			   //showToastMessage("Aborting Joyn Configuration. Server too busy.Please Try later");
  ...
  protected void onResume() {
   ...
   protected void onCreate(Bundle savedInstanceState) {
	...
	private void showToastMessage(){
		 String message = getString(R.string.joyn_wizard_timeout_string);
   ...
   private class WizardUIhandler extends Handler{
   	public void handleMessage(Message msg) {
		  ...
		  return ; 
		  case RcsWizardManagerService.MSG_PROVISON_SUCCESS:  
			   enableWizardCloseTimer();
	      ...
	      return ;
           */
		  case RcsWizardManagerService.MSG_CLOSE_TIMER:  
