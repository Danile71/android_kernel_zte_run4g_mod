...
//    static public final String NOTIFICATION_TITLE = "notification_title";
    ...
    private String[] mIccId = new String[PhoneConstants.GEMINI_SIM_NUM];
...
//                }
    ...
    private void handleCmd(CatCmdMessage cmdMsg, int sim_id) {
        ...
        case DISPLAY_TEXT:
            ...
            if (optr != null && "OP02".equals(optr)) {
                 ...
                 } else if (sim_id == PhoneConstants.GEMINI_SIM_2){
                     resId = R.string.appII_name;
                 } else if (sim_id == PhoneConstants.GEMINI_SIM_3) {
        ...
        case SET_UP_MENU:
            ...
            try {
                ...
                if (optr != null && "OP02".equals(optr)) {
                    ...
                    CatLog.d(LOGTAG, "StkAppService - simState[" + simState + "][" + sim_locked + "]");                            
        ...
        case CALLCTRL_RSP_MSG:
            ...
            CatLog.d(LOGTAG, "[CALLCTRL_RSP_MSG]: text " + msg.text);
            displayAlphaIcon(msg, sim_id);
            waitForUsersResponse = false;
            break;
        }
    ...
    private void delayToCheckIdle(int sim_id) {
        CatLog.d(LOGTAG, "delayToCheckIdle, sim id: " + sim_id);
    ...
    private void delayToCheckIccid(int time) {
        CatLog.d(LOGTAG, "delayToCheckIccid, mServiceHandler: " + mServiceHandler);
        synchronized (mLock) {
            if (mServiceHandler == null) {
                try {
                    mLock.wait();
                } catch (InterruptedException e) {
                    CatLog.d(LOGTAG, "Wait for service handler.");
                }
            }
        }
        Message msg1 = mServiceHandler.obtainMessage();
    ...
    private void launchTextDialog(int sim_id) {
        CatLog.d(LOGTAG, "launchTextDialog, sim id: " + sim_id +
                ", mDelayToCheckTime: " + mStkContext[sim_id].mDelayToCheckTime);
        if (canShowTextDialog(mStkContext[sim_id].mCurrentCmd.geTextMessage(), sim_id) == false) {
            if (0 >= DELAY_TO_CHECK_IDLE_TIME || DELAY_TO_CHECK_NUM <= mStkContext[sim_id].mDelayToCheckTime) {
                mStkContext[sim_id].mDelayToCheckTime = 0;
                CatLog.d(LOGTAG, "launchTextDialog responseNeeded: " +
                mStkContext[sim_id].responseNeeded);
                if (mStkContext[sim_id].responseNeeded) {
                ...
                mStkContext[sim_id].mDelayToCheckTime++;
                delayToCheckIdle(sim_id);
            }
            return;
        }
        mStkContext[sim_id].mDelayToCheckTime = 0;
...
//                    PendingIntent.getActivity(mContext, 0, notificationIntent, 0));
    ...
    private void startPollingIccId() {
        ...
        for(simIdx = PhoneConstants.GEMINI_SIM_1; simIdx < PhoneConstants.GEMINI_SIM_NUM; simIdx++) {
            mIccId[simIdx]= SystemProperties.get(PROPERTY_ICCID_SIM[simIdx]);
        }

        for(simIdx = PhoneConstants.GEMINI_SIM_1; simIdx < PhoneConstants.GEMINI_SIM_NUM; simIdx++) {          
            CatLog.d(LOGTAG, "mIccId" +(simIdx + 1) + ":[" + mIccId[simIdx] + "]");
            if((mIccId[simIdx] == null) || "".equals(mIccId[simIdx])) {// || !((PhoneProxy)mPhone[simIdx]).isRadioAvailable()){
                needWait = true;
                break;
            }
        }
        ...
        CatLog.d(LOGTAG, "polling end.");
        mInitializeWaitCounter = 0;
        StkAppInstaller appInstaller = StkAppInstaller.getInstance();
        for(simIdx = PhoneConstants.GEMINI_SIM_1; simIdx < PhoneConstants.GEMINI_SIM_NUM; simIdx++) {
            if (ICCID_STRING_FOR_NO_SIM.equals(mIccId[simIdx])) {
                CatLog.d(LOGTAG, "Iccid is N/A, uninstall sim[" + simIdx + "]");
