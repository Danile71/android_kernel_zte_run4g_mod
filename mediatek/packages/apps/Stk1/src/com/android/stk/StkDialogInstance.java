...
public class StkDialogInstance {
    ...
    protected void handleOnCreate(){
        if (sOperatorSpec.equals("OP01")) {
            CatLog.d(LOGTAG, "OnCreate, sim id: " + mSimId + " OP01 package");
            mCtx = this.parent.getApplicationContext();
            if (mAlarmManager == null) {
                CatLog.d(LOGTAG, "get mAlarmManager");            
                mAlarmManager = (AlarmManager)mCtx.getSystemService(Context.ALARM_SERVICE);
            }
            if (mTimeoutReceiver == null) {
                CatLog.d(LOGTAG, "new TimeoutReceiver"); 
                mTimeoutReceiver = new TimeoutReceiver();
            }
            IntentFilter filter = new IntentFilter(ACTION_DIALOG_TIMEOUT);
            CatLog.d(LOGTAG, "registerReceiver"); 
            mCtx.registerReceiver(mTimeoutReceiver, filter);
        } else {
            CatLog.d(LOGTAG, "OnCreate, sim id: " + mSimId + " create timeoutThread");
    ...
    protected void handleOnDestroy()
    {
        ...
        cancelTimeOut();
        if (sOperatorSpec.equals("OP01")) {
            mCtx.unregisterReceiver(mTimeoutReceiver);
    ...
    protected void cancelTimeOut() {
        ...
        if (sOperatorSpec.equals("OP01")) {
            if (mTimeOutIntent != null) {
                CatLog.d(LOGTAG, "mAlarmManager cancel");
    ...
    protected void startTimeOut(boolean waitForUserToClear) {
        ...
        if(mTextMsg.userClear == true && mTextMsg.responseNeeded == false) {
            ...
            if (sOperatorSpec.equals("OP01")) {
                ...
                mTimeOutIntent = PendingIntent.getBroadcast(mCtx, 0, timeoutIntent, 0);
                CatLog.d(LOGTAG, "mAlarmManager.set");
                mAlarmManager.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis()
                        + dialogDuration, mTimeOutIntent);
            } else {
                CatLog.d(LOGTAG, "sendMessageDelayed");
    ...
    private class TimeoutHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
            case MSG_ID_TIMEOUT:
                CatLog.d(LOGTAG, "mTimeoutHandler handle timeout");
    ...
    private class TimeoutReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            CatLog.d(LOGTAG, "TimeoutReceiver onReceive");
            if (action.equals(ACTION_DIALOG_TIMEOUT)) {
                CatLog.d(LOGTAG, "TimeoutReceiver handle timeout");
                sendResponse(StkAppService.RES_ID_TIMEOUT);
                parent.finish();
            }
        }
