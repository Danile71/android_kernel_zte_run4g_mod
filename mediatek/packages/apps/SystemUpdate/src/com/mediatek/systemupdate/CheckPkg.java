...
class CheckPkg {
    ...
    private static final String PARTITION_FILE_PATH_NEW = "/proc/partinfo";
    ...
    private boolean comparePartition(List<PartitionInfo> partitionDev, List<PartitionInfo> partitionPkg, int otaType) {
        ...
        Xlog.i(TAG, "comparePartition partitionDev.size() = " + length);
        Xlog.i(TAG, "comparePartition partitionPkg.size() = " + partitionPkg.size());
    ...
    private boolean getPartitionInfoDev(List<PartitionInfo> partitionDev) {
        ...
        try {
            ...
            try {
                ...
                Xlog.i(TAG, "into: getPartitionInfoDev path is " + PARTITION_FILE_PATH_NEW);
                reader = new FileReader(PARTITION_FILE_PATH_NEW);
            ...
            while ((line = bfReader.readLine()) != null) {
                ...
                if (mGptSchemeSupport) {
                    offset = isComboEmmcPreloader ? 0 : (offset + extractSize(partitionInfoArray[2]));
                } else {
