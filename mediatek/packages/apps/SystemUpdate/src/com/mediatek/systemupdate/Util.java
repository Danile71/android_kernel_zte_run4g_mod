...
public final class Util {
    ...
    static String getAvailablePath(Context context) {
        // Remove cache for CR ALPS01614994,SD path will be changed.
        if (FeatureOption.MTK_2SDCARD_SWAP || FeatureOption.MTK_SHARED_SDCARD) {
            sExternalSdCardPath = getExternalSDCardPath(context);
    ...
    public static boolean isLowBattery(Context context) {
        ...
        if (batteryStatus != null) {
            ...
            Xlog.v(TAG, " batteryPct == " + batteryPct);
            final double value = 0.15;
            return batteryPct <= value;
        } 
        Xlog.v(TAG, "batteryStatus is null");
        return false;
    }

    public static boolean isGmsLoad2() {
        String ret;
        ret = SystemProperties.get("ro.com.google.gmsversion");     
        Xlog.i(TAG, " gms version == " + ret);

        //If it is GMS load,the value of ret will like 4.4_rx, otherwise the value will be empty
