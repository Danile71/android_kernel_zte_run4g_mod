...
public class TeledongleDemoActivity extends PreferenceActivity implements
    ...
    private BroadcastReceiver mtedongleReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
	            ...
	            //makeToast("WIFI_AP_STATE_CHANGED_ACTION");
	            int wifistate = intent.getIntExtra(WifiManager.EXTRA_WIFI_AP_STATE, 
				...
				Log.d(LOG_TAG, "mtedongleReceiver receive state =" + state);
                switch (state) {
                	case DISCONNECTED:
						//makeToast("DIS");
						StopWaitingActivity();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mWakeLock = powerManager.newWakeLock(PowerManager.FULL_WAKE_LOCK,"Tedongle");
		if(mDataState != DATA_CONNECTED && mSimState != SIM_STATE_PIN_REQUIRED 
			&& mSimState != SIM_STATE_PUK_REQUIRED && !hasWifiConnectivity()
