    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case OP_LAUNCH_APP:
                ...
                if (mUtkService == null || mMainCmd == null) {
                ...
                if (mCanLaunchStkMenuActivity == false) {
                    Toast toast = Toast.makeText(getApplicationContext(), getString(R.string.lable_on_radio_off), Toast.LENGTH_LONG);
                    toast.setGravity(Gravity.BOTTOM, 0, 0);
                    toast.show();
    ...
    private void handleCmd(UtkCmdMessage cmdMsg) {
        ...
        if (mUtkService == null) {
            ...
            if (mUtkService == null) {
                ...
                UtkLog.d(this, "mUtkService is null when we need to send response in handleCmd");
                return;
            }
        }
        ...
        case CLOSE_CHANNEL:
            if(mCurrentCmd.geTextMessage().text == null)
                mCurrentCmd.geTextMessage().text = new String("Disconnected");
        ...
        case RECEIVE_DATA:
            if(mCurrentCmd.geTextMessage().text == null)
                mCurrentCmd.geTextMessage().text = new String("Receive data");
            launchDataCallMsg(mCurrentCmd.geTextMessage().text.subSequence(0, 
                              mCurrentCmd.geTextMessage().text.length()));
            waitForUsersResponse = false;
            break;
        case SEND_DATA:
            if(mCurrentCmd.geTextMessage().text == null)
                mCurrentCmd.geTextMessage().text = new String("Send data");
            launchDataCallMsg(mCurrentCmd.geTextMessage().text.subSequence(0, 
                              mCurrentCmd.geTextMessage().text.length()));
            waitForUsersResponse = false;
            break;
    ...
    private void handleCmdResponse(Bundle args) {
        ...
        if (mUtkService == null) {
            ...
            if (mUtkService == null) {
                // This should never happen (we should be responding only to a message
                // that arrived from UtkService). It has to exist by this time
                UtkLog.d(this, "mUtkService is null when we need to send response in handleCmdResponse");
                return;
            }
        }
        ...
        case RES_ID_CONFIRM:
            ...
            case OPEN_CHANNEL:
                ...
                resMsg.setConfirmation(confirmed);
                if (confirmed) {
                    launchDataCallMsg("Setup data call...");
                }
                break;
            }
            break;
