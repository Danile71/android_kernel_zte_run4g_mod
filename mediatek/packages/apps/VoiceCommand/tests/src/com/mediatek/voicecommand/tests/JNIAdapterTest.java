...
public class JNIAdapterTest extends InstrumentationTestCase {
    ...
    public void test02_voiceTraining() {
        isNativePrepared();
        setCurHeadsetMode(true);
        String pwdpath = mCfgMgr
                .getPasswordFilePath(VoiceCommandListener.VOW_MODE_UNLOCK);
        String patternpath = mCfgMgr
                .getVoiceRecognitionPatternFilePath(VoiceCommandListener.VOW_MODE_UNLOCK);
        String featurepath = mCfgMgr
                .getFeatureFilePath(VoiceCommandListener.VOW_MODE_UNLOCK);
        String umbpath = mCfgMgr.getUBMFilePath();
        String wakeupinfopath = mCfgMgr.getWakeupInfoPath();
        int[] commandMask = { 5, 4 };
        // Test JNITestAdapter
        int errorid = mJNITestAdapter.startVoiceTraining(pwdpath, patternpath,
                featurepath, umbpath, 1, commandMask,
                VoiceCommandListener.VOW_MODE_UNLOCK, wakeupinfopath,
                mProcessName, mPid);
        TestSleep.sleep(1000);
        assertEquals(errorid, VoiceCommandListener.VOICE_NO_ERROR);
        getNativeIntensity();
