...
public final class engineadapterforjni {
    ...
    public static int setMsgLockFlag(int nSource, int nSmsIdIn, int savedFlag) {
        return NmsSMSMMSManager.getInstance(null).updateLockedFlag(nSource, (long) nSmsIdIn,
                savedFlag);
    }
    ...
    public static int nmsSetTimer(int timerId, int seconds) {
        return NmsTimer.NmsSetTimer(timerId, seconds);
    }
    ...
    public static String nmsGetReadModeString(int readMode, int flag) {
        ...
        case NmsIpMessageConsts.NmsIpMessageType.PICTURE:
            if ((flag & NmsIpMessageConsts.NmsIpMessageFlag.NMS_MSG_FLAG_BURN_AFTER_READ) != 0) {
                strRM = NmsService.getInstance().getString(
                        R.string.STR_NMS_READ_MODE_READEDBURN_PIC);
            } else {
    ...
    public static String nmsGetServerAutoEchoMsg() {
        return String.format(NmsService.getInstance().getString(
                R.string.STR_NMS_SERVER_AUTO_ECHO_MSG));
    }
    ...
    public static String nmsGetGroupAddMsgBody(String self, String members) {
        return String.format(NmsService.getInstance().getString(R.string.STR_NMS_GROUP_ADD_MSG),
                members);
    }
    ...
    static public void sendOfflineStatusToServer(String imsi, String number, int clientSessionId,
            int serverSessionId) {
    ...
    public static void tellActivateOK(int activateType) {
        ...
        else if (activateType == NmsIpMessageConsts.NmsUIActivateType.WAKEUP)
            keyIndex = NmsStatistics.KEY_WAKEUP_ACTIVATE_OK;
    ...
    public static void sendRegStatistics(int simId, int type, String val) {
        NmsRegInfoStatistics.sendNewStatistics(simId, type, val);
    }
}
