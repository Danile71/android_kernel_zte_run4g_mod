...
public class NmsSendMessage {
    ...
    public int resendSms(int nSmsIdIn) {
        ...
        NmsSMSMMSManager.getInstance(null).deleteSMS(NmsSMSMMS.PROTOCOL_SMS,
                new int[] { nSmsIdIn }, true);
    ...
    public void processFail(long lSmdId) {
        ...
        NmsSMSMMSManager.getInstance(null).updateSmsStatusViaId(NmsSMSMMS.PROTOCOL_SMS,
                (int) lSmdId, NmsSMSMMS.SMS_TYPE_FAILED, NmsSMSMMS.SMS_DELIVER_UNKNOWN);
    ...
    public int inserSmsToVector(String threadAddress, String address, String message, int saveFlag,
        ...
        long lThreadId = NmsCreateSmsThread.getOrCreateThreadId(mContext, setThreadAddr);
        long lSmsId = -1;
    ...
    public long divideSms(String strMessageIn, String strAddrSingleIn, int saveFlagIn,
        ...
        if (isMultiPartSmsOk()) {
            ...
            smsContent = new SmsCont(strAddrSingleIn, strMessageIn, lThreadIdIn, lSmsId, simId);
        ...
        for (int i = 0; i < arrayMsg.size(); i++) {
            ...
            smsContent = new SmsCont(strAddrSingleIn, arrayMsg.get(i), lThreadIdIn, lSmsId, simId);
            mVecSms.add(smsContent);
        }
        return lSmsId;
    }
    ...
    private synchronized void nmsSendMultiPartSms(SmsCont smsContent) {
        ...
        for (int i = 0; i < messageCount; i++) {
            ...
            if (requestDeliveryReport && i == messageCount - 1) {
                ...
                intDeliver
                        .setAction("com.android.mms.transaction.MessageStatusReceiver.MESSAGE_STATUS_RECEIVED");
                Uri mUri = ContentUris.withAppendedId(NmsSMSMMS.SMS_CONTENT_URI, smsContent.lSmsId);
                intDeliver.setData(mUri);
                intDeliver.setClassName("com.android.mms",
        ...
        if (-1 != smsContent.lSmsId) {
            NmsPlatformAdapter.getInstance(mContext).sendMultipartTextMessage(
                    smsContent.strAddr.trim(), null, arrayMsg, smsContent.threadId, smsContent.simId, arrayPiSent,
                    arrayPiDeliver);
            try {
            ...
            NmsPlatformAdapter.getInstance(mContext).sendMultipartTextMessage(
                    smsContent.strAddr.trim(), null, arrayMsg, smsContent.threadId, smsContent.simId, null, null);
        }
