...
public class NmsSmsMmsSync extends Handler {
    ...
    private String[] PROJECTION_THREADS = new String[] { NmsSMSMMS._ID + ","
            + NmsSMSMMS.RECIPIENT_IDS + " from threads -- " };
    ...
    public void handleMessage(Message message) {
        ...
        if ((0 == nEventCount) && mFlag) {
            nEventCount++;
			//fix bug ISMS-1171
            mTimer.schedule(new OwnTimerTask(), 2000);
    ...
    private void getAllMsgCursor() {
        ...
        mCsrThreads = NmsContentResolver.query(getCR(), NmsSMSMMS.SMS_CONTENT_URI,
                PROJECTION_THREADS, null, null, "_id ASC");
    }
    ...
    private void processDelAction(NmsDirtyMsg dt, int protocol) {
        if (mExtetionFieldExsit && dt.nmsRecId != 0) {
            ...
            /*
             * TODO: remove it when release if (NmsConfig.isAndroidKitKatOnward
             * && !NmsSMSMMSManager.isDefaultSmsApp())
             * engineadapter.get().nmsUIDeleteMsg(new short[] { (short)
             * dt.nmsRecId }, 1, 1, 0);
             */
        } else {
    ...
    public synchronized void processThreadChange() {
        ...
        try {
            ...
            releaseAllMsgCursor();
            NmsLog.trace(TAG, "process thread change cost time:"
                    + (System.currentTimeMillis() - lDate) + "ms");
        } catch (Exception e) {
            NmsLog.nmsPrintStackTrace(e);
        }
