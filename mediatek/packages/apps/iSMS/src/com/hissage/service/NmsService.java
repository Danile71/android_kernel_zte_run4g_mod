...
import com.hissage.util.statistics.NmsUpgradeFailStatistics;
...
public class NmsService extends Service {
    ...
    public void onCreate() {
        ...
        NmsUpgradeFailStatistics.init(this);
        ...
        /*
         * we can't call engineadapter.get().nmsUIIsActivated() here to check
         * the iSMS is activated or not for some ANR issue, so we always call
         * the loadPn() in notifyRegStatus()
         */
        if (Config.PN && Config.LOAD_PN_ATONCE)
            hpnsReceiver.loadPn(this);
        NmsLog.error(TAG, "bootstart service on create new in 10-30. LOAD_PN_IN_STARTUP flag: "
                + Config.LOAD_PN_ATONCE);
    }

    public void startPushInfoTimer() {
        /*
         * int initEngine =
         * engineadapter.msgtype.NMS_ENG_MSG_INIT_REQ.ordinal();
         * engineadapter.get().nmsSendMsgToEngine(initEngine, null, 0);
         */
        new Thread(new Runnable() {

            public void run() {
                ArrayList<NmsPushInfo> pushInfoLists = NmsDBUtils.getDataBaseInstance(
                        NmsService.getInstance()).getPushInfoList(false);
                if (pushInfoLists != null) {
                    int size = pushInfoLists.size();
                    for (int i = 0; i < size; i++) {
                        NmsPushInfo pushInfo = pushInfoLists.get(i);
                        long time = Long.parseLong(pushInfo.pushShowTime);
                        if (time <= 0) {
                            time = 0;
                        } else {
                            time = (long) ((time - System.currentTimeMillis()) / 1000 + (Math
                                    .random() * pushInfo.randomRange) * 60 * 60);
                        }
                        int id = NmsTimer.NmsCreateTimer(
                                (int) (pushInfo._id + NmsTimer.NMS_TIMERID_PUSH_INFO), time + 20);
                    }
                }
    ...
    public void activeAllSimCard() {
        ...
        NmsLog.trace(TAG, "activeAllSimCard try to activate sim");
        ...
        while (true) {
            ...
            idList = engineadapter.get().nmsUIGetNeedActiveSimIdList();
            ...
            if ((System.currentTimeMillis() - startTime) >= (1000 * 60)) {
                NmsLog.trace(TAG, "activeAllSimCard got not sim card in 60 second");
                break;
            }
        }

        NmsLog.trace(TAG, "activeAllSimCard get activate SimId list service on first:" + idList);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // }
        // if (intent != null && intent.getBooleanExtra(pushEngineConnecd,
        // false)) {
        // int connectNet =
        // engineadapter.msgtype.NMS_ENG_MSG_CONNECT_REQ.ordinal();
