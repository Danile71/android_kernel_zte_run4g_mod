...
public class NmsSystemSettingsActivity extends PreferenceActivity {
    ...
    private void updateStatus() {
        ...
        if (NmsSMSMMSManager.isDefaultSmsApp()
                && ((simId1 > 0 && NmsIpMessageApiNative.nmsGetActivationStatus((int) simId1) == NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED) || (simId2 > 0 && NmsIpMessageApiNative
                        .nmsGetActivationStatus((int) simId2) == NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED))) {
        ...
        if (mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id > 0 && NmsPlatformAdapter.getInstance(this).isMtkGeminiSupport()) {
    ...
    private void showNetUsage() {
        ...
        if (mSimInfo[NmsConsts.SIM_CARD_SLOT_1].sim_id > 0) {
            if (mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id > 0) {
                dlgContent = String.format(getString(R.string.STR_NMS_DOUBLE_NETWORK_DLG_CONTENT),
                        ...
                        (float) ((float) sysStatus.recvBytesSim2) / 1024, time); // SIM1
                                                                                 // wifi
                                                                                 // and
                                                                                 // SIM2
            } else {
                dlgContent = String.format(getString(R.string.STR_NMS_SINGLE_NETWORK_DLG_CONTENT),
                        (float) ((float) sysStatus.sendBytesWifi) / 1024,
                        (float) ((float) sysStatus.sendBytesSim1) / 1024,
                        (float) ((float) sysStatus.recvBytesWifi) / 1024,
                        (float) ((float) sysStatus.recvBytesSim1 / 1024), time); // SIM1
                                                                                 // wifi
            }
        } else {
            if (mSimInfo[NmsConsts.SIM_CARD_SLOT_2].sim_id > 0) {
                dlgContent = String.format(getString(R.string.STR_NMS_SINGLE_NETWORK_DLG_CONTENT),
                        ...
                        (float) ((float) sysStatus.sendBytesSim2) / 1024,
                        (float) ((float) sysStatus.recvBytesWifi) / 1024,
                        (float) ((float) sysStatus.recvBytesSim2 / 1024), time); // SIM2
                                                                                 // wifi
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    if (isChecked1) {
                        if (NmsSimActivateStatus.NMS_SIM_STATUS_DISABLED != info1.status) {
                            ...
                            NmsStatistics.incKeyVal(NmsStatistics.KEY_DISABLE_ISMS) ;
                            showWaitDlg();
                            return false;
                        } else {
                            updateStatus();
                        ...
                        if (NmsSimActivateStatus.NMS_SIM_STATUS_ACTIVATED != info1.status) {
                            ...
                            NmsStatistics.incKeyVal(NmsStatistics.KEY_ENABLE_ISMS);
                            showWaitDlg();
                            return false;
