...
public class NmsBottomPanelView extends LinearLayout {
    ...
    private LinearLayout mShareAndEmoticonContainer;
    private NmsSharePanel mSharePanel = null;
    private NmsEmoticonPanel mEmoticonPanel = null;
    ...
    private void initializeSharePanel() {
        LayoutInflater.from(mContext).inflate(
                R.layout.share_panel_for_chat_detail, mShareAndEmoticonContainer);
        mSharePanel = (NmsSharePanel) mShareAndEmoticonContainer.findViewById(R.id.share_panel);
        mSharePanel.setHandler(mHandler);
        showSharePanel(false);
    }

    private void initializeEmoticonPanel() {

        LayoutInflater.from(mContext).inflate(
                R.layout.emoticon_panel_for_chat_detail, mShareAndEmoticonContainer);
        mEmoticonPanel = (NmsEmoticonPanel) mShareAndEmoticonContainer.findViewById(R.id.emoticon_panel);
    ...
    protected void onFinishInflate() {
        ...
        mShareAndEmoticonContainer = (LinearLayout) findViewById(R.id.ll_message_bottom_panel);
    ...
    private void sendIpMessage(String caption, int flag) {
        ...
        if (mAttachType == NmsIpMessageConsts.NmsIpMessageType.PICTURE) {
            ...
            if (NmsConfig.getSendAsSMSFlag()
                    && mContact.getType() != NmsContactType.HISSAGE_GROUP_CHAT) {
                msg.send(mContext, flag, mContact.getNumber(), mAttachPath, body,
    ...
    public boolean isShow() {
        if (mSharePanel != null && mSharePanel.isShown()) {
            return true;
        }
        if (mEmoticonPanel != null && mEmoticonPanel.isShown()) {
            return true;
        }
        return false;
    }
