...
public class NmsUpgradeManager {
    ...
    private void handleNetworkInavailable() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                NmsUpgradeFailStatistics.updateToDb(true,false,false);
            }
        }).start();
    }
    ...
    private boolean doRequestNewVersion() {
        try {
            ...
            //NmsUpgradeFailStatistics.get().updateToDb(0, 1, 1);
            // M: Activation Statistics
            ...
            JSONObject lastUpgradeFailObject = NmsUpgradeFailStatistics.getUpgrageFailData();
            if(null != lastUpgradeFailObject){
                data.put("lastUpgradeFail",lastUpgradeFailObject);
            }else{
                data.put("lastUpgradeFail", new JSONObject());
            }
            String sendStr = data.toString();
            ...
            // M: Activation Statistics
            NmsStatistics.clear();
            NmsUpgradeFailStatistics.Clear();
            doClearData();
                        ...
                        try {
                            ...
                            if (tryTime < REQ_MAX_TRY_TIME) {
                                ...
                                if(networkName.equals("wifi")){
                                    NmsUpgradeFailStatistics.updateToDb(false, true, false);
                                }else if(networkName.equals("gprs")){
                                    NmsUpgradeFailStatistics.updateToDb(false, false, true);
                                }else if(networkName.equals("no_network")){
                                    NmsUpgradeFailStatistics.updateToDb(true, false, false);
                                }
                            }
                        } catch (Exception e) {
