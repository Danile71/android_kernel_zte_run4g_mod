...
public class VLWMediaController extends MediaController {
    ...
    public void initControllerState(int start, int end, int duration) {
        ...
        //Fix ALPS01583341 by mtk54031
        if (start > duration || end > duration) {
            Xlog.i(TAG, "init controller error. because the start > duration or end > duration.");
            return;
        }
        ...
        public void handleMessage(Message msg) {
            ...
            case SHOW_PROGRESS:
                ...
                if (!mDragging && mPlayer.isPlaying()) {
                    long delay = MAX_SEEK - (pos % MAX_SEEK);
                    if (mEndPosition - pos < 1000) {
                        delay = mEndPosition - pos;
                        Xlog.i(TAG, "mEndPosition - pos < 1000 : delay : "
                                + delay);
                    }
    ...
    protected int setProgress() {
        ...
        if (mProgressBar != null) {
            if (mDuration > 0) {
                ...
                if (mNewPosition > mEndPosition) {
                    pos = (int) (MAX_SEEK * (double) mEndPosition / mDuration + ELIMINATE_ERROR);
                    Xlog.d(TAG, "mNewPosition > mEndPosition  mEndPosition: "
                            + mEndPosition + " pos: " + pos);
                } else {
                    if (DEBUG) {
                        Xlog.d(TAG, "setProgress() mNewPosition="
                                + mNewPosition + ", mDuration=" + mDuration);
                    }
                }
