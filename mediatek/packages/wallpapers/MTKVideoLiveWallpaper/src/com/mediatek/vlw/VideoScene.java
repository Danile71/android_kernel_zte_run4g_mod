...
public class VideoScene {
    ...
    /**  Filter unwanted plug broadcast */
    private boolean mSmartBookPlug;
    /**  When service restart by MHL plug in/out, no need to clear previous video info */
    private boolean mIsSmbStateChanged;
    ...
    private final MediaPlayer.OnInfoListener mInfoListener = new MediaPlayer.OnInfoListener() {
        public boolean onInfo(MediaPlayer mp, int what, int extra) {
            Xlog.v(TAG, "OnInfoListener,  what: " + what + ",extra: " + extra);
        ...
        if (DEBUG) {
            Xlog.d(TAG, String.format(
                    "relayout, display: (%d,%d), video: (%d,%d), "
                            + "previous wallpaper: (%d,%d)," + "wallpaper wanted size: (%d,%d)", dispW, dispH,
                    videoWidth, videoHeight, curWpW, curWpH, wpW, wpH));
        }
    ...
    public void destroy() {
        ...
        if (!mPreview && !mIsSmbStateChanged) {
            if (iSavedUserID == ActivityManager.getCurrentUser()) {
                clear(true, true);
            }
        }
        mIsSmbStateChanged = false;
