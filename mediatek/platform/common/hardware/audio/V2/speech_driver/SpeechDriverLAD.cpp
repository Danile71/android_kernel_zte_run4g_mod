...
status_t SpeechDriverLAD::SpeechOn()
{
    ...
    // modem external (MT6290) needs to acquire wakelock for audio hw
    if (mModemIndex == MODEM_EXTERNAL)
    {
...
status_t SpeechDriverLAD::SpeechRouterOn()
{
    ...
    return NO_ERROR; //pCCCI->SendMessageInQueue(pCCCI->InitCcciMailbox(MSG_A2M_SPH_ROUTER_ON, true, 0));
}
...
status_t SpeechDriverLAD::SpeechRouterOff()
{
    ...
    return NO_ERROR; //pCCCI->SendMessageInQueue(pCCCI->InitCcciMailbox(MSG_A2M_SPH_ROUTER_ON, false, 0));
}
