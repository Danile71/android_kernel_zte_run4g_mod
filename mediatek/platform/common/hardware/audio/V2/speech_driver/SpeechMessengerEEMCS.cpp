...
RingBuf SpeechMessengerEEMCS::GetM2AUplinkRingBuffer(const ccci_buff_t &ccci_buff)
{
    ...
    if (ul_ring_buf.pWrite >= mM2AShareBufEnd)
    {
        ...
        SLOGV("%s(), mM2AShareBufEnd(0x%x), payloadLen(0x%x), size_copy1(0x%x), size_copy2(0x%x), pRead(0x%x), pWrite(0x%x)",
    ...
    SLOGV("%s(), pBufBase(0x%x), bufLen(0x%x), payloadLen(0x%x), mM2AShareBufRead(0x%x), pRead(0x%x), pWrite(0x%x)",
...
void SpeechMessengerEEMCS::SetModemSideModemStatus(const modem_status_mask_t modem_status_mask)
{
    ...
    char property_value[PROPERTY_VALUE_MAX];
    char property_valueR[PROPERTY_VALUE_MAX];
    sprintf(property_value, "%u", mModemSideModemStatus);
    property_set(PROPERTY_KEY_MODEM_STATUS[mModemIndex], property_value);
    
    property_get(PROPERTY_KEY_MODEM_STATUS[mModemIndex], property_valueR, "0");  //"0": default all off
    ALOGD("%s(), mModemIndex(%d), modem_status_mask(0x%x), property after set(0x%x)", __FUNCTION__, mModemIndex, modem_status_mask, atoi(property_valueR));
}
