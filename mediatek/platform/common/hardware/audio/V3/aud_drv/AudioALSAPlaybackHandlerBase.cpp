...
status_t AudioALSAPlaybackHandlerBase::openPcmDriver(const unsigned int device)
{
    ...
    ASSERT(mPcm == NULL);
    mPcm = pcm_open(kAudioSoundCardIndex, device, PCM_OUT | PCM_MONOTONIC, &mConfig);
    if (mPcm == NULL)
    {
...
status_t AudioALSAPlaybackHandlerBase::setLowLatencyMode(bool mode, size_t lowLatencyHalBufferSize, bool bforce)
{
    return NO_ERROR;
}
...
status_t AudioALSAPlaybackHandlerBase::getHardwareBufferInfo(time_info_struct_t *HWBuffer_Time_Info)
{
    ...
    if (pcm_get_htimestamp(mPcm, &HWBuffer_Time_Info->frameInfo_get, &HWBuffer_Time_Info->timestamp_get) == 0)
    {
        ...
        ALOGV("%s, frameInfo_get = %d, hw_buf_size_frame=%d,mStreamAttributeTarget.buffer_size=%d, buffer_per_time=%d\n",
              __FUNCTION__, HWBuffer_Time_Info->frameInfo_get, hw_buf_size_frame, mStreamAttributeTarget.buffer_size
              , HWBuffer_Time_Info->buffer_per_time);
    }
    else
    {
...
status_t AudioALSAPlaybackHandlerBase::doPostProcessing(void *pInBuffer, uint32_t inBytes, void **ppOutBuffer, uint32_t *pOutBytes)
{
    ...
    {
        mAudioFilterManagerHandler->start(); // TODO(Harvey, Hochi), why start everytime in write() ??
