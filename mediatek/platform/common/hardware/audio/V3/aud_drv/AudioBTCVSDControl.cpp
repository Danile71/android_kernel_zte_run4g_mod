...
int AudioBTCVSDControl::getFd()
{
    ...
    AudioAutoTimeoutLock _l(mGetFdLock);
...
/*
btsco_process_RX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint8_t packetvalid)
...
void AudioBTCVSDControl::btsco_process_RX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint8_t packetvalid)
{
    ...
    else if (mBTSCOCVSDContext->pRX->PacketValid[index1] == 1)
    {
        ...
        status = -1;
        ALOGW("btsco_process_RX_MSBC() wrong header, but packet not lost, sync to next 30 byte block!!!");
    }
    else
    {
        status = -1;
        ...
        mBTSCOCVSDContext->pRX->iPacket_r++;   // Consume another 30 bytes for entire packet
        ALOGW("btsco_process_RX_MSBC() wrong header, packet invalid!!!");
    }
    ...
    if (mBTSCOCVSDContext->pRX->fEnablePLC)
    {
        ...
        {
            //packet lost
            ALOGW("btsco_process_RX_MSBC(), packet lost, do PLC!!!");
...
/*
btsco_process_TX_CVSD(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint32_t src_fs_s)
...
/*
btsco_process_TX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint32_t src_fs_s)
...
void AudioBTCVSDControl::btsco_process_TX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint32_t src_fs_s)
{
    ...
    if (*insize != 0 && *outsize != 0)
    {
        ...
        *insize = uConsumeByte;
        ...
        ALOGVV("MSBC_ENC_Process iInSample=%d, iOutByte=%d, status=%d", iInSample, iOutByte, status);
...
AudioBTCVSDControl::AudioBTCVSDLoopbackRxThread::AudioBTCVSDLoopbackRxThread(void *mAudioManager, uint32_t Mem_type, char *RingBuffer, uint32_t BufferSize)
{
