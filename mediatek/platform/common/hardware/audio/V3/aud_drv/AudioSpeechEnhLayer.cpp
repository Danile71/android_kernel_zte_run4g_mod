...
bool    SPELayer::SetFrameRate(SPE_MODE mode, long frame_rate)
{
    ...
    {
        ALOGD("%s, only support 10ms and 20ms framerate", __FUNCTION__);
        return false;
    }
    ...
    ALOGD("%s, SPE_MODE=%d, frame_rate=%d", __FUNCTION__, mode, frame_rate);
...
bool    SPELayer::SetRoute(SPE_ROUTE route)
{
    ...
    {
        ALOGD("%s, route not correct", __FUNCTION__);
        return false;
    }
...
long    SPELayer::GetSampleRate(SPE_MODE mode)
{
    ...
    ALOGD("%s, SPE_MODE=%d, retSampleRate=%d", __FUNCTION__, mode, retSampleRate);
...
long    SPELayer::GetFrameRate(SPE_MODE mode)
{
    ...
    ALOGD("%s, SPE_MODE=%d, retFrameRate=%d", __FUNCTION__, mode, retFrameRate);
    return retFrameRate;
}
...
long    SPELayer::GetMICDigitalGain(SPE_MODE mode)
{
    ...
    ALOGD("%s, SPE_MODE=%d, retPGAGain=%d", __FUNCTION__, mode, retPGAGain);
    return retPGAGain;
}
...
long    SPELayer::GetFeaCfgTable(SPE_MODE mode)
{
    ...
    ALOGD("%s, SPE_MODE=%d, retFeaCfgTable=%x", __FUNCTION__, mode, retFeaCfgTable);
...
bool SPELayer::GetUPlinkIntrStartTime()
{
    ...
    ALOGD("%s, sec=%d, nsec=%d", __FUNCTION__, mUplinkIntrStartTime.tv_sec, mUplinkIntrStartTime.tv_nsec);
    mFirstVoIPUplink = true;
    mDLPreQLimit = false;
    return true;;
}
...
bool SPELayer::GetDownlinkIntrStartTime()
{
    ...
    ALOGD("%s, sec=%d, nsec=%d, size=%d, mDLDelayBufferQ size()=%d", __FUNCTION__, mDownlinkIntrStartTime.tv_sec, mDownlinkIntrStartTime.tv_nsec, mDLInBufferQ.size(), mDLDelayBufferQ.size());
    if (mDLInBufferQ.size() > 0)
    {
...
void SPELayer::DropDownlinkData(uint32_t dropsample)
{
    ...
    while (diffBufLength > 0)
    {
        ...
        ALOGD("%s, drop DL data diffBufLength=%d, mDLInBufferQ.size()=%d, mDLInBufferQ[0]->BufLen=%d!!!", __FUNCTION__, diffBufLength, mDLInBufferQ.size(), mDLInBufferQ[0]->BufLen);
        if (diffBufLength >= mDLInBufferQ[0]->BufLen)
        {
            ...
            while (droplength > 0)
            {
                ALOGD("%s, drop DL Delay data droplength=%d, mDLDelayBufferQ.size()=%d, mDLDelayBufferQ[0]->BufLen4Delay=%d!!!", __FUNCTION__, droplength, mDLDelayBufferQ.size(), mDLDelayBufferQ[0]->BufLen4Delay);
...
void SPELayer::CompensateBuffer(size_t BufLength, struct timespec CompenStartTime)
{
    ...
    if (!mFirstVoIPDownlink && mOutputStreamRunning) //compensate happen after DL first time queue, and only count when downlink is playback
    {
        ...
        ALOGD("%s, mCompensatedBufferSize=%d", __FUNCTION__, mCompensatedBufferSize);
        if ((mCompensatedBufferSize >= 160) && mPrepareProcessDataReady)  //5ms data
        {
            ReSync();
        }
    }
...
bool SPELayer::PrepareProcessData()
{
    ...
    if (mDLNewStart || (DLdataPrepareCount > 0)) //the first queue downlink buffer is not ready to play yet, only need to process uplink data, so add zero data on downlink
    {
        ...
        ALOGD("%s, DL data is not ready yet, size %d, %p", __FUNCTION__, mDLInBufferQ.size(), mDLInBufferQ[0]->pBufBase);
    ...
    else    //when all data is ready, check the estimate time to let DL/UL could start together.
    {
        ...
        {
            //compensate data to DL and DL delay as zero data for first uplink buffer process
            ALOGD("%s, downlink data is not ready yet, no need to sync, size %d, %p", __FUNCTION__, mDLInBufferQ.size(), mDLInBufferQ[0]->pBufBase);
            BypassDLBuffer();
            return bRet;
        }
#endif
        ...
        if (TimeStampCompare(mULInBufferQ[0], mDLInBufferQ[0], 0))  //drop downlink and downlink delay data
        {
            ...
            while ((!mDLInBufferQ.isEmpty()) && (TimeStampCompare(mULInBufferQ[0], mDLInBufferQ[0], 1)))
            {
                ...
                while (droplength > 0)
                {
                    ALOGD("%s, 1 drop DL Delay data droplength=%d, mDLDelayBufferQ.size()=%d, mDLDelayBufferQ[0]->BufLen4Delay=%d!!!", __FUNCTION__, droplength, mDLDelayBufferQ.size(), mDLDelayBufferQ[0]->BufLen4Delay);
                    if (droplength < mDLDelayBufferQ[0]->BufLen4Delay)
                    {
        ...
        ALOGD("%s, diffnsec %lld, diffSample=%ld, diffBufLength=%d", __FUNCTION__, diffnsec, diffSample, diffBufLength);
#if 1
        while (diffBufLength > 0)
        {
...
#endif
        ALOGD("%s, finish, mDLInBufferQ.size = %d, mDLInBufQLenTotal=%d, mULInBufferQ.size = %d, mULInBufQLenTotal=%d", __FUNCTION__, mDLInBufferQ.size(), mDLInBufQLenTotal, mULInBufferQ.size(),
    ...
    if (mULInBufQLenTotal < mSPEProcessBufSize) //not enough UL input buffer for process
    {
        ...
        while (tmpInBufLength)
        {
            ...
            else    //consume all the data in first queue buffer
            {
                ...
                if (!mULOutBufferQ.isEmpty())
                {
                    ALOGD("%s, mULOutBufferQ not empty, get next one, size=%d", __FUNCTION__, mULOutBufferQ.size());
                    tempULCopy = mULOutBufferQ[0]->BufLen >> 2;
                }
                else
                {
        ...
        ALOGD("%s, has UL Output buffer but not enough UL Input buffer", __FUNCTION__);
        //pthread_mutex_unlock(&mBufMutex);
        return true;
    }
