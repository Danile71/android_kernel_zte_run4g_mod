...
typedef struct
{
    ...
    size_t frameInfo_get; //for input: remains data(frame) in kernel buffer.  for output: empty data(frame) in kernel buffer size
    size_t buffer_per_time; //for input: frames read per read. For output: kernel buffer size(frame)
    long kernelbuffer_ns;   //calculated kernel buffer time
} time_info_struct_t;
