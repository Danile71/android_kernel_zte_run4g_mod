...
bool AudioFtmBase::mIsInAndroidMode = true; // default in Android Mode

void AudioFtmBase::setIsInAndroidMode(const bool is_in_android_mode)
{
    ALOGD("%s(), is_in_android_mode = %d", __FUNCTION__, is_in_android_mode);
    mIsInAndroidMode = is_in_android_mode;

    if (mIsInAndroidMode == true)
    {
        setUseBinderToAccessNVRam(true); // when in Android mode, use binder to access nvram
    }
    else
    {
        setUseBinderToAccessNVRam(false); // when in factory/meta mode, use kernel file descriptor to access nvram
    }        
}
...
int AudioFtmBase::SineGenTest(char sinegen_test)
{
    ALOGW("%s()", __FUNCTION__);
    return true;
}
...
int AudioFtmBase::SetSpkMonitorParam(void *pParam)
{
    ALOGW("%s()", __FUNCTION__);
    return 0;
}

int AudioFtmBase::GetSpkMonitorParam(void *pParam)
{
    ALOGW("%s()", __FUNCTION__);
    return 0;
}

void AudioFtmBase::EnableSpeakerMonitorThread(bool enable)
{
    ALOGW("%s()", __FUNCTION__);
    return;
}

void AudioFtmBase::SetStreamOutPostProcessBypass(bool flag)
{
    ALOGW("%s()", __FUNCTION__);
    return;
}
