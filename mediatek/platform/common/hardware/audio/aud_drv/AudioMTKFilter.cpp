...
{
    init();
    MtkAudioLoud::getBesSoundVer();//print ver
}
...
{
    SXLOGV("setSpkMntrGain() target gain %d, sample %d", gain, ramp_sample_cnt);
...
//        memcpy(&cali_param.bes_loudness_bpf_coeff_L, &AUD_VIBR_FILTER_COEF_Table[(VIBSPK_DEFAULT_FREQ - VIBSPK_FREQ_LOWBOUND + 1) / VIBSPK_FILTER_FREQSTEP], sizeof(uint32_t)*VIBSPK_AUD_PARAM_SIZE);
        memcpy((void *)&cali_param.bes_loudness_f_param.V5ViVSPK.bes_loudness_bpf_coeff, (void *)&AUD_VIBR_FILTER_COEF_Table[(VIBSPK_DEFAULT_FREQ - VIBSPK_FREQ_LOWBOUND + 1) / VIBSPK_FILTER_FREQSTEP], sizeof(uint32_t)*VIBSPK_AUD_PARAM_SIZE);
#endif
