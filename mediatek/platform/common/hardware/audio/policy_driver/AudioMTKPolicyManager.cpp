...
AUDIO_CUSTOM_HAC_PARAM_STRUCT AudioMTKPolicyManager::Audio_HAC_Custom_Param;
                                                         ...
                                                         const char *device_address)
{
    ...
    if (audio_is_output_device(device))
    {
        ...
        ds1ConfigureRoutingDevice(audioOutputDevice);
#endif //DOLBY_END
...
void AudioMTKPolicyManager::setPhoneState(int state)
{
    ...
    int delayMs = 0;
#ifdef MTK_AUDIO
    if ((isStateInCall(state)) || (isStateInCall(oldState) && !isStateInCall(state) && (getA2dpOutput() != 0)))
    {
#else
...
{
    ALOGV("getProfileForDirectOutput() device:%d, samplingRate:%d, format:%d, channelMask:0x%x,flags:0x%x",
    ...
    for (size_t i = 0; i < mHwModules.size(); i++)
    {
        ...
        for (size_t j = 0; j < mHwModules[i]->mOutputProfiles.size(); j++)
        {
            ...
            if (flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD)
            {
                ...
                {
                    ALOGV("mAvailableOutputDevices=0x%x, profile->mSupportedDevices=0x%x",
                          mAvailableOutputDevices, profile->mSupportedDevices);
                                                   ...
                                                   const audio_offload_info_t *offloadInfo)
{
...
#ifdef MTK_LOSSLESS_BT_AUDIO_SUPPORT
        if (profile->mSupportedDevices & AUDIO_DEVICE_OUT_ALL_A2DP)
        {
            String8 para = String8("Pcm_Playing");
            String8 ret = mpClientInterface->getParameters(getA2dpOutput(), para);
            ALOGD("isOffloadSupported() getParameter Pcm_Playing=%s", ret.string());
            if (ret == String8("yes"))
            {
                ALOGD("Pcm_Playing, reject offload a2dp output");
                return 0;
            }
        }
#endif
    ...
    if (audio_is_linear_pcm((audio_format_t)format))
    {
        ...
        {
            String8 para = String8("Offload_Playing");
            String8 ret = mpClientInterface->getParameters(output, para);
            ALOGD("isOffloadSupported() getParameter Offload_Playing=%s", ret.string());
            if (ret == String8("yes"))
            {
                //if a2dp offload playing, force this audiotrack to primary output
...
#ifdef MTK_AUDIO
            bool bforceapplyoutput = false;
            if (bForce2MuteFM)
            {
                bForce2MuteFM = false;
                bforceapplyoutput = true;
            }
            setOutputDevice(output, newDevice, bforceapplyoutput ? true : false, outputDesc->mLatency * 2);
#else
...
bool AudioMTKPolicyManager::isOffloadSupported(const audio_offload_info_t &offloadInfo)
{
    ...
    IOProfile *profile = getProfileForDirectOutput(AUDIO_DEVICE_NONE /*ignore device */,
                                                   ...
                                                   AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD);
    ALOGD("isOffloadSupported() profile %sfound", profile != NULL ? "" : "NOT ");

#ifdef MTK_LOSSLESS_BT_AUDIO_SUPPORT
    // A2DP offload modified by ship , ask A2DP hal is support now ?
    audio_io_handle_t a2dp_handle = getA2dpOutput();
    if (profile != NULL && a2dp_handle != 0)
    {
        String8 para = String8("Offload_Support");
        String8 ret = mpClientInterface->getParameters(a2dp_handle, para);
        ALOGD("isOffloadSupported() getParameter Offload_Support=%s", ret.string());
        if (ret != String8("yes"))
        {
            return false;
        }
        ...
        for (size_t j = 0; j < mHwModules[i]->mOutputProfiles.size(); j++)
        {
                ...
                ((outProfile->mFlags & AUDIO_OUTPUT_FLAG_DIRECT) == 0))
            {
                ...
                else
                {
                    ...
                    ALOGD("addoutput, 0x%x, 0x%x ", mDefaultOutputDevice, outProfile->mSupportedDevices);
                    addOutput(output, outputDesc);
                                                                           ...
                                                                           DefaultKeyedVector<audio_io_handle_t, AudioOutputDescriptor *> openOutputs)
{
    ...
    ALOGD("getOutputsForDevice() device %04x", device);
    for (size_t i = 0; i < openOutputs.size(); i++)
    {
        ALOGD("output %d isDuplicated=%d device=%04x",
              i, openOutputs.valueAt(i)->isDuplicated(), openOutputs.valueAt(i)->supportedDevices());
        if ((device & openOutputs.valueAt(i)->supportedDevices()) == device)
        {
            ALOGD("getOutputsForDevice() found output %d", openOutputs.keyAt(i));
                                                            ...
                                                            bool fromCache)
{
    ...
    switch (strategy)
    {
        ...
        case STRATEGY_MEDIA:
        {
...
#endif
            ALOGD("mHasA2dp:%d, getA2dpOutput:%d, bA2DPForeceIgnore:%d, mA2dpSuspended:%d,force =%d ,device2:%lu,", mHasA2dp ? 1 : 0, (int)getA2dpOutput(), bA2DPForeceIgnore, mA2dpSuspended, mForceUse[AudioSystem::FOR_MEDIA], device2);
            ...
            if (device2 == AUDIO_DEVICE_NONE && bFMTxEnable && !(device & AUDIO_DEVICE_OUT_SPEAKER))
            {
                                                          ...
                                                          uint32_t delayMs)
{
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++)
    {
        ...
        bool bFM2MusicStarTempMute = ((outputDesc->isStrategyActive((routing_strategy)STRATEGY_PROPRIETARY_FM) ||
                                      ...
                                      && outputDesc->mRefCount[AudioSystem::MUSIC] && (i == STRATEGY_MEDIA) && bFMPreStopSignal && outputDesc->mMuteCount[AudioSystem::MUSIC] == 0);
#endif
        if (doMute || tempMute
#ifdef MTK_AUDIO
                ...
                if (desc->isStrategyActive((routing_strategy)i))
                {
                       ...
                       )
                    {
                        ...
                        if ((STRATEGY_PROPRIETARY_FM == i || STRATEGY_PROPRIETARY_MATV == i) || (STRATEGY_MEDIA == i && bFM2MusicStarTempMute)) //[ALPS00414767] Add STRATEGY_MEDIA case
                        {
                            ...
                            usleep(50 * 1000); //ALPS01660407
                            setStrategyMute((routing_strategy)i, false, curOutput, (desc->latency() * 12 > 250 ? desc->latency() * 12 : 250), device); //6->12 for ALPS1375377
                        }
                        else
                        {
                                           ...
                                           audio_devices_t device)
{
        ...
        ((stream_strategy == STRATEGY_SONIFICATION) || (stream_strategy == STRATEGY_SONIFICATION_RESPECTFUL)))
    {
        ...
        if (isStreamActive(AudioSystem::MUSIC, SONIFICATION_HEADSET_MUSIC_DELAY))
        {
            if (volume > 0)
            {
                volume = SONIFICATION_AUX_IGITAL_VOLUME_FACTOR + (1 - SONIFICATION_AUX_IGITAL_VOLUME_FACTOR) * volume;
            }
        }
    }
#endif
                                                  ...
                                                  bool force)
{
    ...
    {
        volume = 0.0;
        bForce2MuteFM = true;//ALPS01674814
    }
...
uint32_t AudioMTKPolicyManager::GetSampleRateIndex(int sampleRate)
{
...
bool AudioMTKPolicyManager::CheckFirstActive()
{
    uint32_t index = 0;
...
bool AudioMTKPolicyManager::CheckStreamActive()
{
    for (int i = 0 ; i < OUTPUT_NUM_RATE_INDEX ; i++)
    {
...
void AudioMTKPolicyManager::DumpSampleRateArray()
{
    ALOGD("DumpSampleRateArray");
...
bool AudioMTKPolicyManager::SetFMIndirectMode(int SampleRate)
{
    ALOGD("SetFMIndirectMode SampleRate = %d ", SampleRate);
    if (SampleRate > 44100)
    {
        // set property to tell FM not use direct mode
        property_set(PROPERTY_KEY_FM_FORCE_DIRECT_MODE_TYPE, "2");
        return true;
    }
    else
    {
...
bool AudioMTKPolicyManager::ReleaseFMIndirectMode(int SampleRate)
{
    ALOGD("ReleaseFMIndirectMode ");
    // set property to tell FM not use direct mode
    property_set(PROPERTY_KEY_FM_FORCE_DIRECT_MODE_TYPE, "0");
    return true;
}

uint32_t AudioMTKPolicyManager::GetFirstTrackSampleRate()
{
...
bool AudioMTKPolicyManager::PrimaryDeviceSupportSampleRate(int SampleRate)
{
    return true;
}


bool AudioMTKPolicyManager::PrimarySupportSampleRate(int SampleRate)
{
    ALOGD("PrimarySupportSampleRate SampleRate = %d", SampleRate);
    ssize_t index = mOutputs.indexOfKey(mPrimaryOutput);
    AudioOutputDescriptor *outputDesc = mOutputs.valueAt(index);
    const IOProfile *profile = outputDesc->mProfile;
    for (size_t i = 0; i < profile->mSamplingRates.size(); i++)
    {
        ALOGD("PrimarySupportSampleRate mSamplingRates[%d] = %d", i, profile->mSamplingRates[i]);
...
status_t AudioMTKPolicyManager::PolicyFirstStart(AudioSystem::stream_type stream , int sampleRate)
{
    uint32_t SampleRate = 0;
    ALOGD("PolicyFirstStart stream = %d sampleRate = %d ", stream, sampleRate);
...
int AudioMTKPolicyManager::PolicyPropertyCheck()
{
    int result;
    char value[PROPERTY_VALUE_MAX];
...
status_t AudioMTKPolicyManager::PolicyRestore(void)
{
    ALOGD("PolicyRestore");
    return NO_ERROR;
    ...
    if (mSampleRate_Policy != SampleRate_Do_nothing)
    {
        ...
        AudioParameter param = AudioParameter();
        param.addInt(String8(AudioParameter::keySamplingRate), (int)PolicySampleRate);
        mpClientInterface->setParameters(mPrimaryOutput, param.toString(), 1000);
    }
    return NO_ERROR;
}

status_t AudioMTKPolicyManager::PolicyForceReplace(AudioSystem::stream_type stream, int sampleRate)
{
    ...
    if (stream == AudioSystem::MUSIC && (!isStreamActive(AudioSystem::FM, 0)))
    {
        if (PrimarySupportSampleRate(sampleRate) && (PolicySampleRate != sampleRate))
        {
            PolicySampleRate = sampleRate;
            AudioParameter param = AudioParameter();
            param.addInt(String8(AudioParameter::keySamplingRate), (int)sampleRate);
            mpClientInterface->setParameters(mPrimaryOutput, param.toString());
        }
        SetFMIndirectMode(sampleRate);
    }
...
status_t AudioMTKPolicyManager::SampleRatePolicy(AudioSystem::stream_type stream, int sampleRate)
{
    if (PolicyPropertyCheck())
    {
        mSampleRate_Policy = PolicyPropertyCheck();
...
status_t AudioMTKPolicyManager::AddSampleRateArray(AudioSystem::stream_type stream, int sampleRate)
{
    uint32_t index = 0;
    // only take care about music content samplerate
    if (stream == AudioSystem::MUSIC)
    {
...
status_t AudioMTKPolicyManager::RemoveSampleRateArray(AudioSystem::stream_type stream, int sampleRate)
{
    uint32_t index = 0;
    if (stream == AudioSystem::MUSIC)
    {
...
status_t AudioMTKPolicyManager::StartOutputSamplerate(audio_io_handle_t output,
                                                      AudioSystem::stream_type stream,
                                                      int session , int samplerate)
{
    ALOGD("StartOutputSamplerate output = %d stream = %d session = %d samplerate=%d ", output, stream, session, samplerate);
    int ret = 0;
    AddSampleRateArray(stream, samplerate);
    startOutput(output, stream, session);
    ret = SampleRatePolicy(stream, samplerate);
    return ret;
}

status_t AudioMTKPolicyManager::StopOutputSamplerate(audio_io_handle_t output,
                                                     AudioSystem::stream_type stream,
                                                     int session , int samplerate)
{
    ALOGD("StopOutputSampletateoutput = %d stream = %d session = %d samplerate=%d ", output, stream, session, samplerate);
    int ret = 0;
    RemoveSampleRateArray(stream, samplerate);
    if (CheckStreamActive() == false)
    {
        ReleaseFMIndirectMode(samplerate);
        PolicyRestore();
    }
    ret = stopOutput(output, stream, session);
    return ret;
}
...
status_t AudioMTKPolicyManager::SetPolicyManagerParameters(int par1, int par2 , int par3, int par4)
{
    ...
    switch (par1)
    {
        ...
        {
            ALOGD("Set POLICY_SET_FM_TX_ENABLE [%d] => [%d]", bFMTxEnable, par2);
            bFMTxEnable = par2;
        ...
        {
            ALOGD("Set POLICY_SET_FM_RX_FORCE_DISABLE_TX [%d] => [%d]", bFMTxEnable, par2);
            bFMTxEnable = (par2 ? 0 : 1);
            setForceUse(AudioSystem::NUM_FORCE_USE, AudioSystem::FORCE_NONE);
            break;
        }
        ...
        case POLICY_SET_LOSSLESS_BT_STATUS:
        {
            ALOGD("POLICY_SET_LOSSLESS_BT_STATUS=%d", par2);
            if (par2 == 0)  //close output if use close the config
            {
                AudioOutputDescriptor *outputDesc = NULL;
                for (size_t i = 0; i < mOutputs.size(); i++)
                {
                    AudioOutputDescriptor *desc = mOutputs.valueAt(i);
                    //get lossless output desc
                    if (!desc->isDuplicated() && (desc->mFlags &AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD != 0) &&
                        (desc->mFlags &AUDIO_DEVICE_OUT_ALL_A2DP != 0))
                    {
                        outputDesc = desc;
                    }
                }
...
#if defined(MTK_FM_SUPPORT_WFD_OUTPUT)||defined(MTK_FM_SUPPORT_A2DP_OUT)
...
void AudioMTKPolicyManager::LoadCustomVolume()
{
    ...
    ALOGD("LoadCustomVolume Audio_HAC_Custom_Param");
    android::GetHACSpeechParamFromNVRam(&Audio_HAC_Custom_Param);
#endif
}

void AudioMTKPolicyManager::InitSamplerateArray()
{
...
#if defined(MTK_HAC_SUPPORT)
                    String8 sHACEnable = mpClientInterface->getParameters(0, String8("GET_HAC_ENABLE"));
                    AudioParameter param = AudioParameter(sHACEnable);

                    if ((param.getInt(String8("GET_HAC_ENABLE"), value) == NO_ERROR) && (value == 1))
                    {
                        ALOGD("computeCustomVolume AudioSystem::VOICE_CALL DEVICE_OUT_EARPIECE HAC value=%d", value);
                        volmax = Audio_HAC_Custom_Param.audiovolume_sph_hac[GetStreamMaxLevels(stream) - 1];
                        volmin = Audio_HAC_Custom_Param.audiovolume_sph_hac[0];
                        volume = MapVoiceVoltoCustomVol(Audio_HAC_Custom_Param.audiovolume_sph_hac, 0, VOICE_VOLUME_MAX, volInt);

                    }
                    else
#endif
                    {
                        ALOGD("computeCustomVolume AudioSystem::VOICE_CALL DEVICE_OUT_EARPIECE");
