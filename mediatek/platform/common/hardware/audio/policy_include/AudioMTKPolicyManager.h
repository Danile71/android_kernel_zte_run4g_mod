...
// is switched
#define SONIFICATION_AUX_IGITAL_VOLUME_FACTOR 0.4
...
public:
        ...
        virtual status_t StartOutputSamplerate (audio_io_handle_t output,AudioSystem::stream_type stream,int session ,int samplerate);
        virtual status_t StopOutputSamplerate (audio_io_handle_t output,AudioSystem::stream_type stream,int session ,int samplerate);
        uint32_t GetSampleRateIndex(int sampleRate);
        status_t AddSampleRateArray(AudioSystem::stream_type stream,int sampleRate);
        status_t RemoveSampleRateArray(AudioSystem::stream_type stream,int sampleRate);
        void DumpSampleRateArray(void);
        bool CheckFirstActive(void);
        bool CheckStreamActive(void);
        status_t SampleRatePolicy(AudioSystem::stream_type stream,int sampleRate);
        status_t PolicyFirstStart(AudioSystem::stream_type stream,int sampleRate);
        status_t PolicyForceReplace(AudioSystem::stream_type stream,int sampleRate);
        status_t PolicyRestore(void);
        int  PolicyPropertyCheck(void);
        uint32_t GetFirstTrackSampleRate(void);
        bool SetFMIndirectMode(int SampleRate);
        bool ReleaseFMIndirectMode(int SampleRate);
        bool PrimarySupportSampleRate(int SampleRate);
        bool PrimaryDeviceSupportSampleRate(int SampleRate);
#endif
...
public:
        ...
        static AUDIO_CUSTOM_HAC_PARAM_STRUCT Audio_HAC_Custom_Param;
