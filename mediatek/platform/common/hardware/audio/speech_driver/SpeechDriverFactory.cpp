...
SpeechDriverFactory::SpeechDriverFactory()
{
    ...
    else if (MTK_ENABLE_MD5 == true) // if modem 1 & 2 not enabled, default use LTE/DSDA case
    {
        mActiveModemIndex = MODEM_EXTERNAL;
    }
    else
    {
...
#else
        ALOGD("Create SpeechDriverLAD for MODEM_EXTERNAL"); // for external modem LTE: use EEMCS
        mSpeechDriverExternal = SpeechDriverLAD::GetInstance(MODEM_EXTERNAL);
#endif
    }
    else
    {
#if defined (EVDO_DT_SUPPORT) // evdo modem speech driver
