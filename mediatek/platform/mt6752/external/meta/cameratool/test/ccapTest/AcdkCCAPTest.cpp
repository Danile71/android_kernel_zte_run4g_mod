...
MRESULT mrCCAPSingleShot_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    if (a_u4Argc != 4 && a_u4Argc != 6)
    {
        ACDK_LOGD("Usage: cap <mode, prv:0, cap:1> <format, 1:raw, 0:jpg> <MFLL: on/off> <Multi-NR: off(0),hw(1),sw(2)> <width (Option)> <height (Option)>\n");
        return E_CCT_CCAP_API_FAIL;
    }
    ...
    rCCTStillCapConfig.MFLL_En = atoi((char *)a_pprArgv[2]);
    rCCTStillCapConfig.eMultiNR = (EMultiNR_Mode)atoi((char *)a_pprArgv[3]);
    ...
    if  (a_u4Argc == 6)
    {
        rCCTStillCapConfig.u2JPEGEncWidth = atoi((char *)a_pprArgv[4]);
        rCCTStillCapConfig.u2JPEGEncHeight = atoi((char *)a_pprArgv[5]);
    }
    else
    {
...
MRESULT mrCCAPISPGetNvramData_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    nvram_buff.Mode= (CAMERA_NVRAM_STRUCTURE_ENUM)atoi((char *)a_pprArgv[0]); // 2: 3A,   3: ISP,   4: len
...
*/
    ...
    for (MUINT32 i = 0; i < EIsp_Num_Of_Category; i++)
    {
        ...
        printf("[%s]index(%d), ctgy(%d)", __FUNCTION__, acdk_cct_reg_idx.u4Index, acdk_cct_reg_idx.eCategory);
...
*/
    ...
    for (MUINT32 i = 0; i < EIsp_Num_Of_Category; i++)
    {
        ...
        ACDK_LOGD("Get: (bRet, u4RetLen)=(%d, %d) (Category, Index)=(%d, %d)\n", bRet, u4RetLen, eCategory, u4Index);
...
MRESULT mrCCAPGetTuningParas_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
	...
	ACDK_LOGD("param.stIspNvramRegs.OBC[0].offst0.bits = %d\n", param.stIspNvramRegs.OBC[0].offst0.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[1].offst0.bits = %d\n", param.stIspNvramRegs.OBC[1].offst1.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[2].offst2.bits = %d\n", param.stIspNvramRegs.OBC[2].offst2.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[3].offst3.bits = %d\n", param.stIspNvramRegs.OBC[3].offst3.bits);

	ACDK_LOGD("param.stIspNvramRegs.OBC[0].gain0.bits = %d\n", param.stIspNvramRegs.OBC[0].gain0.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[1].gain1.bits = %d\n", param.stIspNvramRegs.OBC[1].gain1.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[2].gain2.bits = %d\n", param.stIspNvramRegs.OBC[2].gain2.bits);
	ACDK_LOGD("param.stIspNvramRegs.OBC[3].gain3.bits = %d\n", param.stIspNvramRegs.OBC[3].gain3.bits);
/*
...
*/
    ...
    //only for OBC test
    param.u4Index   = 0;//u4Index;
    param.eCategory = EIsp_Category_OBC;//(ACDK_CCT_ISP_REG_CATEGORY)u4Category;
    int len = sizeof(ISP_NVRAM_OBC_T)*NVRAM_OBC_TBL_NUM;
    char* pBuf = (char*)malloc(len);
    ::memset(pBuf, 0xFF, len);  //  Set all to 0xFFFFFFFF
    
    param.bufAccess.length = len;
    param.bufAccess.pBuffer = pBuf;
...
MRESULT mrCCAPSetShadingTsfOnOff_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
    ...
    ACDK_LOGD("ACDK_CCT_V2_OP_ISP_SET_SHADING_TSF_ONOFF");
    if (a_u4Argc != 1)
    {
        ACDK_LOGD("Usage: setTsfOnOff <On/Off, ON:1, OFF:0>\n");
        return E_CCT_CCAP_API_FAIL;
    }

    UINT32 cctOnOff;

    cctOnOff = atoi((char *)a_pprArgv[0]);

    MUINT32 u4RetLen = 0;

    BOOL bRet = bSendDataToCCT(ACDK_CCT_V2_OP_ISP_SET_SHADING_TSF_ONOFF,
                                                       (UINT8 *)&cctOnOff,
                                                       sizeof(UINT32),
                                                       NULL,
                                                       0,
    ...
    ACDK_LOGD("CCAP Set TSF On/Off Success, Enable(%d)\n", cctOnOff);

    return S_CCT_CCAP_OK;
}

MRESULT mrCCAPGetShadingTsfOnOff_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
    ...
    ACDK_LOGD("ACDK_CCT_V2_OP_ISP_GET_SHADING_TSF_ONOFF");

    UINT32 cctOnOff;

    MUINT32 u4RetLen = 0;

    BOOL bRet = bSendDataToCCT(ACDK_CCT_V2_OP_ISP_GET_SHADING_TSF_ONOFF,
                                                       NULL,
                                                       0,
                                                       (UINT8 *)&cctOnOff,
                                                       sizeof(UINT32),
    ...
    ACDK_LOGD("CCAP Get TSF On/Off Success, Enable(%d)\n", cctOnOff);
...
MRESULT mrCCAPGetShadingTablePOLYCOEF_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    for (MINT32 i = 0; i < 3; i++)
    {
        ...
        ACDK_LOGD("Get Shading Preview Table_ColorTemp:%d\n", i);
        for (MINT32 j = 0; j  < MAX_SHADING_Preview_SIZE/8; j+=4)
        {
            printf("0x%08x,0x%08x,0x%08x,0x%08x,\n",
                uShadingTable[j], uShadingTable[j+1], uShadingTable[j+2], uShadingTable[j+3]);
        }
        printf("\n\n");
    }
    ...
    for (MINT32 i = 0; i < 3; i++)
    {
        ...
        ACDK_LOGD("Get Shading Capture Table_ColorTemp:%d\n", i);
        for (MINT32 j = 0; j  < MAX_SHADING_Capture_SIZE/8; j+=4)
        {
            printf("0x%08x,0x%08x,0x%08x,0x%08x,\n",
                uShadingTable[j], uShadingTable[j+1], uShadingTable[j+2], uShadingTable[j+3]);
        }
...
MRESULT mrCCAPSetShadingTablePOLYCOEF_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    // this table is for ov5642 capture , 15x15 blocks setting
    // preview should show some artifact after apply this table.
    UINT32 uShadingTable[MAX_SHADING_Capture_SIZE] ={0x11111111};

    memset(uShadingTable, 0x11, sizeof(UINT32)*MAX_SHADING_Capture_SIZE);
...
/*NeedUpdate*///ACDK_LOGD("Pre SVD Size :%d\n", pshadistr->PreviewSVDSize);
/*NeedUpdate*///ACDK_LOGD("Cap SVD Size :%d\n", pshadistr->CaptureSVDSize);
...
/////////////////////////////////////////////////////////////////////////
// ACDK_CCT_OP_SDTBL_LOAD_FROM_NVRAM, ACDK_CCT_OP_SDTBL_SAVE_TO_NVRAM
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPShadingNVRAM_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
    ...
    if (a_u4Argc != 1)
    {
        ACDK_LOGD("[%s] shadingNv read(0)/write(1)", __FUNCTION__);
        return S_CCT_CCAP_OK;
    }

    BOOL bRet;
    MINT32 mode = atoi((char *)a_pprArgv[0]);
    if (mode == 0)
    {
        ACDK_LOGD("[%s] Load from NVRAM", __FUNCTION__);
        bRet = bSendDataToCCT(ACDK_CCT_OP_SDTBL_LOAD_FROM_NVRAM, NULL, 0, NULL, 0, &u4RetLen);
    }
    else
    {
        ACDK_LOGD("[%s] Save to NVRAM", __FUNCTION__);
        bRet = bSendDataToCCT(ACDK_CCT_OP_SDTBL_SAVE_TO_NVRAM, NULL, 0, NULL, 0, &u4RetLen);        
    }

    if (!bRet)
    {
        ACDK_LOGD("[%s] Fail", __FUNCTION__);
        return E_CCT_CCAP_API_FAIL;
    }
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPAEGetPline_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    AE_PLINETABLE_T rAEPlineNVRAM;

    memset(&rAEPlineNVRAM,0, sizeof(AE_PLINETABLE_T));
    ...
    ACDK_LOGD("ACDK_CCT_OP_DEV_AE_GET_PLINE\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_OP_DEV_AE_GET_PLINE,
                                NULL,
                                0,
                                (UINT8 *)&rAEPlineNVRAM,
    ...
    // TEST ONLY (check AE parameter)
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].eAEScene);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[0]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[1]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[2]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[3]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[4]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[5]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[6]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[7]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[8]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[9]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAM.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[10]);
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPAEApplyPline_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    // AE NVRAM test data
    AE_PLINETABLE_T rAEPlineNVRAMTestData;

    memset(&rAEPlineNVRAMTestData,0, sizeof(AE_PLINETABLE_T));
    ...
    // TEST ONLY (check AE parameter)
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].eAEScene = LIB3A_AE_SCENE_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[0] = AETABLE_RPEVIEW_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[1] = AETABLE_CAPTURE_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[2] = AETABLE_VIDEO_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[3] = AETABLE_VIDEO1_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[4] = AETABLE_VIDEO2_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[5] = AETABLE_VIDEO_NIGHT;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[6] = AETABLE_CUSTOM1_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[7] = AETABLE_CUSTOM2_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[8] = AETABLE_CUSTOM3_AUTO;
    rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[9] = AETABLE_CUSTOM4_AUTO;

    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].eAEScene);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[0]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[1]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[2]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[3]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[4]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[5]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[6]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[7]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[8]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[9]);
    ACDK_LOGD("Mapping[0] = %d\n", rAEPlineNVRAMTestData.sAEScenePLineMapping.sAESceneMapping[0].ePLineID[10]);
    ...
    ACDK_LOGD("ACDK_CCT_OP_DEV_AE_APPLY_PLINE\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    bRet = bSendDataToCCT(ACDK_CCT_OP_DEV_AE_APPLY_PLINE,
                                (UINT8 *)&rAEPlineNVRAMTestData, // TEST ONLY
                                sizeof(AE_PLINETABLE_T),
                                NULL,
                                0,
                                &u4RetLen);
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPAESavePlineToNVRAM_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
    ...
    ACDK_LOGD("ACDK_CCT_OP_DEV_AE_SAVE_PLINE\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_OP_DEV_AE_SAVE_PLINE,
                                NULL,
                                0,
                                NULL,
                                0,
...
MRESULT mrCCAPAWBApplyCameraPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    memset(&rAWBNVRAMTestData,0, sizeof(rAWBNVRAMTestData));
...
MRESULT mrCCAPAWBGetAWBPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    ACDK_LOGD("rUnitGain.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rUnitGain.i4R);
    ACDK_LOGD("rUnitGain.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rUnitGain.i4G);
    ACDK_LOGD("rUnitGain.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rUnitGain.i4B);
    ACDK_LOGD("rGoldenGain.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rGoldenGain.i4R);
    ACDK_LOGD("rGoldenGain.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rGoldenGain.i4G);
    ACDK_LOGD("rGoldenGain.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rGoldenGain.i4B);
    ACDK_LOGD("rTuningUnitGain.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rTuningUnitGain.i4R);
    ACDK_LOGD("rTuningUnitGain.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rTuningUnitGain.i4G);
    ACDK_LOGD("rTuningUnitGain.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rTuningUnitGain.i4B);
    ACDK_LOGD("rD65Gain.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rD65Gain.i4R);
    ACDK_LOGD("rD65Gain.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rD65Gain.i4G);
    ACDK_LOGD("rD65Gain.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCalData.rD65Gain.i4B);


    ACDK_LOGD("rOriginalXY.rStrobe.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rStrobe.i4X);
    ACDK_LOGD("rOriginalXY.rStrobe.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rStrobe.i4Y);
    ACDK_LOGD("rOriginalXY.rHorizon.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rHorizon.i4X);
    ACDK_LOGD("rOriginalXY.rHorizon.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rHorizon.i4Y);
    ACDK_LOGD("rOriginalXY.rA.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rA.i4X);
    ACDK_LOGD("rOriginalXY.rA.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rA.i4Y);
    ACDK_LOGD("rOriginalXY.rTL84.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rTL84.i4X);
    ACDK_LOGD("rOriginalXY.rTL84.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rTL84.i4Y);
    ACDK_LOGD("rOriginalXY.rCWF.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rCWF.i4X);
    ACDK_LOGD("rOriginalXY.rCWF.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rCWF.i4Y);
    ACDK_LOGD("rOriginalXY.rDNP.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rDNP.i4X);
    ACDK_LOGD("rOriginalXY.rDNP.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rDNP.i4Y);
    ACDK_LOGD("rOriginalXY.rD65.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rD65.i4X);
    ACDK_LOGD("rOriginalXY.rD65.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rD65.i4Y);
    ACDK_LOGD("rOriginalXY.rDF.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rDF.i4X);
    ACDK_LOGD("rOriginalXY.rDF.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rOriginalXY.rDF.i4Y);

    ACDK_LOGD("rRotatedXY.rStrobe.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rStrobe.i4X);
    ACDK_LOGD("rRotatedXY.rStrobe.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rStrobe.i4Y);
    ACDK_LOGD("rRotatedXY.rHorizon.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rHorizon.i4X);
    ACDK_LOGD("rRotatedXY.rHorizon.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rHorizon.i4Y);
    ACDK_LOGD("rRotatedXY.rA.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rA.i4X);
    ACDK_LOGD("rRotatedXY.rA.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rA.i4Y);
    ACDK_LOGD("rRotatedXY.rTL84.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rTL84.i4X);
    ACDK_LOGD("rRotatedXY.rTL84.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rTL84.i4Y);
    ACDK_LOGD("rRotatedXY.rCWF.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rCWF.i4X);
    ACDK_LOGD("rRotatedXY.rCWF.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rCWF.i4Y);
    ACDK_LOGD("rRotatedXY.rDNP.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rDNP.i4X);
    ACDK_LOGD("rRotatedXY.rDNP.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rDNP.i4Y);
    ACDK_LOGD("rRotatedXY.rD65.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rD65.i4X);
    ACDK_LOGD("rRotatedXY.rD65.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rD65.i4Y);
    ACDK_LOGD("rRotatedXY.rDF.i4X = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rDF.i4X);
    ACDK_LOGD("rRotatedXY.rDF.i4Y = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotatedXY.rDF.i4Y);

    ACDK_LOGD("rStrobe.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rStrobe.i4R);
    ACDK_LOGD("rStrobe.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rStrobe.i4G);
    ACDK_LOGD("rStrobe.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rStrobe.i4B);
    ACDK_LOGD("rHorizon.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rHorizon.i4R);
    ACDK_LOGD("rHorizon.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rHorizon.i4G);
    ACDK_LOGD("rHorizon.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rHorizon.i4B);
    ACDK_LOGD("rA.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rA.i4R);
    ACDK_LOGD("rA.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rA.i4G);
    ACDK_LOGD("rA.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rA.i4B);
    ACDK_LOGD("rTL84.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rTL84.i4R);
    ACDK_LOGD("rTL84.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rTL84.i4G);
    ACDK_LOGD("rTL84.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rTL84.i4B);
    ACDK_LOGD("rCWF.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rCWF.i4R);
    ACDK_LOGD("rCWF.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rCWF.i4G);
    ACDK_LOGD("rCWF.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rCWF.i4B);
    ACDK_LOGD("rDNP.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDNP.i4R);
    ACDK_LOGD("rDNP.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDNP.i4G);
    ACDK_LOGD("rDNP.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDNP.i4B);
    ACDK_LOGD("rD65.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rD65.i4R);
    ACDK_LOGD("rD65.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rD65.i4G);
    ACDK_LOGD("rD65.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rD65.i4B);
    ACDK_LOGD("rDF.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDF.i4R);
    ACDK_LOGD("rDF.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDF.i4G);
    ACDK_LOGD("rDF.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rLightAWBGain.rDF.i4B);

    ACDK_LOGD("rRotationMatrix.i4RotationAngle = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotationMatrix.i4RotationAngle);
    ACDK_LOGD("rRotationMatrix.i4Cos = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotationMatrix.i4Cos);
    ACDK_LOGD("rRotationMatrix.i4Sin = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rRotationMatrix.i4Sin);

    ACDK_LOGD("rDaylightLocus.i4SlopeNumerator = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rDaylightLocus.i4SlopeNumerator);
    ACDK_LOGD("rDaylightLocus.i4SlopeDenominator = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rDaylightLocus.i4SlopeDenominator);

    ACDK_LOGD("rAWBNVRAM.rPredictorGain.i4PrefRatio100 = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.i4PrefRatio100);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_H.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_H.i4R);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_H.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_H.i4G);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_H.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_H.i4B);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_L.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_L.i4R);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_L.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_L.i4G);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rSpatial_DaylightLocus_L.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rSpatial_DaylightLocus_L.i4B);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rTemporal_General.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rTemporal_General.i4R);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rTemporal_General.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rTemporal_General.i4G);
    ACDK_LOGD("rAWBNVRAM.rPredictorGain.rTemporal_General.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPredictorGain.rTemporal_General.i4B);

    ACDK_LOGD("rAWBLightArea.rStrobe.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rStrobe.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rStrobe.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rStrobe.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rStrobe.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rStrobe.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rStrobe.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rStrobe.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rTungsten.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rTungsten.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rTungsten.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rTungsten.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rTungsten.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rTungsten.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rTungsten.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rTungsten.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rWarmFluorescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rWarmFluorescent.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rWarmFluorescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rWarmFluorescent.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rWarmFluorescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rWarmFluorescent.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rWarmFluorescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rWarmFluorescent.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rFluorescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rFluorescent.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rFluorescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rFluorescent.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rFluorescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rFluorescent.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rFluorescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rFluorescent.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rCWF.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rCWF.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rCWF.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rCWF.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rCWF.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rCWF.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rCWF.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rCWF.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rDaylight.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylight.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rDaylight.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylight.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rDaylight.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylight.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rDaylight.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylight.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rShade.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rShade.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rShade.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rShade.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rShade.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rShade.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rShade.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rShade.i4LowerBound);
    ACDK_LOGD("rAWBLightArea.rDaylightFluorescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylightFluorescent.i4RightBound);
    ACDK_LOGD("rAWBLightArea.rDaylightFluorescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylightFluorescent.i4LeftBound);
    ACDK_LOGD("rAWBLightArea.rDaylightFluorescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylightFluorescent.i4UpperBound);
    ACDK_LOGD("rAWBLightArea.rDaylightFluorescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAWBLightArea.rDaylightFluorescent.i4LowerBound);

    ACDK_LOGD("rPWBLightArea.rReferenceArea.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rReferenceArea.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rReferenceArea.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rReferenceArea.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rReferenceArea.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rReferenceArea.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rReferenceArea.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rReferenceArea.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rDaylight.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rDaylight.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rDaylight.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rDaylight.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rDaylight.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rDaylight.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rDaylight.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rDaylight.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rCloudyDaylight.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rCloudyDaylight.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rCloudyDaylight.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rCloudyDaylight.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rCloudyDaylight.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rCloudyDaylight.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rCloudyDaylight.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rCloudyDaylight.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rShade.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rShade.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rShade.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rShade.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rShade.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rShade.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rShade.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rShade.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rTwilight.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rTwilight.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rTwilight.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rTwilight.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rTwilight.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rTwilight.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rTwilight.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rTwilight.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rFluorescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rFluorescent.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rFluorescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rFluorescent.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rFluorescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rFluorescent.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rFluorescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rFluorescent.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rWarmFluorescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rWarmFluorescent.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rWarmFluorescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rWarmFluorescent.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rWarmFluorescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rWarmFluorescent.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rWarmFluorescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rWarmFluorescent.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rIncandescent.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rIncandescent.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rIncandescent.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rIncandescent.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rIncandescent.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rIncandescent.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rIncandescent.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rIncandescent.i4LowerBound);
    ACDK_LOGD("rPWBLightArea.rGrayWorld.i4RightBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rGrayWorld.i4RightBound);
    ACDK_LOGD("rPWBLightArea.rGrayWorld.i4LeftBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rGrayWorld.i4LeftBound);
    ACDK_LOGD("rPWBLightArea.rGrayWorld.i4UpperBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rGrayWorld.i4UpperBound);
    ACDK_LOGD("rPWBLightArea.rGrayWorld.i4LowerBound = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBLightArea.rGrayWorld.i4LowerBound);

    ACDK_LOGD("rPWBDefaultGain.rDaylight.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rDaylight.i4R);
    ACDK_LOGD("rPWBDefaultGain.rDaylight.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rDaylight.i4G);
    ACDK_LOGD("rPWBDefaultGain.rDaylight.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rDaylight.i4B);
    ACDK_LOGD("rPWBDefaultGain.rCloudyDaylight.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rCloudyDaylight.i4R);
    ACDK_LOGD("rPWBDefaultGain.rCloudyDaylight.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rCloudyDaylight.i4G);
    ACDK_LOGD("rPWBDefaultGain.rCloudyDaylight.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rCloudyDaylight.i4B);
    ACDK_LOGD("rPWBDefaultGain.rShade.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rShade.i4R);
    ACDK_LOGD("rPWBDefaultGain.rShade.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rShade.i4G);
    ACDK_LOGD("rPWBDefaultGain.rShade.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rShade.i4B);
    ACDK_LOGD("rPWBDefaultGain.rTwilight.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rTwilight.i4R);
    ACDK_LOGD("rPWBDefaultGain.rTwilight.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rTwilight.i4G);
    ACDK_LOGD("rPWBDefaultGain.rTwilight.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rTwilight.i4B);
    ACDK_LOGD("rPWBDefaultGain.rFluorescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rFluorescent.i4R);
    ACDK_LOGD("rPWBDefaultGain.rFluorescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rFluorescent.i4G);
    ACDK_LOGD("rPWBDefaultGain.rFluorescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rFluorescent.i4B);
    ACDK_LOGD("rPWBDefaultGain.rWarmFluorescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rWarmFluorescent.i4R);
    ACDK_LOGD("rPWBDefaultGain.rWarmFluorescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rWarmFluorescent.i4G);
    ACDK_LOGD("rPWBDefaultGain.rWarmFluorescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rWarmFluorescent.i4B);
    ACDK_LOGD("rPWBDefaultGain.rIncandescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rIncandescent.i4R);
    ACDK_LOGD("rPWBDefaultGain.rIncandescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rIncandescent.i4G);
    ACDK_LOGD("rPWBDefaultGain.rIncandescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rIncandescent.i4B);
    ACDK_LOGD("rPWBDefaultGain.rGrayWorld.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rGrayWorld.i4R);
    ACDK_LOGD("rPWBDefaultGain.rGrayWorld.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rGrayWorld.i4G);
    ACDK_LOGD("rPWBDefaultGain.rGrayWorld.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPWBDefaultGain.rGrayWorld.i4B);

    ACDK_LOGD("rPreferenceColor.rTungsten.i4SliderValue = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rTungsten.i4SliderValue);
    ACDK_LOGD("rPreferenceColor.rTungsten.i4OffsetThr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rTungsten.i4OffsetThr);
    ACDK_LOGD("rPreferenceColor.rWarmFluorescent.i4SliderValue = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rWarmFluorescent.i4SliderValue);
    ACDK_LOGD("rPreferenceColor.rWarmFluorescent.i4OffsetThr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rWarmFluorescent.i4OffsetThr);
    ACDK_LOGD("rPreferenceColor.rShade.i4SliderValue = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rShade.i4SliderValue);
    ACDK_LOGD("rPreferenceColor.rShade.i4OffsetThr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rShade.i4OffsetThr);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Strobe.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Strobe.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Strobe.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Strobe.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Strobe.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Strobe.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Tungsten.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Tungsten.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Tungsten.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Tungsten.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Tungsten.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Tungsten.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_WarmFluorescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_WarmFluorescent.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_WarmFluorescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_WarmFluorescent.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_WarmFluorescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_WarmFluorescent.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Fluorescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Fluorescent.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Fluorescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Fluorescent.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Fluorescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Fluorescent.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_CWF.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_CWF.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_CWF.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_CWF.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_CWF.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_CWF.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Daylight.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Daylight.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Daylight.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Daylight.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Daylight.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Daylight.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Shade.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Shade.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Shade.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Shade.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_Shade.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_Shade.i4B);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4R);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4G);
    ACDK_LOGD("rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rPreferenceColor.rPreferenceGain_DaylightFluorescent.i4B);

    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.bAWBBackupEnable = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.bAWBBackupEnable);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rAWBGain_LSC.i4R = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rAWBGain_LSC.i4R);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rAWBGain_LSC.i4G = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rAWBGain_LSC.i4G);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rAWBGain_LSC.i4B = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rAWBGain_LSC.i4B);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rParentBlkWeightParam.bEnable = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rParentBlkWeightParam.bEnable);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rParentBlkWeightParam.i4ScalingFactor = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rParentBlkWeightParam.i4ScalingFactor);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rPredictorLVThr.i4EnqueueLVThr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rPredictorLVThr.i4EnqueueLVThr);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rPredictorLVThr.i4InitLVThr_H = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rPredictorLVThr.i4InitLVThr_H);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rPredictorLVThr.i4InitLVThr_L = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rPredictorLVThr.i4InitLVThr_L);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rTemporalEnqueueThr.i4Neutral_ParentBlk_Thr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rTemporalEnqueueThr.i4Neutral_ParentBlk_Thr);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rFeaturePropThr.rSunsetProp.i4LVThr = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rFeaturePropThr.rSunsetProp.i4LVThr);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rLowCCTGainLimit.rNormalLowCCT.bEnable = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rLowCCTGainLimit.rNormalLowCCT.bEnable);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rLowCCTGainLimit.rNormalLowCCT.i4GainRatio = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rLowCCTGainLimit.rNormalLowCCT.i4GainRatio);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rLowCCTGainLimit.rPrefLowCCT.bEnable = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rLowCCTGainLimit.rPrefLowCCT.bEnable);
    ACDK_LOGD("rAWBNVRAM.rAlgoTuningParam.rLowCCTGainLimit.rPrefLowCCT.i4GainRatio = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rAlgoTuningParam.rLowCCTGainLimit.rPrefLowCCT.i4GainRatio);

    ACDK_LOGD("rCCTEstimation.i4CCT[0] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4CCT[0]);
    ACDK_LOGD("rCCTEstimation.i4CCT[1] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4CCT[1]);
    ACDK_LOGD("rCCTEstimation.i4CCT[2] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4CCT[2]);
    ACDK_LOGD("rCCTEstimation.i4CCT[3] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4CCT[3]);
    ACDK_LOGD("rCCTEstimation.i4CCT[4] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4CCT[4]);

    ACDK_LOGD("rCCTEstimation.i4RotatedXCoordinate[0] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4RotatedXCoordinate[0]);
    ACDK_LOGD("rCCTEstimation.i4RotatedXCoordinate[1] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4RotatedXCoordinate[1]);
    ACDK_LOGD("rCCTEstimation.i4RotatedXCoordinate[2] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4RotatedXCoordinate[2]);
    ACDK_LOGD("rCCTEstimation.i4RotatedXCoordinate[3] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4RotatedXCoordinate[3]);
    ACDK_LOGD("rCCTEstimation.i4RotatedXCoordinate[4] = %d\n", rAWBNVRAM[AWB_NVRAM_IDX_VHDR].rCCTEstimation.i4RotatedXCoordinate[4]);
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPFlashAWBApplyCameraPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    // Flash AWB NVRAM test data
    FLASH_AWB_NVRAM_T rFlashAWBNVRAMTestData;

    memset(&rFlashAWBNVRAMTestData,0, sizeof(rFlashAWBNVRAMTestData)); // TEST ONLY
    ...
    ACDK_LOGD("ACDK_CCT_OP_AWB_APPLY_FLASH_AWB_PARA\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_OP_AWB_APPLY_FLASH_AWB_PARA,
                                (UINT8 *)&rFlashAWBNVRAMTestData, // TEST ONLY
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPFlashAWBGetAWBPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    FLASH_AWB_NVRAM_T rFlashAWBNVRAM; // TEST ONLY

    memset(&rFlashAWBNVRAM,0, sizeof(rFlashAWBNVRAM)); // TEST ONLY
    ...
    ACDK_LOGD("ACDK_CCT_OP_AWB_GET_FLASH_AWB_PARA\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_OP_AWB_GET_FLASH_AWB_PARA,
                                NULL,
                                0,
                                (UINT8 *)&rFlashAWBNVRAM,
                                sizeof(FLASH_AWB_NVRAM_T),
                                &u4RetLen);

    // TEST ONLY (check AWB parameter)
    ACDK_LOGD("ForeGroundPercentage = %d\n", rFlashAWBNVRAM.rTuningParam.ForeGroundPercentage);
    ACDK_LOGD("BackGroundPercentage = %d\n", rFlashAWBNVRAM.rTuningParam.BackGroundPercentage);
    ACDK_LOGD("FgPercentage_Th1 = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th1);
    ACDK_LOGD("FgPercentage_Th2 = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th2);
    ACDK_LOGD("FgPercentage_Th3 = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th3);
    ACDK_LOGD("FgPercentage_Th4 = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th4);
    ACDK_LOGD("FgPercentage_Th1_Val = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th1_Val);
    ACDK_LOGD("FgPercentage_Th2_Val = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th2_Val);
    ACDK_LOGD("FgPercentage_Th3_Val = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th3_Val);
    ACDK_LOGD("FgPercentage_Th4_Val = %d\n", rFlashAWBNVRAM.rTuningParam.FgPercentage_Th4_Val);
    ACDK_LOGD("location_map_th1 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_th1);
    ACDK_LOGD("location_map_th2 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_th2);
    ACDK_LOGD("location_map_th3 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_th3);
    ACDK_LOGD("location_map_th4 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_th4);
    ACDK_LOGD("location_map_val1 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_val1);
    ACDK_LOGD("location_map_val2 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_val2);
    ACDK_LOGD("location_map_val3 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_val3);
    ACDK_LOGD("location_map_val4 = %d\n", rFlashAWBNVRAM.rTuningParam.location_map_val4);
    ACDK_LOGD("SelfTuningFbBgWeightTbl = %d\n", rFlashAWBNVRAM.rTuningParam.SelfTuningFbBgWeightTbl);
    ACDK_LOGD("FgBgTbl_Y0 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y0);
    ACDK_LOGD("FgBgTbl_Y1 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y1);
    ACDK_LOGD("FgBgTbl_Y2 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y2);
    ACDK_LOGD("FgBgTbl_Y3 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y3);
    ACDK_LOGD("FgBgTbl_Y4 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y4);
    ACDK_LOGD("FgBgTbl_Y5 = %d\n", rFlashAWBNVRAM.rTuningParam.FgBgTbl_Y5);
    ACDK_LOGD("YPrimeWeightTh[0] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeightTh[0]);
    ACDK_LOGD("YPrimeWeightTh[1] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeightTh[1]);
    ACDK_LOGD("YPrimeWeightTh[2] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeightTh[2]);
    ACDK_LOGD("YPrimeWeightTh[3] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeightTh[3]);
    ACDK_LOGD("YPrimeWeightTh[4] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeightTh[4]);
    ACDK_LOGD("YPrimeWeight[0] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeight[0]);
    ACDK_LOGD("YPrimeWeight[1] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeight[1]);
    ACDK_LOGD("YPrimeWeight[2] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeight[2]);
    ACDK_LOGD("YPrimeWeight[3] = %d\n", rFlashAWBNVRAM.rTuningParam.YPrimeWeight[3]);
    ACDK_LOGD("flashWBGain[0].i4R = %d\n", rFlashAWBNVRAM.rCalibrationData.flashWBGain[0].i4R);
    ACDK_LOGD("flashWBGain[0].i4G = %d\n", rFlashAWBNVRAM.rCalibrationData.flashWBGain[0].i4G);
    ACDK_LOGD("flashWBGain[0].i4B = %d\n", rFlashAWBNVRAM.rCalibrationData.flashWBGain[0].i4B);
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPFlashAWBSaveAWBPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
    ...
    ACDK_LOGD("ACDK_CCT_OP_AWB_SAVE_FLASH_AWB_PARA\n");
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_OP_AWB_SAVE_FLASH_AWB_PARA,
                                NULL,
                                0,
                                NULL,
                                0,
...
/*NeedUpdate*///sLensPara.rAFNVRAM.sAF_TH.i4HW_TH[0][0] = 1;
...
MRESULT mrCCAPSetPcaTable_Cmd(const MUINT32 a_u4Argc, MUINT8*a_pprArgv[])
{
    ...
    ::memset(&access_set, 0xFF, sizeof(access_set));

    if (a_u4Argc != 2)
    {
        ACDK_LOGD("Usage: setPcaTable <color temperature: 0~2 > <0: origianl PCA, 1: HDR PCA>\n");
        return E_CCT_CCAP_API_FAIL;
    }
    access_set.u4Offset = 0;
    access_set.u4Count  = PCA_BIN_NUM;
    access_set.u8ColorTemperature = ::atoi((char *)a_pprArgv[0]);
    access_set.u8Index= ::atoi((char *)a_pprArgv[1]);
    ...
    access_get.u8Index = access_set.u8Index;
...
MRESULT mrCCAPGetPcaTable_Cmd(const MUINT32 a_u4Argc, MUINT8*a_pprArgv[])
{
    ...
    if (a_u4Argc != 2)
    {
        ACDK_LOGD("Usage: getPcaTable <color temperature: 0~2 > <0: origianl PCA, 1: HDR PCA>\n");
        return E_CCT_CCAP_API_FAIL;
    }
    access.u4Offset = 0;
    access.u4Count  = PCA_BIN_NUM;
    access.u8ColorTemperature = ::atoi((char *)a_pprArgv[0]);
    access.u8Index= ::atoi((char *)a_pprArgv[1]);
...
// flash start
//=====================================
MRESULT mrCCAP_FlashCalibration(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
    }

    ACDK_LOGD("mrCCAP_FlashCalibration\n");

    int ret;
    ret = bSendDataToCCT(ACDK_CCT_OP_FLASH_CALIBRATION,
                                0,
                                0,
                                0,
                                0,
...
MRESULT mrCCAP_FlashGetInfo(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ACDK_LOGD("%s() line=%d\n", __FUNCTION__, __LINE__);
	if (!g_bAcdkOpend )
    {
    ...
    int ret;
    int isOn;
    ret = bSendDataToCCT(ACDK_CCT_OP_FLASH_GET_INFO,
                                (UINT8*)0,
                                0,
                                (UINT8*)&isOn,
                                0,
...
MRESULT mrCCAP_WriteNvram(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    MUINT32 u4RetLen = 0;
    BOOL ret = bSendDataToCCT(ACDK_CCT_OP_STROBE_WRITE_NVRAM,
                                (UINT8*)0,
                                0,
                                (UINT8*)0,
                                0,
                                &u4RetLen);
    ACDK_LOGD("mrCCAP_WriteNvram- ret=%d\n",ret);
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
...
MRESULT mrCCAP_ReadNvram(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    buf = new char[10000];
    MUINT32 u4RetLen = 0;
    BOOL ret = bSendDataToCCT(ACDK_CCT_OP_STROBE_READ_NVRAM_TO_PC_META,
                                (UINT8*)0,
                                0,
                                (UINT8*)buf,
                                0,
                                &u4RetLen);
    ACDK_LOGD("mrCCAP_ReadNvram- ret=%d\n",ret);
    delete []buf;
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
...
MRESULT mrCCAP_SetNvram(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
	ACDK_LOGD("mrCCAP_SetNvram\n");
	if (!g_bAcdkOpend )
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
    ...
    BOOL ret = bSendDataToCCT(ACDK_CCT_OP_STROBE_READ_NVRAM_TO_PC_META,
                                ...
                                0,
                                &u4RetLen);
    ACDK_LOGD("mrCCAP_SetNvram- line=%d ret=%d\n", __LINE__, ret);
    ret = bSendDataToCCT(ACDK_CCT_OP_STROBE_SET_NVDATA_META,
                                (UINT8*)buf,
                                sizeof(ACDK_STROBE_STRUCT),
                                (UINT8*)0,
                                0,
                                &u4RetLen);
    ACDK_LOGD("mrCCAP_SetNvram- ret=%d\n",ret);
    delete []buf;
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
...
MRESULT mrCCAP_flashLedTest(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
	ACDK_LOGD("mrCCAP_flashLedTest\n");
	if (!g_bAcdkOpend )
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
    ...
    int ret;
    ret = bSendDataToCCT(ACDK_CCT_OP_FLASH_CONTROL,
                                0,
                                0,
                                0,
                                0,
                                0);
    ACDK_LOGD("mrCCAP_flashLedTest- ret=%d\n",ret);
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
...
MRESULT mrCCAP_flashEnable(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
	ACDK_LOGD("mrCCAP_flashEnable\n");
	if (!g_bAcdkOpend )
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
    ...
    int ret;
    ret = bSendDataToCCT(ACDK_CCT_OP_FLASH_ENABLE,
                                0,
                                0,
                                0,
                                0,
                                0);
    ACDK_LOGD("mrCCAP_flashEnable- ret=%d\n",ret);
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
...
MRESULT mrCCAP_flashDisable(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
	ACDK_LOGD("mrCCAP_flashDisable\n");
	if (!g_bAcdkOpend )
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
    ...
    int ret;
    ret = bSendDataToCCT(ACDK_CCT_OP_FLASH_DISABLE,
                                0,
                                0,
                                0,
                                0,
                                0);
    ACDK_LOGD("mrCCAP_flashDisable- ret=%d\n",ret);
    if (!ret)
    {
        ACDK_LOGD("%s() line=%d merror:\n", __FUNCTION__, __LINE__);
        return E_CCT_CCAP_API_FAIL;
...
MRESULT mrAEPlineTableLinearity_cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    ...
    memset (&rAEPlineTableTestOutput, 0, sizeof(ACDK_CDVT_AE_PLINE_TEST_OUTPUT_T));
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPCaptureFeatureApplyPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    // Flash AWB NVRAM test data
    NVRAM_CAMERA_FEATURE_STRUCT rCaptureFeatureNVRAM;

    memset(&rCaptureFeatureNVRAM,0, sizeof(rCaptureFeatureNVRAM)); // TEST ONLY
	rCaptureFeatureNVRAM.mfll.mfll_iso_th = (MINT32)atoi((char *)a_pprArgv[0]);
    ...
    BOOL bRet = bSendDataToCCT(ACDK_CCT_APPLY_CAPTURE_FEATURE_PARAS,
                                (UINT8 *)&rCaptureFeatureNVRAM, // TEST ONLY
...
/////////////////////////////////////////////////////////////////////////
// ACDK_CCT_GET_CAPTURE_FEATURE_PARAS
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPCaptureFeatureGetPara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    NVRAM_CAMERA_FEATURE_STRUCT rCaptureFeatureNVRAM; // TEST ONLY

    ACDK_LOGD("mrCCAPCaptureFeatureGetPara_Cmd\n");
    memset(&rCaptureFeatureNVRAM,0, sizeof(rCaptureFeatureNVRAM)); // TEST ONLY
    ...
    ACDK_LOGD("ACDK_CCT_GET_CAPTURE_FEATURE_PARAS\n");
    ...
    ACDK_LOGD("\n[swnr]\n");
    //
    ACDK_LOGD("ITUNE_ANR_PTU_STD = %d\n", rCaptureFeatureNVRAM.swnr[0].ITUNE_ANR_PTU_STD);
    ACDK_LOGD("ITUNE_ANR_PTV_STD = %d\n", rCaptureFeatureNVRAM.swnr[0].ITUNE_ANR_PTV_STD);
    ACDK_LOGD("ANR_ACT_TH_C = %d\n", rCaptureFeatureNVRAM.swnr[0].ANR_ACT_TH_C);
    ACDK_LOGD("ANR_ACT_BLD_BASE_C = %d\n", rCaptureFeatureNVRAM.swnr[0].ANR_ACT_BLD_BASE_C);
    ACDK_LOGD("ANR_ACT_BLD_TH_C = %d\n", rCaptureFeatureNVRAM.swnr[0].ANR_ACT_BLD_TH_C);
    ACDK_LOGD("ANR_ACT_SLANT_C = %d\n", rCaptureFeatureNVRAM.swnr[0].ANR_ACT_SLANT_C);
#endif
...
/////////////////////////////////////////////////////////////////////////
MRESULT mrCCAPCaptureFeatureSavePara_Cmd(const MUINT32 a_u4Argc, MUINT8 *a_pprArgv[])
{
    if (!g_bAcdkOpend )
    {
        return E_CCT_CCAP_API_FAIL;
...
static const Acdk_CLICmd g_prAcdkCCAPCLICmds [] =
{
    ...
    {"cap",                            "FT_MSDK_CCT_OP_SINGLE_SHOT_CAPTURE, (cap <mode> <type> <MFLL> <MultiNR> <width (Option)> <height (Option)>)",   mrCCAPSingleShot_Cmd},
    ...
    {"gAEPline",         "ACDK_CCT_OP_DEV_AE_GET_PLINE",           mrCCAPAEGetPline_Cmd},
    {"aAEPline",          "ACDK_CCT_OP_DEV_AE_APPLY_PLINE",         mrCCAPAEApplyPline_Cmd},
    {"sAEPline",         "ACDK_CCT_OP_DEV_AE_SAVE_PLINE",    mrCCAPAESavePlineToNVRAM_Cmd},
    ...
    {"aFlashAWBParam",    "ACDK_CCT_OP_AWB_APPLY_FLASH_AWB_PARA",  mrCCAPFlashAWBApplyCameraPara_Cmd},
    {"gFlashAWBParam",    "ACDK_CCT_OP_AWB_GET_FLASH_AWB_PARA",    mrCCAPFlashAWBGetAWBPara_Cmd},
    {"sFlashAWBParam",    "ACDK_CCT_OP_AWB_SAVE_FLASH_AWB_PARA",   mrCCAPFlashAWBSaveAWBPara_Cmd},
    ...
    {"setLscPara",          "FT_MSDK_CCT_V2_OP_ISP_SET_SHADING_PARA",           mrCCAPSetShadingPara_Cmd},
    {"getLscPara",          "FT_MSDK_CCT_V2_OP_ISP_GET_SHADING_PARA",           mrCCAPGetShadingPara_Cmd},
    {"setTsfAwbForce",      "ACDK_CCT_V2_OP_ISP_SET_SHADING_TSFAWB_FORCE",      mrCCAPSetShadingTsfAwbForce_Cmd},
    {"getTsfOnOff",         "ACDK_CCT_V2_OP_ISP_GET_SHADING_TSF_ONOFF",         mrCCAPGetShadingTsfOnOff_Cmd},
    {"setTsfOnOff",         "ACDK_CCT_V2_OP_ISP_SET_SHADING_TSF_ONOFF",         mrCCAPSetShadingTsfOnOff_Cmd},
    ...
    {"shadingNv",           "SDTBL_LOAD_FROM_NVRAM/SDTBL_SAVE_TO_NVRAM",        mrCCAPShadingNVRAM_Cmd},
    ...
    {"flash_calibration", "ACDK_CCT_OP_FLASH_CALIBRATION", mrCCAP_FlashCalibration},
	{"flash_read_nvram", "ACDK_CCT_OP_STROBE_READ_NVRAM_TO_PC_META", mrCCAP_ReadNvram},
	{"flash_set_nvram", "ACDK_CCT_OP_STROBE_SET_NVDATA_META", mrCCAP_SetNvram},
	{"flash_write_nvram", "ACDK_CCT_OP_STROBE_WRITE_NVRAM", mrCCAP_WriteNvram},
	{"flash_enable", "ACDK_CCT_OP_FLASH_ENABLE", mrCCAP_flashEnable},
	{"flash_disable", "ACDK_CCT_OP_FLASH_DISABLE", mrCCAP_flashDisable},
	{"flash_led_test", "ACDK_CCT_OP_FLASH_CONTROL", mrCCAP_flashLedTest},
	{"flash_get_info", "ACDK_CCT_OP_FLASH_GET_INFO", mrCCAP_FlashGetInfo},
...
*/
    ...
    {"aCaptureFeatureParam",    "ACDK_CCT_APPLY_CAPTURE_FEATURE_PARAS",  mrCCAPCaptureFeatureApplyPara_Cmd},
    {"gCaptureFeatureParam",    "ACDK_CCT_GET_CAPTURE_FEATURE_PARAS",    mrCCAPCaptureFeatureGetPara_Cmd},
    {"sCaptureFeatureParam",    "ACDK_CCT_SAVE_CAPTURE_FEATURE_PARAS",   mrCCAPCaptureFeatureSavePara_Cmd},
