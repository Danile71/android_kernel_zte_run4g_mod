...
static dfo_boot_info g_dfoBoots[DFO_BOOT_COUNT];
...
bool    META_Dfo_Init(void)
{
    ...
    if (fp)
    {
        ...
        if (lkSize > 0)
        {
            ...
            do
            {
                ...
                if (strcmp(name, "DFO") == 0)
                {
                    ...
                    for (; i < g_dfoBootCount; i++)
                    {
                        ...
                        strcpy(g_dfoBoots[i].name, dfoName);
                        g_dfoBoots[i].value = atoi(dfoValue);
                    }
            ...
            if (!g_hasDfoBoot)
            {
                memcpy(&g_dfoBoots, &dfo_boot_default, sizeof(dfo_boot_info)*DFO_BOOT_COUNT);
                g_dfoBootCount = DFO_BOOT_COUNT;
            }
...
unsigned char    Dfo_ReadValue_OP(Dfo_read_req* req, Dfo_read_cnf* cnf)
{
    ...
    else
    {
        ...
        strcpy(cnf->name, g_dfoBoots[index].name);
        cnf->value = g_dfoBoots[index].value;
...
unsigned char    Dfo_WriteValue_OP(Dfo_write_req* req)
{
    ...
    else    // MISC, partition == 1
    {
        ...
        for (; i < g_dfoBootCount; i++)
        {
            if (strcmp(g_dfoBoots[i].name, req->name) == 0)
            {
                // find it
                ALOGI("[MISC] Dfo write: (%s, %d)", req->name, req->value);
                g_dfoBoots[i].value = req->value;
                break;
            }
        }
    }
    ...
    if (req->save)
    {
        ...
        for (; i < g_dfoBootCount; i++)
        {
            sprintf(tmp, "%d", g_dfoBoots[i].value);
            if (i != 0)
                size += 1;     // ','
            size += strlen(g_dfoBoots[i].name);
            size += 1;     // ','
            size += strlen(tmp);
        }
        ...
        {
            if (i != 0)
                rtn = snprintf(ptr, remain, ",%s,%d", g_dfoBoots[i].name, g_dfoBoots[i].value);
            else
                rtn = snprintf(ptr, remain, "%s,%d", g_dfoBoots[i].name, g_dfoBoots[i].value);
            ptr += rtn;
            remain -= rtn;
...
void    META_Dfo_OP(FT_DFO_REQ* req)
{
    ...
    case DFO_OP_COMBO_UPDATE:
        ...
        break;
    default:	
        ALOGE("Error: unsupport DFO_OP code = %d\n", req->op);	  	
        break;
    }
