...
{
    if (devices == AUDIO_DEVICE_IN_FM) // FM
    {
        ...
        {
            ALOGE("%s(), AUDIO_DEVICE_IN_FM, wrong sampleRate %d, use %d instead.", __FUNCTION__, *sampleRate, fm_uplink_sampling_rate);
    ...
    else // Normal record
    {
        ...
        {
            ALOGE("%s(), wrong sampleRate %d, use %d instead.", __FUNCTION__, *sampleRate, kDefaultInputSourceSampleRate);
...
{
    ALOGD("%s(), devices = 0x%x, format = 0x%x, channels = 0x%x, sampleRate = %d, acoustics = 0x%x",
          __FUNCTION__, devices, *format, *channels, *sampleRate, acoustics);
...
uint32_t AudioALSAStreamIn::bufferSize() const
{
    ALOGV("%s(), return 0x%x", __FUNCTION__, mStreamAttributeTarget.buffer_size);
    return 0x1000; //mStreamAttributeTarget.buffer_size; // TODO(Harvey): Check
}
...
void AudioALSAStreamIn::OpenPCMDump()
{
    ...
    char Buf[10];
    sprintf(Buf, "%d.pcm", mDumpFileNum);
