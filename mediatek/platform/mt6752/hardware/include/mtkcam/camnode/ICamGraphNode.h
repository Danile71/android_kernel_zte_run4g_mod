...
#define NG_TRACE_NAME_LENGTH                 64

#define NG_TRACE_CALL()                      ATRACE_CALL()
#define NG_TRACE_NAME(name)                  ATRACE_NAME(name)
#define NG_TRACE_INT(name, value)            ATRACE_INT(name, value)
#define NG_TRACE_BEGIN(name)                 ATRACE_BEGIN(name)
#define NG_TRACE_END()                       ATRACE_END()
#define NG_TRACE_FMT_BEGIN(fmt, arg...)                  \
do{                                                      \
    if( ATRACE_ENABLED() )                               \
    {                                                    \
        char buf[NG_TRACE_NAME_LENGTH];                  \
        snprintf(buf, NG_TRACE_NAME_LENGTH, fmt, ##arg); \
        NG_TRACE_BEGIN(buf);                             \
    }                                                    \
}while(0)
#define NG_TRACE_FMT_END()                   NG_TRACE_END()
//#define NG_TRACE_ASYNC_BEGIN(name, cookie)   ATRACE_ASYNC_BEGIN(name, cookie)
//#define NG_TRACE_ASYNC_END(name, cookie)     ATRACE_ASYNC_END(name, cookie)
#else
#define NG_TRACE_CALL()
#define NG_TRACE_NAME(name)
#define NG_TRACE_INT(name, value)
#define NG_TRACE_BEGIN(name)
#define NG_TRACE_END()
#define NG_TRACE_FMT_BEGIN(fmt, arg)
#define NG_TRACE_FMT_END()
//#define NG_TRACE_ASYNC_BEGIN(name, cookie)
//#define NG_TRACE_ASYNC_END(name, cookie)
#endif
