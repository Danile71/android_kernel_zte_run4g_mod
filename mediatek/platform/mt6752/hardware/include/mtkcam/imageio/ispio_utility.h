...
static MUINT32 _queryXsize_imgo(EImageFormat imgFmt,MUINT32& xsize, MUINT32& outWidth,MUINT32 pixMode){    
...
/**
    ...
    input param:
        imgWidth: image horizontal resolution, uint: pix
    output param:
...
static MUINT32 ISP_QuerySize(MUINT32 portId,/*E_ISP_QUERY*/MUINT32 e_Op,EImageFormat imgFmt,MUINT32 imgWidth,ISP_QUERY_RST &QueryRst ,E_ISP_PIXMODE e_PixMode=ISP_QUERY_UNKNOWN_PIX_MODE){
    MUINT32 xsize,outWidth=imgWidth;
    bool p2_cfg=false;
    if(portId == _BY_PASSS_PORT){
        switch(imgFmt){
            ...
            case eImgFmt_BAYER12:
            case eImgFmt_BLOB:
                if(_queryXsize_imgo(imgFmt,xsize,outWidth,e_PixMode) == 0)
                    return 0;
    ...
    ISPUTIL_MSG("id:%d fmt:0x%x in_w:%d pixMode:%d pix_x:%d stride_pix:%d stride_byte:%d, xsize_byte:%d\n",portId,\
        imgFmt,imgWidth,e_PixMode,QueryRst.x_pix,QueryRst.stride_pix,QueryRst.stride_byte,QueryRst.xsize_byte);
    return 1;
}
