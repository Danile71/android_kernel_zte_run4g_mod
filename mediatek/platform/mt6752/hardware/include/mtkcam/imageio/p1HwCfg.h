...
#include "../mtkcam/core/imageio/inc/imageio_log.h"
...
public:
    ...
    inline MBOOL cqAddModule(IspDrv* pVirIspDrv,MINT32 cq,MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
    {
        return pVirIspDrv->cqAddModule(cq,0, dupCqIdx, moduleId);//p1 burst Q num 0
    }

    inline MBOOL cqDelModule(IspDrv* pVirIspDrv, MINT32 cq,MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
    {
        return pVirIspDrv->cqDelModule(cq,0, dupCqIdx, moduleId);//p1 burst Q num 0
    }
...
public:
   ...
   MBOOL releaseModuleHandle(MINT32 handle, char const* szCallerName);
   MBOOL finishModuleCfg(MINT32 handle);

   MBOOL setModuleEn(CAM_MODULE_ENUM module, MBOOL en);
   MBOOL setModuleSel(HWRWCTRL_P1SEL_MODULE module, MUINT8 val);
   void Lock(void) {mLock.lock();}
   void UnLock(void){mLock.unlock();}
   MBOOL dbgDump(MINT32 handle, MUINT32 tgIdx);
