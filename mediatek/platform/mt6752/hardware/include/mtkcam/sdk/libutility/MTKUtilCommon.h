...
/**
 *  \details utility image processing order for multi-core
 */
typedef enum UTIL_IMAGE_PROC_ORDER
{
    UTIL_ROW_MAJOR_PROCESSING,
    UTIL_COL_MAJOR_PROCESSING,
    UTIL_IMAGE_PROC_ORDER_NUM
} UTIL_IMAGE_PROC_ORDER;

/**
 *  \details utility image structure for multi-core
 */
typedef struct UTIL_MP_IMAGE_STRUCT : UTIL_BASE_IMAGE_STRUCT
{
    MINT32 proc_idx;                    ///< processing index
    UTIL_IMAGE_PROC_ORDER proc_order;   ///< processing order: 0 = row major, 1 = column major
    MINT32 proc_width;                  ///< processing width
    MINT32 proc_height;                 ///< processing height
} UTIL_MP_IMAGE_STRUCT, *P_UTIL_MP_IMAGE_STRUCT;
