...
int vAllocate_Deallocate_basic()
{
    ...
    ret = CM4u.m4u_cache_sync(M4U_PORT_DISP_OVL0, M4U_CACHE_FLUSH_BY_RANGE, 
                    (unsigned int)BufAddr, BufSize, BufMVA);
    if(ret)
    {
    ...
    ret = CM4u.m4u_dealloc_mva(M4U_PORT_DISP_OVL0, (unsigned int)BufAddr, BufSize, BufMVA);
    if(ret)
    {
...
int ion_m4u_misc_using()
{
    ...
    ret = CM4u.m4u_cache_sync(0, M4U_CACHE_FLUSH_BY_RANGE, 
                    (unsigned int)pBuf,bufsize, BufMVA);
    if(ret)
    {
        printf("cache flush fail. ret=%d,va=0x%x,size=0x%x\n", ret,pBuf,bufsize);
        return ret;
    }
    ret = CM4u.m4u_dealloc_mva(0, (unsigned int)pBuf,bufsize, BufMVA);
    if(ret)
    {
        printf("m4u_dealloc_mva fail. ret=%d, mva=0x%x\n", ret, BufMVA);
    }
