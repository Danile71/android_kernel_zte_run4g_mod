...
static jboolean enablePQColor(JNIEnv *env, jobject thiz, int isEnable)
{
    ...
    XLOGD("[MiraVision][JNI] enablePQColor(), enable[%d]", isEnable);

    if (drvID < 0)
    {
        XLOGE("[MiraVision][JNI] open device fail!!");
        return JNI_FALSE;
    }
...
static jint getPictureMode(JNIEnv *env, jobject thiz)
{
    ...
    mode = atoi(value);
    XLOGD("[MiraVision][JNI] getPictureMode(), property get [%d]", mode);
...
static jboolean setPictureMode(JNIEnv *env, jobject thiz, int mode)
{
    ...
    XLOGD("[MiraVision][JNI] property set... picture mode[%d]", mode);

    if (mode == PQ_PIC_MODE_STANDARD)
    {
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, gsat[%d], cont[%d], bri[%d] ", pqparam_standard.u4SatGain, pqparam_standard.u4Contrast, pqparam_standard.u4Brightness);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, hue0[%d], hue1[%d], hue2[%d], hue3[%d] ", pqparam_standard.u4HueAdj[0], pqparam_standard.u4HueAdj[1], pqparam_standard.u4HueAdj[2], pqparam_standard.u4HueAdj[3]);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, sat0[%d], sat1[%d], sat2[%d], sat3[%d] ", pqparam_standard.u4SatAdj[0], pqparam_standard.u4SatAdj[1], pqparam_standard.u4SatAdj[2], pqparam_standard.u4SatAdj[3]);
    ...
    {
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, gsat[%d], cont[%d], bri[%d] ", pqparam_vivid.u4SatGain, pqparam_vivid.u4Contrast, pqparam_vivid.u4Brightness);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, hue0[%d], hue1[%d], hue2[%d], hue3[%d] ", pqparam_vivid.u4HueAdj[0], pqparam_vivid.u4HueAdj[1], pqparam_vivid.u4HueAdj[2], pqparam_vivid.u4HueAdj[3]);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, sat0[%d], sat1[%d], sat2[%d], sat3[%d] ", pqparam_vivid.u4SatAdj[0], pqparam_vivid.u4SatAdj[1], pqparam_vivid.u4SatAdj[2], pqparam_vivid.u4SatAdj[3]);
    ...
    else if (mode == PQ_PIC_MODE_USER_DEF)
    {
        ...
        i = atoi(value);
        XLOGD("[MiraVision][JNI] property get... tdshp[%d]", i);
        pqparam.u4SHPGain = i;

        property_get(PQ_GSAT_PROPERTY_STR, value, PQ_GSAT_INDEX_DEFAULT);
        i = atoi(value);
        XLOGD("[MiraVision][JNI] property get... gsat[%d]", i);
        pqparam.u4SatGain = i;

        property_get(PQ_CONTRAST_PROPERTY_STR, value, PQ_CONTRAST_INDEX_DEFAULT);
        i = atoi(value);
        XLOGD("[MiraVision][JNI] property get... contrast[%d]", i);
        pqparam.u4Contrast = i;

        property_get(PQ_PIC_BRIGHT_PROPERTY_STR, value, PQ_PIC_BRIGHT_INDEX_DEFAULT);
        i = atoi(value);
        XLOGD("[MiraVision][JNI] property get... pic bright[%d]", i);
        pqparam.u4Brightness = i;

        ioctl(drvID, DISP_IOCTL_SET_PQPARAM, &pqparam);
        ioctl(drvID, DISP_IOCTL_SET_PQ_GAL_PARAM, &pqparam);

        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, shp[%d], gsat[%d], cont[%d], bri[%d] ", pqparam.u4SHPGain, pqparam.u4SatGain, pqparam.u4Contrast, pqparam.u4Brightness);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, hue0[%d], hue1[%d], hue2[%d], hue3[%d] ", pqparam.u4HueAdj[0], pqparam.u4HueAdj[1], pqparam.u4HueAdj[2], pqparam.u4HueAdj[3]);
        XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, sat0[%d], sat1[%d], sat2[%d], sat3[%d] ", pqparam.u4SatAdj[0], pqparam.u4SatAdj[1], pqparam.u4SatAdj[2], pqparam.u4SatAdj[3]);
    }
    else
    {
        XLOGE("[MiraVision][JNI] unknown picture mode!!");
...
static jboolean setPQColorRegion(JNIEnv *env, jobject thiz, int isEnable, int startX, int startY, int endX, int endY)
{
    ...
    XLOGD("[MiraVision][JNI] setPQColorRegion(), en[%d], sX[%d], sY[%d], eX[%d], eY[%d]", isEnable, startX, startY, endX, endY);
...
static jint getContrastIndex(JNIEnv *env, jobject thiz)
{
    ...
    property_get(PQ_CONTRAST_PROPERTY_STR, value, PQ_CONTRAST_INDEX_DEFAULT);
    index = atoi(value);
    XLOGD("[MiraVision][JNI] getContrastIndex(), property get [%d]", index);
...
static void setContrastIndex(JNIEnv *env, jobject thiz, int index)
{
    ...
    XLOGD("[MiraVision][JNI] setContrastIndex...index[%d]", index);
    ...
    //XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, gsat[%d], cont[%d], bri[%d] ", pqparam.u4SatGain, pqparam.u4Contrast, pqparam.u4Brightness);
    //XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, hue0[%d], hue1[%d], hue2[%d], hue3[%d] ", pqparam.u4HueAdj[0], pqparam.u4HueAdj[1], pqparam.u4HueAdj[2], pqparam.u4HueAdj[3]);
    //XLOGD("[MiraVision][JNI] --DISP_IOCTL_SET_PQPARAM, sat0[%d], sat1[%d], sat2[%d], sat3[%d] ", pqparam.u4SatAdj[0], pqparam.u4SatAdj[1], pqparam.u4SatAdj[2], pqparam.u4SatAdj[3]);
...
static jint getSaturationIndex(JNIEnv *env, jobject thiz)
{
    ...
    property_get(PQ_GSAT_PROPERTY_STR, value, PQ_GSAT_INDEX_DEFAULT);
    index = atoi(value);
    XLOGD("[MiraVision][JNI] getSaturationIndex(), property get [%d]", index);
...
static void setSaturationIndex(JNIEnv *env, jobject thiz, int index)
{
    ...
    XLOGD("[MiraVision][JNI] setSaturationIndex...index[%d]", index);
...
static jint getPicBrightnessIndex(JNIEnv *env, jobject thiz)
{
    ...
    property_get(PQ_PIC_BRIGHT_PROPERTY_STR, value, PQ_PIC_BRIGHT_INDEX_DEFAULT);
    index = atoi(value);
    XLOGD("[MiraVision][JNI] getPicBrightnessIndex(), property get [%d]", index);
...
static void setPicBrightnessIndex(JNIEnv *env, jobject thiz, int index)
{
    ...
    XLOGD("[MiraVision][JNI] setPicBrightnessIndex...index[%d]", index);
...
static jint getSharpnessIndex(JNIEnv *env, jobject thiz)
{
    ...
    //property_get(PQ_TDSHP_PROPERTY_STR, value, PQ_TDSHP_INDEX_DEFAULT);
    //index = atoi(value);
    XLOGD("[MiraVision][JNI] getSharpnessIndex(), property get...tdshp[%d]", index);
...
static void setSharpnessIndex(JNIEnv *env, jobject thiz , int index)
{
    ...
    XLOGD("[MiraVision][JNI] setSharpnessIndex...index[%d]", index);
...
static jint getDynamicContrastIndex(JNIEnv *env, jobject thiz)
{
    ...
    index = atoi(value);
    XLOGD("[MiraVision][JNI] getDynamicContrastIndex(), property get [%d]", index);
...
static void setDynamicContrastIndex(JNIEnv *env, jobject thiz, int index)
{
    ...
    XLOGD("[MiraVision][JNI] setDynamicContrastIndex...index[%d]", index);
...
static void nativeGetUserBrightnessRange(JNIEnv* env, jclass clz, jobject range)
{
    Range_set(env, range, 0, 255, 127);
}


static void nativeSetUserBrightness(JNIEnv* env, jclass clz, jint level)
{
#if defined(MTK_AAL_SUPPORT)
    AALClient::getInstance().setUserBrightness(level);
#else
    XLOGE("nativeSetUserBrightness(): MTK_AAL_SUPPORT disabled");
#endif
}
...
static JNINativeMethod g_methods[] = {
  ...
  {"nativeGetUserBrightnessRange", "(L" JNI_CLASS_NAME "$Range;)V", (void*)nativeGetUserBrightnessRange},
  {"nativeSetUserBrightness", "(I)V", (void*)nativeSetUserBrightness},
