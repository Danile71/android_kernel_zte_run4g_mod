...
{  
    ...
    if (!fp)
    { //check phone storage
        XLOGE("sdcard0 SUPPORT_CLEARMOTION not exist");     
        fp = fopen(SUPPORT_CLEARMOTION_PATH1, "rb+");
        if(!fp)
        {
            XLOGE("sdcard1 SUPPORT_CLEARMOTION not exist");  
        }    
    }                
    ...
    if (fp) {
        ...
        {
            XLOGD("value %x %x",value[0],value[1]);
            value[0]=value[0]>>8;
            value[1]=value[1]>>8;            
    ...
    XLOGE("getMjcIndex[%d]=%d",a_eIndex, u4Indics[a_eIndex]); 
...
static jboolean setMjcIndex(MJC_TuningIndex_t a_eIndex , unsigned int a_u4Index)
{
    ...
    XLOGE("setMjcIndex[%d]=%d",a_eIndex, u4Indics[a_eIndex]); 
    return JNI_TRUE;
}
...
//JNI register
////////////////////////////////////////////////////////////////
static const char *classPathName = "com/mediatek/gallery3d/clearMotion/ClearMotionQualityJni";
