...
#if defined(HAVE_COMMAND_effect)
 ...
 *
 ******************************************************************************/
bool CmdImp::isInstantiate = CmdMap::inst().addCommand(HAVE_COMMAND_effect, new CmdImp(HAVE_COMMAND_effect));
};  // NSCmd_test_preview
using namespace NSCmd_effect;
...
onOpen()
{
    ...
    mDevice->ops->set_parameters(mDevice, FACE_EFFECT_SMOOTH_LEVEL, &smoothLevel, 0); 
    mDevice->ops->set_parameters(mDevice, FACE_EFFECT_SMOOTH_LEVEL, &skinColor, 0); 
    mDevice->ops->set_parameters(mDevice, FACE_EFFECT_SMOOTH_LEVEL, &enlargeEyeLevel, 0); 
    mDevice->ops->set_parameters(mDevice, FACE_EFFECT_SMOOTH_LEVEL, &SlimFaceLevel, 0); 

    //
    mDevice->ops->apply(mDevice, pSrcBuffer, pDestBuffer, 0); 
...
bool 
CmdImp::
openDevice(hw_module_t *module, effect_device_t **device) 
{
    //    
    if (0 == module)
    {
    ...
    // 
    bool ret = module->methods->open(module, MMSDK_HARDWARE_EFFECT, 
                                   (hw_device_t**)device); 
...
bool 
CmdImp::
closeDevice(effect_device_t  **device)
{
    //
    if (0 == *device)
    {
    ...
    if (rc != OK)
    {
        MY_LOGE("Could not close effect device:%d", rc); 
        return false; 
    }
