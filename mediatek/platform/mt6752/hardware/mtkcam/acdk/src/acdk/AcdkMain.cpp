...
#include "mtkfb.h"  //TO Do : Temp for MT6595DEV
}
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ...
        ACDK_LOGD("rrzoVA[%d] = 0x%x, rrzoPA[%d]  = 0x%x",i,mPrvIMemInfoRRZO[i].virtAddr,i,mPrvIMemInfoRRZO[i].phyAddr);
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ...
        else
        {
            ...
            memset((MVOID *)mPrvIMemInfoRRZO[i].virtAddr, 0, mPrvIMemInfoRRZO[i].size);
        }
                ...
                MINT32  const isSaveImg)
{
    ...
    memcpy((MUINT8 *)mCapIMemInfo.virtAddr,(MUINT8 *)PureRaw_BufAddr,mCapIMemInfo.size);
...
*
********************************************************************************/
MINT32 AcdkMain::startPreview_CameraDev(Func_CB prvCb)
{
    ACDK_LOGD("+");
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
         ACDK_LOGD("dispVA[%d] = 0x%x, dispPA[%d] = 0x%x, dispmemID[%d] = 0x%x",i,mDispIMemInfo[i].virtAddr,i,mDispIMemInfo[i].phyAddr,i,mDispIMemInfo[i].memID);
    }
                ...
                MINT32  const isSaveImg)
{
    ...
    if(capCb != NULL)
    {
        ...
        {
            acdkCapInfo.imgBufInfo.bufAddr   = (MUINT8 *)Capture_BufAddr;
            acdkCapInfo.imgBufInfo.imgWidth  = mCapWidth;
            acdkCapInfo.imgBufInfo.imgHeight = mCapHeight;
...
MINT32 AcdkMain::quickViewImg(MUINT32 qvFormat)
{
    ...
    memcpy((MVOID *)mSurfaceIMemInfo[mSurfaceIndex].virtAddr, (MVOID *)mQvIMemInfo.virtAddr, mQvIMemInfo.size);
...
MBOOL AcdkMain::camShotDataCB(MVOID* user, CamShotDataInfo const msg)
{
    ...
    if(NULL != _This)
    {
        if(ECamShot_DATA_MSG_RAW == msg.msgType)
        {
            ...
            acdkCbParam.addr1 = (MUINT32)msg.pBuffer->getBufVA(0);    // pointer to RAW data buffer
            acdkCbParam.addr2 = 0;
        ...
        else if(ECamShot_DATA_MSG_JPEG == msg.msgType)
        {
            ...
            acdkCbParam.addr1 = (MUINT32)msg.pBuffer->getBufVA(0); // pointer to JPEG data buffer
            acdkCbParam.addr2 = (MUINT32)pThumbImg->getBufVA(0);        // pointer to thumbnail data buffer
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(0x%x),PA(0x%x),Size(0x%x)",mSurfaceIndex,mSurfaceIMemInfo[mSurfaceIndex].virtAddr,mSurfaceIMemInfo[mSurfaceIndex].phyAddr,mSurfaceIMemInfo[mSurfaceIndex].size);
    ACDK_LOGD_DYN(g_acdkMainDebug,"mOrientation(%u)",mOrientation);
    ACDK_LOGD_DYN(g_acdkMainDebug,"addr1(0x%x)addr2(0x%x)imgsize(0x%x)thumbsize(0x%x)",pCBParam->addr1,pCBParam->addr2,pCBParam->imgSize,pCBParam->thubSize);
    ...
    else
    {
        ...
        memcpy((MVOID *)mDispIMemInfo[2].virtAddr, pVirBufin, displaySize);
        ...
        ACDK_LOGD("procQvMemInfo : vir(0x%x),phy(0x%x)",ISrcMemInfo.virtAddr,ISrcMemInfo.phyAddr);
        ...
        ACDK_LOGD("Dst VA(0x%x), Src VA(0x%x) size(0x%x)", (MVOID *)mSurfaceIMemInfo[mSurfaceIndex].virtAddr, pVirBufin, displaySize);
    }
...
MINT32 AcdkMain::handleCapCB(MVOID *param)
{
    ...
    MUINT32 captureAddr = pCBParam->addr1;
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"JPEG src VA(0x%x)",(MUINT32)captureAddr);

    // Copy jpeg to output buffer
    memcpy((MUINT8 *)mCapIMemInfo.virtAddr,(MUINT8 *)captureAddr,captureSize);
    Capture_BufAddr = (MUINT32)mCapIMemInfo.virtAddr;
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"captureAddr(0x%x),captureSize(0x%x)",captureAddr,captureSize);
...
)
{
    ...
    else if(a_u4Ioctl == ACDK_CMD_PREVIEW_START)
    {
        ...
        {
            mIsISPEng = 1;
            err = startPreview_CameraDev(pCamPreview->fpPrvCB);
        }
