...
MINT32 AcdkMhalPure::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    if(0)//m_imgo.u4ImgWidth > 2304)
    {
        ...
        rrz.m_num = 0;
        if (!m_pICamIOPipe->sendCommand(EPIPECmd_SET_RRZ,(MINT32)&rrz,0,0) ){
            return -1;
        }
...
//#define PASS2_FULLG
    ...
    if(is_yuv_sensor == 0)
    {
        m_imgi.u4BufVA[0]     = (MUINT32)mAcdkMhalPrvParam.rrzoImemBuf[0].virtAddr; //  Vir Address of pool
        m_imgi.u4BufPA[0]     = (MUINT32)mAcdkMhalPrvParam.rrzoImemBuf[0].phyAddr; //  Phy Address of pool
    ...
    {
        m_imgi.u4BufVA[0]     = (MUINT32)mAcdkMhalPrvParam.imgImemBuf[0].virtAddr; //  Vir Address of pool
        m_imgi.u4BufPA[0]     = (MUINT32)mAcdkMhalPrvParam.imgImemBuf[0].phyAddr; //  Phy Address of pool
    ...
    m_wroto.u4BufVA[0]     = (MUINT32)mAcdkMhalPrvParam.dispImemBuf[0].virtAddr;
    m_wroto.u4BufPA[0]     = (MUINT32)mAcdkMhalPrvParam.dispImemBuf[0].phyAddr;
...
MINT32 AcdkMhalPure::acdkMhalPreviewProc()
{
    ...
    MUINT32 rrzo_virAddr[OVERLAY_BUFFER_CNT] = {0}, rrzo_phyAddr[OVERLAY_BUFFER_CNT] = {0};
    MUINT32 imgi_memID, imgi_virAddr, imgi_phyAddr, dispo_memID, dispo_virAddr, dispo_phyAddr;
    MUINT32 imgo_virA, imgo_phyA;
    ...
    while((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState()))
    {
        ...
        else
        {
            ...
            ACDK_LOGD("imgo_virAddr(0x%x),imgo_phyAddr(0x%x) size%d",imgo_virA,imgo_phyA,mAcdkMhalPrvParam.imgImemBuf[0].size);
        }
#ifdef ALL_SENSOR_USE_TG1
        mPortID.index = EPortIndex_RRZO;
#else
       ...
       else
       {
           ...
           for(MUINT32 i = 0; i < rrzo_num; ++i)
           {
               ...
               ACDK_LOGD_DYN(g_acdkMhalPureDebug,"rrzo_virAddr[%d](0x%x),rrzo_phyAddr[%d](0x%x)",i,imgo_virAddr[i],i,imgo_phyAddr[i]);
           }
        ...
        m_p3AHal->sendCommand(ECmd_Update, reinterpret_cast<MINT32>(&_3A_profile));
#endif
        m_i4FrameCount++;
        ...
        if(is_yuv_sensor == 0)
        {
            m_imgi.u4BufVA[0]     = (MUINT32)rrzo_virAddr[0]; //  Vir Address of pool
            m_imgi.u4BufPA[0]     = (MUINT32)rrzo_phyAddr[0]; //  Phy Address of pool
            m_imgi.memID[0]       = (MINT32)rrzo_memID[0];
            m_imgi.u4BufSize[0]   = (MUINT32)mAcdkMhalPrvParam.rrzoImemBuf[0].size;
        }
        else
        {
            m_imgi.u4BufVA[0]     = (MUINT32)imgo_virAddr[0]; //  Vir Address of pool
            m_imgi.u4BufPA[0]     = (MUINT32)imgo_phyAddr[0]; //  Phy Address of pool
        ...
        g_vPostProcInPorts.at(0) = &m_imgi;
        g_vPostProcOutPorts.at(0) = &m_wroto;
        if(MTRUE != m_pPostProcPipe->configPipe(g_vPostProcInPorts, g_vPostProcOutPorts,0,0,&g_CookieVa))
        {
            ACDK_LOGE("m_pPostProcPipe->configPipe() fail");
        }
        ...
        else
        {
            ...
            {
               acdkMhalCBHandle(ACDK_CB_PREVIEW, dispo_virAddr, imgo_virA);
            }
            else
            {
               delayframecnt++;
