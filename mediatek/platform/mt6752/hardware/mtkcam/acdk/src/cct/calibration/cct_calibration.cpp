...
//            sprintf(filename, "/data/Raw_%dx%d%d.raw", m_u2RawImgWidth, m_u2RawImgHeight,i4Count++);
                                        ...
                                        ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    */
    ...
    //CCTIF_FeatureCtrl(ACDK_CCT_OP_AE_CAPTURE_MODE, (MUINT8 *)&a_i4SensorMode, sizeof(a_i4SensorMode), NULL, 0, &u4RetLen);
    ...
    for (MINT32 i4Exp = a_i4ExpStart; i4Exp <= a_i4ExpEnd; i4Exp += a_i4ExpInterval)
    {
        ...
        *   Take Picture
        */
        if((mrRet = takePicture(a_i4SensorMode, PURE_RAW8_TYPE)) != S_CCT_CALIBRATION_OK)
        {
                                     ...
                                     ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    *   Take Picture
    */
    if((mrRet = takePicture(a_i4SensorMode, PROCESSED_RAW8_TYPE)) != S_CCT_CALIBRATION_OK)
    {
        ACDK_LOGE("[takePicture] Get RAW Fail \n");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
...
Exit:
    ...
    if(backup_bypass != -1)
        NSIspTuning::IspTuningMgr::getInstance().setDynamicBypass(0, (MBOOL)backup_bypass);
