...
IMP_CCT_CTRL( ACDK_CCT_OP_QUERY_ISP_ID )
{
    ...
    *reinterpret_cast<MUINT32 *>(puParaOut) = 0x65958A00;;
    *pu4RealParaOutLen = sizeof(MUINT32);
    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_OP_QUERY_ISP_ID ) done, %d\n", *reinterpret_cast<MUINT32 *>(puParaOut));
    return CCTIF_NO_ERROR;
}
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AE_GET_GAMMA_TABLE )
{
    ...
    for(int i=0;i<GAMMA_STEP_NO; i++) {
        pGammaAccessOut->gamma.r_tbl[i] = pGGM->lut_rb.lut[i].GGM_R;//ggm.rb_gmt.lut[i].R_GAMMA;
        pGammaAccessOut->gamma.g_tbl[i] = pGGM->lut_g.lut[i].GGM_G;//ggm.g_gmt.lut[i].G_GAMMA;
        pGammaAccessOut->gamma.b_tbl[i] = pGGM->lut_rb.lut[i].GGM_B;//ggm.rb_gmt.lut[i].B_GAMMA;
...
*/
        ...
        {
            ggm.lut_rb.lut[i].GGM_R = m_rISPRegs.GGM[setIndex].lut_rb.lut[i].GGM_R = pGammaAccess->gamma.r_tbl[i];
            ggm.lut_g.lut[i].GGM_G = m_rISPRegs.GGM[setIndex].lut_g.lut[i].GGM_G = pGammaAccess->gamma.g_tbl[i];
            ggm.lut_rb.lut[i].GGM_B = m_rISPRegs.GGM[setIndex].lut_rb.lut[i].GGM_B = pGammaAccess->gamma.b_tbl[i];
        }
...
setIspOnOff_CFA(MBOOL const fgOn)
{
    ...
    ISP_MGR_CFA::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
...
setIspOnOff_ANR(MBOOL const fgOn)
{
    ...
    ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTANREnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
...
{
    ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTCCREnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
    //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuning::EIspProfile_NormalCapture);
    MY_LOG("[%s] fgOn = %d", __FUNCTION__, fgOn);
...
setIspOnOff_EE(MBOOL const fgOn)
{
    ...
    ISP_MGR_SEEE_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_SEEE_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
...
{
    return  ISP_MGR_CFA::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
{
    return (ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTANREnable());
}
...
{
    return ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTCCREnable();
}
...
{
    return ISP_MGR_SEEE_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
IMP_CCT_CTRL( ACDK_CCT_OP_ISP_SET_PCA_TABLE )
{
    ...
    case 0:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA */) ? 
            &m_rISPPca.PCA_LUTS.lut_lo[u4Offset] : &m_rISPPca.PCA_LUTS.lut_lo2[u4Offset];
        break;
    case 1:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA */) ? 
            &m_rISPPca.PCA_LUTS.lut_md[u4Offset] : &m_rISPPca.PCA_LUTS.lut_md2[u4Offset];
        break;
    case 2:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA */) ? 
            &m_rISPPca.PCA_LUTS.lut_hi[u4Offset] : &m_rISPPca.PCA_LUTS.lut_hi2[u4Offset];
        break;
    default:
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_ISP_SET_SHADING_PARA )
{
    ...
    rLscCfg.ctl2.bits.LSC_SDBLK_XNUM        = pShadingPara->pShadingComp->SHADINGBLK_XNUM - 1;
    rLscCfg.ctl3.bits.LSC_SDBLK_YNUM        = pShadingPara->pShadingComp->SHADINGBLK_YNUM - 1;
    rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH       = pShadingPara->pShadingComp->SHADINGBLK_WIDTH;
    rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT      = pShadingPara->pShadingComp->SHADINGBLK_HEIGHT;
    rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH    = pShadingPara->pShadingComp->SD_LWIDTH;
    rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT   = pShadingPara->pShadingComp->SD_LHEIGHT;
    rLscCfg.ratio.bits.LSC_RA0              = pShadingPara->pShadingComp->SDBLK_RATIO00;
    rLscCfg.ratio.bits.LSC_RA1              = pShadingPara->pShadingComp->SDBLK_RATIO01;
    rLscCfg.ratio.bits.LSC_RA2              = pShadingPara->pShadingComp->SDBLK_RATIO10;
    rLscCfg.ratio.bits.LSC_RA3              = pShadingPara->pShadingComp->SDBLK_RATIO11;
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_ISP_GET_SHADING_PARA )
{
    ...
    pShadingPara->pShadingComp->SHADINGBLK_XNUM     = prLscCfg->ctl2.bits.LSC_SDBLK_XNUM+1;
    pShadingPara->pShadingComp->SHADINGBLK_YNUM     = prLscCfg->ctl3.bits.LSC_SDBLK_YNUM+1;
    pShadingPara->pShadingComp->SHADINGBLK_WIDTH    = prLscCfg->ctl2.bits.LSC_SDBLK_WIDTH;
    pShadingPara->pShadingComp->SHADINGBLK_HEIGHT   = prLscCfg->ctl3.bits.LSC_SDBLK_HEIGHT;
    pShadingPara->pShadingComp->SHADING_RADDR       = 0;
    pShadingPara->pShadingComp->SD_LWIDTH           = prLscCfg->lblock.bits.LSC_SDBLK_lWIDTH;
    pShadingPara->pShadingComp->SD_LHEIGHT          = prLscCfg->lblock.bits.LSC_SDBLK_lHEIGHT;
    pShadingPara->pShadingComp->SDBLK_RATIO00       = prLscCfg->ratio.bits.LSC_RA0;
    pShadingPara->pShadingComp->SDBLK_RATIO01       = prLscCfg->ratio.bits.LSC_RA1;
    pShadingPara->pShadingComp->SDBLK_RATIO10       = prLscCfg->ratio.bits.LSC_RA2;
    pShadingPara->pShadingComp->SDBLK_RATIO11       = prLscCfg->ratio.bits.LSC_RA3;
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_ISP_SET_SHADING_TABLE_POLYCOEF )
{
    ...
    MY_LOG("[%s +] SET_SHADING_TABLE: SensorMode(%d),CT(%d),Src(0x%08x),Size(%d)", __FUNCTION__,
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_ISP_GET_SHADING_TABLE_POLYCOEF )
{
    ...
    MY_LOG("[%s +] GET_SHADING_TABLE: SensorMode(%d),CT(%d),Src(0x%08x),Size(%d)", __FUNCTION__,
