...
threadLoopUpdate()
{
    ...
    if( dstCount == 0 )
    {
        ...
        handleReturnBuffer(postBuf_0.data, (MUINT32)postBuf_0.buf);
        handleReturnBuffer(postBuf_1.data, (MUINT32)postBuf_1.buf);
        return MTRUE;
    }
    // muSensorPosition:
    //    0 is main-main2 (main in L)
    ...
    if ( pDstBuf_SBS != NULL )
    {
        ...
        handlePostBuffer( DUALIT_DST_2, (MUINT32)pDstBuf_SBS);
    }
    handleReturnBuffer(postBuf_0.data, (MUINT32)postBuf_0.buf);
    handleReturnBuffer(postBuf_1.data, (MUINT32)postBuf_1.buf);
    //
...
)
{
    ...
    //
    NG_TRACE_FMT_BEGIN("execP2:%d", muDeqFrameCnt);
    ...
    if ( pDstBuf != NULL )
    {
        handlePostBuffer( dstDataType, (MUINT32)pDstBuf );
    }
    //
