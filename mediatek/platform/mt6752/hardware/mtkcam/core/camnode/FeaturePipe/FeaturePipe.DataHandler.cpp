...
allocBuffer_GB(AllocInfo const & info, sp<GraphicBuffer>* pGraphicBuffer)
{
    ...
    int err = ::gralloc_extra_getIonFd(handle, &idx, &num);
    if  ( num > 0 ) // current num should be 1
    {
        fdIon = handle->data[idx];
        MY_LOGD("getIonFd(): fdIon=%d, idx/num=%d/%d, status[%s(%d)]", fdIon, idx, num, ::strerror(-err), -err);
    }
    else
    {
        fdIon = -1;
        MY_LOGE("getIonFd(): fdIon=-1, idx/num=%d/%d, status[%s(%d)]", idx, num, ::strerror(-err), -err);
        return MFALSE;
    }
