...
static MINT32
convertRGBA8888toYV12(MVOID * srcBufAddr, int srcBufWidth, int srcBufHeight, IImageBuffer* dstIImageBuffer)
{
    bool ret = true;
    FUNCTION_LOG_START;
    DpBlitStream dpBlit;
    ...
    int plane_num = 1;
    src_addr_list[0] = (unsigned int)srcBufAddr;
...
FeaturePipe::initFeatureGraph()
{
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        ...
        reinterpret_cast<FeaturePipeVfbNode*>(mpVfbSwNode)->set(RECORDING_HINT, (void*) mRecordingHint);
#endif
    }
...
{
    //FUNCTION_LOG_START;
    NG_TRACE_FMT_BEGIN("FPipeEnq:%d", rParams.mFrameNo);
    ...
    ret = reinterpret_cast<FeaturePipeP2aNode*>(mpP2aNode)->enqueWrapper(0, (MUINT32) &rParams, getMagicNo(rParams));
...
{
    //FUNCTION_LOG_START;
    NG_TRACE_FMT_BEGIN("FPipeCb:%d", rParams.mFrameNo);
...
MBOOL
FeaturePipe::set(FeaturePipeSetType setType, const void* setData)
{
...
MBOOL
FeaturePipeP2aNode::
enqueWrapper(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
#if 0
...
onPostBuffer(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
       ...
       !FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask) )    //Vent@20140228: Add for 3DNR.
    {
        ...
        NG_TRACE_FMT_BEGIN("P2ANStrm:%d", getMagicNo((*pQParams)));
...
MBOOL
FeaturePipeP2aNode::
set(FeaturePipeSetType setType, const void* setData)
{
    switch (setType)
    {
        case MAX_INPUT_IMAGE_SIZE:
...
{
    //FUNCTION_LOG_START;
    NG_TRACE_FMT_BEGIN("P2ACb:%d", getMagicNo(rParams));
...
handleP2Done(QParams& rParams)
{
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        ...
        handlePostBuffer(P2A_TO_SWITCHER, (MUINT32) &fullImgReq, getMagicNo(rParams));
        LOG_DBG("Posting P2A->VFB node DS image of frame %d", getMagicNo(rParams));
        handlePostBuffer(P2A_TO_VFBSW_DSIMG, (MUINT32) &dsImgReq, getMagicNo(rParams));
        LOG_DBG("Posting P2A->FD node DS image of frame %d", getMagicNo(rParams));
        handlePostBuffer(P2A_TO_FD_DSIMG, (MUINT32) &dsImgReq, getMagicNo(rParams));
    }
    
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        LOG_DBG("Posting P2A->EIS node full image buffer of frame %d", getMagicNo(rParams));
        handlePostBuffer(P2A_TO_EISSW, (MUINT32) &fullImgReq, getMagicNo(rParams));
        if (!FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
        {
            LOG_DBG("Posting P2A->GPU node full image buffer of frame %d", getMagicNo(rParams));
            handlePostBuffer(P2A_TO_GPU, (MUINT32) &fullImgReq, getMagicNo(rParams));
        }
    }
...
threadLoopUpdate()
{
    ...
    NG_TRACE_FMT_BEGIN("P2AThread:%d", getMagicNo(InQParams));
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask))
    {
        ...
        MINT32 i4Nr3dGain = 0;
        mpIFeatureStream->sendCommand(EPIPECmd_GET_NR3D_GAIN, getMagicNo(InQParams), (MINT32)&i4Nr3dGain);
        if (i4Nr3dGain)
        {
...
static const NSCam::NSIoPipe::PortID WDMAO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_WDMAO, 1); 
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask) || FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) || FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        NG_TRACE_FMT_BEGIN("P2AFStrm:%d", getMagicNo(InQParams));
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        handlePostBuffer(P2A_TO_P2B, (MUINT32) &InQParams, getMagicNo(InQParams));
    }
    else if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        handlePostBuffer(P2A_TO_MDP_QPARAMS, (MUINT32) &InQParams, getMagicNo(InQParams));
    }
...
threadLoopUpdate()
{
    ...
    if (!mqJobFromGpu.empty())
    {
        ...
        handlePostBuffer(SWITCHER_TO_P2B, (MUINT32) &job.imgReq, job.mFrameNo);
            ...
            (mGpuProcCount == 0))
        {
            while(!mqJobFromP2a.empty())
            {
                    ...
                    FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
                {
                    ...
                    handlePostBuffer(SWITCHER_TO_GPU, (MUINT32) &job.imgReq, job.mFrameNo);
                    mGpuProcCount++;
                }
                else
                {
                ...
                handlePostBuffer(SWITCHER_TO_VFBSW, (MUINT32) &tagImgReq, job.mFrameNo);
            }
        }
    }
...
//    warp_info.Height = FULL_IMAGE_MAX_HEIGHT;
    ...
    //warp_info.demo = 0;
    LOG_DBG("WarpInit features=0x%X, src buffer=0x%X, dst buffer=0x%x, width=%d, height=%d", warp_info.Features, (MUINT32) warp_info.SrcGraphicBuffer, (MUINT32) warp_info.DstGraphicBuffer, warp_info.Width, warp_info.Height);
...
threadLoopUpdate()
{
    ...
    NG_TRACE_FMT_BEGIN("GPUThread:%d", frameNo);
    ...
    NG_TRACE_FMT_BEGIN("GPUMemcpy:%d", frameNo);
    ...
    warp_info.WarpMapSize[0][0] = (MUINT32) const_cast<IImageBuffer*>(jobWarpMap.imgReq.mBuffer)->getImgSize().w / 4;
    warp_info.WarpMapSize[0][1] = (MUINT32) const_cast<IImageBuffer*>(jobWarpMap.imgReq.mBuffer)->getImgSize().h / 2;
    warp_info.WarpMapAddr[0][0] = (MUINT32) const_cast<IImageBuffer*>(jobWarpMap.imgReq.mBuffer)->getBufVA(0);
    warp_info.WarpMapAddr[0][1] = warp_info.WarpMapAddr[0][0] + const_cast<IImageBuffer*>(jobWarpMap.imgReq.mBuffer)->getBufSizeInBytes(0) / 2;
    ...
    NG_TRACE_FMT_BEGIN("GPUWarp:%d", frameNo);
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        ...
        handleReturnBuffer(VFBSW_TO_GPU, (MUINT32) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
    }
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ImgRequest fullImgReq;       
        ...
        if(EnableOption[0] == '2' || EnableOption[0] == '3') 
        {
            ...
            //::sprintf(szFileName, "/sdcard/gfxo_%dx%d_%04d.rgb", FULL_IMAGE_MAX_WIDTH, FULL_IMAGE_MAX_HEIGHT, frameNo);
            //saveBufToFile(szFileName, (MUINT8*) gfxRgbaBuf, FULL_IMAGE_MAX_WIDTH * FULL_IMAGE_MAX_HEIGHT * 4);
            convertRGBA8888toYV12(gfxRgbaBuf, mOutputImageWidth, mOutputImageHeight, const_cast<IImageBuffer*>(fullImgReq.mBuffer));
        ...
        handleReturnBuffer(EISSW_TO_GPU, (MUINT32) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
    }
    ...
    handleReturnBuffer(jobFullImg.dataType, (MUINT32) &jobFullImg.imgReq, jobFullImg.mFrameNo);

    if(FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) || FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        /* Post buffer to next node */
        LOG_DBG("Posting GPU->MDP node GB of frame %d", frameNo);
        handlePostBuffer(GPU_TO_MDP, (MUINT32) &fullImgReq, frameNo);
    }
...
threadLoopUpdate()
{
    ...
    NG_TRACE_FMT_BEGIN("MDPThread:%d", frameNo);
    ...
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        if(EnableOption[0] == '1' || EnableOption[0] == '3') 
        {
            ...
            char szFileName[100];
            convertRGBA8888toYV12((MVOID*) pImageBuffer->getBufVA(0), pImageBuffer->getImgSize().w, pImageBuffer->getImgSize().h, const_cast<IImageBuffer*>(fullImgReq.mBuffer));
    ...
    handleReturnBuffer(GPU_TO_MDP, (MUINT32) &jobFullImg.imgReq, jobFullImg.mFrameNo);

    if(FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        /* Post buffer to next node */
        LOG_DBG("Posting MDP->SWITCHER node full image of frame %d", frameNo);
        handlePostBuffer(MDP_TO_SWITCHER, (MUINT32) &fullImgReq, frameNo);
    }
...
p2bCbFunc(QParams& rParams)
{
    ...
    NG_TRACE_FMT_BEGIN("P2BCb:%d", getMagicNo(rParams));
...
handleP2Done(QParams& rParams)
{
    for(MUINT32 i = 0; i < rParams.mvIn.size(); i++)
    {
        ...
        handleReturnBuffer(dType, (MUINT32) &imgReq, getMagicNo(rParams));
    }
...
static const NSCam::NSIoPipe::PortID LCEI( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_LCEI, 0);
    ...
    NG_TRACE_FMT_BEGIN("P2BThread:%d", getMagicNo(OutQParams));
    ...
    NG_TRACE_FMT_BEGIN("P2BFStrm:%d", getMagicNo(mQParams));
    ...
    handleReturnBuffer(VFBSW_TO_P2B_PCA, (MUINT32) &jobPCA.imgReq, jobPCA.mFrameNo);
