...
threadLoopUpdate()
{
    ...
    NG_TRACE_FMT_BEGIN("FDThread:%d", frameNo);
    ...
    handleReturnBuffer(P2A_TO_FD_DSIMG, (MUINT32) &jobDsImg.imgReq, jobDsImg.mFrameNo);

    /* Post buffer to next node */
    LOG_DBG("Posting FD->P2B node of frame %d", frameNo);
    handlePostBuffer(FD_TO_VFBSW, (MUINT32) &faceMetaDataReq, frameNo);
...
FeaturePipeVfbNode::onInit()
{
    ...
    {
        LOG_ERR("mHalVFBInit init failed! (%d)", mret);
        goto _Exit;
    }
...
threadLoopUpdate()
{
    ...
    NG_TRACE_FMT_BEGIN("VFBThread:%d", frameNo);
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        VfbProcInfo.PreWarpMapY = (MINT32*) ((MUINT32) VfbProcInfo.PreWarpMapX + (const_cast<IImageBuffer*>(jobWarpMap.imgReq.mBuffer)->getBufSizeInBytes(0) / 2));
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        handleReturnBuffer(EISSW_TO_VFBSW, (MUINT32) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
    }
    LOG_DBG("Returning P2A node DS image buffer of frame %d", jobDsImg.mFrameNo);
    handleReturnBuffer(P2A_TO_VFBSW_DSIMG, (MUINT32) &jobDsImg.imgReq, jobDsImg.mFrameNo);
    LOG_DBG("Returning FD node face metadata buffer of frame %d", jobDsImg.mFrameNo);
    handleReturnBuffer(FD_TO_VFBSW, (MUINT32) &jobFaceMetadata.imgReq, jobDsImg.mFrameNo);
    ...
    handlePostBuffer(VFBSW_TO_P2B_ALPCL, (MUINT32) &alphaCLImgReq, frameNo);
    handlePostBuffer(VFBSW_TO_P2B_ALPNR, (MUINT32) &alphaNRImgReq, frameNo);
    handlePostBuffer(VFBSW_TO_P2B_PCA, (MUINT32) &pcaImgReq, frameNo);
    handlePostBuffer(VFBSW_TO_P2B_DSIMG, (MUINT32) &dsImgReq, frameNo);
        ...
        FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        handlePostBuffer(VFBSW_TO_GPU, (MUINT32) &warpImgReq, frameNo);
    }
    else
    {
...
MBOOL
FeaturePipeVfbNode::set(FeaturePipeSetType setType, const void* setData)
{
    switch (setType)
    {
        case RECORDING_HINT:
...
FeaturePipeVfbNode::setPCA(IImageBuffer* pImageBuffer)
{
    ...
    halVFBTuning::getInstance().mHalVFBTuningSetPCA(pImageBuffer->getBufVA(0));
}
MVOID
FeaturePipeVfbNode::setLCE(IImageBuffer* pImageBuffer)
{
    LOG_DBG("Set vFB LCE@0x%x", pImageBuffer->getBufPA(0));
    halVFBTuning::getInstance().mHalVFBTuningSetLCE(pImageBuffer->getBufPA(0));
}
...
FeaturePipeFdNode::doGestureShot(IImageBuffer* pImageBuffer)
{
    ...
    mpGDHalObj->halGSInit(SrcW, SrcH, (MUINT32) mpFDWorkingBuffer, FD_WORKING_BUF_SIZE);

    mpGDHalObj->halGSDo((MUINT32)mFDYBuffer, mRotation);
