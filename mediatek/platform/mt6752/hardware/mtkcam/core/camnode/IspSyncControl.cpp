...
updatePass2ZoomRatio(MINT32 targetZoomRatio)
{
    ...
    if(muLastPass2ZoomRatio != muCurPass2ZoomRatio)
    {
        ...
        muLastPass2ZoomRatio = muCurPass2ZoomRatio;
    }
    #endif
    return MTRUE;
}
    ...
    MBOOL           inIsRrzo)
{
    ...
    //
    MY_LOGD("%d: TS(%d),# 0x%X,VA(0x%08X),IsR(%d),IsNew(%d)",
            getSensorIdx(),
            ...
            isNewMagicNum);
    //
    MY_LOGV("metadata: time %lld, C(%d,%d,%dx%d), priv 0x%x/%d",
            inMetadata.mTimeStamp,
            inMetadata.mCropRect.p.x,
            inMetadata.mCropRect.p.y,
            inMetadata.mCropRect.s.w,
            inMetadata.mCropRect.s.h,
    ...
    MCropRect&      outInCrop)
{
    ...
    {
        MY_LOGE("Can't find ImgBuf(0x%08X)",(MUINT32)inpImgBuf);
    }
    else
    {
    ...
    if(eisEnable)
    {
        MY_LOGD("%d: ImgBuf(0x%08X),# 0x%X,PD(0x%08X/%d),ZR(%d,%d/%d),S(%dx%d),C(%d/%10d,%d/%10d,%dx%d),EIS:X(%d/%10d),Y(%d/%10d)",
                getSensorIdx(),
                (MUINT32)inpImgBuf,
                outMagicNum,
                (MUINT32)outpPrivateData,
                outPrivateDataSize,
                zoomIdx,
                currentZoomRatio,
                targetZoomRatio,
    ...
    {
        MY_LOGD("%d: ImgBuf(0x%08X),# 0x%X,PD(0x%08X/%d),ZR(%d,%d/%d),S(%dx%d),C(%d,%d,%dx%d)",
                getSensorIdx(),
                (MUINT32)inpImgBuf,
                outMagicNum,
                (MUINT32)outpPrivateData,
...
dropPass2Info(IImageBuffer* pImgBuf)
{
    ...
    for(iterMetadata = mlPass1Metadata.begin(); iterMetadata != mlPass1Metadata.end(); iterMetadata++)
    {
        ...
        {
            MY_LOGD("Erase # 0x%X,ImgBuf(0x%08X),IsR(%d)",
                    (*iterMetadata).magicNum,
                    (MUINT32)((*iterMetadata).pImgBuf),
                    (*iterMetadata).isRrzo);
    ...
    if(!isFind)
    {
        MY_LOGD("Can't find ImgBuf(0x%08X)",(MUINT32)pImgBuf);
    }
    //
    return MTRUE;
}
    ...
    MUINT32&        outPrivateDataSize)
{
    ...
    //
    MY_LOGE("cannot find infos for buf(0x%08X)",(MUINT32)inpImgBuf);
    return MFALSE;
}
    ...
    EIS_CROP_INFO   eisCrop)
{
    ...
    */
    ...
    if(isFind)
    {
        //MY_LOGD("Find VA(0x%08X)",(MUINT32)pImageBuffer);
        return MTRUE;
    }
    else
    {
        MY_LOGE("Can't find VA(0x%08X)",(MUINT32)pImageBuffer);
        return MFALSE;
    }
}
    ...
    CbZoomFunc  pFunc)
{
    Mutex::Autolock _l(mLock);
    FUNC_NAME;
    MY_LOGD("Obj(0x%08X),Func(0x%08X)",
            (MUINT32)pObj,
            (MUINT32)pFunc);
    mpCbZoomObj = pObj;
    mpCbZoomFunc = pFunc;
    ...
    MUINT32&    cropH)
{
    ...
    //MY_LOGD("ZR(%d),Crop(%d,%d,%dx%d)",zoomRatio,cropX,cropY,cropW,cropH);
    return ret;
}
