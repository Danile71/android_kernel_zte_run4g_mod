...
class StereoCtrlNodeImpl : public StereoCtrlNode
{
    ...
    public: // operations
        ...
        MBOOL           pushBuf(MUINT32 const data, MUINT32 const buf, MUINT32 const ext);
...
#define RET_BUFFER( postbuf, iter )                                             \
    ...
    {                                                                           \
        ...
        handleReturnBuffer((*iter).data, (MUINT32)((*iter).buf));               \
    }
...
MBOOL
StereoCtrlNodeImpl::
pushBuf(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
    Mutex::Autolock lock(mLock);
...
lbExit:
    //
    handlePostBuffer( STEREO_CTRL_DST_M , (MUINT32)postBufImg.buf, (MUINT32)&sDataOut.algo_main1 );
//    MY_LOGD("retBuffer: data(%d/%d)  buf(0x%x/0x%x)", postBufFeo.data, postBufFeo_Main2.data, postBufFeo.buf, postBufFeo_Main2.buf);
    handleReturnBuffer(postBufFeo.data, (MUINT32)postBufFeo.buf);
    handleReturnBuffer(postBufFeo_Main2.data, (MUINT32)postBufFeo_Main2.buf);
    //
    if ( getStereoType() == STEREO_CTRL_CAPTURE )
    {
        handlePostBuffer( STEREO_CTRL_MAIN_DST , (MUINT32)postBufMain.buf, (MUINT32)&sDataOut.main_crop );
        handlePostBuffer( STEREO_CTRL_DST_S , (MUINT32)postBufImg_Main2.buf, (MUINT32)&sDataOut.algo_main2 );
//        MY_LOGD("retBuffer: data(%d/%d)  buf(0x%x/0x%x)", postBufRgb.data, postBufRgb_Main2.data, postBufRgb.buf, postBufRgb_Main2.buf);
        handleReturnBuffer(postBufRgb.data, (MUINT32)postBufRgb.buf);
        handleReturnBuffer(postBufRgb_Main2.data, (MUINT32)postBufRgb_Main2.buf);
    }
    //
//    FUNC_END;
    return ret;
