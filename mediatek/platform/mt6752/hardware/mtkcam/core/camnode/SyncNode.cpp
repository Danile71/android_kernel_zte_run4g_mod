...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    if( isReadyToPostBuf() )
    {
        ...
        handlePostBuffer(SYNC_DST_0, (MUINT32)postBuf_0.buf);
        handlePostBuffer(SYNC_DST_1, (MUINT32)postBuf_1.buf);
    }
    else
    {
        ...
        while( mlPostBuf_0.size() > 1 ) {
            ...
            mlPostBuf_0.pop_front();
            handleReturnBuffer(retBufData.data, (MUINT32)retBufData.buf);
        }
