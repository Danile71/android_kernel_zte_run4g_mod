...
class Pass1NodeImpl : public Pass1Node
{
    private:
        ...
        class ringbuffer
        {
            ...
            public:
                ...
                MVOID         setBufHandler(ICamBufHandler* pBufHdl) { mpBufHdl = pBufHdl; }
    ...
    private:
        ...
        MUINT32         mapToNodeDataType(PortID const portId) const;
    ...
    private:
        ...
        MBOOL                       mbCfgImgo;
        MBOOL                       mbCfgRrzo;
        MUINT32                     mHwToSw[2]; //map imgo, rrzo to sw
...
getHwPortConfig(list<HwPortConfig_t>* plPortCfg)
{
    ...
    if( mbCfgImgo )
    {
        ...
        HwPortConfig_t full = {
            ...
            (mInitCfg.muRawType == 0 || mInitCfg.muRawType == 2 ), //if raw type != 1 -> pure-raw
...
startHw(list<HwPortConfig_t> & plPortCfg)
{
    ...
    { //configPipe
        ...
        QInitParam halCamIOinitParam(
                (mInitCfg.muRawType == 2), // 2: sensor uses pattern
                mInitCfg.muBitDepth,
                vSensorCfg,
                vPortInfo);
        //
...
dequeLoop()
{
    ...
    for(MUINT32 i =0; i<2; i++)
    {
        ...
        ret = ret && mpCamIO->deque(dequeBufInfo);
    ...
    //
    NG_TRACE_NAME("dequeP1");
    //
    if(!ret)
    {
        MY_LOGE("deque fail");
        AEE_ASSERT("ISP pass1 deque fail");
        return MFALSE;
    }
    //
    muFrameCnt++;
    ...
    for( iter = dequeBufInfo.mvOut.begin(); iter != dequeBufInfo.mvOut.end(); iter++ )
    {
        ...
        //
        ret = ret && handlePostBuffer( mapToNodeDataType(iter->mPortID), (MUINT32)iter->mBuffer, 0);
        //
        MUINT32 fakedata = mapToFakeDataType(iter->mPortID);
        if( fakedata != 0  ) {
            ret = ret && handlePostBuffer( fakedata, (MUINT32)iter->mBuffer, 0);
        }
    }
...
dropFrame(QBufInfo const & qBufInfo)
{
    ...
    for( iter = qBufInfo.mvOut.begin(); iter != qBufInfo.mvOut.end(); iter++ )
    {
        ...
        MUINT32 fakedata = mapToFakeDataType(iter->mPortID);
        //
        onReturnBuffer( nodeType, (MUINT32)iter->mBuffer, 0);
        if( fakedata != 0  ) {
            onReturnBuffer( fakedata, (MUINT32)iter->mBuffer, 0);
        }
    }
    return MTRUE;
}
...
getSensorPixelMode(MUINT32* pPixelMode) const
{
    ...
    pSensorHalObj->sendCommand(
            ...
            (MINT32)&mInitCfg.muScenario,
            (MINT32)&mInitCfg.muFrameRate,
            (MINT32)pPixelMode);
...
{
    cfgImgo = (mSensorInfo.sensorType == SENSOR_TYPE_YUV) || isDataConnected(PASS1_FULLRAW);
...
MUINT32
Pass1NodeImpl::
mapToNodeDataType(PortID const portId) const
{
    if( portId == PORT_IMGO )
        return mHwToSw[0];
