...
threadLoopUpdate()
{
    ...
    if( mPass2Type == PASS2_PURERAW_CAPTURE )
    {
        ...
        handleReturnBuffer(PASS2_CAP_SRC, (MUINT32)postBufData.buf);
    ...
    if( muNRType != 0 )
    {
        ...
        for( int i = 0 ; i < mvOutRequest.size() ; i++ )
        {
            handlePostBuffer( mvOutData[i], (MUINT32)mvOutRequest[i].mBuffer );
        }
        mvOutRequest.clear();
        mvOutData.clear();
    }
...
{
    NG_TRACE_FMT_BEGIN("cap_deqP2:%d", muDeqFrameCnt);
    MBOOL ret = MFALSE;
