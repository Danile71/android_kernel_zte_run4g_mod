...
onReturnBuffer(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
    ...
    if( !ret )
    {
        MY_LOGE("wrong order: data(%d), buf(0x%08X), ext(0x%08X)", data, buf, ext);
    }
    else
    {
        MY_LOGD("enque: data(%d), buf(0x%08X), ext(0x%08X)", data, buf, ext);
    }
...
{
    MY_LOGD("pCapBufMgr(0x%08X)",(MUINT32)pCapBufMgr);
    if(pCapBufMgr == NULL)
    {
...
{
    MY_LOGD("pImageBuffer(0x%08X)",(MUINT32)pImageBuffer);
    if(pImageBuffer == NULL)
    {
