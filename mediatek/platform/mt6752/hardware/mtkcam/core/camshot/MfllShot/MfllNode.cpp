...
//#define MY_LOGE(fmt, arg...)	XLOGE("error" LOG_TAG fmt, ##arg)
//#define MY_LOGD                 XLOGD
//#define MY_LOGE                 XLOGE
#endif
...
{
    FUNC_START;
    MY_LOGD("data(0x%x), buf(0x%x), ext(0x%x)", data, buf, ext);
...
onReturnBuffer(MUINT32 const data, MUINT32 const buf, MUINT32 const ext)
{
    ...
    if(mPass2Cnt >= MAX_REFERENCE_FRAMES+1) {
        if(mCallback && mUser) {
            MY_LOGD("exec mCallback", data, buf, ext);
            mCallback(mUser);
        }
        else
...
{
    FUNC_START;
    printf("data(0x%x), buf(0x%x), ext(0x%x)\n", data, buf, ext);
    MY_LOGD("data(0x%x), buf(0x%x), ext(0x%x)", data, buf, ext);
    switch( data )
    {
        case PASS1_FULLRAW:
...
{
    FUNC_START;
    MY_LOGD("data(0x%x), buf(0x%x)", data, buf);
    // return buffer
...
threadLoopUpdate()
{
    ...
    MY_LOGD("imagebuffer(0x%x)", imgRequest.mBuffer);
    ...
    //send to next node
    handlePostBuffer(mPort, (MUINT32)pImageBuffer, count);
    count++;
    FUNC_END;
    return MTRUE;
}
