...
#if TEST_ALGORITHM
#include <mtkcam/core/featureio/pipe/mfll/mfll_hal.h>
...
#define MY_LOGI(fmt, arg...)	do {XLOGI(MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
#define MY_LOGD(fmt, arg...)	do {XLOGD(MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
#define MY_LOGE(fmt, arg...)	do {XLOGE("error" MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
...
allocImageBuffer(IImageBufferHeap **pHeap, sp<IImageBuffer>* pBuf, MUINT32 width, MUINT32 height, int const format)
{
    ...
    *pBuf = (*pHeap)->createImageBuffer();
    ...
    (*pBuf)->syncCache(eCACHECTRL_INVALID);  //hw->cpu
     ...
     MY_LOGD("allocImageBuffer - bufSize=%d, imgSize=[%d,%d], plane=%d, stride=[%d,%d,%d]], boundary=[%d,%d,%d]"
            , imgParam.bufSize
    ...
    MY_LOGD("<YUV> ImgBitsPerPixel:%d BufSizeInBytes:%d %d", (*pBuf)->getImgBitsPerPixel(), (*pBuf)->getBufSizeInBytes(0), imgSize.size());
    ...
    ********************************************************************************/
    {
        ...
        MY_LOGD("mfllthread all(%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld)"
                , mThreadMemory
                , mThreadCapture
                , mThreadYuy2ToYv16
                , mThreadMfll
        ...
        MY_LOGD("mfllthread mem(%ld), cap(%ld), yuy2ToYv16(%ld) mfll(%ld), memc(%ld), blend(%ld), pv(%ld), full(%ld), thumbnail(%ld)"
                , mThreadMemory
                , mThreadCapture
                , mThreadYuy2ToYv16
                , mThreadMfll
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    switch(cmd) {
        case ECamShot_CMD_SET_PREVIEW_BUFHDL:
            mpPrvBufHandler = reinterpret_cast<ICamBufHandler*>((MVOID*)arg1);
            MY_LOGD("set prv buf handle %p", mpPrvBufHandler);
            break;
...
flowCapture()
{
    ...
    ret = ret && mpBurstShot->sendCommand( NSCamShot::ECamShot_CMD_SET_PREVIEW_BUFHDL, (MINT32)mpPrvBufHandler,0,0);
...
flowMfll()
{
    ...
    if(mi4DataMsgSet & ECamShot_DATA_MSG_JPEG) {
        handleDataCallback(ECamShot_DATA_MSG_JPEG
                            , (MUINT32)mMemoryJpeg_Thumbnail.get()
                            , (MUINT32)mDebugInfo
                            , mMemoryJpeg.get()
                            );
    }
...
initInStartOne(SensorParam const & rSensorParam)
{
    ...
    ********************************************************************************/
...
#if TEST_ALGORITHM
    {
        ...
        ret = ret && MfllShot::getCaptureInfo(mMfbMode, mSensorParam.u4OpenID, frameNumber, exp, iso);
        if(mMfbMode == MFB_MODE_AIS) {
...
doEis(MUINT32 ref, MINT32 &gmvX, MINT32 &gmvY) 
{
    ...
    //
	MINT32  eis_trush_th  = CUST_MFLL_EIS_TRUST_TH;
	...
	//for(int i = 1; i < FrameNum; i++)	//for each frame
	{
		...
		for( int j = 0; j < EIS_MAX_WIN_NUM; j++ )	//for each bin
		{
			...
			if( motion_data.eis_data2[2*j] > eis_trush_th )	//if confidence x > confidence threahold
			{
				sum_x_tmp += motion_data.eis_data1[2*j];		//calculate sum of MVX
				cnt_x_tmp++;
			}
			if( motion_data.eis_data2[2*j+1] > eis_trush_th ) //if confidence y > confidence threahold
			{
...
doMfb(MUINT32 ref)
{
...

    {
        ...
        ParamIspProfile_T _3A_profile((muNRType==ECamShot_NRTYPE_SWNR ? EIspProfile_MFB_Blending_All_Off_SWNR : EIspProfile_MFB_Blending_All_Off)
                                    , 0  //magicNum
                                    , MFALSE
    ...
    if(!ret) {
        ...
        MY_LOGE("[MFB BLD] deque mvIn.size(%d), mvOut.size(%d)"
...
doEncodeYuvB()
{
    ...
    ret = ret && encodeRawToYuv(mMemoryRawB
                                , mMemoryYuvB
                                , EIspProfile_MFB_PostProc_EE_Off
                                , mPrivateData);
...
doEncodeNormalYuvB()
{
    ...
    ret = ret && encodeRawToYuv(mMemoryRawB
                                , mMemoryYuvB
                                , (muNRType==ECamShot_NRTYPE_SWNR ? EIspProfile_MFB_PostProc_ANR_EE_SWNR : EIspProfile_MFB_PostProc_ANR_EE)
                                , mPrivateData
                                );
#if TEST_DBGINFO
...
encodeRawToYuv(sp<IImageBuffer> &rawBuffer, sp<IImageBuffer> &yuvBuffer, EIspProfile_T profile, void* privateData)
{
    ...
    MY_LOGD("deque mvIn.size(%d),mvOut.size(%d)", dequeParams.mvIn.size(), dequeParams.mvOut.size());
...
doSingleFrameOutput()
{
    ...
    ret = ret && convertImage(mMemoryYuvB, mMemoryYuvMixing, mShotParam.u4PictureTransform);
...
doMixing()
{
    ...
    {
        ParamIspProfile_T _3A_profile((muNRType==ECamShot_NRTYPE_SWNR ? EIspProfile_MFB_PostProc_Mixing_SWNR : EIspProfile_MFB_PostProc_Mixing)
                                    , 0  //magicNum
                                    , MFALSE
                                    , ParamIspProfile_T::EParamValidate_P2Only
                                    );
    ...
    if(TEST_BITTURE_LDVT) {
        // blending yuv = imgi
        mMemoryYuv->unlockBuf(LOG_TAG);
        mMemoryYuv->loadFromFile("/sdcard/input_mixer/imgi_h_5280x960.yuy2");
        mMemoryYuv->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_MASK);

        // normal yuv = vipi
        mMemoryYuvB->unlockBuf(LOG_TAG);
        mMemoryYuvB->loadFromFile("/sdcard/input_mixer/vipi_h_5280x960.yuy2");
        mMemoryYuvB->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_MASK);
        ...
        // blending yuv
        mMemoryYuv->unlockBuf(LOG_TAG);
        mMemoryYuv->loadFromFile("/sdcard/input/Mixer/Result211_blend.yuv_3200x2400.yuy2");
        mMemoryYuv->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_MASK);

        // normal yuv
        mMemoryYuvB->unlockBuf(LOG_TAG);
        mMemoryYuvB->loadFromFile("/sdcard/input/Mixer/Result211_sf.yuv_3200x2400.yuy2");
        mMemoryYuvB->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READ|eBUFFER_USAGE_SW_MASK);
    ...
    if(mDump & MFLL_DUMP_MIXING) {
        ...
        MY_LOGD("flowCapture saveToFile %s", filename);
        mMemoryYuv->saveToFile(filename);
        // normal yuv
    ...
    params.mvIn.clear();
    // - mfb output
    minput_imgi.mBuffer = mMemoryYuv.get();
    minput_imgi.mPortID.inout =  0; //in
    minput_imgi.mPortID.index = NSCam::NSIoPipe::NSPostProc::EPipePortIndex_IMGI;
    minput_imgi.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
    params.mvIn.push_back(minput_imgi);
    MY_LOGD("minput_imgi.mBuffer = {VA=%0x%x, size=%d}", mMemoryYuv->getBufVA(0), mMemoryYuv->getBufSizeInBytes(0));
    ...
    // - normal capture
    minput_vipi.mBuffer = mMemoryYuvB.get();
    minput_vipi.mPortID.inout = 0; //in
    minput_vipi.mPortID.index = NSCam::NSIoPipe::NSPostProc::EPipePortIndex_VIPI;
    minput_vipi.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
    params.mvIn.push_back(minput_vipi);
    MY_LOGD("minput_vipi.mBuffer = {VA=0x%x, size=%d}", mMemoryYuvB->getBufVA(0), mMemoryYuvB->getBufSizeInBytes(0));
    ...
    minput_vip3i.mPortID.type = NSCam::NSIoPipe::EPortType_Memory;
    params.mvIn.push_back(minput_vip3i);
    MY_LOGD("minput_vip3i.mBuffer = {VA=0x%x, size=%d}"
            , mMemoryWeightingMap[mMemoryFinalWeightingMapIndex]->getBufVA(0)
    ...
    MY_LOGD("minput_ufdi.mBuffer = {VA=0x%x, size=%d}"
            , mMemoryWeightingMap[mMemoryFinalWeightingMapIndex]->getBufVA(0)
            , mMemoryWeightingMap[mMemoryFinalWeightingMapIndex]->getBufSizeInBytes(0)
            );
    ...
    if(!ret) 
    {
        ...
        MY_LOGE("[MFB MIX] deque mvIn.size(%d), mvOut.size(%d)", dequeParams.mvIn.size(), dequeParams.mvOut.size());
        goto lbExit;
    }
...
MBOOL
MfllShot::
getCaptureInfo(MUINT32 mfbMode, MUINT32 sensorId, MUINT32 &frameNumber, MUINT32 &exp, MUINT32 &iso)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    ...
    NVRAM_CAMERA_FEATURE_MFLL_STRUCT rTuningData;
    CaptureParam_T rCap3AParam;
    MINT32 needFlash = 1;
