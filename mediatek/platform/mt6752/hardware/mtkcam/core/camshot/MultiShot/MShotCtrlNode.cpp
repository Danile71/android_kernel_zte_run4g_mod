...
onStop()
{
    ...
    if( mpP1Buffer != NULL ) {
        handleReturnBuffer(CONTROL_FULLRAW, (MUINT32)mpP1Buffer);
        mpP1Buffer = NULL;
    }
    ...
    switch(data)
    {
        case CONTROL_FULLRAW:
            if( getFlag(DO_CAPTURE) ) {
                ...
                if( old )
                    handleReturnBuffer(CONTROL_FULLRAW, (MUINT32)old);
            }
            break;
...
triggerCaptureP2()
{
    ...
    if( pDbg )
        handlePostBuffer(CONTROL_DBGINFO, (MUINT32)pDbg, 0);

    handlePostBuffer(CONTROL_CAP_SRC, (MUINT32)buf);
}
