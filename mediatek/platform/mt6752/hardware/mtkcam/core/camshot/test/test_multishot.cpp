...
static MUINT32 u4DataCbFlag = 0;

static IMultiShot *g_pMultiShot = NULL;
/******************************************************************************
*
*******************************************************************************/
static
...
static MBOOL fgCamShotDataCb(MVOID* user, CamShotDataInfo const msg)
{
    ...
    switch (rDataInfo.msgType)
    {
        case ECamShot_DATA_MSG_RAW:
            if( u4DataCbFlag & ECamShot_DATA_MSG_RAW )
            {
                u4DataCbFlag = 0x0;
                u4CapCnt++;
            }
            u4DataCbFlag |= ECamShot_DATA_MSG_RAW;
            handleBayerDataCallback(msg);
            if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_RAW )
                g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_RAW, msg.pBuffer);
        break;
        case ECamShot_DATA_MSG_YUV:
            if( u4DataCbFlag & ECamShot_DATA_MSG_YUV )
            {
                u4DataCbFlag = 0x0;
                u4CapCnt++;
            }
            u4DataCbFlag |= ECamShot_DATA_MSG_YUV;
            handleYuvDataCallback(msg);
            if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_YUV )
            {
                g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_YUV, msg.pBuffer);
            }
        break;
        case ECamShot_DATA_MSG_POSTVIEW:
            if( u4DataCbFlag & ECamShot_DATA_MSG_POSTVIEW )
            {
                u4DataCbFlag = 0x0;
                u4CapCnt++;
            }
            u4DataCbFlag |= ECamShot_DATA_MSG_POSTVIEW;
            handlePostViewCallback(msg);
            if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_POSTVIEW )
                g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_POSTVIEW, msg.pBuffer);
        break;
        case ECamShot_DATA_MSG_JPEG:
            if( u4DataCbFlag & ECamShot_DATA_MSG_JPEG )
            {
                u4DataCbFlag = 0x0;
                u4CapCnt++;
            }
            u4DataCbFlag |= ECamShot_DATA_MSG_JPEG;
            handleJpegCallback(msg);
            if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_JPEG )
                g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_JPEG, msg.pBuffer);
        break;
...
static MUINT32 u4Capture_Cmd(int argc, char** argv)
{
    ...
    g_pMultiShot = IMultiShot::createInstance(eShotMode_NormalShot, "testshot");
    //
    g_pMultiShot->init();
    //
    g_pMultiShot->enableDataMsg( g_u4EnableMsg );
    // set buffer
    //
    // register buffer
...
//    //
    if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_RAW )
        g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_RAW, g_ImgBufRaw.get());
    if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_YUV )
        g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_YUV, g_ImgBufYuv.get());
    if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_POSTVIEW )
        g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_POSTVIEW, g_ImgBufPostview.get());
    if( g_u4RegisterDataMsg & ECamShot_BUF_TYPE_JPEG )
        g_pMultiShot->registerImageBuffer(ECamShot_BUF_TYPE_JPEG, g_ImgBufJpeg.get());
...
//    g_pMultiShot->registerImgBufInfo(ECamShot_BUF_TYPE_JPEG, g_rJpegBufInfo);
    ...
    g_pMultiShot->start(rSensorParam, g_u4ShotCnt);
    //
    g_pMultiShot->uninit();
    //
...
{
    printf("\n***********************************************************\n");
    printf("* CamShot MultiShot CLI Test                             *\n");
