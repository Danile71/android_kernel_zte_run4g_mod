...
MBOOL IMemDrvImp::init(void)
{
    ...
    IMEM_INF("mCount(%d/%d),fd", mInitCount,mLocal_InitCount,mIspFd);
    //
	...
	if( IMEM_MIN_ION_FD > pInfo->memID)
	{
		    switch(ctrl)
	    {
	    	case IMEM_CACHECTRL_ENUM_FLUSH:
	    		mpM4UDrv->m4u_cache_sync(M4U_PORT_IMGI,M4U_CACHE_FLUSH_BY_RANGE,pInfo->virtAddr,pInfo->size,pInfo->phyAddr);
	    		break;
	   		case IMEM_CACHECTRL_ENUM_INVALID:
	   			mpM4UDrv->m4u_cache_sync(M4U_PORT_IMGI,M4U_CACHE_INVALID_BY_RANGE,pInfo->virtAddr,pInfo->size,pInfo->phyAddr);
	   			break;
	   		default:
