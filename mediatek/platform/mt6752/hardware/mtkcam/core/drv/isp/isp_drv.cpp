...
//
    ...
    =   {
            ...
            {CAM_P1_MAGIC_NUM,       0x75DC,  2    },
            {CAM_P1_MAGIC_NUM_D,     0x75E4,  2    },
...
{
    //LOG_DBG("");
    return IspDrvVirImp::getInstance(cq,(MUINT32)mpIspVirRegAddrVA[cq],(MUINT32)mpIspVirRegMap);
}
//-----------------------------------------------------------------------------
...
MBOOL IspDrv::cqAddModule(MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
{
    ...
    mpIspCQDescriptorVirt[realCQIdx][moduleId].v_reg_addr = (MUINT32)&mpIspVirRegAddrPA[realCQIdx][mIspCQModuleInfo[moduleId].addr_ofst >>2 ]; // >>2 for MUINT32* pointer
...
{
	int realCQIdx=getRealCQIndex(cq,burstQIdx,dupCqIdx);
    LOG_INF(" - E. isp_cq[0x%x],[%d]",realCQIdx,moduleId);
    //
    Mutex::Autolock lock(cqVirDesLock);
        ...
        for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
            for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
                if((mTuningQueInf[i][j].queFd>=0) && (mTuningQueInf[i][j].pTuningQue!=0)){
                    ...
                    m_ispTmpBufInfo.virtAddr = (MUINT32)mTuningQueInf[i][j].pTuningQue;
            ...
            if((mTuningQueIdx[i].keepP2PreviewQue.queFd>=0) && (mTuningQueIdx[i].keepP2PreviewQue.pTuningQue!=0)){
                ...
                m_ispTmpBufInfo.virtAddr = (MUINT32)mTuningQueIdx[i].keepP2PreviewQue.pTuningQue;
                if ( m_pIMemDrv->freeVirtBuf(&m_ispTmpBufInfo) ) {
                    LOG_ERR("ERROR:m_pIMemDrv->freeVirtBuf");
                }
                mTuningQueIdx[i].keepP2PreviewQue.queFd = -1;
            ...
            if((mTuningQueIdx[i].keepP1Que.queFd>=0) && (mTuningQueIdx[i].keepP1Que.pTuningQue!=0)){
                ...
                m_ispTmpBufInfo.virtAddr = (MUINT32)mTuningQueIdx[i].keepP1Que.pTuningQue;
                if ( m_pIMemDrv->freeVirtBuf(&m_ispTmpBufInfo) ) {
                    LOG_ERR("ERROR:m_pIMemDrv->freeVirtBuf");
                }
                mTuningQueIdx[i].keepP1Que.queFd = -1;
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            mpIspVirRegAddrPA=(MUINT32 **)malloc(mTotalCQNum*sizeof(MUINT32 *)+mTotalCQNum*1*sizeof(MUINT32));
            ...
            mpIspVirRegAddrVA=(MUINT32 **)malloc(mTotalCQNum*sizeof(MUINT32 *)+mTotalCQNum*1*sizeof(MUINT32));
            for( int i=0; i<mTotalCQNum; i++ )
            {
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                ...
                LOG_INF("cq:[%d],virtIspAddr:virt[0x%08x]/phy[0x%08x]",i,(MUINT32)mpIspVirRegAddrVA[i], (MUINT32)mpIspVirRegAddrPA[i] );
            }
            ...
            for (int i = 0; i < (CAM_MODULE_MAX-1); i++) {
              mIspCQDescInit[i].v_reg_addr = (MUINT32)( mpIspVirRegAddrPA[0] + (ISP_DRV_CQ_DUMMY_WR_TOKEN/4) );     //why use mpIspVirRegAddrPA[0] for all initialization
            }
            for (int i = 0; i < mTotalCQNum; i++)
            {
                if ( 0 == i ) {
                ...
                LOG_INF("cq:[%d],mpIspCQDescriptor:Virt[0x%08x]/Phy[0x%08x],size/num(%d/%d/%d)",i,(MUINT32)mpIspCQDescriptorVirt[i],(MUINT32)mpIspCQDescriptorPhy[i],sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT),CAM_MODULE_MAX,sizeof(mIspCQDescInit));
            }
...
MBOOL IspDrvImp::dumpP2DebugLog(IspDumpDbgLogP2Package* pP2Package)
{
    ...
    //check tpipe
    LOG_INF("CAM_CTL_TCM_EN=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_TCM_EN-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_TDR_DBG_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_TDR_DBG_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check DMA error
    LOG_INF("CAM_IMGI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_IMGI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_BPCI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_BPCI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_LSCI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_LSCI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_UFDI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_UFDI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
//     
//QQ    LOG_INF("CAM_LCEI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_LCEI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));

    LOG_INF("CAM_VIPI_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_VIPI_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_VIP2I_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_VIP2I_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_VIP3I_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_VIP3I_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_ESFKO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_ESFKO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_MFBO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_MFBO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_IMG3BO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_IMG3BO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_IMG3CO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_IMG3CO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_IMG2O_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_IMG2O_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_IMG3O_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_IMG3O_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
//QQ    LOG_INF("CAM_FEO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_FEO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check RAW
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x1030, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RAW]raw_cfa_debug_0=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x1031, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RAW]raw_cfa_debug_1=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x1032, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RAW]raw_cfa_debug_2=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x1033, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RAW]raw_cfa_debug_3=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2107, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RAW]mfb_debug_data=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check YUV
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3016, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]lce_out_debug_0=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3017, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]lce_out_debug_1=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3018, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]lce_out_debug_2=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3019, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]lce_out_debug_3=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301A, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]crsp_out_debug_0=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301B, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]crsp_out_debug_1=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301C, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]crsp_out_debug_2=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301D, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]crsp_out_debug_3=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301E, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]c24b_out_debug_0=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x301F, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]c24b_out_debug_1=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3020, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]c24b_out_debug_2=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3021, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]c24b_out_debug_3=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x3014, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[YUV]mdp_crop_debug=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check RGB
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2010, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RGB]rgb_RGB_debug_0=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2011, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RGB]rgb_RGB_debug_1=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2012, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RGB]rgb_RGB_debug_2=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2013, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RGB]rgb_RGB_debug_3=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x2017, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[RGB]mfb_debug_data=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check TOP
    LOG_INF("CAM_CTL_RAW_REQ_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_RAW_REQ_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_DMA_REQ_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DMA_REQ_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_RGB_REQ_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_RGB_REQ_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_YUV_REQ_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_YUV_REQ_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_RAW_RDY_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_RAW_RDY_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_DMA_RDY_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DMA_RDY_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_RGB_RDY_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_RGB_RDY_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("CAM_CTL_YUV_RDY_STATUS=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_YUV_RDY_STATUS-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check CQ
    writeReg((((UINT32)&ispReg.CAM_CTL_DBG_SET-(UINT32)&ispReg)), 0x6000, ISP_DRV_USER_ISPF, 0);
    LOG_INF("[CQ]cq status=0x%08x",readReg((((UINT32)&ispReg.CAM_CTL_DBG_PORT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //check MDP
    LOG_INF("[MDP]CAM_MDP_CROP_X=0x%08x",readReg((((UINT32)&ispReg.CAM_MDP_CROP_X-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    LOG_INF("[MDP]CAM_MDP_CROP_Y=0x%08x",readReg((((UINT32)&ispReg.CAM_MDP_CROP_Y-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    //
    //dump tpipe
    regTpipePA = readReg(((UINT32)&ispReg.CAM_TDRI_BASE_ADDR-(UINT32)&ispReg));
...
MBOOL IspDrvImp::dumpP1DebugLog(MUINT32* p1)
{
	if(mFd >= 0 ){
    	...
    	if(p1[1] == MTRUE){
		    ...
		    _flag[1] = p1[0];//_IRQ
		    if(ioctl(mFd,ISP_DEBUG_FLAG,_flag) < 0){
	    ...
	    if(p1[2] == MTRUE){
			...
			if(ioctl(mFd,ISP_GET_CUR_SOF,_flag) < 0){
			...
			MUINT32 _flag[18] = {0};
			if(ioctl(mFd,ISP_GET_DMA_ERR,_flag) < 0){
				LOG_ERR("dump sof error\n");
			}
			DMA_CHK(_flag,p1[4]);
		}
			...
			MUINT32 _flag[4] = {0};
			if(ioctl(mFd,ISP_GET_INT_ERR,_flag) < 0){
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::dumpCQTable(ISP_DRV_CQ_ENUM cq, MINT32 burstQIdx,MUINT32 dupCqIdx, MUINT32 cqDesVa, MUINT32 cqVirVa)
{
    MUINT32 i;
    MUINT32 cqLabel;
    ...
    if(cqDesVa==0 || cqVirVa==0){
        ...
        LOG_INF("[BasePA]CQ1(0x%x),CQ2(0x%x),CQ3(0x%x)",readReg((((UINT32)&ispReg.CAM_CTL_CQ1_BASEADDR-(UINT32)&ispReg)), ISP_DRV_USER_ISPF),
            readReg((((UINT32)&ispReg.CAM_CTL_CQ2_BASEADDR-(UINT32)&ispReg)), ISP_DRV_USER_ISPF),readReg((((UINT32)&ispReg.CAM_CTL_CQ3_BASEADDR-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
        LOG_INF("[REG]P2_EN(0x%x),P2_EN_DMA(0x%x),SCENARIO_REG(0x%x),CQ_EN_P2(0x%x)",readReg((((UINT32)&ispReg.CAM_CTL_EN_P2-(UINT32)&ispReg)), ISP_DRV_USER_ISPF),
            ...
            readReg((((UINT32)&ispReg.CAM_CTL_CQ_EN_P2-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
	...
	if((pwaitIrq->bDumpReg == 0xfe) || ((waitirq.Status & dumpStatus) == 0))
    ...
    if(pwaitIrq->bDumpReg == 0XFE){
    	LOG_INF("ISP_WAIT_IRQ tiemout info.(%d). Type(%d), Status(0x%08x), Timeout(%d).", Ret, pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout);
    }else {
	    if(Ret < 0) {
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::registerIrq(ISP_DRV_WAIT_IRQ_STRUCT waitIrq)
{
	MINT32 Ret;
	//
	Ret = ioctl(mFd,ISP_REGISTER_IRQ,&waitIrq);
	if(Ret < 0) {
        LOG_ERR("ISP_REGISTER_IRQ fail(%d). Type(%d), Status(0x%08x), Timeout(%d).", Ret, waitIrq.Type, waitIrq.Status, waitIrq.Timeout);
        return MFALSE;
    }
	return MTRUE;
}
...
MUINT32 IspDrvImp::getCurHWRegValues()
{
	...
	return ((MUINT32)mpTempIspHWRegValues);
}
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (MUINT32)(va) - (MUINT32)(bva) ) + (MUINT32)(bpa) )
    ...
    if ( 0 == (cam_tg_vf & 0x01) ) {
        ...
        pcqrtbcring_va->rtbc_ring[i].cq_rtbc.rrzo.data_ptr_pa = \
            (MUINT32)RTBC_GET_PA_FROM_VA(&pcqrtbcring_va->rtbc_ring[i].cq_rtbc.rrzo_base_pAddr,pcqrtbcring_va,pcqrtbcring_pa);
        //
        ...
        if ( _imgo_ == pbuf_ctrl->buf_id || _rrzo_ == pbuf_ctrl->buf_id) {
            ...
            ISP_WRITE_REG(this,CAM_CTL_CQ0C_BASEADDR,this->getCQDescBufPhyAddr(ISP_DRV_CQ0C,0,ISP_DRV_P1_DEFAULT_DUPCQIDX) + ((1==size)?0:sizeof(CQ_RING_CMD_ST)),ISP_DRV_USER_ISPF);
        }
        else if ( _imgo_d_ == pbuf_ctrl->buf_id || _rrzo_d_ == pbuf_ctrl->buf_id) {
            //LOG_INF("[rtbc] Update CAM_CTL_CQ0C_D_BASEADDR");
            ISP_WRITE_REG(this,CAM_CTL_CQ0C_D_BASEADDR,this->getCQDescBufPhyAddr(ISP_DRV_CQ0C_D,0,ISP_DRV_P1_DEFAULT_DUPCQIDX) + ((1==size)?0:sizeof(CQ_RING_CMD_ST)),ISP_DRV_USER_ISPF);
        }
        //
...
)
{
    ...
    LOG_INF("+,path(%d),eCq(%d),p2Cq(%d),magic#(0x%x),curRP2Idx(%d),curWIdx(%d)", \
...
//-----------------------------------------------------------------------------
IspDrv* IspDrvVirImp::getInstance(MINT32 cq, MUINT32 ispVirRegAddr, MUINT32 ispVirRegMap)
{
	...
	singleton[cq].mpIspVirRegMap = (isp_reg_t*)ispVirRegMap;
    return &singleton[cq];
}
//-----------------------------------------------------------------------------
