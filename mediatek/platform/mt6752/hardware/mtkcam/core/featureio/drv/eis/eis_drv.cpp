...
#define EIS_INF(fmt, arg...)    XLOGI("[%s]" fmt, __func__, ##arg)
#define EIS_WRN(fmt, arg...)    XLOGW("[%s] WRN(%5d):" fmt, __func__, __LINE__, ##arg)
#define EIS_ERR(fmt, arg...)    XLOGE("[%s] %s ERROR(%5d):" fmt, __func__,__FILE__, __LINE__, ##arg)
...
MINT32 EisDrvImp::Init()
{
    ...
    m_pNormalPipe->sendCommand(EPIPECmd_SET_EIS_CBFP,(MINT32)m_pEisP1Cb,-1,-1);
...
MINT32 EisDrvImp::EnableEis(const MBOOL &aEn)
{
    ...
    // [mark] This is only temp usage for ROME ISP driver design. HW doesn't have EISO_EN bit.
    // EISO_EN
...
MINT32 EisDrvImp::UpdateEiso()
{
    ...
    if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_GET_MODULE_HANDLE, EModule_EISO,(MINT32)(&handle),(MINT32)(&(EIS_DRV_NAME))))
    {
    ...
    if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_SET_MODULE_CFG_DONE, handle,-1,-1))
    {
        ...
        m_pNormalPipe->sendCommand(EPIPECmd_RELEASE_MODULE_HANDLE,handle,(MINT32)(&(EIS_DRV_NAME)),-1);
        return EIS_RETURN_API_FAIL;
    }

    //====== Release Handle ======
    
    if(MFALSE == m_pNormalPipe->sendCommand(EPIPECmd_RELEASE_MODULE_HANDLE,handle,(MINT32)(&(EIS_DRV_NAME)),-1))
    {
...
MINT32 EisDrvImp::UpdateEisoIdx()
{
    ...
    if(MTRUE != m_pISPDrvObj->waitIrq(&waitIrq))
    {
        if(EIS_SW_STATE_UNINIT != GetEisState())
        {
            ...
            EIS_LOG("[miss]post mTsSem");
        }
        else
        {
            EIS_LOG("[miss]post mTsSem,state(%d)",GetEisState());            
        }
...
MINT32 EisDrvImp::SetFirstFrame(const MUINT32 &aFirst)
{
    ...
    //> get handle                
    if(MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_GET_MODULE_HANDLE, EModule_EIS,(MINT32)(&handle),(MINT32)(&(EIS_DRV_NAME))))
    {
    ...
    if(MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_RELEASE_MODULE_HANDLE,handle,(MINT32)(&(EIS_DRV_NAME)),-1))
    {
        EIS_ERR("release EISO handle fail");
        return EIS_RETURN_API_FAIL;
    }
...
MBOOL EisDrvImp::GetEisSupportInfo(const MUINT32 &aSensorIdx)
{
    ...
    m_pNormalPipe->sendCommand(EPIPECmd_GET_EIS_INFO,aSensorIdx,(MINT32)&eisInfo, -1);
...
*
********************************************************************************/
MVOID EisDrvImp::GetEisHwStatistic(EIS_STATISTIC_STRUCT *apEisStat,const MINT64 &aTimeStamp)
{
    //====== Wait Semaphore ======

    EIS_LOG("wait mTsSem(%lld)",aTimeStamp);
    ::sem_wait(&mTsSem);
    EIS_LOG("got mTsSem");
    ...
    if(aTimeStamp == 0)
    {
        ...
        EIS_LOG("drop:(va,pa)=(0x%08x,0x%08x)",mEisoData.front().va,mEisoData.front().pa);
        mEisoData.pop();
    }
    else
    {
        ...
        //if(g_debugDump == 3)
        //{
            EIS_LOG("pEisoAddr(0x%08x)",(MUINT32)pEisoAddr);
        //}
        ...
        for(MINT32 i = 0; i < EIS_MAX_WIN_NUM; ++i)
        {
            if(i != 0)
            {
                ...
                if(g_debugDump == 3)
                {
                    EIS_LOG("i(%d),pEisoAddr(0x%08x)",i,(MUINT32)pEisoAddr);
                }
            }
...
*
********************************************************************************/
MVOID EisDrvImp::GetEisHwStatistic(EIS_STATISTIC_T *apEisStat,const MINT64 &aTimeStamp)
{
    //====== Wait Semaphore ======
...
MVOID EisDrvImp::GetFeoStatistic(FEO_STAT_DATA *aFeoStatData)
{
    ...
    if(g_debugDump >= 1)    
    {
        ...
        EIS_LOG("0.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
    } 
    ...
    for(MUINT32 y = 0; y < mFeoStaticNumY; y++)
    {
        ...
        {            
            if(g_debugDump == 3)
            {                
                EIS_LOG("1.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
            }
            ...
            if(g_debugDump == 3)
            {                
                EIS_LOG("2.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
            }
            ...
            for(MUINT32 i = 0; i < 4; i++)
            {
                ...
                if(g_debugDump == 3)                
                {                
                    EIS_LOG("3.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
                }
                ...
                if(g_debugDump == 3)
                {                
                    EIS_LOG("4.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
                }
                ...
                if(g_debugDump == 3)
                {                
                    EIS_LOG("5.pFeoAddr(0x%x)",(MUINT32)pFeoAddr);
                }
...
MINT32 EisDrvImp::WriteReg(const REG_NAME_ENUM &aRegName)
{
    ...
    switch(aRegName)
    {
        ...
        case REG_EISO:
            ...
            if(MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_GET_MODULE_HANDLE, EModule_EISO,(MINT32)(&handle),(MINT32)(&(EIS_DRV_NAME))))
            {
                EIS_ERR("get EISO handle fail");
                return EIS_RETURN_API_FAIL;
            }
        ...
        case REG_SGG2:
            ...
            if(MTRUE != m_pNormalPipe->sendCommand(EPIPECmd_GET_MODULE_HANDLE, EModule_SGG2,(MINT32)(&handle),(MINT32)(&(EIS_DRV_NAME))))
            {
