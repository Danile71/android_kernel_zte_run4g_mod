...
MCUDrv::lensSearch( unsigned int a_u4CurrSensorDev, unsigned int a_u4CurrSensorId)
{
    ...
    if (a_u4CurrSensorDev == MCU_DEV_MAIN )
    {
		...
		MCU_DRV_DBG("[LensInitTable-0][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main[0].SensorId, MCUDrv::m_LensInitFunc_main[0].LensId);
		MCU_DRV_DBG("[LensInitTable-1][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main[1].SensorId, MCUDrv::m_LensInitFunc_main[1].LensId);
		MCU_DRV_DBG("[LensInitTable-2][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main[2].SensorId, MCUDrv::m_LensInitFunc_main[2].LensId);	
		MCU_DRV_DBG("[LensInitTable-3][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main[3].SensorId, MCUDrv::m_LensInitFunc_main[3].LensId);
	...
	else if( a_u4CurrSensorDev == MCU_DEV_MAIN_2)
	{
		...
		MCU_DRV_DBG("[LensInitTable-0][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main2[0].SensorId, MCUDrv::m_LensInitFunc_main2[0].LensId);
		MCU_DRV_DBG("[LensInitTable-1][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main2[1].SensorId, MCUDrv::m_LensInitFunc_main2[1].LensId);
		MCU_DRV_DBG("[LensInitTable-2][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main2[2].SensorId, MCUDrv::m_LensInitFunc_main2[2].LensId);	
		MCU_DRV_DBG("[LensInitTable-3][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_main2[3].SensorId, MCUDrv::m_LensInitFunc_main2[3].LensId);
		MCUDrv::m_u4CurrLensIdx_main2 = 0;

	    for (i=0; i<MAX_NUM_OF_SUPPORT_LENS; i++) 
    {
	        if ((MCUDrv::m_LensInitFunc_main2[i].LensId == DUMMY_LENS_ID) ||
	            (MCUDrv::m_LensInitFunc_main2[i].LensId == SENSOR_DRIVE_LENS_ID) ||
	            (MCUDrv::m_LensInitFunc_main2[i].LensId == FM50AF_LENS_ID)
                )
            {
	    ...
	    for (i=0; i<MAX_NUM_OF_SUPPORT_LENS; i++) 
		{
            ...
            {
	            MCUDrv::m_u4CurrLensIdx_main2 = i;
	            MCU_DRV_DBG("[idx]%d [CurrSensorId]0x%04x,[CurrLensIdx]0x%04x\n", i, a_u4CurrSensorId, MCUDrv::m_u4CurrLensIdx_main2);                
                break;
            }
        }
	...
	else if( a_u4CurrSensorDev == MCU_DEV_SUB)
	{
		...
		MCU_DRV_DBG("[LensInitTable-0][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_sub[0].SensorId, MCUDrv::m_LensInitFunc_sub[0].LensId);
		MCU_DRV_DBG("[LensInitTable-1][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_sub[1].SensorId, MCUDrv::m_LensInitFunc_sub[1].LensId);
		MCU_DRV_DBG("[LensInitTable-2][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_sub[2].SensorId, MCUDrv::m_LensInitFunc_sub[2].LensId);	
		MCU_DRV_DBG("[LensInitTable-3][SensorId]0x%04x,[LensId]0x%04x\n", MCUDrv::m_LensInitFunc_sub[3].SensorId, MCUDrv::m_LensInitFunc_sub[3].LensId);

		MCUDrv::m_u4CurrLensIdx_sub = 0;
	    for (i=0; i<MAX_NUM_OF_SUPPORT_LENS; i++) 
		{
	        if ((MCUDrv::m_LensInitFunc_sub[i].LensId == DUMMY_LENS_ID) ||
	            (MCUDrv::m_LensInitFunc_sub[i].LensId == SENSOR_DRIVE_LENS_ID) ||
	            (MCUDrv::m_LensInitFunc_sub[i].LensId == FM50AF_LENS_ID)
	            )
	        {
	    ...
	    for (i=0; i<MAX_NUM_OF_SUPPORT_LENS; i++) 
		{
	        ...
	        {
	            MCUDrv::m_u4CurrLensIdx_sub = i;
	            MCU_DRV_DBG("[idx]%d [CurrSensorId]0x%04x,[CurrLensIdx]0x%04x\n", i, a_u4CurrSensorId, MCUDrv::m_u4CurrLensIdx_sub);                
	            break;
	        }
	    }
