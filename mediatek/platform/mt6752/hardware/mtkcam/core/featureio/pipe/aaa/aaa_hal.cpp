...
{
	MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3A::postCommand] enter, eCmd(%d)\n", eCmd);

    if ((eCmd == ECmd_CameraPreviewEnd) || (eCmd == ECmd_CamcorderPreviewEnd))
    {
        MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "3A PreviewEnd, start wait semAFProcEnd");
	...
	MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3A::postCommand] before StateMgr::sendCmd(%d)\n", eCmd);
	...
	MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3A::postCommand] after StateMgr::sendCmd(%d)\n", eCmd);
...
if (ENABLE_ISPTUNE & m_3ACtrlEnable) {
    ...
    if (eCmd == ECmd_CameraPreviewEnd || eCmd == ECmd_CaptureEnd || eCmd == ECmd_PrecaptureEnd)
    {
        mbListenVsync = MFALSE;
    }

	MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3A::postCommand] exit, eCmd(%d)\n", eCmd);
...
if (ENABLE_AF & _this->m_3ACtrlEnable) {
    ...
    while (_this->mbAFThreadLoop) {
        ...
        if (_this->mbAFThreadLoop != 0)
        {
            ...
            {
                MY_LOG_IF(1, "[Hal3A::AFThreadFunc] AF waitIrq done\n");
...
MBOOL  Hal3A::sem_wait_relativeTime(sem_t *pSem, nsecs_t reltime, const char* info)
{
    MY_LOG_IF(1, "[%s] pSem(%p), reltime(%lld), info(%s)\n", __FUNCTION__, pSem, reltime, info);
    struct timespec ts;
