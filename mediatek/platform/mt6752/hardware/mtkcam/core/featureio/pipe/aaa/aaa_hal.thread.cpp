...
*******************************************************************************/
MVOID
Hal3A::addCommandQ(ECmd_T const &r3ACmd, MINT32 i4Arg)
{
    Mutex::Autolock autoLock(mModuleMtx);
...
Hal3A::onThreadLoop(MVOID *arg)
{
    ...
    // (2) thread-in-loop
    #if 1
    while (_this->waitVSirqThenGetCommand(rCmd))
    {
        ...
        switch (rCmd.eCmd)
        {
            case ECmd_PrecaptureStart:
                ...
                //
                if ( ! _this->postCommand(rCmd.eCmd, reinterpret_cast<MINT32>(&rCmd.rParamIspProfile)))
                {
            ...
            case ECmd_Update:
                ...
                //::sem_post(&_this->mSem);
                //
                bHaveEverWaitedVsync = MTRUE; //already waited Vsync in waitVSirqThenGetCommand()
                     ...
                     frameIdx++)
                {
                    ...
                    {   
					    MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::onThreadLoop] start waitVSirq.");
                        AAA_TRACE_BEGIN("waitVsync(B)");
					    if (/*frameIdx != 1*/!bHaveEverWaitedVsync) 
                        {
                            _this->waitVSirq();
                            MY_LOG_IF(1, "[Hal3A::onThreadLoop] waitVSirq done.");
                        }
					    bHaveEverWaitedVsync = MFALSE; //reset flag 
                        AAA_TRACE_END();
...
Hal3A::AESensorThreadLoop(MVOID *arg)
{
    ...
    while(1)
	{
		...
		MY_LOG_IF(1, "[AESensorThreadLoop] updateSensorbyI2C\n");
		...
		//sync with 3A thread when PreviewEnd
		{
			...
			MY_LOG_IF(1, "[AESensorThreadLoop] semAESenThdEnd before post = %d\n", Val);
...
Hal3A::postToAESenThread()
{
    ...
    ::sem_post(&semAESen);
    MY_LOG_IF(1, "[postToAESenThread] sem_post semAESen, semAESenThdEnd before wait = %d\n", Val);
}
