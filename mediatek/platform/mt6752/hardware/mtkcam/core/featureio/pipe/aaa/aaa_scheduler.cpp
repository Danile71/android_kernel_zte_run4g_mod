...
{
    XLOGD("[AAA_Scheduler::importWork]+, mNormalizeM=%d, mSensorDevId=%d\n", mNormalizeM, mSensorDevId);
    ...
    //XLOGD("[AAA_Scheduler::importWork]-\n");
}
...
long long int AAA_Scheduler::getVsTimeOutLimit_ns()
{
    ...
    if (mAllLogEn) 
        XLOGD("[%s] (mfps, mNormalizeM, TimeOutLimit) = (%d, %d, %d)\n"
            , __FUNCTION__
            , mfps
            , mNormalizeM
            , ret);
...
MVOID AAA_Scheduler::setSchdParams(MUINT32 sensorDevId, MUINT32 fps, MBOOL logEn, MUINT32 time_mrg_ms)
{
    ...
    if (logEn)
        XLOGD("[%s] (sensorDevId, fps, mNormalizeM, logEn, time_mrg_ms) = (%d, %d, %d, %d, %d)\n", __FUNCTION__
            , sensorDevId
            , fps
            , mNormalizeM
            , logEn
...
{
    if (mAllLogEn)
        XLOGD("[%s]+, (mNormalizeM, mFrameIdx, eJob, bClrTime, mSensorDevId) = (%d, %d, %d, %d, %d)\n", __FUNCTION__
            , mNormalizeM
            , mFrameIdx
            , eJob
            , bClrTime
