...
struct WorkContainerPerFrame
{
    ...
    {
        XLOGD("[WorkPerFrame: (AAO, Awb, Af, Flk, Lsc, AeFlare) = (%d,%d,%d,%d,%d,%d)]\n"
...
public:
    ...
    {
        XLOGD("mValidFrameIdx = %d\n", mValidFrameIdx);
        for (int i=0; i<MAX_FRAME_PER_CYCLE; i++)
        {
            XLOGD("FrameIdx (%d) in Cycle: ", i+1); //frameIdx starts from 1
...
private:
    NS3A::AaaTimer jobTimer[E_Job_NUM];
