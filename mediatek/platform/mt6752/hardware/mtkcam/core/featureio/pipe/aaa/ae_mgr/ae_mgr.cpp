...
#define LONG_CAPTURE_THRES 100000
#define ENABLE_LONG_CAPTURE_CTRL 1
...
#define MAX_CONVERGE_ISO 3600  // ISO1200
#define START_CONVERGE_SHUTTER 50000   // 50ms
...
MRESULT AeMgr::setAEMeteringArea(CameraMeteringArea_T const *sNewAEMeteringArea)
{
    ...
    if(CUST_ENABLE_TOUCH_AE() == MFALSE) {
        MY_LOG("[%s()] i4SensorDev:%d line:%d The set face AE is disable:%d ", __FUNCTION__, m_eSensorDev, __LINE__, CUST_ENABLE_TOUCH_AE());
        return S_AE_OK;
    }
...
MRESULT AeMgr::setSceneMode(MUINT32 u4NewAEScene)
{
    ...
    if (eNewAEScene < MTK_CONTROL_SCENE_MODE_UNSUPPORTED) {
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doPvAE(MINT32 i4FrameCount, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate)
{
strAEInput rAEInput;
strAEOutput rAEOutput;
...
mpAEBuf = new BufInfo_T;
BufInfo_T & rBufInfo = *(BufInfo_T*)(mpAEBuf);
...
//    MY_LOG("%s() : AEMode:%d AEState:%d i4ActiveAEItem:%d u4AAOUpdate:%d\n", __FUNCTION__, m_eAEMode, m_eAEState, i4ActiveAEItem, u4AAOUpdate);
    ...
    if(m_pIAeAlgo != NULL) {
        if((pAEStatisticBuf != NULL) && ((u4AAOUpdate == 1) || (i4ActiveAEItem & E_AE_FLARE) || (i4ActiveAEItem & E_AE_AE_CALC))) {
           {
...
//                        }
        ...
        } else if (i4ActiveAEItem & E_AE_AE_APPLY) {   // apply AE
            if(m_pIAeAlgo != NULL) {
                MY_LOG("[%s()] i4SensorDev:%d AE Wait Vd frame:%d Enable:%d avgY:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum, m_bEnableAE, m_pIAeAlgo->getBrightnessAverageValue());
            ...
            MY_LOG("[%s()] i4SensorDev:%d AE idle command:%d\n", __FUNCTION__, m_eSensorDev, i4ActiveAEItem);
        }
    }else {
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doAFAE(MINT32 i4FrameCount, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate)
{
strAEInput rAEInput;
strAEOutput rAEOutput;
AE_INFO_T rAEInfo2ISP;

    MY_LOG("[doAFAE]:%d %d frameCnt:%d ActiveAE:%d eAETargetMode:%d bTouchAEAreaChage:%d\n", m_i4TimeOutCnt, m_i4WaitVDNum, i4FrameCount, i4ActiveAEItem, m_eAETargetMode, m_bTouchAEAreaChage);
    m_bTouchAEAreaChage = MFALSE;
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doPreCapAE(MINT32 i4FrameCount, MBOOL bIsStrobeFired, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate)
{
strAEInput rAEInput;
strAEOutput rAEOutput;
AE_INFO_T rAEInfo2ISP;
        ...
        if(m_bAEReadyCapture == MFALSE) {
            ...
            if(m_bAEStable == MFALSE) {
                ...
                if (i4ActiveAEItem & E_AE_AE_CALC) {
                    ...
                    if(m_bLockExposureSetting == MTRUE) {
                        m_bAEStable = MTRUE;
                        MY_LOG("[%s()] Lock sensor setting:%d m_bAEStable:%d\n", __FUNCTION__, m_bLockExposureSetting, m_bAEStable);
                        return S_AE_OK;
                    }
                    ...
                    } else if (i4ActiveAEItem & E_AE_AE_APPLY) {
                        // Using preview state to do AE before AE stable
                        UpdateSensorISPParams(AE_AUTO_FRAMERATE_STATE);
                    }
                } else if (i4ActiveAEItem & E_AE_AE_APPLY) {   // apply AE
                    MY_LOG("[%s()] i4SensorDev:%d AE Wait Vd frame:%d Enable:%d State:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum, m_bEnableAE, m_AEState);
                    // continue update for preview or AF state
                    UpdateSensorISPParams(AE_AUTO_FRAMERATE_STATE);
               }
            } else {
                MY_LOG("[%s()] AE stable already\n", __FUNCTION__);
                UpdateSensorISPParams(AE_PRE_CAPTURE_STATE);
                doBackAEInfo();   // do back up AE for Precapture AF state.
            }
...
strAEOutput rAEOutput;
    ...
    copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
    MY_LOG("[getPreviewParams3] Exp. time: %d Preview Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
...
MRESULT AeMgr::doCapFlare(MVOID *pAEStatBuf, MBOOL bIsStrobe)
{
    ...
    MY_LOG("[%s()] i4SensorDev:%d i2FlareOffset:%d i2FlareGain:%d Sensor Mode:%d Strobe:%d Enable:%d %d %d\n", __FUNCTION__, m_eSensorDev,
                   ...
                   m_rAEInitInput.rAENVRAM.rCCTConfig.bEnableVideoThres, m_rAEInitInput.rAENVRAM.rCCTConfig.bEnableStrobeThres);
...
MUINT32 u4NewShutter;
MUINT32 u4NewSensorGain;
MUINT32 u4NewISPGain;
MUINT32 u4PreviewBaseGain=1024;
MUINT32 u4PreviewBaseISO=100;
MUINT32 u4FinalGain;
MUINT32 u4RealISO;
    ...
    if(((m_eAETargetMode == AE_MODE_AOE_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)) && (m_bAEReadyCapture == MFALSE)){
        ...
        // Improve the preview converge speed
        if((m_bAEReadyCapture == MFALSE) && (sAEInfo->bAEStable == MFALSE)) {
                if(sAEOutputInfo->u4Eposuretime > START_CONVERGE_SHUTTER) {
                    rSensorInputData.u4SensorExpTime = START_CONVERGE_SHUTTER;
                    ...
                    u4FinalGain = (rSensorInputData.u4SensorGain*rSensorInputData.u4IspGain)>>10;
                    u4RealISO = u4PreviewBaseISO*u4FinalGain/u4PreviewBaseGain;

                    if(u4RealISO > MAX_CONVERGE_ISO) {
                    	   rSensorInputData.u4SensorGain = 1024* u4PreviewBaseGain * MAX_CONVERGE_ISO / u4PreviewBaseISO / rSensorInputData.u4IspGain;
                    	   rSensorInputData.u4SensorExpTime = u4RealISO*START_CONVERGE_SHUTTER/MAX_CONVERGE_ISO;
                    }
                    ...
                    MY_LOG("[%s()] i4SensorDev:%d Speed up New Shutter:%d %d New Sesnor Gain:%d %d ISP Gain:%d %d\n", __FUNCTION__, m_eSensorDev,
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    m_rAEInitInput.eAEISOSpeed = m_eAEISOSpeed;
    ...
    MY_LOG("[%s()] i4SensorDev:%d SensorIdx:%d AE max block width:%d heigh:%d AE meter:%d CamMode:%d AEScene:%d Flicker :%d %d EVcomp:%d ISO:%d %d MinFps:%d MaxFps:%d Limiter:%d\n", __FUNCTION__, m_eSensorDev, m_i4SensorIdx, m_rAEInitInput.i4AEMaxBlockWidth, m_rAEInitInput.i4AEMaxBlockHeight, m_eAEMeterMode, m_eAECamMode, m_eAEScene, m_eAEFlickerMode, m_eAEAutoFlickerMode, m_eAEEVcomp, m_eAEISOSpeed, m_bRealISOSpeed, m_i4AEMinFps, m_i4AEMaxFps, m_bAElimitor);
#if USE_OPEN_SOURCE_AE
    m_pIAeAlgo = IAeAlgo::createInstance<EAAAOpt_OpenSource>(m_eSensorDev);
#else
    m_pIAeAlgo = IAeAlgo::createInstance<EAAAOpt_MTK>(m_eSensorDev);
#endif
    if (!m_pIAeAlgo) {
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch(eNewAEState)
    {
        ...
        case AE_REINIT_STATE:
            ...
            err = IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, MFALSE, m_rAEOutput.rPreviewMode.u4IspGain>>1);
            m_u4PrevIspGain = m_rAEOutput.rPreviewMode.u4IspGain; // v1.2
            if (FAILED(err)) {
                MY_ERR("setIspAEGain() fail\n");
                return err;
            }

            err = IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));
            if (FAILED(err)) {
            ...
            if(m_pIAeAlgo != NULL) {
                ...
                if(m_p3ANVRAM != NULL) {
                    rAEInfo2ISP.u4RealISOValue = (m_p3ANVRAM->rAENVRAM.rDevicesInfo.u4MiniISOGain*((rAEInfo2ISP.u4AfeGain*rAEInfo2ISP.u4IspGain)>>10))>>10;
                } else {
                    rAEInfo2ISP.u4RealISOValue = (85*((rAEInfo2ISP.u4AfeGain*rAEInfo2ISP.u4IspGain)>>10))>>10;
                    MY_LOG("[%s()] m_i4SensorDev:%d NVRAM data is NULL\n", __FUNCTION__, m_eSensorDev);
                }
            } else {
                MY_LOG("[%s()] The AE algo class is NULL  i4SensorDev:%d line:%d", __FUNCTION__, m_eSensorDev, __LINE__);
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            MY_LOG("[%s:s] i4SensorDev:%d VDNum %d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d Smooth:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                    m_u4PreExposureTime,
                    m_u4PreSensorGain,
                    m_u4PreIspGain,
                    ...
                    m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableAESmoothGain);
...
#define max(a,b)  ((a) > (b) ? (a) : (b))
                        ...
                        w1 = 12;
                        CurTotalGain = (m_rAEOutput.rPreviewMode.u4Eposuretime * m_rAEOutput.rPreviewMode.u4AfeGain) /
                            m_u4PreExposureTime *
                            m_rAEOutput.rPreviewMode.u4IspGain /
                            m_u4PreSensorGain;
                    ...
                    if(m_u4SmoothIspGain != m_rAEOutput.rPreviewMode.u4IspGain) {
                        ...
                        m_u4PrevIspGain = m_u4SmoothIspGain;
...
//                        m_rAEOutput.rPreviewMode.u4IspGain);
...
#endif
                MY_LOG("[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d %d %d Flare Offset:%d", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                    ...
                    m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain);
        ...
        case AE_AF_STATE:
            ...
            if((m_rAEOutput.rPreviewMode.u4Eposuretime != m_rAEOutput.rAFMode.u4Eposuretime) || (m_rAEOutput.rPreviewMode.u4AfeGain != m_rAEOutput.rAFMode.u4AfeGain) ||
                (m_rAEOutput.rPreviewMode.u4IspGain != m_rAEOutput.rAFMode.u4IspGain)) {
                if(m_i4WaitVDNum == m_i4ShutterDelayFrames) {
                    m_bSetShutterValue = MTRUE;
                ...
                if(m_i4WaitVDNum >= m_i4IspGainDelayFrames) {
                    IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, MFALSE, m_rAEOutput.rAFMode.u4IspGain>>1);
                    IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, m_rAEOutput.rAFMode.i2FlareGain, (-1*m_rAEOutput.rAFMode.i2FlareOffset));
                    if(m_pIAeAlgo != NULL) {
                        m_pIAeAlgo->getAEInfoForISP(rAEInfo2ISP, LIB3A_SENSOR_MODE_PRVIEW);
                    } else {
                        MY_LOG("[%s()] The AE algo class is NULL  i4SensorDev = %d line:%d", __FUNCTION__, m_eSensorDev, __LINE__);
                    ...
                    m_u4PrevSensorGain = 0;
                    m_u4PrevIspGain = 0;
                    MY_LOG("[doAFAE] ISP Gain:%d Gamma:%d\n", m_rAEOutput.rAFMode.u4IspGain, rAEInfo2ISP.i4GammaIdx);
                    m_bAEStable = MTRUE;
                    prepareCapParams();
                }
                m_i4WaitVDNum ++;
        ...
        case AE_CAPTURE_STATE:
            if( (m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) &&  (mCaptureMode.u4Eposuretime > LONG_CAPTURE_THRES) && (ENABLE_LONG_CAPTURE_CTRL == MTRUE)) {
            ...
            IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, MFALSE, mCaptureMode.u4IspGain>>1);
            IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, mCaptureMode.i2FlareGain, (-1*mCaptureMode.i2FlareOffset));
        ...
        case AE_POST_CAPTURE_STATE:
            MY_LOG("[PostCapAE] SensorDev:%d Capture Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d m_i4GammaIdx:%d\n", m_eSensorDev, mCaptureMode.u4Eposuretime,
                mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain, mCaptureMode.u2FrameRate, mCaptureMode.i2FlareGain, mCaptureMode.i2FlareOffset, mCaptureMode.u4RealISO, m_i4GammaIdx);
        ...
        case AE_TWO_SHUTTER_INIT_STATE:
            ...
            IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, m_strIVHDROutputSetting.bEnableWorkaround, m_strIVHDROutputSetting.u4LEISPGain>>1);
            IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));
        ...
        case AE_TWO_SHUTTER_FRAME_STATE:
            ...
            } else if(m_i4WaitVDNum == m_i4IspGainDelayFrames) {   // restart
                IspTuningMgr::getInstance().setISPAEGain((ESensorDev_T)m_eSensorDev, m_strIVHDROutputSetting.bEnableWorkaround, m_strIVHDROutputSetting.u4LEISPGain>>1);
                IspTuningMgr::getInstance().setIspFlareGainOffset((ESensorDev_T)m_eSensorDev, m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));
            ...
            MY_LOG("[%s] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, iVHDR Output %d/%d/%d/%d Flare gain:%d offset:%d State:%d FrameRate:%d %d %d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                m_i4ShutterDelayFrames,
                m_i4SensorGainDelayFrames,
                m_i4IspGainDelayFrames,
                m_strIVHDROutputSetting.u4LEExpTimeInUS,
...
MRESULT AeMgr::updatePreviewParamsByiVHDR(strAEOutput *sAEInfo)
{
    ...
    MY_LOG("[%s] i4SensorDev:%d iVHDR 60Hz:%d Shutter:%d Gain:%d %d %d 1xGainISO:%d SatGain:%d Workaround:%d SE:%d %d %d LE:%d %d %d Ratio:%d\n", __FUNCTION__, m_eSensorDev,
    ...
    if(m_eAETargetMode == AE_MODE_MVHDR_TARGET) {   // mVHDR mode used
        ...
        MY_LOG("[%s] i4SensorDev:%d mVHDR SE:%d %d %d LE:%d %d %d Ratio:%d\n", __FUNCTION__, m_eSensorDev,
                    strIVHDROutputSetting.u4SEExpTimeInUS,
                    strIVHDROutputSetting.u4SESensorGain,
                    strIVHDROutputSetting.u4SEISPGain,
                    strIVHDROutputSetting.u4LEExpTimeInUS,
    ...
    MY_LOG("[%s] i4SensorDev:%d iVHDR workound:%d New SE:%d %d %d LE:%d %d %d Ratio:%d SGG1Gain:%d\n", __FUNCTION__, m_eSensorDev,
...
//MINT8 idx;
    ...
    if((m_bEnableAE == MTRUE) && (mCaptureMode.u4Eposuretime > LONG_CAPTURE_THRES) && (m_bMultiCap == MFALSE) && (ENABLE_LONG_CAPTURE_CTRL == MTRUE)) {
    ...
    MY_LOG("[%s()] i4SensorDev:%d i4CaptureDelayFrame:%d %d %d Shutter:%d MultiCap:%d Thres:%d Enable:%d\n", __FUNCTION__, m_eSensorDev, i4CaptureDelayFrame, m_i4SensorCaptureDelayFrame, 
    	m_i4TotalCaptureDelayFrame, mCaptureMode.u4Eposuretime, m_bMultiCap, LONG_CAPTURE_THRES, ENABLE_LONG_CAPTURE_CTRL);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AeMgr::getAEGammaIdx(MINT32 *i4AEGammaIdx)
{
    *i4AEGammaIdx = m_i4GammaIdx;
    MY_LOG("[%s()] i4SensorDev:%d m_i4GammaIdx:%d\n", __FUNCTION__, m_eSensorDev, m_i4GammaIdx);
    return S_AE_OK;    
}
