...
updateDMABaseAddr(MUINT32 u4BaseAddr)
{
    ...
    if (m_eSensorTG == ESensorTG_1) { // TG1
        ...
        if (MFALSE == sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE, 
                           NSImageio::NSIspio::EModule_AAO, (MINT32)&handle, (MINT32)(&("AAOBufMgr::DMAConfig()"))))
        {
            // Error Handling
        ...
        IOPIPE_SET_MODUL_REG(handle, CAM_AAO_STRIDE, (AAO_STRIDE_BUS_SIZE << 16));
         ...
         // get module handle
        if (MFALSE == sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE, 
                           NSImageio::NSIspio::EModule_AAO_D, (MINT32)&handle, (MINT32)(&("AAOBufMgr::DMAConfig()"))))
        {
            // Error Handling
            MY_ERR("EPIPECmd_GET_MODULE_HANDLE fail");
            goto lbExit;
...
lbExit:
    // release handle
    if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINT32)(&("AAOBufMgr::DMAConfig()")), MNULL))
    {            
        // Error Handling  
        MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE fail");
    }
