...
MBOOL AFOBufMgr::updateDMABaseAddr(MUINT32 u4BaseAddr)
{
    ...
    MINT32 handle,istwinmode=0;

	if (MFALSE ==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_TWIN_INFO, (MINT32)&istwinmode, -1,-1))
    ...
    if (m_eSensorTG == ESensorTG_1) 
    {
        if (MFALSE ==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE,
                                            ...
                                            (MINT32)&handle, (MINT32)(&("AFOBufMgr::DMAConfig()"))))
        {
    ...
    if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINT32)(&("AFOBufMgr::DMAConfig()")), MNULL))
    {            
        MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE fail");
    }
...
//=================================================================
    if (m_eSensorTG == ESensorTG_1) 
    {
        if (MFALSE ==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE,
                                            ...
                                            (MINT32)&handle, (MINT32)(&("AF::DMAConfig()")))
            )
        {
    ...
    if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINT32)(&("AF::DMAConfig()")), MNULL))
    {            
        MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE AF fail");
    }
    m_DMAInitDone=1;
