...
#define logW(fmt, ...)
#else
#define logI(fmt, arg...) XLOGD(fmt, ##arg)
#define logE(fmt, arg...) XLOGD("FlashError: func=%s line=%d: "fmt, __FUNCTION__, __LINE__, ##arg)
#define logW(fmt, arg...) XLOGD("FlashWarning: %5d: "fmt, __LINE__, ##arg)
#endif
...
logI("cctCalibration st=%d ln=%d",state, __LINE__);
	...
	if(state==STATE_INIT)
	{
		...
		//void DecideCalFlashComb(int CalNum, short *yTab, int totalStep, int totalStepLT, ChooseResult *pChoose, bool dualFlashEnable)
		ChooseResult choose[7];
		if(cust_isDualFlashSupport(m_sensorDev)==1)
		    g_flashQuickCalibrationNum=7;
        else
            g_flashQuickCalibrationNum=3;
...
#if 1
        logI("DecideCalFlashComb input:");
        logI("CaliNum =%d", g_flashQuickCalibrationNum);
        logI("duty=%d, dutyLt=%d, isDual=%d", dutyNum, prjPara.dutyNumLT, (int)cust_isDualFlashSupport(m_sensorDev));
#endif

        if(cust_isDualFlashSupport(m_sensorDev)==1)
		    err = mpFlashAlg->DecideCalFlashComb(g_flashQuickCalibrationNum, pNv->engTab.yTab, dutyNum, prjPara.dutyNumLT, choose, 1);
        else
            err = mpFlashAlg->DecideCalFlashComb(g_flashQuickCalibrationNum, pNv->engTab.yTab, dutyNum, prjPara.dutyNumLT, choose, 0);
		if(err != S_FLASH_OK)
		{
		    logE("DecideCalFlashComb error err=%d",err);
		...
		for (i = 0; i < g_flashQuickCalibrationNum; i++)
		{
			...
			logI("choose [%d] duty=%d dutyLt=%d",i,choose[i].duty, choose[i].dutyLT);
	...
	else if(state==STATE_END)
	{
		...
		for (i = 0; i < calTestN; i++)
		{
			for (j = 0; j < g_CaliData1.size(); j++)
			{
					...
					g_algCalData[i].dutyLT == g_CaliData1[j].dutyLT)
				{
					...
					double fac;
					fac = (double)expBase*afeBase*ispBase / g_CaliData1[j].exp / g_CaliData1[j].afe / g_CaliData1[j].isp;
	   ...
	   nv3A = getNv3ABuf();
		...
		int ret;
        {
            ...
            logI("InterpolateCalData input:");
            logI("isDual = %d\n", (int)cust_isDualFlashSupport(m_sensorDev));
            logI("CalNum = %d\n", calTestN);
            logI("dutyNum = %d\n", dutyNum);
            logI("dutyNumLt = %d\n", dutyNumLt);
            logI("Itab:");
        	...
        	for(i=0;i<dutyNum;i++)
        	{
        	    logI("[%d] yrgb= %d %d %d %d", i, (int)nv->engTab.yTab[i], nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain[i].i4R,
        	        nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain[i].i4G, nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain[i].i4B);
        	}
        ...
        if(cust_isDualFlashSupport(m_sensorDev)==1)
        {
		    ...
		    ret = mpFlashAlg->InterpolateCalData(calTestN, ITab1m, ITab2m, g_algCalData, nv->engTab.yTab, nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain,
			    dutyNum, dutyNumLt, awbTab, yTab, 1);
		}
		else
		{
		    ret = mpFlashAlg->InterpolateCalData(calTestN, ITab1m, ITab2m, g_algCalData, nv->engTab.yTab, nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain,
			    dutyNum, dutyNumLt, awbTab, yTab, 0);
        }
        logI("InterpolateCalData result=%d",ret);
		...
		//AWB_GAIN_T *pGoldenWBCalData, int totalStep, int totalStepLT, AWB_GAIN_T *outWB, short *outYTab, bool dualFlashEnable)
#ifdef WIN32
    		if(cust_isDualFlashSupport(m_sensorDev)==1)
    		{
