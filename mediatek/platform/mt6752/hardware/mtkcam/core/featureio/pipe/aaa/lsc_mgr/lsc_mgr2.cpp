...
LscMgr2* 
LscMgr2::
createInstance(ESensorDev_T const eSensorDev, MINT32 i4SensorIdx, ESensorTG_T eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    case ESensorDev_Main:       //  Main Sensor
        static LscMgr2 singleton_main(ESensorDev_Main, i4SensorIdx, eSensorTG, rIspNvram);
        _pLscMain = &singleton_main;
        LSC_LOG_END("ESensorDev_Main(0x%08x), i4SensorIdx(%d), eSensorTG(%d)\n", _pLscMain, i4SensorIdx, eSensorTG);
        return _pLscMain;
    case ESensorDev_MainSecond: //  Main Second Sensor
        static LscMgr2 singleton_mainsecond(ESensorDev_MainSecond, i4SensorIdx, eSensorTG, rIspNvram);
        _pLscMain2 = &singleton_mainsecond;
        LSC_LOG_END("ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d), eSensorTG(%d)\n", _pLscMain2, i4SensorIdx, eSensorTG);
        return _pLscMain2;
    case ESensorDev_Sub:        //  Sub Sensor
        static LscMgr2 singleton_sub(ESensorDev_Sub, i4SensorIdx, eSensorTG, rIspNvram);
        _pLscSub = &singleton_sub;
        LSC_LOG_END("ESensorDev_Sub(0x%08x), i4SensorIdx(%d), eSensorTG(%d)\n", _pLscSub, i4SensorIdx, eSensorTG);
        return _pLscSub;
    }
}
...
LscMgr2::
LscMgr2(ESensorDev_T eSensorDev, MINT32 i4SensorIdx, ESensorTG_T eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
...
loadTableFlow(MBOOL fgForceRestore)
{
    ...
    if (!check123InNvram())
    {
        ...
        else
        {
            ...
            {
            case E_LSC_NO_OTP:
                if (isLscTransformFromCapture(m_eSensorDev))
                {
...
{   
    ...
    LSC_LOG("m_prShadingLut(0x%08x)", m_prShadingLut);

    m_prNvram3A = pNvram3A;
    LSC_LOG("m_prNvram3A(0x%08x)", m_prNvram3A);
...
getTsfCfgTbl(void)
{
    ...
    if (fgLoadOK)
    {
        ...
        LSC_LOG("Load TSF table OK, TSF(%d), CtIdx(%d), data(0x%08x), para(0x%08x)",
    ...
    else
    {
        ...
        LSC_ERR("Load TSF table Fail, use default: TSF(%d), CtIdx(%d), data(0x%08x), para(0x%08x)",
            m_rTsfCfgTbl.TSF_CFG.isTsfEn, m_rTsfCfgTbl.TSF_CFG.tsfCtIdx,
            m_rTsfCfgTbl.TSF_DATA,
            m_rTsfCfgTbl.TSF_PARA);
    }
...
LscMgr2::restoreNvram(void)
{
    ...
    {
        LSC_LOG("Read deafault OK, restore shading NVRAM, m_prShadingLut(0x%08x)",
            m_prShadingLut);
...
LscMgr2::readNvramTbl(MBOOL fgForce)
{
    ...
    {
        LSC_LOG("m_prShadingLut(0x%08x) <= prShadingLut(0x%08x)", m_prShadingLut, prShadingLut);
...
shadingTblAlign(const ShadingTblAlignCfg_T& rAlignCfg, MUINT32* pu4OutputCoefTbl)
{
    ...
    {
        LSC_LOG("gWorkinBuffer(0x%08x)", gWorkinBuffer);
    }
...
RawLscTblInit()
{
    ...
    if (!m_pIMemDrv)
    {
        ...
        {
            LSC_LOG("m_pIMemDrv(0x%08x) create OK\n", (MUINT32) m_pIMemDrv);
            ret = m_pIMemDrv->init();
            if (ret == MTRUE)
            {
    ...
    {
        LSC_LOG("m_pIMemDrv(0x%08x) already exists.\n", (MUINT32) m_pIMemDrv);
    }
...
allocMemBuf(IMEM_BUF_INFO& rBufInfo, MUINT32 const u4Size)
{
    ...
    if (!rBufInfo.virtAddr)
    {
        ...
        if (0 == m_pIMemDrv->allocVirtBuf(&rBufInfo))
        {
            ...
            {
                LSC_ERR("mapPhyAddr error, virtAddr(0x%08x), size(%d)\n", rBufInfo.virtAddr, rBufInfo.size);
                ret = MFALSE;
            }
            else
            {
    ...
    {
        ret = MTRUE;
        LSC_LOG("Already Exist! virtAddr(0x%8x), size(%d)\n", rBufInfo.virtAddr, u4Size);
    }
    return ret;
}
...
freeMemBuf(IMEM_BUF_INFO& rBufInfo)
{
    ...
    if (0 == m_pIMemDrv->unmapPhyAddr(&rBufInfo))
    {
        ...
        {
            LSC_LOG("freeVirtBuf OK, memID(%d), virtAddr(0x%08x), phyAddr(0x%08x)\n", rBufInfo.memID, rBufInfo.virtAddr, rBufInfo.phyAddr);
            rBufInfo.virtAddr = 0;
            ret = MTRUE;
        }
        else
        {
            LSC_LOG("freeVirtBuf Fail, memID(%d), virtAddr(0x%08x), phyAddr(0x%08x)\n", rBufInfo.memID, rBufInfo.virtAddr, rBufInfo.phyAddr);
            ret = MFALSE;
        }
    }
    else
...
{
    LSC_LOG("memID(%d), virtAddr(0x%08x), phyAddr(0x%08x), size(%d)\n",
...
copyLutToMemBuf(IMEM_BUF_INFO& rBufInfo, const MUINT8* pLut, MUINT32 u4Size)
{
    ...
    {
        LSC_LOG("OK: virtAddr(0x%08x) <- pLut(0x%08x), size(%d)\n", pDest, pLut, u4Size);
        ::memcpy(pDest, pLut, u4Size);
        m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH, &rBufInfo);
    }
    else
    {
        LSC_ERR("Fail: virtAddr(0x%08x) <- pLut(0x%08x), size(%d)\n", pDest, pLut, u4Size);
    }
}
...
doShadingAlign(MVOID* pMtkLsc)
{
    ...
    {    
        ...
        // just align for capture
        {
            ...
            rAlign.u1CoefXNum = m_rIspLscCfg[eLscScn].ctl2.bits.LSC_SDBLK_XNUM + 2;
            rAlign.u1CoefYNum = m_rIspLscCfg[eLscScn].ctl3.bits.LSC_SDBLK_YNUM + 2;
            for (MUINT32 i = 0; i < SHADING_SUPPORT_CT_NUM; i++)
            {
                ...
                // NVRAM as input
                MUINT32 u4Addr = ((MUINT32) getLut(eLscScn)) + i*u4Size;
                ::memcpy(pLut, (MUINT8*)u4Addr, u4Size);
                rAlign.pu4CoefTbl = (MUINT32*)pLut;
                // IMEM as output
                MUINT32 u4Output = stRawLscInfo[eLscScn].virtAddr + i*u4Size;
                if (0 == shadingTblAlign(rAlign, (MUINT32*)u4Output))
                {
                    LSC_LOG("Align OK: eLscScn(%d), CT(%d), Grid(%d x %d), Input(0x%08x), Output(0x%08x)",
                        eLscScn, i, rAlign.u1CoefXNum, rAlign.u1CoefYNum, pLut, u4Output);
                }
                else
                {
                    LSC_ERR("Align NG: eLscScn(%d), CT(%d), Grid(%d x %d), Input(0x%08x), Output(0x%08x)",
                        eLscScn, i, rAlign.u1CoefXNum, rAlign.u1CoefYNum, pLut, u4Output);
                }
...
doShadingTrfm()
{
    ...
    for (MINT32 i4Scn = 0; i4Scn < (MINT32)m_u4NumSensorModes; i4Scn++)
    {
        ...
        rShadingInput.u1CoefXNum    = m_rIspLscCfg[LSC_SCENARIO_CAP].ctl2.bits.LSC_SDBLK_XNUM + 2;
        rShadingInput.u1CoefYNum    = m_rIspLscCfg[LSC_SCENARIO_CAP].ctl3.bits.LSC_SDBLK_YNUM + 2;
        ...
        {
            // NVRAM as input
            MUINT32 u4Addr = stRawLscInfo[LSC_SCENARIO_CAP].virtAddr + i*getPerLutSize(LSC_SCENARIO_CAP);
            // IMEM as output
            MUINT32 u4Output = stRawLscInfo[eLscScn].virtAddr + i*getPerLutSize(eLscScn);
            rShadingInput.pu4CoefTbl = (MUINT32*) u4Addr;
            if (0 == shadingTblCrop(rShadingCrop, rShadingInput, (MUINT32*)u4Output))
            {
                LSC_LOG("Crop OK: eLscScn(%d), CT(%d), Grid(%dx%d), Input(0x%08x) => (17x17), Output(0x%08x)",
                    eLscScn, i, rShadingInput.u1CoefXNum, rShadingInput.u1CoefYNum, u4Addr, u4Output);
            }
            else
            {
                LSC_ERR("Crop NG: eLscScn(%d), CT(%d), Grid(%dx%d), Input(0x%08x) => (17x17), Output(0x%08x)",
                    eLscScn, i, rShadingInput.u1CoefXNum, rShadingInput.u1CoefYNum, u4Addr, u4Output);
                return MFALSE;
            }
        }
...
MBOOL
LscMgr2::
do123LutToSysram(MVOID* pMtkLsc)
{
    LSC_LOG_BEGIN("MTK 1-to-3");
...
//        for (MINT32 i4Scn = 0; i4Scn < LSC_SCENARIO_NUM; i4Scn++)
        ...
        for (MINT32 i4Scn = 0; i4Scn < (MINT32)m_u4NumSensorModes; i4Scn++)
        {
            ...
            for (MUINT32 i = 0; i < SHADING_SUPPORT_CT_NUM; i++)
            {
                ...
                if (0 == shadingTblCrop(rShadingCrop, rShadingInput, (MUINT32*)u4Output))
                {
                    LSC_LOG("Crop OK: eLscScn(%d), CT(%d), Grid(%d x %d), Input(0x%08x), Output(0x%08x)",
                        eLscScn, i, rAlign.u1CoefXNum, rAlign.u1CoefYNum, u4Addr, u4Output);
                }
                else
                {
                    LSC_ERR("Crop NG: eLscScn(%d), CT(%d), Grid(%d x %d), Input(0x%08x), Output(0x%08x)",
                        eLscScn, i, rAlign.u1CoefXNum, rAlign.u1CoefYNum, u4Addr, u4Output);
                }
            }
        ...
        if (!isByp123ToNvram())
        {
            ...
            // write unit gain table to NVRAM
            m_prShadingLut->SensorGoldenCalTable.IspLSCReg[1] = _pMtkLsc->CapIspReg[1];
            m_prShadingLut->SensorGoldenCalTable.IspLSCReg[3] = _pMtkLsc->CapIspReg[3];
            m_prShadingLut->SensorGoldenCalTable.IspLSCReg[4] = _pMtkLsc->CapIspReg[4];
            m_prShadingLut->SensorGoldenCalTable.TblSize = u4GainTblSize;
            ::memcpy((void*)m_prShadingLut->SensorGoldenCalTable.UnitGainTable,
                    (void*)_pMtkLsc->CapTable, u4GainTblSize);
            // flush to NVRAM
            if (0 != saveToNVRAM())
            {
                LSC_ERR("Fail to write NVRAM");
...
importEEPromData()
{
    ...
    LSC_LOG("u4Rot(%d), pLscData(0x%08x)", u4Rot, pLscData);
...
calLscConfig(const SensorCropInfo_T& rCrop, MUINT8 u1XNum, MUINT8 u1YNum, ISP_NVRAM_LSC_T& rLscCfg)
{
    ...
    rLscCfg.ctl2.bits.LSC_SDBLK_XNUM = u1XNum;
    rLscCfg.ctl3.bits.LSC_SDBLK_YNUM = u1YNum;
    rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH = u4BlkW;   
    rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT = u4BlkH;
    rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH = u4LastW;
    rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT = u4LastH;
...
//    rLscCfg.gain_th.val = 0x03F000000;
    ...
    LSC_LOG("XNum(%d)/YNum(%d)/W(%d)/H(%d)/LW(%d)/LH(%d)/OffX(%d)/OffY(%d)",
            rLscCfg.ctl2.bits.LSC_SDBLK_XNUM,
            rLscCfg.ctl3.bits.LSC_SDBLK_YNUM,
            rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH,
            rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT,
            rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH,
            rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT,
...
calSl2Config(const SensorCropInfo_T& rCrop, ISP_NVRAM_SL2_T& rSl2Cfg)
{
    ...
    rSl2Cfg.cen.bits.SL2_CENTR_X = u4Rx;
    rSl2Cfg.cen.bits.SL2_CENTR_Y = u4Ry;
...
setSensorMode(ESensorMode_T eSensorMode, MBOOL fgForce)
{
    ...
    if (eSensorMode != ePrevSensorMode || m_fgInit == MFALSE || fgForce)
    {
        ...
        calLscConfig(
            rCrop,
            m_rIspLscCfg[eLscScn].ctl2.bits.LSC_SDBLK_XNUM,
            m_rIspLscCfg[eLscScn].ctl3.bits.LSC_SDBLK_YNUM,
            m_rLscCfg);
...
updateLsc()
{
    ...
    MUINT32 u4PhyAddr, u4VirAddr, u4CTIdx;
    ...
    if (fgUserTbl)
    {
        ...
        u4VirAddr = m_rBufInfo[m_u4DoubleBufIdx].virtAddr;
    }
    else
    {
        MUINT32 u4Ofst = getPerLutSize(m_eLscScenario) * u4CTIdx;
        u4PhyAddr = stRawLscInfo[m_eLscScenario].phyAddr + u4Ofst;
        u4VirAddr = stRawLscInfo[m_eLscScenario].virtAddr + u4Ofst;
    }
    // update virtual address
    m_u4BaseAddr = u4VirAddr;

    // update ratio
    MUINT32 u4Rto = MAX(0, MIN(32, m_i4Rto));
    ...
    // put to isp_mgr buf
    ISP_MGR_LSC_T::getInstance(m_eSensorDev, m_eSensorTG).enableLsc(getOnOff());
    ISP_MGR_LSC_T::getInstance(m_eSensorDev, m_eSensorTG).putAddr(u4PhyAddr);
    ISP_MGR_LSC_T::getInstance(m_eSensorDev, m_eSensorTG).put(m_rLscCfg);
