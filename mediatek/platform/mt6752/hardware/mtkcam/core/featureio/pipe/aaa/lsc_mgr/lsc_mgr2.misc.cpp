...
dumpSdblk(const char* table_name, const ISP_NVRAM_LSC_T& LscConfig, const MUINT32 *ShadingTbl)
{
    ...
    LSC_LOG_BEGIN("ShadingTbl(0x%08x)", ShadingTbl);
...
getDebugInfo(SHADING_DEBUG_INFO_T &rShadingDbgInfo)
{
    ...
    if (m_pTsfResultInfo)
    {
        ...
        LSC_LOG("pTsfExif(0x%08x), ExifData(0x%08x)", (MUINT32)pTsfExif, (MUINT32)&m_pTsfResultInfo->ExifData[0]);
    }
    ...
    if (m_bDumpSdblk)
    {
        if (dumpSdblk(m_strSdblkFile.c_str(), debug, reinterpret_cast<MUINT32*>(m_u4BaseAddr)) != 0)
...
getDebugTbl(DEBUG_SHAD_ARRAY_INFO_T &rShadingDbgTbl, DEBUG_SHAD_ARRAY_2_T& rShadRestTbl)
{
    ...
    MUINT32* pu4Addr = reinterpret_cast<MUINT32*>(m_u4BaseAddr);
