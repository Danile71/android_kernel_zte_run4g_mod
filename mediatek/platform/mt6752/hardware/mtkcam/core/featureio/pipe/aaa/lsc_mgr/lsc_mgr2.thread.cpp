...
updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce)
{
    ...
    MBOOL fgBlkChg = 
        (m_rTsfLscParam.x_grid_num != rLscCfg.ctl2.bits.LSC_SDBLK_XNUM+2)||
        (m_rTsfLscParam.y_grid_num != rLscCfg.ctl3.bits.LSC_SDBLK_YNUM+2);
    MBOOL fgAspectChg = 
    ...
    m_rTsfLscParam.block_wd      = rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH;
    m_rTsfLscParam.block_ht      = rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT;
    m_rTsfLscParam.x_grid_num    = rLscCfg.ctl2.bits.LSC_SDBLK_XNUM+2;
    m_rTsfLscParam.y_grid_num    = rLscCfg.ctl3.bits.LSC_SDBLK_YNUM+2;
    m_rTsfLscParam.block_wd_last = rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH;
    m_rTsfLscParam.block_ht_last = rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT;
...
initThreadFunc()
{
    ...
    else
    {
        ...
        LSC_LOG("AWB stat alloc size(%d), tmp(0x%08x), algo(0x%08x)", m_rTsfRefInfo.u4SizeAwbStat, m_rTsfRefInfo.prAwbStat, m_prAwbStat);
...
tsfResetTbl(ELscScenario_T eLscScn)
{
    ...
    LSC_LOG("eLscScn(%d), m_pu4ProcTbl(0x%08x), m_pu4ResultTbl(0x%08x), src(0x%08x)", eLscScn, m_pu4ProcTbl, m_pu4ResultTbl, pSrc);
...
tsfSetProcInfo(MTK_TSF_SET_PROC_INFO_STRUCT& rProcInfo, MBOOL fgDump, const char* filename)
{
    ...
    MY_LOG_IF(fgLogEn, "[%s] m_prAwbStat(0x%08x), tmpBuf(0x%08x), size(%d)",
...
#define UNIT_GAIN_FROM_NVRAM 0
    ...
    LSC_LOG("Golden(0x%08x), Unit(0x%08x)", rTsfGainGolden.Tbl, rTsfGainUnit.Tbl);
...
tsfInit()
{
    ...
    m_rTsfEnvInfo.EnableORCorrection = 1;
    // AWB NVRAM
    m_rTsfEnvInfo.AwbNvramInfo.rUnitGain.i4R   = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rUnitGain.i4R; // 512;
    m_rTsfEnvInfo.AwbNvramInfo.rUnitGain.i4G   = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rUnitGain.i4G; // 512;
    m_rTsfEnvInfo.AwbNvramInfo.rUnitGain.i4B   = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rUnitGain.i4B; // 512;
    m_rTsfEnvInfo.AwbNvramInfo.rGoldenGain.i4R = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rGoldenGain.i4R; //512;
    m_rTsfEnvInfo.AwbNvramInfo.rGoldenGain.i4G = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rGoldenGain.i4G; //512;
    m_rTsfEnvInfo.AwbNvramInfo.rGoldenGain.i4B = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rCalData.rGoldenGain.i4B; //512;
    m_rTsfEnvInfo.AwbNvramInfo.rD65Gain.i4R    = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rAlgoTuningParam.rAWBGain_LSC.i4R; //809;
    m_rTsfEnvInfo.AwbNvramInfo.rD65Gain.i4G    = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rAlgoTuningParam.rAWBGain_LSC.i4G; //512;
    m_rTsfEnvInfo.AwbNvramInfo.rD65Gain.i4B    = m_prNvram3A->rAWBNVRAM[AWB_NVRAM_IDX_NORMAL].rAlgoTuningParam.rAWBGain_LSC.i4B; //608;
    ...
    LSC_LOG("ImgWidth(%d), ImgHeight(%d), BayerOrder(%d), ImgAddr(0x%08x), ShadingTbl(0x%08x), Raw16_9Mode(%d), EnableSL2(%d), pLscConfig(0x%08x)",
...
tsfBatchCap()
{
    ...
    if (fgDumpEn)
    {
        ...
        rLscCfg.ctl2.bits.LSC_SDBLK_WIDTH       = m_rTsfLscParam.block_wd;
        rLscCfg.ctl3.bits.LSC_SDBLK_HEIGHT      = m_rTsfLscParam.block_ht;
        rLscCfg.ctl2.bits.LSC_SDBLK_XNUM        = m_rTsfLscParam.x_grid_num - 2;
        rLscCfg.ctl3.bits.LSC_SDBLK_YNUM        = m_rTsfLscParam.y_grid_num - 2;
        rLscCfg.lblock.bits.LSC_SDBLK_lWIDTH    = m_rTsfLscParam.block_wd_last;
        rLscCfg.lblock.bits.LSC_SDBLK_lHEIGHT   = m_rTsfLscParam.block_ht_last;
...
tsfRun()
{
    ...
    if (m_u4PerFrameStep == 0)
    {
        ...
        m_rTsfSetProc.LscRA = m_i4RtoFor1stFrm;
        ret = m_prTsf->TsfFeatureCtrl(MTKTSF_FEATURE_SET_PROC_INFO, &m_rTsfSetProc, NULL);
    }
