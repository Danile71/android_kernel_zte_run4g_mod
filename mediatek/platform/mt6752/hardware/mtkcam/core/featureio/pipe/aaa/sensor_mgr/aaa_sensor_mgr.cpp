...
uninit()
{
    ...
    if (m_Users == 0) { // There is no more User after decrease one User
        if(m_pHalSensorObj) {
            m_pHalSensorObj->destroyInstance();
...
//    MY_LOG_IF(m_bDebugEnable,"[getSensorVCinfo]  \n");
    ...
    if(i4SensorDev == ESensorDev_Main) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_GET_SENSOR_VC_INFO, (MINT32)&rSensorVCInfo, (MINT32)&i4SensorSenraio, 0);
    } else if(i4SensorDev == ESensorDev_Sub) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_GET_SENSOR_VC_INFO, (MINT32)&rSensorVCInfo, (MINT32)&i4SensorSenraio, 0);
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_GET_SENSOR_VC_INFO, (MINT32)&rSensorVCInfo, (MINT32)&i4SensorSenraio, 0);
    } else {
...
setSensorExpTime(MINT32 i4SensorDev, MUINT32 a_u4ExpTime)
{
    ...
    // Set exposure time
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MINT32)&a_u4ExpTime, 0, 0);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MINT32)&a_u4ExpTime, 0, 0);            
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MINT32)&a_u4ExpTime, 0, 0);            
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
setSensorGain(MINT32 i4SensorDev, MUINT32 a_u4SensorGain)
{
    ...
    // Set sensor gain
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_GAIN, (MINT32)&a_u4SensorGain, 0, 0);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_GAIN, (MINT32)&a_u4SensorGain, 0, 0);            
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_GAIN, (MINT32)&a_u4SensorGain, 0, 0);            
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
setSensorFrameRate(MINT32 i4SensorDev, MUINT32 a_u4SensorFrameRate)
{
    ...
    u4SensorFrameRate = a_u4SensorFrameRate;    // 10 base frame rate from AE
    ...
    if(u4SensorFrameRate == m_u4SensorFrameRate){
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_VIDEO_FRAME_RATE, (MINT32)&u4SensorFrameRate, 0, 0);            
        } else if(i4SensorDev == ESensorDev_Sub) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_VIDEO_FRAME_RATE, (MINT32)&u4SensorFrameRate, 0, 0);            
        } else if(i4SensorDev == ESensorDev_MainSecond) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_VIDEO_FRAME_RATE, (MINT32)&u4SensorFrameRate, 0, 0);            
        } else {
            MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
            return E_AAA_SENSOR_NULL;    
        }
...
setSensorExpLine(MINT32 i4SensorDev, MUINT32 a_u4ExpLine)
{
    ...
    // Set exposure line
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_EXP_LINE, (MINT32)&a_u4ExpLine, 0, 0);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_EXP_LINE, (MINT32)&a_u4ExpLine, 0, 0);            
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_EXP_LINE, (MINT32)&a_u4ExpLine, 0, 0);            
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
setFlickerFrameRateActive(MINT32 i4SensorDev, MBOOL a_bFlickerFPSAvtive)
{
    ...
    if(a_bFlickerFPSAvtive == m_bFlickerState){
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_FLICKER_FRAME_RATE, (MINT32)&u4FlickerInfo, 0, 0);            
        } else if(i4SensorDev == ESensorDev_Sub) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_FLICKER_FRAME_RATE, (MINT32)&u4FlickerInfo, 0, 0);            
        } else if(i4SensorDev == ESensorDev_MainSecond) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_FLICKER_FRAME_RATE, (MINT32)&u4FlickerInfo, 0, 0);            
        } else {
            MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
            return E_AAA_SENSOR_NULL;    
        }
...
MRESULT AAASensorMgr::setPreviewMaxFrameRate(MINT32 i4SensorDev, MUINT32 frameRate, MUINT32 u4SensorMode)
{
    ...
    // Set max frame rate
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frameRate, 0);          
        } else if(i4SensorDev == ESensorDev_Sub) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frameRate, 0);          
        } else if(i4SensorDev == ESensorDev_MainSecond) {
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frameRate, 0);          
        } else {
            MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
            return E_AAA_SENSOR_NULL;    
        }
...
MUINT32 AAASensorMgr::getPreviewDefaultFrameRate(MINT32 i4SensorDev, MUINT32 u4SensorMode)
{
    ...
    // Get default frame rate
    if(i4SensorDev == ESensorDev_Main) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frmRate, 0);
    } else if(i4SensorDev == ESensorDev_Sub) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frmRate, 0);
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MINT32)&u4SensorMode, (MINT32)&frmRate, 0);
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
set2ShutterParams(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime, MUINT32 a_u4SensorGain)
{
    ...
    // Set exposure time
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER_GAIN, (MINT32)&a_u4LEExpTime, (MINT32)&a_u4SEExpTime, (MINT32)&a_u4SensorGain);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER_GAIN, (MINT32)&a_u4LEExpTime, (MINT32)&a_u4SEExpTime, (MINT32)&a_u4SensorGain);            
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER_GAIN, (MINT32)&a_u4LEExpTime, (MINT32)&a_u4SEExpTime, (MINT32)&a_u4SensorGain);            
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
...
setSensorAWBGain(MINT32 i4SensorDev, strSensorAWBGain *strSensorAWBInfo)
{
    ...
    // Set exposure time
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_HDR_AWB_GAIN, (MUINT32)strSensorAWBInfo, 0, 0);            
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_HDR_AWB_GAIN, (MUINT32)strSensorAWBInfo, 0, 0);                     
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_HDR_AWB_GAIN, (MUINT32)strSensorAWBInfo, 0, 0);                   
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;    
    }
