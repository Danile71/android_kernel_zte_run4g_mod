...
IState(char const*const pcszName, MINT32 sensorDevId, StateMgr* pStateMgr)
    : m_pHal3A(Hal3A::getInstance(sensorDevId))
    , m_pcszName(pcszName)
...
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    // AWB init
    if (!IAwbMgr::getInstance().camcorderPreviewInit(m_SensorDevId, i4SensorIdx, rParam)) {
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    // AE init
    err = IAeMgr::getInstance().camcorderPreviewInit(m_SensorDevId, i4SensorIdx, rParam);
    if (FAILED(err)) {
        MY_ERR("IAeMgr::getInstance().camcorderPreviewInit() fail\n");
        return err;
    }
}
if (ENABLE_AF & m_pHal3A->m_3ACtrlEnable) {
...
if (ENABLE_FLASH & m_pHal3A->m_3ACtrlEnable) {
    // Flash init + videoPreviewStart
    if (!FlashMgr::getInstance().init(m_SensorDevId, i4SensorIdx)) {
        MY_ERR("FlashMgr::getInstance().init() fail\n");
        return E_3A_ERR;
    }
    if (!FlashMgr::getInstance().videoPreviewStart(m_SensorDevId)) {
        MY_ERR("FlashMgr::getInstance().videoPreviewStart() fail\n");
        return E_3A_ERR;
    }
}
if (ENABLE_FLICKER & m_pHal3A->m_3ACtrlEnable) {
    MY_LOG("FlickerHalBase sensor idnex: %d", i4SensorIdx);
    AAASensorMgr::getInstance().setFlickerFrameRateActive(m_SensorDevId, 1);
    FlickerHalBase::getInstance().open(m_SensorDevId, i4SensorIdx, m_pHal3A->getTGInfo());
}
}
    ...
    m_pStateMgr->transitState(eState_Init, eState_CamcorderPreview);
