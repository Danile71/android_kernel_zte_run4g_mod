...
MRESULT StateAF::exitPreview()
{
    ...
    // AE uninit
    IAeMgr::getInstance().uninit(m_SensorDevId);

    // AWB uninit
    IAwbMgr::getInstance().uninit(m_SensorDevId);

    // AF uninit
    IAfMgr::getInstance().uninit(m_SensorDevId);


    // Flash uninit
    FlashMgr::getInstance().uninit(m_SensorDevId);
    ...
    err = IAFOBufMgr::getInstance().AFStatEnable(1/*sensordev*/, MFALSE);
    if (FAILED(err)) {
        MY_ERR("IAFOBufMgr::getInstance().AFStatEnable() fail\n");
        return err;
    }

    err = IAFOBufMgr::getInstance().DMAUninit(1/*sensordev*/);
    if (FAILED(err)) {
        MY_ERR("IAFOBufMgr::getInstance().DMAunInit() fail\n");
        return err;
    }
