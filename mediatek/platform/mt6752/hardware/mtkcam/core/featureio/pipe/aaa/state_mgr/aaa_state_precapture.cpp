...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    IAeMgr::getInstance().doPreCapAE(m_SensorDevId, m_pStateMgr->getFrameCount(), bIsStrobeFired, reinterpret_cast<MVOID *>(pBuf->virtAddr), m_pScheduler->jobAssignAndTimerStart(E_Job_AeFlare), MTRUE);
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AF_BeforeFlash>)
{
    ...
    if(CUST_ENABLE_PRECAPTURE_AF() && !CUST_PRECAPTURE_AF_AFTER_PREFLASH())
    {
        ...
        if(isDoAF==1 && hasHw==1)
        {
            XLOGD("[sendPrecapIntent]isDoAF=1 line=%d",__LINE__);
        ...
        {
            XLOGD("[sendPrecapIntent]isDoAF=0 || no flash hw");
        }
    }
    else
    {
