...
MVOID
SwNRParam::
setParam(MUINT32 const iso, MBOOL const isMfll)
{
    MY_LOGD("iso = %d, mfll %d", iso, isMfll);
    mIso = iso;
    mbMfll = isMfll;
}
...
#define CHECK_NR_RET( NR_ret )                     \
    ...
    //
    {
        char core_no[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("debug.swnr.coreno", core_no, "0");
        MUINT32 u4CoreNo = atoi(core_no);
        if (u4CoreNo)
        {
            mpNRInitInfo->CoreNumber = u4CoreNo;
            MY_LOGD("overwrite core number (%d)", u4CoreNo);
        }
    }
    //    
