...
//**********************************************************************//        
    ...
    FDFTInitInfo.WorkingBufAddr = (unsigned char*)WorkingBuffer;
...
MUINT32 ImageBuffer2, //Source Buffer (Y)
MBOOL   SDEnable,
MINT32 rRotation_Info,
MUINT32 ImageBufferPhy    //Source Buffer Physical
)
{
    ...
    if(EnableSWResizerFlag)
    {
        ...
        MY_LOGD("FDVTMain IN");
        m_pMTKFDVTObj->FDVTMain( ImageScaleBuffer, (MUINT8*)ImageBuffer1, FDVT_GFD_MODE, direction, 0, (MUINT8*)ImageBufferPhy);
        /*
        FILE* fp;
        char filename[200];
        static int count = 0;
        ...
        */
        ...
        if(SDEnable)
        {
            ...
            m_pMTKFDVTObj->FDVTMain( ImageScaleBuffer, y_vga, FDVT_SD_MODE, direction, 0, (MUINT8*)ImageBufferPhy);
			MY_LOGD("FDVT_SD_MODE out");
        }
    }
    else
    {
    		m_pMTKFDVTObj->FDVTMain( (MUINT8*)ScaleImages, (MUINT8*)ImageBuffer1, FDVT_GFD_MODE, direction, 0, (MUINT8*)ImageBufferPhy);
    		
    		if(SDEnable)
    		{
    		    m_pMTKFDVTObj->FDVTMain( (MUINT8*)ScaleImages, (MUINT8*)ImageBuffer2, FDVT_SD_MODE, direction, 0, (MUINT8*)ImageBufferPhy);
    	  }
    }
...
)
{
    ...
    faceCnt=m_pMTKFDVTObj->FDVTGetResult( pbuf, FACEDETECT_TRACKING_DISPLAY);
    
    m_DispW = image_width_array[0];
    m_DispH = image_height_array[0];;
    
	MY_LOGD("[%s]m_DispW(%d) m_DispH(%d)", __FUNCTION__, m_DispW, m_DispH);
    m_pMTKFDVTObj->FDVTGetICSResult(  (MUINT8*)fd_result,  pbuf, m_DispW, m_DispH, g_Lcm_rotate, g_Sensor_rotate, g_CameraTYPE, DrawMode);
