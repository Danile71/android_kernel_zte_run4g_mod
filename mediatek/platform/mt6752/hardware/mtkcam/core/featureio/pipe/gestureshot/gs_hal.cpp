...
MINT32
halGS::halGSDo(
MUINT32 ImageBuffer2, //Preview Size Image
...
{	
    ...
    {        
        ...
        m_pMTKGSObj->GdMain( (MUINT32)ImageScaleBuffer, MTK_GD_GFD_MODE, direction, 0);
        
        //result  DetectResult[MAX_FACE_NUM];
        GD_RESULT DetectResult[MAX_FACE_NUM];
        m_pMTKGSObj->GdGetResult( (MUINT32)&DetectResult, GD_TRACKING_DISPLAY);
        ...
        if( last_face_num == 1 && DetectResult->face_num ==0 )
        {
            ...
            for(int i=0; i<2;i++)
            {
                m_pMTKGSObj->GdMain( (MUINT32)ImageScaleBuffer, MTK_GD_GFD_MODE, direction, i);
                m_pMTKGSObj->GdGetResult( (MUINT32)DetectResult, GD_TRACKING_DISPLAY);
...
)
{
    ...
    faceCnt=m_pMTKGSObj->GdGetResult((MUINT32) pbuf, GD_TRACKING_DISPLAY);
    
    m_DispW = image_width_array[0];
    m_DispH = image_height_array[0];;
    
    m_pMTKGSObj->GdGetICSResult((MUINT32) GS_result, (MUINT32) pbuf, m_DispW, m_DispH, 0, 0, 0, 0);
