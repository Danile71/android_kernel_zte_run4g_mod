...
#define PIP_DBG(fmt, arg...)    XLOGD("[%s]" fmt, __func__, ##arg)
#define PIP_INF(fmt, arg...)    XLOGI("[%s]" fmt, __func__, ##arg)
#define PIP_WRN(fmt, arg...)    XLOGW("[%s] WRN(%5d):" fmt, __func__, __LINE__, ##arg)
#define PIP_ERR(fmt, arg...)    XLOGE("[%s] %s ERROR(%5d):" fmt, __func__,__FILE__, __LINE__, ##arg)
...
MBOOL PipHalImp::GetHwMaxFrameRate(MINT32 i4SensorDev, MUINT32 &ZsdHwMaxFrameRate, MUINT32 &NonZsdHwMaxFrameRate)
{
    ...
    MINT32 i4SensorSize = i4SensorFullWidth * i4SensorFullHeight;
    // Get RAW or YUV
    ...
    if (i4SensorType == NSCam::SENSOR_TYPE_RAW) // RAW sensor.
    {
        ...
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_16M)   // SensorSize > 16M. K2 PIP only support 16M+5M. So main > 16M will return 0 (indicate error).
            {
                ZsdHwMaxFrameRate    = 0;    // Not support this size, so frame rate return 0.
                NonZsdHwMaxFrameRate = 0;    // Not support this size, so frame rate return 0.
            }
            else if (i4SensorSize > PIP_SENSOR_SIZE_13M)   // 16M >= SensorSize > 13M
            {
                ZsdHwMaxFrameRate    = 20;
                NonZsdHwMaxFrameRate = 30;
            }
            else    // 13M >= SensorSize
            {
                ZsdHwMaxFrameRate    = 24;
                NonZsdHwMaxFrameRate = 30;
            }
        }
        else if (i4SensorDev == SENSOR_DEV_SUB) // Sub Cam.
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_5M)   // SensorSize > 5M. K2 PIP only support 16M+5M. So sub > 5M will return 0 (indicate error).
            {
                ZsdHwMaxFrameRate    = 0;
