...
halVFB::mHalVFBSetWorkingBuf(void* WorkingBufAddr, MINT32 WorkingBufSize)
{
    ...
    MRESULT Retcode = S_VIDEO_FACEBEAUTY_OK;
    MTKVideoFaceBeautyWorkBufInfoStruct vFaceBeautyWorkBufInfo; 
    vFaceBeautyWorkBufInfo.ext_mem_start_addr = (MUINT32) WorkingBufAddr;
...
halVFB::mHalVFBProc(MTKPipeVfbProcInfo VfbProcInfo, MTKPipeVfbResultInfo* VfbResultInfo)
{
    ...
    vFaceBeautyProcInfo.pTuningPara->SmoothLevel = VfbProcInfo.SmoothLevel + 5;
    vFaceBeautyProcInfo.pTuningPara->BrightLevel = VfbProcInfo.SkinColor + 5;
    vFaceBeautyProcInfo.pTuningPara->RuddyLevel = mRuddyLevel;
    vFaceBeautyProcInfo.pTuningPara->EnlargeEyeLevel = VfbProcInfo.EnlargeEyeLevel + 5;
    vFaceBeautyProcInfo.pTuningPara->SlimFaceLevel = VfbProcInfo.SlimFaceLevel + 5;
...
void halVFBTuning::mHalVFBTuningSetPCA(MINT32 PCAAdr) {
    MY_LOGD("[mHalVFBTuningSetPCA] Set vFB PCA@0x%x", PCAAdr); 
    for (int i=0;i<360;i++)
    {
        mPCALut[4*i+2] = ((int*) PCAAdr)[i];
        mPCALut[4*i+1] = ((int*) PCAAdr)[360+i];
        mPCALut[4*i+0] = ((int*) PCAAdr)[720+i];
    }
#ifdef _DEBUG_DUMP
    static int count = 0;
    char szFileName[100];
...
MUINT32* halVFBTuning::mHalVFBTuningGetPCA() {
    MY_LOGD("[mHalVFBTuningGetPCA] Get vFB PCA@0x%x", mPCALut); 
    return (MUINT32*) mPCALut;
}
void halVFBTuning::mHalVFBTuningSetLCE(MINT32 LCEAdr) {
    MY_LOGD("[mHalVFBTuningSetPCA] Set vFB LCE@0x%x", LCEAdr); 
    mLCEAdr = LCEAdr;
}
MUINT32 halVFBTuning::mHalVFBTuningGetLCE() {
    MY_LOGD("[mHalVFBTuningGetPCA] Get vFB LCE@0x%x", mLCEAdr); 
    return (MUINT32) mLCEAdr;
}
