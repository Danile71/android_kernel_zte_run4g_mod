...
/*-----------------------------------------------------------------------------
    Functions
  -----------------------------------------------------------------------------*/
//sMutex CamPath_B::mCQBufferLock;
...
int CamPath_B::_waitIrq( int type,int userEnum,unsigned int irq  )
{
    return 0;
}
...
int CamPath_B::enqueueBuf( MUINT32 const dmaChannel, stISP_BUF_INFO bufInfo, MVOID* private_data )
{
    //ISP_PATH_DBG("+,dmaChannel(%d)",dmaChannel);
    ...
    int ret = 0;
    //
    if ( 0 != this->ispBufCtrl.enqueueHwBuf(dmaChannel, bufInfo, private_data) ) {
        ISP_PATH_ERR("ERROR:enqueueHwBuf");
        goto EXIT;
        //return -1;
    }
