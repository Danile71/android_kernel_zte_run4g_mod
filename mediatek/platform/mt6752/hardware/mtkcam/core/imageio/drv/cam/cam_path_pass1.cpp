...
ISP_DRV_WAIT_IRQ_STRUCT WaitIrq;
	...
	//remove, no reason to lock at waitirq
    ...
    WaitIrq.Status = irq;
    WaitIrq.Timeout = 500;//CAM_INT_WAIT_TIMEOUT_MS;//ms ,0 means pass through.
...
int CamPathPass1::setP1RrzCfg(IspRrzCfg const rrz_cfg)
{
    ...
    if ( rrz_cfg.isTwinMode || (ISP_PASS1 == rrz_cfg.pass1_path) ) {
        ...
        ISP_QuerySize(EPortIndex_RRZO,ISP_QUERY_XSIZE_BYTE,(EImageFormat)DMARrzo.dma_cfg.lIspColorfmt,DMARrzo.dma_cfg.size.w,queryRst);        
		...
		ISP_QuerySize(EPortIndex_RRZO_D,ISP_QUERY_XSIZE_BYTE,(EImageFormat)DMARrzo_d.dma_cfg.lIspColorfmt,DMARrzo_d.dma_cfg.size.w,queryRst);        
...
MBOOL CamPathPass1::P1Notify_Mapping(MUINT32 cmd,MUINT32 obj){
	MBOOL ret = MTRUE;
	MUINT32 tbl_len = 0;
