...
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT cfgData,T* pDpStream)
{
    ...
    srcVirList[0] = (MVOID *)cfgData.srcVirAddr;
    srcVirList[1] = (MVOID *)cfgData.srcVirAddr;
    srcVirList[2] = (MVOID *)cfgData.srcVirAddr;

    err = pDpStream->queueSrcBuffer(&srcVirList[0],&srcPhyList[0],srcSizeList,cfgData.mdpSrcPlaneNum);
    if(err != DP_STATUS_RETURN_SUCCESS)
    {
        ...
        if(cfgData.dstPortCfg[index] == 1)
        {
			...
			else
			{
	            ...
	            if(index == ISP_MDP_DL_IMGXO){
					...
					DpJpegEncCfg.memSWAddr[0] = cfgData.dstJpgParaCfg.workingMemBuf.base_pAddr;
