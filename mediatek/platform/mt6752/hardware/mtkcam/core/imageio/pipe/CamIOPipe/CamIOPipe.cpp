    ...
    switch (pxlMode) {
        case ePxlMode_One_Single:
            if (eImgFmt_YUY2==fmt || eImgFmt_YVYU==fmt || eImgFmt_VYUY==fmt || eImgFmt_UYVY==fmt) {
                PIPE_ERR("ERROR:Not support YUV+1-pixel");
                return -1;
            }
            break;
        ...
        case ePxlMode_Two_Single:
            if (!(eImgFmt_YUY2==fmt || eImgFmt_YVYU==fmt || eImgFmt_VYUY==fmt || eImgFmt_UYVY==fmt)) {
                PIPE_ERR("Not YUV sensor!!!!! Not support one isp+2-pixel");
                return -1;
            }

            PIPE_INF("YUV sensor!!!!! one isp+2-pixel");
...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo)
{
    ...
    //
    if ( 0 != this->m_CamPathPass1.enqueueBuf( dmaChannel, bufInfo, rQBufInfo.vBufInfo[0].header) ) {
...
{
	//ef test, 1: crop > 16M, 2: 16M> crop >13M
...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        if ( EPortIndex_IMGO == vOutPorts[i]->index ) {
            ...
            idx_rrzo_d = i;
            en_p1_dma_d |= CAM_CTL_EN_P1_DMA_RRZO_EN;
    ...
    if (-1 != idx_imgo ) { // use output dma crop
        ...
        // not support x crop
        portInfo_imgo.crop1.x      = 0; // TODO:
        portInfo_imgo.crop1.floatX = 0;
        portInfo_imgo.crop1.w = portInfo_imgo.u4ImgWidth;

        if((portInfo_imgo.crop1.y!=0)|| (portInfo_imgo.crop1.floatX!=0) || (portInfo_imgo.crop1.h != portInfo_imgo.u4ImgHeight))
            PIPE_DBG("[IMGO warning] crop_y=%d crop_floatY(%d) crop_h=%d ", \
                    portInfo_imgo.crop1.y, portInfo_imgo.crop1.floatY, portInfo_imgo.crop1.h);
...
//        m_camPass1Param.imgo.format = portInfo_imgo.eImgFmt;
    ...
    if (-1 != idx_rrzo ) { // use output dma crop
        ...
        if((portInfo_rrzo.crop1.y!=0)|| (portInfo_rrzo.crop1.floatX!=0) || (portInfo_rrzo.crop1.h != portInfo_rrzo.u4ImgHeight))
            PIPE_DBG("[RRZO warning] crop_y=%d crop_floatY(%d) crop_h=%d ", \
                    portInfo_rrzo.crop1.y, portInfo_rrzo.crop1.floatY, portInfo_rrzo.crop1.h);
        ...
        //We use RRZ to crop, thus after passing info to RRZ, reset the RRZOCrop to 0,0
    ...
    if (-1 != idx_imgo_d ) {
        ...
        if((portInfo_imgo_d.crop1.y!=0) || (portInfo_imgo_d.crop1.h == portInfo_imgo_d.u4ImgHeight))
            PIPE_DBG("[warning] crop_y=%d crop_floatY(%d) crop_h=%d ", \
                portInfo_imgo_d.crop1.y,portInfo_imgo_d.crop1.floatY,portInfo_imgo_d.crop1.h);
...
//        m_camPass1Param.imgo_d.format = portInfo_imgo_d.eImgFmt;
    ...
    if (-1 != idx_rrzo_d ) { // use output dma crop
        ...
        if((portInfo_rrzo_d.crop1.y!=0)|| (portInfo_rrzo_d.crop1.floatX!=0) || (portInfo_rrzo_d.crop1.h != portInfo_rrzo_d.u4ImgHeight))
            PIPE_DBG("[RRZOD warning] crop_y=%d crop_floatY(%d) crop_h=%d, u4ImgHeight=%d ", \
                    portInfo_rrzo_d.crop1.y, portInfo_rrzo_d.crop1.floatY, portInfo_rrzo_d.crop1.h, portInfo_rrzo_d.u4ImgHeight);
...
MUINT32 irq = 0;
    ...
    if ( EPIPEIRQ_VSYNC == irq_int ) {
        type = (EPipePass_PASS1_TG1 == pass)?ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
        irq = (EPipePass_PASS1_TG1 == pass)?CAM_CTL_INT_P1_STATUS_VS1_INT_ST:CAM_CTL_INT_P1_STATUS_D_VS1_INT_ST;
    }
    else if ( EPIPEIRQ_PATH_DONE == irq_int ) {
        type = (EPipePass_PASS1_TG1 == pass)?ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
        irq = (EPipePass_PASS1_TG1 == pass)?CAM_CTL_INT_P1_STATUS_PASS1_DON_ST:CAM_CTL_INT_P1_STATUS_D_PASS1_DON_ST;
    }
    else if ( EPIPEIRQ_SOF == irq_int ) {
        type = (EPipePass_PASS1_TG1 == pass)?ISP_DRV_IRQ_TYPE_INT_P1_ST:ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
        irq = (EPipePass_PASS1_TG1 == pass)?CAM_CTL_INT_P1_STATUS_SOF1_INT_ST:CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST;
    }
    ...
    //
    ret = m_CamPathPass1.waitIrq(type,userEnum,irq);
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_EIS_CBFP:
       	case EPIPECmd_SET_LCS_CBFP:
        	...
        	m_CamPathPass1.P1Notify_Mapping(cmd,arg1);
        ...
        case EPIPECmd_SET_RRZ:
            ...
            if (0 == pUserRrzPara->out.stride) {
                ...
                //workaround first
                pUserRrzPara->out.stride = (((pUserRrzPara->out.w*3)>>1) * 10)>> 3;
            }
            ...
            if (0 == pUserRrzPara->out.w * pUserRrzPara->out.w) {
            ...
            if ( EPortIndex_TG1I == arg2){
                ...
                if (mMagicNum0 == mMagicNum1) {
                    ...
                    PIPE_DBG("[3ASync]SET_RRZ:TG1I Magic0(0x%x)!= Magic1(0x%x)-", mMagicNum0, mMagicNum1);
                }
            }
            else if ( EPortIndex_TG2I == arg2){
                ...
                if (mMagicNum0_D == mMagicNum1_D) {
                    ...
                    if ( ISP_SUB_MODE_YUV != this->m_camPass1Param.sub_mode) {
                        if ( 0 != m_CamPathPass1.setP1TuneCfg(mMagicNum0_D, arg3) ){
                            PIPE_ERR("[3ASync]setP1TuneCfg Fail");
                            ret = -1;
                        }
                    }

                    if ( 0 != m_CamPathPass1.setP1RrzCfg(rrz_cfg) ){
                        PIPE_ERR("[3ASync]setP1RrzCfg Fail");
                        ret = -1;
                    }
					if(1 != m_CamPathPass1.setP1Notify()){
                    	ret = -1;
                    ...
                    PIPE_DBG("[3ASync]SET_RRZ:TG2I Magic0(0x%x)!= Magic1(0x%x)-", mMagicNum0_D, mMagicNum1_D);
                }
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            if ( EPortIndex_TG1I == arg1 ) {
                ...
                if (mMagicNum0_D == mMagicNum1_D){
                    ...
                    PIPE_DBG("[3ASync]:TG2I Magic0(0x%x)!= Magic1(0x%x)-", mMagicNum0_D, mMagicNum1_D);
                }
            }
            //
            this->mRunTimeCfgLock.unlock();
