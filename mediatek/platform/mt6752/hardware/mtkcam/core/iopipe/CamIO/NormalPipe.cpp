 ...
 *
 ******************************************************************************/
MUINT32 NormalPipe::mTwinEnable[CAM_SV_2+1][2] = {{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single}};//record this & twin mode,0 for this, 1 for twinmode
...
replace(BufInfo const& bufOld, BufInfo const& bufNew)
{
    ...
    rQBufInfo.vBufInfo[0].u4BufVA[0] =     (MUINT32)bufOld.mBuffer->getBufVA(0); //
    rQBufInfo.vBufInfo[0].u4BufPA[0] =     (MUINT32)bufOld.mBuffer->getBufPA(0); //
#else
    rQBufInfo.vBufInfo[0].u4BufSize[0] =   (MUINT32)bufOld.mSize;
    rQBufInfo.vBufInfo[0].u4BufVA[0] =     (MUINT32)bufOld.mVa;
    rQBufInfo.vBufInfo[0].u4BufPA[0] =     (MUINT32)bufOld.mPa;
#endif
    ...
    rQBufInfo.vBufInfo[1].u4BufVA[0] =     (MUINT32)bufNew.mBuffer->getBufVA(0); //
    rQBufInfo.vBufInfo[1].u4BufPA[0] =     (MUINT32)bufNew.mBuffer->getBufPA(0); //
#else
    rQBufInfo.vBufInfo[1].u4BufSize[0] =   (MUINT32)bufNew.mSize;
    rQBufInfo.vBufInfo[1].u4BufVA[0] =     (MUINT32)bufNew.mVa;
    rQBufInfo.vBufInfo[1].u4BufPA[0] =     (MUINT32)bufNew.mPa;
#endif
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if ((portID.index == PORT_RRZO.index) ||(portID.index == PORT_RRZO_D.index)) {
...
#ifdef USE_IMAGEBUF_HEAP
                 pframe = mpFrameMgr->deque(rQTSBufInfo.vBufInfo.at(idx).u4BufVA[0]);
                 if (NULL == pframe) {
                 ...
                 ResultMetadata result(MRect(MPoint(0, 0), MSize(rQTSBufInfo.vBufInfo.at(idx).img_w,rQTSBufInfo.vBufInfo.at(idx).img_h)), \
                                           0/*transform*/, \
                                           rQTSBufInfo.vBufInfo.at(idx).m_num_0, \
                                           rQTSBufInfo.vBufInfo.at(idx).m_num_1,\
                                           getTimeStamp_ns(rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_sec,  rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_us),\
                                           rQTSBufInfo.vBufInfo.at(idx).header,\
                                           rQTSBufInfo.vBufInfo.at(idx).headerSize);

                 BufInfo buff(rQBuf.mvOut.at(ii).mPortID, pframe,
                            rQTSBufInfo.vBufInfo.at(idx).u4BufSize[0],
                            rQTSBufInfo.vBufInfo.at(idx).u4BufVA[0],
                            rQTSBufInfo.vBufInfo.at(idx).u4BufPA[0],
                            result);
...
//    pixelCnt2 = _NORMALPIPE_GET_PIX_MODE(S2Idx);
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        ...
        mbTwoPixelEnable =  pixelCnt;
        // RAW: two-pixel mode, twin isp
        if (enable2PixelTwin){
            if (tg <= CAM_TG_2){
                mTwinEnable[tg][1] = ePxlMode_Two_Twin;
            }
        }
        // YUV: two-pixel mode, one isp
        else if (MFALSE == enable2PixelTwin && 1 == pixelCnt){
            if (tg <= CAM_TG_2){
...
NormalPipe::Reset()
{
    ...
    if (!mpHalSensor){
        ...
        CAM_LOGE("[Error] ESDRecovery ReEntry...");
    }
    CAM_LOGD("Reset+, SenIdx=%d", mpSensorIdx[0]);

    //we need to power off sensor and power on the sensor.
    //Power off
    mpHalSensor->powerOff(szCallerName, mpEnabledSensorCnt, mpSensorIdx);
    //Power On
    mpHalSensor->powerOn(szCallerName, mpEnabledSensorCnt, mpSensorIdx);
    //Configure the sensor again.
    mpHalSensor->configure(mpEnabledSensorCnt, mpsensorConfigParam);
    CAM_LOGD("-");
    return MTRUE;
}
...
NormalPipe::configPipe(QInitParam const& vInPorts)
{
    ...
    ///2 Get Info
    memset(mpsensorConfigParam, 0, sizeof(IHalSensor::ConfigParam));
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++)  {
        ...
        pAllSensorInfo->mSenInfo[SIdx].mOccupiedOwner = (MUINT32)this;

        mpsensorConfigParam[i] = vInPorts.mSensorCfg.at(i); //used for sensorCfg
        sensorScenId = mpsensorConfigParam[i].scenarioId;
        ...
        for (int j=0; j<vInPorts.mPortInfo.size();j++) {
            if (( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID )|| ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID )) {
            	if(vInPorts.mPortInfo.at(j).mDstSize.w > RRZ_LIMITATION){
            		mpsensorConfigParam[i].twopixelOn = MTRUE;
                    break;
            	}
            }
         }
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++) {
	    ...
	    if(vInPorts.mRawType){
	    	...
	    	mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId,SENSOR_CMD_SET_TEST_PATTERN_OUTPUT,(MUINT)&vInPorts.mRawType,0,0);
	    }
	    //
    ...
    if (!mpCamIOPipe)
    {
        ...
        if((CAM_TG_2 == _NORMALPIPE_GET_TG_IDX(mpSensorIdx[0])) && (MFALSE == mbTwoPixelEnable)) //CAM_TG_2
        {
        ...
        for (int i = 0; i < mpEnabledSensorCnt;i++)
        {
            ...
            // Rule: YUV sensor only uses two-pixel mode and one-isp.
            // So check whether sensor type is YUV and it uses two-pixel mode and one isp. If not, show error msg and return MFALSE.
            if (NSCam::SENSOR_TYPE_YUV == _NORMALPIPE_GET_SENSORTYPE(sensorIdx)) {
                if (mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1] != ePxlMode_Two_Single){
                    CAM_LOGE("error: YUV sensor not use two-pixel mode and one isp, sensorIdx=%d, tg=%d, PixelMode=%d", sensorIdx, _NORMALPIPE_GET_TG_IDX(sensorIdx), mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1]);
	...
	//update cq table num if mw member update burst queue number
        ...
        for (int i=0; i<vInPorts.mPortInfo.size();i++) {
            ...
            if ((pOut->index  == EPortIndex_RRZO)||(pOut->index  == EPortIndex_RRZO_D)) {
                if ( (0 == mbTwoPixelEnable) && (/*CAM_RRZ_MAX_LINE_BUFFER_IN_PIXEL*/RRZ_LIMITATION < pOut->u4ImgWidth) ) {
        ...
        for (int i = 0; i < mpEnabledSensorCnt; i++)
        {
            switch ( _NORMALPIPE_GET_TG_IDX((mpSensorIdx[i])) )
            {
               case CAM_TG_1:
                   ...
                   mpCamIOPipe->irq(EPipePass_PASS1_TG1, EPIPEIRQ_VSYNC,(int)ISP_DRV_IRQ_USER_ISPDRV);
                   break;
               case CAM_TG_2:
                   ...
                   mpCamIOPipe->irq(EPipePass_PASS1_TG1_D, EPIPEIRQ_VSYNC,(int)ISP_DRV_IRQ_USER_ISPDRV);
                   break;
               default:
                   break;
            }
    ...
    {
        CamManager* pCamMgr = CamManager::getInstance();
        pCamMgr->setAvailableHint((0 == mbTwoPixelEnable)? MTRUE : MFALSE);
    }
...
NormalPipe::configFrame(QFrameParam const& rQParam)
{
    ...
    //CAM_LOGD("size(%d),mbTwoPixelEnable(%d),mpEnabledSensorCnt(%d),", \
    //        rQParam.mvOut.size(),\
    //        mbTwoPixelEnable, \
    //        mpEnabledSensorCnt);
    ...
    for ( i =0 ; i < rQParam.mvOut.size();i++ ) {
        ...
        if (( PORT_RRZO == port )  || ( PORT_RRZO_D == port) || (PORT_IMGO == port) )
        {
            ...
            if (mpCamIOPipe) {
                ...
                //Arg3: sensorDev
                MUINT32 sensorDev = eNpipeUpdateTgtDev_Main;
                if (SENSOR_DEV_SUB == _NORMALPIPE_GET_SENSOR_DEV_ID((mpSensorIdx[0]))) {
                    sensorDev = eNpipeUpdateTgtDev_Sub;
                }
                //lock rrz in order to update rrzo size for EIS to get rrz size
                NormalPipeGLock();
                if ( MFALSE == mpCamIOPipe->sendCommand(EPIPECmd_SET_RRZ,(MINT32)&rrz, tgIdx, sensorDev) ){
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    if ( !mConfigDone ) {
        CAM_LOGD("[warn]sendCommand+, mConfigDone=0");
        return MFALSE;
    }
    ...
    switch (cmd) {
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            //Arg2: magic number
            //Arg3: sensorDev
            sensorDev = eNpipeUpdateTgtDev_Main;
            if (SENSOR_DEV_SUB == _NORMALPIPE_GET_SENSOR_DEV_ID((mpSensorIdx[0]))) {
                sensorDev = eNpipeUpdateTgtDev_Sub;
            }
       ...
       case EPIPECmd_GET_HBIN_INFO:
           ...
           CAM_LOGD("EPIPECmd_GET_HBIN_INFO SIdx(%d),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",arg1,mbTwoPixelEnable, hbin->mEnabled,hbin->size.w, hbin->size.h);
           break;
