...
#define BD_SEG_NUM (3*3+2) //5M, 8M, 13M, 16M, no limitration
#define TG_grabH 40
...
efMappingTable efTable[BD_SEG_NUM]
{
	...
	{3363,   "Boundary case of 8M (-1)"},
	{3364,   "Bonding at 8M"},
	{3365,   "Boundary case of 8M (+1)"},
	{4323,   "Boundary case of 13M (-1)"},
	{4324,   "Bonding at 13M"},
	{4325,   "Boundary case of 13M (+1)"},
	{4707,   "Boundary case of 16M (-1)"},
	{4708,   "Bonding at 16M"},
	{4709,   "Boundary case of 16M (+1)"},
	{5264,   "No limitation"}
};
	...
	for(int i=0;i<BD_SEG_NUM;i++)
	 {
		...
		{
		    index               : 0, //modify
		    crop                : MSize(efTable[i].TG_grabW,TG_grabH), //FIXME
		...
		m_tgi.u4ImgWidth  = efTable[i].TG_grabW;  // Image Width
		...
		m_imgo.u4Stride[ESTRIDE_1ST_PLANE] = queryRawStride((MUINT32)m_imgo.eImgFmt , m_imgo.u4ImgWidth);//efTable[i].TG_grabW*10/8;  
		...
		//
		LOG_INF("%s pass",efTable[i].resultStr);
...
EXIT:
	LOG_INF("resultIdx(%d)",resultIdx);
	LOG_INF("BONDING RESULT: %s",efTable[resultIdx].resultStr);
	printf("resultIdx(%d)\n",resultIdx);
	printf("BONDING RESULT: %s",efTable[resultIdx].resultStr);
