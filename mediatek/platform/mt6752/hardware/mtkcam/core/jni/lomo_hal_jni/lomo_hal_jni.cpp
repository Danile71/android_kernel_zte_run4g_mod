#define LOG_TAG "lomohj_v0821_00"  //eCACHECTRL_INVALID after allocation pHeap->createImageBuffer
...
//XLOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)	// <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
 ...
 **************************************************************************/
   ...
   static MUINT32 pBufDstHeapBuffIdx[LOMO_EFFECT_DST_BUFF_NUM]={0};
                                                                     ...
                                                                     MUINT32 ppSrcImgHeight) 
{
    ...
    {
        gu32ppSrcImgVA [count] = pBufSrc.get() ->getBufVA(count);  //This is the source image VA ADDR to postprocess (YUY2)
        gu32ppSrcImgMVA[count] = pBufSrc.get() ->getBufPA(count);//This is the source image MVA ADDR to postprocess (YUY2)
    }
...
//Output Image
    ...
    {
        gu32ppDstImgVA [count] = pBufDst.get() ->getBufVA(count);  //This is the source image VA ADDR to postprocess (YUY2)
        gu32ppDstImgMVA[count] = pBufDst.get() ->getBufPA(count);//This is the source image MVA ADDR to postprocess (YUY2)
    }       
/////    
                                                                       ...
                                                                       )
{
    ...
    LOMOHALJNI_RETURN_TYPE err = LOMOHALJNI_NO_ERROR;    
    if(pBufDstHeapBuffIdx[Number]!=0)
    {
        if(pBufDstHeapBuffIdx[Number]!= (MUINT32)ppDstImgVA[0])
        {
            LOG_ERR("ERROR !! pBufDstHeapBuffIdx[%d] is NOT matching SOURCE 0x%8x",Number,(MUINT32)ppDstImgVA[0]);
...
/////
        if (pBufDstHeap[Number].get() == 0)
        {
            MY_LOG("NULL Buffer!- skip this time");
            return LOMOHALJNI_NULL_OBJECT;
        }
        pBufDstHeapBuffIdx[Number]=pBufDstHeap[Number].get() ->getBufVA(0);      
        //140522>>
        pBufDstHeap[Number].get()->syncCache(eCACHECTRL_INVALID);    
        //140522<<
        if(pBufDstHeap[Number].get() ->getBufVA(0)==0)
        {
        ...
        if(Number==(LOMO_EFFECT_DST_BUFF_NUM-1))
        {
            ...
            {
                MY_LOG("pBufDstHeapBuffIdx[%d]=0x%x",NumberIdx,pBufDstHeapBuffIdx[NumberIdx]);
            }
        }
        #endif
/////       
...
MINT32 LomoHalJniImp::UploadLomoSrcImage(MUINT8* pvCBVA) 
{
    ...
    pBuffer[1] = (void*)((uint32_t)pBuffer[0]+((gu32pvCBWidth*gu32pvCBHeight))); //YV12 YUV420
    pBuffer[2] = (void*)((uint32_t)pBuffer[1]+((gu32pvCBWidth*gu32pvCBHeight)>>2));//YV12 YUV420
...
MINT32 LomoHalJniImp::FreeLomoSrcImage(void)
{
    ...
    {
        MY_LOG("Free\n gu32ppSrcImgVA[%d] 0x%8x",count,gu32ppSrcImgVA[count]);
        MY_LOG("gu32ppSrcImgVA[%d] 0x%8x",count,gu32ppSrcImgVA[count]);
        gu32ppSrcImgVA[count] = 0;
        gu32ppSrcImgVA[count] = 0;
    }
    pBufSrc = NULL;
...
MINT32 LomoHalJniImp::LomoImageEnque(uint32_t* ppDstImgVA, MUINT32 ppEffectIdx) 
{
    LOMOHALJNI_RETURN_TYPE err = LOMOHALJNI_NO_ERROR;
...
//          MY_LOG("Buffer #%d is matching",Number);    
        ...
        {
            NoMatchingVA= MTRUE;
            MY_LOG("Buffer #%d is NOT matching 0x%8x!",Number,ppDstImgVA[0]);    
            return LOMOHALJNI_WRONG_PARAM;
        }
    }
#else
...
MINT32 LomoHalJniImp::LomoImageDeque(uint32_t* ppDstImgMVA, MUINT32 ppEffectIdx) 
{
    LOMOHALJNI_RETURN_TYPE err = LOMOHALJNI_NO_ERROR;
    LOMO_TRACE_CALL();
#ifdef USE_IMAGE_BUFFER_HEAP     	
    ...
    NSCam::NSIoPipe::NSPostProc::Output moutput;    
    MBOOL NoMatchingVA= MFALSE; // 1   ok, 0 NG
    MUINT32 mu32VaAddr= 0; //
    MUINT32 mu32FlsuhVaAddr= 0; //
        ...
        {    
                ...
                { 
                    ...
                    if(brawdumpEnable)
                    {
                        char fileName[64];
                        int fileSize=0;
                        if((lu32BuffeffectIdx>>ppEffectIdx)&0x01)
                        {
                            if(frameCntEffectIdx==ppEffectIdx)
                            {
                                MY_LOG(" lu32BuffIdx=0x%8x (%d)",lu32BuffeffectIdx,lu32BuffeffectIdx);
                                frameCnt++;
                                sprintf(fileName, "/sdcard/lomo/lomo%4dx%4d_%2d_%5d_YV12_s.bin", gu32ppDstImgWidth,gu32ppDstImgHeight,ppEffectIdx,frameCnt);
                            }
                            else
                            {
                                sprintf(fileName, "/sdcard/lomo/lomo%4dx%4d_%2d_%5d_YV12.bin", gu32ppDstImgWidth,gu32ppDstImgHeight,ppEffectIdx,frameCnt);
                            }
                ...
                {
                    NoMatchingVA= MTRUE;
                    MY_LOG("deque buffer no matching! VA : 0x%8x",mu32VaAddr);
                    return LOMOHALJNI_WRONG_PARAM;
                }
        }
    }
...
//                    IImageBuffer* tempBuffer = const_cast<IImageBuffer*>(pImageBuffer);
    for(MUINT32 Number=0; Number<LOMO_EFFECT_DST_BUFF_NUM; Number++)
    {
        ...
        {        
            MY_LOG("Free\n pBufDstHeap[%d] 0x%8x",Number,pBufDstHeap[Number].get());
...
///////////////////        
    ...
    else
    {
        for (MUINT32 count = 0; count < LOMO_HAL_JNI_MAX_PLANE; count++)
        {
            MY_LOG("Free\n gu32ppDstImgVA[%d] 0x%8x",count,gu32ppDstImgVA[count]);
            MY_LOG("gu32ppDstImgVA[%d] 0x%8x",count,gu32ppDstImgVA[count]);
            gu32ppDstImgVA[count] = 0;
            gu32ppDstImgVA[count] = 0;
        }
    }
    pBufDst = NULL;
...
//    MY_LOG("se.out_edge_ctrl.val=0x%8x \n",se.out_edge_ctrl.val);
//    MY_LOG("se.y_ctrl.val=0x%8x \n",se.y_ctrl.val);
    MY_LOG("se.edge_ctrl_1.val=0x%8x \n",se.edge_ctrl_1.val);
//    MY_LOG("se.edge_ctrl_2.val=0x%8x \n",se.edge_ctrl_2.val);
//    MY_LOG("se.edge_ctrl_3.val=0x%8x \n",se.edge_ctrl_3.val);
//    MY_LOG("se.special_ctrl.val=0x%8x \n",se.special_ctrl.val);
//    MY_LOG("se.core_ctrl_1.val=0x%8x \n",se.core_ctrl_1.val);
//    MY_LOG("se.core_ctrl_2.val=0x%8x \n",se.core_ctrl_2.val);
...
//    MY_LOG("lomo magicNum = (%d)",magicNum);
    if(deqRet!=1)
    {
        ...
        LOG_ERR("ERROR !! dequeParams.mvOut[0].mBuffer->getBufVA(0)=0x%x",dequeParams.mvOut[0].mBuffer->getBufVA(0));
        err = LOMOHALJNI_PASS2_NOT_READY;
    }
    mu32OutPortSize = dequeParams.mvOut.size();
//    MY_LOG("lomo mu32OutPortSize = (%d)",mu32OutPortSize);
    for(int j=0;j<mu32OutPortSize;j++)
    {
        mu32VaAddr = dequeParams.mvOut[j].mBuffer->getBufVA(0); 
        mu32MVaAddr = dequeParams.mvOut[j].mBuffer->getBufPA(0); 
        if(j==0)
        {
        ...
        if(bAddrDumpEnable)
        { 
            MY_LOG("Deque#%d magicNum(%d) mvOut VA(0x%8x) MVA(0x%8x)",j,magicNum,mu32VaAddr,mu32MVaAddr);
        }
    }       
////
    for(MUINT32 Number=0; Number<LOMO_EFFECT_DST_BUFF_NUM; Number++)
    {    
        ...
        {
            NoMatchingVA= MTRUE;
            LOG_ERR("deque buffer no matching! VA : 0x%8x",mu32VaAddr);
            return LOMOHALJNI_WRONG_PARAM;
        }
    }
///
