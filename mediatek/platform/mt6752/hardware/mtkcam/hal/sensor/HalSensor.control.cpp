...
)
{
    ...
    //Test Mode use property parameter
    {
        ...
        if((hdrModeTest == 2)||(hdrModeTest == 9))
        {
    ...
    if(currSensorDev & SENSOR_DEV_MAIN ) {
        ...
        if(halSensorIFParam[idx].framerate == 0)
        {
            ...
            ret = pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, &data1, &data2);
        ...
        {
            data1 = halSensorIFParam[0].scenarioId;
            ret = pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_SENSOR_VC_INFO, (MUINT32 *)&VcInfo, &data1);
    ...
    if(currSensorDev & SENSOR_DEV_SUB ) {
        ...
        if(halSensorIFParam[idx].framerate == 0)
        {
            ...
            ret = pSensorDrv->sendCommand(SENSOR_SUB, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, &data1, &data2);
            halSensorIFParam[idx].framerate = data2;
            currFPS = halSensorIFParam[idx].framerate;
            MY_LOGD("configure Scenario=%d, FPS=%d\n",data1,data2);
        }
    ...
    if(currSensorDev & SENSOR_DEV_MAIN_2 ) {
        ...
        if(halSensorIFParam[idx].framerate == 0)
        {
            ...
            ret = pSensorDrv->sendCommand(SENSOR_MAIN_2, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, &data1, &data2);
            halSensorIFParam[idx].framerate = data2;
            currFPS = halSensorIFParam[idx].framerate;
            MY_LOGD("configure Scenario=%d, FPS=%d\n",data1,data2);
        }
...
lbExit:
 ...
 MINT HalSensor::setTgPhase(MINT32 sensorIdx, MINT32 pcEn) //CMMCLK: Main/sub, CMMCLK2:Main_2 (external signal design is not sync with internal signal in TG/I2C)
 {
     ...
     case SENSOR_DEV_MAIN:
         ...
         break;
     case SENSOR_DEV_SUB:
         ret = pSensorDrv->sendCommand(SENSOR_SUB, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv2);
         break;
     case SENSOR_DEV_PIP:
         ret = pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv1);
         ret2 = pSensorDrv->sendCommand(SENSOR_SUB, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv2);
         break;
     case SENSOR_DEV_MAIN_2:
         ret = pSensorDrv->sendCommand(SENSOR_MAIN_2, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv3);
         break;
     case SENSOR_DEV_MAIN_3D:
         ret = pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv1);
         ret2 = pSensorDrv->sendCommand(SENSOR_MAIN_2, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv3);
         break;
     default:
         u4PadPclkInv1 = 0;
         u4PadPclkInv2 = 0;
    ...
    MUINT arg3)
{
    ...
    case SENSOR_CMD_SET_SENSOR_EXP_TIME:
        ...
        pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
    ...
    case SENSOR_CMD_SET_CCT_FEATURE_CONTROL:
        ...
        pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1, (MUINT32 *) arg2, (MUINT32 *) arg3);
        break;
    ...
    case SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO:
        ...
        pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1, (MUINT32 *) arg2);
    ...
    case SENSOR_CMD_SET_N3D_I2C_POS:
        ret = pSeninfDrv->setN3DI2CPos(*(MUINT32 *)arg1);
        if(ret < 0) {
            MY_LOGE("[sendCommand] SENSOR_CMD_SET_N3D_I2C_POS fail! \n");
        }
        break;
    case SENSOR_CMD_SET_N3D_I2C_TRIGGER:
        ret = pSeninfDrv->setN3DTrigger(*(MUINT32 *)arg1, *(MUINT32 *)arg2);
        if(ret < 0) {
    ...
    case SENSOR_CMD_GET_UNSTABLE_DELAY_FRAME_CNT:
        ...
        pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1, (MUINT32 *) arg2); //arg1 = mode
        break;
   ...
   case SENSOR_CMD_GET_SENSOR_VC_INFO:
       ret = pSensorDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId, CMD_SENSOR_GET_SENSOR_VC_INFO, (MUINT32 *)arg1, (MUINT32 *)arg2);
       if(ret < 0) {
...
MVOID HalSensor::pixelModeArrange(MUINT currSensorDev)
{
    ...
    if(currSensorDev & SENSOR_DEV_MAIN ) {
        ...
        MY_LOGI("Main cam pixel mode: %d, sensorType: %d\n", sensorDynamicInfo[0].pixelMode, sensorStaticInfo[0].sensorType);
    }
   if(currSensorDev & SENSOR_DEV_SUB ) {
        ...
        MY_LOGI("Sub cam pixel mode: %d, sensorType: %d\n", sensorDynamicInfo[1].pixelMode, sensorStaticInfo[1].sensorType);
    }
   if(currSensorDev & SENSOR_DEV_MAIN_2 ) {
        ...
        MY_LOGI("Main2 cam pixel mode: %d, sensorType: %d\n", sensorDynamicInfo[2].pixelMode, sensorStaticInfo[2].sensorType);
    }
    ...
    if (1 == enable) {
        ...
        switch(meSensorDev) {
            case SENSOR_DEV_MAIN:
                ...
                ret = pSeninfDrv->setSeninf1NCSI2(0, 0, 0, 0, 0, 0, 0, 0, 0);   // Disable main's NCSI first
                ret = pSeninfDrv->setSeninf2NCSI2(0, 0, 0, 0, 0, 0, 0, 0, 0);   // Disable sub's NCSI first
                ...
                if (sensorPara[0].inSrcTypeSel >= VIRTUAL_CHANNEL_1) {
                    ...
                    if(sensorPara[0].mipiPad == 0) {
                        if(sensorPara[0].MIPI_OPHY_TYPE == 1) // CSI2
                        {
                            ...
                            ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[0].u1MIPIDataTermDelay,sensorPara[0].u1MIPIDataSettleDelay,sensorPara[0].u1MIPIClkTermDelay,
                                sensorPara[0].u1VsyncPol,sensorPara[0].u1MIPILaneNum,1,sensorPara[0].u1MIPIPacketECCOrder,1,0);
                        }
                        else
                        {
                            ret = pSeninfDrv->setSeninf1Ctrl((PAD2CAM_DATA_ENUM)sensorPara[0].padSel, (SENINF_SOURCE_ENUM)sensorPara[0].inSrcTypeSel);
                            ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[0].u1MIPIDataTermDelay,sensorPara[0].u1MIPIDataSettleDelay,sensorPara[0].u1MIPIClkTermDelay,
                                sensorPara[0].u1VsyncPol,sensorPara[0].u1MIPILaneNum,1,sensorPara[0].u1MIPIPacketECCOrder,0,sensorPara[0].NCSI2_HSRXDET_MODE);
                        }
            ...
            case SENSOR_DEV_SUB:
                ...
                /// [Important] The rule of TG path for sub cam
                /// [reason] The line buffer in TG2 is limited in this platform, and TG1 is not limited.
                ///             So we need sub cam to use TG2 and main cam to use TG1 when main cam needs to output larger image size, e.x.: full size.
                /// [Rule] If the current scenario (sensor mode) is VT mode (.slim_video), let sub cam use TG2.
                ///        Or, let sub cam use TG1.

                // 1. Sub cam uses TG1 if not in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode (VT)
                if(SENSOR_SCENARIO_ID_SLIM_VIDEO2 != sensorPara[1].scenarioId) {
                    ...
                    MY_LOGD("Sub is not in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode. Use TG1.\n");
                }
                // 2. Sub cam uses TG2 if in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode (VT)
                else {
                    ...
                    MY_LOGD("Sub is in SENSOR_SCENARIO_ID_SLIM_VIDEO2 mode. Use TG2.\n");
                }
                    ...
                    if(sensorPara[1].mipiPad == 2) {
                        ...
                        if(sensorPara[1].MIPI_OPHY_TYPE == 1) // CSI2
                        {
                            ...
                            ret = pSeninfDrv->setSeninf3NCSI2(sensorPara[1].u1MIPIDataTermDelay,sensorPara[1].u1MIPIDataSettleDelay,sensorPara[1].u1MIPIClkTermDelay,
                                sensorPara[1].u1VsyncPol,sensorPara[1].u1MIPILaneNum,1,sensorPara[1].u1MIPIPacketECCOrder,1,0);
                        }
                        else
                        {
                            ret = pSeninfDrv->setSeninf3Ctrl((PAD2CAM_DATA_ENUM)sensorPara[1].padSel, (SENINF_SOURCE_ENUM)sensorPara[1].inSrcTypeSel);
                            ret = pSeninfDrv->setSeninf3NCSI2(sensorPara[1].u1MIPIDataTermDelay,sensorPara[1].u1MIPIDataSettleDelay,sensorPara[1].u1MIPIClkTermDelay,
                            sensorPara[1].u1VsyncPol,sensorPara[1].u1MIPILaneNum,1,sensorPara[1].u1MIPIPacketECCOrder,0,sensorPara[1].NCSI2_HSRXDET_MODE);
                        }
                        ...
                        if(sensorPara[2].mipiPad == 0) {
                            if(sensorPara[2].MIPI_OPHY_TYPE == 1) // CSI2
                            {
                                ...
                                ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[2].u1MIPIDataTermDelay,sensorPara[2].u1MIPIDataSettleDelay,sensorPara[2].u1MIPIClkTermDelay,
                                    sensorPara[2].u1VsyncPol,sensorPara[2].u1MIPILaneNum,1,sensorPara[2].u1MIPIPacketECCOrder,1,0);
                            }
                            else
                            {
                                ret = pSeninfDrv->setSeninf1Ctrl((PAD2CAM_DATA_ENUM)sensorPara[2].padSel, (SENINF_SOURCE_ENUM)sensorPara[2].inSrcTypeSel);
                                ret = pSeninfDrv->setSeninf1NCSI2(sensorPara[2].u1MIPIDataTermDelay,sensorPara[2].u1MIPIDataSettleDelay,sensorPara[2].u1MIPIClkTermDelay,
                                sensorPara[2].u1VsyncPol,sensorPara[2].u1MIPILaneNum,1,sensorPara[2].u1MIPIPacketECCOrder,0,sensorPara[1].NCSI2_HSRXDET_MODE);
                            }
                            ret = pSeninfDrv->setSeninfTopMuxCtrl(SENINF_TOP_TG1,SENINF_1);
                        }
                    ...
                    case (SENSOR_DEV_MAIN|SENSOR_DEV_MAIN_2):
                        ...
                        if ((sensorPara[0].inSrcTypeSel >= MIPI_SENSOR) && (sensorPara[2].inSrcTypeSel >= MIPI_SENSOR)) {
                            ...
                            if(sensorPara[2].mipiPad == 1) {
                                ret = pSeninfDrv->setSeninf2Ctrl((PAD2CAM_DATA_ENUM)sensorPara[2].padSel, (SENINF_SOURCE_ENUM)sensorPara[2].inSrcTypeSel);
                                ret = pSeninfDrv->setSeninf2NCSI2(sensorPara[2].u1MIPIDataTermDelay,sensorPara[2].u1MIPIDataSettleDelay,sensorPara[2].u1MIPIClkTermDelay,
                                    sensorPara[2].u1VsyncPol,sensorPara[2].u1MIPILaneNum,1,sensorPara[2].u1MIPIPacketECCOrder,0,0);
                                ret = pSeninfDrv->setSeninfTopMuxCtrl(SENINF_TOP_TG2,SENINF_2);
                            }
                            else if(sensorPara[2].mipiPad == 2) {
                                ret = pSeninfDrv->setSeninf3Ctrl((PAD2CAM_DATA_ENUM)sensorPara[2].padSel, (SENINF_SOURCE_ENUM)sensorPara[2].inSrcTypeSel);
