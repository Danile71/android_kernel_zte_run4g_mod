...
{
    LOG_INF("singleton[0x%08x],getpid[0x%08x],gettid[0x%08x] ",(int)&singleton,getpid() ,gettid());
    return &singleton;
}
        ...
        {   //set sensor dev if sensorHal.Control do configure
            ...
            switch(senTg)
            {
                case CAM_TG_1:
                    ...
                    if(currfps==0)
                    {	//default frame rate	
					    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO,(MINT32)&senScen,(MINT32)&fps,0);
					 	if(ret<0)
						{
...
changeMode()
{
    ...
    MINT32 pclk=0;
    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_GET_PIXEL_CLOCK_FREQ,(MUINT32)&pclk,0,0);
	if(ret<0)
	{
    ...
    mInitPclk[0]=pclk;
    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_GET_FRAME_SYNC_PIXEL_LINE_NUM,(MUINT32)&periodPara,0,0);
 	if(ret<0)
	{
	...
	periodPara=0x0;
    pclk=0;
    ret=mHalSensor->sendCommand(mSensorDevArray[1],SENSOR_CMD_GET_PIXEL_CLOCK_FREQ,(MUINT32)&pclk,0,0);
	if(ret<0)
	{
		LOG_INF("get pclk of sensorDev(0x%x) failed\n",mSensorDevArray[0]);
		return -1;
    ...
    mInitPclk[1]=pclk;
    ret=mHalSensor->sendCommand(mSensorDevArray[1],SENSOR_CMD_GET_FRAME_SYNC_PIXEL_LINE_NUM,(MUINT32)&periodPara,0,0);
	if(ret<0)
	{
		LOG_ERR("get initial period information of sensorDev(0x%x) failed\n",mSensorDevArray[1]);
	}
...
MBOOL HWSyncDrvImp::checkNeedSync()
{
    ...
    {
        case 1:
            ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_GET_SENSOR_N3D_DIFFERENCE_COUNT,(MUINT32)&mN3DregInfo,0,0);
        	if(ret<0)
        	{
...
MINT32 HWSyncDrvImp::getSensorInfo(MUINT32 senDev,MINT32* pclk,MINT32* periodInfo,MINT32* dummyInfo,MINT32 curfps)
{
    ...
    ret=mHalSensor->sendCommand(senDev,SENSOR_CMD_GET_PIXEL_CLOCK_FREQ,(MUINT32)&pclk[0],0,0);
	if(ret<0)
	{
		LOG_INF("get pclk of sensorDev(0x%x) failed\n",senDev);
		return -1;
...
MINT32 HWSyncDrvImp::revertSensorFps()
{
    ...
    MUINT32 periodPara=0x0;
    MUINT32 period[2];
    ret=mHalSensor->sendCommand(mSensorDevArray[mAdjIdx],SENSOR_CMD_GET_FRAME_SYNC_PIXEL_LINE_NUM,(MUINT32)&periodPara,0,0);
 	if(ret<0)
	{
    ...
    LOG_INF("fps_ori(%d),adj_period(%d,%d),period(1_ %d,%d/2_ %d,%d)",fps_ori,mAdjSrInfo.period[0],mAdjSrInfo.period[1],\
        mInitperiodSr1[0],mInitperiodSr1[1],mInitperiodSr2[0],mInitperiodSr2[1]);
    ret=mHalSensor->sendCommand(mSensorDevArray[mAdjIdx],SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO,(MINT32)&(mSensorScenArray[mAdjIdx]),(MUINT32)&fps_ori,0);
	if(ret<0)
	{
...
MINT32 HWSyncDrvImp::doI2CSync()
{
    ...
    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_SET_N3D_CONFIG,(MUINT32)(&senN3Dconf),0,0);
	if(ret<0)
	{
		LOG_ERR("enable n3d fail \n");
		return -3;
	}
    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_SET_N3D_I2C_POS,(MUINT32)(&n3dPos),0,0);
	if(ret<0)
	{
    ...
    ret=mHalSensor->sendCommand(mSensorDevArray[0],SENSOR_CMD_SET_N3D_I2C_TRIGGER,(int)(&n3dI2CTrig1),(int)(&n3dI2CTrig2),0);	
	if(ret<0)
	{
...
MINT32 HWSyncDrvImp::doOneTimeSync()
{
    ...
    SensorN3dDiffCnt mN3DregInfo;
    ret=mHalSensor->sendCommand(mSensorDevArray[mAdjIdx],SENSOR_CMD_GET_SENSOR_N3D_DIFFERENCE_COUNT,(MUINT32)&mN3DregInfo,0,0);
    if(ret<0)
    {
        LOG_ERR("query N3D diff fail \n");
        return -1;
    ...
    ret=mHalSensor->sendCommand(mSensorDevArray[tmpAdjSrIdx],SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO,(MINT32)&(mSensorScenArray[mAdjIdx]),(MUINT32)&fps_final,0);
	if(ret<0)
	{
    ...
    if (gpispDrv->waitIrq(&mWaitIrq) <= 0)
    { LOG_ERR("wait vsync timeout\n"); }
    ret=mHalSensor->sendCommand(mSensorDevArray[tmpAdjSrIdx],SENSOR_CMD_GET_SENSOR_N3D_DIFFERENCE_COUNT,(MUINT32)&mN3DregInfo,0,0);
    if(ret<0)
    {
        LOG_ERR("query N3D diff fail \n");
        return -1;
...
MINT32 HWSyncDrvImp::doGeneralSync()
{
    ...
    do
	{
        ...
        ret=mHalSensor->sendCommand(mSensorDevArray[mAdjIdx],SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO,(MINT32)&(mSensorScenArray[mAdjIdx]),(MUINT32)&fps_final,0);
    	if(ret<0)
    	{
    	    LOG_ERR("set frame rate to sensor failed\n");
    		return -2;
