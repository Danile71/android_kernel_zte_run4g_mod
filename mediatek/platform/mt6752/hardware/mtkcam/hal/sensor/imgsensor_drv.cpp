...
ImgSensorDrv::setScenario(SENSOR_DRIVER_SCENARIO_T scenarioconf)
{
    ...
    for(i=0; i<1; i++) {
        if(SENSOR_SCENARIO_ID_UNNAMED_START != tempId[i]) {//ToDo: remove
            ...
            //FPS
            pFeaturePara16[i] = scenarioconf.InitFPS;
            FeatureParaLen = sizeof(MUINT32);
            ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i], SENSOR_FEATURE_SET_FRAMERATE,  (MUINT8*)pFeaturePara16,(MUINT32*)&FeatureParaLen);
            if (ret < 0) {
            ...
            {
                pFeaturePara16[i] = scenarioconf.HDRMode;
                FeatureParaLen = sizeof(MUINT32);
                ret = featureControl((CAMERA_DUAL_CAMERA_SENSOR_ENUM)tempDevId[i], SENSOR_FEATURE_SET_HDR,  (MUINT8*)pFeaturePara16,(MUINT32*)&FeatureParaLen);
                if (ret < 0) {
                     LOG_ERR("[setScenario]: SENSOR_FEATURE_SET_FRAMERATE error\n");
                     return SENSOR_UNKNOWN_ERROR;
                }
...
{ 
    ...
    case CMD_SENSOR_SET_SENSOR_EXP_TIME:
        ...
        temp = 1000;//Long time exposure

        if(SENSOR_MAIN == sensorDevId) {
            temp = temp *(MUINT64)(*parg1)/m_LineTimeInus[0];//long time exposure
            u4FeaturePara[0] = (MUINT32)temp;
            if(u4FeaturePara[0] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[0]);
                u4FeaturePara[0] = 1;
            }
            FeatureParaLen = sizeof(MUINT32);
            ...
            temp = temp *(MUINT64)(*parg1)/m_LineTimeInus[1];
            u4FeaturePara[0] = (MUINT32)temp;
            if(u4FeaturePara[0] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[1]);
                u4FeaturePara[0] = 1;
            }
            FeatureParaLen = sizeof(MUINT32);
            pFeaturePara = (MUINT8*)&u4FeaturePara[0];
...
//        LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[0]);
//        LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] parg1 = %d\n", *parg1);
        break;

    case CMD_SENSOR_SET_SENSOR_EXP_LINE:
        FeatureId = SENSOR_FEATURE_SET_ESHUTTER;
        u4FeaturePara[0] = (MUINT32)(*parg1);//long time exposure
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];
//        LOG_MSG("[CMD_SENSOR_SET_SENSOR_EXP_LINE] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[0]);
        break;
    ...
    case CMD_SENSOR_SET_MAX_FRAME_RATE_BY_SCENARIO:
        ...
        {        
            case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
                break;
            case SENSOR_SCENARIO_ID_NORMAL_CAPTURE:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG;
                break;
            case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_VIDEO_PREVIEW;
                break;                    
            case SENSOR_SCENARIO_ID_SLIM_VIDEO1:     
                u4FeaturePara[0] = MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO;
                break;                     
            case SENSOR_SCENARIO_ID_SLIM_VIDEO2:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_SLIM_VIDEO;
                break;
			case SENSOR_SCENARIO_ID_CUSTOM1:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CUSTOM1;
                break;
            case SENSOR_SCENARIO_ID_CUSTOM2:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CUSTOM2;
                break; 
            case SENSOR_SCENARIO_ID_CUSTOM3:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CUSTOM3;
                break; 
            case SENSOR_SCENARIO_ID_CUSTOM4:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CUSTOM4;
                break; 
            case SENSOR_SCENARIO_ID_CUSTOM5:
                u4FeaturePara[0] = MSDK_SCENARIO_ID_CUSTOM5;
            default:
                LOG_ERR("[setScenario] error scenario id\n");
                return SENSOR_UNKNOWN_ERROR;
        }
    ...
    case CMD_SENSOR_GET_SENSOR_VIEWANGLE:
     ...
     case CMD_SENSOR_GET_SENSOR_CROP_WIN_INFO:
        ...
        //u4FeaturePara[0] = *parg1; 
        u4FeaturePara[1] = (MUINT32)parg2;         
        FeatureParaLen = sizeof(SENSOR_WINSIZE_STRUCT); 
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];       
        break;   
    ...
    case CMD_SENSOR_GET_SENSOR_VC_INFO:
        ...
        u4FeaturePara[1] = (MUINT32)parg1;  
        FeatureParaLen = sizeof(SENSOR_VC_INFO_STRUCT)+sizeof(MUINT32);
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];         
        break;
    ...
    case CMD_SENSOR_SET_YUV_GAIN_AND_EXP_LINE:
        ...
        u4FeaturePara[0] =(MUINT32)(*parg1); //from 10b to 6b base
        u4FeaturePara[1] = (MUINT32)(*parg2);
        FeatureParaLen = sizeof(MUINT32) * 2;
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];
        break;
    ...
    case CMD_SENSOR_SET_N3D_I2C_STREAM_REGDATA:
		...
		pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32)*4;	//writeI2CID, writeAddr, writePara, writeByte
        pFeaturePara = (MUINT8*)pu4FeaturePara;
		break;
    ...
    case CMD_SENSOR_SET_IHDR_SHUTTER_GAIN:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            u4FeaturePara[0] = (MUINT16)((1000*(*parg1))/m_LineTimeInus[0]);
            u4FeaturePara[1] = (MUINT16)((1000*(*parg2))/m_LineTimeInus[0]);
            if(u4FeaturePara[0] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[0]);  
                u4FeaturePara[0] = 1;          
            }
            if(u4FeaturePara[1] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[1], m_LineTimeInus[0]);  
                u4FeaturePara[1] = 1;          
            }
        }
        else if((SENSOR_MAIN_2 == sensorDevId)||(SENSOR_SUB == sensorDevId)) {
            u4FeaturePara[0] = (MUINT16)((1000*(*parg1))/m_LineTimeInus[1]);
            u4FeaturePara[1] = (MUINT16)((1000*(*parg2))/m_LineTimeInus[1]);
            if(u4FeaturePara[0] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[0], m_LineTimeInus[1]);  
                u4FeaturePara[0] = 1;          
            }
            if(u4FeaturePara[1] ==0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = %d %d\n", u4FeaturePara[1], m_LineTimeInus[1]);  
                u4FeaturePara[1] = 1;          
            }
        }
        else{
        ...
        u4FeaturePara[2]=(MUINT16)(*parg3 >>= 4); //from 10b to 6b base
        
        FeatureParaLen = sizeof(MUINT32) * 3; 
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];  
        break;
