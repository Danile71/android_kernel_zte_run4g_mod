...
dequeBuffer(MUINT32 const data, ImgRequest * pImgReq)
{
    ...
    if(isDequeProvider)
    {
        MUINT32 bufStridesInBytes[] = { GET_IIMGBUF_IMG_STRIDE_IN_BYTE(node.getImgBuf(), 0),
        ...
        MY_LOGD("buf(%p),heap(%p),VA(0x%08X/0x%08X/0x%08X),PA(0x%08X/0x%08X/0x%08X),BS(%d=%d+%d+%d),Id(%d),F(0x%08X),S(%dx%d),Str(%d,%d,%d),R(%d),U(%d)", 
                ...
                pImgReq->mUsage);
        */
        MY_LOGD("buf(%p),VA(0x%08X/0x%08X/0x%08X),BS(%d=%d+%d+%d),Id(%d),F(0x%08X),S(%dx%d),Str(%d,%d,%d),R(%d),U(%d)", 
                pImgReq->mBuffer,
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 0),
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 1),
                GET_IIMAGEBUFFER_BUF_VA(pImgReq->mBuffer, 2),
...
enqueBuffer(MUINT32 const data, IImageBuffer const * pImageBuffer)
{
    ...
    //
    MUINT32 keepAddr = (MUINT32)(mvBufQueNode[bufQueIdx][0].getImgBuf()->getVirAddr());
    MUINT32 enqueAddr = (MUINT32)(GET_IIMAGEBUFFER_BUF_VA(pImageBuffer, 0));
    //
    #if 0
    ...
    if(keepAddr == enqueAddr)
    {
        ...
        {
            #if 0
            MY_LOGD("Node:VA(0x%08X),S(%d),Id(%d)",
                    (MUINT32)keepImgBufQueNode.getImgBuf()->getVirAddr(),
            ...
            */
            sprintf(debugMsg, "buf(%p),VA(0x%08X),BS(%d),Id(%d),TS(%d.%06d),F(0x%08X),S(%dx%d),Str(%d,%d,%d)",
                    pImageBuffer,
                    GET_IIMAGEBUFFER_BUF_VA(pImageBuffer, 0),
                    (   GET_IIMAGEBUFFER_BUF_SIZE(pImageBuffer, 0)+
                        GET_IIMAGEBUFFER_BUF_SIZE(pImageBuffer, 1)+
    ...
    {
        MY_LOGE("Addr(0x%08X != 0x%08X),Port(%d), drop it!",
                keepAddr,
                enqueAddr,
                mvBufQueNode[bufQueIdx][0].getCookieDE());
        return MFALSE;
