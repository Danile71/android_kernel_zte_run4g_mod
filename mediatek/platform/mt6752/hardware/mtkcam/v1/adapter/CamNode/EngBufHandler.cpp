...
//
#include <CamNode/EngBufHandler.h>
//
#if defined(__func__)
#undef __func__
#endif
...
class EngBufHandlerImpl : public EngBufHandler
{
    public:
        EngBufHandlerImpl(
            const char*                 userName,
...
createInstance(
    ...
    sp<ImgBufProvidersManager>  spImgBufProvidersMgr)
{
    return new EngBufHandlerImpl(userName, spImgBufProvidersMgr);
}
...
#endif
    //
    MY_LOGD("bufType(%d),nodeType(%d),bPushFront(%d)",
            bufType,
            nodeType,
            bPushFront);
