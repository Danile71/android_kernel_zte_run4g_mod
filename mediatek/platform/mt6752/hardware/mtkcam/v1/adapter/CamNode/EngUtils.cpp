...
*
*******************************************************************************/
void EngUtil(MINT32 sensorDev, IHal3A* pHal3a, sp<IParamsManager> spParamsMgr)
{
    FUNCTION_IN;
    ...
    { // EV Calibration
        ...
        NS3A::AeMgr::getInstance(sensorDev).CCTOPAEGetCurrentEV(&iAECurrentEV, &iOutLen); // MINT32 AeMgr::CCTOPAEGetCurrentEV(MINT32 *a_pAECurrentEV, MUINT32 *a_pOutLen)

        AE_NVRAM_T AENVRAM;
        AENVRAM.rCCTConfig.i4BVOffset = 0;
        NS3A::AeMgr::getInstance(sensorDev).CCTOPAEGetNVRAMParam(&AENVRAM, &iOutLen); // MINT32 AeMgr::CCTOPAEGetNVRAMParam(MVOID *a_pAENVRAM, MUINT32 *a_pOutLen)
