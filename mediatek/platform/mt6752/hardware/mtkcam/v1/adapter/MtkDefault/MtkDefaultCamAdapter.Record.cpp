...
onHandleStartRecording()
{
    ...
    {
        mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Rec, PASS2_PRV_DST_3, 0, MTRUE);
        //
    ...
    else
    {
        if(mbTwoRunP2)
        {
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Rec, TRANSFORM_DST_1, 0, MTRUE);
        }
        else
        {
            mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Rec, PASS2_PRV_DST_1, 0, MTRUE);
        }
        //
        mpPass2Node->startRecording(
                        vdoWidth,
    ...
    {
        mpDefaultCtrlNode->setMode(DefaultCtrlNode::MODE_VIDEO_RECORD);
        //
        if( mpDefaultCtrlNode->isRecBufSnapshot() ||
...
onHandleStopRecording()
{
    ...
    if(mpDefaultCtrlNode)
    {
        mpDefaultCtrlNode->setMode(DefaultCtrlNode::MODE_VIDEO_PREVIEW);
        if( mpDefaultCtrlNode->isRecBufSnapshot() ||
            (   mbLowPowerVideoRecord == MTRUE &&
                mpParamsMgr->getInt(CameraParameters::KEY_PREVIEW_FRAME_RATE) <= 30))
        {
    ...
    //
    mpDefaultBufHdl->unmapPort(DefaultBufHandler::eBuf_Rec);
    if(mbTwoRunP2)
    {
