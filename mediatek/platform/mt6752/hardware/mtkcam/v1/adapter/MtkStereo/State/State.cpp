 ...
 ******************************************************************************/
StateBase::
StateBase(char const*const pcszName, ENState const eState, IStateManager* pStateMgr)
    : IState()
    , m_pcszName(pcszName)
    , m_eState(eState)
    , m_pStateManager(pStateMgr)
{
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StateIdle::
StateIdle(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StatePreview::
StatePreview(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
onCapture(IStateHandler* pHandler)
{
    ...
    status_t status = OK;
    //
    status = pHandler->onHandlePreviewCapture();
    if  ( OK != status ) {
        goto lbExit;
    }
    //
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StatePreCapture::
StatePreCapture(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StateCapture::
StateCapture(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StatePreviewCapture::
StatePreviewCapture(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
onCaptureDone(IStateHandler* pHandler)
{
    ...
    status_t status = OK;
    //
    status = pHandler->onHandlePreviewCaptureDone();
    if  ( OK != status ) {
        goto lbExit;
    }
    //
...
onCancelCapture(IStateHandler* pHandler)
{
    ...
    status_t status = OK;
    //
    status = pHandler->onHandleCancelPreviewCapture();
    if  ( OK != status ) {
        goto lbExit;
    }
    //
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StateRecording::
StateRecording(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
StateVideoSnapshot::
StateVideoSnapshot(ENState const eState, IStateManager* pStateMgr)
    : StateBase(__FUNCTION__, eState, pStateMgr)
{
}
...
onCaptureDone(IStateHandler* pHandler)
{
    ...
    status_t status = OK;
    //
    status = pHandler->onHandleVideoSnapshotDone();
    if  ( OK != status ) {
        goto lbExit;
    }
    //
...
onCancelCapture(IStateHandler* pHandler)
{
    ...
    status_t status = OK;
    //
    status = pHandler->onHandleCancelVideoSnapshot();
    if  ( OK != status ) {
        goto lbExit;
    }
    //
