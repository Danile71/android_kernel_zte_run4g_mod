...
{ 
...

    {
        ...
        //
        ret = mpMultiShot->sendCommand(NSCamShot::ECamShot_CMD_SET_PREVIEW_BUFHDL, (MINT32)mpPrvBufHandler, 0, 0); 
        if( mu4ShotMode == eShotMode_ContinuousShotCc )
        {
            // zsd flow
            if( mpCapBufMgr != NULL ) {
                ret = mpMultiShot->sendCommand(
                        NSCamShot::ECamShot_CMD_SET_CAPBUF_MGR, (MINT32)mpCapBufMgr, 0, 0);
            } else {
