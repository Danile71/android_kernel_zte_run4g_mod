...
#undef  scenario_case           
    	    ...
    	    mpEngParam->mi4EngSensorMode != EngParam::ENG_SENSOR_MODE_NORMAL_CAPTURE) 
    {
        ...
        mShotParam.mi4PictureWidth = ((mpEngParam->u4SensorWidth/mShotParam.mi4PostviewWidth) * mShotParam.mi4PostviewWidth) & (~0xF); 
        mShotParam.mi4PictureHeight = ((mpEngParam->u4SensorHeight/mShotParam.mi4PostviewHeight) * mShotParam.mi4PostviewHeight) & (~0xF); 
    }
    ...
    {
        ret = setAEEnable(0); 
        ret = setManualAEControl(mpEngParam->u4Exposuretime, mpEngParam->u4SensorGain);
    }
    //
...
bool 
EngShot::
setManualAEControl(MUINT32 u4ExpTime, MUINT32 u4ExpGain)
{
    ...
    rCap3AParam.u4Eposuretime = mpEngParam->u4Exposuretime * 1000; 
    rCap3AParam.u4AfeGain = mpEngParam->u4SensorGain; 
    pHal3A->updateCaptureParams(rCap3AParam); 
    pHal3A->destroyInstance(LOG_TAG); 
    MY_LOGD("manual AE: exp time, gain = (%d, %d)",  rCap3AParam.u4Eposuretime, rCap3AParam.u4AfeGain); 
    return MTRUE; 
}
