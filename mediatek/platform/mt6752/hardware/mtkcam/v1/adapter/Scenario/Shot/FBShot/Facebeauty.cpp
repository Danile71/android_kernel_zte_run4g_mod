        ...
        FACE_BEAUTY_SDK_HAL_PARAMS const* pParam)
{
    ...
    sp<Mhal_facebeauty>  pImpShot = NULL;
    CAM_LOGD("new FBShot");
    pImpShot = new Mhal_facebeauty(SrcImgBuffer, DstImgBuffer);
    if  ( pImpShot == 0 ) {
        CAM_LOGE("[%s] new FBShot \n", __FUNCTION__);
        goto lbExit;
    }
    ...
    //(3)  tuning parameter
    {
        ...
        int LS = pParam->SmoothLevel;
        int LC = pParam->SkinColor;
        int LE = pParam->EnlargeEyeLevel;
        int LF = pParam->SlimFaceLevel;
        LS+=5;
        LC+=5;
        LE+=5;
        LF+=5;
...
*******************************************************************************/
Mhal_facebeauty::
Mhal_facebeauty(IImageBuffer* SrcImgBuffer, IImageBuffer* DstImgBuffer)
...
apply()
{
    ...
    ImgProcess(mpSource, 0, 0, eImgFmt_I422, mDstImgBuffer, 0, 0, eImgFmt_I422, transform);
...
MBOOL
Mhal_facebeauty::
doFaceDetection(IImageBuffer* Srcbufinfo, int u4SrcWidth, int u4SrcHeight, int u4PostviewWidth, int u4PostviewHeight)
{
    ...
    mpFDHalObj->halFDInit(u4SrcWidth, u4SrcHeight, (MUINT32) mpFDWorkingBuffer, FD_WORKING_BUF_SIZE, 1, 3); //3:enhanced for CFB
    ...
    do {
        ...
        mpFDHalObj->halFDDo(0, (MINT32)qVGABuffer, (MINT32)Srcbufinfo->getBufVA(0), false, rotation, Srcbufinfo->getBufPA(0));
