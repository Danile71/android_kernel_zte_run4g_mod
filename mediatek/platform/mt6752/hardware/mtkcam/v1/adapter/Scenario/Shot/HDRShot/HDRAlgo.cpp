...
do_Normalization(void)
{
	...
	{
		rHdrPipeConfigParam.pSourceImgBufAddr[i] = mpSourceImgBuf[i]->getBufVA(0);
		rHdrPipeConfigParam.pSmallImgBufAddr[i] = mpSmallImgBuf[i]->getBufVA(0);
	}
	...
	if(HDR_DEBUG_SAVE_NORMALIZED_SMALL_IMAGE || mDebugMode) {
		...
		{
			char szFileName[100];
			::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "%04d_3_normalized_mpSmallImgBuf[%d]_%dx%d.y", mu4RunningNumber, i, mu4W_small, mu4H_small);
...
do_SE(void)
{
	...
	for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
	{
		rHdrPipeSEInputInfo.pSEImgBufAddr[i] = mpSEImgBuf[i]->getBufVA(0);
	}
...
do_FeatureExtraction(void)
{
	...
	for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
	{
		rHdrPipeFeatureExtractInputInfo.pSmallImgBufAddr[i] = mpSmallImgBuf[i]->getBufVA(0);
	}
	//     Assign working buffer
	rHdrPipeFeatureExtractInputInfo.pWorkingBuffer = mpHdrWorkingBuf->getBufVA(0);
...
do_Alignment(void)
{
	...
	ret = ret 
	    && mpHdrHal->HdrWorkingBufSet(mpHdrWorkingBuf->getBufVA(0)
...
do_OriWeightMapGet(void)
{
	...
	for (MUINT32 i = 0; i < u4OutputFrameNum; i++) {
		...
		if((MUINT32)OriWeight[i]->weight_table_data != mHdrSetBmapInfo.bmap_image_addr[i]) {
			MY_ERR("OriWeight[%d]->weight_table_data=%p, it should be 0x%x"
					, i
					, OriWeight[i]->weight_table_data
					, mHdrSetBmapInfo.bmap_image_addr[i]);
		}
...
do_Fusion(void)
{
	...
	//[ION]
	ret = ret && mpHdrHal->ResultBufferSet(mBlendingBuf->getBufVA(0)
...
do_HdrNR(void)
{
    ...
    {
        char szFileName[100];
        ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrCroppedBuf_%dx%d.i420", mrHdrCroppedResult.output_image_width, mrHdrCroppedResult.output_image_height);
    ...
    MRect cropRect = calCrop(srcRect, dstRect, mShotParam.mu4ZoomRatio);
    ...
    switch(mNrtype) 
    {
        case ECamShot_NRTYPE_NONE:
            { 
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrNoNrBlendingBuf_%dx%d.yuy2", pImageHdrNr->getImgSize().w, pImageHdrNr->getImgSize().h);
        ...
        case ECamShot_NRTYPE_HWNR:
            { 
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrHwNrWorkingBuf1_%dx%d.yuy2", mrHdrCroppedResult.output_image_width, mrHdrCroppedResult.output_image_height);
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrHwNrWorkingBuf2_%dx%d.yuy2", mrHdrCroppedResult.output_image_width, mrHdrCroppedResult.output_image_height);
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrHwNrBlendingBuf_%dx%d.yuy2", pImageHdrNr->getImgSize().w, pImageHdrNr->getImgSize().h);
                    pImageHdrNr->saveToFile(szFileName);
                }
             }   
            break;
        ...
        case ECamShot_NRTYPE_SWNR:
            {
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrSwNrBlendingBuf_%dx%d.i420", pImageCropped->getImgSize().w, pImageCropped->getImgSize().h);
                    pImageCropped->saveToFile(szFileName);
                }
                ...
                {
                    char szFileName[100];
                    ::sprintf(szFileName, HDR_DEBUG_OUTPUT_FOLDER "8_NR_HdrSwNrForJpegBuf_%dx%d.yuy2", pImageHdrNr->getImgSize().w, pImageHdrNr->getImgSize().h);
                    pImageHdrNr->saveToFile(szFileName);
                }
            }
            break;
