...
requestSmallImgBuf(void)
{
	...
	switch (mu4OutputFrameNum)	// Allocate buffers according to u4OutputFrameNum, note that there are no "break;" in case 3/case 2.
	{
		...
		case 1: ret = ret && allocBuffer(&mpSmallImgBuf[0], mu4W_small, mu4H_small, eImgFmt_Y800);
	}
lbExit:
	if(!ret) {
		releaseSmallImgBuf();
...
requestOriWeightMapBuf(void)
{
	...
	for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
	{
		...
		mHdrSetBmapInfo.bmap_image_addr[i] = mWeightingBuf[i]->getBufVA(0);
		MY_LOGD("[requestOriWeightMapBuf] addr[%d] = 0x%x", i, mWeightingBuf[i]->getBufVA(0));
        #else
...
createSourceAndSmallImg(void)
{
    ...
    ret = ret && pBurstShot->setIspProfile( mNrtype==ECamShot_NRTYPE_SWNR ? EIspProfile_Capture_SWNR : EIspProfile_Capture );
    if(ret) {
...
allocateCaptureMemoryTask_First(MVOID* arg)
{
    ...
    XLOGD("Event_HdrShot_AllocMem, CPTFlagStart");
...
lbExit:
    ...
    XLOGD("Event_HdrShot_AllocMem, CPTFlagEnd");
...
init_compoment()
{
	...
	for(MUINT32 i=0; i<OutputFrameNumGet(); i++) {
		...
		rHdrPipeInitInfo.pSourceImgBufAddr[i] = 0;
	}
    ...
    CHECK_OBJECT("HdrHalBase::createInstance fail.");
...
makeBufferAlias(IImageBuffer *pBaseBuf, MINT32 w, MINT32 h, MINT32 imgFormat)
{
    ...
    // create new buffer
    pBuf = pBufHeap->createImageBuffer_FromBlobHeap(0
                                                    , imgFormat
                                                    , imgSize
                                                    , bufStridesInBytes
                                                    );
