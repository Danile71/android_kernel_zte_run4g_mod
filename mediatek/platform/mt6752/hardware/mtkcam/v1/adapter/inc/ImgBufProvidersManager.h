    ...
    inline String8                  queryFormat(int32_t const i4PvdrId) const
                                    {
                                        ...
                                        if  ( pImgBufPvdr != 0 ) {
                                            if ( pImgBufPvdr->queryProvider(node) ) {
                                                s8Format = node.getImgBuf()->getImgFormat();
                                            }
                                            else {
                                                pImgBufPvdr->queryFormat(s8Format);
                                            }
                                        }
                                        return  s8Format;
                                    }
...
public:     ////                    Operations.
    ...
    //  get [Face Detection] Image Buffer Provider
    sp<IImgBufProvider> const       getOTBufPvdr() const    { return getProvider(IImgBufProvider::eID_OT); }
    ...
    //  get [Sdk Gesture Client] Image Buffer Provider 
    sp<IImgBufProvider> const       getAPClientBufPvdr() const {return getProvider(IImgBufProvider::eID_AP_CLIENT); }
