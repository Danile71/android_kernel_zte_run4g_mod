...
static int getIndex(PQ_DC_index_t a_eIndex)
{
    ...
    if (drvID < 0)
    {
        XLOGE("[PQDC_JNI] open /proc/mtk_mira fail...");
    }


    XLOGD("[PQDC_JNI] getIndex[%d]", (int)a_eIndex);

    ioctl(drvID, DISP_IOCTL_PQ_GET_DC_PARAM, &dcparam);
...
static jboolean setIndex(PQ_DC_index_t a_eIndex , unsigned int a_u4Index)
{
    ...
    if (drvID < 0)
    {
        XLOGE("[PQ] open /proc/mtk_mira fail...");
        return JNI_FALSE;
    }

    XLOGD("[PQDC_JNI] setIndex[%d], value[%u]", (int)a_eIndex, a_u4Index);

    ioctl(drvID, DISP_IOCTL_PQ_GET_DC_PARAM, &dcparam);

    dcparam.param[a_eIndex] = a_u4Index;

    ioctl(drvID, DISP_IOCTL_PQ_SET_DC_PARAM, &dcparam);
...
static jboolean setTuningMode(JNIEnv *env, jobject thiz, int mode)
{
    ...
    XLOGD("[PQDC_JNI] setTuningMode[%d]", mode);
    ...
    ioctl(drvID, DISP_IOCTL_PQ_SET_TDSHP_FLAG, &tdshp_flag);
...
//JNI register
////////////////////////////////////////////////////////////////
static const char *classPathName = "com/mediatek/gallery3d/pq/dcfilter/DCFilter";
