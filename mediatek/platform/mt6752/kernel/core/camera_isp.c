...
static MINT32 ISP_DumpReg(void)
{
    ...
    RegDump(0xe00,0xe30);
    //Mipi source
    LOG_DBG("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(0x10217000), (unsigned int)ISP_RD32(0xf0217000),(unsigned int)(0x10217004), (unsigned int)ISP_RD32(0xf0217004));
    LOG_DBG("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(0x10217008), (unsigned int)ISP_RD32(0xf0217008),(unsigned int)(0x1021700c), (unsigned int)ISP_RD32(0xf021700c));
    LOG_DBG("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(0x10217030), (unsigned int)ISP_RD32(0xf0217030),(unsigned int)(0x10219030), (unsigned int)ISP_RD32(0xf0219030));
                    ...
                    if ( CurVF_En ) {
                        if(_bFlag == MTRUE) {
                            ...
                            if ( 0 != rt_buf_ctrl.ex_data_ptr ) {
                                ...
                                if(copy_from_user(&deque_buf.data[0], (void*)rt_buf_ctrl.ex_data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0) {
                                    //
                                    i = 0;
                                ...
                                for (i=0;i<ISP_RT_BUF_SIZE;i++) {
                                    //
                                    if ( pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr == rt_buf_info.base_pAddr ) {
                                        //LOG_DBG("[rtbc]dma(%d),idx(%d) PA(0x%x) VA(0x%x)",rt_dma,i,pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr,pstRTBuf->ring_buf[rt_dma].data[i].base_vAddr);
                                        //spin_lock_irqsave(&(IspInfo.SpinLockIrq[irqT]), flags);
                                        pstRTBuf->ring_buf[rt_dma].data[i].bFilled = ISP_RTBC_BUF_EMPTY;
                                        pstRTBuf->ring_buf[rt_dma].data[i].image.frm_cnt = _INVALID_FRM_CNT_;
#ifdef _rtbc_buf_que_2_0_
                                ...
                                if (i == ISP_RT_BUF_SIZE) {
                                    ...
                                    for (x=0;x<ISP_RT_BUF_SIZE;x++) {
                                        LOG_DBG("[rtbc]dma(%d),idx(%d) PA(0x%x) VA(0x%x)",rt_dma,x,pstRTBuf->ring_buf[rt_dma].data[x].base_pAddr,pstRTBuf->ring_buf[rt_dma].data[x].base_vAddr);
                                    }
                                    //LOG_ERR("[rtbc]insert address PA(0x%x), VA(0x%x)",rt_buf_info.base_pAddr,rt_buf_info.base_vAddr);
                    ...
                    if(MTRUE == _bFlag) {
                        ...
                        //for (i=0;i<deque_buf.count;i++) {
                            ...
                            if ( (_INVALID_FRM_CNT_ == deque_buf.data[i].image.frm_cnt) || \
                                 (m_LastMNum[rt_dma] > deque_buf.data[i].image.m_num_0) ) {
                                 //
                            ...
                            if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
                                LOG_DBG("[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(0x%x)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d)/m(0x%x)/fc(%d)/rrz(%d,%d,%d,%d,%d,%d),lm#(0x%x)", \
                                    iBuf+i,\
                                    rt_dma, \
                            ...
                            if ( 0 != prv_tstamp_s[rt_dma] ) {
                                if ( ( prv_tstamp_s[rt_dma] > deque_buf.data[i].timeStampS) ||
                                    ...
                                    LOG_ERR("[rtbc]TS rollback,D(%d),prv\"%d.%06d\",cur\"%d.%06d\"",rt_dma,prv_tstamp_s,prv_tstamp_us,deque_buf.data[i].timeStampS,deque_buf.data[i].timeStampUs);
                                }
                            }
...
static MINT32 ISP_SOF_Buf_Get(eISPIrq irqT, CQ_RTBC_FBC* pFbc,MUINT32* pCurr_pa, unsigned long long sec,unsigned long usec,MBOOL bDrop)
{
    ...
    //LOG_INF("RTBC_DBG7 rrzo:%d %d %d\n",pstRTBuf->ring_buf[_rrzo_].data[0].bFilled,pstRTBuf->ring_buf[_rrzo_].data[1].bFilled,pstRTBuf->ring_buf[_rrzo_].data[2].bFilled);
        ...
        if(_IRQ == irqT) {
            ...
            pstRTBuf->ring_buf[ch_imgo].data[imgo_idx].image.h = ISP_RD32(ISP_INNER_REG_ADDR_IMGO_YSIZE)&0x1FFF + 1;
            ...
            pstRTBuf->ring_buf[ch_imgo].data[rrzo_idx].image.w = pstRTBuf->ring_buf[ch_imgo].data[imgo_idx].image.w;
            pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].image.h = ISP_RD32(ISP_INNER_REG_ADDR_RRZO_YSIZE)&0x1FFF + 1;
            ...
            //LOG_INF("rrz src111--(%d,%d,%d,%d)=-_tmp(0x%x)", \
            //    pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcX,pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcY,\
            //    pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcW,pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcH,_tmp);
            ...
            pstRTBuf->ring_buf[ch_imgo].data[imgo_idx].image.w = ((_tmp>>16)&0x7FFF) - _tmp&0x7FFF;
            pstRTBuf->ring_buf[ch_imgo].data[imgo_idx].image.h = ISP_RD32(ISP_INNER_REG_ADDR_IMGO_D_YSIZE)&0x1FFF + 1;
            ...
            pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].image.h = ISP_RD32(ISP_INNER_REG_ADDR_RRZO_D_YSIZE)&0x1FFF + 1;
            ...
            //LOG_INF("rrz22222 src(%d,%d,%d,%d),_tmp(0x%x)", \
            //    pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcX,pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcY,\
            //    pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcW,pstRTBuf->ring_buf[ch_rrzo].data[rrzo_idx].rrzInfo.srcH,_tmp);
...
static MINT32 ISP_DONE_Buf_Time(eISPIrq irqT, CQ_RTBC_FBC* pFbc ,unsigned long long sec,unsigned long usec)
{
    ...
    //MUINT32 reg_val = 0;
    MUINT32 ch_imgo,ch_rrzo;//, ch_imgo_fbc, ch_rrzo_fbc; mark , becouase fbc can't read at local
    CQ_RTBC_FBC imgo_fbc;
    CQ_RTBC_FBC rrzo_fbc;
...
static MINT32 ISP_WaitIrq(ISP_WAIT_IRQ_STRUCT* WaitIrq)
{
    ...
    {
        spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
        LOG_ERR("WaitIrq Timeout Clear(%d), Type(%d), IrqStatus(0x%08X), WaitStatus(0x%08X), Timeout(%d)",
                                    WaitIrq->Clear,
                                    WaitIrq->Type,
    ...
    void*   DeviceId)
{
    ...
    if(IspInfo.DebugMask & ISP_DBG_INT) {
        IRQ_LOG_KEEPER(_CAMSV_IRQ,m_CurrentPPB,_LOG_INF,CAMSV_TAG"Type(%d), IrqStatus(0x%x | 0x%x)\n",ISP_IRQ_TYPE_INT_CAMSV,(unsigned int)IspInfo.IrqInfo.Status[ISP_IRQ_TYPE_INT_CAMSV], (unsigned int)(IrqStatus_CAMSV));
    }
    ...
    void*   DeviceId)
{
    ...
    if(IspInfo.DebugMask & ISP_DBG_INT)
    {
        IRQ_LOG_KEEPER(_CAMSV_D_IRQ,m_CurrentPPB,_LOG_INF,CAMSV2_TAG"Type(%d), IrqStatus(0x%08X | 0x%08x)\n",ISP_IRQ_TYPE_INT_CAMSV2,(unsigned int)IspInfo.IrqInfo.Status[ISP_IRQ_TYPE_INT_CAMSV2], (unsigned int)(IrqStatus_CAMSV2));
    }
...
//    LOG_DBG("- E.");
    ...
    mt_kernel_trace_begin("isp_irq_cam");
    ...
    {
        //met_tag_start(0x2002, "ISP_EOF");//systrace

        //mt_kernel_trace_begin("P1_DONE");
#ifdef _rtbc_buf_que_2_0_
        ISP_DONE_Buf_Time(_IRQ,p1_fbc, 0,0);//time stamp move to sof
        if(IspInfo.DebugMask & ISP_DBG_INT) {
    ...
    #endif
#endif
        //met_tag_end(0x2002, "ISP_EOF");//systrace
        //mt_kernel_trace_end();
    }
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] & ISP_IRQ_P1_STATUS_SOF1_INT_ST)
    {
        ...
        //mt_kernel_trace_begin("P1_SOF");
        //met_tag_start(0x2002, "ISP_SOF");//systrace
        ...
        }
        {
            ...
            ISP_SOF_Buf_Get(_IRQ, p1_fbc, curr_pa, sec,usec,sof_pass1done[0]);    
        }
        //met_tag_end(0x2002, "ISP_SOF");//systrace
        //mt_kernel_trace_end();
    ...
    {
        //mt_kernel_trace_begin("P1_D_DONE");
        if(IspInfo.DebugMask & ISP_DBG_INT) {
...
#ifdef _rtbc_buf_que_2_0_
        ISP_DONE_Buf_Time(_IRQ_D,p1_fbc, 0,0);//time stamp move to sof
#endif
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT_P1_ST_D] & ISP_IRQ_P1_STATUS_D_SOF1_INT_ST)
    {
        MUINT32 _dmaport = 0;
        MUINT32 rt_dma = 0;
        //mt_kernel_trace_begin("P1_D_SOF");
        ...
        }
        {
            ...
            //
            ISP_SOF_Buf_Get(_IRQ_D,p1_fbc,curr_pa,sec,usec,sof_pass1done[1]);
        }
        //mt_kernel_trace_end();
    }
    //
    ...
    unsigned long   Param)
{
    ...
    switch(Cmd)
    {
        ...
        case ISP_GET_CUR_SOF:
            if(copy_to_user((void*)Param, &sof_count[_IRQ], sizeof(MUINT32)) != 0){
    ...
    void*   pData)
{
    ...
    MINT32 ret = 0;
    //
    LOG_DBG("- E. pPage: 0x%08x. off: %d. Count: %d.", (int)pPage, (int)off, Count);
    //
    ...
    void*           pData)
{
    ...
    if (sscanf(RegBuf, "%x %x",  &Addr, &Data) == 2)
    {
        ...
        LOG_DBG("Write => Addr: 0x%08X, Write Data: 0x%08X. Read Data: 0x%08X.", (int)(GPIO_BASE + Addr), (int)Data, (int)ioread32(GPIO_BASE + Addr));
    }
    else if (sscanf(RegBuf, "%x", &Addr) == 1)
    {
        proc_regOfst = Addr;
        LOG_DBG("Read => Addr: 0x%08X, Read Data: 0x%08X.", (int)(GPIO_BASE + Addr), (int)ioread32(GPIO_BASE + Addr));
    }
    //
    LOG_DBG("- X. Count: %d.", (int)Count);
    return Count;
...
static uint32_t *addressToDump[10] = {
#if 0
