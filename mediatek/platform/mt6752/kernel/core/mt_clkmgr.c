...
//        .base_addr = CLK_CFG_2,
        ...
        .name = __stringify(MUX_MSDC30_1),//10
//        .base_addr = CLK_CFG_3,
        .sel_mask = 0x07000000,
        .pdn_mask = 0x80000000,
        .upd_mask = 1<<14,
        ...
        .name = __stringify(MUX_MSDC50_0),//11
//        .base_addr = CLK_CFG_3,
        .sel_mask = 0x000F0000,
        .pdn_mask = 0x00800000,
        .upd_mask = 1<<13,
        ...
        .name = __stringify(MUX_MSDC50_0_hclk),//12
//        .base_addr = CLK_CFG_3,
        .sel_mask = 0x00000300,
        .pdn_mask = 0x00008000,
        .upd_mask = 1<<12,
...
//        .base_addr = CLK_CFG_6,
        ...
        .name = __stringify(MT_MUX_USB),//24
        //.sel_mask = 0x00000007,
        //.pdn_mask = 0x00000080,
        //.upd_mask = 1<<23,
        //.offset = 0,
...
//    volatile unsigned int reg;
    ...
    clk_writel(CLK_CFG_UPDATE, mux->upd_mask);
#else
...
static int infra0_clk_enable_op(struct cg_clk *clk)
{
    ...
    if(clk->mask == 0x04000000)
        clk_writel(clk->grp->clr_addr, (1<<MT_CG_INFRA_USB_MCU)|(1<<MT_CG_INFRA_ICUSB)|(1<<MT_CG_INFRA_USB));
    else
        clk_writel(clk->grp->clr_addr, clk->mask);
    return 0;
}
...
static int infra0_clk_disable_op(struct cg_clk *clk)
{
    ...
    if(clk->mask == 0x04000000)
        clk_writel(clk->grp->set_addr, (1<<MT_CG_INFRA_USB_MCU)|(1<<MT_CG_INFRA_ICUSB)|(1<<MT_CG_INFRA_USB));	
    else
        clk_writel(clk->grp->set_addr, clk->mask);
    return 0;
}
...
static int infra1_clk_disable_op(struct cg_clk *clk)
{
    ...
    if (clk->mask == 0x40000) {
        ...
        if(flag_md32_addr != 0) {
            ...
            //if(*(volatile unsigned int*)flag_md32_addr == 0)
            if(cnt == 0)
                clk_writel(clk->grp->set_addr, clk->mask);
...
return 0;
    ...
    if((id == MT_CG_INFRA_USB) || (id == MT_CG_INFRA_ICUSB))
    {
...
#define INFRA0_CG  0xFFFFFFFF
#define INFRA1_CG  0xFFFFFFFF
#define AUD_CG    0x000C03C4
...
{
    //INFRA CG
    clk_writel(INFRA_PDN_CLR0, INFRA0_CG);
    clk_writel(INFRA_PDN_CLR1, INFRA1_CG);
...
static void cg_bootup_pdn(void)
{
    ...
    //INFRA CG
    clk_writel(INFRA_PDN_SET0, 0x8427B180);
    clk_writel(INFRA_PDN_SET1, 0x6f0876); //apdma default PDN
...
static void mt_muxs_init(void)
{
    ...
    //clk_writel(CLK_CFG_0+4, 0x800000); //pdn ddrphycfg
    //clk_writel(CLK_CFG_5+4, 0x8000); //pdn scp 
    clk_writel(CLK_CFG_6+4, 0x8000); //pdn scam
    clk_writel(CLK_CFG_UPDATE, 1<<24);
}
...
static void mt_clks_init(void)
{
    ...
    clks[MT_CG_INFRA_USB_MCU].mux = &muxs[MT_MUX_USB];

    clks[MT_CG_INFRA_MSDC_0].mux = &muxs[MT_MUX_MSDC50_0];
    clks[MT_CG_INFRA_MSDC_1].mux = &muxs[MT_MUX_MSDC30_1];
    clks[MT_CG_INFRA_MSDC_2].mux = &muxs[MT_MUX_MSDC30_2];
    clks[MT_CG_INFRA_MSDC_3].mux = &muxs[MT_MUX_MSDC30_3];
...
bool clkmgr_idle_can_enter(unsigned int *condition_mask, unsigned int *block_mask, enum idle_mode mode)
{
    ...
    if (mode == dpidle) {
        if (sta & (MFG_PWR_STA_MASK | ISP_PWR_STA_MASK | VDE_PWR_STA_MASK | MJC_PWR_STA_MASK | VEN_PWR_STA_MASK | DIS_PWR_STA_MASK))
            return false;
    } else if (mode == soidle) {
    	if (sta & (MFG_PWR_STA_MASK | ISP_PWR_STA_MASK | VDE_PWR_STA_MASK | MJC_PWR_STA_MASK | VEN_PWR_STA_MASK))
            return false;
    }
#endif
    return true;
...
{
    mt_enable_clock(MT_CG_INFRA_SPI, "clkmgr");
    mt_disable_clock(MT_CG_INFRA_SPI, "clkmgr");

    mt_enable_clock(MT_CG_INFRA_MSDC_2, "clkmgr");
    mt_disable_clock(MT_CG_INFRA_MSDC_2, "clkmgr");
    mt_enable_clock(MT_CG_INFRA_MSDC_3, "clkmgr");
    mt_disable_clock(MT_CG_INFRA_MSDC_3, "clkmgr");
    ...
    enable_pll(VENCPLL, "clkmgr");
    disable_pll(VENCPLL, "clkmgr");
