...
//      parse_tag_videofb_fixup(tags);
    ...
    if (mdesc_best)
    {
        u32 i = get_devinfo_with_index(3);
        u32 j = get_devinfo_with_index(4);
        //u32 k = get_devinfo_with_index(28);
		
        if(j & ((1<<0)|(1<<1)|(1<<15))){
            switch ((i >> 8) & 0xF)
            {
                case 0xF:
                strcpy((char *)mdesc_best->name, "MT8165");
                break;
                case 0x0:
                strcpy((char *)mdesc_best->name, "MT8182");
                break;
                default:
                break;
            }
            ...
            switch ((i >> 8) & 0xF)
            {
                case 0xF:
                if((i & 0xF) == 0x4)
                    strcpy((char *)mdesc_best->name, "MT8732T");
                else
                    strcpy((char *)mdesc_best->name, "MT8732");
                break;
                case 0x0:
                strcpy((char *)mdesc_best->name, "MT8752");
                break;
                default:
                break;
            }
		...
		if(i & (1 << 31))
			strcpy((char *)mdesc_best->name, "MT8752T");
    }
#endif
...
static void display_early_memory_info(void)
{
	int node, i;
...
{
#ifdef CONFIG_OF
	display_early_memory_info();
#endif /* end of CONFIG_OF */
#if 0
	memblock_reserve(0x4e200000, 0x1e00000);;
	memblock_free(0x4e200000, 0x1e00000);
	memblock_remove(0x4e200000, 0x1e00000);
#endif
	memblock_reserve(0x5fdb5000, 0x1000);
