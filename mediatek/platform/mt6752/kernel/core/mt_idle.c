...
#define idle_setl(addr, val) \
    mt_reg_sync_writel(idle_readl(addr) | (val), addr)
...
static unsigned int dpidle_condition_mask[NR_GRPS] = {
    ...
    0xffffffff, //DISP0:
    0x000003ff, //DISP1:
    0x00000be1, //IMAGE,use SPM MTCMOS off as condition:
    0x00000001, //MFG,use SPM MTCMOS off as condition:
    ...
    0x0000002f, //MJC,use SPM MTCMOS off as condition:
    0x00001111, //VENC_JPEG,use SPM MTCMOS off as condition:
};
#else//temporary bypass block cg
static unsigned int dpidle_condition_mask[NR_GRPS] = {
    0x00500900, //INFRA0:
    0x00273020, //INFRA1:
    0xffffffff, //DISP0:
...
#else //skip some not gating clock 
static unsigned int soidle_condition_mask[NR_GRPS] = {
    0x07273980, //INFRA0:
    0x00440002, //INFRA1:
    0xffe023fc, //DISP0:
...
bool soidle_can_enter(int cpu)
{
	...
	unsigned int mempllState = mt_get_clk_mem_sel();
	...
	//check current mempll mode
	//// 2:1pll , 1:3pll, 0: 26M
	if(mempllState == 2) 
	{	
		//printk("sodi return : 1pll\n");
		reason = BY_OTH;
			goto out;
	}
...
void soidle_after_wfi(int cpu)
{
    ...
    soidle_cnt[cpu]++;
#if SODI_DVT_APxGPT
	//printk("soidle_cnt:%d",soidle_cnt[cpu]);
#endif
}
...
do {    \
    ...
    idle_writel(CLK_CFG_UPDATE, 1<<18);  \
} while (0);
