...
static void hal_dma_set_default_setting(ENUM_DMA_DIR dma_dir)
{
	...
	if(DMA_DIR_RX == dma_dir){
		...
		if (of_property_read_u32_index(node, "reg", 0, &phy_base)){
			...
			BTIF_INFO_FUNC("get register phy base dma_dir(%d)(0x%lx)\n",
				dma_dir,(unsigned long)phy_base);
		}
...
/*if write 1 to STOP bit, DMA will be stoped after current transaction finished*/
		...
		while((0x1 & BTIF_READ32(TX_DMA_STOP(base))))
		{	
			if(++counter > 10000)
			{
				BTIF_ERR_FUNC("BTIF DMA RX STOP BIT can not reset 0,maybe DMA bus hang...\n");
...
/*if write 1 to STOP bit, DMA will be stoped after current transaction finished*/
		...
		while((0x1 & BTIF_READ32(RX_DMA_STOP(base))))
		{	
			if(++counter > 10000)
			{
				BTIF_ERR_FUNC("BTIF DMA RX STOP BIT can not reset 0,maybe DMA bus hang...\n");
...
int _btif_dma_dump_dbg_reg(void)
{
	...
	addr2 = ioremap(g_dma_dbg_regs[5].reg_addr, 0x04);
	g_dma_dbg_regs[5].reg_val = *(volatile unsigned int*)(addr2);
	iounmap(addr2);
	
	addr3 = ioremap(g_dma_dbg_regs[6].reg_addr, 0x10);
    if (addr3)
	{
        g_dma_dbg_regs[6].reg_val = *(volatile unsigned int*)(addr3);
        g_dma_dbg_regs[7].reg_val = *(volatile unsigned int*)(addr3+4);
        g_dma_dbg_regs[8].reg_val = *(volatile unsigned int*)(addr3+8);
        iounmap(addr3);
	  }

    for (i = 0; i < array_num; i++)
    {
        BTIF_INFO_FUNC("<reg, val>-<0x%08x, 0x%08x>\n", g_dma_dbg_regs[i].reg_addr, g_dma_dbg_regs[i].reg_val);
    }
    return 0;
}
