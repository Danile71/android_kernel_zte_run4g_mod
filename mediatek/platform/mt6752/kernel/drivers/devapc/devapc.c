...
static unsigned int dapc_irq = 0;
...
{    
    if(dapc_irq !=0){
        mt_irq_set_sens(dapc_irq, MT_LEVEL_SENSITIVE);
        mt_irq_set_polarity(dapc_irq, MT_POLARITY_LOW);
    }
 ...
 * no return value.
 */
extern void __clear_emi_mpu_vio(void);
...
{  
    ...
    else
    {
     ...
     xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG , "D0_VIO_STA_0:0x%x ,D0_VIO_STA_1:0x%x , D0_VIO_STA_2:0x%x, D0_VIO_STA_3:0x%x, D0_VIO_STA_4:0x%x, R\n", 
      readl(DEVAPC0_D0_VIO_STA_0 ), readl(DEVAPC0_D0_VIO_STA_1 ), readl(DEVAPC0_D0_VIO_STA_2 ),readl(DEVAPC0_D0_VIO_STA_3 ),readl(DEVAPC0_D0_VIO_STA_4 ));
    ...
    if ((dbg0 !=0) || (dbg1 !=0)) 
    {
        xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG ,"[DEVAPC] Multi-violation!\n");
        xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG ,"[DEVAPC] DBG0 = %x, DBG1 = %x\n", dbg0, dbg1);
    }
...
static int devapc_probe(struct platform_device *dev)
{
    ...
    int ret;
    xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC] module probe. \n");
    ...
    if(node){
       ...
       xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC] AO_ADDRESS %x \n",DEVAPC0_AO_BASE );
    } else{
         xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC] can't find DAPC_AO compatible node \n");
         return -1;
    }
    ...
    if(node){
       ...
       xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC] PD_ADDRESS %x, IRD: %d \n",DEVAPC0_PD_BASE,dapc_irq );
    } else{
         xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC] can't find DAPC_PD compatible node \n");
         return -1;
    }
