...
#define MIPITX_OUTREG32(addr, val) \
  	{\
  		if(dsi_reg_op_debug) \
		{	printk("[mipitx/reg]0x%lx=0x%08x\n", addr, val);}\
...
#define MIPITX_OUTREG32(addr, val) \
  	{\
  		if(dsi_reg_op_debug) \
		{	\
			printk("[mipitx/reg]0x%08x=0x%08x\n", addr, val);\
		}\
...
static void DSI_WaitForNotBusy(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	while(1)
	{
		...
		{
			DISPERR("dsi wait not busy timeout,start at 0x%d, end at 0x%d\n", start_time, end_time);
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
	...
	for(d = DSI_MODULE_BEGIN(module);d <= DSI_MODULE_END(module);d++)
	{
		...
		do
		{
			...
			/// 4: read data
			ret = wait_event_interruptible_timeout(_dsi_dcs_read_wait_queue[d], ready_r_ack, WAIT_TIMEOUT);
            ready_r_ack = 0;
            if(ret == 0)
			{
				DISPERR("dsi wait read ready ack timeout\n");
				DSI_DumpRegisters(module, 2);
            ...
            } while(DSI_REG[d]->DSI_INTSTA.BUSY && (timeout<10000));
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			DISPCHECK("[DSI]enter cmp read_data0=0x%x\n",read_data0);
			DISPCHECK("[DSI]enter cmp check_table=0x%x\n",dsi_params->lcm_esd_check_table[i]);
