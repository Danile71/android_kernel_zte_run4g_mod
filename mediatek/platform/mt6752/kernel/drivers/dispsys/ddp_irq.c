...
int disp_unregister_module_irq_callback(DISP_MODULE_ENUM module, DDP_IRQ_CALLBACK cb)
{
    ...
    if (i == DISP_MAX_IRQ_CALLBACK)
    {
        DDPERR("Try to unregister callback function with was not registered. module=%d cb=0x%08X\n", module, (unsigned int)cb);
        return -1;
    }
    return 0;
}
    ...
    //switch(irq)
    {
        ...
        else if(irq==dispsys_irq[DISP_REG_WDMA0] || irq==dispsys_irq[DISP_REG_WDMA1])
		{
                ...
                if(reg_val&(1<<0))
                {
                    DDPIRQ("IRQ: WDMA%d frame done!\n",index);
		 		    if(index == 0)
	                    MMProfileLogEx(ddp_mmp_get_events()->ovl_trigger, MMProfileFlagEnd, 0, 0);
                }
                if(reg_val&(1<<1))
                {
        ...
        else if(irq==dispsys_irq[DISP_REG_RDMA0] || irq==dispsys_irq[DISP_REG_RDMA1])
		{
                ...
                if(reg_val&(1<<1))
                {
                      ...
                      {
                          // reset ovl by cmdq
                          disp_irq_log_module = (1<<DISP_MODULE_UNKNOWN);
                ...
                if(reg_val&(1<<4))
                {
                      DDPMSG("r_in_p_cnt=%d, r_in_l_cnt=%d, r_out_p_cnt=%d, r_out_l_cnt=%d, ovl0_x=%d, ovl0_y=%d, ovl1_x=%d, ovl1_y=%d \n",
                                   ...
                                   DISP_REG_GET_FIELD(ADDCON_DBG_FLD_ROI_X, DISP_REG_OVL_ADDCON_DBG),
                                   DISP_REG_GET_FIELD(ADDCON_DBG_FLD_ROI_Y, DISP_REG_OVL_ADDCON_DBG),
                                   DISP_REG_GET_FIELD(ADDCON_DBG_FLD_ROI_X, DISP_REG_OVL_ADDCON_DBG+DISP_OVL_INDEX_OFFSET),
                                   DISP_REG_GET_FIELD(ADDCON_DBG_FLD_ROI_Y, DISP_REG_OVL_ADDCON_DBG+DISP_OVL_INDEX_OFFSET));
                      DDPERR("IRQ: RDMA%d underflow! cnt=%d \n",index, cnt_rdma_underflow[index]++);
                      disp_irq_log_module |= 1<<module;
                      //disp_irq_log_module |= 1<<DISP_MODULE_OVL0;
                      //disp_irq_log_module |= 1<<DISP_MODULE_OVL1;
        ...
        else if(irq==dispsys_irq[DISP_REG_MUTEX])
		{
            ...
            for(mutexID = 0; mutexID<5; mutexID++)
            {
                if(reg_val & (0x1<<mutexID))
                {
		 		   ...
		 		   if(mutexID == 1)
	                    MMProfileLogEx(ddp_mmp_get_events()->ovl_trigger, MMProfileFlagStart, 0, 0);
                }
...
extern int primary_display_diagnose(void);
