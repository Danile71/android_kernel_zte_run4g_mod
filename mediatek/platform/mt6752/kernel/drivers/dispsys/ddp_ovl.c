...
static char* ovl_get_status_name(DISP_OVL1_STATUS status)
{
    switch(status)
    {
        case DDP_OVL1_STATUS_IDLE: return "idle";
        case DDP_OVL1_STATUS_PRIMARY: return "primary";
        case DDP_OVL1_STATUS_SUB: return "sub";
        case DDP_OVL1_STATUS_SUB_REQUESTING: return "sub_requesting";
        case DDP_OVL1_STATUS_PRIMARY_RELEASED: return "primary_released";
        case DDP_OVL1_STATUS_PRIMARY_RELEASING: return "primary_releasing";
        default: return "unknown";
    }
}
// get ovl1 status
DISP_OVL1_STATUS ovl_get_status()
{
    return ovl1_status;
}
		...
		void *handle)
{
	...
	DDPDBG("ovl%d, layer=%d, source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d,"
		"fmt=%s, addr=%lx, keyEn=%d, key=%d, aen=%d, alpha=%d,sur_aen=%d,sur_alpha=0x%x, constant_color=0x%x, yuv_range=%d\n",
		idx,
		layer,
		(source==0)?"memory":"dim", 
		src_x,
...
static unsigned int ovl1_clock_cnt = 0;
int  ovl_clock_on(DISP_MODULE_ENUM module, void *handle)
{
	int idx = ovl_index(module);
	DDPDBG("ovl%d_clock_on\n", idx);
#ifdef ENABLE_CLK_MGR
	enable_clock(MT_CG_DISP0_DISP_OVL0+idx, "DDP");
	if(idx==1) 
	{ 
	    ovl1_clock_cnt++; 
	}	
...
#ifdef ENABLE_CLK_MGR
    if(idx==1 && ovl1_clock_cnt==0) 
    {
        DDPERR("ovl_deinit, clock off OVL1, but OVL1 is already in clock off status! \n");
        return 0;
    }
	disable_clock(MT_CG_DISP0_DISP_OVL0+idx, "DDP");
	if(idx==1) { ovl1_clock_cnt--; }
#endif
	return 0;
}
			...
			void *handle)
{
    ...
    if(cnt<=4) //less than 5 layers, just use ovl0
    {
        *layer_min = 0;
        *layer_max = 4;
    }        
    else if(cnt>4 && module==DISP_MODULE_OVL0)  // more than 5 layers, use ovl1+ovl0
    {
        *layer_min = 4;
    ...
    if(cnt>4 && (DISP_REG_GET(DISP_OVL_INDEX_OFFSET+DISP_REG_OVL_EN)==0) && handle==NULL)
    {
...
void disp_ovl_enable_sw_trigger(void * handle)
{
    static unsigned int cnt = 0;

    DDPMSG("disp_ovl_enable_sw_trigger start! cnt=%d\n", cnt++);
    
    DISP_REG_SET(handle, DISP_REG_OVL_EN, 1);
    DISP_REG_SET_FIELD(handle,MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL0,DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0,1);
    DISP_REG_SET(handle, DISP_REG_OVL_TRIG, 1);
    DISP_REG_SET(handle, DISP_REG_OVL_TRIG, 0);
    DISP_REG_SET_FIELD(handle,MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL0,DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0,0);
    DISP_REG_SET(handle, DISP_REG_OVL_EN, 0);

    if(ovl1_status!=DDP_OVL1_STATUS_SUB)
    {
        DISP_REG_SET(handle, DISP_REG_OVL_EN+DISP_OVL_INDEX_OFFSET, 1);
        DISP_REG_SET_FIELD(handle,MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL1,DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0,1);
        DISP_REG_SET(handle, DISP_REG_OVL_TRIG+DISP_OVL_INDEX_OFFSET, 1);
        DISP_REG_SET(handle, DISP_REG_OVL_TRIG+DISP_OVL_INDEX_OFFSET, 0);
        DISP_REG_SET_FIELD(handle,MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL1,DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0,0);
        DISP_REG_SET(handle, DISP_REG_OVL_EN+DISP_OVL_INDEX_OFFSET, 0);
    }

    DDPMSG("disp_ovl_enable_sw_trigger end! \n");
}

extern unsigned int rdma_underflow_irq_cnt[2];
extern unsigned int gFrameDoneTimeout;
			...
			void *handle)
{
	...
	static unsigned int rdma_underflow_cnt = 0;
	int layer_enable;

#ifdef OVL_CASCADE_SUPPORT
    if(module==DISP_MODULE_OVL0 && 
      (ovl_get_status()==DDP_OVL1_STATUS_PRIMARY || ovl_get_status()==DDP_OVL1_STATUS_SUB_REQUESTING))  //OVL0 always on top
    {
        layer_min = 4;
        layer_max = 8;
    }
	...
	if(handle) {
		if( (primary_display_is_decouple_mode()==1) ||
		    (primary_display_is_video_mode()==0)    || // directlink
		    ((gFrameDoneTimeout==1) || (rdma_underflow_cnt<rdma_underflow_irq_cnt[0])) //direclink+video mode+underflow or wait frame done timeout happen
		    )
			
		{
			...
			if(ovl_index(module)==0)
			{
                rdma_underflow_cnt = rdma_underflow_irq_cnt[0];
                gFrameDoneTimeout = 0;
			}
		}
	}
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			...
			ovl_layer_config(module,
					...
					handle);
			DDPMLOG("O%d/L%d/S%d/%s/0x%x/(%d,%d)/P%d/(%d,%d,%d,%d).\n", 
					...
					pConfig->ovl_config[i].dst_w,
					pConfig->ovl_config[i].dst_h );		
			MMProfileLogEx(ddp_mmp_get_events()->ovl_enable, MMProfileFlagPulse, (module-DISP_MODULE_OVL0)<<4 + (i%4), pConfig->ovl_config[i].addr);		
		}
		else
		{
            // from enable to disable
            if(DISP_REG_GET(DISP_REG_OVL_SRC_CON+(module-DISP_MODULE_OVL0)*DISP_OVL_INDEX_OFFSET)&(0x1<<i))
            {
                MMProfileLogEx(ddp_mmp_get_events()->ovl_disable, MMProfileFlagPulse, (module-DISP_MODULE_OVL0)<<4 + (i%4), 0);
            }
		}
		ovl_layer_switch(module, i%4,
	...
	void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if(primary_display_is_video_mode()==1 && primary_display_is_decouple_mode()==0) 
	{
	   if (state == CMDQ_CHECK_IDLE_AFTER_STREAM_EOF) {
			if (module == DISP_MODULE_OVL0) {
				ret = cmdqRecPoll(cmdq_trigger_handle, 0x14007240, 2, 0x3ff);
			} else if (module == DISP_MODULE_OVL1) {
				ret = cmdqRecPoll(cmdq_trigger_handle, 0x14008240, 2, 0x3ff);
			} else {
...
void ovl_dump_analysis(DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("ovl_en=%d,layer_enable(%d,%d,%d,%d),bg(w=%d, h=%d),"
		"cur_pos(x=%d,y=%d),layer_hit(%d,%d,%d,%d), ovl_complete_cnt=(%d, %d)\n",
		DISP_REG_GET(DISP_REG_OVL_EN+offset),
		...
		ovl_complete_irq_cnt[0], 
		ovl_complete_irq_cnt[1]
	       );
	for (i = 0; i < 4; i++) {
...
static int ovl_io(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
    ...
    switch (msg) {
        ...
        {
            DISP_REG_SET_FIELD(cmdq, DATAPATH_CON_FLD_BGCLR_IN_SEL, DISP_REG_OVL_DATAPATH_CON, 1);
