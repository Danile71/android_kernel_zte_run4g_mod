...
#define OUTREG32(x, y)      WRITE_REGISTER_UINT32((unsigned long*)(x), (UINT32)(y))
...
#define DISP_REG_GET_FIELD(field, reg32) ((__raw_readl(reg32) & REG_FLD_MASK(field)) >> REG_FLD_SHIFT(field))
...
#define DISP_CPU_REG_SET(reg32, val) \
	do {\
		if(0)dprec_reg_op(NULL, reg32, val, 0x00000000);\
		mt_reg_sync_writel(val, (volatile unsigned long*)(reg32));\
		} while(0)
#define DISP_CPU_REG_SET_FIELD(field, reg32, val)  \
    do {                                \
	   if(0)dprec_reg_op(NULL, (unsigned long)(reg32), val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field));\
           mt_reg_sync_writel( ((unsigned int)(*(volatile unsigned long*)(reg32)) & ~REG_FLD_MASK(field))|REG_FLD_VAL((field), (val)), (volatile unsigned long*)reg32);  \
    } while (0)
...
#define MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL0                     REG_FLD(1,6)
#define MMSYS_HW_DCM_DIS0_FLD_DCM_DIS0_OVL1                     REG_FLD(1,7)
