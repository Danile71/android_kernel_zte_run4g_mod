...
static void internal_md_power_down(void)
{
	...
	md_power_off(SYS_MD1, 0); // no need to poll, as MD SW didn't run and enter sleep mode, polling will not get result
...
static void internal_md2_power_down(void)
{
	...
	printk("[ccci-off]shutdown MDSYS2 !!!\n");
	...
	printk("[ccci-off]9.power on MD-2G/HSPA\n");
	// power on MD2G
	sync_write32(ioread32(MODEM_LITE_TOPSM_RM_PWR_CON0(modem_lite_topsm_base))|0x44, 
		MODEM_LITE_TOPSM_RM_PWR_CON0(modem_lite_topsm_base));

	printk("[ccci-off]10.power off MD-2G/HSPA\n");
	// power off MD2G
	sync_write32(0xFFFFFFFF, MD_TOPSM_SM_REQ_MASK(md_topsm_base));
	sync_write32(0, MODEM_LITE_TOPSM_RM_TMR_PWR0(modem_lite_topsm_base));
	sync_write32(0, MODEM_LITE_TOPSM_RM_TMR_PWR1(modem_lite_topsm_base));
	...
	printk("[ccci-off]11.power off ABB\n");
	// power off ABB
	sync_write32(ioread32(MD_TOPSM_RM_PWR_CON1(md_topsm_base)) & ~(0x1<<2) & ~(0x1<<6), 
		MD_TOPSM_RM_PWR_CON1(md_topsm_base));
	sync_write32(ioread32(MD_TOPSM_RM_PWR_CON1(md_topsm_base))| 0x00000090, MD_TOPSM_RM_PWR_CON1(md_topsm_base));
	...
	printk("[ccci-off]12.power off MD_INFRA/MODEM_TOP\n");
	md_power_off(SYS_MD2, 0); // no need to poll, as MD SW didn't run and enter sleep mode, polling will not get result
}
#endif

#ifndef CONFIG_MTK_ECCCI_DRIVER
//unsigned int get_modem_is_enabled(int md_id) {return 0;}
int exec_ccci_kern_func_by_md_id(int md_id, unsigned int id, char *buf, unsigned int len) {return 0;}
int register_ccci_sys_call_back(int md_id, unsigned int id, int (*func)(int, int)) {return 0;}
#endif
