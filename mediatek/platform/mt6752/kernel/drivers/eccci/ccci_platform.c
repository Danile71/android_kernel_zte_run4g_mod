...
static struct md2_ext_reg md2_dbg_reg;
int io_remap_md_side_register(int md_id, void *md_ptr)
{
	...
	if(md_id == 0) {
		...
		cldma_dbg_reg.md_pll = ioremap_nocache(MD_PLL_BASE, MD_PLL_LENGTH);
	...
	if(md_id == 1) {
		...
		md2_dbg_reg.md_bus_status = ioremap_nocache(MD2_BUS_STATUS_BASE, MD2_BUS_STATUS_LENGTH);
		return 0; 
	}
...
int plat_misc_api(void* data, unsigned int api_id, void* output)
{
	...
	case resv_api_1:
		md_id=(int)data;
		if(md_id==0){
			CCCI_INF_MSG((int)md_id, TAG, "Dump MD Bus status %x\n", MD_BUS_STATUS_BASE);
			ccci_mem_dump((int)md_id, cldma_dbg_reg.md_bus_status, MD_BUS_STATUS_LENGTH);
			CCCI_INF_MSG((int)md_id, TAG, "Dump MD PC monitor %x\n", MD_PC_MONITOR_BASE);
			ccci_write32(cldma_dbg_reg.md_pc_monitor, 0, 0x80000000); // stop MD PCMon
			ccci_mem_dump((int)md_id, cldma_dbg_reg.md_pc_monitor, MD_PC_MONITOR_LENGTH);
			CCCI_INF_MSG((int)md_id, TAG, "Dump MD TOPSM status %x\n", MD_TOPSM_STATUS_BASE);
			ccci_mem_dump((int)md_id, cldma_dbg_reg.md_topsm_status, MD_TOPSM_STATUS_LENGTH);
			CCCI_INF_MSG((int)md_id, TAG, "Dump MD OST status %x\n", MD_OST_STATUS_BASE);
			ccci_mem_dump((int)md_id, cldma_dbg_reg.md_ost_status, MD_OST_STATUS_LENGTH);
			CCCI_INF_MSG((int)md_id, TAG, "Dump MD PLL %x\n", MD_PLL_BASE);
			ccci_mem_dump((int)md_id, cldma_dbg_reg.md_pll, MD_PLL_LENGTH);
		}else if(md_id==1){
				CCCI_INF_MSG((int)md_id, TAG, "Dump MD Bus status %x\n", MD2_BUS_STATUS_BASE);
				ccci_mem_dump((int)md_id, md2_dbg_reg.md_bus_status, MD2_BUS_STATUS_LENGTH);
		}
	break;
