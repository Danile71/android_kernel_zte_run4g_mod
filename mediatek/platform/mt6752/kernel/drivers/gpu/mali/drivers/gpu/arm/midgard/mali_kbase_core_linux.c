...
int g_custom_gpu_boost_id = 0; /* MTK for mtk_custom_boost_gpu_freq (always set as lower bound of frequency)*/
...
void mtk_gpu_input_boost_CB(unsigned int ui32BoostFreqID)
{
    ...
    if(ui32BoostFreqID < iCurrentFreqID)
    {
        ...
        mt_gpufreq_target(ui32BoostFreqID);
    }
...
/* MTK custom boost. 0 is the lowest frequency index. */
...
/* MTK get custom boost. 0 is the lowest frequency index. */
unsigned int mtk_kbase_get_custom_boost_gpu_freq(void)
{
    unsigned int uiTableNum = mt_gpufreq_get_dvfs_table_num();

    return uiTableNum - 1 - g_custom_gpu_boost_id;
}
...
int mtk_gpu_dvfs()
{
    ...
    for(i=0 ; i<mt6752_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index_count ; i++)
    {
        if( iCurrentFreqID <=mt6752_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index[i] )
        {
    ...
    iTargetVirtualFreqID = _mtk_dvfs_index_clipping(iTargetVirtualFreqID, 0, mt6752_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index_count-1);

    iTargetFreqID = mt6752_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index[iTargetVirtualFreqID];
...
#if 0	
	if (kbasep_gpu_utilization_debugfs_init(kbdev)) {
		dev_err(kbdev->dev, "Couldn't create gpu_utilization debugfs file\n");
		goto out_partial;
	}
	inited |= inited_gpu_utilization;
#endif
#endif /* CONFIG_DEBUG_FS */
...
#if 0		
	if (inited & inited_gpu_utilization)
		kbasep_gpu_utilization_debugfs_term(kbdev);
#endif
#endif /* CONFIG_DEBUG_FS */
#ifdef CONFIG_PROC_FS
...
{
	printk(KERN_EMERG "Using mali midgard r4p1 DDK kernel device driver. GPU probe() here\n");
	struct kbase_device *kbdev;
