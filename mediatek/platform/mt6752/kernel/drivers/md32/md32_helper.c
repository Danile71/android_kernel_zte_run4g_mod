...
#endif
    default:
        printk("[MD32] helper get unkown action %d\n", action);
        break;
    }
    return NOTIFY_OK;
}
...
ssize_t md32_get_log_buf(unsigned char *md32_log_buf, size_t b_len)
{
    ...
    {
        printk("[MD32] b_len %d > log_buf_max_len %d\n", b_len, log_buf_max_len);
        b_len = log_buf_max_len;
    }
...
void apdma_ipi_handler(int id, void *data, unsigned int len)
{
    ...
    printk("[MD32] flag_md32_addr = 0x%x\n", flag_md32_addr);
    printk("[MD32] flag_apmcu_addr = 0x%x\n", flag_apmcu_addr);
    printk("[MD32] INFRA APDMA clock count = %d\n", apmcu_clk_init_count);
}
#else
void apdma_ipi_handler(int id, void *data, unsigned int len)
{
...
int md32_dt_init(void)
{
    ...
    printk("[MD32] md32 sram base=0x%08x\n", mreg->sram);
    ...
    printk("[MD32] md32 irq=%d, cfgreg=0x%08x\n", mreg->irq, mreg->cfg);
