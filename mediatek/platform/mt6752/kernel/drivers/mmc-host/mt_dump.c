...
#ifdef MTK_EMMC_SUPPORT
#endif
char test_kdump[]={6,7,5,2,'k','d','u','m','p','t','e','s','t'};
//==============
#if MTK_MMC_DUMP_DBG
...
#define msdc_retry(expr, retry, cnt,id) \
    do { \
        ...
        while (retry) { \
            if (!(expr)) break; \
            if (cnt-- == 0) { \
                retry--; msdc_mdelay(1); cnt = backup; \
            } \
        } \
        if (retry == 0) { \
...
static void msdc_mdelay(u32 time)
{
    ...
    t_start = localtimer_get_phy_count(); /* return 13M clock count */
    t_end = t_start + time * 13 * 1000;

    while(t_end > localtimer_get_phy_count()) 
        cpu_relax();
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
    ...
    if(host->id == 0) {
        printk(KERN_EMERG "R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n", 
      ...
      #if 0
        printk(KERN_EMERG "R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n", 
        ...
        printk(KERN_EMERG "R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x  R[%x]=0x%x\n", 
...
#if 0
static unsigned int simp_msdc_ldo_power(unsigned int on, MT_POWER powerId, MT_POWER_VOLTAGE powerVolt)
{
...
{
    struct device_node *apmixedn = NULL;
    struct device_node *topckgenn = NULL;
    struct device_node *infracfgn = NULL;
    void __iomem *apmixedb;
    void __iomem *topckgenb;
    void __iomem *infracfgb;

    apmixedn = of_find_compatible_node(NULL, NULL, "mediatek,APMIXED");
    apmixedb = of_iomap(apmixedn, 0);

    topckgenn = of_find_compatible_node(NULL, NULL, "mediatek,TOPCKGEN");
    topckgenb = of_iomap(topckgenn, 0);

    infracfgn = of_find_compatible_node(NULL, NULL, "mediatek,INFRACFG_AO");
    infracfgb = of_iomap(infracfgn, 0);

    /* step1: open pll */
    clk_setl(apmixedb+0x25C, 0x3);
    msdc_mdelay(1);
    clk_clrl(apmixedb+0x25C, 0x2);

    clk_setl(apmixedb+0x254, 0x80000000);

    clk_setl(apmixedb+0x250, 0x1);
    msdc_mdelay(1);

    /* step2: enable mux */
    mt_reg_sync_writel(0x02060200, (topckgenb+0x070));

    /* step3: enable clock */
    clk_setl(infracfgb+0x8C, 0x14);

#if MTK_MMC_DUMP_DBG
    printk(KERN_EMERG "[pll]0xF000C25C =0x%x, 0xF000C250 =0x%x, [mux]0xF0000070 =0x%x, [cg]0xF0001094 =0x%x\n", 
    		sdr_read32(apmixedb+0x25C), sdr_read32(apmixedb+0x250), sdr_read32(topckgenb+0x070), sdr_read32(infracfgb+0x094));
#endif
...
extern void __iomem *io_cfg_left_base;
extern void __iomem *io_cfg_top_base;
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    switch(host->id){
        case 0:
            ...
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, msdc0_hw.clk_drv);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DSL_DRVING, msdc0_hw.cmd_drv);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT0_7_DRVING, msdc0_hw.dat_drv);
            break;
        ...
        case 1: 
            ...
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, msdc1_hw.clk_drv);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, msdc1_hw.cmd_drv);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, msdc1_hw.dat_drv);
            break;
      #endif
...
#if MTK_MMC_DUMP_DBG
        printk(KERN_EMERG "msdc cmd<%d> arg<0x%x> resp<0x%x>Ready \r\n", cmd,arg, *resp);
#endif
    } else {
        error = 4;  
        goto end;            
...
static unsigned int simp_mmc_get_status(struct simp_mmc_host *host, unsigned int* status)
{
    ...
    rca = phost->card->rca << 16;  
#endif
    //printk("rca=0x%x, mtk_msdc_host[%d]->mmc->card->rca=0x%x, phost->card->rca=0x%x\n", rca, host->index, mtk_msdc_host[host->index]->mmc->card->rca, phost->card->rca); 
...
static int simp_emmc_dump_write(unsigned char* buf, unsigned int len, unsigned int offset,unsigned int dev)
{
    ...
    if (emmc_init == 0) {
        for (i = 0; i < ARRAY_SIZE(g_freqs); i++) {
            ...
            if (g_freqs[i] <= HOST_MIN_MCLK){
                printk("failed to init eMMC, line:%d\n", __LINE__); 
                return ret;
            }
        }
        emmc_init = 1;
    ...
    for (i = 0; i < (len/512); i++) {
        ...
        if (l_addr >= (lp_start_sect + lp_nr_sects)) {
            printk("write 512 Bytes address over boundary at 0x%x\n", l_addr);
            return ret;
        }

        err = simp_mmc_single_write(pmmc_boot_host, l_addr, l_buf, 512);
        if (err) {
            printk("write 512 Bytes fail at 0x%x\n", l_addr);
            return ret;
        }
...
static int __init get_emmc_dump_info(void)
{
    ...
    if (likely(lp_hd_struct)) {
        ...
        partition_ready_flag = -1;
        printk(KERN_ERR "There is no expdb info\n");
    } 
