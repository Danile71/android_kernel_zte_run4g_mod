...
static struct device_node *io_cfg_left_node = NULL;
static struct device_node *io_cfg_top_node = NULL;
static struct device_node *io_cfg_bottom_node = NULL;
static struct device_node *io_cfg_right_node = NULL;
...
void __iomem *io_cfg_left_base;
void __iomem *io_cfg_top_base;
void __iomem *io_cfg_bottom_base;
void __iomem *io_cfg_right_base;

#ifdef MTK_MSDC_BRINGUP_DEBUG
void __iomem *apmixed_base1;
...
void msdc_dump_register(struct msdc_host *host)
{
    ...
    INIT_MSG("Reg[%x] PAD_TUNE       = 0x%.8x", OFFSET_MSDC_PAD_TUNE,  sdr_read32(MSDC_PAD_TUNE));
...
{
#ifdef MTK_MSDC_BRINGUP_DEBUG
    INIT_MSG(" msdcpll en[0xF000C250][bit0 should be 0x1]=0x%x\n", sdr_read32(apmixed_base1+0x250));
    INIT_MSG(" msdcpll on[0xF000C25C][bit0~1 should be 0x1]=0x%x\n", sdr_read32(apmixed_base1+0x25C));

    INIT_MSG(" pdn, mux[0xF0000070][bit23, 16~19 for msdc0, bit31, 24~26 for msdc1]=0x%x\n", sdr_read32(topckgen_base+0x070));
#endif
}
...
void msdc_dump_gpio(struct msdc_host *host){
    if ( host->id==0 ) {
        ...
        printk(KERN_ERR "GPIO_MODE0=%x\n", sdr_read32(gpio_base+0x300));
        printk(KERN_ERR "GPIO_MODE1=%x\n", sdr_read32(gpio_base+0x310)&0xffff);
    } else if ( host->id==1 ) {
        printk(KERN_ERR "MSDC1 shall set pin mode to 1\n");
        printk(KERN_ERR "GPIO_MODE1=%x\n", sdr_read32(gpio_base+0x310)&0xffff0000);
        printk(KERN_ERR "GPIO_MODE2=%x\n", sdr_read32(gpio_base+0x320)&0xff);
    } else if ( host->id==2 ) {
        printk(KERN_ERR "MSDC2 shall set pin mode to 2\n");
        printk(KERN_ERR "GPIO_MODE8=%x\n", sdr_read32(gpio_base+0x380)&0xffff0000);
        printk(KERN_ERR "GPIO_MODE9=%x\n", sdr_read32(gpio_base+0x300)&0xff);
    } else if ( host->id==3 ) {
        printk(KERN_ERR "MSDC3 shall set pin mode to 2\n");
        printk(KERN_ERR "GPIO_MODE12=%x\n", sdr_read32(gpio_base+0x3C0)&0xffffff00);
    }
}

#define MSDC_IOCONFIG_ITEMS     8
static unsigned int msdc_ioconfg_reg_offsets[HOST_MAX_NUM][MSDC_IOCONFIG_ITEMS]={
    {MSDC0_IES_CFG_OFFSET,   MSDC0_SR_CFG_OFFSET,      MSDC0_SMT_CFG_OFFSET,      MSDC0_TDSEL_CFG_OFFSET,
     MSDC0_RDSEL_CFG_OFFSET, MSDC0_PULL_EN_CFG_OFFSET, MSDC0_PULL_SEL_CFG_OFFSET, MSDC0_DRVING_OFFSET},

    {MSDC1_IES_CFG_OFFSET,   MSDC1_SR_CFG_OFFSET,      MSDC1_SMT_CFG_OFFSET,      MSDC1_TDSEL_CFG_OFFSET,
     MSDC1_RDSEL_CFG_OFFSET, MSDC1_PULL_EN_CFG_OFFSET, MSDC1_PULL_SEL_CFG_OFFSET, MSDC1_DRVING_OFFSET},

    {MSDC2_IES_CFG_OFFSET,   MSDC2_SR_CFG_OFFSET,      MSDC2_SMT_CFG_OFFSET,      MSDC2_TDSEL_CFG_OFFSET,
     MSDC2_RDSEL_CFG_OFFSET, MSDC2_PULL_EN_CFG_OFFSET, MSDC2_PULL_SEL_CFG_OFFSET, MSDC2_DRVING_OFFSET},

    {MSDC3_IES_CFG_OFFSET,   MSDC3_SR_CFG_OFFSET,      MSDC3_SMT_CFG_OFFSET,      MSDC3_TDSEL_CFG_OFFSET,
     MSDC3_RDSEL_CFG_OFFSET, MSDC3_PULL_EN_CFG_OFFSET, MSDC3_PULL_SEL_CFG_OFFSET, MSDC3_DRVING_OFFSET},
};

static unsigned int msdc_ioconfg_reg_masks[HOST_MAX_NUM][MSDC_IOCONFIG_ITEMS]={
    {MSDC0_IES_ALL_MASK,   MSDC0_SR_ALL_MASK,      MSDC0_SMT_ALL_MASK,      MSDC0_TDSEL_ALL_MASK,
     MSDC0_RDSEL_ALL_MASK, MSDC0_PULL_EN_ALL_MASK, MSDC0_PULL_SEL_ALL_MASK, MSDC0_DRVING_ALL_MASK},

    {MSDC1_IES_ALL_MASK,   MSDC1_SR_ALL_MASK,      MSDC1_SMT_ALL_MASK,      MSDC1_TDSEL_ALL_MASK,
     MSDC1_RDSEL_ALL_MASK, MSDC1_PULL_EN_ALL_MASK, MSDC1_PULL_SEL_ALL_MASK, MSDC1_DRVING_ALL_MASK},

    {MSDC2_IES_ALL_MASK,   MSDC2_SR_ALL_MASK,      MSDC2_SMT_ALL_MASK,      MSDC2_TDSEL_ALL_MASK,
     MSDC2_RDSEL_ALL_MASK, MSDC2_PULL_EN_ALL_MASK, MSDC2_PULL_SEL_ALL_MASK, MSDC2_DRVING_ALL_MASK},

    {MSDC3_IES_ALL_MASK,   MSDC3_SR_ALL_MASK,      MSDC3_SMT_ALL_MASK,      MSDC3_TDSEL_ALL_MASK,
     MSDC3_RDSEL_ALL_MASK, MSDC3_PULL_EN_ALL_MASK, MSDC3_PULL_SEL_ALL_MASK, MSDC3_DRVING_ALL_MASK},
};
...
void msdc_dump_ioconfig_padctl(struct msdc_host *host)
{
    ...
    if ( host->id==0 ) base=io_cfg_left_base;
    else if ( host->id==1 ) base=io_cfg_top_base;
    else if ( host->id==2 ) base=io_cfg_bottom_base;
    else if ( host->id==3 ) base=io_cfg_left_base;
    else return;
...
static int msdc_clk_stable(struct msdc_host *host,u32 mode, u32 div, u32 hs400_src){
    ...
    do{
        retry = 3;
        sdr_set_field(MSDC_CFG, MSDC_CFG_CKMOD_HS400 |MSDC_CFG_CKMOD|MSDC_CFG_CKDIV,(hs400_src << 10)|(mode << 8)|((div + retry_cnt) % 0xff));
...
#else
static u32 hclks_msdc50[] = {26000000, 800000000, 400000000, 156000000, 182000000,
                                    0, 200000000, 624000000, 312000000};

static u32 hclks_msdc30[] = {26000000, 208000000, 200000000, 156000000, 182000000,
                                    0, 178280000};
...
void msdc_set_rdtdsel(struct msdc_host *host,bool sd_18)
{
    switch(host->id){
        case 0:
            sdr_set_field(MSDC0_TDSEL_BASE, MSDC0_TDSEL_ALL_MASK ,0x0);
    ...
    switch(host->id){
        ...
        case 2:
            if(sd_18){
                ...
                sdr_set_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, 0xc);
            }
            break;
        case 3:
            sdr_set_field(MSDC3_GPIO_PAD_BASE, GPIO_PAD_RDSEL_MASK, 0x0);
            break;
        default:
            break;
    }
...
void msdc_set_tdsel(struct msdc_host *host,bool sleep)
{
    switch(host->id){
        ...
        case 1:
            if(sleep){
                sdr_set_field(MSDC1_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xf);
            } else{
        ...
        case 2:
            if(sleep){
                sdr_set_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xf);
            } else{
                sdr_set_field(MSDC2_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0xa);
            }
            break;
        case 3:
            sdr_set_field(MSDC3_GPIO_PAD_BASE, GPIO_PAD_TDSEL_MASK, 0x0);
            break;
        default:
            break;
    }
...
static void msdc_pin_pnul(struct msdc_host *host, int mode)
{
    switch(host->id){
        case 0:
            if (!mode)
                sdr_write32(MSDC0_PULL_EN_CFG_CLR, MSDC0_PULL_EN_CMD_DAT_MASK);
            break;

        case 1:
            if (!mode)
                sdr_write32(MSDC1_PULL_EN_CFG_CLR, MSDC1_PULL_EN_CMD_DAT_MASK);
            break;

#if defined(CFG_DEV_MSDC2)
        case 2:
            if (!mode)
                sdr_write32(MSDC2_PULL_EN_CFG_CLR, MSDC2_PULL_EN_CMD_DAT_MASK);
            break;
#endif
        ...
        case 3:
            if (!mode)
                sdr_write32(MSDC3_PULL_EN_CFG_CLR, MSDC3_PULL_EN_CMD_DAT_MASK);
            break;
#endif
    ...
    switch(host->id){
        case 0:
            if (mode) {
                ...
                sdr_set_field(MSDC0_PULL_SEL_CFG_BASE, MSDC0_PULL_SEL_ALL_MASK, 0x7FE);
            } else {
                sdr_write32(MSDC0_PULL_SEL_CFG_CLR, MSDC0_PULL_SEL_ALL_MASK);
            }
            //Write to MSDC0_PULL_EN_CFG_SET directly
            //Assume another pull-resistor is never set before, therefore no need to clear their correpsonding bit
            sdr_write32(MSDC0_PULL_EN_CFG_SET, 0xC013FE);
            break;

        case 1:
            if (mode) {
                sdr_set_field(MSDC1_PULL_SEL_CFG_BASE, MSDC1_PULL_SEL_ALL_MASK, 0x3E);
            } else {
                sdr_write32(MSDC1_PULL_SEL_CFG_CLR, MSDC1_PULL_SEL_ALL_MASK);
            }
            //Write to MSDC1_PULL_EN_CFG_SET directly
            sdr_write32(MSDC1_PULL_EN_CFG_SET, 0xFC0);
            break;

#if defined(CFG_DEV_MSDC2)
        case 2:
            if (mode) {
                //CLK pull down with 50K, CMD/DATs pull up with 10K
                sdr_set_field(MSDC2_PULL_SEL_CFG_BASE, MSDC2_PULL_SEL_ALL_MASK, 0x3E);
                sdr_write32(MSDC2_PULL_EN_CFG_SET, 0x7E);
            } else {
                //All pull down with 50K
                sdr_write32(MSDC2_PULL_SEL_CFG_CLR, MSDC2_PULL_SEL_ALL_MASK);
                sdr_write32(MSDC2_PULL_EN_CFG_SET, 0xFC0);
            }
            break;
#endif
        ...
        case 3:
            if (mode) {
                //CLK pull down with 50K, CMD/DATs pull up with 10K
                sdr_set_field(MSDC3_PULL_SEL_CFG_BASE, MSDC3_PULL_SEL_ALL_MASK, 0x3E);
                sdr_write32(MSDC3_PULL_EN_CFG_SET, 0x7E);
            } else {
                //All pull down with 50K
                sdr_write32(MSDC3_PULL_SEL_CFG_CLR, MSDC3_PULL_SEL_ALL_MASK);
                sdr_write32(MSDC3_PULL_EN_CFG_SET, 0xFC0);
            }
            break;
#endif
...
static void msdc_pin_pud(struct msdc_host *host, u32 pin, u32 mode, u32 val)
{
    switch(pin){
        case GPIO_CLK_CTRL:
            sdr_set_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_R1R0_MASK, val);
            sdr_set_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_PUPD_MASK, mode);
            break;
        case GPIO_CMD_CTRL:
            sdr_set_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_R1R0_MASK, val);
            sdr_set_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_PUPD_MASK, mode);
            break;
        case GPIO_DAT_CTRL:
            if(host->id == 0){
                sdr_set_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_R1R0_MASK, val);
                sdr_set_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_PUPD_MASK, mode);
            } else {
                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT0_R1R0_MASK, val);
                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT0_PUPD_MASK, mode);

                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT1_R1R0_MASK, val);
                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT1_PUPD_MASK, mode);

                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT2_R1R0_MASK, val);
                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT2_PUPD_MASK, mode);

                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT3_R1R0_MASK, val);
                sdr_set_field(msdc_dat1_ctrl_reg[host->id], GPIO_MSDC_DAT3_PUPD_MASK, mode);
            }
            break;
        case GPIO_RST_CTRL:
            if(host->id == 0){
                sdr_set_field(MSDC0_GPIO_RST_BASE, GPIO_MSDC_R1R0_MASK, val);
                sdr_set_field(MSDC0_GPIO_RST_BASE, GPIO_MSDC_PUPD_MASK, mode);
            }
            break;
        case GPIO_DS_CTRL:
            if(host->id == 0){
                sdr_set_field(MSDC0_GPIO_DS_BASE, GPIO_MSDC_R1R0_MASK, val);
                sdr_set_field(MSDC0_GPIO_DS_BASE, GPIO_MSDC_PUPD_MASK, mode);
            }
            break;
        default:
            break;
...
void msdc_set_smt(struct msdc_host *host, u32 pin, u32 val)
{
    switch(pin){
        case GPIO_CLK_CTRL:
            sdr_set_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_SMT_MASK, val);
            break;
        case GPIO_CMD_CTRL:
            sdr_set_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_SMT_MASK, val);
            break;
        case GPIO_DAT_CTRL:
            sdr_set_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_SMT_MASK, val);
            break;
        case GPIO_RST_CTRL:
            if(host->id == 0)
                sdr_set_field(MSDC0_GPIO_RST_BASE, GPIO_MSDC_SMT_MASK, val);
            break;
        case GPIO_DS_CTRL:
            if(host->id == 0)
                sdr_set_field(MSDC0_GPIO_DS_BASE, GPIO_MSDC_SMT_MASK, val);
            break;
        default:
            break;
    }
...
void msdc_set_driving(struct msdc_host *host, struct msdc_hw *hw, bool sd_18)
{
    switch (host->id){
        case 0:
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, hw->clk_drv);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DSL_DRVING, hw->cmd_drv);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT0_7_DRVING, hw->dat_drv);
            break;
        case 1:
            if (sd_18) {
                ...
                sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, hw->dat_drv_sd_18);
            } else {
                sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, hw->clk_drv);
                sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, hw->cmd_drv);
                sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, hw->dat_drv);
            }
            break;
        ...
        case 2:
            if (sd_18) {
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_CLK_DRVING, hw->clk_drv_sd_18);
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_CMD_DRVING, hw->cmd_drv_sd_18);
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_DAT0_3_DRVING, hw->dat_drv_sd_18);
            } else {
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_CLK_DRVING, hw->clk_drv);
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_CMD_DRVING, hw->cmd_drv);
                sdr_set_field(MSDC2_DRVING_BASE, MSDC2_DAT0_3_DRVING, hw->dat_drv);
            }
            break;
#endif
...
#if defined(CFG_DEV_MSDC3)
        case 3:
            sdr_set_field(MSDC3_DRVING_BASE, MSDC3_CLK_DRVING, hw->clk_drv_sd_18);
            sdr_set_field(MSDC3_DRVING_BASE, MSDC3_CMD_DRVING, hw->cmd_drv_sd_18);
            sdr_set_field(MSDC3_DRVING_BASE, MSDC3_DAT0_3_DRVING, hw->dat_drv_sd_18);
            break;
#endif
        default:
...
void msdc_get_driving(struct msdc_host *host,struct msdc_ioctl* msdc_ctl)
{
    ...
    switch (host->id){
        case 0:
            sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, msdc_ctl->clk_pu_driving);
            sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DSL_DRVING, msdc_ctl->cmd_pu_driving);
            sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DSL_DRVING, msdc_ctl->rst_pu_driving);
            sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DSL_DRVING, msdc_ctl->ds_pu_driving);
            sdr_get_field(MSDC0_DRVING_BASE, MSDC0_DAT0_7_DRVING, msdc_ctl->dat_pu_driving);
            break;
        case 1:
            ...
            sdr_get_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, msdc_ctl->dat_pu_driving);
            break;

#if defined(CFG_DEV_MSDC2)
        case 2:
            sdr_get_field(MSDC2_DRVING_BASE, MSDC2_CLK_DRVING, msdc_ctl->clk_pu_driving);
            sdr_get_field(MSDC2_DRVING_BASE, MSDC2_CMD_DRVING, msdc_ctl->cmd_pu_driving);
            sdr_get_field(MSDC2_DRVING_BASE, MSDC2_DAT0_3_DRVING, msdc_ctl->dat_pu_driving);
            break;
#endif

#if defined(CFG_DEV_MSDC3)
        case 3:
            sdr_get_field(MSDC3_DRVING_BASE, MSDC3_CLK_DRVING, msdc_ctl->clk_pu_driving);
            sdr_get_field(MSDC3_DRVING_BASE, MSDC3_CMD_DRVING, msdc_ctl->cmd_pu_driving);
            sdr_get_field(MSDC3_DRVING_BASE, MSDC3_DAT0_3_DRVING, msdc_ctl->dat_pu_driving);
            break;
#endif
...
void msdc_set_driving(struct msdc_host *host,struct msdc_hw *hw,bool sd_18)
{
    if(sd_18 && ((host->id == 1) || (host->id == 2))) {
        sdr_set_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->clk_drv_sd_18);
        sdr_set_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->cmd_drv_sd_18);
        sdr_set_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->dat_drv_sd_18);
    }else {
        sdr_set_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->clk_drv);
        sdr_set_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->cmd_drv);
        sdr_set_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, hw->dat_drv);
        if(host->id == 0) {
            sdr_set_field(MSDC0_GPIO_RST_BASE, GPIO_MSDC_DRV_MASK, hw->rst_drv);
            sdr_set_field(MSDC0_GPIO_DS_BASE, GPIO_MSDC_DRV_MASK, hw->ds_drv);
        }
    }
}
...
void msdc_get_driving(struct msdc_host *host,struct msdc_ioctl* msdc_ctl)
{
    sdr_get_field(msdc_clk_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, msdc_ctl->clk_pu_driving);
    sdr_get_field(msdc_cmd_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, msdc_ctl->cmd_pu_driving);
    sdr_get_field(msdc_dat_ctrl_reg[host->id], GPIO_MSDC_DRV_MASK, msdc_ctl->dat_pu_driving);
    if(host->id == 0) {
        sdr_get_field(MSDC0_GPIO_RST_BASE, GPIO_MSDC_DRV_MASK, msdc_ctl->rst_pu_driving);
        sdr_get_field(MSDC0_GPIO_DS_BASE, GPIO_MSDC_DRV_MASK, msdc_ctl->ds_pu_driving);
    }else {
        msdc_ctl->rst_pu_driving = 0;
        msdc_ctl->ds_pu_driving = 0;
    }
...
void msdc_set_pad_init(struct msdc_host *host)
{
    switch (host->id) {
        ...
        case 0:
             ...
             */
            sdr_set_field(MSDC0_PULL_EN_CFG_BASE, MSDC0_PULL_EN_ALL_MASK, 0xC013FE);
            ...
            /*
             * set msdc mode. (MC0_DAT0~5, MC0_CMD, MC0_CLK)
             */
            sdr_set_field(MSDC0_GPIO_MODE0_ADDR,
                     MSDC0_DAT5_PINMUX_BITS | MSDC0_DAT4_PINMUX_BITS | MSDC0_DAT3_PINMUX_BITS | MSDC0_DAT2_PINMUX_BITS |
                     MSDC0_DAT1_PINMUX_BITS | MSDC0_DAT0_PINMUX_BITS | MSDC0_CMD_PINMUX_BITS | MSDC0_CLK_PINMUX_BITS,
                     0x11111111);

            /*
             * set msdc mode. (MC0_RST, MC0_DSL, MC0_DAT6, MC0_DAT7)
             */
            sdr_set_field(MSDC0_GPIO_MODE1_ADDR,
                     MSDC0_RST_PINMUX_BITS | MSDC0_DSL_PINMUX_BITS | MSDC0_DAT7_PINMUX_BITS | MSDC0_DAT6_PINMUX_BITS ,
                     0x1111);
            break;
        ...
        case 1:
             ...
             * set pull enable.
             */
            sdr_write32(MSDC1_PULL_EN_CFG_SET,
                     MSDC1_CLK_PULL50K_EN  | MSDC1_CMD_PULL50K_EN  |
                     MSDC1_DAT0_PULL50K_EN | MSDC1_DAT1_PULL50K_EN | MSDC1_DAT2_PULL50K_EN | MSDC1_DAT3_PULL50K_EN);

            /*
             * set pull_sel to pull up cmd/dat. (clk default low).
             */
            sdr_write32(MSDC1_PULL_SEL_CFG_CLR, MSDC1_CLK_PULL_SEL);
            sdr_write32(MSDC1_PULL_SEL_CFG_SET,
                    MSDC1_CMD_PULL_SEL |
                    MSDC1_DAT0_PULL_SEL | MSDC1_DAT1_PULL_SEL | MSDC1_DAT2_PULL_SEL | MSDC1_DAT3_PULL_SEL);

            /*
             * set gpio to msdc mode. (clk/cmd/dat0/dat1 in GPIO_MODE1)
             */
            sdr_set_field(MSDC1_GPIO_MODE1_ADDR,
                    MSDC1_CMD_PINMUX_BITS |
                    MSDC1_DAT0_PINMUX_BITS | MSDC1_DAT1_PINMUX_BITS, 0x111);

            /*
             * set gpio to msdc mode. (dat2/dat3 in GPIO_MODE2)
             */
            sdr_set_field(MSDC1_GPIO_MODE2_ADDR, MSDC1_DAT2_PINMUX_BITS | MSDC1_DAT3_PINMUX_BITS, 0x11);

            /*
             * Set CLK pinmux after all other pinmux have been set
             * set gpio to msdc mode. (clk in GPIO_MODE1)
             */
            sdr_set_field(MSDC1_GPIO_MODE1_ADDR, MSDC1_CLK_PINMUX_BITS, 0x1);
            break;
        ...
        case 2:
             ...
             * set pull enable.
             */
            sdr_write32(MSDC2_PULL_EN_CFG_SET,
                     MSDC2_CLK_PULL50K_EN  | MSDC2_CMD_PULL50K_EN  |
                     MSDC2_DAT0_PULL50K_EN | MSDC2_DAT1_PULL50K_EN | MSDC2_DAT2_PULL50K_EN | MSDC2_DAT3_PULL50K_EN);

            /*
             * set pull_sel to pull up cmd/dat. (clk default low).
             */
            sdr_write32(MSDC2_PULL_SEL_CFG_CLR, MSDC2_CLK_PULL_SEL);
            sdr_write32(MSDC2_PULL_SEL_CFG_SET,
                    MSDC2_CMD_PULL_SEL |
                    MSDC2_DAT0_PULL_SEL | MSDC2_DAT1_PULL_SEL | MSDC2_DAT2_PULL_SEL | MSDC2_DAT3_PULL_SEL);

            /*
             * set msdc mode. (MC1_DAT0~3, MC1_CMD)
             */
            sdr_set_field(MSDC2_GPIO_MODE8_ADDR,
                     MSDC2_CMD_PINMUX_BITS | MSDC2_DAT1_PINMUX_BITS | MSDC2_DAT0_PINMUX_BITS,
                     0x222);
            sdr_set_field(MSDC2_GPIO_MODE9_ADDR,
                     MSDC2_DAT3_PINMUX_BITS | MSDC2_DAT2_PINMUX_BITS,
                     0x22);
        ...
        case 3:
             ...
             * set pull enable.
             */
            sdr_write32(MSDC3_PULL_EN_CFG_SET,
                     MSDC3_CLK_PULL50K_EN  | MSDC3_CMD_PULL50K_EN  |
                     MSDC3_DAT0_PULL50K_EN | MSDC3_DAT1_PULL50K_EN | MSDC3_DAT2_PULL50K_EN | MSDC3_DAT3_PULL50K_EN);

            /*
             * set pull_sel to pull up cmd/dat. (clk default low).
             */
            sdr_write32(MSDC3_PULL_SEL_CFG_CLR, MSDC2_CLK_PULL_SEL);
            sdr_write32(MSDC3_PULL_SEL_CFG_SET,
                    MSDC3_CMD_PULL_SEL |
                    MSDC3_DAT0_PULL_SEL | MSDC3_DAT1_PULL_SEL | MSDC3_DAT2_PULL_SEL | MSDC3_DAT3_PULL_SEL);

            /*
             * set msdc mode. (MC1_DAT0~3, MC1_CMD)
             */
            sdr_set_field(MSDC3_GPIO_MODE12_ADDR,
                     MSDC3_CMD_PINMUX_BITS | MSDC3_DAT1_PINMUX_BITS | MSDC3_DAT0_PINMUX_BITS |
                     MSDC3_DAT3_PINMUX_BITS | MSDC3_DAT2_PINMUX_BITS,
                     0x22222);
...
static void msdc_emmc_power(struct msdc_host *host,u32 on)
{
     ...
     * 4'b0101: -100 mV
     */
    msdc_ldo_power(on, MT6325_POWER_LDO_VEMC33, VOL_3000, &g_msdc0_flash);
...
static void msdc_sd_power(struct msdc_host *host,u32 on)
{
    ...
    switch(host->id){
        case 1:
            ...
            if (host->sd_keep_pwr) {
                msdc_ldo_power(1, MT6325_POWER_LDO_VMC, VOL_3000, &g_msdc1_io);
                msdc_ldo_power(1, MT6325_POWER_LDO_VMCH, VOL_3000, &g_msdc1_flash);
            }
            else {
                msdc_ldo_power(on, MT6325_POWER_LDO_VMC, VOL_3000, &g_msdc1_io);
                msdc_ldo_power(on, MT6325_POWER_LDO_VMCH, VOL_3000, &g_msdc1_flash);
            }
            break;
        case 2:
            msdc_set_driving(host,host->hw,0);
            msdc_set_rdtdsel(host,0);
            msdc_ldo_power(on, MT6325_POWER_LDO_VMC, VOL_3000, &g_msdc2_io);
            msdc_ldo_power(on, MT6325_POWER_LDO_VMCH, VOL_3000, &g_msdc2_flash);
            break;
        default:
            break;
    }
...
{
#ifndef FPGA_PLATFORM
    int mux_id[] = {MT_MUX_MSDC50_0, MT_MUX_MSDC30_1, MT_MUX_MSDC30_2, MT_MUX_MSDC30_3};
    char name[6];
        ...
        unsigned long       timeout)
{
    ...
    if (intsts & cmdsts) {
        ...
        if ((intsts & MSDC_INT_CMDRDY) || (intsts & MSDC_INT_ACMDRDY) ||
            ...
            IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_RSPCRCERR Arg<0x%.8x>", __func__, host->id, cmd->opcode, cmd->arg);
            //msdc_dump_info(host->id);
            msdc_reset_hw(host->id);
        } else if (intsts & MSDC_INT_CMDTMO) {
            cmd->error = (unsigned int)-ETIMEDOUT;
            IRQ_MSG("[%s]: msdc%d XXX CMD<%d> MSDC_INT_CMDTMO Arg<0x%.8x>", __func__, host->id, cmd->opcode, cmd->arg);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
    ...
    if (io_cfg_left_node == NULL) {
 	    io_cfg_left_node = of_find_compatible_node(NULL, NULL, "mediatek,IOCFG_L");
        io_cfg_left_base = of_iomap(io_cfg_left_node, 0);
        printk("of_iomap for IOCFG_L base @ 0x%p\n", io_cfg_left_base);
    }

    if (io_cfg_top_node == NULL) {
 	    io_cfg_top_node = of_find_compatible_node(NULL, NULL, "mediatek,IOCFG_T");
        io_cfg_top_base = of_iomap(io_cfg_top_node, 0);
        printk("of_iomap for IOCFG_T base @ 0x%p\n", io_cfg_top_base);
    }

    if (io_cfg_bottom_node == NULL) {
 	    io_cfg_bottom_node = of_find_compatible_node(NULL, NULL, "mediatek,IOCFG_B");
        io_cfg_bottom_base = of_iomap(io_cfg_bottom_node, 0);
        printk("of_iomap for IOCFG_B base @ 0x%p\n", io_cfg_bottom_base);
    }

    if (io_cfg_right_node == NULL) {
 	    io_cfg_right_node = of_find_compatible_node(NULL, NULL, "mediatek,IOCFG_R");
        io_cfg_right_base = of_iomap(io_cfg_right_node, 0);
    ...
    if (apmixed_node == NULL) {
        ...
        apmixed_base1 = of_iomap(apmixed_node, 0);
	...
	host->sd_keep_pwr    = !!(host->hw->flags & MSDC_SD_NEED_POWER);
#ifndef FPGA_PLATFORM
    ...
    printk(KERN_ERR "[%s]: msdc%d, msdcpll en[0xF000C250][bit0 should be 0x1]=0x%x, msdcpll on[0xF000C25C][bit0~1 should be 0x1]=0x%x, mux[0xF0000070][bit23, 16~19 for msdc0, bit31, 24~26 for msdc1]=0x%x\n",
...
static int msdc_drv_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    if (mmc && state.event == PM_EVENT_SUSPEND && (host->hw->flags & MSDC_SYS_SUSPEND)) { /* will set for card */
		if (mmc->card && mmc_card_sd(mmc->card) && (host->hw->flags & MSDC_CD_PIN_EN) && (host->hw->flags & MSDC_SD_NEED_POWER)) {
			msdc_enable_cd_irq(host, 0);
		}
