...
static S32 mt_pwrap_store_hal(const char *buf,size_t count)
{
	...
	if(!strncmp(buf, "-h", 2))
	{
		PWRAPREG("PWRAP debug: [-dump_reg][-trace_wacs2][-init][-rdpmic][-wrpmic][-readtest][-writetest]\n");
...
static void pwrap_int_test(void)
{
	...
	while(1) {
		...
		PWRAPREG("Pwrap INT status check,PMIC_WRAP_EINT_STA=0x%x,MT6325_INT_STA[0x01B4]=0x%x\n", rdata1,rdata2);
	   #endif
	   #ifdef SLV_6332
		rdata1 = WRAP_RD32(PMIC_WRAP_EINT_STA);
...
static int pwrap_of_iomap(void)
{
	...
	scp_clk_ctrl_node =
		of_find_compatible_node(NULL, NULL, "mediatek,SCP_CLK_CTRL");
	...
	pwrap_p2p_node =
		of_find_compatible_node(NULL, NULL, "mediatek,SCP_PMICWP2P");
	if (!pwrap_p2p_node) {
