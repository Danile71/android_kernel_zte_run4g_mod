...
{
    battery_xlog_printk(BAT_LOG_FULL,"[bq24196_shutdown] driver shutdown \n");
...
kal_uint32 bq24196_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //battery_xlog_printk(BAT_LOG_FULL,"[bq24196_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24196_reg);
...
{
    battery_xlog_printk(BAT_LOG_CRTI,"[show_bq24196_access] 0x%x\n", g_reg_value_bq24196);
...
static ssize_t store_bq24196_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"[store_bq24196_access] \n");
    
    if(buf != NULL && size != 0)
    {
        battery_xlog_printk(BAT_LOG_CRTI,"[store_bq24196_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            battery_xlog_printk(BAT_LOG_CRTI,"[store_bq24196_access] write bq24196 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            battery_xlog_printk(BAT_LOG_CRTI,"[store_bq24196_access] read bq24196 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq24196);
            battery_xlog_printk(BAT_LOG_CRTI,"[store_bq24196_access] Please use \"cat bq24196_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"[bq24196_init] init start. ch=%d\n", BQ24196_BUSNUM);
