...
{
	67000, 79000, 91000, 100000, 115000, 125000, 140000, 151500
};
#else 
const kal_uint32 CS_VTH[]=
{
 ...
 static kal_uint32 charging_hw_init(void *data)
 {
   ...
   // bq24158_config_interface_liao(0x04,0x1A); //146mA
	if ( !charging_init_flag ) {
        ...
        bq24158_config_interface_liao(0x06,0x79); // ISAFE = 1250mA, VSAFE = 4.38V
    #else
 ...
 { 
	...
	{//use tsu6721 detect charer type
		...
		*(CHARGER_TYPE*)(data) = g_charger_type;
		printk("%s:*********charger_type %d,%d\n", __func__, g_charger_type, *(CHARGER_TYPE*)(data));		charging_type_det_done = KAL_TRUE;
	    #endif
		return status;
	} 
	else 
#endif
	{//use mt6325 bc1.1 detect charer type
		kal_uint32 status = STATUS_OK;
