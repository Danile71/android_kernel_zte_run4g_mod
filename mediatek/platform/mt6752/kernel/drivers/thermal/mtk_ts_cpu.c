...
/*
	TS_MCU1=>	g_x_roomt[0]  g_x_roomt1   * 10000
	TS_MCU2=>	g_x_roomt[1]  g_x_roomt2   * 10000
	TS_MCU3=>	g_x_roomt[2]  g_x_roomt3   * 10000
	TS_MCU4=>	g_x_roomt[3]  g_x_roomt4   * 10000
	TS_ABB =>	g_x_roomt[4]  g_x_roomtabb * 10000
...
//    THERMAL_WRAP_WR32(0x00010507, TEMPMONCTL2);	 // filt interval is 1 * 46.540us = 46.54us, sen interval is 1287 * 46.540us = 59.89 ms
	...
	//THERMAL_WRAP_WR32((UINT32) virt_to_phys(AUXADC_CON1_CLR), TEMPADCMUXADDR);// AHB address for auxadc mux selection
	//THERMAL_WRAP_WR32(0x1100100C, TEMPADCMUXADDR);// AHB address for auxadc mux selection
	THERMAL_WRAP_WR32((auxadc_ts_phy_base+0x00C), TEMPADCMUXADDR);// AHB address for auxadc mux selection

	THERMAL_WRAP_WR32(0x800, TEMPADCEN);                          // AHB value for auxadc enable
	//	THERMAL_WRAP_WR32((UINT32) virt_to_phys(AUXADC_CON1_SET), TEMPADCENADDR); // AHB address for auxadc enable (channel 0 immediate mode selected)
	//THERMAL_WRAP_WR32(0x11001008, TEMPADCENADDR); // AHB address for auxadc enable (channel 0 immediate mode selected)																  // this value will be stored to TEMPADCENADDR automatically by hw
	THERMAL_WRAP_WR32((auxadc_ts_phy_base+0x008), TEMPADCENADDR); // AHB address for auxadc enable (channel 0 immediate mode selected)																  // this value will be stored to TEMPADCENADDR automatically by hw

	//	THERMAL_WRAP_WR32((UINT32) virt_to_phys(AUXADC_DAT11), TEMPADCVALIDADDR); // AHB address for auxadc valid bit
	//	THERMAL_WRAP_WR32((UINT32) virt_to_phys(AUXADC_DAT11), TEMPADCVOLTADDR);  // AHB address for auxadc voltage output
	...
	THERMAL_WRAP_WR32((auxadc_ts_phy_base+0x040), TEMPADCVALIDADDR); // AHB address for auxadc valid bit
	THERMAL_WRAP_WR32((auxadc_ts_phy_base+0x040), TEMPADCVOLTADDR);  // AHB address for auxadc voltage output
...
#if 0 //debug
{
    ...
    //if(cunt==0)  printk("[Power/CPU_Thermal]0 hw_protect temp=%d\n",temp1);
    while(temp1 <= 3000 && cunt <20){
		cunt++;
    	printk("[Power/CPU_Thermal]0 hw_protect temp=%d\n",temp1);
        temp1 = DRV_Reg32(TEMPMSR0)& 0x0fff;
    ...
    //if(cunt==0)  printk("[Power/CPU_Thermal]1 hw_protect temp=%d\n",temp1);
    while(temp1 <= 3000 &&  cunt <20){
		cunt++;
    	printk("[Power/CPU_Thermal]1 hw_protect temp=%d\n",temp1);
        temp1 = DRV_Reg32(TEMPMSR1)& 0x0fff;
        tscpu_printk("TS_CON1=0x%x\n",DRV_Reg32(TS_CON1));
    	udelay(2);
	}
    ...
    //if(cunt==0) printk("[Power/CPU_Thermal]2 hw_protect temp=%d\n",temp1);
    while(temp1 <= 3000 &&  cunt <20){
		cunt++;
    	printk("[Power/CPU_Thermal]2 hw_protect temp=%d\n",temp1);
        temp1 = DRV_Reg32(TEMPMSR2)& 0x0fff;
        tscpu_printk("TS_CON1=0x%x\n",DRV_Reg32(TS_CON1));
    	udelay(2);
	}
    ...
    //if(cunt==0)    printk("[Power/CPU_Thermal]3 hw_protect temp=%d\n",temp1);
    while(temp1 <= 3000 &&  cunt <20){
		cunt++;
    	printk("[Power/CPU_Thermal]3 hw_protect temp=%d\n",temp1);
        temp1 = DRV_Reg32(TEMPMSR3)& 0x0fff;
        tscpu_printk("TS_CON1=0x%x\n",DRV_Reg32(TS_CON1));
    	udelay(2);
	}
...
static void thermal_cal_prepare(void)
{
	...
	temp0 = DRV_Reg32(0xF0206184);//K2
    temp1 = DRV_Reg32(0xF0206180);//K2
    temp2 = DRV_Reg32(0xF0206188);//K2
...
static int read_tc_raw_and_temp(UINT32 tempmsr_name,thermal_sensor_name ts_name,int *ts_raw)
{
	int temp=0,raw=0;
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
	    case THERMAL_BANK0:
			...
			CPU_TS_MCU1_T = read_tc_raw_and_temp((UINT32)TEMPMSR0,THERMAL_SENSOR1,&CPU_TS_MCU1_R);
			CPU_TS_MCU2_T = read_tc_raw_and_temp((UINT32)TEMPMSR1,THERMAL_SENSOR2,&CPU_TS_MCU2_R);
	        break;
	    case THERMAL_BANK1:
			//Bank1 : GPU (TS_MCU3)
			//read extra ABB here
			GPU_TS_MCU3_T = read_tc_raw_and_temp((UINT32)TEMPMSR0,THERMAL_SENSOR3,&GPU_TS_MCU3_R);
			ABB_TS_ABB_T  = read_tc_raw_and_temp((UINT32)TEMPMSR1,THERMAL_SENSORABB,&ABB_TS_ABB_R);
	        break;
	    case THERMAL_BANK2:
            //Bank2 : SOC (TS_MCU4,TS_MCU2,TS_MCU3)
			SOC_TS_MCU4_T = read_tc_raw_and_temp((UINT32)TEMPMSR0,THERMAL_SENSOR4,&SOC_TS_MCU4_R);
			SOC_TS_MCU2_T = read_tc_raw_and_temp((UINT32)TEMPMSR1,THERMAL_SENSOR2,&SOC_TS_MCU2_R);
			SOC_TS_MCU3_T = read_tc_raw_and_temp((UINT32)TEMPMSR2,THERMAL_SENSOR3,&SOC_TS_MCU3_R);
	        break;
		default:
            //Bank0 : CPU (TS_MCU1,TS_MCU2)
			CPU_TS_MCU1_T = read_tc_raw_and_temp((UINT32)TEMPMSR0,THERMAL_SENSOR1,&CPU_TS_MCU1_R);
...
#if 0
	//tscpu_printk("=================================================\n");
	tscpu_printk("bank0_T =%d,bank1_T =%d,bank2_T =%d\n",bank0_T,bank1_T,bank2_T);
	//tscpu_printk("=================================================\n");
	tscpu_printk("tscpu_get_temp MAX CPU T=%d\n",curr_temp);
#else
...
//	printk("Bank 1 : GPU  (TS_MCU3 = %d,TS_ABB = %d)              \n",GPU_TS_MCU3_T,ABB_TS_ABB_T);
...
static void thermal_pause_all_periodoc_temp_sensing(void)
{
    ...
    for(i=0;i<THERMAL_BANK_NUM;i++){
		...
		DRV_WriteReg32(TEMPMSRCTL1, (temp | 0x10E));//set bit8=bit1=bit2=bit3=1 to pause sensing point 0,1,2,3
    }
...
//    THERMAL_WRAP_WR32(0x000101AD, TEMPMONCTL2);	 // filt interval is 1 * 46.540us = 46.54us, sen interval is 429 * 46.540us = 19.96ms
//    THERMAL_WRAP_WR32(0x0001035A, TEMPMONCTL2);	 // filt interval is 1 * 46.540us = 46.54us, sen interval is 858 * 46.540us = 39.93ms
    THERMAL_WRAP_WR32(0x00100010, TEMPMONCTL2);	 // filt interval is 1 * 46.540us = 46.54us, sen interval is 1287 * 46.540us = 59.89 ms
    THERMAL_WRAP_WR32(0x00000001, TEMPAHBPOLL);  // poll is set to 1 * 46.540us = 46.540us
    THERMAL_WRAP_WR32(0x00000000, TEMPMSRCTL0);  // temperature sampling control, 1 sample
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	a_tscpu_all_temp[1]=MAX(CPU_TS_MCU2_T,SOC_TS_MCU2_T); //temp of TS_MCU2
	a_tscpu_all_temp[2]=MAX(GPU_TS_MCU3_T,SOC_TS_MCU3_T); //temp of TS_MCU3
    a_tscpu_all_temp[3]=SOC_TS_MCU4_T;				      //temp of TS_MCU4
	a_tscpu_all_temp[4]=ABB_TS_ABB_R;					  //temp of TS_ABB
	tscpu_met_unlock(&flags);
...
#else
    DRV_WriteReg32(TEMPPNPMUXADDR, (UINT32)(thermal_phy_base+0x420));    // AHB address for pnp sensor mux selection
    DRV_WriteReg32(TEMPADCMUXADDR, (UINT32) (thermal_phy_base+0x420));    // AHB address for auxadc mux selection
    DRV_WriteReg32(TEMPADCENADDR,  (UINT32) (thermal_phy_base+0x424));     // AHB address for auxadc enable
    DRV_WriteReg32(TEMPADCVALIDADDR,(UINT32) (thermal_phy_base+0x428));  // AHB address for auxadc valid bit
    DRV_WriteReg32(TEMPADCVOLTADDR, (UINT32) (thermal_phy_base+0x428));   // AHB address for auxadc voltage output
#endif
...
static int get_io_reg_base(void)
{
    ...
    if(node){
		...
		//printk("[THERM_CTRL] thermal_base=0x%x\n",thermal_base);
    }
    ...
    //printk("[THERM_CTRL] thermal_irq_number=0x%x\n",thermal_irq_number);
	if (!thermal_irq_number) {
		//printk("[THERM_CTRL] get irqnr failed=0x%x\n",thermal_irq_number);
		return 0;
	}
    ...
    if(node){
		...
		//printk("[THERM_CTRL] auxadc_ts_base=0x%x\n",auxadc_ts_base);
    }
	of_property_read_u32(node, "reg", &auxadc_ts_phy_base);
	//printk("[THERM_CTRL] auxadc_ts_phy_base=0x%x\n",auxadc_ts_phy_base);
    ...
    if(node){
		...
		//printk("[THERM_CTRL] infracfg_ao_base=0x%x\n",infracfg_ao_base);
    }
    ...
    if(node){
		...
		//printk("[THERM_CTRL] apmixed_base=0x%x\n",apmixed_base);
    }
	of_property_read_u32(node, "reg", &apmixed_phy_base);
	//printk("[THERM_CTRL] apmixed_phy_base=0x%x\n",apmixed_phy_base);
...
static int __init tscpu_init(void)
{
	...
	*/
	...
	if (!mtktscpu_dir)
	{
		tscpu_printk("tscpu_init mkdir /proc/mtktscpu failed\n");
	}
	else
	{

		entry = proc_create("mtktscpu", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir, &mtktscpu_fops);
		if (entry) {
		...
		entry = proc_create("mtktscpu_log", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_log_fops);

		entry = proc_create("mtktscpu_opp", S_IRUGO, mtktscpu_dir, &mtktscpu_opp_fops);

        entry = proc_create("mtktscpu_cal", S_IRUSR, mtktscpu_dir, &mtktscpu_cal_fops);

        entry = proc_create("mtktscpu_read_temperature", S_IRUGO, mtktscpu_dir, &mtktscpu_read_temperature_fops);

        entry = proc_create("mtktscpu_set_temperature", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_set_temperature_fops);

        entry = proc_create("mtktscpu_talking_flag", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_talking_flag_fops);
#if CPT_ADAPTIVE_AP_COOLER
        entry = proc_create("mtktscpu_dtm_setting", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir, &mtktscpu_dtm_setting_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
        ...
        entry = proc_create("mtktscpu_gpu_threshold", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir, &mtktscpu_gpu_threshold_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
...
#if MTKTSCPU_FAST_POLLING
		entry = proc_create("mtktscpu_fastpoll", S_IRUGO | S_IWUSR | S_IWGRP, mtktscpu_dir, &mtktscpu_fastpoll_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
		...
		entry = proc_create("mtktscpu_Tj_out_via_HW_pin", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_Tj_out_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
        ...
        /* +ASC+ */
		entry = proc_create("mtktscpu_atm", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_atm_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
...
#if THERMAL_HEADROOM
        entry = proc_create("thp", S_IRUGO | S_IWUSR, mtktscpu_dir, &mtktscpu_thp_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
