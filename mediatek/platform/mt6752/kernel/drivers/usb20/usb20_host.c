...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
    ...
    #if defined(MTK_DUAL_INPUT_CHARGER_SUPPORT)&&defined(MTK_LOAD_SWITCH_FPF3040)
    if(is_on)
    {
    ...
    #elif defined(MTK_BQ24261_SUPPORT) || defined(MTK_BQ24196_SUPPORT)
        #if !defined(MTK_DUAL_INPUT_CHARGER_SUPPORT) 
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	if (mtk_musb->is_host) {
		//detect ACA type, do nothing for USB OTG
		hw_otg_get_aca_type();
	}
