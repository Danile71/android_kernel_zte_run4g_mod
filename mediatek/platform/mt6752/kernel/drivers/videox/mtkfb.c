...
static int _convert_fb_layer_to_disp_input(struct fb_overlay_layer* src, disp_input_config *dst)
{
	...
	dst->src_base_addr = (unsigned int)src->src_base_addr;
	dst->security = src->security;
	dst->src_phy_addr = (unsigned int)src->src_phy_addr;
	...
	DISPDBG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
						...
						dst->layer_enable,
						dst->next_buff_idx,
						(unsigned int)(dst->src_base_addr),
						(unsigned int)(dst->src_phy_addr),
						src->src_fmt,
						dst->src_fmt,
						dst->src_pitch,
						dst->src_offset_x,
...
#if 1
			DISPMSG("_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%x, paddr=0x%x, fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
								dst->layer,
								dst->layer_en,
								dst->buff_idx,
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	input->layer_id = 0;
	input->src_phy_addr = (unsigned int)paStart;
	input->src_base_addr = (unsigned int)vaStart;
...
//		cci400_sel_for_ddp();
	...
	else 
	{
		...
		for (i = 0; i < VIDEO_LAYER_COUNT; ++i) 
		{
			...
			{
                DDPAEE("MTKFB_SET_VIDEO_LAYERS, layer_id invalid=%d \n", layerInfo[i].layer_id);
                continue;
			}
...
int _parse_tag_videolfb(void)
{
	...
	node = of_scan_flat_dt(fb_early_init_dt_get_chosen, NULL);
	if(node)
	{
...
static int mtkfb_probe(struct device *dev)
{
...

	{
	        ...
	        _parse_tag_videolfb();
		printk("mtkfb_probe: fb_pa = 0x%lx\n", fb_base);
  ...
  primary_display_set_frame_buffer_address(fbdev->fb_va_base,fb_pa);
    ...
    DISPMSG("[FB Driver] fbdev->fb_pa_base = %lx, fbdev->fb_va_base = %lx\n", fbdev->fb_pa_base, (unsigned long)(fbdev->fb_va_base));
...
int mtkfb_get_debug_state(char* stringbuf, int buf_len)
{
	...
	unsigned int va = (UINT32)fbdev->fb_va_base;
	unsigned int mva =(UINT32)fbdev->fb_pa_base;
	unsigned int pa = fbdev->fb_pa_base;
	unsigned int resv_size = vramsize;
	len += scnprintf(stringbuf+len, buf_len - len, "|--------------------------------------------------------------------------------------|\n");	
	//len += scnprintf(stringbuf+len, buf_len - len, "********MTKFB Driver General Information********\n");
	len += scnprintf(stringbuf+len, buf_len - len, "|Framebuffer VA:0x%08x, PA:0x%08x, MVA:0x%08x, Reserved Size:0x%08x|%d\n", va, pa, mva, resv_size, resv_size);
...
int mtkfb_get_reserved_buffer_info(unsigned int *base, unsigned int *width, unsigned int *height, unsigned int *bpp, unsigned int *current_offset)
{
