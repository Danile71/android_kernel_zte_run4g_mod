...
{
	mutex_lock(&(pgc->esd_check_lock));
}

static void _primary_path_esd_check_unlock()
{
	mutex_unlock(&(pgc->esd_check_lock));
}
...
extern void disp_ovl_enable_sw_trigger(void * handle);
...
extern unsigned int gEnableSODILog;
void disp_set_sodi(unsigned int enable, void* cmdq_handle)
{
    unsigned int old_value;

     if(gEnableSODILog==1) 
         old_value = DISP_REG_GET(SPM_PCM_SRC_REQ);
    
    if(primary_display_is_video_mode()==1 && gEnableSODIControl==1)
    {        
        if(cmdq_handle!=NULL)
        {
            if(enable==1)
    ...
    if(gEnableSODILog==1) 
        DISPMSG("sodi, enable=%d, cmdq_handle=0x%p, old_value=0x%x, new_value=0x%x \n", enable, cmdq_handle, old_value, DISP_REG_GET(SPM_PCM_SRC_REQ));
}

extern unsigned int gDumpConfigThreadCMD;
extern unsigned int gEnableCMDQProfile;
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	static unsigned int cnt = 0;

    // insert update ovl status to slot command
...
#ifdef DISP_CMDQ_PROFILE_ENABLE
    // record config done time
	...
	// disable prefetch after config done.
	if(gPrefetchControl==1 && primary_display_is_video_mode()==1 && cnt>=20)
	{
	...
	if(gEnableSWTrigger==1 && primary_display_is_video_mode()==1)
	{
	    //DISP_REG_SET(pgc->cmdq_handle_config, DISP_REG_CONFIG_MUTEX_EN(DISP_OVL_SEPARATE_MUTEX_ID), 1);
	}
	...
	// enable prefetch as the first cmd
	if(gPrefetchControl==1 && primary_display_is_video_mode()==1 && cnt>=20)
	{
	...
	if(_should_insert_wait_frame_done_token()&& (!_is_decouple_mode(pgc->session_mode)))
	{
		...
		if(gEnableCMDQProfile==1)
		{
		    cmdqRecProfileMarker(pgc->cmdq_handle_config, "wait_done");
		}
#endif
...
static int _fence_release_worker_thread(void *data)
{
	...
	{   
		...
		{
			if(!dpmgr_is_moudule_in_mutex(handle, DISP_MODULE_OVL1))
			{
...
extern unsigned int gEnableconfigErrorAEE;
static int _ovl_status_check_callback(uint32_t userdata)
{
#ifdef CMDQ_OVL_STATUS_CHECK
    ...
    {       
       ...
       {
           if(gEnableconfigErrorAEE==1)
           {
               DDPAEE("disp ovl status error, 0x%x, 0x%x \n", ovl_status[0], ovl_status[1]); 
           }
           DISPMSG("ovl status, 0x%x, 0x%x \n", ovl_status[0], ovl_status[1]);
...
#ifdef CMDQ_OVL_STATUS_CHECK
		if(callback==NULL)
		    _trigger_display_interface(blocking, _ovl_status_check_callback, userdata);
		else
#endif		
...
void primary_display_reconfig_ovl(void)
{
    DISPMSG("primary_display_reconfig_ovl called! \n");
    disp_ddp_path_config *pconfig = dpmgr_path_get_last_config(pgc->dpmgr_handle);    
    pconfig->ovl_dirty  = 1;
    pconfig->rdma_dirty  = 0;
    pconfig->wdma_dirty  = 0;
    pconfig->dst_dirty  = 0;
    dpmgr_path_config(pgc->dpmgr_handle, pconfig, 0);
}
...
int primary_display_diagnose(void)
{
    ...
    if(_is_decouple_mode(pgc->session_mode))
	{
	    if(NULL==pgc->ovl2mem_path_handle)
	    {
	        DISPERR("pgc->ovl2mem_path_handle is NULL in primary_display_diagnose\n");
	    }
	    else
	    {
...
int primary_display_set_cmd(int *lcm_cmd, unsigned int cmd_num)
{
	...
	else
	{
		...
		{	//cmdq
			...
			DISPCHECK("primary set cmd, handle=0x%08x\n", cmdq_handle_cmd);
			if(ret!=0)
			{
