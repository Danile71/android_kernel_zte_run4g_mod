...
static void md_gpio_config(unsigned int boot_md_id)
{
  ...
  printf("SIM1 sclk= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM1 srst= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM1 sio= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM2 sclk= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM2 srst= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM2 sio= en=%d,dir=%d,in=%d,out=%d\n", \
  ...
  printf("SIM1(sclk=%d, srst=%d , sio=%d) SIM2(sclk=%d, srst=%d , sio=%d)\n", \        
...
static void md_emi_remapping(unsigned int boot_md_id)
{
	...
	printf("  ---> Map 0x00000000 to %x for MD%d\n", md_img_start_addr+0x42000000, boot_md_id+1);
    ...
    switch(boot_md_id){
        case 0:// For MD1
            ...
            *((volatile unsigned int*)0x10001F00) = reg_value;
            printf("md_bootup: md1 set SRCLKEN infra_misc(0x1000_1F00), bit(28,27,26,25)=0x%x\n",(*((volatile unsigned int*)0x10001F00)&0x1E000000));           
            ...
            *((volatile unsigned int*)0x10001F08) = reg_value;
            printf("md_bootup: md1 set PLL misc_config(0x1000_1F08), bit(20,19,18,17,16,6,5,4,3)=0x%x\n",(*((volatile unsigned int*)0x10001F08)&0x001F0078)); 
            mdelay(3);// sleep 3            
            *((volatile unsigned int*)0x1000C004) = 0x00000101;
            printf("md_bootup: md1 set AP_PLL_CON1(0x1000C004)=0x%x\n",(*((volatile unsigned int*)0x1000C004)&0x00000101));
            mdelay(1);
...
void md_uart_config(int md_id)
{
    ...
    {
        case 0:
#if 0       // need hw support, AP uart & md uart have share pinmuxd
            mt_set_gpio_mode(GPIO113,3);
            mt_set_gpio_mode(GPIO114,3);
            mt_set_gpio_mode(GPIO117,0);
            mt_set_gpio_mode(GPIO118,0);
#endif
						printf("md_uart_config:%d,enable md1, disable AP & MD2 uart!\n", md_id);
            //disable AP uart
            //RX
            ...
            //enable  MD1 uart
            //RX
            mt_set_gpio_mode(GPIO144,6);
            mt_set_gpio_dir(GPIO144, GPIO_DIR_IN);
            mt_set_gpio_pull_enable(GPIO144, GPIO_PULL_ENABLE); 
            mt_set_gpio_pull_select(GPIO144, GPIO_PULL_UP);
            //TX
            mt_set_gpio_mode(GPIO145,6);
