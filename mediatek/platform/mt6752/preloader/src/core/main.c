...
static int bldr_handshake(struct bldr_command_handler *handler)
{
    ...
    case NORMAL_BOOT:
        ...
        // while battery vol is under 3.7v, bypass download flow and goto charging flow in LK.
        #if CFG_LOW_BATTERY_DWL_CHECK
		if(get_bat_sense_volt(5) < 3700)
            print("%s Battery Voltage is under 3.7v, bypass usb handshaking...\n");
        else
        #endif
        {
