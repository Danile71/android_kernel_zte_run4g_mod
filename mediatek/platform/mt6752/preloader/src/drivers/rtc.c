...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	if (measureSrc == FQMTR_FQM26M_CK) {
		pmic_config_interface(MT6325_TOP_CLKSQ, 1, MT6325_PMIC_RG_CLKSQ_EN_FQR_MASK, MT6325_PMIC_RG_CLKSQ_EN_FQR_SHIFT);
	} else if (measureSrc == FQMTR_FQM32K_CK) {
		pmic_config_interface(MT6325_TOP_CKPDN_CON2, 1, MT6325_PMIC_RG_FQMTR_32K_CK_PDN_MASK, MT6325_PMIC_RG_FQMTR_32K_CK_PDN_SHIFT);
	}
...
static bool rtc_android_init(void)
{
	...
	RTC_Write(RTC_AL_YEA, RTC_Read(RTC_AL_YEA) | ((1970 - RTC_MIN_YEAR) & RTC_AL_YEA_MASK));
...
bool rtc_boot_check(void)
{
	...
	RTC_Write(RG_FQMTR_PON, 1 << MT6325_PMIC_RG_FQMTR_CK_PDN_SHIFT);
...
void rtc_save_2sec_stat(void)
{
	...
	print("rtc_2sec_reboot_check 0x%x\n", reboot);
