...
#include <cust_nvram.h>
#endif

#define USBD_ISERIAL_STR     "0123456789ABCDEF"
...
usbdl_init (void)
{
    ...
    int info_buffer_size = sizeof(PRODUCT_INFO);
    u8 *info_buffer = (u8*)malloc(info_buffer_size);
    ...
    string->bDescriptorType = USB_DESCRIPTOR_TYPE_STRING;
    memcpy(serial_num, USBD_ISERIAL_STR, sizeof (USBD_ISERIAL_STR));
    string->bLength = sizeof (iserial);

    if(nvram_get_data_by_lid(AP_CFG_REEB_PRODUCT_INFO_LID, info_buffer, info_buffer_size))
    {
        product_info = (PRODUCT_INFO *)info_buffer;
        if (product_info->usb_serial_num[0] != NULL)
        {
            memcpy(serial_num, product_info->usb_serial_num, sizeof(product_info->usb_serial_num));
        }
        else
        {
             print ("nvram don't have usb serial data\n");
        }
    }
    else
    {
        print ("get nvram data error\n");
    }
